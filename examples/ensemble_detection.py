...
def create_complex_dataset():
    ...
    anomaly_types = {
        ...
        'local_outliers': np.random.randn(10, 5) + [2, 0, 0, 0, 0],  # Shifted in one dimension
        'cluster_anomalies': np.random.randn(10, 5) * 0.1 + [3, 3, 0, 0, 0],  # Small cluster away
        'dependency_anomalies': np.array([  # Break correlation patterns
            [x, -x, x, -x, x] + np.random.randn(5) * 0.1 for x in np.linspace(2, 4, 10)
        ])
    }
...
async def ensemble_detect(container, detector_ids: List[str], dataset: Dataset, method: str = 'voting'):
    ...
    for detector_id in detector_ids:
        result = await detection_service.detect_anomalies(detector_id=detector_id, dataset=dataset)
    ...
    if method == 'voting':
        # Majority voting
        ensemble_predictions = (np.sum(all_predictions, axis=0) >= len(detector_ids) / 2).astype(int)
    elif method == 'unanimous':
        # All detectors must agree
        ensemble_predictions = (np.sum(all_predictions, axis=0) == len(detector_ids)).astype(int)
    elif method == 'any':
...
async def main():
    ...
    for i, detector_id in enumerate(detector_ids):
        ...
        individual_metrics = evaluate_predictions(true_labels, individual_predictions[i])
