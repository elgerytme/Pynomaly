    ...
    def detect_fraud(self, dataset, true_labels):
        ...
        for algo in self.recommended_algorithms:
            try:
                ...
                precision = np.sum((result.labels == 1) & (true_labels == 1)) / max(np.sum(result.labels == 1), 1)
                recall = np.sum((result.labels == 1) & (true_labels == 1)) / max(np.sum(true_labels == 1), 1)
        ...
        print(f"{'Algorithm':<15} {'Accuracy':<10} {'Precision':<11} {'Recall':<8} {'Detected':<9} {'Time (s)':<9}")
        print("-" * 75)
        
        for result in sorted(results, key=lambda x: x['precision'], reverse=True):
            print(f"{result['algorithm']:<15} {result['accuracy']:<10.3f} {result['precision']:<11.3f} "
...
class IndustrialIoTMonitor:
    ...
    def create_sample_data(self, n_samples=5000):
        ...
        power_normal = (temp_normal + pressure_normal) * 0.3 + np.random.normal(0, 5, n_normal)
    ...
    def monitor_equipment(self, dataset, true_labels):
        ...
        print(f"   Processing speed: {len(dataset.data)/result.execution_time_ms*1000:.0f} readings/sec")
        ...
        for i, idx in enumerate(critical_indices[:3], 1):
            ...
            print(f"      Temp: {sample['temperature_celsius']:.1f}°C, "
...
class CybersecurityThreatDetector:
    ...
    def create_sample_data(self, n_samples=8000):
        ...
        for i, atype in enumerate(attack_type):
            if atype == 0:
                ...
                packet_size_attack[i] = np.random.uniform(32768, 65536) # Large data transfers
            else:
        ...
        duration_attack = np.random.choice([
            ...
            np.random.uniform(300, 3600, n_attacks//2)   # Very long (exfiltration)
        ], n_attacks)
    ...
    def detect_threats(self, dataset, true_labels):
        ...
        print(f"   Analysis speed: {len(dataset.data)/result.execution_time_ms*1000:.0f} connections/sec")
        ...
        for i, idx in enumerate(threat_indices[np.argsort(scores[threat_indices])[-3:]][::-1], 1):
            sample = dataset.data.iloc[idx]
            score = scores[idx]
            ...
            print(f"      Duration: {sample['connection_duration_sec']:.1f}s, "
...
def run_industry_templates():
    ...
    security_results = security_detector.detect_threats(security_dataset, security_labels)
