...
def analyze_financial_fraud():
    ...
    print(f"Samples: {len(df):,} | Features: {len(df.columns)-1} | Anomaly Rate: {df['is_anomaly'].mean():.1%}")
    ...
    print(f"Normal transaction amounts:")
    ...
    print(f"Fraudulent transaction amounts:")
    ...
    print(f"Large amounts (>95th percentile): {(large_amounts & (df['is_anomaly'] == 1)).sum()} fraud / {large_amounts.sum()} total")
    print(f"Micro amounts (<$1): {(micro_amounts & (df['is_anomaly'] == 1)).sum()} fraud / {micro_amounts.sum()} total")
    
    # Time-based indicators
    unusual_hours = df['hour_of_day'] < 6
    print(f"Unusual hours (midnight-6am): {(unusual_hours & (df['is_anomaly'] == 1)).sum()} fraud / {unusual_hours.sum()} total")
    
    # Velocity indicators
    high_velocity = df['velocity_score'] > df['velocity_score'].quantile(0.95)
    print(f"High velocity (>95th percentile): {(high_velocity & (df['is_anomaly'] == 1)).sum()} fraud / {high_velocity.sum()} total")
...
def analyze_network_intrusion():
    ...
    print(f"Normal traffic packets/second: {normal_traffic['packets_per_second'].mean():.2f} ± {normal_traffic['packets_per_second'].std():.2f}")
    print(f"Attack traffic packets/second: {attack_traffic['packets_per_second'].mean():.2f} ± {attack_traffic['packets_per_second'].std():.2f}")
    
    print(f"Normal traffic bytes/packet: {normal_traffic['bytes_per_packet'].mean():.2f} ± {normal_traffic['bytes_per_packet'].std():.2f}")
    print(f"Attack traffic bytes/packet: {attack_traffic['bytes_per_packet'].mean():.2f} ± {attack_traffic['bytes_per_packet'].std():.2f}")
...
def analyze_iot_sensors():
    ...
    sensor_cols = ['temperature_celsius', 'humidity_percent', 'pressure_hpa', 'vibration_level']
    ...
    for col in sensor_cols:
        ...
        print(f"{col}:")
        print(f"  Normal: {normal_values.mean():.2f} ± {normal_values.std():.2f} (range: {normal_values.min():.2f} - {normal_values.max():.2f})")
        print(f"  Anomaly: {anomaly_values.mean():.2f} ± {anomaly_values.std():.2f} (range: {anomaly_values.min():.2f} - {anomaly_values.max():.2f})")
    
    print("\n2. TEMPORAL PATTERN ANALYSIS")
    print("-" * 40)
    
    # Daily patterns
    hourly_anomalies = df.groupby('hour_of_day')['is_anomaly'].agg(['count', 'sum', 'mean'])
    peak_anomaly_hours = hourly_anomalies.nlargest(3, 'mean').index
    print(f"Peak anomaly hours: {list(peak_anomaly_hours)}")
    
    # Correlation analysis
    correlation_features = ['temp_humidity_ratio', 'pressure_deviation', 'vibration_log']
...
def analyze_manufacturing_quality():
    ...
    for feature in process_features:
        ...
        print(f"  Defective: {defective_process.mean():.1f} ± {defective_process.std():.1f}")
        print(f"  Process variation increase: {(defective_process.std() / normal_process.std()):.2f}x")
...
def analyze_ecommerce_behavior():
    ...
    behavior_metrics = ['session_duration_minutes', 'pages_viewed', 'items_clicked', 'pages_per_minute']
    
    for metric in behavior_metrics:
        print(f"{metric}:")
        print(f"  Normal: {normal_behavior[metric].mean():.2f} ± {normal_behavior[metric].std():.2f}")
        print(f"  Anomaly: {anomaly_behavior[metric].mean():.2f} ± {anomaly_behavior[metric].std():.2f}")
    ...
    high_value_normal = (normal_behavior['purchase_amount'] > high_value_threshold).sum()
    high_value_anomaly = (anomaly_behavior['purchase_amount'] > high_value_threshold).sum()
...
def analyze_time_series_anomalies():
    ...
    print(f"Peak anomaly hour: {peak_anomaly_hour} with {anomaly_counts[peak_anomaly_hour]} anomalies")
    ...
    print(f"Deviation from 5-period moving average:")
...
def analyze_high_dimensional():
    ...
    print(f"Dataset: {metadata.get('description', 'High-dimensional anomaly detection')}")
    print(f"Samples: {len(df):,} | Features: {len(df.columns)-1} | Anomaly Rate: {df['is_anomaly'].mean():.1%}")
    ...
    if n_samples / n_features < 10:
        print("⚠️  Warning: High dimensional, low sample count - curse of dimensionality may apply")
    
    # Feature variance analysis
    feature_vars = df[feature_cols].var()
    print(f"Feature variance range: {feature_vars.min():.3f} to {feature_vars.max():.3f}")
    print(f"Feature variance mean: {feature_vars.mean():.3f} ± {feature_vars.std():.3f}")
    ...
    moderate_corr_pairs = ((abs(upper_triangle) > 0.3) & (abs(upper_triangle) <= 0.7)).sum().sum()
    ...
    engineered_features = ['feature_sum', 'feature_mean', 'feature_std', 'feature_range']
    ...
    for feature in engineered_features:
        if feature in df.columns:
            ...
            print(f"  Anomaly: {anomaly_values.mean():.3f} ± {anomaly_values.std():.3f}")
            print(f"  Separation: {abs(normal_values.mean() - anomaly_values.mean()) / normal_values.std():.2f} σ")
    ...
    if successful > 0:
        ...
        for name, result in results.items():
            if result['status'] == 'success':
                ...
                print(f"  • {dataset_name}: {result['samples']:,} samples, {result['features']} features, {result['anomaly_rate']:.1%} anomalies")
    
    print(f"\n📋 For detailed documentation, see:")
    print(f"   📖 examples/sample_datasets/README.md")
    print(f"   📁 examples/sample_datasets/")
    print(f"   🧪 This analysis script: scripts/analyze_dataset_comprehensive.py")
