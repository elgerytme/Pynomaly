...
T010,42.15,Supermarket,Local,10,3,0"""
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if results_file.exists():
                    ...
                    anomaly_count = sum(1 for score in results.get('predictions', []) if score == 1)
                    total_samples = len(results.get('predictions', []))
                    
                    print(f"📊 Results: {anomaly_count}/{total_samples} anomalies detected")
    ...
    def demo_autonomous_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                str(financial_file),
                "--output", str(Path(self.demo_data_dir) / "autonomous_results.json")
            ]
            ...
            if result.returncode == 0:
                ...
                if results_file.exists():
                    ...
                    algorithm_used = results.get('algorithm_info', {}).get('name', 'Unknown')
                    anomaly_count = sum(1 for score in results.get('predictions', []) if score == 1)
                    total_samples = len(results.get('predictions', []))
    ...
    def demo_export_capabilities(self) -> Dict[str, Any]:
        ...
        successful_exports = [fmt for fmt, result in export_results.items() 
                            if result.get("status") == "success"]
        
        print(f"📊 Export summary: {len(successful_exports)}/{len(formats)} formats successful")
...
8.0,9.0,10.0,0"""
    ...
    def demo_performance_benchmarking(self) -> Dict[str, Any]:
        ...
        try:
            cmd = [
                "python3", "-m", "pynomaly", "benchmark",
                str(simple_file),
                "--algorithms", "IsolationForest,LOF,OneClassSVM",
                "--output", str(Path(self.demo_data_dir) / "benchmark_results.json")
            ]
            ...
            if result.returncode == 0:
                ...
                return {"status": "warning", "message": "Benchmark command not implemented"}
    ...
    def run_demo_suite(self, demo_type: str = "all") -> Dict[str, Any]:
        ...
        if demo_type in ["all", "performance"]:
            demo_results["performance_benchmarking"] = self.demo_performance_benchmarking()
        
        # Generate summary
        successful_demos = [name for name, result in demo_results.items() 
        ...
        for demo_name, result in demo_results.items():
            status_icon = {"success": "✅", "error": "❌", "warning": "⚠️"}.get(result.get("status"), "❓")
...
Examples:
  ...
  python demo_pynomaly.py --demo export
    ...
    parser.add_argument(
        "--demo",
        choices=["all", "basic", "autonomous", "export", "preprocessing", "performance"],
        default="all",
    ...
    success_rate = results["summary"]["successful_demos"] / results["summary"]["total_demos"]
    if success_rate >= 0.8:
