...
def create_demo_data():
    ...
    network_anomaly = np.random.choice([0, 200], n_anomalies) + np.random.normal(0, 10, n_anomalies)
    ...
    print(f"   📊 Created: {len(df)} samples ({n_normal} normal, {n_anomalies} anomalies)")
...
def demo_basic_detection(dataset, true_labels):
    ...
    for i, idx in enumerate(top_threats, 1):
        ...
        print(f"   {i}. Sample {idx}: Score={threat_score:.3f} ({'✓ Real threat' if is_real_threat else '✗ False alarm'})")
        print(f"      CPU: {sample['cpu_usage_percent']:.1f}%, "
...
def demo_algorithm_speed_test(dataset):
    ...
    print(f"{'Algorithm':<15} {'Train (s)':<10} {'Detect (s)':<11} {'Throughput':<12} {'Anomalies':<10}")
    print("-" * 65)
    
    for result in sorted(results, key=lambda x: x['throughput'], reverse=True):
        print(f"{result['algorithm']:<15} {result['train_time']:<10.3f} "
