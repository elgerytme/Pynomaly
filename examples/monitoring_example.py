...
from src.pynomaly.infrastructure.batch.batch_processor import BatchProcessor, BatchConfig, BatchEngine
...
async def demonstrate_alerting_system():
    ...
    for alert in active_alerts:
        logger.info(f"Alert: {alert.rule.name} - {alert.rule.severity.value} - {alert.message}")
...
async def demonstrate_metrics_collection():
    ...
    metrics_collector.record_job_duration(120.5, "batch", "isolation_forest", "multiprocessing", "completed")
    metrics_collector.increment_anomalies_found(15, "batch", "isolation_forest", "medium", "sample_data.csv")
    ...
    metrics_collector.record_processing_latency(2.5, "streaming", "isolation_forest", "medium")
    metrics_collector.set_session_count(5, "streaming", "running")
    metrics_collector.increment_alert_count("high_error_rate", "medium", "alerting_system")
    metrics_collector.increment_notification_count("email", "admin@example.com", "success")
