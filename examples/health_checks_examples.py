...
async def comprehensive_health_manager_example():
    ...
    print(f"   Total Components: {health_report['monitoring_status']['total_components']}")
    print(f"   Critical Components: {health_report['monitoring_status']['critical_components']}")
...
async def kubernetes_probes_example():
    ...
    if liveness_result.details:
        print(f"   Uptime: {liveness_result.details.get('uptime_seconds', 'N/A')} seconds")
...
async def monitoring_loop_simulation():
    ...
    for i in range(6):  # 6 iterations, 5 seconds each
        ...
        print(f"   Check #{i+1}: {system_health.status.value} "
