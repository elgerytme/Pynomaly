...
def create_tutorial_dataset():
    ...
    pattern1 = np.random.multivariate_normal([2, 2, 1], [[1, 0.5, 0], [0.5, 1, 0.2], [0, 0.2, 0.8]], n_normal//3)
    pattern2 = np.random.multivariate_normal([-1, -1, 0], [[0.8, -0.3, 0.1], [-0.3, 0.8, -0.1], [0.1, -0.1, 0.6]], n_normal//3)
    pattern3 = np.random.multivariate_normal([0, 3, -1], [[1.2, 0.2, 0.3], [0.2, 1.2, -0.2], [0.3, -0.2, 1.0]], n_normal//3 + n_normal%3)
    ...
    cluster_outliers = np.random.multivariate_normal([5, -5, 3], [[0.5, 0, 0], [0, 0.5, 0], [0, 0, 0.5]], n_anomalies//2)
    ...
    print(f"   📊 Normal samples: {np.sum(labels == 0)}, Anomalous: {np.sum(labels == 1)}")
...
def tutorial_01_basic_detection(dataset, true_labels):
    ...
    print(f"   📈 Trained on {dataset.n_samples} samples with {dataset.n_features} features")
    ...
    for i, idx in enumerate(top_indices, 1):
        ...
        print(f"   {i}. Sample {idx}: Score={score:.3f}, True anomaly={is_true_anomaly}, "
...
def tutorial_02_algorithm_comparison(dataset, true_labels):
    ...
    for adapter_class, algo_name, params in algorithms:
        ...
        try:
            ...
            print(f"      ✅ Accuracy: {accuracy:.3f}, F1: {f1_score:.3f}, Time: {total_time:.3f}s")
    ...
    print(f"{'Algorithm':<18} {'Accuracy':<10} {'Precision':<11} {'Recall':<8} {'F1-Score':<9} {'Time (s)':<9}")
    print("-" * 75)
    
    for result in sorted(results, key=lambda x: x['f1_score'], reverse=True):
        print(f"{result['algorithm']:<18} {result['accuracy']:<10.3f} {result['precision']:<11.3f} "
...
def tutorial_03_automl_selection(dataset, true_labels):
    ...
    try:
        ...
        print("   ⚠️ AutoML functionality requires automl_example.py - using simplified version")
    ...
    if 'error' not in summary:
        ...
        for i, result in enumerate(summary['top_3'], 1):
            print(f"   {i}. {result['algorithm']:15} | Score: {result['avg_score']:.3f} | "
...
def tutorial_04_explainable_ai(dataset, true_labels):
    ...
    try:
        ...
        print("   ⚠️ Explainable AI example not available, skipping detailed explanations...")
        return
    ...
    for i, explanation in enumerate(explanations[:2], 1):
        ...
        for feature, contrib in sorted_contributions[:2]:
            print(f"         {feature}: {contrib['value']:.2f} "
    ...
    for size in sizes:
        ...
        for algo in algorithms:
            try:
                ...
                print(f"      {algo:15}: {total_time:.3f}s ({throughput:.0f} samples/sec)")
...
def run_comprehensive_tutorial():
    ...
    print("   🔗 Try real-time monitoring: python examples/realtime_monitoring_example.py")
