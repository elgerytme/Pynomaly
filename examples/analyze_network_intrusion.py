...
def load_network_intrusion_data():
    ...
    data_path = Path(__file__).parent / "sample_datasets" / "synthetic" / "network_intrusion.csv"
    ...
    print(f"✅ Loaded network intrusion dataset: {len(df)} samples, {len(df.columns)-1} features")
    return df
...
def network_traffic_analysis(df):
    ...
    for protocol in protocol_dist.index:
        normal_count = protocol_dist.loc[protocol, 0] if 0 in protocol_dist.columns else 0
        attack_count = protocol_dist.loc[protocol, 1] if 1 in protocol_dist.columns else 0
    ...
    volume_metrics = ['packet_count', 'bytes_transferred', 'packets_per_second', 'bytes_per_packet']
    ...
    normal_std_ports = normal_traffic[normal_traffic['destination_port'].isin(standard_ports)]['destination_port'].value_counts()
    attack_std_ports = attack_traffic[attack_traffic['destination_port'].isin(standard_ports)]['destination_port'].value_counts()
...
def intrusion_pattern_detection(df):
    ...
    high_packet_rate = df['packets_per_second'] > df['packets_per_second'].quantile(0.95)
    ...
    print(f"🔍 DDoS Pattern Detection:")
    print(f"  Pattern criteria: High packet rate (>95th %) + Short duration (<25th %)")
    ...
    port_variety = df.groupby(df.index // 100)['destination_port'].nunique()  # Port variety in chunks
    ...
    print(f"  Attack rate in small packets: {df[small_packets]['is_anomaly'].mean():.1%}")
    ...
    print(f"  Pattern criteria: Large transfer (>95th %) + Long duration (>95th %)")
    ...
    minority_protocols = unusual_protocols[unusual_protocols < unusual_protocols.quantile(0.1)].index
...
def feature_engineering_network(df):
    ...
    df_enhanced['traffic_intensity'] = df_enhanced['packet_count'] * df_enhanced['packets_per_second']
    df_enhanced['bandwidth_usage'] = df_enhanced['bytes_transferred'] / (df_enhanced['duration_seconds'] + 0.001)
    
    # Protocol risk features
    high_risk_protocols = ['ICMP']  # ICMP often used in attacks
    df_enhanced['is_high_risk_protocol'] = df_enhanced['protocol'].isin(high_risk_protocols).astype(int)
    
    # Port classification features
    df_enhanced['is_well_known_port'] = (df_enhanced['destination_port'] <= 1023).astype(int)
    df_enhanced['is_registered_port'] = ((df_enhanced['destination_port'] >= 1024) & 
                                        (df_enhanced['destination_port'] <= 49151)).astype(int)
    df_enhanced['is_dynamic_port'] = (df_enhanced['destination_port'] > 49151).astype(int)
    
    # Traffic pattern features
    df_enhanced['is_high_packet_rate'] = (df_enhanced['packets_per_second'] > 
                                         df_enhanced['packets_per_second'].quantile(0.9)).astype(int)
    df_enhanced['is_large_transfer'] = (df_enhanced['bytes_transferred'] > 
                                       df_enhanced['bytes_transferred'].quantile(0.9)).astype(int)
    df_enhanced['is_short_connection'] = (df_enhanced['duration_seconds'] < 1).astype(int)
    df_enhanced['is_long_connection'] = (df_enhanced['duration_seconds'] > 
                                        df_enhanced['duration_seconds'].quantile(0.9)).astype(int)
    
    # Efficiency features
    df_enhanced['packet_efficiency'] = df_enhanced['bytes_transferred'] / (df_enhanced['packet_count'] + 1)
    df_enhanced['time_efficiency'] = df_enhanced['packet_count'] / (df_enhanced['duration_seconds'] + 0.001)
    
    # Anomaly score features (simplified scoring)
    df_enhanced['volume_score'] = (
        (df_enhanced['packet_count'] - df_enhanced['packet_count'].mean()) / df_enhanced['packet_count'].std()
    ).abs()
    
    df_enhanced['timing_score'] = (
        (df_enhanced['duration_seconds'] - df_enhanced['duration_seconds'].mean()) / df_enhanced['duration_seconds'].std()
    ).abs()
...
def algorithm_recommendations_network(df):
    ...
    print(f"  • Feature types: Mixed (numerical + categorical)")
    print(f"  • Data complexity: Medium-High")
    ...
    algorithms = [
        {
            ...
            'suitability': 0.90,
            'pros': ['Excellent for network data', 'Handles mixed features well', 'Fast training'],
        ...
        },
        {
            ...
            'suitability': 0.85,
            'pros': ['Good for local anomalies', 'Effective for port scanning', 'No assumptions about data'],
        ...
        },
        {
            ...
            'cons': ['Assumes elliptical distribution', 'May not fit network data well'],
            'use_case': 'Clean network environments, baseline detection'
        },
        {
            'name': 'OneClassSVM',
            'suitability': 0.75,
            'pros': ['Non-linear boundaries', 'Kernel flexibility', 'Good generalization'],
            'cons': ['Slow on large datasets', 'Memory intensive', 'Parameter sensitive'],
    ...
    print(f"  1. Start with IsolationForest for general detection")
    print(f"  2. Use LOF for detailed local pattern analysis")
    print(f"  3. Consider ensemble methods for comprehensive coverage")
    print(f"  4. Implement real-time streaming detection")
    print(f"  5. Use different algorithms for different attack types")
...
def real_time_detection_strategy(df):
    ...
    medium_connections = ((connection_durations >= 1) & (connection_durations < 10)).sum()
    long_connections = (connection_durations >= 10).sum()
    
    print(f"⏱️  Connection Duration Analysis:")
    print(f"  • Quick (<1s): {quick_connections} ({quick_connections/len(df):.1%})")
    print(f"  • Medium (1-10s): {medium_connections} ({medium_connections/len(df):.1%})")
    ...
    print(f"   • Challenge: Limited data for analysis")
    print(f"   • Approach: Focus on packet patterns and port scanning")
    print(f"   • Algorithm: IsolationForest with minimal features")
    print(f"   • Threshold: Lower sensitivity to avoid false positives")
    
    print(f"\n2. Medium Connections (1-10s) - {medium_connections/len(df):.1%} of traffic:")
    print(f"   • Challenge: Balance speed vs accuracy")
    print(f"   • Approach: Multi-stage detection (quick + detailed)")
    print(f"   • Algorithm: IsolationForest + LOF for suspicious connections")
    print(f"   • Threshold: Adaptive based on traffic patterns")
    
    print(f"\n3. Long Connections (>10s) - {long_connections/len(df):.1%} of traffic:")
    print(f"   • Challenge: Data exfiltration and APT detection")
    print(f"   • Approach: Comprehensive analysis with all features")
    print(f"   • Algorithm: Ensemble methods with multiple algorithms")
    print(f"   • Threshold: High sensitivity for persistent threats")
    
    # Implementation architecture
    print(f"\n🏗️  Implementation Architecture:")
    print(f"  1. Stream Processing:")
    print(f"     • Kafka/Pulsar for high-throughput ingestion")
    print(f"     • Sliding window aggregation")
    print(f"     • Feature extraction in real-time")
    
    print(f"\n  2. Multi-tier Detection:")
    print(f"     • Tier 1: Fast rule-based screening")
    print(f"     • Tier 2: ML-based anomaly scoring")
    print(f"     • Tier 3: Deep analysis for high-risk connections")
    
    print(f"\n  3. Response System:")
    print(f"     • Automated blocking for high-confidence threats")
    print(f"     • Alert generation for suspicious activity")
    print(f"     • Forensic data collection for investigation")
...
def create_network_visualizations(df, analysis_results):
    ...
    try:
        ...
        axes[0, 1].hist(normal_packets, bins=50, alpha=0.7, label='Normal', density=True, range=(0, np.percentile(normal_packets, 95)))
        axes[0, 1].hist(attack_packets, bins=50, alpha=0.7, label='Attack', density=True, range=(0, np.percentile(attack_packets, 95)))
        ...
        axes[1, 0].hist(normal_bpp, bins=50, alpha=0.7, label='Normal', density=True, range=(0, np.percentile(normal_bpp, 95)))
        axes[1, 0].hist(attack_bpp, bins=50, alpha=0.7, label='Attack', density=True, range=(0, np.percentile(attack_bpp, 95)))
        ...
        axes[1, 1].scatter(normal_sample['duration_seconds'], normal_sample['packets_per_second'], 
                          alpha=0.6, label='Normal', s=20)
        axes[1, 1].scatter(attack_sample['duration_seconds'], attack_sample['packets_per_second'], 
                          alpha=0.8, label='Attack', s=20, color='red')
...
def main():
    ...
    print(f"   • IsolationForest recommended as primary algorithm")
    print(f"   • DDoS patterns: High packet rate + short duration")
    print(f"   • Port scanning: Small packets + port variety")
    print(f"   • Multi-tier real-time detection recommended")
