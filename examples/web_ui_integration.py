    ...
    while (time.time() - start_time) < simulation_duration:
        ...
        for use_case, detector in provider.detectors.items():
            ...
            try:
                ...
                if anomalies > 0:
                    high_score_results = [r for r in results if r.is_anomaly and r.anomaly_score.value > 0.8]
                    if high_score_results:
                        timestamp_str = current_time.strftime('%H:%M:%S')
                        print(f"   🚨 [{timestamp_str}] {detector.name}: {len(high_score_results)} high-confidence alerts")
    ...
    api_endpoints = {
        ...
        'detectors': {
            ...
            'POST /api/detectors/{id}/detect': {
                ...
                'response_example': {
                    'results': [
                        {'is_anomaly': False, 'score': 0.3, 'explanation': 'Normal transaction'},
                        {'is_anomaly': True, 'score': 0.95, 'explanation': 'High-risk transaction'}
                    ]
                }
            }
        },
        ...
        'visualization': {
            'GET /api/visualization/time-series': {
                ...
                'query_params': ['detector_id', 'start_time', 'end_time', 'granularity'],
                'response_example': {
...
document.body.addEventListener('htmx:sseMessage', function(e) {
    ...
    for pattern, code in htmx_examples.items():
        ...
        print(f"      - Server-side rendering with dynamic updates")
        print(f"      - Minimal JavaScript required")
