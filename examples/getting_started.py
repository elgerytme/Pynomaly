...
def compare_algorithms():
    ...
    for algorithm in algorithms:
        ...
        try:
            ...
            precision = true_positives / (true_positives + false_positives) if (true_positives + false_positives) > 0 else 0
            recall = true_positives / np.sum(true_labels) if np.sum(true_labels) > 0 else 0
            f1_score = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0
            ...
            print(f"   ✅ {algorithm}: {len(result.anomalies)} anomalies, F1={f1_score:.3f}")
    ...
    print(f"{'Algorithm':<20} {'Anomalies':<10} {'Precision':<10} {'Recall':<10} {'F1-Score':<10} {'Time(ms)':<10}")
    print("-" * 70)
    
    for algorithm, metrics in results.items():
        if 'error' not in metrics:
            print(f"{algorithm:<20} {metrics['anomalies_detected']:<10} {metrics['precision']:<10.3f} {metrics['recall']:<10.3f} {metrics['f1_score']:<10.3f} {metrics['execution_time_ms']:<10.2f}")
        else:
...
def advanced_usage():
    ...
    print(f"   Results after parameter change:")
    ...
    print(f"   Score range: {min(s.value for s in scores):.4f} - {max(s.value for s in scores):.4f}")
