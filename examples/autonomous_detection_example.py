...
def create_sample_datasets():
    ...
    for i in range(300):
        ...
        record = {
            "user_id": i,
            "profile": {
                "age": np.random.randint(18, 80) if not is_anomaly else np.random.randint(100, 200),
                "score": np.random.normal(0.5, 0.1) if not is_anomaly else np.random.normal(2.0, 0.5),
                "category": np.random.choice(['premium', 'basic', 'trial'])
            },
            "activity": {
                "sessions": np.random.poisson(10) if not is_anomaly else np.random.poisson(100),
                "duration_minutes": np.random.exponential(30) if not is_anomaly else np.random.exponential(300),
    ...
    seasonal = 0.5 * np.sin(2 * np.pi * np.arange(len(time_range)) / 24)  # Daily pattern
...
async def demonstrate_data_profiling():
    ...
    df = pd.DataFrame({
        ...
        'categorical_high_card': np.random.choice([f'cat_{i}' for i in range(100)], 1000),
