...
class RealTimeMonitor:
    ...
    def process_batch(self, data_points):
        ...
        if len(self.recent_data) >= 10:  # Minimum samples for detection
            ...
            dataset = Dataset(
                name=f"RealTime_Window_{datetime.now().strftime('%H%M%S')}",
                data=df
            )
            ...
            try:
                ...
                print(f"[{current_time}] Processed {len(data_points):2} samples | "
    ...
    def get_statistics(self):
        ...
        if self.detection_history:
            ...
            recent_stats = {
                'recent_avg_score': np.mean([d['avg_score'] for d in recent_detections]),
                'recent_detection_rate': np.mean([d['detection_rate'] for d in recent_detections]),
                'recent_processing_time': np.mean([d['processing_time'] for d in recent_detections])
            }
...
class DataStreamSimulator:
    ...
    def generate_normal_data(self):
        ...
        return np.random.multivariate_normal(
            mean=[0, 0, 1],
            cov=[[1, 0.3, 0], [0.3, 1, 0.1], [0, 0.1, 0.8]],
            size=1
        )[0]
...
def run_realtime_monitoring_example():
    ...
    training_df = pd.DataFrame(training_data, columns=['sensor_1', 'sensor_2', 'sensor_3'])
    ...
    for i, detection in enumerate(monitor.detection_history[-5:], 1):
        ...
        print(f"   {i}. [{timestamp}] {detection['anomalies_detected']:2} anomalies | "
