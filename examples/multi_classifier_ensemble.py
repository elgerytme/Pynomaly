...
class MultiClassifierEnsemble:
    ...
    def add_detector(self, config: DetectorConfig):
        ...
        print(f"Added detector: {config.name} ({config.algorithm}) with weight {config.weight}")
    
    async def train_ensemble(self, datasets: Dict[str, pd.DataFrame]):
        """Train all detectors in the ensemble on multiple datasets."""
        
        print(f"Training ensemble with {len(self.detector_configs)} detectors on {len(datasets)} datasets")
    ...
    def _apply_preprocessing(self, data: pd.DataFrame, preprocessing: Dict[str, Any]) -> pd.DataFrame:
    ...
    async def detect_ensemble(
        ...
        for config in self.detector_configs:
            ...
            processed_data = data.copy()
            if config.preprocessing:
                processed_data = self._apply_preprocessing(processed_data, config.preprocessing)
            
            # Run detection
            results = await detection_service.detect_batch(
                detector.id, 
                processed_data.to_dict('records')
            )
    ...
    def _combine_predictions(
        ...
        if self.voting_strategy == VotingStrategy.MAJORITY:
            ...
            weights = np.array([
                next(config.weight for config in self.detector_configs if config.name == name)
            ...
            confidence = 1 - np.std(scores_matrix, axis=1) / np.mean(scores_matrix, axis=1)
            ...
            for i, scores in enumerate(scores_matrix.T):
                rankings[:, i] = stats.rankdata(-scores, method='ordinal')  # Higher score = lower rank
    ...
    def _rank_anomalies(
        ...
        if ranking_method == RankingMethod.AVERAGE_SCORE:
            ...
            for i in range(rankings.shape[0]):
                ...
                concordance_scores[i] = concordance / (n_detectors * (n_detectors - 1) / 2)
    ...
    def _calculate_consensus(self, individual_predictions: Dict[str, np.ndarray]) -> float:
    ...
    def filter_anomalies(
        ...
        filters['high_confidence'] = np.where(high_confidence_mask & result.predictions)[0]
    ...
    def analyze_detector_performance(
        ...
        for name, predictions in result.individual_predictions.items():
            ...
            if true_labels is not None:
                ...
                if len(np.unique(true_labels)) > 1:
                    stats['auc_roc'] = roc_auc_score(true_labels, result.individual_scores[name])
        ...
        # Diversity metrics
        predictions_matrix = np.stack(list(result.individual_predictions.values()), axis=1)
        ...
        analysis['diversity_metrics']['avg_q_statistic'] = np.mean(q_statistics) if q_statistics else 0
    ...
    def _print_training_summary(self):
        ...
        for detector_name, dataset_results in self.training_stats.items():
            ...
            for dataset_name, stats in dataset_results.items():
                print(f"  {dataset_name}: {stats['training_time']:.2f}s, "
    ...
    def visualize_results(self, result: EnsembleResult, data: pd.DataFrame, save_path: Optional[str] = None):
        """Create comprehensive visualization of ensemble results."""
        
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle('Multi-Classifier Ensemble Analysis', fontsize=16, fontweight='bold')
        ...
        ax3.axvline(np.mean(result.confidence), color='red', linestyle='--', 
        ...
        sns.heatmap(correlation_matrix, 
                   ...
                   annot=True, fmt='.2f', ax=ax4, cmap='coolwarm', center=0)
...
class DatasetGenerator:
    ...
    @staticmethod
    def generate_fraud_dataset(n_samples: int = 1000, contamination: float = 0.1) -> pd.DataFrame:
        ...
        fraud_data = {
            ...
            'transaction_hour': np.random.choice(range(0, 6), fraud_samples),  # Unusual hours
            ...
            'is_weekend': np.random.choice([0, 1], fraud_samples, p=[0.3, 0.7])  # Weekend bias
        }
    ...
    @staticmethod
    def generate_network_dataset(n_samples: int = 1000, contamination: float = 0.05) -> pd.DataFrame:
        ...
        intrusion_data = {
            'packet_size': np.concatenate([
                np.random.normal(64, 10, intrusion_samples//3),    # Small packets (scanning)
                np.random.normal(1500, 100, intrusion_samples//3), # Large packets (DDoS)
                np.random.normal(1024, 256, intrusion_samples//3)  # Normal size (stealth)
            ]),
            'connection_duration': np.concatenate([
                np.random.exponential(0.1, intrusion_samples//2),  # Very short (scanning)
                np.random.exponential(100, intrusion_samples//2)   # Very long (persistent)
            ]),
            'bytes_sent': np.random.lognormal(12, 3, intrusion_samples),  # High volume
            'bytes_received': np.random.lognormal(6, 1, intrusion_samples),  # Low response
            'packets_per_second': np.random.gamma(10, 10, intrusion_samples),  # High rate
            'unique_ports': np.random.poisson(20, intrusion_samples),  # Port scanning
            'protocol_tcp': np.random.choice([0, 1], intrusion_samples, p=[0.8, 0.2])  # More UDP
        }
    ...
    @staticmethod
    def generate_sensor_dataset(n_samples: int = 1000, contamination: float = 0.08) -> pd.DataFrame:
        ...
        normal_data = {
            'temperature': 20 + 5 * np.sin(time_points * 0.1) + np.random.normal(0, 0.5, normal_samples),
            'pressure': 1013 + 10 * np.cos(time_points * 0.05) + np.random.normal(0, 1, normal_samples),
            'humidity': 50 + 20 * np.sin(time_points * 0.08) + np.random.normal(0, 2, normal_samples),
            'vibration': 0.1 + 0.05 * np.random.random(normal_samples),
            'power_consumption': 100 + 20 * np.sin(time_points * 0.12) + np.random.normal(0, 3, normal_samples)
        }
        ...
        anomaly_data = {
            ...
            'humidity': np.random.choice([10, 95], anomaly_samples) + np.random.normal(0, 5, anomaly_samples),
...
async def main():
    ...
    for name, data in datasets.items():
        ...
        print(f"  {name}: {len(data)} samples, {len(data.columns)-1} features, {anomaly_rate:.1%} anomalies")
    ...
    for dataset_name, test_data in training_datasets.items():
        ...
        # Analyze performance
        analysis = ensemble.analyze_detector_performance(result, true_labels[dataset_name])
        ...
        if true_labels[dataset_name] is not None:
            ...
            for detector_name, stats in analysis['detector_stats'].items():
                if 'f1_score' in stats:
                    print(f"    {detector_name}: F1={stats['f1_score']:.3f}, "
        ...
        if dataset_name == 'fraud':
            ...
            ensemble.visualize_results(result, test_data, f"{dataset_name}_ensemble_analysis.png")
    ...
    for dataset_name, result in all_results.items():
        ...
        if true_labels[dataset_name] is not None:
            ...
            ensemble_precision = ensemble._safe_precision(true_labels[dataset_name], result.predictions)
            ensemble_recall = ensemble._safe_recall(true_labels[dataset_name], result.predictions)
            ensemble_f1 = ensemble._safe_f1(true_labels[dataset_name], result.predictions)
