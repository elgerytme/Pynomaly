...
class CustomStatisticalAdapter(DetectorProtocol):
    ...
    async def detect_batch(self, detector: Detector, data: List[Dict[str, Any]]) -> List[DetectionResult]:
    ...
    async def detect_single(self, detector: Detector, data_point: Dict[str, Any]) -> DetectionResult:
    ...
    def _generate_explanation(self, data_point: Dict[str, Any], score: float, prediction: int) -> str:
...
async def demonstrate_custom_algorithm():
    ...
    detector = await adapter.create_detector(
        name="Custom Statistical Detector",
        z_threshold=2.5,
        iqr_multiplier=1.5
    )
    ...
    for i, result in enumerate(results[:10]):
        ...
        print(f"   {i+1:2d}. {status} | Score: {score:.3f} | F1: {f1:6.2f}, F2: {f2:6.2f}")
...
class CustomEnsembleAdapter(DetectorProtocol):
    ...
    async def create_detector(self, name: str, **parameters) -> Detector:
        ...
        detector = Detector(
            ...
            parameters={'voting_strategy': voting_strategy, 'n_base_detectors': len(base_detectors)},
            metadata={
...
async def demonstrate_custom_ensemble():
    ...
    for strategy in strategies:
        ...
        detector = await adapter.create_detector(
            name=f"Custom Ensemble ({strategy})",
