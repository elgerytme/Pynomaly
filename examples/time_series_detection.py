...
def generate_time_series_data(n_points: int = 1000, anomaly_rate: float = 0.05):
    ...
    # Add anomalies
    anomaly_indices = np.random.choice(n_points, int(n_points * anomaly_rate), replace=False)
...
def extract_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['dist_from_ma_168'] = df['value'] - df['value'].rolling(window=168).mean()  # Weekly MA
...
async def basic_time_series_detection():
    ...
    print(f"   - Features created: {len(df.columns) - 3}")  # Exclude timestamp, value, is_anomaly
    ...
    false_positives = sum((~test_df_subset['is_anomaly']) & (test_df_subset['predicted']))
    true_negatives = sum((~test_df_subset['is_anomaly']) & (~test_df_subset['predicted']))
    false_negatives = sum((test_df_subset['is_anomaly']) & (~test_df_subset['predicted']))
    ...
    if len(detected_anomalies) > 0:
        ...
        for _, row in detected_anomalies.iterrows():
            ...
            print(f"   [{timestamp}] Value: {value:6.1f} | Score: {score:.3f} | Actual: {actual}")
...
async def multi_algorithm_time_series():
    ...
    dataset = await dataset_service.create_from_data(
        data=train_data,
        name="Multi-Algorithm Time Series Data"
    )
    ...
    for algo_name, params in algorithms:
        ...
        try:
            ...
            results[algo_name] = {'f1_score': 0, 'precision': 0, 'recall': 0, 'detected_anomalies': 0}
...
async def seasonal_decomposition_example():
    ...
    df['residual_normalized'] = (df['residual'] - df['residual'].mean()) / df['residual'].std()
    ...
    feature_columns = [
        'value_normalized', 'residual_normalized', 
        'value_lag_1', 'value_lag_24', 'residual_lag_1',
        'residual_rolling_std', 'residual_rolling_mean'
    ]
    ...
    dataset = await dataset_service.create_from_data(
        data=train_data,
        name="Seasonal Time Series Data"
    )
