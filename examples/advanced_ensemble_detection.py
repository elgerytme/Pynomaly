...
class AdvancedEnsembleDetector:
    ...
    async def get_individual_predictions(self, detector_ids: List[str], dataset: Dataset) -> Tuple[np.ndarray, List[np.ndarray]]:
        ...
        for detector_id in detector_ids:
            try:
                ...
                if hasattr(result, 'anomaly_scores') and result.anomaly_scores is not None:
                    ...
                    soft_scores = predictions.astype(float) + np.random.normal(0, 0.1, len(predictions))
    ...
    def calculate_performance_weights(self, predictions: np.ndarray, true_labels: np.ndarray) -> np.ndarray:
    ...
    def weighted_voting(self, predictions: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    def uncertainty_weighted_ensemble(self, scores: List[np.ndarray]) -> Tuple[np.ndarray, List[ConfidenceInterval], np.ndarray]:
    ...
    def stacking_ensemble(self, scores: List[np.ndarray], predictions: np.ndarray, 
        ...
        if true_labels is not None:
            ...
            weights = self.calculate_performance_weights(predictions, true_labels) if true_labels is not None else np.ones(len(scores)) / len(scores)
    ...
    def adaptive_ensemble(self, scores: List[np.ndarray], predictions: np.ndarray,
                         true_labels: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.adaptive_weights = (
            (1 - self.adaptation_rate) * self.adaptive_weights + 
            self.adaptation_rate * recent_weights
        )
    ...
    async def ensemble_detect(self, detector_ids: List[str], dataset: Dataset, 
                            method: EnsembleMethod, true_labels: Optional[np.ndarray] = None) -> EnsembleResult:
        """Perform ensemble detection using specified advanced method."""
        
        # Get individual predictions and scores
        predictions, scores = await self.get_individual_predictions(detector_ids, dataset)
        ...
        if method == EnsembleMethod.SIMPLE_VOTING:
            ensemble_predictions = (np.sum(predictions, axis=0) >= len(detector_ids) / 2).astype(int)
            ...
            ensemble_predictions, confidence_intervals, ensemble_scores = self.uncertainty_weighted_ensemble(scores)
            ...
            ensemble_predictions = self.stacking_ensemble(scores, predictions, true_labels)
            ensemble_scores = np.mean(scores, axis=0)
            
        elif method == EnsembleMethod.ADAPTIVE_ENSEMBLE:
            ensemble_predictions, weights = self.adaptive_ensemble(scores, predictions, true_labels)
            ensemble_scores = np.average(scores, axis=0, weights=weights)
        ...
        performance_metrics = {}
        if true_labels is not None:
            performance_metrics = self.evaluate_predictions(true_labels, ensemble_predictions)
    ...
    def evaluate_predictions(self, true_labels: np.ndarray, predictions: np.ndarray) -> Dict[str, float]:
...
def create_comprehensive_dataset():
    ...
    anomaly_types = {
        ...
        'multivariate_outliers': np.random.randn(12, 6) * [0.4, 2.5, 0.4, 2.5, 0.4, 2.5] + [0, 0, 3, 0, 3, 0],
        'density_anomalies': np.random.randn(15, 6) * 0.15 + [0.5, 0.5, 2.5, 2.5, 0, 0],
        'contextual_anomalies': np.array([
            [np.sin(i/5), np.cos(i/5), i/10, np.random.randn(), np.random.randn(), np.random.randn()]
...
async def create_diverse_ensemble_detectors(container, dataset: Dataset) -> List[str]:
    ...
    detector_configs = [
        # Statistical detectors
        {
            ...
            'params': {'contamination': 0.08, 'n_estimators': 200, 'max_samples': 256, 'random_state': 42}
        },
        {
...
async def demonstrate_ensemble_comparison(container, detector_ids: List[str], dataset: Dataset, true_labels: np.ndarray):
    ...
    for method in ensemble_methods:
        ...
        if ensemble_result.confidence_intervals:
            avg_width = np.mean([ci.width() for ci in ensemble_result.confidence_intervals])
            print(f"   - Avg Confidence Width: {avg_width:.3f}")
        
        # Show diversity metrics
        print(f"   - Diversity (disagreement): {ensemble_result.diversity_metrics['disagreement']:.3f}")
        print(f"   - Diversity (entropy): {ensemble_result.diversity_metrics['entropy']:.3f}")
    
    return results, ensemble_results


async def analyze_ensemble_performance(container, detector_ids: List[str], results: Dict, ensemble_results: Dict, true_labels: np.ndarray):
    ...
    ensemble_detector = AdvancedEnsembleDetector(container)
    individual_predictions, _ = await ensemble_detector.get_individual_predictions(detector_ids, dataset)
    
    for i, detector_id in enumerate(detector_ids):
        detector = detector_repo.get(detector_id)
        individual_metrics = ensemble_detector.evaluate_predictions(true_labels, individual_predictions[i])
        individual_f1_scores.append(individual_metrics['f1_score'])
        print(f"   {detector.name}: F1={individual_metrics['f1_score']:.3f}, Precision={individual_metrics['precision']:.3f}, Recall={individual_metrics['recall']:.3f}")
    ...
    for method, result in results.items():
        ...
        print(f"   {method}:")
        print(f"     vs Average: {improvement_vs_avg:+.3f} ({(improvement_vs_avg/baseline_f1)*100:+.1f}%)")
        print(f"     vs Best: {improvement_vs_best:+.3f} ({(improvement_vs_best/best_individual_f1)*100:+.1f}%)")
...
async def demonstrate_uncertainty_analysis(ensemble_results: Dict):
    ...
    if uncertainty_method in ensemble_results:
        ...
        if uncertainty_result.confidence_intervals:
            ...
            high_uncertainty_indices = np.where(np.array(widths) > high_uncertainty_threshold)[0]
            
            print(f"   - High uncertainty samples: {len(high_uncertainty_indices)} ({len(high_uncertainty_indices)/len(widths)*100:.1f}%)")
            ...
            high_score_indices = np.where(ensemble_scores > np.percentile(ensemble_scores, 90))[0]
            high_uncertainty_high_score = len(set(high_uncertainty_indices) & set(high_score_indices))
            
            print(f"   - High uncertainty + high anomaly score: {high_uncertainty_high_score}")
            print(f"     (These are the most interesting/uncertain predictions)")
...
async def main():
    ...
    print(f"   Data complexity: {data.shape[0]/data.shape[1]:.1f} samples per feature")
    
    # Feature correlation analysis
    feature_correlations = np.corrcoef(data.T)
    avg_correlation = np.mean(np.abs(feature_correlations[np.triu_indices_from(feature_correlations, k=1)]))
    ...
    for method, metrics in results.items():
        print(f"   {method:<25} {metrics['precision']:.3f}    {metrics['recall']:.3f}   {metrics['f1_score']:.3f}   {metrics['mcc']:.3f}   {metrics['balanced_accuracy']:.3f}")
    
    print("   " + "="*85)
    
    # Key insights
    print(f"\n7. Key Insights:")
    print(f"   • Best performing method: {best_method[0]} (F1: {best_method[1]['f1_score']:.3f})")
    
    # Find most improved method
    best_improvement = max(improvements.items(), key=lambda x: x[1]['vs_average'])
    print(f"   • Most improved vs average: {best_improvement[0]} (+{best_improvement[1]['vs_average']:.3f})")
    
    # Diversity insights
    most_diverse = max(ensemble_results.items(), key=lambda x: x[1].diversity_metrics['disagreement'])
    print(f"   • Most diverse ensemble: {most_diverse[0].value} (disagreement: {most_diverse[1].diversity_metrics['disagreement']:.3f})")
    ...
    print("improve anomaly detection performance through diversity and intelligent aggregation.")
