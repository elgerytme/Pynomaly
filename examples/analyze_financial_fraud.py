...
def load_financial_fraud_data():
    ...
    data_path = (
        Path(__file__).parent / "sample_datasets" / "synthetic" / "financial_fraud.csv"
    )
    ...
    print(
        f"✅ Loaded financial fraud dataset: {len(df)} samples, {len(df.columns) - 1} features"
    )
    return df
...
def exploratory_data_analysis(df):
    ...
    fraud_by_hour = df.groupby("hour_of_day")["is_anomaly"].agg(
        ["count", "sum", "mean"]
    )
    ...
    merchant_fraud = df.groupby("merchant_category")["is_anomaly"].agg(
        ["count", "sum", "mean"]
    )
...
def feature_engineering(df):
    ...
    df_enhanced["amount_zscore"] = (
        df_enhanced["transaction_amount"] - df_enhanced["transaction_amount"].mean()
    ) / df_enhanced["transaction_amount"].std()
    df_enhanced["is_large_amount"] = (
        df_enhanced["transaction_amount"]
        > df_enhanced["transaction_amount"].quantile(0.95)
    ).astype(int)
    df_enhanced["is_micro_amount"] = (df_enhanced["transaction_amount"] < 1).astype(int)

    # Time-based features
    df_enhanced["is_night_transaction"] = (
        (df_enhanced["hour_of_day"] >= 23) | (df_enhanced["hour_of_day"] <= 5)
    ).astype(int)
    df_enhanced["is_business_hours"] = (
        (df_enhanced["hour_of_day"] >= 9) & (df_enhanced["hour_of_day"] <= 17)
    ).astype(int)

    # Velocity features
    df_enhanced["high_velocity"] = (
        df_enhanced["velocity_score"] > df_enhanced["velocity_score"].quantile(0.9)
    ).astype(int)
    df_enhanced["very_high_frequency"] = (df_enhanced["daily_frequency"] > 10).astype(
        int
    )

    # Merchant risk features
    high_risk_categories = ["atm", "casino", "luxury", "crypto", "unknown"]
    df_enhanced["is_high_risk_merchant"] = (
        df_enhanced["merchant_category"].isin(high_risk_categories).astype(int)
    )
...
def autonomous_detection_example(df):
    ...
    print("  • Data complexity: Medium (mixed numerical/categorical)")
    ...
    recommended_algorithms = [
        (
            "IsolationForest",
            0.85,
            "Excellent for mixed data types and financial patterns",
        ),
...
def manual_algorithm_comparison(df):
    ...
    for algo, metrics in results.items():
        print(
            f"{algo:<20} {metrics['precision']:<10.3f} {metrics['recall']:<8.3f} {metrics['f1']:<8.3f} {metrics['time']:<8.1f}"
        )
...
def fraud_detection_insights(df, analysis_results):
    ...
    print(
        f"  • Large amounts (>95th percentile): {large_amount_fraud_rate:.1%} fraud rate"
    )
    print(f"  • Micro amounts (<$1): {micro_amount_fraud_rate:.1%} fraud rate")

    # Time patterns
    night_fraud_rate = df[((df["hour_of_day"] >= 23) | (df["hour_of_day"] <= 5))][
        "is_anomaly"
    ].mean()
    print(f"  • Night transactions (11pm-5am): {night_fraud_rate:.1%} fraud rate")

    # Velocity patterns
    high_velocity_fraud_rate = df[
        (df["velocity_score"] > df["velocity_score"].quantile(0.9))
    ]["is_anomaly"].mean()
    print(f"  • High velocity transactions: {high_velocity_fraud_rate:.1%} fraud rate")

    print("\n💡 Recommendations:")
    print("  • Deploy real-time scoring using IsolationForest or LOF")
    print("  • Implement velocity-based rules for immediate blocking")
    print("  • Add merchant category risk scoring")
    print("  • Monitor for time-based anomaly patterns")
    print("  • Use ensemble methods for improved detection")

    print("\n⚠️  Implementation Considerations:")
    print("  • Balance precision vs recall based on fraud costs")
    print("  • Implement human review queues for edge cases")
    print("  • Monitor for concept drift over time")
    print("  • Consider customer impact of false positives")
    print("  • Ensure compliance with financial regulations")
...
def create_visualizations(df, analysis_results):
    ...
    try:
        ...
        axes[0, 0].hist(
            analysis_results["normal_amounts"],
            bins=50,
            alpha=0.7,
            label="Normal",
            density=True,
        ...
        axes[0, 0].hist(
            analysis_results["fraud_amounts"],
            bins=50,
            alpha=0.7,
            label="Fraud",
            density=True,
        ...
        axes[1, 1].set_xticklabels(
            [col.replace("_", " ").title() for col in feature_cols], rotation=45
        )
...
def main():
    ...
    print("   • IsolationForest recommended as top algorithm")
    print("   • Time and amount patterns are strong indicators")
    print("   • Feature engineering significantly improves detection")
