...
class SimpleAutoML:
    ...
    def run_automl(self, dataset, max_algorithms=10):
        ...
        for algo_name, params in sklearn_algorithms[:max_algorithms//2]:
            ...
            if result['success']:
                print(f"✅ Score: {result['avg_score']:.3f}, Time: {result['execution_time']:.2f}s")
            else:
    ...
    def get_summary(self):
        ...
        # Sort by score
        sorted_results = sorted(successful_results, key=lambda x: x['avg_score'], reverse=True)
        
        summary = {
            'total_tested': len(self.results),
            'successful': len(successful_results),
            'best_algorithm': self.best_algorithm['name'] if self.best_algorithm else None,
