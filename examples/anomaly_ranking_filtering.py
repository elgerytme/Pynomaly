...
class AnomalyRankingSystem:
    ...
    async def get_detector_predictions(self, detector_ids: List[str], dataset: Dataset) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
            ...
            try:
                ...
                if hasattr(result, 'anomaly_scores') and result.anomaly_scores is not None:
                    ...
                    if iqr > 0:
                        ...
                        normalized_scores = (raw_scores - np.min(raw_scores)) / (np.max(raw_scores) - np.min(raw_scores) + 1e-8)
    ...
    def calculate_performance_weights(self, predictions: Dict[str, np.ndarray], 
        ...
        if true_labels is None:
            ...
            return {detector_id: 1.0 for detector_id in predictions.keys()}
    ...
    def rank_anomalies_average_score(self, scores: Dict[str, np.ndarray], 
                                   weights: Optional[Dict[str, float]] = None) -> np.ndarray:
        """Rank anomalies by average score."""
        score_matrix = np.array(list(scores.values()))
        
        if weights is not None:
            weight_array = np.array([weights.get(detector_id, 1.0) for detector_id in scores.keys()])
    ...
    def rank_anomalies_consensus(self, scores: Dict[str, np.ndarray], 
        ...
        for i in range(n_samples):
            ...
            for j in range(n_samples):
                if i != j:
                    ...
                    agreements = np.sum((sample_scores > other_scores) == 
    ...
    def rank_anomalies_geometric_mean(self, scores: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # Normalize
        geometric_means = (geometric_means - np.min(geometric_means)) / (np.max(geometric_means) - np.min(geometric_means) + epsilon)
    ...
    def rank_anomalies_harmonic_mean(self, scores: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # Normalize
        harmonic_means = (harmonic_means - np.min(harmonic_means)) / (np.max(harmonic_means) - np.min(harmonic_means) + epsilon)
        
        return harmonic_means
    
    def calculate_confidence_intervals(self, scores: Dict[str, np.ndarray]) -> List[ConfidenceInterval]:
    ...
    async def rank_anomalies(self, detector_ids: List[str], dataset: Dataset, 
                           method: RankingMethod, true_labels: Optional[np.ndarray] = None) -> List[RankedAnomaly]:
        ...
        for rank, sample_idx in enumerate(sorted_indices):
            ...
            individual_scores = [scores[detector_id][sample_idx] for detector_id in detector_ids]
            
            # Detector agreement (which detectors flagged this as anomaly)
            detector_agreement = {}
            for detector_id in detector_ids:
                detector_agreement[detector_id] = predictions[detector_id][sample_idx] == 1
            ...
            metadata = {
                ...
                'weight_contribution': sum([weights[detector_id] * scores[detector_id][sample_idx] 
                                          for detector_id in detector_ids])
            }
    ...
    def filter_anomalies_top_k(self, ranked_anomalies: List[RankedAnomaly], k: int) -> FilteringResult:
        ...
        statistics = {
            ...
            'rejection_rate': (len(ranked_anomalies) - len(filtered)) / len(ranked_anomalies),
            'score_range_selected': (filtered[0].final_score, filtered[-1].final_score) if filtered else (0, 0)
        }
    ...
    def filter_anomalies_confidence_threshold(self, ranked_anomalies: List[RankedAnomaly], 
                                            confidence_threshold: float = 0.8) -> FilteringResult:
        ...
        for anomaly in ranked_anomalies:
            ...
            if confidence >= confidence_threshold:
                ...
                rejection_reasons[anomaly.sample_index] = f"Confidence {confidence:.3f} < {confidence_threshold}"
        
        statistics = {
            'total_candidates': len(ranked_anomalies),
            'selected': len(filtered),
            'avg_confidence_selected': np.mean([1 - a.confidence_interval.width() for a in filtered]) if filtered else 0,
            'confidence_threshold': confidence_threshold
        }
    ...
    def filter_anomalies_consensus_level(self, ranked_anomalies: List[RankedAnomaly], 
                                       min_consensus: float = 0.6) -> FilteringResult:
        ...
        for anomaly in ranked_anomalies:
            if anomaly.consensus_level >= min_consensus:
                ...
                rejection_reasons[anomaly.sample_index] = f"Consensus {anomaly.consensus_level:.3f} < {min_consensus}"
        
        statistics = {
            'total_candidates': len(ranked_anomalies),
            'selected': len(filtered),
            'avg_consensus_selected': np.mean([a.consensus_level for a in filtered]) if filtered else 0,
    ...
    def filter_anomalies_statistical(self, ranked_anomalies: List[RankedAnomaly], 
                                   z_threshold: float = 2.0) -> FilteringResult:
        ...
        for anomaly in ranked_anomalies:
            ...
            if z_score >= z_threshold:
                ...
                rejection_reasons[anomaly.sample_index] = f"Z-score {z_score:.3f} < {z_threshold}"
    ...
    def filter_anomalies_combined(self, ranked_anomalies: List[RankedAnomaly], 
                                 top_k: int = 50, min_consensus: float = 0.5, 
                                 confidence_threshold: float = 0.7) -> FilteringResult:
        ...
        stage2 = self.filter_anomalies_consensus_level(stage1.filtered_anomalies, min_consensus)
        
        # Stage 3: Confidence threshold
        stage3 = self.filter_anomalies_confidence_threshold(stage2.filtered_anomalies, confidence_threshold)
...
def create_multi_pattern_dataset():
    ...
    anomaly_patterns = {
        ...
        'multi_dimensional': np.random.randn(15, 8) * [0.5, 3, 0.5, 3, 0.5, 3, 0.5, 3] + [0, 0, 4, 0, 4, 0, 4, 0],
        'contextual': np.array([
    ...
    detector_configs = [
        {
            ...
            'params': {'contamination': 0.05, 'n_estimators': 300, 'max_samples': 256, 'random_state': 42},
...
async def demonstrate_ranking_methods(ranking_system: AnomalyRankingSystem, detector_ids: List[str], 
                                    dataset: Dataset, true_labels: np.ndarray):
    ...
    for method in ranking_methods:
        ...
        true_anomalies_top10 = sum([1 for a in top_10 if true_labels[a.sample_index] == 1])
        precision_at_10 = true_anomalies_top10 / 10
        
        print(f"   - Top 10 precision: {precision_at_10:.3f}")
        print(f"   - Score range: {top_10[0].final_score:.3f} - {top_10[-1].final_score:.3f}")
        print(f"   - Avg consensus in top 10: {np.mean([a.consensus_level for a in top_10]):.3f}")
        print(f"   - Avg confidence width: {np.mean([a.confidence_interval.width() for a in top_10]):.3f}")
    
    return ranking_results


async def demonstrate_filtering_strategies(ranking_system: AnomalyRankingSystem, 
    ...
    filtering_strategies = [
        ...
        ('High Confidence', FilteringStrategy.CONFIDENCE_THRESHOLD, {'confidence_threshold': 0.8}),
        ('Strong Consensus', FilteringStrategy.CONSENSUS_LEVEL, {'min_consensus': 0.7}),
        ('Statistical Filter', FilteringStrategy.STATISTICAL_FILTER, {'z_threshold': 2.5}),
        ('Combined Filter', FilteringStrategy.COMBINED_FILTER, {
    ...
    for strategy_name, strategy, params in filtering_strategies:
        ...
        if strategy == FilteringStrategy.TOP_K:
            result = ranking_system.filter_anomalies_top_k(ranked_anomalies, params['k'])
            ...
            result = ranking_system.filter_anomalies_combined(
                ranked_anomalies, params['top_k'], params['min_consensus'], params['confidence_threshold']
            )
        
        print(f"   - Selected: {result.filter_statistics['selected']} anomalies")
        print(f"   - Rejection rate: {result.filter_statistics.get('rejection_rate', 0):.1%}")
...
async def analyze_ranking_effectiveness(ranking_results: Dict, true_labels: np.ndarray, severity_labels: List[str]):
    ...
    print(f"   Method\\K\t\t" + "\t".join([f"@{k}" for k in top_k_values]))
    print(f"   " + "-" * 60)
    ...
    for method, ranked_anomalies in ranking_results.items():
        ...
        for k in top_k_values:
            ...
            true_anomalies_in_top_k = sum([1 for a in top_k_anomalies if true_labels[a.sample_index] == 1])
    ...
    print(f"   Method\\Severity\t\tHigh\tMedium\tLow")
    print(f"   " + "-" * 50)
    ...
    for method, ranked_anomalies in ranking_results.items():
        ...
        print(f"   {method.value:<20}\t\t{severity_counts['high']}\t{severity_counts['medium']}\t{severity_counts['low']}")
...
async def main():
    ...
    data, true_labels, severity_labels, anomaly_patterns = create_multi_pattern_dataset()
    print(f"   Dataset: {len(data)} samples, {data.shape[1]} features")
    print(f"   True anomalies: {np.sum(true_labels)} ({np.mean(true_labels)*100:.1f}%)")
    print("   Anomaly patterns included:")
    for pattern_name, pattern_data in anomaly_patterns.items():
        print(f"   - {pattern_name}: {len(pattern_data)} samples")
    
    # Severity distribution
    severity_dist = {sev: severity_labels.count(sev) for sev in ['normal', 'low', 'medium', 'high']}
    ...
    print(f"   Production workflow results:")
    print(f"   - Input candidates: {len(consensus_ranked)}")
    print(f"   - After combined filtering: {production_filter.filter_statistics['final_selected']}")
    ...
    if production_filter.filtered_anomalies:
        ...
        final_true_anomalies = sum([
            1 for a in production_filter.filtered_anomalies 
            if true_labels[a.sample_index] == 1
        ])
        final_precision = final_true_anomalies / len(production_filter.filtered_anomalies)
        
        print(f"   - Final precision: {final_precision:.3f}")
        print(f"   - Score range: {production_filter.filtered_anomalies[0].final_score:.3f} - {production_filter.filtered_anomalies[-1].final_score:.3f}")
        
        # Top 5 anomalies for manual review
        print(f"\n   Top 5 anomalies for manual review:")
        for i, anomaly in enumerate(production_filter.filtered_anomalies[:5]):
            true_label = "ANOMALY" if true_labels[anomaly.sample_index] == 1 else "NORMAL"
            severity = severity_labels[anomaly.sample_index]
            agreeing_detectors = anomaly.metadata['agreeing_detectors']
            
            print(f"   {i+1}. Sample {anomaly.sample_index}: Score={anomaly.final_score:.3f}, "
