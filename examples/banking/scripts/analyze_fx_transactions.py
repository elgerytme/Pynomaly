...
class FXAnomalyDetector:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features_df['is_market_hours'] = ((features_df['hour'] >= 8) & (features_df['hour'] <= 17)).astype(int)
        ...
        features_df['near_threshold'] = ((features_df['amount_from'] >= 9500) & 
                                        (features_df['amount_from'] < 10000)).astype(int)
        
        # Exchange rate features
        # Calculate expected rates based on historical data (simplified)
        rate_stats = features_df.groupby('to_currency')['exchange_rate'].agg(['mean', 'std']).reset_index()
        rate_stats.columns = ['to_currency', 'rate_mean', 'rate_std']
        features_df = features_df.merge(rate_stats, on='to_currency')
        
        # Rate deviation
        features_df['rate_deviation'] = abs(features_df['exchange_rate'] - features_df['rate_mean']) / (features_df['rate_std'] + 0.01)
        ...
        customer_stats.columns = ['customer_id', 'fx_count', 'avg_amount', 'std_amount', 
                                 'total_amount', 'first_fx', 'last_fx']
        ...
        features_df = pd.concat([features_df, currency_encoded, purpose_encoded, method_encoded], axis=1)
        
        # Fee analysis
        features_df['fee_rate'] = features_df['fee_usd'] / features_df['amount_from']
        features_df['low_fee'] = (features_df['fee_rate'] < 0.001).astype(int)  # Suspiciously low fee
    ...
    def select_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        feature_columns = [
            'amount_from', 'amount_log', 'large_amount', 'near_threshold',
            'exchange_rate', 'rate_deviation', 'suspicious_rate',
            'hour', 'day_of_week', 'is_weekend', 'is_market_hours',
            'fx_count', 'avg_amount', 'std_amount',
            'fx_last_day', 'amount_last_day',
            'fee_usd', 'fee_rate', 'low_fee'
        ]
        
        # Add encoded features
        curr_cols = [col for col in df.columns if col.startswith('curr_')][:5]  # Top 5 currencies
    ...
    def detect_anomalies(self, df: pd.DataFrame, contamination: float = 0.1):
        ...
        features_df['ensemble_score'] = (
            features_df['iso_score'] + features_df['svm_score'] + features_df['lof_score']
        ) / 3
    ...
    def analyze_anomalies(self, df: pd.DataFrame):
        ...
        if len(anomalies) > 0:
            ...
            print(f"Average amount: ${anomalies['amount_from'].mean():,.2f} vs ${normal['amount_from'].mean():,.2f} (normal)")
            ...
            print(f"\nTiming Patterns:")
            print(f"Off-market-hours transactions: {(1-anomalies['is_market_hours']).sum()}")
            ...
            print(f"\nPurpose Analysis:")
            purpose_cols = [col for col in anomalies.columns if col.startswith('purpose_')]
            if purpose_cols:
                purpose_sums = anomalies[purpose_cols].sum().sort_values(ascending=False)
    ...
    def generate_report(self, df: pd.DataFrame, anomalies: pd.DataFrame, output_dir: str = None):
        ...
        report.append(f"- Suspicious transactions detected: {len(anomalies):,} ({len(anomalies)/len(df)*100:.1f}%)")
        report.append(f"- Total suspicious amount: ${anomalies['amount_from'].sum():,.2f}" if len(anomalies) > 0 else "- No suspicious transactions detected")
        ...
        if len(anomalies) > 0:
            ...
            if len(large_amounts) > 0:
                report.append(f"- {len(large_amounts)} large FX transactions (>$50,000)")
                report.append(f"  - Total value: ${large_amounts['amount_from'].sum():,.2f}")
            
            # Structuring
            structuring = anomalies[anomalies['near_threshold'] == 1]
            if len(structuring) > 0:
                report.append(f"- {len(structuring)} potential structuring transactions (near $10K threshold)")
            
            # Rate manipulation
            rate_manip = anomalies[anomalies['suspicious_rate'] == 1]
            if len(rate_manip) > 0:
                report.append(f"- {len(rate_manip)} transactions with suspicious exchange rates")
            ...
            if len(high_velocity) > 0:
                report.append(f"- {len(high_velocity)} customers with high daily FX activity")
            
            report.append("\n## Highest Risk Transactions")
            top_anomalies = anomalies.nlargest(10, 'ensemble_score')[
                ['transaction_id', 'customer_id', 'amount_from', 'to_currency', 'purpose', 'ensemble_score']
            ]
            report.append(top_anomalies.to_string(index=False))
            
            report.append("\n## Regulatory Recommendations")
            report.append("1. File Suspicious Activity Reports (SARs) for transactions with score > 0.8")
            report.append("2. Enhanced due diligence for customers with multiple high-risk FX transactions")
            ...
            report.append("5. Verify business purpose for large commercial FX transactions")
