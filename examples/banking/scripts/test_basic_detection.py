...
def test_basic_anomaly_detection():
    ...
    # Load deposit data
    try:
        deposits = pd.read_csv('../datasets/deposits.csv')
        print(f"✓ Loaded {len(deposits)} deposit records")
    except FileNotFoundError:
        print("✗ Deposit data not found. Run generate_sample_data.py first.")
        return False
    ...
    try:
        ...
        detector = SklearnAdapter("IsolationForest", contamination_rate=contamination_rate)
...
def test_data_quality():
    ...
    for dataset_file in datasets:
        try:
            df = pd.read_csv(f'../datasets/{dataset_file}')
            anomalies = df['is_anomaly'].sum() if 'is_anomaly' in df.columns else 0
            
            print(f"✓ {dataset_file}: {len(df):,} records, {anomalies} anomalies ({anomalies/len(df)*100:.1f}%)")
...
def main():
    ...
    if data_ok and detection_ok:
        ...
        print("\nNext steps:")
        print("1. Run individual analysis scripts: analyze_deposits.py, analyze_credit_cards.py")
