...
class BankingDataGenerator:
    ...
    def _generate_customer_base(self) -> List[Dict[str, Any]]:
        ...
        for i in range(1000):
            customers.append({
                ...
                'location': np.random.choice(['domestic', 'international'], p=[0.85, 0.15]),
    ...
    def generate_deposits(self, n_records: int = 10000) -> pd.DataFrame:
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.95:  # 95% normal
                ...
                source_type = np.random.choice(['cash', 'check', 'wire', 'ach'], p=[0.3, 0.4, 0.2, 0.1])
                
            else:  # 5% anomalies
                # Anomaly types: unusually large amounts, odd timing, suspicious sources
                anomaly_type = random.choice(['large_amount', 'odd_timing', 'rapid_sequence'])
                ...
                if anomaly_type == 'large_amount':
                    ...
                    amount = np.random.uniform(9000, 9999)  # Just under reporting threshold
                    timestamp = start_date + timedelta(
                        days=random.randint(0, 365),
                        hours=random.randint(8, 18),
                        minutes=random.randint(0, 59)
            ...
            data.append({
                ...
                'teller_id': f'T_{random.randint(1, 200):04d}' if source_type != 'ach' else None,
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.92:  # 92% normal
                ...
                if loan_type == 'mortgage':
                    ...
                    credit_score = np.random.uniform(500, 600)  # Low score for high amount
                else:
            ...
            data.append({
                ...
                'customer_id': customer['customer_id'],
                'loan_type': loan_type,
                'application_date': start_date + timedelta(days=random.randint(0, 365)),
                ...
                'loan_to_value': round(np.random.uniform(0.6, 0.95), 3) if loan_type in ['mortgage', 'auto'] else None,
                'debt_to_income': round(np.random.uniform(0.1, 0.5), 3),
                'employment_years': random.randint(0, 20),
                'status': np.random.choice(['approved', 'denied', 'pending'], p=[0.7, 0.2, 0.1]),
    ...
    def generate_investments(self, n_records: int = 8000) -> pd.DataFrame:
        ...
        securities = ['AAPL', 'GOOGL', 'MSFT', 'AMZN', 'TSLA', 'BRK.A', 'JPM', 'JNJ', 'V', 'PG']
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.94:  # 94% normal
                ...
                anomaly_type = random.choice(['large_volume', 'timing_pattern', 'price_manipulation'])
            ...
            data.append({
                ...
                'account_type': np.random.choice(['taxable', 'ira', '401k'], p=[0.5, 0.3, 0.2]),
    ...
    def generate_fx_transactions(self, n_records: int = 3000) -> pd.DataFrame:
        ...
        currencies = ['EUR', 'GBP', 'JPY', 'CHF', 'CAD', 'AUD', 'CNY', 'MXN', 'BRL', 'INR']
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.90:  # 90% normal
                ...
                base_rates = {'EUR': 0.85, 'GBP': 0.75, 'JPY': 110, 'CHF': 0.92, 'CAD': 1.25, 
                             'AUD': 1.35, 'CNY': 6.5, 'MXN': 20, 'BRL': 5.2, 'INR': 75}
                ...
                anomaly_type = random.choice(['large_amount', 'rate_anomaly', 'structuring'])
                ...
                if anomaly_type == 'large_amount':
                    ...
                    exchange_rate = base_rates[to_currency] * np.random.uniform(1.1, 1.3)
                else:  # structuring
                    amount_usd = np.random.uniform(9500, 9999)  # Just under reporting threshold
                    base_rates = {'EUR': 0.85, 'GBP': 0.75, 'JPY': 110, 'CHF': 0.92, 'CAD': 1.25, 
                                 'AUD': 1.35, 'CNY': 6.5, 'MXN': 20, 'BRL': 5.2, 'INR': 75}
                    exchange_rate = base_rates[to_currency] * np.random.normal(1, 0.02)
            ...
            data.append({
                ...
                'purpose': np.random.choice(['travel', 'business', 'investment', 'family'], p=[0.4, 0.3, 0.2, 0.1]),
                'method': np.random.choice(['wire', 'cash', 'card'], p=[0.5, 0.3, 0.2]),
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.97:  # 97% normal
                ...
                if transaction_type == 'withdrawal':
                    amount = random.choice([20, 40, 60, 80, 100, 120, 140, 160, 180, 200])
                ...
                anomaly_type = random.choice(['multiple_attempts', 'unusual_amount', 'location_jump'])
                
                transaction_type = 'withdrawal'
                if anomaly_type == 'multiple_attempts':
                    amount = random.choice([20, 40, 60])
                elif anomaly_type == 'unusual_amount':
                    amount = random.choice([500, 600, 700, 800])  # Max daily limit attempts
                else:  # location_jump
            ...
            if customer['location'] == 'domestic':
                atm_location = random.choice(['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix'])
            else:
                atm_location = random.choice(['London', 'Tokyo', 'Singapore', 'Dubai', 'Sydney'])
            ...
            data.append({
                ...
                'fee': 2.50 if customer['location'] != 'domestic' or random.random() < 0.3 else 0,
                'status': np.random.choice(['success', 'declined', 'error'], p=[0.9, 0.08, 0.02]),
                'is_anomaly': i >= n_records * 0.97
            })
        
        return pd.DataFrame(data)
    
    def generate_card_transactions(self, n_records: int = 25000, card_type: str = 'debit') -> pd.DataFrame:
        ...
        merchants = [
            ...
            'Home Depot', 'CVS', 'Walgreens', 'Best Buy', 'Costco', 'Whole Foods',
            'Uber', 'Lyft', 'Netflix', 'Spotify', 'Apple', 'Google', 'Microsoft'
        ]
        
        categories = [
            'grocery', 'gas', 'restaurant', 'retail', 'entertainment', 'travel',
            'healthcare', 'utilities', 'subscription', 'other'
        ]
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.95:  # 95% normal
                ...
                anomaly_type = random.choice(['large_amount', 'velocity', 'location_anomaly', 'unusual_merchant'])
                ...
                if anomaly_type == 'large_amount':
                    ...
                    location = 'international' if customer['location'] == 'domestic' else 'domestic'
                else:  # unusual_merchant
                    amount = np.random.uniform(500, 2000)
                    merchant = random.choice(['Unknown Merchant', 'Cash Advance', 'Money Transfer'])
                    timestamp = start_date + timedelta(
                        days=random.randint(0, 365),
                        hours=random.randint(0, 23),
                        minutes=random.randint(0, 59)
            ...
            data.append({
                ...
                'status': np.random.choice(['approved', 'declined'], p=[0.95, 0.05]),
    ...
    def generate_expense_transactions(self, n_records: int = 8000) -> pd.DataFrame:
        ...
        expense_types = ['travel', 'food', 'learning', 'certification', 'business_event', 'equipment', 'software']
        
        for i in range(n_records):
            customer = random.choice([c for c in self.customers if c['customer_type'] == 'business'])
            ...
            if i < n_records * 0.93:  # 93% normal
                if expense_type == 'travel':
                    ...
                    description = random.choice(['Flight to NYC', 'Hotel - Chicago', 'Rental Car', 'Taxi'])
                elif expense_type == 'food':
                    amount = np.random.lognormal(4, 0.5)  # $50 average
                    description = random.choice(['Client Dinner', 'Team Lunch', 'Conference Meal'])
                elif expense_type == 'learning':
                    amount = np.random.lognormal(8, 0.5)  # $3000 average
                    description = random.choice(['AWS Training', 'Data Science Course', 'Leadership Program'])
                elif expense_type == 'certification':
                    amount = np.random.normal(500, 200)
                    description = random.choice(['PMP Certification', 'AWS Certification', 'CPA Exam'])
                elif expense_type == 'business_event':
                    amount = np.random.lognormal(9, 1)  # $8000 average
                    description = random.choice(['Trade Show', 'Conference', 'Client Event'])
                elif expense_type == 'equipment':
                    amount = np.random.lognormal(7.5, 0.8)  # $1800 average
                    description = random.choice(['Laptop', 'Monitor', 'Phone', 'Desk Setup'])
                else:  # software
                    amount = np.random.lognormal(6, 0.5)  # $400 average
                    description = random.choice(['Adobe License', 'Microsoft Office', 'Salesforce'])
                
            else:  # 7% anomalies
                # Anomalies: expense fraud, duplicate submissions, inflated amounts
                anomaly_type = random.choice(['inflated', 'duplicate', 'personal_expense'])
                ...
                if anomaly_type == 'inflated':
                    ...
                    description = random.choice(['Personal Shopping', 'Family Dinner', 'Personal Travel'])
            ...
            data.append({
                ...
                'submitted_date': start_date + timedelta(days=random.randint(0, 365)),
                'status': np.random.choice(['submitted', 'approved', 'reimbursed', 'rejected'], p=[0.2, 0.3, 0.4, 0.1]),
    ...
    def generate_gl_transactions(self, n_records: int = 12000) -> pd.DataFrame:
        ...
        gl_accounts = {
            'asset': ['1001-Cash', '1100-Accounts Receivable', '1200-Inventory', '1500-Equipment'],
            'liability': ['2001-Accounts Payable', '2100-Accrued Expenses', '2200-Notes Payable'],
            'equity': ['3001-Common Stock', '3100-Retained Earnings'],
            'revenue': ['4001-Sales Revenue', '4100-Interest Income', '4200-Service Revenue'],
            'expense': ['5001-Salaries', '5100-Rent', '5200-Utilities', '5300-Depreciation']
        }
        ...
        for i in range(n_records):
            ...
            if i < n_records * 0.96:  # 96% normal
                ...
                anomaly_type = random.choice(['large_amount', 'off_hours', 'manual_override'])
            ...
            data.append({
                ...
                'source_system': random.choice(['ERP', 'Manual', 'Interface', 'Automated']),
    ...
    def save_all_datasets(self, output_dir: str = 'examples/banking/datasets'):
        ...
        datasets = {
            ...
            'debit_card_transactions': self.generate_card_transactions(card_type='debit'),
            'credit_card_transactions': self.generate_card_transactions(card_type='credit'),
        ...
        for name, df in datasets.items():
            ...
            print(f"Generated {name}: {len(df)} records with {df['is_anomaly'].sum()} anomalies")
