...
class CreditCardAnomalyDetector:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        customer_stats.columns = ['customer_id', 'tx_count', 'avg_amount', 'std_amount', 
                                 'min_amount', 'max_amount', 'first_tx', 'last_tx']
        customer_stats['std_amount'] = customer_stats['std_amount'].fillna(0)
        customer_stats['spending_range'] = customer_stats['max_amount'] - customer_stats['min_amount']
        customer_stats['account_age_days'] = (customer_stats['last_tx'] - customer_stats['first_tx']).dt.days
        ...
        features_df = pd.concat([features_df, merchant_encoded, category_encoded, location_encoded], axis=1)
        
        # Deviation from normal patterns
        features_df['amount_dev_from_avg'] = abs(features_df['amount'] - features_df['avg_amount']) / (features_df['std_amount'] + 1)
        features_df['unusual_amount'] = (features_df['amount_dev_from_avg'] > 3).astype(int)
    ...
    def select_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        feature_columns = [
            'amount', 'amount_log', 'amount_zscore', 'high_amount',
            'hour', 'day_of_week', 'is_weekend', 'is_night',
            'tx_count', 'avg_amount', 'std_amount', 'spending_range',
            'tx_last_hour', 'tx_last_day', 'amount_last_hour', 'amount_last_day',
            'amount_dev_from_avg', 'unusual_amount', 'card_present_int',
            'mcc_code'
        ]
    ...
    def detect_anomalies(self, df: pd.DataFrame, contamination: float = 0.05):
        ...
        # Flag anomalies
        threshold = np.percentile(features_df['ensemble_score'], (1 - contamination) * 100)
        features_df['predicted_anomaly'] = (features_df['ensemble_score'] > threshold).astype(int)
    ...
    def analyze_anomalies(self, df: pd.DataFrame):
        ...
        if len(anomalies) > 0:
            ...
            print(f"Average amount: ${anomalies['amount'].mean():,.2f} vs ${normal['amount'].mean():,.2f} (normal)")
            print(f"Max fraudulent amount: ${anomalies['amount'].max():,.2f}")
            print(f"High-value transactions (>$1000): {anomalies['high_amount'].sum()}")
            print(f"Card-not-present transactions: {(1-anomalies['card_present_int']).sum()} ({(1-anomalies['card_present_int']).mean()*100:.1f}%)")
            print(f"Night-time transactions: {anomalies['is_night'].sum()} ({anomalies['is_night'].mean()*100:.1f}%)")
            print(f"Weekend transactions: {anomalies['is_weekend'].sum()} ({anomalies['is_weekend'].mean()*100:.1f}%)")
            
            print(f"\nLocation Patterns:")
            if 'location_international' in anomalies.columns:
                intl_count = anomalies['location_international'].sum()
                print(f"International transactions: {intl_count} ({intl_count/len(anomalies)*100:.1f}%)")
            
            print(f"\nVelocity Patterns:")
            print(f"Multiple transactions in same hour: {(anomalies['tx_last_hour'] > 0).sum()}")
            print(f"High daily transaction volume: {(anomalies['tx_last_day'] > 5).sum()}")
            print(f"High daily spending: {(anomalies['amount_last_day'] > 2000).sum()}")
            
            print(f"\nMerchant Categories (Top 5 for anomalies):")
            category_cols = [col for col in anomalies.columns if col.startswith('category_')]
            if category_cols:
                category_sums = anomalies[category_cols].sum().sort_values(ascending=False)
    ...
    def visualize_results(self, df: pd.DataFrame, output_dir: str = None):
        ...
        # Amount distribution
        axes[0, 0].hist(df[df['predicted_anomaly'] == 0]['amount'], bins=50, alpha=0.7, label='Normal', density=True)
        axes[0, 0].hist(df[df['predicted_anomaly'] == 1]['amount'], bins=50, alpha=0.7, label='Fraud', density=True)
        ...
        # Time patterns
        hour_counts = df.groupby(['hour', 'predicted_anomaly']).size().unstack(fill_value=0)
        ...
        card_present_data = df.groupby(['card_present', 'predicted_anomaly']).size().unstack(fill_value=0)
        ...
        axes[1, 0].scatter(df['tx_last_hour'], df['amount'], 
        ...
        # Amount deviation
        axes[1, 1].scatter(df['amount_dev_from_avg'], df['amount'], 
                          c=df['predicted_anomaly'], cmap='coolwarm', alpha=0.6)
        ...
        score_cols = [col for col in df.columns if col.endswith('_score') and col != 'ensemble_score']
        if len(score_cols) >= 2:
            axes[1, 2].scatter(df[score_cols[0]], df[score_cols[1]], 
                              c=df['predicted_anomaly'], cmap='coolwarm', alpha=0.6)
        ...
        if output_dir:
            ...
            plt.savefig(os.path.join(output_dir, 'credit_card_fraud_analysis.png'), dpi=300, bbox_inches='tight')
        
        plt.show()
    
    def generate_fraud_report(self, df: pd.DataFrame, anomalies: pd.DataFrame, output_dir: str = None):
        ...
        report.append(f"- Fraudulent transactions detected: {len(anomalies):,} ({len(anomalies)/len(df)*100:.1f}%)")
        report.append(f"- Total fraud amount: ${anomalies['amount'].sum():,.2f}" if len(anomalies) > 0 else "- No fraudulent transactions detected")
        report.append(f"- Average fraud amount: ${anomalies['amount'].mean():,.2f}" if len(anomalies) > 0 else "")
        ...
        if len(anomalies) > 0:
            ...
            if len(high_value) > 0:
                report.append(f"- {len(high_value)} high-value fraudulent transactions (>$1,000)")
                report.append(f"  - Total value: ${high_value['amount'].sum():,.2f}")
            
            # Card-not-present fraud
            cnp_fraud = anomalies[anomalies['card_present_int'] == 0]
            if len(cnp_fraud) > 0:
                report.append(f"- {len(cnp_fraud)} card-not-present fraudulent transactions")
            
            # Velocity-based fraud
            velocity_fraud = anomalies[anomalies['tx_last_hour'] > 0]
            if len(velocity_fraud) > 0:
                report.append(f"- {len(velocity_fraud)} accounts with multiple transactions in same hour")
            
            # Timing patterns
            night_fraud = anomalies[anomalies['is_night'] == 1]
            if len(night_fraud) > 0:
                report.append(f"- {len(night_fraud)} night-time fraudulent transactions")
            
            # Location-based fraud
            if 'location_international' in anomalies.columns:
                intl_fraud = anomalies[anomalies['location_international'] == 1]
                if len(intl_fraud) > 0:
                    report.append(f"- {len(intl_fraud)} international fraudulent transactions")
            
            report.append("\n## Highest Risk Transactions")
            top_fraud = anomalies.nlargest(10, 'ensemble_score')[
                ['transaction_id', 'customer_id', 'amount', 'merchant', 'timestamp', 'ensemble_score']
            ]
            report.append(top_fraud.to_string(index=False))
            
            report.append("\n## Recommended Actions")
            report.append("1. Immediately flag transactions with ensemble score > 0.8 for manual review")
            ...
            report.append("5. Set up alerts for transactions deviating significantly from customer patterns")
        
        report_text = '\n'.join(report)
        
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)
            with open(os.path.join(output_dir, 'credit_card_fraud_report.md'), 'w') as f:
                f.write(report_text)
