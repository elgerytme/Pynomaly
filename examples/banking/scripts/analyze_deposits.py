...
class DepositAnomalyDetector:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        customer_stats = features_df.groupby('customer_id')['amount'].agg(['count', 'mean', 'std', 'min', 'max']).reset_index()
        customer_stats.columns = ['customer_id', 'deposit_count', 'avg_deposit', 'std_deposit', 'min_deposit', 'max_deposit']
        ...
        features_df['near_threshold'] = ((features_df['amount'] >= 9000) & (features_df['amount'] < 10000)).astype(int)
        
        # Source type encoding
        source_type_encoded = pd.get_dummies(features_df['source_type'], prefix='source')
    ...
    def select_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        feature_columns = [
            ...
            'deposit_count', 'avg_deposit', 'std_deposit',
            'deposits_last_hour', 'deposits_last_day',
            'near_threshold'
        ]
    ...
    def detect_anomalies(self, df: pd.DataFrame, contamination: float = 0.05):
        ...
        # Isolation Forest
        iso_adapter = SklearnAdapter("IsolationForest", contamination_rate=contamination_rate)
        iso_result = iso_adapter.detect_anomalies(dataset)
        results['isolation_forest'] = iso_result.anomaly_scores
        
        # One-Class SVM
        svm_adapter = SklearnAdapter("OneClassSVM", contamination_rate=contamination_rate)
        svm_result = svm_adapter.detect_anomalies(dataset)
        results['one_class_svm'] = svm_result.anomaly_scores
        
        # Local Outlier Factor
        lof_adapter = SklearnAdapter("LocalOutlierFactor", contamination_rate=contamination_rate)
    ...
    def analyze_anomalies(self, df: pd.DataFrame):
        ...
        if len(anomalies) > 0:
            ...
            print(f"Weekend deposits: {anomalies['is_weekend'].sum()} ({anomalies['is_weekend'].mean()*100:.1f}%)")
            ...
            print(f"\nVelocity Patterns:")
            print(f"Multiple deposits in same hour: {(anomalies['deposits_last_hour'] > 0).sum()}")
            print(f"Multiple deposits in same day: {(anomalies['deposits_last_day'] > 3).sum()}")
    ...
    def visualize_results(self, df: pd.DataFrame, output_dir: str = None):
        ...
        axes[0, 0].hist(df[df['predicted_anomaly'] == 1]['amount'], bins=50, alpha=0.7, label='Anomaly', density=True)
        ...
        if source_cols:
            source_data = df[source_cols + ['predicted_anomaly']].groupby('predicted_anomaly').sum()
        ...
        # Anomaly scores
        axes[1, 0].scatter(df['iso_score'], df['svm_score'], 
        ...
        threshold_data = df.groupby(['near_threshold', 'predicted_anomaly']).size().unstack(fill_value=0)
        ...
        # Velocity analysis
        axes[1, 2].scatter(df['deposits_last_hour'], df['deposits_last_day'], 
                          c=df['predicted_anomaly'], cmap='coolwarm', alpha=0.6)
        ...
        if output_dir:
            ...
            plt.savefig(os.path.join(output_dir, 'deposit_anomaly_analysis.png'), dpi=300, bbox_inches='tight')
    ...
    def generate_report(self, df: pd.DataFrame, anomalies: pd.DataFrame, output_dir: str = None):
        ...
        report.append(f"- Anomalies detected: {len(anomalies):,} ({len(anomalies)/len(df)*100:.1f}%)")
        report.append(f"- Highest risk transaction: ${anomalies['amount'].max():,.2f}" if len(anomalies) > 0 else "- No high-risk transactions detected")
        ...
        if len(anomalies) > 0:
            ...
            if len(high_velocity) > 0:
                report.append(f"- {len(high_velocity)} accounts with multiple deposits in same hour")
            
            report.append("\n## Top 10 Highest Risk Transactions")
            top_anomalies = anomalies.nlargest(10, 'ensemble_score')[
                ['transaction_id', 'customer_id', 'amount', 'source_type', 'timestamp', 'ensemble_score']
            ]
