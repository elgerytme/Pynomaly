...
class SimpleExplainer:
    ...
    def calculate_feature_importance(self, dataset):
        ...
        self.feature_names = dataset.feature_names or [f"feature_{i}" for i in range(dataset.n_features)]
        ...
        for i, feature_name in enumerate(self.feature_names):
            ...
            permuted_dataset = Dataset(
                name=f"{dataset.name}_permuted_{feature_name}",
                data=permuted_data
            )
        ...
        if total_importance > 0:
            self.feature_importance = [score / total_importance for score in importance_scores]
        else:
            self.feature_importance = [1.0 / len(importance_scores)] * len(importance_scores)
    ...
    def explain_prediction(self, dataset, sample_idx=None):
        ...
        for idx in top_indices:
            ...
            if self.feature_importance is not None:
                for i, (feature_name, importance) in enumerate(zip(self.feature_names, self.feature_importance)):
                    value = sample.iloc[i]
...
def create_explainable_dataset():
    ...
    feature1_anom = np.random.choice([-4, 4], n_anomalies) + np.random.normal(0, 0.5, n_anomalies)
    ...
    feature_data = df[['primary_feature', 'correlated_feature', 'noise_feature', 'constant_feature']]
...
def run_explainable_ai_example():
    ...
    print(f"📊 Created dataset: {len(dataset.data)} samples, {len(dataset.feature_names)} features")
    ...
    for name, importance in zip(dataset.feature_names, importance_scores):
        print(f"   {name:20}: {importance:.3f} {'🔥' if importance > 0.3 else '🟡' if importance > 0.1 else '🔵'}")
    ...
    for i, (feature, importance) in enumerate(global_explanation['top_features'][:3], 1):
