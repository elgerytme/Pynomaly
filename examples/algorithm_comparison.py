...
def create_sample_data(n_samples: int = 500, n_features: int = 10, anomaly_fraction: float = 0.1):
...
async def evaluate_algorithm(container, dataset: Dataset, algorithm: str, parameters: Dict[str, Any], true_labels: np.ndarray):
    """Evaluate a single algorithm and return metrics."""
    # Create detector
    detector = Detector(
        name=f"{algorithm} Detector",
        algorithm=algorithm,
    ...
    # Detect anomalies
    start_time = time.time()
    result = await detection_service.detect_anomalies(detector_id=detector.id, dataset=dataset)
...
async def main():
    ...
    data, true_labels = create_sample_data(n_samples=500, n_features=10, anomaly_fraction=0.1)
    ...
    algorithms = [
        ...
        {
            'name': 'KNN',
            'params': {'contamination': 0.1, 'n_neighbors': 5}
        }
    ]
    ...
    print("\n3. Results Summary:")
    print("-" * 100)
    print(f"{'Algorithm':<20} {'Precision':<10} {'Recall':<10} {'F1-Score':<10} {'Accuracy':<10} {'Time (s)':<10}")
    print("-" * 100)
    ...
    for r in results:
        ...
        print(f"{r['algorithm']:<20} {r['precision']:<10.3f} {r['recall']:<10.3f} "
