...
def create_sample_datasets():
    ...
    ecommerce_data = {
        ...
        'customer_rating': np.random.choice([1, 2, 3, 4, 5, np.nan], n_samples, p=[0.05, 0.1, 0.15, 0.3, 0.35, 0.05]),
        ...
        'mostly_zeros': np.random.choice([0, 1, 2], n_samples, p=[0.95, 0.03, 0.02])  # Low variance
    }
    ...
    iot_data = {
        'sensor_id': np.random.choice(['TEMP_01', 'TEMP_02', 'HUMID_01', 'HUMID_02', 'PRESS_01'], n_samples),
        ...
        'status_code': np.random.choice(['OK', 'WARNING', 'ERROR', 'OFFLINE'], n_samples, p=[0.8, 0.1, 0.05, 0.05])
    }
    
    # Add seasonal patterns and anomalies
    iot_data['temperature'] += 10 * np.sin(np.arange(n_samples) * 2 * np.pi / 24)  # Daily cycle
...
def demonstrate_data_quality_analysis():
    ...
    if success:
        ...
        run_command(
            "pynomaly dataset list --limit 1",
            "Getting dataset ID for quality analysis"
        )
...
def demonstrate_data_cleaning():
    ...
    run_command(
        f"pynomaly data clean ecommerce_cleaned_missing --outliers clip --outlier-threshold 3.0 --save-as ecommerce_cleaned_outliers",
        "Remove outliers using 3-sigma clipping"
    )
    
    print("🔄 Removing duplicates...")
    run_command(
        f"pynomaly data clean ecommerce_cleaned_outliers --duplicates --save-as ecommerce_cleaned_final",
...
def demonstrate_data_transformation():
    ...
    run_command(
        f"pynomaly data transform ecommerce_scaled --encoding onehot --save-as ecommerce_encoded",
    ...
    run_command(
        f"pynomaly data transform ecommerce_encoded --feature-selection variance_threshold --save-as ecommerce_selected",
    ...
    run_command(
        f"pynomaly data transform ecommerce_scaled --polynomial 2 --save-as ecommerce_poly",
...
def demonstrate_pipeline_management():
    ...
    run_command(
        ...
        "List all available preprocessing pipelines"
    )
...
def demonstrate_anomaly_detection_workflow():
    ...
    run_command(
        f"pynomaly data transform iot_cleaned --scaling robust --save-as iot_preprocessed",
...
def main():
    ...
    try:
        ...
        if result.returncode != 0:
            print("❌ Pynomaly CLI not available. Please install and set up the CLI first.")
            return
