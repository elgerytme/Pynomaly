...
class ComplexityThresholdChecker:
    ...
    def check_metric(self, metric_name: str, current_value: float, 
                    baseline_value: float = None) -> Tuple[bool, str]:
        ...
        return True, f"{metric_name}: {current_value:.2f} within threshold {threshold:.2f}"
    
    def check_all_metrics(self, current_metrics: Dict, baseline_metrics: Dict = None) -> bool:
        ...
        for metric_name, threshold in self.thresholds.items():
            ...
            passed, message = self.check_metric(metric_name, current_value, baseline_value)
            if not passed:
                all_passed = False
...
def check_trend_regressions(current_data: Dict, baseline_data: Dict = None) -> List[Dict]:
...
def main():
    ...
    parser.add_argument('--config', '-c', type=Path,
                       help='Threshold configuration JSON file')
    ...
    parser.add_argument('--max-total-lines', type=int, default=100000,
                       help='Maximum total lines')
    parser.add_argument('--max-avg-complexity', type=float, default=5.0,
                       help='Maximum average complexity')
    parser.add_argument('--min-maintainability', type=float, default=60.0,
                       help='Minimum maintainability index')
    parser.add_argument('--max-dependencies', type=int, default=100,
    ...
    try:
        ...
        overall_passed = (
            passed and 
            (not args.fail_on_regression or not has_critical_regressions) and
            (not args.fail_on_warnings or not has_quality_failures)
        )
        ...
        if args.output_format == 'json':
            ...
            if regressions:
                ...
                for regression in regressions:
                    ...
                    print(f"  {icon} {regression['metric']}: {regression['change_percent']:+.1f}% "
