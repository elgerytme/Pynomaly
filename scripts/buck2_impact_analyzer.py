...
class Buck2ImpactAnalyzer:
                ...
                for line in import_lines:
                    if 'pynomaly' in line:
                        ...
                        if line.startswith('from pynomaly'):
                            ...
                            module = line.split()[1].split('.')[1] if '.' in line else 'core'
                            dependencies.add(f"pynomaly.{module}")
        except Exception as e:
            ...
            for category, patterns in self.component_categories.items():
                if any(file_path.startswith(pattern) for pattern in patterns):
                    if category == "critical":
                        ...
                        component_risk = 0.6
                        risk_reasons.append(f"Important component modified: {file_path}")
    ...
    def recommend_test_strategy(self, change_analysis: ChangeAnalysis, risk: ImpactRisk) -> TestStrategy:
        ...
        if risk.level == "critical":
            priority = "full"
            test_targets = {"test-all", "benchmarks", "property-tests", "security-tests", "mutation-tests"}
