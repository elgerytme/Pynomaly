...
class PynomAlyDeploymentManager:
    ...
    def __init__(self, project_root: Path, environment: DeploymentEnvironment, platform: DeploymentPlatform):
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "replicas": 1 if self.environment == DeploymentEnvironment.DEVELOPMENT else 3,
            "resources": {
    ...
    def _initialize_clients(self):
        ...
        try:
            ...
            # Kubernetes client
            if self.platform in [DeploymentPlatform.KUBERNETES, DeploymentPlatform.HELM]:
                try:
    ...
    def _log_deployment_step(self, step: str, status: str, details: Optional[str] = None):
        """Log deployment step."""
        log_entry = {
    ...
    async def deploy(self, strategy: Optional[DeploymentStrategy] = None) -> bool:
        """Execute deployment with specified strategy."""
        self.start_time = datetime.utcnow()
        self.deployment_status = DeploymentStatus.IN_PROGRESS
        
        try:
            self._log_deployment_step("DEPLOYMENT_START", "INFO", f"Starting {self.environment.value} deployment")
            ...
            deployment_strategy = strategy or DeploymentStrategy(self.deployment_config.get("strategy", "rolling_update"))
            ...
            self._log_deployment_step("DEPLOYMENT_COMPLETE", "SUCCESS", 
    ...
    async def _pre_deployment_validation(self) -> bool:
        ...
        self._log_deployment_step("PRE_VALIDATION", "INFO", "Starting pre-deployment validation")
        ...
        try:
            ...
            for file_path in required_files:
                if not (self.project_root / file_path).exists():
                    self._log_deployment_step("PRE_VALIDATION", "ERROR", f"Missing required file: {file_path}")
                    return False
            ...
            if self.environment == DeploymentEnvironment.PRODUCTION:
                ...
                for env_var in required_env_vars:
                    if not os.getenv(env_var):
                        self._log_deployment_step("PRE_VALIDATION", "ERROR", f"Missing environment variable: {env_var}")
                        return False
            
            # Check platform connectivity
            if self.platform == DeploymentPlatform.DOCKER_COMPOSE:
                if not self.docker_client.ping():
                    self._log_deployment_step("PRE_VALIDATION", "ERROR", "Docker daemon not accessible")
                    return False
            
            elif self.platform in [DeploymentPlatform.KUBERNETES, DeploymentPlatform.HELM]:
                try:
                    v1 = client.CoreV1Api(self.k8s_client)
                    v1.list_namespace()
                except ApiException as e:
                    self._log_deployment_step("PRE_VALIDATION", "ERROR", f"Kubernetes API not accessible: {e}")
                    return False
            
            self._log_deployment_step("PRE_VALIDATION", "SUCCESS", "Pre-deployment validation passed")
            return True
    ...
    async def _create_backup(self) -> bool:
        ...
        try:
            ...
            if self.platform == DeploymentPlatform.DOCKER_COMPOSE:
                backup_cmd = [
                    "docker", "exec", "pynomaly-postgres",
                    "pg_dump", "-U", "pynomaly", "-d", "pynomaly",
                    "-f", f"/backup/db_backup_{self.deployment_id}.sql"
                ]
            else:
                # Kubernetes backup
                backup_cmd = [
                    "kubectl", "exec", "-n", f"pynomaly-{self.environment.value}",
                    "deployment/postgres", "--",
                    "pg_dump", "-U", "pynomaly", "-d", "pynomaly",
                    "-f", f"/backup/db_backup_{self.deployment_id}.sql"
                ]
            ...
            if result.returncode != 0:
                self._log_deployment_step("BACKUP", "ERROR", f"Database backup failed: {result.stderr}")
                return False
            ...
            self._log_deployment_step("BACKUP", "SUCCESS", f"Backup created at {backup_dir}")
            return True
    ...
    async def _build_and_push_images(self) -> bool:
        ...
        try:
            ...
            build_args = {
                ...
                "VCS_REF": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
            }
            
            dockerfile_path = self.project_root / "deploy" / "docker" / "Dockerfile.production"
            ...
            if registry and self.environment != DeploymentEnvironment.DEVELOPMENT:
                ...
                self._log_deployment_step("BUILD", "INFO", f"Pushed image to registry: {registry_tag}")
    ...
    async def _deploy_docker_compose(self) -> bool:
        ...
        self._log_deployment_step("DEPLOY_COMPOSE", "INFO", "Deploying with Docker Compose")
        
        try:
            compose_file = self.project_root / "deploy" / "docker" / f"docker-compose.{self.environment.value}.yml"
            
            if not compose_file.exists():
                compose_file = self.project_root / "deploy" / "docker" / "docker-compose.production.yml"
            ...
            if result.returncode != 0:
                self._log_deployment_step("DEPLOY_COMPOSE", "ERROR", f"Docker Compose deployment failed: {result.stderr}")
                return False
            
            self._log_deployment_step("DEPLOY_COMPOSE", "SUCCESS", "Docker Compose deployment completed")
            return True
    ...
    async def _deploy_kubernetes(self, strategy: DeploymentStrategy) -> bool:
        ...
        self._log_deployment_step("DEPLOY_K8S", "INFO", f"Deploying to Kubernetes with {strategy.value} strategy")
    ...
    async def _deploy_helm(self, strategy: DeploymentStrategy) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                self._log_deployment_step("DEPLOY_HELM", "ERROR", f"Helm deployment failed: {result.stderr}")
                return False
            
            self._log_deployment_step("DEPLOY_HELM", "SUCCESS", "Helm deployment completed")
            return True
    ...
    async def _post_deployment_validation(self) -> bool:
        ...
        self._log_deployment_step("POST_VALIDATION", "INFO", "Starting post-deployment validation")
        ...
        try:
            ...
            for attempt in range(retries):
                if await self._check_service_health():
                    self._log_deployment_step("POST_VALIDATION", "SUCCESS", "All services are healthy")
                    return True
                
                if attempt < retries - 1:
                    self._log_deployment_step("POST_VALIDATION", "RETRY", f"Health check failed, retrying ({attempt + 1}/{retries})")
                    await asyncio.sleep(30)
            
            self._log_deployment_step("POST_VALIDATION", "ERROR", "Health checks failed after all retries")
            return False
    ...
    async def _check_docker_health(self) -> bool:
        ...
        try:
            containers = self.docker_client.containers.list(filters={"label": "app=pynomaly"})
            ...
            for container in containers:
                ...
                if hasattr(container.attrs, "State") and "Health" in container.attrs["State"]:
                    ...
                    if health != "healthy":
                        logger.error(f"Container {container.name} is not healthy: {health}")
                        return False
        ...
        try:
            ...
            for deployment in deployments.items:
                if deployment.status.ready_replicas != deployment.status.replicas:
                    logger.error(f"Deployment {deployment.metadata.name} not ready: {deployment.status.ready_replicas}/{deployment.status.replicas}")
                    return False
            
            # Check pods
            pods = v1.list_namespaced_pod(namespace, label_selector="app.kubernetes.io/name=pynomaly")
            
            for pod in pods.items:
                if pod.status.phase != "Running":
                    logger.error(f"Pod {pod.metadata.name} not running: {pod.status.phase}")
                    return False
    ...
    async def _ensure_namespace(self, namespace: str):
        ...
        try:
            ...
            if e.status == 404:
                ...
                self._log_deployment_step("NAMESPACE", "INFO", f"Created namespace: {namespace}")
    ...
    async def _setup_monitoring(self):
        ...
        self._log_deployment_step("MONITORING", "INFO", "Setting up monitoring and alerting")
        
        try:
            # Implementation would depend on monitoring stack
            # This is a placeholder for monitoring setup
            await asyncio.sleep(1)
            self._log_deployment_step("MONITORING", "SUCCESS", "Monitoring setup completed")
...
@click.option("--platform", "-p", type=click.Choice(["docker_compose", "kubernetes", "helm"]), 
              default="docker_compose", help="Deployment platform")
@click.option("--strategy", "-s", type=click.Choice(["rolling_update", "blue_green", "canary", "recreate"]), 
              help="Deployment strategy")
@click.option("--project-root", default=".", help="Project root directory")
def deploy(environment: str, platform: str, strategy: Optional[str], project_root: str):
...
@cli.command()
@click.option("--environment", "-e", type=click.Choice(["development", "staging", "production"]), 
              required=True, help="Environment to check")
@click.option("--platform", "-p", type=click.Choice(["docker_compose", "kubernetes", "helm"]), 
              default="docker_compose", help="Deployment platform")
@click.option("--project-root", default=".", help="Project root directory")
