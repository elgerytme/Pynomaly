...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("env_matrix_test.log"), logging.StreamHandler()],
)
...
class EnvMatrixTester:
    ...
    def _create_venv(
        self, python_version: str, extra: str, temp_dir: Path
    ) -> Tuple[bool, str]:
    ...
    def _install_package_with_retry(
        self, pip_path: str, packages: List[str], max_retries: int = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"Attempt {attempt + 1}/{max_retries}: Installing {packages}"
                )
                ...
                logger.error(
                    f"Attempt {attempt + 1} timed out after {self.chunk_timeout} seconds"
                )
                if attempt == max_retries - 1:
    ...
    def _install_chunked_all(
        self, pip_path: str, project_root: str
    ) -> Tuple[bool, str, str, int, float]:
    ...
    def _install_package(self, python_version: str, extra: str, venv_path: str) -> Dict:
        ...
        if extra == "all":
            ...
            success, stdout, stderr, returncode, duration = (
                self._install_package_with_retry(pip_path, [install_cmd])
            )
    ...
    def _get_dependency_info(self, venv_path: str) -> Dict:
        ...
        dependency_info = {"tree_text": "", "tree_json": {}, "freeze_output": ""}

        # Get pip freeze output
        try:
            result = subprocess.run(
                [pip_path, "freeze"], capture_output=True, text=True, timeout=60
            )
            if result.returncode == 0:
    ...
    def _get_lockfile_info(self, venv_path: str) -> Dict:
        ...
        lockfile_info = {"pip_freeze": "", "pip_list": [], "pip_show_pynomaly": ""}
    ...
    def test_environment(self, python_version: str, extra: str) -> Dict:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create virtual environment
            venv_success, venv_path = self._create_venv(
                python_version, extra, temp_path
            )
            ...
            result = {
                ...
                "venv_creation": {"success": venv_success},
                "installation": {},
                "dependency_tree": {},
                "lockfile": {},
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Test completed. {summary['successful_tests']}/{summary['total_tests']} tests passed"
        )
