...
class WAFManager:
    ...
    async def block_ip(self, ip: str, reason: str = "Manual block", duration: int = 3600) -> bool:
        """Block an IP address."""
        try:
    ...
    def add_signature(self, signature: ThreatSignature) -> bool:
        ...
        try:
            ...
            console.print(f"✅ Signature '{signature.name}' added successfully", style="green")
            return True
    ...
    def update_signature(self, name: str, enabled: bool) -> bool:
        ...
        try:
            if self.signatures_path.exists():
                ...
                console.print(f"✅ Signature '{name}' updated successfully", style="green")
                return True
    ...
    def generate_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_attacks_in_window = [
            attack for attack in recent_attacks
        ...
        return {
            ...
            "threat_level_distribution": self._get_threat_distribution(recent_attacks_in_window),
...
def print_signatures_table(signatures: List[ThreatSignature]):
    ...
    for sig in signatures:
        ...
        table.add_row(sig.name, sig.attack_type.value, sig.threat_level.value, enabled, custom)
...
async def main():
    ...
    list_blocked_parser = ip_subparsers.add_parser("list-blocked", help="List blocked IPs")
    list_suspicious_parser = ip_subparsers.add_parser("list-suspicious", help="List suspicious IPs")
    ...
    block_parser.add_argument("--duration", type=int, default=3600, help="Block duration in seconds")
    ...
    add_sig_parser.add_argument("attack_type", choices=[t.value for t in AttackType], help="Attack type")
    add_sig_parser.add_argument("threat_level", choices=[t.value for t in ThreatLevel], help="Threat level")
    ...
    list_attacks_parser = attacks_subparsers.add_parser("list", help="List recent attacks")
    list_attacks_parser.add_argument("--limit", type=int, default=100, help="Maximum number of attacks")
    
    # Report command
    report_parser = subparsers.add_parser("report", help="Generate security report")
    report_parser.add_argument("--hours", type=int, default=24, help="Report time window in hours")
