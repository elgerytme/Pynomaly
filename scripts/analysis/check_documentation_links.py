...
class DocumentationLinkChecker:
    ...
    def __init__(self, project_root: Path):
        ...
        self.code_ref_pattern = re.compile(
            r'`([^`]+\.[a-zA-Z0-9]+)`',
            re.MULTILINE
        )
        ...
        for file_path in doc_files:
            try:
                ...
                if file_link_count > 0:
                    print(f"Analyzed {file_path.relative_to(self.project_root)}: {file_link_count} links, {len(file_broken_links)} broken")
    ...
    def _get_documentation_files(self) -> List[Path]:
        ...
        dirs_to_check = [
            ...
            self.examples_dir,
            self.scripts_dir / "documentation" if (self.scripts_dir / "documentation").exists() else None
        ]
    ...
    def _check_markdown_links(self, file_path: Path, content: str) -> Tuple[List[BrokenLink], int]:
    ...
    def _check_file_references(self, file_path: Path, content: str) -> Tuple[List[BrokenLink], int]:
    ...
    def _check_code_references(self, file_path: Path, content: str) -> Tuple[List[BrokenLink], int]:
    ...
    def _looks_like_file_path(self, path: str) -> bool:
        ...
        # Has file extension
        if '.' in path and path.split('.')[-1] in ['md', 'py', 'yml', 'yaml', 'json', 'txt', 'html', 'rst']:
            return True
    ...
    def _generate_recommendations(self, broken_links: List[BrokenLink], critical_missing: List[str]) -> List[str]:
        """Generate recommendations for fixing issues"""
        recommendations = []
        
        if critical_missing:
            recommendations.append(f"Create missing critical files: {', '.join(critical_missing)}")
        ...
        for issue_type, links in by_type.items():
            recommendations.append(f"Fix {len(links)} broken {issue_type.replace('_', ' ')} links")
    ...
    if result.broken_links:
        ...
        for source_file, links in by_file.items():
            ...
            for link in links:
                ...
                print(f"   Context: {link.context.split(chr(10))[1] if chr(10) in link.context else link.context}")
