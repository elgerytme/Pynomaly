...
async def test_streaming_integration():
    ...
    try:
        ...
        print(f"   Distributed processing: {streaming_use_case.enable_distributed_processing}")
        ...
        if response.success:
            ...
            realtime_response = await streaming_use_case.start_streaming(realtime_request)
            if realtime_response.success:
                ...
                for i in range(3):
                    sample = StreamingSample(data=np.random.randn(4))
                    await streaming_use_case.add_sample(realtime_response.stream_id, sample)
                
                await asyncio.sleep(0.2)
                
                realtime_results = await streaming_use_case.get_results(realtime_response.stream_id)
        ...
        print(f"   Batch size range: {api_request.configuration.min_batch_size}-{api_request.configuration.max_batch_size}")
        
        # Test sample DTO with different data formats
        array_sample = StreamingSampleDTO(
            data=[1.0, 2.0, 3.0],
            metadata={"type": "array_format"}
        )
