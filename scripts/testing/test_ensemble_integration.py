...
async def test_ensemble_integration():
    ...
    try:
        ...
        print(f"✅ Valid request DTO created: {len(valid_request.detector_ids)} detectors")
        
        # Test validation errors
        try:
            invalid_request = EnsembleDetectionRequestDTO(
                detector_ids=["only_one"],  # Too few detectors
                data=[[1.0, 2.0]]
            )
        ...
        print(f"   Performance tracking: {ensemble_use_case.enable_performance_tracking}")
        ...
        detector_repo.get = AsyncMock(side_effect=lambda detector_id: 
            next((d for d in mock_detectors if d.id == detector_id), None)
        )
        ...
        if response.success:
            ...
            if response.explanations:
                ...
                print(f"     Sample explanation keys: {list(explanation.keys())[:4]}...")
        ...
        print(f"   Target strategies: {[s.value for s in opt_request.target_voting_strategies]}")
        ...
        if opt_response.success:
            ...
            print(f"   Optimal strategy: {opt_response.optimal_voting_strategy.value if opt_response.optimal_voting_strategy else 'None'}")
        ...
        for detector_id, metrics in list(ensemble_use_case._performance_tracker.items())[:2]:
            print(f"  {detector_id}: F1={metrics.f1_score:.3f}, Stability={metrics.stability_score:.3f}")
