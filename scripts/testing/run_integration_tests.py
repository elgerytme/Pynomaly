...
class IntegrationTestRunner:
    ...
    def run_test_suite(
        ...
        if config.get("junit_report", True):
            junit_file = (
                self.reports_dir
                / f"junit_{test_pattern.replace('*', 'all').replace('/', '_')}.xml"
            )
...
def main():
    ...
    parser.add_argument(
        "--auth", action="store_true", help="Enable authentication for tests"
    )
    parser.add_argument(
        "--fail-fast", action="store_true", help="Stop on first test suite failure"
    )
    parser.add_argument(
        "--max-failures", type=int, default=5, help="Maximum failures per test suite"
    )
    parser.add_argument(
        "--timeout", type=int, default=1800, help="Timeout per test suite in seconds"
    )
