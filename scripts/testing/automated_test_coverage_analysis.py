...
class TestCoverageAnalyzer:
    ...
    def _calculate_coverage_ratios(self, source_files: Dict, test_files: Dict) -> Dict[str, float]:
        """Calculate coverage ratios."""
        ratios = {}
        
        # Overall ratio
        ratios["overall"] = (test_files["total"] / source_files["total"]) * 100 if source_files["total"] > 0 else 0
        ...
        for layer in ["domain", "application", "infrastructure", "presentation"]:
            ...
            ratios[f"layer_{layer}"] = (test_count / source_count) * 100 if source_count > 0 else 0
        ...
        for area in ["core", "sdk", "cli", "web_api", "web_ui"]:
            ...
            ratios[f"area_{area}"] = (test_count / source_count) * 100 if source_count > 0 else 0
    ...
    def identify_gaps(self, structure: Dict[str, Any], coverage_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for area, config in areas.items():
            ...
            if current_coverage < target_coverage:
                gap = {
                    ...
                    "recommendations": self._get_area_recommendations(area, current_coverage, target_coverage)
                }
                gaps.append(gap)
        ...
        for layer, config in layers.items():
            ...
            if current_coverage < target_coverage:
                gap = {
                    ...
                    "priority": config["priority"],
                    "description": config["description"],
                    "recommendations": self._get_layer_recommendations(layer, current_coverage, target_coverage)
                }
                gaps.append(gap)
        ...
        if test_types.get("unit", 0) == 0:
            gaps.append({
                ...
                "recommendations": ["Create tests/system/ directory", "Add end-to-end system tests", "Implement deployment validation tests"]
            })

        return sorted(gaps, key=lambda x: {"critical": 0, "high": 1, "medium": 2, "low": 3}[x["priority"]])

    def _get_area_recommendations(self, area: str, current: float, target: float) -> List[str]:
        ...
        return recommendations.get(area, ["Add comprehensive testing", "Improve test coverage"])

    def _get_layer_recommendations(self, layer: str, current: float, target: float) -> List[str]:
        ...
        return recommendations.get(layer, ["Add comprehensive testing", "Improve test coverage"])

    def generate_reports(self, structure: Dict[str, Any], coverage_results: Dict[str, Any], gaps: List[Dict[str, Any]]) -> None:
    ...
    def _generate_prioritized_recommendations(self, gaps: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    def _generate_markdown_report(self, summary: Dict[str, Any]) -> None:
        """Generate markdown report."""
        report_file = self.reports_dir / f"automated_test_coverage_report_{self.timestamp}.md"
        
        with open(report_file, 'w') as f:
            f.write(f"# Automated Test Coverage Report\n\n")
            ...
            f.write(f"- **Overall Coverage Ratio**: {structure['coverage_ratios']['overall']:.1f}%\n\n")
            ...
            for area in ["core", "sdk", "cli", "web_api", "web_ui"]:
                ...
                status = "✅" if coverage >= target else "⚠️" if coverage >= target * 0.7 else "❌"
                f.write(f"| {area.title()} | {coverage:.1f}% | {target}% | {status} |\n")
            f.write("\n")
            ...
            for layer in ["domain", "application", "infrastructure", "presentation"]:
                ...
                status = "✅" if coverage >= target else "⚠️" if coverage >= target * 0.7 else "❌"
                f.write(f"| {layer.title()} | {coverage:.1f}% | {target}% | {status} |\n")
            f.write("\n")
    ...
    def run_full_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage_results": coverage_results,
            "gaps": gaps,
            "critical_gaps_count": len([g for g in gaps if g["priority"] == "critical"]),
...
@click.option("--output-format", default="both", type=click.Choice(["json", "markdown", "both"]), help="Output format")
...
def main(project_root: str, output_format: str, run_tests: bool):
    ...
    # Print summary
    click.echo("\n📋 Analysis Summary:")
    click.echo(f"├── Total Source Files: {results['structure']['source_files']['total']}")
    click.echo(f"├── Total Test Files: {results['structure']['test_files']['total']}")
    click.echo(f"├── Overall Coverage: {results['structure']['coverage_ratios']['overall']:.1f}%")
