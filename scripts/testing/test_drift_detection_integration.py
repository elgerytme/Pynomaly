...
async def test_drift_detection_integration():
    ...
    try:
        ...
        from pynomaly.domain.entities.drift_detection import (
            ModelMonitoringConfig, DriftDetectionMethod, DriftSeverity, DriftType
        )
        ...
        print(f"   Alert severities: {[s.value for s in monitoring_config.alert_on_severity]}")
        ...
        drift_use_case = DriftMonitoringUseCase(
        ...
        print(f"   Report period: {drift_report.report_period_start.strftime('%Y-%m-%d')} to {drift_report.report_period_end.strftime('%Y-%m-%d')}")
        ...
        print(f"   Average health score: {system_health.get('average_health_score', 0):.3f}")
        print(f"   Recent drift detections: {system_health.get('recent_drift_detections', 0)}")
