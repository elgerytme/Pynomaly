...
class MultiVersionTester:
    ...
    def __init__(self, base_dir: Path = None):
        ...
        self.test_configs = {
            ...
            "coverage": {
                "command": ["python", "-m", "pytest", "tests/", "--cov=pynomaly", "--cov-report=xml", "--cov-report=term"],
                "timeout": 600,
...
"""
    ...
    def run_test_for_version(self, version: str, python_exe: Path, 
                           test_config: str = "basic") -> TestResult:
        """Run tests for a specific Python version."""
        logger.info(f"Running {test_config} tests for Python {version}...")
        start_time = time.time()
        
        config = self.test_configs[test_config]
        command = [str(python_exe)] + config["command"][1:]  # Replace 'python' with actual executable
    ...
    def run_all_tests(self, test_config: str = "basic", 
                     parallel: bool = True, max_workers: int = 3) -> MultiVersionTestSummary:
        ...
        logger.info(f"Found {len(environments)} Python environments: {list(environments.keys())}")
        ...
        if parallel and len(environments) > 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(self.run_test_for_version, version, python_exe, test_config): version
    ...
    def _generate_recommendations(self, results: List[TestResult]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("All Python versions are working well! Consider adding more comprehensive tests.")
    ...
    def print_summary(self, summary: MultiVersionTestSummary):
        ...
        for result in summary.results:
            ...
            print(f"Python {result.version:<8} {status:<8} "
        ...
        for version, info in summary.compatibility_matrix.items():
            ...
            print(f"  {status_symbol} Python {version_display:<8} {status:<12} "
...
def main():
    ...
    parser.add_argument("--config", choices=["basic", "coverage", "compatibility", "performance"],
                       default="basic", help="Test configuration to use")
    ...
    try:
        ...
        if args.output:
            ...
            output_file = Path("reports") / f"multi_python_test_{args.config}_{timestamp}.json"
