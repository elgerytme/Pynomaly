...
class StashAnalyzer:
    ...
    def analyze_stash_content(self, stash_ref):
        ...
        try:
            ...
            if relevant_files:
                return True, f"Contains {len(relevant_files)} relevant files", relevant_files
            else:
    ...
    def analyze_stash(self, stash_info):
        ...
        # Analyze content
        is_relevant, analysis_note, files_changed = self.analyze_stash_content(stash_ref)
        ...
        if is_relevant:
            ...
            print(f"  Files: {', '.join(files_changed[:5])}{'...' if len(files_changed) > 5 else ''}")
            
            # Mark as candidate for manual review
            self.update_audit_file(index, hash_val, "CANDIDATE", 
                                 f"Relevant changes found. {analysis_note}. Summary: {summary}", 
                                 files_changed)
        else:
            print(f"⚠ {stash_ref} skipped: {analysis_note}")
            self.update_audit_file(index, hash_val, "SKIPPED", analysis_note, files_changed)
