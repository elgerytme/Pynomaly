...
def categorize_file(filename: str) -> str:
    ...
    if filename.endswith(('.py', '.sh', '.ps1', '.bat')) and any(
        filename.startswith(prefix) for prefix in ['fix_', 'setup_', 'install_', 'run_', 'deploy_']
    ):
        return 'scripts'
    
    # Check for documentation
    if filename.endswith('.md') and any(
        suffix in filename.upper() for suffix in ['_REPORT', '_SUMMARY', '_GUIDE', '_ANALYSIS', '_PLAN', 'TESTING_', 'DEPLOYMENT_', 'IMPLEMENTATION_']
    ):
        return 'documentation'
    
    # Check for temporary files
    if any(pattern in name_lower for pattern in ['temp', 'tmp', 'backup', 'scratch', 'debug']) or filename.endswith(('.backup', '.bak', '.tmp', '.temp')):
        return 'temporary'
