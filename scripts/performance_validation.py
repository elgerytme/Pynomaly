...
def validate_architecture():
    ...
    try:
        ...
        result = subprocess.run([
            sys.executable, 
            "src/packages/simple_domain_check.py"
...
def main():
    ...
    if successful_imports > 0:
        avg_import_time = sum(v for v in import_results.values() if isinstance(v, float)) / successful_imports
    ...
    # Memory summary
    if memory_usage:
        memory_status = "excellent" if memory_usage < 100 else "good" if memory_usage < 200 else "high"
        print(f"💾 Memory Usage: {memory_usage:.1f}MB ({memory_status})")
    
    # Architecture summary
    print(f"🏗️ Architecture: {'✅ Valid' if architecture_valid else '❌ Issues detected'}")
    
    # Overall status
    print("\n" + "=" * 60)
    overall_good = (
        successful_imports >= total_imports * 0.8 and  # 80% imports successful
        (memory_usage is None or memory_usage < 200) and  # Memory under 200MB
        architecture_valid  # Architecture valid
    )
