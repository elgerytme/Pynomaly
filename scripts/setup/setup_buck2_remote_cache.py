...
class Buck2RemoteCacheSetup:
    ...
    def validate_cache_setup(self) -> Dict[str, Any]:
        ...
        if validation_results["cache_directory_exists"]:
            ...
            print("  ⚠️  Cache directory does not exist (will be created on first build)")
        ...
        validation_results["overall_status"] = "ready" if all_critical_passed else "needs_attention"
    ...
    def generate_cache_performance_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            buck2_cmd = "buck2"
            for buck2_path in ["buck2", "/mnt/c/Users/andre/buck2.exe", "/usr/local/bin/buck2"]:
                try:
                    subprocess.run([buck2_path, "--version"], capture_output=True, timeout=5)
            ...
            cache_effectiveness = clean_build_time / cached_build_time if cached_build_time > 0 else 1.0
...
def main():
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate cache setup"
    )
    parser.add_argument(
        "--performance-test",
        action="store_true",
        help="Run cache performance test"
    )
