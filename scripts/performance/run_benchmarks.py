...
class PerformanceBenchmarkRunner:
    ...
    def __init__(self, config_path: Optional[Path] = None, verbose: bool = False):
        self.config_path = config_path or Path(__file__).parent / "performance_config.yml"
    ...
    def _extract_performance_metrics(self, test_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def run_standalone_benchmarks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config = BenchmarkConfig(
                ...
                feature_dimensions=self.config["benchmark_config"]["feature_dimensions"],
                contamination_rates=self.config["benchmark_config"]["contamination_rates"],
                ...
                enable_memory_profiling=self.config["benchmark_config"].get("enable_memory_profiling", True),
                enable_cpu_profiling=self.config["benchmark_config"].get("enable_cpu_profiling", True)
            )
    ...
    def combine_results(self, pytest_results: Dict[str, Any], standalone_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        combined["summary"] = {
            ...
            "standalone_test_count": len(standalone_results.get("individual_results", [])),
            "overall_success": (
                pytest_results.get("failed", 0) == 0 and
                not pytest_results.get("error") and
                not standalone_results.get("error")
            )
        }
