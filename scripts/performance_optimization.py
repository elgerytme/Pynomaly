    ...
    def _collect_application_metrics(self) -> Dict:
        ...
        try:
            ...
            health_response = requests.get(
                f"{self.config['target_host']}/health",
                timeout=10
            )
            ...
            test_response = requests.get(
                f"{self.config['target_host']}/api/v1/health",
                timeout=10
            )
            ...
            try:
                metrics_response = requests.get(
                    f"{self.config['target_host']}/metrics",
                    timeout=10
                )
        ...
        if 'system' in before and 'system' in after:
            ...
            improvements['system'] = {
                'cpu_usage': self._calculate_percentage_improvement(
                    system_before.get('cpu_usage', 0),
                    system_after.get('cpu_usage', 0)
                ),
...
def main():
    ...
    parser.add_argument('--strategies', nargs='+', 
                       choices=['caching', 'database', 'memory', 'cpu', 'network', 'algorithms', 'resources', 'monitoring'],
