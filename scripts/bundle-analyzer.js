...
class BundleAnalyzer {
  ...
  async analyze() {
    ...
    try {
      ...
      console.error('L Bundle analysis failed:', error);
      throw error;
    }
  }

  async analyzeBundles() {
    console.log('=æ Analyzing bundle files...');
  ...
  async analyzeDependencies() {
    console.log('=Ë Analyzing package dependencies...');
  ...
  checkBudgets() {
    ...
    Object.entries(this.analysis.budgetStatus).forEach(([type, status]) => {
      const emoji = status.status === 'exceeded' ? 'L' : 
  ...
  async saveResults() {
    ...
    console.log('=Ä Reports generated:');
...
<body>
    <div class="container">
        ...
        <div class="summary">
            ...
            <div class="card">
                <h3>Health Score</h3>
                <div class="metric ${this.analysis.summary.healthScore >= 80 ? 'good' : this.analysis.summary.healthScore >= 60 ? 'warning' : 'exceeded'}">${this.analysis.summary.healthScore}/100</div>
            </div>
            <div class="card">
...
if (require.main === module) {
  ...
  analyzer.analyze()
    .then(results => {
      console.log(`\n=Ê Bundle analysis completed with ${results.summary.healthScore}/100 health score`);
