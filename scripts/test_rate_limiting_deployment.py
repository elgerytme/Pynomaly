...
class ProductionRateLimitTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not has_rate_limit_headers:
                    self.log_result("Health Endpoint Bypass", True, "Health endpoint bypasses rate limiting")
                    return True
                else:
                    self.log_result("Health Endpoint Bypass", False, "Health endpoint has rate limit headers")
                    return False
            else:
                self.log_result("Health Endpoint Bypass", False, f"Health endpoint returned {response.status_code}")
                return False
                
        except Exception as e:
            self.log_result("Health Endpoint Bypass", False, f"Health endpoint test failed: {str(e)}")
            return False
    ...
    def test_rate_limit_headers_present(self) -> bool:
        ...
        try:
            ...
            if len(present_headers) == len(expected_headers):
                self.log_result("Rate Limit Headers", True, "All rate limit headers present", {
                    "headers": {h: response.headers[h] for h in present_headers}
                })
                return True
            else:
                missing = set(expected_headers) - set(present_headers)
                self.log_result("Rate Limit Headers", False, f"Missing headers: {missing}")
                return False
                
        except Exception as e:
            self.log_result("Rate Limit Headers", False, f"Header test failed: {str(e)}")
            return False
    ...
    def test_concurrent_requests(self, num_requests: int = 10) -> bool:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=num_requests) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        self.log_result("Concurrent Requests", False, f"Request failed: {str(e)}")
                        return False
            ...
            self.log_result("Concurrent Requests", True, f"Handled {num_requests} concurrent requests", {
            ...
            self.log_result("Concurrent Requests", False, f"Concurrent test failed: {str(e)}")
            return False
    ...
    def test_auth_rate_limiting(self) -> bool:
        ...
        try:
            ...
            if register_response.status_code not in [200, 201, 409]:  # 409 = already exists
                self.log_result("Auth Rate Limiting", False, f"User registration failed: {register_response.status_code}")
                return False
            ...
            for i in range(15):  # Auth limit is typically 10/minute
                response = self.session.post(f"{self.base_url}/api/v1/auth/login", data=login_data)
                responses.append(response)
            ...
            if rate_limited_responses:
                ...
                try:
                    ...
                    has_required_fields = all(
                        field in data for field in ["error", "message", "limit", "remaining"]
                    )
                    
                    if has_required_fields:
                        self.log_result("Auth Rate Limiting", True, "Auth endpoints have stricter rate limits", {
                            "attempts_before_limit": len(responses) - len(rate_limited_responses),
                            "rate_limit_response": data
                        })
                        return True
                    else:
                        self.log_result("Auth Rate Limiting", False, "Rate limit response missing required fields")
                        return False
                        
                except json.JSONDecodeError:
                    self.log_result("Auth Rate Limiting", False, "Rate limit response is not valid JSON")
                    return False
            else:
                self.log_result("Auth Rate Limiting", False, "No rate limiting detected on auth endpoints")
                return False
                
        except Exception as e:
            self.log_result("Auth Rate Limiting", False, f"Auth rate limit test failed: {str(e)}")
            return False
    ...
    def test_different_ip_simulation(self) -> bool:
        ...
        try:
            ...
            for ip in ips:
                ...
                results.append((ip, response.status_code, response.headers.get("X-RateLimit-Remaining")))
            
            # All should be successful since they're different IPs
            successful = all(status == 200 for _, status, _ in results)
            
            if successful:
                self.log_result("Different IP Simulation", True, "Different IPs have separate rate limits", {
                    "results": [{"ip": ip, "status": status, "remaining": remaining} 
                              for ip, status, remaining in results]
                })
                return True
            else:
                self.log_result("Different IP Simulation", False, "IP-based rate limiting not working properly")
                return False
                
        except Exception as e:
            self.log_result("Different IP Simulation", False, f"IP simulation test failed: {str(e)}")
            return False
    ...
    def test_api_key_rate_limiting(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("API Key Rate Limiting", True, "API key rate limiting functional", {
                    "limit": limit,
                    "remaining": remaining,
                ...
                self.log_result("API Key Rate Limiting", False, f"API key request failed: {response.status_code}")
                return False
                
        except Exception as e:
            self.log_result("API Key Rate Limiting", False, f"API key test failed: {str(e)}")
            return False
    ...
    def test_rate_limit_recovery(self) -> bool:
        ...
        try:
            ...
            if remaining > 10:
                self.log_result("Rate Limit Recovery", True, "Rate limit recovery test skipped (too many remaining requests)", {
                    "remaining": remaining,
                    "limit": limit
                })
                return True
            ...
            new_remaining = int(recovery_response.headers.get("X-RateLimit-Remaining", 0))
            
            if new_remaining >= remaining:
                self.log_result("Rate Limit Recovery", True, "Rate limit recovery working", {
                    "initial_remaining": remaining,
                    "after_wait": new_remaining
                })
                return True
            else:
                self.log_result("Rate Limit Recovery", False, "Rate limit not recovering properly")
                return False
                
        except Exception as e:
            self.log_result("Rate Limit Recovery", False, f"Recovery test failed: {str(e)}")
            return False
    ...
    def test_security_headers(self) -> bool:
        ...
        try:
            ...
            if coverage >= 80:  # At least 80% of security headers present
                self.log_result("Security Headers", True, f"Security headers present ({coverage:.1f}%)", {
                    "present": present_headers,
                    "missing": list(set(expected_security_headers) - set(present_headers))
                })
                return True
            else:
                self.log_result("Security Headers", False, f"Insufficient security headers ({coverage:.1f}%)")
                return False
                
        except Exception as e:
            self.log_result("Security Headers", False, f"Security header test failed: {str(e)}")
            return False
    ...
    def test_cors_configuration(self) -> bool:
        ...
        try:
            ...
            if len(present_cors_headers) >= 2:
                self.log_result("CORS Configuration", True, "CORS properly configured", {
                    "cors_headers": {h: response.headers[h] for h in present_cors_headers}
                })
                return True
            else:
                self.log_result("CORS Configuration", False, "CORS not properly configured")
                return False
        ...
        with Progress(
            ...
            for description, test_func in tests:
                ...
                try:
                    ...
                    self.log_result(description, False, f"Test failed with exception: {str(e)}")
    ...
    def generate_report(self) -> Dict[str, any]:
        ...
        report = {
            ...
            "passed_tests": [{"name": r.test_name, "message": r.message, "duration": r.duration} for r in passed_tests],
            "failed_tests": [{"name": r.test_name, "message": r.message, "details": r.details} for r in failed_tests],
    ...
    def display_results(self):
        ...
        if passed == total:
            console.print("🎉 [bold green]All tests passed! Rate limiting and security systems are working correctly.[/bold green]")
        else:
            console.print("⚠️  [bold red]Some tests failed. Please review the issues above.[/bold red]")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test rate limiting and security middleware")
