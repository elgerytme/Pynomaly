...
class MultiEnvironmentTester:
    ...
    def run_command(self, cmd: List[str], cwd: Optional[Path] = None, 
                   timeout: int = 120, allow_failure: bool = True) -> Tuple[int, str, str]:
    ...
    def test_current_environment(self, script_path: str, args: List[str] = None) -> Dict:
        ...
        if script_path.endswith('.py'):
            ...
            if sys.platform == "win32":
                cmd = ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(script_abs)] + args
            else:
                return {
                    "success": False,
...
""")
    ...
    def test_windows_environment(self, script_path: str, args: List[str] = None) -> Dict:
...
""")
            ...
            return {
                ...
                "stderr": stderr,
                "test_dir": str(test_dir),
                "command": f"powershell {test_script}" if sys.platform == "win32" else f"bash wrapper simulation"
            }
    ...
    def test_script_validation(self, script_path: str) -> Dict:
        ...
        try:
            ...
            if script_path.endswith('.py'):
                ...
                checks["has_imports"] = any(line.strip().startswith(('import ', 'from ')) for line in content.split('\n'))
    ...
    def test_script_comprehensive(self, script_path: str, args: List[str] = None) -> Dict:
        ...
        if results["summary"]["overall_success"]:
            self.log(f"✅ PASS: {script_path} ({environments_passed}/{environments_tested} environments)", "SUCCESS")
        else:
            self.log(f"❌ FAIL: {script_path} ({environments_passed}/{environments_tested} environments)", "ERROR")
        
        return results
    
    def generate_report(self, all_results: List[Dict], output_file: Optional[str] = None) -> str:
        ...
        report.append(f"- **Overall Success Rate**: {passed_scripts/total_scripts*100:.1f}%")
        report.append("")
        ...
        for result in all_results:
            ...
            report.append(f"- **Environments Passed**: {summary['environments_passed']}/{summary['environments_tested']}")
            report.append(f"- **Pass Rate**: {summary['pass_rate']*100:.1f}%")
            report.append("")
            
            # Environment details
            for env_name, env_key in [("Current", "current_env"), ("Linux", "linux_env"), ("Windows", "windows_env")]:
...
Examples:
  ...
  python scripts/multi_environment_tester.py --batch scripts/*.py
    ...
    parser.add_argument(
        "--batch",
        nargs="+",
        help="Test multiple scripts in batch mode"
    )
    ...
    parser.add_argument(
        "--current-only",
        action="store_true",
        help="Only test in current environment"
    )
    ...
    for script in scripts_to_test:
        ...
        if args.current_only:
            ...
            result = {
                ...
                "current_env": tester.test_current_environment(script, args.script_args),
                "summary": {}
            }
            result["summary"] = {
