...
def validate_environment_organization() -> Dict[str, Any]:
    ...
    if invalid_names:
        results["errors"].append(f"Environment directories without dot-prefix: {invalid_names}")
        results["success"] = False
    ...
    if root_env_dirs:
        results["warnings"].append(f"Environment directories found in root (should be moved): {root_env_dirs}")
    else:
    ...
    if gitignore_path.exists():
        ...
        if "environments/" in gitignore_content and "!environments/README.md" in gitignore_content:
            results["info"].append("✓ .gitignore properly excludes environments/ directory")
        else:
            results["errors"].append(".gitignore does not properly exclude environments/ directory")
            results["success"] = False
    else:
    ...
    if pyproject_path.exists():
        ...
        for tool in tools_to_check:
            if f"[tool.{tool}]" in pyproject_content and "environments" in pyproject_content:
                configured_tools.append(tool)
            else:
                missing_tools.append(tool)
        
        if configured_tools:
            results["info"].append(f"✓ Tools configured to ignore environments: {configured_tools}")
        
        if missing_tools:
            results["warnings"].append(f"Tools may not be configured to ignore environments: {missing_tools}")
    else:
    ...
    if claude_md_path.exists():
        ...
        if "Environment Management Rules" in claude_content and "environments/" in claude_content:
            results["info"].append("✓ CLAUDE.md contains environment management rules")
        else:
            results["warnings"].append("CLAUDE.md may not contain complete environment management rules")
    else:
