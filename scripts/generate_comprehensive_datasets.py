...
def create_financial_fraud_dataset(n_samples=10000, anomaly_rate=0.02):
    ...
    normal_hours = np.random.choice(range(6, 23), size=n_normal, p=[0.05, 0.08, 0.12, 0.15, 0.15, 0.12, 0.08, 0.05, 0.04, 0.03, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.01])
    normal_merchant_categories = np.random.choice(['grocery', 'gas', 'restaurant', 'retail', 'online'], size=n_normal, p=[0.3, 0.2, 0.25, 0.15, 0.1])
    normal_locations = np.random.choice(range(1, 51), size=n_normal)  # 50 different locations
    ...
    fraud_amounts = np.concatenate([
        ...
        np.random.lognormal(mean=6, sigma=2, size=n_anomalies - 2*(n_anomalies//3))  # Very high amounts
    ])
    fraud_hours = np.random.choice([1, 2, 3, 4, 5], size=n_anomalies)  # Unusual hours
    fraud_merchant_categories = np.random.choice(['atm', 'casino', 'luxury', 'crypto', 'unknown'], size=n_anomalies)
    fraud_locations = np.random.choice(range(51, 101), size=n_anomalies)  # Different locations
    fraud_freq = np.random.choice([0, 15, 20, 25], size=n_anomalies)  # Unusual frequency
...
def create_network_intrusion_dataset(n_samples=8000, anomaly_rate=0.05):
    ...
    normal_ports = np.random.choice([80, 443, 22, 21, 25, 53], size=n_normal, p=[0.4, 0.3, 0.1, 0.05, 0.05, 0.1])
    normal_protocols = np.random.choice(['TCP', 'UDP', 'ICMP'], size=n_normal, p=[0.7, 0.25, 0.05])
    
    # Malicious traffic patterns
    attack_packets = np.concatenate([
        np.random.gamma(shape=10, scale=500, size=n_anomalies//3),  # DDoS - high packet count
        np.random.uniform(1, 5, size=n_anomalies//3),              # Port scanning - low packets
        np.random.gamma(shape=5, scale=100, size=n_anomalies - 2*(n_anomalies//3))
    ])
    attack_bytes = attack_packets * np.random.gamma(shape=1, scale=50, size=n_anomalies)  # Different size pattern
    attack_duration = np.concatenate([
        np.random.uniform(0.1, 1, size=n_anomalies//2),           # Very short
        np.random.uniform(100, 500, size=n_anomalies - n_anomalies//2)  # Very long
    ])
    attack_ports = np.random.choice(range(1024, 65536), size=n_anomalies)  # Random high ports
    attack_protocols = np.random.choice(['TCP', 'UDP', 'ICMP'], size=n_anomalies, p=[0.5, 0.4, 0.1])
...
def create_iot_sensor_dataset(n_samples=12000, anomaly_rate=0.03):
    ...
    temp_anomaly = np.concatenate([
        np.random.uniform(-10, 5, size=n_anomalies//3),    # Sensor failure - too cold
        np.random.uniform(35, 60, size=n_anomalies//3),    # Overheating
        np.random.normal(25, 15, size=n_anomalies - 2*(n_anomalies//3))  # High variance
    ])
    
    humidity_anomaly = np.concatenate([
        np.random.uniform(0, 10, size=n_anomalies//2),     # Too dry
        np.random.uniform(90, 100, size=n_anomalies - n_anomalies//2)  # Too humid
    ])
    
    pressure_anomaly = np.random.normal(1013, 20, n_anomalies)  # High variance
    vibration_anomaly = np.random.gamma(shape=10, scale=2, size=n_anomalies)  # High vibration
...
def create_manufacturing_quality_dataset(n_samples=6000, anomaly_rate=0.08):
    ...
    defect_dim1 = np.concatenate([
        ...
        np.random.uniform(70, 130, n_anomalies - 2*(n_anomalies//3))  # Random defects
    ])
    ...
    spec_deviation = np.sqrt((all_dim1 - 100)**2 + (all_dim2 - 50)**2 + ((all_weight - 500)/10)**2)
...
def create_ecommerce_behavior_dataset(n_samples=15000, anomaly_rate=0.04):
    ...
    purchase_amounts = np.where(
        purchases == 1,
        np.random.lognormal(mean=4, sigma=0.8, size=n_normal),
        0
    )
    
    # Time features (normal users) - normalized probabilities
    hour_probs = np.array([0.02, 0.01, 0.01, 0.01, 0.01, 0.02, 0.03, 0.05, 0.06, 0.07, 0.08, 0.08, 0.08, 0.08, 0.07, 0.06, 0.05, 0.04, 0.04, 0.04, 0.04, 0.03, 0.03, 0.02])
    ...
    browse_duration = np.random.uniform(60, 300, size=n_anomalies - 2*(n_anomalies//3))
    ...
    browse_hours = np.random.choice(range(24), size=n_anomalies - 2*(n_anomalies//3))
...
def create_time_series_anomaly_dataset(n_samples=5000, anomaly_rate=0.06):
    ...
    # Additional features
    moving_avg_5 = pd.Series(anomaly_ts).rolling(window=5, center=True).mean().fillna(method='bfill').fillna(method='ffill')
    moving_std_5 = pd.Series(anomaly_ts).rolling(window=5, center=True).std().fillna(0.5)
    ...
    df = pd.DataFrame({
        ...
        'std_normalized_value': (anomaly_ts - moving_avg_5) / (moving_std_5 + 0.001),
        'is_anomaly': labels.astype(int)
    })
...
def create_metadata(dataset_name, df, description, anomaly_types, recommended_algorithms):
...
def main():
    ...
    synthetic_datasets = [
        ("financial_fraud", create_financial_fraud_dataset(), 
         "Financial transaction fraud detection with unusual amounts, timing, and patterns",
         ["Transaction fraud", "Unusual amounts", "Timing anomalies", "Location anomalies"],
         ["IsolationForest", "LocalOutlierFactor", "OneClassSVM"]),
        
        ("network_intrusion", create_network_intrusion_dataset(),
         "Network traffic anomaly detection with DDoS, port scanning, and malicious patterns",
         ["DDoS attacks", "Port scanning", "Traffic volume anomalies", "Protocol anomalies"],
         ["IsolationForest", "EllipticEnvelope", "PyOD.ABOD"]),
        
        ("iot_sensors", create_iot_sensor_dataset(),
         "IoT sensor monitoring with failures, environmental anomalies, and drift",
         ["Sensor failures", "Environmental anomalies", "Measurement drift", "Temporal anomalies"],
         ["LocalOutlierFactor", "EllipticEnvelope", "PyOD.KNN"]),
        
        ("manufacturing_quality", create_manufacturing_quality_dataset(),
         "Manufacturing quality control with defective products and process variations",
         ["Product defects", "Process variations", "Machine malfunctions", "Specification violations"],
         ["IsolationForest", "PyOD.OCSVM", "EllipticEnvelope"]),
        
        ("ecommerce_behavior", create_ecommerce_behavior_dataset(),
         "E-commerce user behavior with bot detection and fraud patterns",
         ["Bot behavior", "Purchase fraud", "Unusual browsing patterns", "Session anomalies"],
         ["LocalOutlierFactor", "IsolationForest", "PyOD.COPOD"]),
        
        ("time_series_anomalies", create_time_series_anomaly_dataset(),
         "Time series with spikes, drops, trend changes, and seasonality breaks",
         ["Value spikes", "Value drops", "Trend changes", "Level shifts", "Seasonality breaks"],
         ["PyOD.KNN", "LocalOutlierFactor", "EllipticEnvelope"]),
        
        ("high_dimensional", create_high_dimensional_dataset(),
         "High-dimensional dataset with correlated features and outliers",
         ["High-dimensional outliers", "Correlation anomalies", "Feature space anomalies"],
         ["IsolationForest", "PyOD.PCA", "PyOD.ABOD", "LocalOutlierFactor"])
    ]
    
    # Save synthetic datasets
    for name, df, description, anomaly_types, recommended_algorithms in synthetic_datasets:
        print(f"  📊 Saving {name}...")
        ...
        # Create metadata
        metadata = create_metadata(name, df, description, anomaly_types, recommended_algorithms)
        ...
        print(f"    ✓ {len(df)} samples, {len(df.columns)-1} features, {df['is_anomaly'].mean():.1%} anomalies")
