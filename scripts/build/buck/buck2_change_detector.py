...
class Buck2ChangeDetector:
    ...
    def get_changed_files(
        ...
        try:
            ...
            logger.info(
                f"Found {len(changed_files)} changed files between {base_commit} and {target_commit}"
            )
            ...
            for pattern, targets in self.target_map.items():
                ...
                if file_path.startswith("src/pynomaly/"):
                    ...
                    affected_targets.update(
                        {"pynomaly-cli", "pynomaly-api", "pynomaly-web"}
                    )

        logger.info(
            f"Mapped {len(changed_files)} files to {len(affected_targets)} targets"
        )
    ...
    def categorize_targets(self, targets: set[str]) -> tuple[set[str], set[str]]:
        ...
        test_targets = {
            ...
            or t in ["benchmarks", "property-tests", "mutation-tests", "security-tests"]
        }
