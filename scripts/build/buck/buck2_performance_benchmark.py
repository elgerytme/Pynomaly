...
class Buck2PerformanceBenchmark:
    ...
    def benchmark_buck2_basic_builds(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_speedup": statistics.mean(build_times)
    ...
    def benchmark_hatch_operations(self) -> Dict[str, Any]:
        ...
        for op_name, command in operations.items():
            ...
            for run in range(3):
                start_time = time.time()
                result = subprocess.run(
                    command, capture_output=True, cwd=self.root_path, timeout=60
                )
    ...
    def benchmark_incremental_builds(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for run in range(3):
                ...
                start_time = time.time()
                subprocess.run(
                    [self.buck2_executable, "build", "//:dev-ready"],
                    capture_output=True,
                    cwd=self.root_path,
                )
        ...
        results.update(
            {
                ...
                "incremental_speedup": statistics.mean(results["full_rebuild_times"])
                / statistics.mean(results["incremental_times"]),
                "no_change_speedup": statistics.mean(results["full_rebuild_times"])
                / statistics.mean(results["no_change_times"]),
            }
        )
    ...
    def benchmark_cache_performance(self) -> Dict[str, Any]:
        ...
        subprocess.run(
            [self.buck2_executable, "clean"], capture_output=True, cwd=self.root_path
        )

        start_time = time.time()
        subprocess.run(
            [self.buck2_executable, "build", "//:validation"],
            capture_output=True,
            cwd=self.root_path,
        )
    ...
    def generate_performance_report(self, results: Dict[str, Any]):
        ...
        print(
            f"   Incremental speedup: {incremental_results['incremental_speedup']:.1f}x"
        )
