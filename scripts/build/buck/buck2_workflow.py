...
class Buck2Workflow:
    ...
    def __init__(self, config: WorkflowConfig = None):
        ...
        self.test_runner = Buck2IncrementalTestRunner(
            self.repo_root, self.config.dry_run
        )
    ...
    def run_standard_workflow(self) -> dict:
        ...
        try:
            ...
            change_analysis = self.change_detector.analyze_changes(
                self.config.base_commit, self.config.target_commit
            )
    ...
    def run_commit_validation_workflow(self) -> dict:
        ...
        try:
            ...
            for i, commit in enumerate(reversed(branch_info.commits)):
                logger.info(
                    f"Validating commit {i+1}/{len(branch_info.commits)}: {commit.hash[:8]}"
                )

                try:
                    # Analyze this specific commit
                    commit_analysis = self.change_detector.analyze_changes(
                    ...
                    if commit_analysis.changed_files:
                        ...
                        test_summary = self.test_runner.run_incremental_tests(
                            prev_commit, commit.hash, fail_fast=self.config.fail_fast
                        )
                        ...
                        results["commit_results"][commit.hash] = {
                            ...
                            "status": "passed"
                            if test_summary.failed_targets == 0
                            else "failed",
                        }
                    else:
                        results["commit_results"][commit.hash] = {
            ...
            failed_commits = sum(
                1
                for r in results["commit_results"].values()
            ...
            results["summary"] = {
                ...
                "success_rate": (total_commits - failed_commits) / total_commits
                if total_commits > 0
                else 1.0,
            }
    ...
    def _run_tests_by_strategy(
        self, strategy: str, change_analysis: ChangeAnalysis
    ) -> TestRunSummary:
...
def main():
    ...
    standard_parser = subparsers.add_parser(
        "standard", help="Standard incremental testing"
    )
    standard_parser.add_argument(
        "--strategy",
        choices=["auto", "minimal", "standard", "comprehensive", "full"],
        default="auto",
        help="Test strategy",
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate-commits", help="Validate each commit"
    )
