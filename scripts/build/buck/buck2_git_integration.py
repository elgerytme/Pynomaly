...
class Buck2GitIntegration:
    ...
    def get_branch_info(self, branch: str = None, base_branch: str = None) -> BranchInfo:
        ...
        try:
            ...
            commit_hashes = [c.strip() for c in commits_result.stdout.split('\n') if c.strip()]
            ...
            total_files_changed = set(f.strip() for f in files_result.stdout.split('\n') if f.strip())
    ...
    def test_commit_range(self, base_commit: str, target_commit: str, **kwargs) -> TestRunSummary:
        """Test a specific commit range."""
        logger.info(f"Testing commit range {base_commit}..{target_commit}")
        return self.test_runner.run_incremental_tests(base_commit, target_commit, **kwargs)
    ...
    def test_current_branch(self, base_branch: str = None, **kwargs) -> TestRunSummary:
        ...
        logger.info(f"Testing branch '{branch_info.name}' against '{base_branch or self.get_main_branch()}'")
        return self.test_runner.run_incremental_tests(
            branch_info.base_commit, 
            branch_info.head_commit, 
            **kwargs
        )
    
    def test_per_commit(self, base_branch: str = None, **kwargs) -> Dict[str, TestRunSummary]:
        ...
        for commit in reversed(branch_info.commits):  # Test in chronological order
            ...
            try:
                ...
                if summary.failed_targets > 0:
                    logger.warning(f"Commit {commit.hash[:8]} has {summary.failed_targets} failing targets")
    ...
    def find_breaking_commit(self, base_branch: str = None, **kwargs) -> Optional[str]:
        ...
        logger.info(f"Binary searching for breaking commit in {len(branch_info.commits)} commits")
        ...
        if breaking_commit:
            ...
            logger.info(f"Breaking commit found: {breaking_commit[:8]} - {commit_info.message}")
    ...
    def test_staged_changes(self, **kwargs) -> Optional[TestRunSummary]:
        ...
        try:
            ...
            temp_commit = subprocess.run([
                "git", "stash", "create"
            ], capture_output=True, text=True, check=True, cwd=self.repo_root).stdout.strip()
            
            if temp_commit:
                return self.test_runner.run_incremental_tests("HEAD", temp_commit, **kwargs)
            else:
                # Fallback to testing current changes
                return self.test_runner.run_incremental_tests("HEAD~1", "HEAD", **kwargs)
...
def main():
    ...
    branch_parser.add_argument("--fail-fast", action="store_true", help="Stop on first failure")
    branch_parser.add_argument("--dry-run", action="store_true", help="Show what would be run")
    
    # Test staged command
    staged_parser = subparsers.add_parser("test-staged", help="Test staged changes")
    staged_parser.add_argument("--fail-fast", action="store_true", help="Stop on first failure")
    staged_parser.add_argument("--dry-run", action="store_true", help="Show what would be run")
    
    # Test per-commit command
    commit_parser = subparsers.add_parser("test-commits", help="Test each commit individually")
    commit_parser.add_argument("--base", help="Base branch name")
    commit_parser.add_argument("--fail-fast", action="store_true", help="Stop on first failure")
    ...
    try:
        if args.command == "test-branch":
            ...
            breaking_commit = git_integration.find_breaking_commit(base_branch=args.base)
            if breaking_commit:
