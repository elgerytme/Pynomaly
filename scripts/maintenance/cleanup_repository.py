...
def display_summary(metrics: CleanupMetrics, dry_run: bool) -> None:
    """Display cleanup summary in a formatted table."""
    table = Table(title="Repository Cleanup Summary")
    table.add_column("Metric", style="cyan")
    table.add_column("Value", style="green")

    mode = "DRY-RUN" if dry_run else "EXECUTED"
    table.add_row("Mode", mode)
    table.add_row("Duration", f"{metrics.duration:.2f} seconds")
    table.add_row("Files Removed", str(metrics.files_removed))
    table.add_row("Directories Removed", str(metrics.directories_removed))
    table.add_row("Space Freed", format_bytes(metrics.bytes_freed))
...
def main(
    ...
    output: str = typer.Option(
        "reports/quality/cleanup_report.json",
        "--output",
        "-o",
        help="Output JSON report file.",
    ),
    no_ide: bool = typer.Option(
        False, "--no-ide", help="Skip cleaning IDE-specific files."
    ),
) -> None:
    """
    Clean up the repository by removing temporary files, cache directories, and other artifacts.

    This tool helps maintain a clean repository by removing:
    - Temporary files (*.tmp, *.bak, *.swp, etc.)
    - Cache directories (__pycache__, .pytest_cache, etc.)
    - Log files (*.log, *.out, etc.)
    - Test artifacts (htmlcov, coverage.xml, etc.)
    ...
    Examples:
        ...
        python cleanup_repository.py --verbose
    ...
    if dry_run:
        console.print(
            "[yellow]Running in DRY-RUN mode - no files will be removed[/yellow]"
        )
    ...
    # Display summary
    display_summary(metrics, dry_run)
