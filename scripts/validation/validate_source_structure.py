...
class SourceStructureValidator:
            ...
            for py_file in layer_path.rglob("*.py"):
                ...
                try:
                    ...
                    self.warnings.append(
                        f"Could not analyze dependencies in {py_file}: {e}"
                    )
    ...
    def _check_naming_conventions(self, rule: ArchitectureRule):
        ...
        for py_file in self.pynomaly_root.rglob("*.py"):
            ...
            if module_name != "__init__" and not re.match(
                self.naming_conventions["modules"], module_name
            ):
                self.warnings.append(
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        if node.name.startswith("_"):
                            if not re.match(
                                self.naming_conventions["private"], node.name
                            ):
                                self.warnings.append(
                            ...
                            if not re.match(
                                self.naming_conventions["functions"], node.name
                            ):
                                self.warnings.append(
    ...
    def _check_module_organization(self, rule: ArchitectureRule):
        ...
        for layer, config in self.expected_structure.items():
            ...
            python_files = [
                f for f in layer_path.glob("*.py") if f.name != "__init__.py"
            ]
    ...
    def _check_circular_imports(self, rule: ArchitectureRule):
        ...
        for py_file in self.pynomaly_root.rglob("*.py"):
            ...
            module_path = (
                str(py_file.relative_to(self.src_root))
                .replace("/", ".")
                .replace(".py", "")
            )
            imports = set()
