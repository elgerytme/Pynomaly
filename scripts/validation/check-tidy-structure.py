...
class TidyStructureChecker:
    ...
    def __init__(self, root_path: Path = None):
        ...
        self.forbidden_root_dirs = {
            'build', 'dist', 'output', 'tmp', 'temp', 'cache', 
            'backup', 'old', 'legacy', 'vendor', 'lib', 'bin', 
            'data', 'samples', 'misc'
        }
        
        # Allowed directories at root level
        self.allowed_root_dirs = {
            'src', 'tests', 'docs', 'examples', 'scripts', 'config',
            'environments', 'deploy', 'tools', 'artifacts', 'reports',
            'storage', 'baseline_outputs', 'test_reports', '.github',
            '.venv', '.env', '.storybook', '.hypothesis', '.mypy_cache',
            '.pytest_cache', '.ruff_cache', '.tox', '.claude', 'node_modules',
            'htmlcov', 'templates', 'stories', 'toolchains'
        }
        ...
        self.config_file_pattern = re.compile(r'^[a-z0-9_.-]+\.(yaml|yml|toml|json|cfg|ini)$')
    ...
    def _check_file_naming(self):
        ...
        for py_file in self.root_path.rglob("*.py"):
            ...
            if py_file.parts and 'test' in py_file.parts:
                if not (self.test_file_pattern.match(filename) or 
        ...
        for md_file in self.root_path.rglob("*.md"):
            ...
            if (self.readme_pattern.match(filename) or 
                self.api_doc_pattern.match(filename) or 
                self.adr_pattern.match(filename)):
                continue
    ...
    def _check_test_organization(self):
        ...
        for py_file in tests_path.rglob("*.py"):
            ...
            if not (py_file.name.startswith("test_") or 
    ...
    def _check_archive_organization(self):
        ...
        expected_archive_dirs = {
            'historical-project-docs', 'legacy-algorithm-docs',
            'experimental', 'deprecated-features', 'old-configs'
        }
    ...
    def _should_skip_path(self, path: Path) -> bool:
        ...
        skip_patterns = [
            ...
            '.tox', '.hypothesis', 'node_modules', 'htmlcov',
            'artifacts', 'reports', 'storage', 'baseline_outputs',
            'test_reports', '.claude'
        ]
