...
class RootDirectoryValidator:
    ...
    ALLOWED_ROOT_FILES = {
        ...
        'Makefile', 'Pynomaly.code-workspace', 'CONTRIBUTING.md'
    }
    
    # Essential directories allowed in root
    ALLOWED_ROOT_DIRECTORIES = {
        'src', 'tests', 'docs', 'scripts', 'config', 'deploy', 'examples',
        'environments', 'reports', 'storage', 'templates', 'tools', 'toolchains'
    }
    
    # File patterns that are PROHIBITED in root
    PROHIBITED_PATTERNS = {
        'test_*.py', '*_test.py', 'conftest.py',
        'setup_*.py', 'install_*.py', 'fix_*.py', 'update_*.py', 'deploy_*.py',
        '*_GUIDE.md', '*_MANUAL.md', '*_REPORT.md', '*_SUMMARY.md', 
        'TESTING_*.md', 'IMPLEMENTATION_*.md',
        'config_*.py', 'settings_*.py', '*.ini', '*.config.*',
    ...
    SHOULD_BE_GITIGNORED = {
        'node_modules', '__pycache__', 'buck-out', 'dist', 'build',
...
def main():
    ...
    parser.add_argument(
        '--fix-suggestions', 
        action='store_true',
        help="Show relocation suggestions"
    )
