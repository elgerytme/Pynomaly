...
class DomainPurityFixer:
    ...
    def analyze_violations(self) -> List[Dict]:
        ...
        print(f"📊 Found {len(self.violations)} violations across {len(set(v['file'] for v in self.violations))} files")
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        elif (module not in self.allowed_stdlib and 
                              not module.startswith('pynomaly.domain') and
                              not module.startswith('pynomaly.shared')):
                            violations.append({
                                'file': str(file_path.relative_to(self.src_root)),
                                'line': node.lineno,
...
def main():
    ...
    parser.add_argument(
        '--detailed',
        action='store_true',
        help="Show detailed remediation examples"
    )
