...
class SDKGenerator:
    ...
    def _generate_custom_readme(self, language: str, output_path: Path, spec: Dict[str, Any]):
...
'''
    ...
    def _post_process_typescript(self, output_path: Path):
        ...
        if package_json_path.exists():
            ...
            package_json["scripts"] = {**package_json.get("scripts", {}), **package_json_scripts}
    ...
    def generate_all_sdks(self, languages: Optional[List[str]] = None) -> Dict[str, bool]:
...
"""
                ...
                summary_content += f"""### TypeScript/JavaScript
```bash
cd sdks/typescript
npm install
npm run build
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate multi-language client SDKs for Pynomaly API")
    parser.add_argument(
        "--languages", 
        nargs="*", 
