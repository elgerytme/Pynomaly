...
class DocumentationPDFGenerator:
    ...
    def generate_pdf_from_markdown(self, markdown_file: Path, output_file: Path) -> bool:
    ...
    def generate_combined_pdf(self, markdown_files: List[Path], output_file: Path) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate PDF versions of Pynomaly documentation")
    parser.add_argument(
        "--docs-dir",
    ...
    parser.add_argument(
        "--output-dir",
        help="Output directory for PDF files (default: {docs-dir}/pdfs)"
    )
