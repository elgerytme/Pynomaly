    ...
    try:
        ...
        for dockerfile in dockerfiles:
            if os.path.exists(dockerfile):
                success, stdout, stderr = run_command(f"docker build --dry-run -f {dockerfile} .")
                if success:
                    print(f"✅ {dockerfile} syntax valid")
                else:
                    print(f"⚠️  {dockerfile} syntax validation skipped (build context issues)")
            else:
        ...
        for compose_file in compose_files:
            if os.path.exists(compose_file):
                success, stdout, stderr = run_command(f"docker-compose -f {compose_file} config", timeout=30)
                if success:
        ...
        if os.path.exists("../../src/pynomaly"):
            ...
            if success:
                ...
                success, stdout, stderr = run_command("docker images pynomaly-test:latest")
                if success:
        ...
        print("   • Integrated monitoring stack with Prometheus, Grafana, and OpenTelemetry")
