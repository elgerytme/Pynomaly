...
class Buck2IntegrationValidator:
        ...
        try:
            ...
            if buck_file.exists():
                ...
                required_targets = [
                    ...
                    "test-domain", "test-application", "test-infrastructure", "test-presentation"
                ]
            ...
            # Overall status
            if (result["details"].get("buck_file") == "exists" and 
                result["details"].get("buckconfig_file") == "exists"):
                result["status"] = "valid"
    ...
    def test_buck2_targets(self) -> Dict:
        ...
        try:
            ...
            if targets_result.returncode == 0:
                targets = [t.strip() for t in targets_result.stdout.split('\n') if t.strip()]
    ...
    def create_fallback_strategy(self, validation_results: Dict) -> Dict:
        ...
        buck2_available = validation_results.get("buck2_installation", {}).get("status") == "available"
        config_valid = validation_results.get("buck_configuration", {}).get("status") == "valid"
        python_compatible = validation_results.get("python_environment", {}).get("status") == "compatible"
        ...
        if buck2_available and config_valid and python_compatible:
            ...
            strategy["alternatives"]["pytest_commands"] = self._generate_pytest_commands()
    ...
    def run_comprehensive_validation(self) -> Dict:
        ...
        for func in validation_functions:
            try:
                ...
                logger.info(f"✓ {component_result['component']}: {component_result['status']}")
            except Exception as e:
                error_result = {
        ...
        results["fallback_strategy"] = self.create_fallback_strategy(results["components"])
        
        # Generate summary
        total_components = len(results["components"])
        successful_components = sum(1 for c in results["components"].values() 
        ...
        results["summary"] = {
            ...
            "overall_status": "ready" if successful_components >= total_components * 0.8 else "needs_work"
        }
    ...
    def print_validation_summary(self, results: Dict):
        ...
        for component, result in results["components"].items():
            status_symbol = {
                "available": "✓", "valid": "✓", "compatible": "✓", "success": "✓",
                "not_installed": "!", "incomplete": "!", "issues": "!",
                "failed": "✗", "error": "✗", "timeout": "!"
            }.get(result["status"], "?")
            
            print(f"  {status_symbol} {component}: {result['status']}")
            
            if result["status"] in ["error", "failed"] and "error" in result.get("details", {}):
    ...
    def save_validation_results(self, results: Dict, output_file: Path = None) -> Path:
        ...
        if output_file is None:
            ...
            output_file = self.repo_root / f"buck2_integration_validation_{timestamp}.json"
