    ...
    for filename in filenames:
        ...
        if expected_dir and expected_dir != current_dir:
            ...
            if expected_dir == 'DELETE':
                ...
                suggestions.append(f"MOVE {filename} from {current_dir} to {expected_dir}")
...
def categorize_file(filename: str) -> str:
    ...
    if filename.endswith('.md'):
        if any(x in filename.upper() for x in ['_REPORT', '_SUMMARY', '_GUIDE', '_ANALYSIS', '_PLAN']):
            return 'documentation'
        elif any(x in filename.upper() for x in ['TESTING_', 'DEPLOYMENT_', 'IMPLEMENTATION_']):
            return 'documentation'
    
    # Script files
    if filename.endswith(('.py', '.sh', '.ps1', '.bat')):
        if any(filename.startswith(x) for x in ['fix_', 'setup_', 'install_', 'run_', 'deploy_', 'build_']):
            return 'scripts'
    
    # Report files
    if any(x in name_lower for x in ['report', 'summary', 'analysis']) and filename.endswith(('.json', '.html', '.xml')):
        return 'reports'
    ...
    # Configuration files
    if filename.endswith(('.json', '.yaml', '.yml', '.ini', '.toml', '.cfg')) and 'config' in name_lower:
...
def print_detection_results(stray_files: List[Dict], suggestions: List[str]):
    ...
    for category, files in by_category.items():
        ...
        for file_info in files[:3]:  # Show first 3 files
            print(f"    • {file_info['file']} ({file_info['current_location']} → {file_info['expected_location']})")
        if len(files) > 3:
