...
class MultiPythonValidator:
    ...
    def run_validation(self) -> ValidationSummary:
        ...
        logger.info(f"Status: {overall_status} ({passed_checks}/{len(results)} checks passed)")
    ...
    def _validate_python_versions(self) -> Tuple[bool, str, Dict]:
        ...
        if pyenv_available:
            try:
                ...
                if missing:
                    return False, f"Missing Python versions: {', '.join(missing)}", details
                
                return True, f"All {len(self.expected_versions)} Python versions available", details
            ...
            if len(system_pythons) >= 2:
                return True, f"System Python versions available: {', '.join(system_pythons)}", details
            else:
    ...
    def _validate_virtual_environments(self) -> Tuple[bool, str, Dict]:
        ...
        if not self.environments_dir.exists():
            return False, "Environments directory not found", {"environments_dir": str(self.environments_dir)}
        ...
        if found_environments == 0:
            ...
            return False, f"Only {found_environments}/{len(self.expected_versions)} environments found", details
        else:
    ...
    def _validate_package_installations(self) -> Tuple[bool, str, Dict]:
        ...
        if success_rate >= 0.9:
            return True, f"Package installation excellent ({successful_checks}/{total_checks})", details
        elif success_rate >= 0.7:
            return True, f"Package installation good ({successful_checks}/{total_checks})", details
        else:
            return False, f"Package installation issues ({successful_checks}/{total_checks})", details
    ...
    def _validate_import_compatibility(self) -> Tuple[bool, str, Dict]:
        ...
        if success_rate >= 0.95:
            return True, f"Import compatibility excellent ({successful_tests}/{total_tests})", details
        elif success_rate >= 0.8:
            return True, f"Import compatibility good ({successful_tests}/{total_tests})", details
        else:
            return False, f"Import compatibility issues ({successful_tests}/{total_tests})", details
    
    def _validate_github_actions(self) -> Tuple[bool, str, Dict]:
        """Validate GitHub Actions workflow configuration."""
        workflow_file = self.base_dir / ".github" / "workflows" / "multi-python-testing.yml"
        ...
        try:
            ...
            if missing_components:
                return False, f"Workflow missing components: {', '.join(missing_components)}", details
            
            if len(python_versions_found) >= 3:
                return True, f"GitHub Actions workflow properly configured ({len(python_versions_found)} Python versions)", details
            else:
                return False, f"Insufficient Python versions in workflow ({len(python_versions_found)})", details
    ...
    def _validate_tox_configuration(self) -> Tuple[bool, str, Dict]:
        ...
        details = {
            "tox_file_exists": tox_file.exists(),
            "tox_file_path": str(tox_file)
        }
        ...
        try:
            ...
            if missing_sections:
                return False, f"tox.ini missing sections: {', '.join(missing_sections)}", details
            
            if len(python_envs) >= 2:
                return True, f"tox configuration valid ({len(python_envs)} Python environments)", details
            else:
                return False, f"Insufficient Python environments in tox.ini ({len(python_envs)})", details
    ...
    def _validate_docker_setup(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            python_installs = []
            for version in ["3.11", "3.12", "3.13"]:
                if f"pyenv install {version}" in content or f"python{version}" in content:
            ...
            if missing_components:
                return False, f"Dockerfile missing components: {', '.join(missing_components)}", details
            
            if len(python_installs) >= 2:
                return True, f"Docker setup valid ({len(python_installs)} Python versions)", details
            else:
                return False, f"Insufficient Python versions in Docker ({len(python_installs)})", details
    ...
    def _validate_performance_baseline(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if computation_time < 0.1 and import_time < 0.05:
                return True, f"Performance excellent (compute: {computation_time:.3f}s, import: {import_time:.3f}s)", details
            elif computation_time < 0.5 and import_time < 0.2:
                return True, f"Performance good (compute: {computation_time:.3f}s, import: {import_time:.3f}s)", details
            else:
                return False, f"Performance issues (compute: {computation_time:.3f}s, import: {import_time:.3f}s)", details
    ...
    def print_summary(self, summary: ValidationSummary):
        ...
        if summary.overall_status == "EXCELLENT":
            ...
            print("⚠️  Setup needs improvement. Address failed checks before proceeding.")
        else:
