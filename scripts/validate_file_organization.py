...
def validate_file_organization() -> Tuple[bool, List[str], List[str]]:
    ...
    for item in project_root.iterdir():
        if item.name.startswith('.') and item.name not in {'.gitignore', '.gitattributes', '.pre-commit-config.yaml', '.github', '.git'}:
            continue
        ...
        if item.is_file():
            ...
            if item.name not in ALLOWED_ROOT_DIRS:
                ...
                if any(pattern in item.name.lower() for pattern in ['test', 'temp', 'tmp', 'env', 'venv']):
                    if 'test' in item.name.lower():
                        suggestions.append(f"Move {item.name}/ contents to tests/ or delete if obsolete")
                    else:
                        suggestions.append(f"DELETE {item.name}/ (temporary/environment directory)")
                else:
                    suggestions.append(f"Review {item.name}/ and move to appropriate location")
