...
class SDKCLIManager:
    ...
    def list_languages(self) -> None:
        ...
        for lang, config in self.generator.languages.items():
            ...
            enabled = self.config.get("languages", {}).get(lang, {}).get("enabled", True)
        ...
        if missing_tools:
            ...
            for tool in missing_tools:
                if tool == "OpenAPI Generator":
                    self.console.print("- OpenAPI Generator: npm install -g @openapitools/openapi-generator-cli")
                elif tool == "Node.js":
    ...
    def generate_sdks(self, languages: Optional[List[str]] = None, 
                     validate_only: bool = False) -> Dict[str, bool]:
        ...
        with Progress(
            ...
            try:
                ...
                progress.update(task, advance=1, description="✅ OpenAPI spec validated")
            except Exception as e:
                progress.update(task, advance=1, description=f"❌ OpenAPI spec validation failed: {e}")
                return {}
            ...
            for lang in languages:
                if lang not in self.generator.languages:
                    self.console.print(f"[yellow]⚠️  Skipping unsupported language: {lang}[/yellow]")
                    continue
                ...
                try:
                    ...
                    if success:
                        progress.update(task, advance=1, description=f"✅ {lang.title()} SDK generated")
                        # Run post-generation steps
                        self._run_post_generation_steps(lang)
                    else:
                        progress.update(task, advance=1, description=f"❌ {lang.title()} SDK generation failed")
                        
                except Exception as e:
                    results[lang] = False
                    progress.update(task, advance=1, description=f"❌ {lang.title()} SDK failed: {str(e)[:50]}")
    ...
    def _run_post_generation_steps(self, language: str) -> None:
        ...
        scripts = self.config.get("post_generation", {}).get("scripts", {}).get(language, [])
        ...
        for script in scripts:
            try:
                ...
                if result.returncode == 0:
                    self.console.print(f"[green]✅ Post-generation script completed: {script}[/green]")
                else:
                    self.console.print(f"[yellow]⚠️  Post-generation script warning: {script}[/yellow]")
                    self.console.print(f"[dim]{result.stderr}[/dim]")
                    
            except subprocess.TimeoutExpired:
                self.console.print(f"[red]❌ Post-generation script timed out: {script}[/red]")
            except Exception as e:
                self.console.print(f"[red]❌ Post-generation script failed: {script} - {e}[/red]")
    ...
    def _display_generation_summary(self, results: Dict[str, bool]) -> None:
        ...
        for lang, success in results.items():
            ...
            package_name = self.generator.languages.get(lang, {}).get("package_name", "N/A")
        ...
        if successful:
            ...
            self.console.print("3. Validate: pynomaly sdk validate --language <language>")
    ...
    def test_sdk(self, language: str) -> bool:
        ...
        if not sdk_dir.exists():
            self.console.print(f"[red]❌ SDK for {language} not found. Generate it first.[/red]")
            return False
        
        self.console.print(f"[blue]🧪 Running tests for {language.title()} SDK...[/blue]")
        ...
        if language not in test_commands:
            self.console.print(f"[yellow]⚠️  No test command configured for {language}[/yellow]")
            return False
        ...
        try:
            with Progress(
                ...
                if result.returncode == 0:
                    progress.update(task, advance=1, description=f"✅ {language.title()} tests passed")
                    self.console.print(f"[green]✅ All tests passed for {language.title()} SDK![/green]")
                    return True
                else:
                    progress.update(task, advance=1, description=f"❌ {language.title()} tests failed")
                    self.console.print(f"[red]❌ Tests failed for {language.title()} SDK[/red]")
                    self.console.print(f"[dim]{result.stdout}[/dim]")
                    self.console.print(f"[dim]{result.stderr}[/dim]")
                    return False
                    
        except subprocess.TimeoutExpired:
            self.console.print(f"[red]❌ Tests timed out for {language.title()} SDK[/red]")
            return False
        except Exception as e:
            self.console.print(f"[red]❌ Error running tests for {language.title()} SDK: {e}[/red]")
            return False
    ...
    def validate_sdk(self, language: str) -> bool:
        ...
        with Progress(
            ...
            for requirement in requirements:
                ...
                progress.update(task, advance=1, description=f"{status} {requirement['name']}")
    ...
    def _check_requirement(self, language: str, requirement: Dict[str, Any]) -> bool:
        ...
        if requirement_name == "authentication_support":
            ...
            return self._check_files_exist(sdk_dir, ["*error*", "*exception*", "*fault*"])
    ...
    def publish_sdk(self, language: str, registry: Optional[str] = None, 
        ...
        if not self.validate_sdk(language):
            self.console.print(f"[red]❌ SDK validation failed. Cannot publish.[/red]")
            return False
        
        # Get registry configuration
        registries = self.config.get("ci_cd", {}).get("registries", {}).get(language, [])
        
        if not registries:
            self.console.print(f"[yellow]⚠️  No registries configured for {language}[/yellow]")
            return False
        
        # Use specified registry or default to first one
        if registry:
            target_registry = next((r for r in registries if r["name"].lower() == registry.lower()), None)
        else:
            target_registry = registries[0]
        
        if not target_registry:
            self.console.print(f"[red]❌ Registry '{registry}' not found for {language}[/red]")
            return False
        
        registry_name = target_registry["name"]
        
        if dry_run:
            self.console.print(f"[blue]🚀 [DRY RUN] Would publish {language.title()} SDK to {registry_name}[/blue]")
            return True
        
        self.console.print(f"[blue]🚀 Publishing {language.title()} SDK to {registry_name}...[/blue]")
        ...
        if language not in publish_commands:
            self.console.print(f"[yellow]⚠️  No publish command configured for {language}[/yellow]")
            return False
        ...
        try:
            with Progress(
                ...
                if result.returncode == 0:
                    progress.update(task, advance=1, description=f"✅ {language.title()} SDK published")
                    self.console.print(f"[green]🎉 {language.title()} SDK published successfully to {registry_name}![/green]")
                    return True
                else:
                    progress.update(task, advance=1, description=f"❌ {language.title()} SDK publish failed")
                    self.console.print(f"[red]❌ Failed to publish {language.title()} SDK[/red]")
                    self.console.print(f"[dim]{result.stderr}[/dim]")
                    return False
                    
        except subprocess.TimeoutExpired:
            self.console.print(f"[red]❌ Publish timed out for {language.title()} SDK[/red]")
            return False
        except Exception as e:
            self.console.print(f"[red]❌ Error publishing {language.title()} SDK: {e}[/red]")
            return False
    ...
    def show_sdk_info(self, language: str) -> None:
        ...
        # Features
        features_config = self.config.get("languages", {}).get(language, {}).get("features", {})
        if features_config:
        ...
        code_files = len(list(sdk_dir.rglob("*.py"))) if language == "python" else len(list(sdk_dir.rglob("*")))
    ...
    def _add_directory_to_tree(self, tree_node, directory: Path, max_depth: int = 2, current_depth: int = 0):
            ...
            for item in items[:10]:  # Limit to first 10 items
                if item.is_dir():
                    ...
                    if current_depth < max_depth - 1:
                        self._add_directory_to_tree(dir_node, item, max_depth, current_depth + 1)
                else:
...
@click.option("--validate-only", is_flag=True, help="Only validate the OpenAPI spec without generating")
def generate(languages, all, validate_only):
    ...
    if all:
        ...
        manager.console.print("[blue]No languages specified. Use --all for all languages or specify with -l[/blue]")
        return
...
@cli.command()
@click.option("--output", "-o", default="sdk_status.json", help="Output file for status report")
def status(output):
    ...
    for language in manager.generator.languages.keys():
        ...
        if sdk_dir.exists():
            ...
            status_report["languages"][language] = {
                "exists": True,
                "last_modified": datetime.fromtimestamp(sdk_dir.stat().st_mtime).isoformat(),
    ...
    # Generate summary
    existing_sdks = sum(1 for lang_status in status_report["languages"].values() if lang_status.get("exists"))
    passing_tests = sum(1 for lang_status in status_report["languages"].values() if lang_status.get("test_passed"))
    passing_validation = sum(1 for lang_status in status_report["languages"].values() if lang_status.get("validation_passed"))
