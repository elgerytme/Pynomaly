...
def main():
    ...
    start_parser.add_argument('--config', '-c', default='config/monitoring/performance_alerts.json',
    ...
    test_parser.add_argument('--severity', '-s', choices=['low', 'medium', 'high', 'critical'],
                            default='medium', help='Alert severity')
    
    # Add metric
    metric_parser = subparsers.add_parser('add-metric', help='Add test metric')
    metric_parser.add_argument('--type', '-t', choices=[m.value for m in MetricType],
                              required=True, help='Metric type')
    metric_parser.add_argument('--value', '-v', type=float, required=True, help='Metric value')
    metric_parser.add_argument('--tags', help='Tags as JSON string')
    
    # List alerts
    alerts_parser = subparsers.add_parser('alerts', help='List alerts')
    alerts_parser.add_argument('--active', '-a', action='store_true', help='Show only active alerts')
    alerts_parser.add_argument('--history', '-h', type=int, default=24, help='History hours')
    
    # Export metrics
    export_parser = subparsers.add_parser('export', help='Export metrics')
    export_parser.add_argument('--format', '-f', choices=['prometheus', 'json'], default='json',
                              help='Export format')
...
def add_metric(metric_type: str, value: float, tags_json: str = None):
    ...
    print(f"✅ Metric added successfully")
...
def list_alerts(active_only: bool, history_hours: int):
    ...
    for alert in alerts:
        ...
        if alert.resolved_at:
            duration = f" ({(alert.resolved_at - alert.triggered_at).total_seconds():.1f}s)"
        
        print(f"{status} {alert.severity.value.upper()} - {alert.message}")
        print(f"  📅 {alert.triggered_at.strftime('%Y-%m-%d %H:%M:%S')}{duration}")
        print(f"  🏷️  {alert.metric_type.value} | {alert.current_value} > {alert.threshold_value}")
        if alert.tags:
