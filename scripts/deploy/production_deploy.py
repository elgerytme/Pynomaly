...
class ProductionDeploymentOrchestrator:
                ...
                if os.path.exists("bandit_report.json"):
                    with open("bandit_report.json") as f:
                        ...
                        if report.get("results", []):
                            high_severity = [r for r in report["results"] if r.get("issue_severity") == "HIGH"]
                            if high_severity:
                                self.logger.error(f"High severity security issues found: {len(high_severity)}")
                                return False
    ...
    async def _integration_tests(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["docker-compose", "-f", "deploy/production/docker-compose.test.yml", "down", "-v"],
                capture_output=True
            )
    ...
    async def _health_check(self, environment: str) -> bool:
        ...
        self.logger.error(f"Health check failed for {environment} after {max_retries} attempts")
        return False
