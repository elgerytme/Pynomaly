        ...
        if not self.config.dry_run:
            try:
                ...
                if self.config.image_tag in result.stdout:
                    logger.info(f"✓ Docker image available: pynomaly:{self.config.image_tag}")
                else:
                    logger.warning(f"⚠ Docker image not found locally: pynomaly:{self.config.image_tag}")
                    self.warnings.append(f"Docker image not found: pynomaly:{self.config.image_tag}")
            except (subprocess.CalledProcessError, FileNotFoundError):
...
"""
    ...
    def verify_deployment(self) -> Dict:
        ...
        try:
            ...
            # Check services
            result = subprocess.run([
                "kubectl", "get", "services", "-n", self.config.namespace, "-o", "json"
            ], capture_output=True, text=True, check=True)
            ...
            # Check ingress
            result = subprocess.run([
                "kubectl", "get", "ingress", "-n", self.config.namespace, "-o", "json"
            ], capture_output=True, text=True, check=True)
...
def main():
    ...
    parser.add_argument("--namespace", default="pynomaly-production", help="Kubernetes namespace")
    ...
    parser.add_argument("--monitoring-domain", default="monitoring.pynomaly.ai", help="Monitoring domain")
    parser.add_argument("--admin-email", default="admin@pynomaly.ai", help="Admin email")
    parser.add_argument("--image-tag", default="production-latest", help="Docker image tag")
    parser.add_argument("--storage-class", default="gp3-encrypted", help="Storage class")
    parser.add_argument("--node-selector", default="ml", help="Node selector for workloads")
    parser.add_argument("--replicas-api", type=int, default=3, help="API replicas")
    parser.add_argument("--replicas-worker", type=int, default=2, help="Worker replicas")
    
    # Control options
    parser.add_argument("--dry-run", action="store_true", help="Dry run mode")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose logging")
    parser.add_argument("--output", type=Path, help="Output file for deployment results")
