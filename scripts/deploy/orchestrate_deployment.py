...
class DeploymentOrchestrator:
    ...
    async def execute_command(self, command: str, cwd: Optional[Path] = None, timeout: int = 300) -> tuple[int, str, str]:
        ...
        try:
            ...
            if return_code == 0:
                ...
                self.log(f"Command failed with exit code {return_code}: {command}", "ERROR")
                if stderr_str.strip():
    ...
    async def _validate_tools(self) -> bool:
        ...
        for tool, version_cmd in tools:
            return_code, _, _ = await self.execute_command(f"{tool} {version_cmd}", timeout=30)
            if return_code != 0:
    ...
    async def _validate_aws_access(self) -> bool:
        ...
        try:
            ...
            return_code, _, _ = await self.execute_command("aws sts get-caller-identity", timeout=30)
            if return_code != 0:
    ...
    async def _validate_terraform_state(self) -> bool:
        ...
        try:
            terraform_dir = self.project_root / "infrastructure" / "terraform" / "production"
    ...
    async def _validate_kubernetes_access(self) -> bool:
        ...
        try:
            ...
            return_code, _, _ = await self.execute_command("kubectl config current-context", timeout=30)
            if return_code != 0:
            ...
            return_code, _, _ = await self.execute_command("kubectl cluster-info", timeout=30)
            if return_code != 0:
    ...
    async def _validate_docker_registry(self) -> bool:
        ...
        try:
            ...
            return_code, _, _ = await self.execute_command(f"docker info", timeout=30)
            if return_code != 0:
                self.log("Docker daemon not running", "ERROR")
                return False
            
            # Try to pull a small image to test registry access
            return_code, _, _ = await self.execute_command("docker pull alpine:latest", timeout=120)
            if return_code != 0:
    ...
    async def _generate_terraform_vars(self):
        ...
        tf_vars = {
            ...
            'domain_name': self.config.get('environment', {}).get('domain', 'pynomaly.ai'),
            'node_desired_capacity': self.config.get('resources', {}).get('api', {}).get('replicas', {}).get('initial', 3),
            'node_max_capacity': self.config.get('resources', {}).get('api', {}).get('replicas', {}).get('max', 10),
            'db_instance_class': 'db.t3.medium' if self.environment != 'production' else 'db.r5.large',
            'redis_instance_type': 'cache.t3.medium' if self.environment != 'production' else 'cache.r5.large',
        }
        
        # Write terraform.tfvars
        tfvars_content = "\n".join([f'{key} = "{value}"' for key, value in tf_vars.items()])
    ...
    async def _save_terraform_outputs(self):
        ...
        if return_code == 0:
            outputs_file = self.project_root / "config" / "deployment" / f"{self.environment}-outputs.json"
    ...
    async def _install_cluster_components(self) -> bool:
        """Install essential cluster components"""
        components = [
            ('AWS Load Balancer Controller', self._install_aws_load_balancer_controller),
    ...
    async def _install_aws_load_balancer_controller(self) -> bool:
        ...
        try:
            ...
            await self.execute_command("helm repo add eks https://aws.github.io/eks-charts")
            ...
            return_code, _, stderr = await self.execute_command(install_command, timeout=300)
    ...
    async def _install_cluster_autoscaler(self) -> bool:
        ...
        try:
            ...
            autoscaler_yaml = self.project_root / "deploy" / "kubernetes" / "cluster-autoscaler.yaml"
            
            return_code, _, stderr = await self.execute_command(
                f"kubectl apply -f {autoscaler_yaml}",
                timeout=120
            )
    ...
    async def _install_external_dns(self) -> bool:
        ...
        try:
            ...
            external_dns_yaml = self.project_root / "deploy" / "kubernetes" / "external-dns.yaml"
            
            return_code, _, stderr = await self.execute_command(
                f"kubectl apply -f {external_dns_yaml}",
                timeout=120
            )
    ...
    async def _install_cert_manager(self) -> bool:
        ...
        try:
            ...
            await self.execute_command("helm repo add jetstack https://charts.jetstack.io")
            await self.execute_command("helm repo update")
    ...
    async def deploy_application(self) -> bool:
        ...
        try:
            ...
            deployment_config = {
                ...
                'image_tag': self.config.get('container', {}).get('image_tag', 'latest'),
                'strategy': self.config.get('deployment', {}).get('strategy', 'rolling'),
                'skip_tests': False,
                'skip_build': False,
            ...
            try:
                ...
                deployment_script = self.project_root / "scripts" / "deploy" / "automated_deployment.py"
    ...
    async def _deploy_prometheus(self) -> bool:
        ...
        try:
            ...
            await self.execute_command("helm repo add prometheus-community https://prometheus-community.github.io/helm-charts")
            await self.execute_command("helm repo update")
            ...
            return_code, _, stderr = await self.execute_command(install_command, timeout=600)
            return return_code == 0
    ...
    async def _deploy_grafana(self) -> bool:
        ...
        try:
            ...
            dashboards_dir = self.project_root / "config" / "monitoring" / "grafana" / "dashboards"
            
            if dashboards_dir.exists():
                return_code, _, stderr = await self.execute_command(
                    f"kubectl apply -f {dashboards_dir} -n monitoring",
                    timeout=120
                )
                return return_code == 0
    ...
    async def _deploy_jaeger(self) -> bool:
        ...
        try:
            ...
            await self.execute_command("helm repo add jaegertracing https://jaegertracing.github.io/helm-charts")
            await self.execute_command("helm repo update")
    ...
    async def _deploy_fluentbit(self) -> bool:
        ...
        try:
            ...
            await self.execute_command("helm repo add fluent https://fluent.github.io/helm-charts")
            await self.execute_command("helm repo update")
...
def main():
    ...
    parser.add_argument('--config', type=Path, required=True, help='Deployment configuration file')
