...
class QualityGatesOrchestrator:
        ...
        for gate in self.gates:
            ...
            try:
                ...
                if not result.passed:
                    self.logger.error(f"Quality gate '{gate.name}' failed: {result.message}")
                    if gate.blocking:
                        overall_success = False
                else:
                    self.logger.info(f"Quality gate '{gate.name}' passed: {result.message}")
    ...
    async def _check_test_coverage(self, gate: QualityGate) -> QualityResult:
        ...
        try:
            ...
            passed = self._evaluate_condition(total_coverage, gate.threshold, gate.operator)
    ...
    async def _check_security_vulnerabilities(self, gate: QualityGate) -> QualityResult:
        ...
        try:
            ...
            passed = self._evaluate_condition(high_severity_count, gate.threshold, gate.operator)
    ...
    async def _check_code_quality(self, gate: QualityGate) -> QualityResult:
        ...
        try:
            ...
            passed = self._evaluate_condition(quality_score, gate.threshold, gate.operator)
    ...
    async def _check_performance_regression(self, gate: QualityGate) -> QualityResult:
        ...
        try:
            ...
            passed = self._evaluate_condition(regression_percentage, gate.threshold, gate.operator)
    ...
    async def _check_build_success(self, gate: QualityGate) -> QualityResult:
        ...
        try:
            ...
            docker_result = subprocess.run(
                ["docker", "build", "-f", "deploy/production/Dockerfile.api", "-t", "test-build", "."],
                capture_output=True, text=True, timeout=gate.timeout
            )
            ...
            passed = self._evaluate_condition(build_success, gate.threshold, gate.operator)
    ...
    def _evaluate_condition(self, actual: Union[int, float], threshold: Union[int, float], operator: str) -> bool:
    ...
    async def _generate_report(self):
        ...
        report = {
            ...
            "overall_success": all(r.passed or not r.gate.blocking for r in self.results),
            "total_gates": len(self.gates),
            "passed_gates": len([r for r in self.results if r.passed]),
            "failed_gates": len([r for r in self.results if not r.passed]),
            "blocking_failures": len([r for r in self.results if not r.passed and r.gate.blocking]),
            "results": [
                {
