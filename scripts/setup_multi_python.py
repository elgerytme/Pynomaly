...
class MultiPythonManager:
    ...
    def check_python_build_deps(self) -> bool:
        ...
        if self.system_info["system"] == "Linux":
            ...
            deps = ["gcc", "make", "zlib1g-dev", "libbz2-dev", "libreadline-dev", 
            ...
            if missing:
                ...
                logger.info("Install with: sudo apt-get install build-essential zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm libncurses5-dev")
                return False
    ...
    def install_pyenv(self) -> bool:
        ...
        if self.system_info["system"] in ["Linux", "Darwin"]:
            try:
                ...
                logger.info('command -v pyenv >/dev/null || export PATH="$PYENV_ROOT/bin:$PATH"')
            ...
            logger.info("Install pyenv-win manually: https://github.com/pyenv-win/pyenv-win")
            return False
    ...
    def list_available_versions(self) -> List[str]:
        ...
        try:
            ...
            for line in result.stdout.splitlines():
                version = line.strip()
                if any(version.startswith(v.split('.')[0] + '.' + v.split('.')[1]) 
    ...
    def install_python_version(self, version: str) -> bool:
        ...
        try:
            ...
            # Install the version
            install_result = subprocess.run(["pyenv", "install", version], 
                                          capture_output=True, text=True, 
            ...
            try:
                ...
                if pip_path.exists():
                    ...
                    subprocess.run([
                        ...
                        "pytest", "pytest-cov", "hypothesis", "tox"
                    ], capture_output=True, check=True)
                    
                    logger.info(f"✓ Basic packages installed in {env_name}")
                
            except subprocess.CalledProcessError as e:
                logger.error(f"✗ Failed to create environment for Python {version}: {e}")
    ...
    def generate_report(self, test_results: Dict) -> str:
        ...
        report_data = {
            ...
            "summary": {
                ...
                "successful_installs": sum(1 for v, r in test_results.items() if r.get("status") == "healthy"),
                "failed_installs": sum(1 for v, r in test_results.items() if r.get("status") != "healthy"),
            }
        }
...
"""
        ...
        for version, info in PYTHON_VERSIONS.items():
            ...
            status_symbol = "✓" if status == "healthy" else "✗" if status == "issues" else "?"
...
"""
    ...
    def setup_all(self, install_missing: bool = True) -> bool:
        ...
        successful = sum(1 for r in test_results.values() if r.get("status") == "healthy")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Multi-Version Python Setup for Pynomaly")
    parser.add_argument("--check", action="store_true", help="Check current setup status")
    parser.add_argument("--install", action="store_true", help="Install missing components")
    parser.add_argument("--test", action="store_true", help="Test existing environments")
    parser.add_argument("--clean", action="store_true", help="Clean up existing environments")
