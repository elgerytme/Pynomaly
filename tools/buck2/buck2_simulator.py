...
class Buck2Simulator:
    ...
    def _create_mock_artifact(self, target: str) -> Path:
        ...
        if "test" in target_name:
            ...
            artifact_path.write_text(f'<testsuites><testsuite name="{target_name}"/></testsuites>')
            ...
            artifact_path.write_text(f"#!/usr/bin/env python3\n# Mock {target_name} binary")
        else:
