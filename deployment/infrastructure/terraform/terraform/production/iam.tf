# IAM roles and policies for Pynomaly production infrastructure
...
# Custom policy for application access to AWS services
resource "aws_iam_policy" "app_policy" {
  name        = "${var.cluster_name}-app-policy"
  description = "Policy for Pynomaly application to access AWS services"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/eks/pynomaly-production/*"
        ]
      }
    ]
  })
...
resource "aws_iam_role" "app_service_account" {
  name = "${var.cluster_name}-app-service-account-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:pynomaly-prod:pynomaly-app"
