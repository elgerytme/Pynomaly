            ...
            for file in module_files:
                ...
                for test_file in test_path.rglob("*.py"):
                    if test_file.name.startswith("test_"):
                        ...
                        if (base_name in test_file.name or 
                            module_path.replace("/", "_") in test_file.name or
                            any(part in test_file.name for part in module_path.split("/"))):
                            corresponding_tests.append(str(test_file.relative_to(test_path)))
...
def print_detailed_analysis():
    ...
    for module_path, data in analysis.items():
        coverage_pct = (data["tested_files"] / data["total_files"] * 100) if data["total_files"] > 0 else 0
...
def identify_missing_tests():
    ...
    for module_path, data in analysis.items():
        missing_tests = [file for file, tests in data["test_coverage"].items() if not tests]
