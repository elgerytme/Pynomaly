...
export class DashboardStateManager {
    ...
    switch (action.type) {
      ...
      case "REMOVE_ANOMALY":
        return this.updateState(
          state,
          "data.anomalies",
          state.data.anomalies.filter(
            (anomaly) => anomaly.id !== action.payload,
          ),
        );
  ...
  actions = {
    ...
    setConfidenceRange: (min, max) => ({
      type: "SET_CONFIDENCE_RANGE",
      payload: { min, max },
    }),
    setFeatureFilter: (features) => ({
      type: "SET_FEATURE_FILTER",
      payload: features,
    }),
    setAlgorithmFilter: (algorithms) => ({
      type: "SET_ALGORITHM_FILTER",
      payload: algorithms,
    }),
    ...
    setHeatmapColorScheme: (scheme) => ({
      type: "SET_HEATMAP_COLOR_SCHEME",
      payload: scheme,
    }),
    setHeatmapAggregation: (aggregation) => ({
      type: "SET_HEATMAP_AGGREGATION",
      payload: aggregation,
    }),
    ...
    getFilteredAnomalies: () => {
      ...
      return anomalies.filter((anomaly) => {
        ...
        if (
          ...
          anomaly.timestamp < filters.dateRange.start
        )
          return false;
        if (filters.dateRange.end && anomaly.timestamp > filters.dateRange.end)
          return false;

        // Confidence filter
        if (
          anomaly.confidence < filters.confidence.min ||
          anomaly.confidence > filters.confidence.max
        )
          return false;
        ...
        if (
          ...
          !filters.features.includes(anomaly.feature)
        )
          return false;
        ...
        if (
          ...
          !filters.algorithms.includes(anomaly.algorithm)
        )
          return false;
