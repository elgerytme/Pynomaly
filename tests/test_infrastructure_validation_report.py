...
async def test_async_repositories():
    ...
    try:
        from pynomaly.infrastructure.repositories.async_wrappers import AsyncDetectorRepositoryWrapper
        ...
        return True, f"Async operations functional (count: {count}, all: {len(all_detectors)})"
...
def test_security_infrastructure():
    ...
    try:
        ...
        return True, f"Security exceptions functional"
    except Exception as e:
        return False, str(e)
...
def test_database_infrastructure():
    ...
    try:
        from tests.conftest_database import test_database_url
...
async def main():
    ...
    for name, test_func in components:
        if asyncio.iscoroutine(test_func):
            ...
            success, message = test_func[0] if isinstance(test_func, tuple) else test_func
