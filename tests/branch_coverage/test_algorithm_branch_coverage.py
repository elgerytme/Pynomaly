...
class TestAlgorithmSelectionBranches:
    ...
    def test_algorithm_availability_branches(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            # Mock version check
            with patch.object(PyODAdapter, '_check_algorithm_compatibility') as mock_check:
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            sample_data = pd.DataFrame({
                'feature1': np.concatenate([
                    ...
                    np.random.normal(5, 1, 50)    # Anomalous data (5% contamination)
                ]),
...
class TestAlgorithmSpecificBranches:
    ...
    def test_lof_algorithm_branches(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            mock_model.decision_function.return_value = np.array([0.1, 0.9, 0.2, 0.8, 0.3])
...
class TestDetectionBranches:
    ...
    def test_score_normalization_branches(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            mock_model.decision_function.return_value = np.array([0.1, 0.5, 0.9, 0.2, 0.8])
            ...
            mock_model.decision_function.return_value = np.array([-1000, -500, 0, 500, 1000])
            ...
            mock_model.decision_function.return_value = np.array([0.5, 0.5, 0.5, 0.5, 0.5])
    ...
    def test_threshold_calculation_branches(self):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.importlib.import_module') as mock_import:
            ...
            for contamination in contamination_rates:
                ...
                scores[n_anomalies:] = np.random.uniform(0.0, 0.7, n_samples - n_anomalies)
                
                mock_model.score_samples.return_value = -scores  # sklearn uses negative scores
