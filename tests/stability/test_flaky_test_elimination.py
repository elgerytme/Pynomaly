...
class TimingManager:
    ...
    def wait_for_condition(self, condition_func: Callable, timeout: float = None, 
                          polling_interval: float = None) -> bool:
...
class TestFlakyTestElimination:
    ...
    def test_flaky_test_detection_and_remediation(self, test_stabilizer):
        ...
        with test_stabilizer.stabilized_test("potentially_flaky"):
            ...
            timing_manager = test_stabilizer.timing_manager
            result = timing_manager.wait_for_condition(lambda: condition_met, timeout=1.0)
            assert result is True
    ...
    def test_retry_decorator_integration(self, test_stabilizer):
        ...
        if self.__class__._attempt_counter < 3:
            raise AssertionError(f"Simulated failure on attempt {self.__class__._attempt_counter}")
    ...
    def test_stabilization_performance_impact(self, test_stabilizer):
        ...
        assert overhead_ratio < 10.0, f"Stabilization overhead too high: {overhead_ratio:.2f}x"
    ...
    def test_complex_flaky_scenario_elimination(self, test_stabilizer):
        ...
        with test_stabilizer.stabilized_test("complex_scenario"):
            ...
            test_stabilizer.resource_manager.register_resource('threads', creator_thread)
    ...
    def test_stabilization_metrics_collection(self, test_stabilizer):
        ...
        for i in range(5):
            with test_stabilizer.stabilized_test(f"metrics_test_{i}"):
                ...
                if i < 2:  # First two tests need retries
                    try:
                        @test_stabilizer.retry_manager.retry_with_stabilization(max_retries=1)
                        def might_fail():
                ...
                test_stabilizer.resource_manager.register_resource('temp_objects', temp_obj)
        ...
        stabilization_report = {
            ...
            'average_retries_per_test': metrics['retries_performed'] / metrics['tests_stabilized'],
