...
def test_complete_workflow_with_adapters():
    ...
    try:
        ...
        normal_data = np.random.multivariate_normal(
            mean=[0, 0, 0], cov=np.eye(3), size=80
        )

        # Anomalous data (20 samples)
        anomaly_data = np.random.multivariate_normal(
            mean=[3, 3, 3], cov=np.eye(3) * 0.5, size=20
        )
        ...
        print(
            f"✓ Test dataset created: {test_dataset.n_samples} samples x {test_dataset.n_features} features"
        )
        ...
        try:
            ...
            print(
                f"     Score range: {min(score_values):.3f} to {max(score_values):.3f}"
            )
        ...
        try:
            ...
            exec_time = result.execution_time_ms or result.metadata.get(
                "execution_time_ms", 0
            )
