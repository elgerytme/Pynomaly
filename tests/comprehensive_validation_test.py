...
4.0,5.0,normal"""
    ...
    def test_core_imports(self) -> None:
        ...
        core_imports = [
            ("container", "from pynomaly.infrastructure.config.container import Container"),
            ("entities", "from pynomaly.domain.entities import Dataset, Detector"),
            ("sklearn_adapter", "from pynomaly.infrastructure.adapters.sklearn_adapter import SklearnAdapter"),
            ("pyod_adapter", "from pynomaly.infrastructure.adapters.pyod_adapter import PyODAdapter"),
    ...
    def test_integration_workflow(self, test_file: Path) -> None:
        ...
        try:
            ...
            cmd = [
                ...
                str(test_file),
                "--output", str(Path(self.test_data_dir) / "results.json")
            ]
    ...
    def run_validation(self) -> Dict[str, Any]:
        ...
        for category, stats in results['by_category'].items():
            print(f"  {category}: {stats['success_rate']:.1f}% ({stats['passed']}/{stats['total']})")
        ...
        if failed_tests:
            ...
            for test in failed_tests[:10]:  # Show first 10 failures
                print(f"  - {test.category}.{test.test_name}: {test.error_message[:80]}...")
        
        return {
            "success_rates": results,
            "detailed_results": [(r.category, r.test_name, r.passed, r.error_message) for r in self.results]
        }
