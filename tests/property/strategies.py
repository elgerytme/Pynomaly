...
def contamination_rate_strategy(draw):
    ...
    rate = draw(st.floats(min_value=0.0, max_value=0.5, exclude_min=False, exclude_max=False))
...
def confidence_interval_strategy(draw):
    ...
    lower = draw(st.floats(min_value=0.0, max_value=0.95, allow_nan=False, allow_infinity=False))
...
def detector_strategy(draw):
    ...
    algorithm = draw(st.sampled_from([
        ...
        "pyod_abod", "pyod_knn", "pyod_lof", "pyod_ocsvm"
    ]))
    contamination = draw(contamination_rate_strategy())
    
    # Optional hyperparameters
    hyperparameters = draw(st.one_of(
        st.none(),
        st.dictionaries(
...
def detection_result_strategy(draw):
    ...
    n_anomalies = draw(st.integers(min_value=0, max_value=min(10, len(dataset.features))))
    anomaly_indices = draw(st.lists(
...
def contamination_strategy(draw):
    ...
    return draw(st.floats(min_value=0.001, max_value=0.5, exclude_min=False, exclude_max=False))
