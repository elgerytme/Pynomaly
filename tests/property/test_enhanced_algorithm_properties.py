...
@st.composite
def anomaly_datasets(draw, min_samples=10, max_samples=1000, min_features=1, max_features=20):
    ...
    if n_anomalies > 0:
        ...
        labels = np.concatenate([np.ones(len(normal_data)), -np.ones(len(anomaly_data))])
    else:
        data = normal_data
...
class TestEnhancedAlgorithmProperties:
    ...
    @settings(max_examples=20, deadline=20000, suppress_health_check=[HealthCheck.too_slow])
    ...
    def test_robustness_properties(self, original_data, noise_level):
        ...
        with warnings.catch_warnings():
            ...
            top_normal_original = np.argsort(scores_original)[-int(0.1 * len(scores_original)):]
            top_normal_perturbed = np.argsort(scores_perturbed)[-int(0.1 * len(scores_perturbed)):]
...
class TestDomainSpecificProperties:
    ...
    def test_clustered_data_properties(self, n_clusters, cluster_size, outlier_fraction):
    ...
    @given(
        data_dimension=st.integers(2, 15),
    ...
    def test_high_dimensional_properties(self, data_dimension, n_samples):
        ...
        data = np.random.multivariate_normal(
            mean=np.zeros(data_dimension),
            cov=np.eye(data_dimension),
            size=n_samples
        )
        ...
        for idx in outlier_indices:
            ...
            extreme_dims = np.random.choice(data_dimension, size=max(1, data_dimension // 3), replace=False)
        ...
        with warnings.catch_warnings():
            ...
            true_outlier_detection = len(set(outlier_indices) & set(detected_outlier_indices))
