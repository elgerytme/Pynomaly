...
class TestDomainServiceProperties:
    ...
    def test_threshold_calculator_properties(self, scores: np.ndarray, contamination: float):
        ...
        if contamination < 0.4:  # Avoid edge cases
            ...
            higher_threshold = calculator.calculate_threshold(scores, higher_contamination)
            assert higher_threshold <= threshold + 1e-6  # Small tolerance for numerical precision
...
class TestBusinessRuleProperties:
    ...
    def test_detection_consistency(self, dataset: Dataset, detector: Detector):
        ...
        expected_max_anomalies = int(n_samples * detector.contamination.value) + 2  # Allow tolerance
