...
from pynomaly.domain.exceptions import (
    DomainValidationError, InvalidContaminationRateError, InvalidAnomalyScoreError
)
...
def valid_contamination_rates(draw):
    ...
    return draw(st.floats(min_value=0.001, max_value=0.999, exclude_min=True, exclude_max=True))

@st.composite
def valid_anomaly_scores(draw):
    """Generate valid anomaly score values."""
    score = draw(st.floats(min_value=-10.0, max_value=10.0, allow_nan=False, allow_infinity=False))
...
def valid_detector_configs(draw):
    ...
    algorithm = draw(st.sampled_from([
        ...
        "EllipticEnvelope", "KNN", "AutoEncoder"
    ]))
...
class TestAnomalyScoreProperties:
    ...
    def test_anomaly_score_creation_properties(self, score_data):
        ...
        assert str(score_data["score"]) in str_repr or f"{score_data['score']:.4f}" in str_repr
    
    @given(
        score1=valid_anomaly_scores(),
        score2=valid_anomaly_scores()
    )
    ...
    def test_anomaly_score_aggregation_properties(self, scores):
        ...
        anomaly_scores = [
            AnomalyScore(value=s["score"], confidence=s["confidence"])
            for s in scores
        ]
...
class TestContaminationRateProperties:
    ...
    @given(
        rate1=valid_contamination_rates(),
        rate2=valid_contamination_rates()
    )
    @settings(max_examples=50)
...
class TestDatasetProperties:
    ...
    @given(
        data1=valid_dataset_data(),
        data2=valid_dataset_data()
    )
    @settings(max_examples=20)
    def test_dataset_comparison_properties(self, data1, data2):
        ...
        if data1["data"] != data2["data"] or data1["feature_names"] != data2["feature_names"]:
            assert (dataset1a.n_samples != dataset2.n_samples or 
                   dataset1a.n_features != dataset2.n_features or
                   dataset1a.feature_names != dataset2.feature_names)
...
class TestDetectorProperties:
    ...
    @given(
        config=valid_detector_configs(),
        training_data=valid_dataset_data()
    )
    @settings(max_examples=30)
    ...
    @given(
        ...
        metrics_data=st.dictionaries(
            keys=st.sampled_from(["precision", "recall", "f1_score", "accuracy", "auc"]),
    ...
    def test_detector_performance_properties(self, config, metrics_data):
        ...
        if detector.performance_metrics:
            avg_performance = sum(detector.performance_metrics.values()) / len(detector.performance_metrics)
...
class TestDetectionResultProperties:
    ...
    def test_detection_result_statistics_properties(self, predictions, score_range):
        ...
        if result.anomaly_count > 0:
            ...
            anomaly_score_values = [result.anomaly_scores[i].value for i in anomaly_indices]
            ...
            if len(anomaly_score_values) > 1:
                ...
                if normal_indices:
                    normal_score_values = [result.anomaly_scores[i].value for i in normal_indices]
    ...
    @settings(max_examples=20)
    def test_detection_result_comparison_properties(self, results_count, base_predictions):
