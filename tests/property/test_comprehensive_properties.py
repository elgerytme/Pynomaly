...
def contamination_rate_strategy(draw):
    ...
    value = draw(st.floats(min_value=0.001, max_value=0.5, allow_nan=False, allow_infinity=False))
...
def confidence_interval_strategy(draw):
    ...
    lower = draw(st.floats(min_value=0.0, max_value=0.8, allow_nan=False, allow_infinity=False))
    upper = draw(st.floats(min_value=lower, max_value=1.0, allow_nan=False, allow_infinity=False))
    confidence = draw(st.floats(min_value=0.5, max_value=0.99, allow_nan=False, allow_infinity=False))
    return ConfidenceInterval(lower=lower, upper=upper, confidence=confidence)


@st.composite
def dataset_strategy(draw, min_samples=1, max_samples=1000, min_features=1, max_features=50):
...
class TestDomainProperties:
    ...
    def test_detection_result_invariants(self, predictions: List[int], scores: List[AnomalyScore]):
...
class TestAlgorithmProperties:
    ...
    def test_algorithm_determinism(self, dataset: Dataset, algorithm: str):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_adapter:
            ...
            mock_model = Mock()
            mock_predictions = [0, 1] * (len(dataset.data) // 2) + [0] * (len(dataset.data) % 2)
            mock_scores = [0.1, 0.9] * (len(dataset.data) // 2) + [0.1] * (len(dataset.data) % 2)
    ...
    def test_algorithm_subset_consistency(self, dataset: Dataset, n_subsets: int):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_adapter:
            ...
            for i in range(n_subsets):
                ...
                mock_model = Mock()
                mock_predictions = [0, 1] * (len(subset_data) // 2) + [0] * (len(subset_data) % 2)
...
class TestDataTransformationProperties:
    ...
    def test_normalization_properties(self, data: np.ndarray):
        ...
        with patch('pynomaly.infrastructure.preprocessing.data_transformer.DataTransformer') as mock_transformer:
            ...
            def mock_normalize(input_data):
                ...
                normalized = (data_array - np.mean(data_array, axis=0)) / (np.std(data_array, axis=0) + 1e-8)
    ...
    def test_dimensionality_reduction_properties(self, data: np.ndarray, target_dimensions: int):
        ...
        with patch('pynomaly.infrastructure.preprocessing.data_transformer.DataTransformer') as mock_transformer:
            ...
            reduced_data = transformer.reduce_dimensions(data.tolist(), target_dimensions)
    ...
    def test_outlier_removal_properties(self, data: np.ndarray, contamination: float):
        ...
        with patch('pynomaly.infrastructure.preprocessing.data_transformer.DataTransformer') as mock_transformer:
            ...
            cleaned_data, kept_indices = mock_remove_outliers(data.tolist(), contamination)
...
class TestServiceProperties:
    ...
    def test_detection_service_properties(self, dataset: Dataset, algorithm: str):
        ...
        with patch('pynomaly.application.services.detection_service.DetectionService') as mock_service:
            ...
            assert result.processing_time < len(dataset.data) * 0.1  # Under 100ms per sample
    
    @given(
        st.lists(dataset_strategy(min_samples=5, max_samples=50, max_features=3), min_size=2, max_size=5),
        st.floats(0.05, 0.3)
    )
    def test_ensemble_service_properties(self, datasets: List[Dataset], contamination: float):
        """Test ensemble service properties."""
        with patch('pynomaly.application.services.ensemble_service.EnsembleService') as mock_service:
            service = mock_service.return_value
            ...
            def mock_ensemble_predict(detector_ids, data, method="majority_vote"):
                ...
                ensemble_scores = [AnomalyScore(np.random.random()) for _ in range(n_samples)]
            ...
            for method in ["majority_vote", "unanimous"]:
                result = service.ensemble_predict(detector_ids, test_data, method=method)
    ...
    def test_cross_validation_properties(self, dataset: Dataset, n_folds: int):
        ...
        with patch('pynomaly.application.services.validation_service.ValidationService') as mock_service:
            service = mock_service.return_value
...
class TestRobustnessProperties:
    ...
    def test_input_validation_robustness(self, input_data: List[float], contamination: float):
    ...
    def test_scalability_properties(self, n_samples: int, n_features: int, contamination: float):
        ...
        expected_memory_mb = (n_samples * n_features * 8) / (1024 * 1024)  # 8 bytes per float64
        ...
        assert expected_anomalies >= 1 if n_samples >= 10 else True  # At least 1 anomaly for reasonable datasets
        
        # Feature dimensions should be manageable
        if n_features > 1000:
            # High-dimensional data might need special handling
            assert n_samples >= n_features  # More samples than features generally preferred