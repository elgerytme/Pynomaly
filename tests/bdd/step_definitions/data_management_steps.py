...
def create_windows1252_csv(temp_dir):
    ...
    data = pd.DataFrame({
        ...
        'text_feature': ['café', 'résumé', 'naïve']  # These will be encoded differently
    })
...
def load_csv_with_encoding():
    ...
    loader = pytest.loaders['csv_loader']
    pytest.loaded_windows1252 = loader.load(str(pytest.windows1252_csv_path), encoding='windows-1252')
...
def create_polars_dataset(temp_dir):
    ...
    data = pd.DataFrame({
        f'feature_{i}': np.random.random(10000) for i in range(10)
    })
...
def create_spark_dataset(temp_dir):
    ...
    for i in range(3):
        data = pd.DataFrame({
            f'feature_{j}': np.random.random(1000) for j in range(5)
        })
...
def verify_numeric_values():
    ...
    if hasattr(pytest, 'loaded_csv'):
        numeric_cols = pytest.loaded_csv.features.select_dtypes(include=[np.number]).columns
        assert len(numeric_cols) > 0, "Should have numeric columns"
        
        for col in numeric_cols:
            assert not pytest.loaded_csv.features[col].isnull().all(), f"Column {col} should have values"
...
def verify_special_characters():
    ...
    if hasattr(pytest, 'loaded_windows1252'):
        ...
        text_cols = pytest.loaded_windows1252.features.select_dtypes(include=['object']).columns
        if len(text_cols) > 0:
...
def verify_memory_efficient_loading():
    ...
    assert not hasattr(pytest, 'load_error'), f"Loading failed: {getattr(pytest, 'load_error', '')}"
...
def verify_complete_dataset():
    ...
    if hasattr(pytest, 'loaded_large_chunked'):
        ...
        assert len(pytest.loaded_large_chunked.columns) > 0, "Dataset should have columns"
...
def verify_missing_value_detection():
    ...
    if hasattr(pytest, 'validation_results'):
        assert 'missing_values' in pytest.validation_results, "Missing values should be tracked"
        assert isinstance(pytest.validation_results['missing_values'], (int, np.integer)), \
...
def verify_data_type_validation():
    ...
    if hasattr(pytest, 'validation_results'):
        assert 'data_types' in pytest.validation_results, "Data types should be validated"
        assert len(pytest.validation_results['data_types']) > 0, "Should have data type information"
...
def verify_outlier_detection():
    ...
    if hasattr(pytest, 'validation_results'):
        assert 'value_ranges' in pytest.validation_results, "Value ranges should be checked"
...
def verify_duplicate_detection():
    ...
    if hasattr(pytest, 'validation_results'):
        assert 'duplicate_rows' in pytest.validation_results, "Duplicate rows should be tracked"
        assert isinstance(pytest.validation_results['duplicate_rows'], (int, np.integer)), \
