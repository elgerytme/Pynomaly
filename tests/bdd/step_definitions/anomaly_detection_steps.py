...
from pynomaly.infrastructure.repositories import InMemoryDetectorRepository, InMemoryResultRepository
...
def clean_dataset():
    ...
    normal_data, _ = make_blobs(n_samples=90, centers=1, n_features=2, 
...
@given(parsers.parse("I have a dataset with {n_samples:d} samples and {n_features:d} features"))
def create_dataset(n_samples, n_features):
    ...
    normal_data, _ = make_blobs(n_samples=n_normal, centers=1, n_features=n_features,
                               random_state=42, cluster_std=1.0)
...
def verify_anomaly_percentage(percentage):
    ...
    if hasattr(pytest, 'dataset') and pytest.dataset.targets is not None:
        actual_percentage = (np.sum(pytest.dataset.targets == 1) / len(pytest.dataset.targets)) * 100
        assert abs(actual_percentage - percentage) < 5, f"Expected {percentage}% anomalies, got {actual_percentage:.1f}%"
...
def create_isolation_forest_default():
    ...
    pytest.detector = adapter.create_detector("isolation_forest", contamination=0.1, random_state=42)


@when(parsers.parse("I create a {algorithm} detector with {contamination:f} contamination"))
def create_detector_with_contamination(algorithm, contamination):
    ...
    pytest.detector = adapter.create_detector(algo_name, contamination=contamination, random_state=42)
...
def create_ensemble(algorithms):
    ...
    for algo in algorithm_list:
        ...
        detector = adapter.create_detector(algo_name, contamination=0.1, random_state=42)
...
def setup_pretrained_detector():
    ...
    adapter = pytest.system['sklearn_adapter']
    detector = adapter.create_detector("isolation_forest", contamination=0.1, random_state=42)
    
    # Train on some initial data
    training_data, _ = make_blobs(n_samples=100, centers=1, n_features=2, random_state=42)
...
# Validation steps
@then(parsers.parse("the detector should identify approximately {expected:d} anomalies"))
...
def verify_score_range():
    ...
    assert np.all(pytest.scores >= 0), f"Found negative scores: {pytest.scores[pytest.scores < 0]}"
    assert np.all(pytest.scores <= 1), f"Found scores > 1: {pytest.scores[pytest.scores > 1]}"
...
def verify_deterministic_results():
    ...
    assert np.array_equal(pytest.predictions, predictions2), "Predictions should be deterministic"
...
def verify_score_aggregation():
    ...
    assert np.all(np.isfinite(pytest.ensemble_scores)), "Ensemble scores should be finite"
...
def verify_confidence_intervals():
    ...
    anomaly_indices = np.where(pytest.ensemble_scores > np.percentile(pytest.ensemble_scores, 90))[0]
    assert len(anomaly_indices) > 0, "Should identify some anomalies for confidence intervals"
...
def verify_streaming_consistency():
    ...
    batch_scores = pytest.pretrained_detector.decision_function(pytest.streaming_data[:len(pytest.streaming_scores)])
    
    for i, (stream_score, batch_score) in enumerate(zip(pytest.streaming_scores, batch_scores)):
