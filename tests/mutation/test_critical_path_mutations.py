...
class ConstantMutator(MutationOperator):
    ...
    def __init__(self):
        super().__init__(
            "constant",
            "Mutates constants (numbers and strings)"
        )
...
class MutationTester:
    ...
    def test_mutant_with_original_tests(self, original_func: Callable, 
                                       mutant_code: str, 
...
class TestCriticalPathMutations:
    ...
    def test_anomaly_score_value_object_mutations(self, mutation_tester):
        ...
        test_cases = [
            ...
            {'args': (None, 0.5, 1.5), 'kwargs': {}, 'expected': 'error'},  # Invalid confidence
            {'args': (None, float('inf'), 0.5), 'kwargs': {}, 'expected': 'error'},  # Invalid score
        ]
        ...
        if total_mutants > 0:
            ...
            assert mutation_score >= 0.7, f"Mutation score too low: {mutation_score:.2%}"
    ...
    def test_contamination_rate_validation_mutations(self, mutation_tester):
        ...
        if total_mutants > 0:
            mutation_score = killed_mutants / total_mutants
            assert mutation_score >= 0.8, f"Validation mutation score too low: {mutation_score:.2%}"
    
    def test_algorithm_selection_logic_mutations(self, mutation_tester):
        """Test mutations in algorithm selection logic."""
        
        def select_best_algorithm(algorithms: List[str], performance_scores: Dict[str, float]) -> str:
            if not algorithms:
        ...
        test_cases = [
            ...
            {
                'args': (["IsolationForest", "LOF"], {"IsolationForest": 0.8, "LOF": 0.9}),
                'kwargs': {},
                'expected': "LOF"
            },
            {
        ...
        if total_mutants > 0:
            mutation_score = killed_mutants / total_mutants
            assert mutation_score >= 0.6, f"Algorithm selection mutation score too low: {mutation_score:.2%}"
    ...
    def test_data_validation_mutations(self, mutation_tester):
        ...
        test_cases = [
            ...
            {'args': (np.array([[1, np.inf], [2, 3]]),), 'kwargs': {}, 'expected': False},  # Invalid values
        ]
        ...
        if total_mutants > 0:
            mutation_score = killed_mutants / total_mutants
            assert mutation_score >= 0.7, f"Data validation mutation score too low: {mutation_score:.2%}"
    ...
    def test_error_handling_mutations(self, mutation_tester):
        ...
        test_cases = [
            ...
            {'args': (float('inf'), 1), 'kwargs': {}, 'expected': None},  # Infinite result
        ...
        if total_mutants > 0:
            mutation_score = killed_mutants / total_mutants
            assert mutation_score >= 0.5, f"Error handling mutation score too low: {mutation_score:.2%}"
    ...
    def test_property_based_mutation_testing(self, mutation_tester, data):
        ...
        def calculate_outlier_score(data_point: List[float], dataset: List[List[float]]) -> float:
    ...
    def test_configuration_validation_mutations(self, mutation_tester):
        ...
        test_cases = [
            ...
            {
                'args': ({'algorithm': 'IsolationForest', 'contamination': 1.5},),  # Invalid contamination
                'kwargs': {},
                'expected': False
            },
            {
                'args': ({'algorithm': 'IsolationForest', 'contamination': 0.1, 'n_estimators': -1},),  # Invalid n_estimators
                'kwargs': {},
                'expected': False
            }
        ]
        ...
        if total_mutants > 0:
            mutation_score = killed_mutants / total_mutants
            assert mutation_score >= 0.6, f"Configuration validation mutation score too low: {mutation_score:.2%}"
    ...
    def test_mutation_testing_statistics(self, mutation_tester):
        ...
        report = {
            ...
            'quality_assessment': {
                ...
                'components_above_threshold': sum(1 for r in all_results.values() if r['score'] >= 0.70),
                'components_below_threshold': sum(1 for r in all_results.values() if r['score'] < 0.70)
            }
        }
        ...
        assert overall_score >= 0.65, f"Overall mutation score {overall_score:.2%} below threshold"
        assert report['quality_assessment']['components_above_threshold'] >= 3, \
        ...
        print(f"Components above threshold: {report['quality_assessment']['components_above_threshold']}")