...
class TestPropertyBasedDatabaseTests:
    ...
    def test_detector_repository_property_roundtrip(
        self, in_memory_db, name, algorithm, contamination
    ):
...
class TestDatabaseErrorHandling:
    ...
    def test_repository_with_invalid_database(self):
        ...
        mock_db = Mock()
        mock_db.get_session.side_effect = sa.exc.DatabaseError(
    ...
    def test_repository_transaction_rollback(self):
        ...
        with pytest.raises(sa.exc.IntegrityError):
            with db.get_session() as session:
                session.execute(
                    sa.text("INSERT INTO test_table (id, value) VALUES (1, 'test')")
                )
                session.execute(
                    sa.text(
        ...
        with db.get_session() as session:
            result = session.execute(
                sa.text("SELECT COUNT(*) FROM test_table")
            ).scalar()
            assert result == 0
...
class TestMockBasedInfrastructure:
    ...
    def test_fastapi_endpoint_mock(self):
        ...
        mock_detector_service.create_detector.return_value = Mock(
            id="detector-123", name="test_detector", algorithm="IsolationForest"
        )
