...
class TestRedisCache:
    ...
    def test_redis_cache_complex_data_serialization(self, mock_redis_client, sample_cache_data):
        ...
        with patch('redis.Redis') as mock_redis:
            ...
            for key, value in sample_cache_data.items():
                ...
                serialized_value = json.dumps(value) if not isinstance(value, bytes) else value.decode()
    ...
    def test_redis_cache_batch_operations(self, mock_redis_client):
        ...
        with patch('redis.Redis') as mock_redis:
            ...
            assert mock_redis_client.mset.called or mock_redis_client.set.call_count == 3
...
class TestCacheManager:
    ...
    def test_cache_manager_invalidation_strategies(self, mock_redis_client):
        ...
        with patch('redis.Redis') as mock_redis:
            ...
            manager.set("user:123:settings", {"theme": "dark"}, tags=["user", "settings"])
    ...
    def test_cache_manager_serialization_formats(self, mock_redis_client, sample_detector):
        ...
        with patch('redis.Redis') as mock_redis:
            ...
            manager.set("detector_object", sample_detector, serialization_format="pickle")
            
            # Test msgpack serialization
            manager.set("binary_data", b"binary_content", serialization_format="msgpack")
...
class TestCacheDecorator:
    ...
    def test_cache_decorator_conditional_caching(self):
        ...
        @decorator.cache_result(
            ttl=300,
            condition=lambda result: result != "error"
        )
        ...
        cache_key_success = decorator._generate_cache_key(conditional_function, (5,), {})
...
class TestCacheKeyBuilder:
    ...
    def test_cache_key_builder_complex_keys(self):
        ...
        key = builder.build_key_from_components([
            "users",
            "123",
            "profile",
            "settings"
    ...
    def test_cache_key_builder_object_keys(self, sample_detector):
        ...
        custom_key = builder.build_key_from_object(
            sample_detector,
            key_func=lambda obj: f"custom:{obj.algorithm}:{obj.name}"
        )
        assert custom_key == f"custom:{sample_detector.algorithm}:{sample_detector.name}"
    ...
    def test_cache_key_builder_hash_keys(self):
        ...
        assert "hash:" in long_key or len(long_key) < 100  # Either hashed or within reasonable limit
