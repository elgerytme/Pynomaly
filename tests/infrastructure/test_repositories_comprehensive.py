...
def multiple_detectors():
    ...
    for i, (algo, cont_rate) in enumerate(zip(algorithms, contamination_rates)):
        detector = Detector(
            name=f"detector_{i}",
            algorithm=algo,
...
class TestInMemoryDatasetRepository:
    ...
    async def test_dataset_find_by_size_range(self):
        ...
        medium_datasets = await repo.find_by_size_range(min_samples=100, max_samples=500)
        assert len(medium_datasets) == 1
        assert medium_datasets[0].name == "medium"
        
        large_datasets = await repo.find_by_size_range(min_samples=500, max_samples=2000)
...
class TestInMemoryResultRepository:
    ...
    async def test_result_find_by_detector(self, sample_detection_result):
        ...
        detector_results = await repo.find_by_detector_id(sample_detection_result.detector.id)
    ...
    async def test_result_find_by_dataset(self, sample_detection_result):
        ...
        dataset_results = await repo.find_by_dataset_id(sample_detection_result.dataset.id)
    ...
    async def test_result_find_by_anomaly_count(self, sample_detection_result):
        ...
        results = await repo.find_by_anomaly_count_range(min_anomalies=1, max_anomalies=5)
        assert len(results) == 1
        
        # Test find outside range
        no_results = await repo.find_by_anomaly_count_range(min_anomalies=10, max_anomalies=20)
...
class TestDatabaseRepositories:
    ...
    async def test_database_detector_repository(self, sample_detector, mock_db_session):
        ...
        with patch('pynomaly.infrastructure.persistence.database.get_session') as mock_get_session:
...
class TestRepositoryFactory:
    ...
    def test_factory_auto_detection(self):
        ...
        auto_memory = RepositoryFactory.create_detector_repository("auto", prefer_memory=True)
        assert isinstance(auto_memory, InMemoryDetectorRepository)
        
        with patch('pynomaly.infrastructure.persistence.database.get_session'):
            auto_db = RepositoryFactory.create_detector_repository("auto", prefer_memory=False)
...
class TestRepositoryIntegration:
    ...
    @pytest.mark.asyncio
    async def test_cross_repository_consistency(self, sample_detector, sample_dataset, sample_detection_result):
    ...
    @pytest.mark.asyncio
    async def test_repository_transaction_coordination(self, sample_detector, sample_dataset):
    ...
    async def test_repository_data_migration(self, multiple_detectors):
        ...
        target_repo = InMemoryDetectorRepository()  # In practice, this could be DatabaseDetectorRepository
...
class TestRepositoryErrorRecovery:
    ...
    async def test_connection_recovery(self, sample_detector):
        ...
        with patch('pynomaly.infrastructure.persistence.database.get_session', mock_get_session):
    ...
    async def test_data_consistency_recovery(self, sample_detector):
        ...
        detectors = [
            sample_detector,
            Detector(name="fail_detector", algorithm="test", contamination=ContaminationRate(0.1)),
            Detector(name="success_detector", algorithm="test", contamination=ContaminationRate(0.1))
        ]
