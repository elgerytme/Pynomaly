...
class MLAdapterTestBase:
    ...
    def mock_dataset(self, sample_data):
        ...
        dataset.data = pd.DataFrame(sample_data["X_train"], columns=sample_data["features"])
    ...
    def verify_protocol_compliance(self, adapter_class: Type, adapter_instance: Any):
        ...
        for method in required_methods:
            assert hasattr(adapter_instance, method), f"Missing required method: {method}"
            assert callable(getattr(adapter_instance, method)), f"Method {method} is not callable"
        
        # Check properties
        assert hasattr(adapter_instance, 'is_fitted'), "Missing is_fitted property"
        assert hasattr(adapter_instance, 'contamination'), "Missing contamination property"
        ...
        for size in data_sizes:
            ...
            try:
                ...
                results[size] = {
                    ...
                    "samples_per_second_fit": size / fit_time if fit_time > 0 else float('inf'),
                    "samples_per_second_predict": 50 / predict_time if predict_time > 0 else float('inf')
                }
...
class TestPyTorchAdapterIntegration(MLAdapterTestBase):
    ...
    def test_pytorch_adapter_autoencoder(self, sample_data, mock_dataset):
        ...
        with memory_monitor():
            ...
            scores = adapter.decision_function(sample_data["X_test"])
            assert len(scores) == len(sample_data["X_test"])
            assert all(score >= 0 for score in scores), "Reconstruction scores should be non-negative"
...
class TestMLAdapterInteroperability:
    ...
    def test_adapter_consistency(self, sample_data):
        ...
        if "sklearn" in results and "pyod" in results:
            ...
            assert abs(sklearn_rate - pyod_rate) < 0.3, "Adapters should produce similar results"
