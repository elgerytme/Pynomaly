...
def sample_dataset():
    ...
    return Dataset(
        name="test_dataset",
        data=data,
        description="Test dataset for Excel adapter"
    )
...
class TestExcelAdapter:
    ...
    def test_initialization_no_dependencies(self):
        ...
        with patch('pynomaly.infrastructure.adapters.excel_adapter.OPENPYXL_AVAILABLE', False), \
             patch('pynomaly.infrastructure.adapters.excel_adapter.XLSXWRITER_AVAILABLE', False):
            
            with pytest.raises(ImportError, match="Excel adapter requires either openpyxl or xlsxwriter"):
                ExcelAdapter()
    
    def test_initialization_with_dependencies(self):
        """Test successful adapter initialization with dependencies."""
        with patch('pynomaly.infrastructure.adapters.excel_adapter.OPENPYXL_AVAILABLE', True), \
    ...
    def test_export_results_with_xlsxwriter(self, sample_detection_result):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            with patch('xlsxwriter.Workbook') as mock_workbook_class:
                ...
                options = ExportOptions(use_advanced_formatting=True, include_charts=True)
                result = adapter.export_results(sample_detection_result, tmp_path, options)
    ...
    def test_export_results_no_libraries(self, sample_detection_result):
        ...
        with patch('pynomaly.infrastructure.adapters.excel_adapter.OPENPYXL_AVAILABLE', False), \
            ...
            with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
                ...
                with pytest.raises(RuntimeError, match="No Excel library available for export"):
    ...
    def test_import_dataset_no_openpyxl(self):
        ...
        with patch('pynomaly.infrastructure.adapters.excel_adapter.OPENPYXL_AVAILABLE', False), \
            ...
            with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
                ...
                with pytest.raises(RuntimeError, match="openpyxl is required for Excel import"):
    ...
    def test_import_dataset_no_numeric_columns(self, mock_read_excel):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            with pytest.raises(ValueError, match="No numeric columns found for anomaly detection"):
                adapter.import_dataset(tmp_path)
...
class TestExcelAdapterIntegration:
    ...
    @pytest.mark.skipif(
        ...
        pytest.importorskip('openpyxl', reason='openpyxl not available') or
        pytest.importorskip('xlsxwriter', reason='xlsxwriter not available'),
    ...
    def test_end_to_end_export_import(self, sample_detection_result, sample_dataset):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            export_result = adapter.export_results(sample_detection_result, export_path, options)
            ...
            assert list(imported_dataset.feature_names) == list(sample_dataset.feature_names)