...
class TestPyODAdapterComprehensive:
    ...
    def test_pyod_adapter_parameter_management(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            mock_model_class = Mock()
            mock_model = Mock()
            mock_model.get_params.return_value = {"contamination": 0.1, "n_estimators": 100}
            ...
            mock_model.set_params.assert_called_once_with(n_estimators=200, random_state=123)
    ...
    def test_pyod_version_detection(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            with patch('pynomaly.infrastructure.adapters.pyod_adapter.pyod', side_effect=ImportError):
                version = adapter._get_pyod_version()
...
class TestSklearnAdapterComprehensive:
    ...
    def test_sklearn_adapter_initialization_all_algorithms(self):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.importlib.import_module') as mock_import:
    ...
    def test_sklearn_adapter_fallback_scoring(self, sample_dataset):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.importlib.import_module') as mock_import:
            ...
            with patch.object(SklearnAdapter, 'ALGORITHM_MAPPING', 
    ...
    def test_sklearn_version_detection(self):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.importlib.import_module') as mock_import:
            ...
            with patch('pynomaly.infrastructure.adapters.sklearn_adapter.sklearn', side_effect=ImportError):
                version = adapter._get_sklearn_version()
                assert version == "unknown"
...
class TestMLAdapterEdgeCases:
    ...
    def test_adapter_high_dimensional_data(self):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.importlib.import_module') as mock_import:
            ...
            high_dim_df = pd.DataFrame(high_dim_data, columns=[f"feature_{i}" for i in range(1000)])
            ...
            high_dim_dataset.created_at = datetime.now(timezone.utc)
            high_dim_dataset.get_numeric_features.return_value = list(high_dim_df.columns)
    ...
    def test_adapter_concurrent_access(self):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.importlib.import_module') as mock_import:
            ...
            test_dataset.created_at = datetime.now(timezone.utc)
