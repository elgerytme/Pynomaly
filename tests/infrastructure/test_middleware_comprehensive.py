...
from pynomaly.domain.exceptions import AuthenticationError, AuthorizationError, RateLimitError
...
class TestAuthenticationMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_with_bearer_token(self, auth_middleware, mock_auth_service, mock_user):
    ...
    @pytest.mark.asyncio
    async def test_authenticate_with_api_key(self, auth_middleware, mock_auth_service, mock_user):
    ...
    async def test_authenticate_invalid_token(self, auth_middleware, mock_auth_service):
        ...
        mock_auth_service.get_current_user.side_effect = AuthenticationError("Invalid token")
        
        with pytest.raises(AuthenticationError, match="Invalid token"):
            await auth_middleware.authenticate(mock_request)
    
    @pytest.mark.asyncio
    async def test_middleware_dispatch_protected_endpoint(self, test_app, mock_auth_service, mock_user):
...
class TestPermissionMiddleware:
    ...
    def test_check_permissions_admin_user(self, permission_middleware, mock_user_admin):
        ...
        has_permission = permission_middleware._check_permissions(mock_user_admin, required_permissions)
        assert has_permission is True
    
    def test_check_permissions_regular_user_allowed(self, permission_middleware, mock_user_regular):
        ...
        has_permission = permission_middleware._check_permissions(mock_user_regular, required_permissions)
        assert has_permission is True
    
    def test_check_permissions_regular_user_denied(self, permission_middleware, mock_user_regular):
    ...
    @pytest.mark.asyncio
    async def test_permission_middleware_dispatch_allowed(self, permission_middleware, mock_user_regular):
    ...
    @pytest.mark.asyncio
    async def test_permission_middleware_dispatch_denied(self, permission_middleware, mock_user_regular):
...
class TestCORSMiddleware:
    ...
    def test_add_cors_headers_preflight_request(self, cors_middleware):
        ...
        assert mock_response.headers["Access-Control-Allow-Origin"] == origin
        assert mock_response.headers["Access-Control-Allow-Methods"] == "GET, POST, PUT, DELETE"
        assert mock_response.headers["Access-Control-Allow-Headers"] == "Authorization, Content-Type, X-API-Key"
...
class TestMiddlewareIntegration:
    ...
    def integrated_app(self):
        ...
        app.add_middleware(BaseHTTPMiddleware, dispatch=SecurityHeadersMiddleware().dispatch)
        app.add_middleware(BaseHTTPMiddleware, dispatch=CORSMiddleware(allowed_origins=["*"]).dispatch)
        app.add_middleware(BaseHTTPMiddleware, dispatch=RateLimitMiddleware(requests_per_minute=100).dispatch)
    ...
    def test_middleware_configuration_customization(self):
        ...
        custom_rate_limit = RateLimitMiddleware(
        ...
        with TestClient(app) as client:
            ...
            response = client.get("/test", headers={"Origin": "https://specific-domain.com"})
            assert "Access-Control-Allow-Origin" in response.headers
            
            response = client.get("/test", headers={"Origin": "https://other-domain.com"})
