...
München,3.0,Größe value"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', encoding='latin-1', delete=False) as f:
            f.write(csv_data_unicode)
...
class TestParquetLoaderComprehensive:
    ...
    def test_parquet_advanced_features(self, multiformat_test_data):
        ...
        try:
            ...
            loader_cols = ParquetLoader(columns=['sensor_temp', 'sensor_pressure', 'is_anomaly'])
...
class TestPolarsLoaderComprehensive:
    ...
    def test_polars_performance_comparison(self, large_csv_data):
        ...
        try:
            with patch('polars.read_csv') as mock_polars, \
                 patch('pandas.read_csv') as mock_pandas:
                
                # Mock both implementations
                mock_polars_result = Mock()
                mock_polars_result.collect.return_value.to_pandas.return_value = pd.DataFrame({
                    'col1': range(1000)
                })
                ...
                loader = PolarsLoader()
                performance_stats = loader.compare_performance(temp_path, methods=['polars', 'pandas'])
...
class TestArrowLoaderComprehensive:
    ...
    def test_arrow_native_compute_functions(self, sample_csv_data):
        ...
        try:
            with patch('pyarrow.csv.read_csv') as mock_read, \
    ...
    def test_arrow_format_support(self, multiformat_test_data):
        ...
        for fmt in formats:
            ...
            try:
                ...
                with patch('pyarrow.feather.read_table') as mock_read, \
