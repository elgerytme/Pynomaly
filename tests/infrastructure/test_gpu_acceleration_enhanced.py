...
from pynomaly.domain.exceptions import GPUError, DeviceError, MemoryError
...
class TestDeviceManager:
    ...
    def test_device_selection_strategy(self, device_manager):
        ...
        with patch('torch.cuda.is_available', return_value=True), \
             patch('torch.cuda.device_count', return_value=2), \
             patch('torch.cuda.memory_allocated', side_effect=[1024**3, 512**3]):  # 1GB, 512MB
...
class TestGPUMemoryManager:
    ...
    def test_memory_pool_management(self, memory_manager):
        ...
        with patch('torch.cuda.memory_allocated', side_effect=[0, 512**3, 0]), \
    ...
    def test_memory_fragmentation_detection(self, memory_manager):
        ...
        with patch('torch.cuda.memory_allocated', return_value=6*1024**3), \
    ...
    def test_memory_leak_detection(self, memory_manager):
        ...
        with patch('torch.cuda.memory_allocated', side_effect=[initial_memory, leaked_memory]):
...
class TestCUDAUtils:
    ...
    def test_cuda_error_checking(self, cuda_utils):
        ...
        with patch('torch.cuda.is_available', return_value=True), \
             patch('torch.cuda.current_device', side_effect=RuntimeError("CUDA error: out of memory")):
...
class TestPerformanceBenchmarks:
    ...
    def test_cpu_vs_gpu_performance_comparison(self):
        ...
        for size in data_sizes:
            with patch('torch.cuda.is_available', return_value=True):
                ...
                # CPU timing
                cpu_time = benchmark.time_cpu_operation(
    ...
    def test_memory_bandwidth_benchmark(self):
        ...
        with patch('torch.cuda.is_available', return_value=True):
            ...
            for size in data_sizes:
                bandwidth = benchmark.measure_memory_bandwidth(
                    size,
                    device="cuda"
                )
...
class TestErrorHandling:
    ...
    def test_driver_compatibility_check(self):
        ...
        with patch('nvidia_ml_py.nvmlSystemGetDriverVersion', return_value="470.129.06"):
            
            is_compatible = checker.check_driver_compatibility(
                required_version="460.0"
            )
...
class TestGPUIntegration:
    ...
    def test_gpu_memory_optimization_integration(self):
        ...
        with patch('torch.cuda.memory_allocated', side_effect=[0, 2*1024**3, 1*1024**3]):
