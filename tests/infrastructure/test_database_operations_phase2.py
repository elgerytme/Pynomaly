...
from pynomaly.domain.exceptions import RepositoryError, DatabaseError
...
def requires_async_database(database_type: str):
    ...
    def decorator(test_func):
        try:
            ...
            return pytest.mark.skip(f"Requires {database_type} async dependencies")(test_func)
    return decorator
...
class TestInMemoryRepositories(DatabaseTestBase):
    ...
    def test_detection_result_repository_crud_operations(self, sample_detection_result):
        ...
        from pynomaly.infrastructure.persistence.repositories import InMemoryResultRepository
    ...
    def test_repository_concurrent_access(self, sample_dataset):
        ...
        assert len(repo._data) == 10, f"Expected 10 datasets in repository, got {len(repo._data)}"
...
class TestSQLiteRepository(DatabaseTestBase):
    ...
    def test_sqlite_dataset_repository(self, sample_dataset):
        ...
        with temp_database() as db_path:
            try:
                from pynomaly.infrastructure.persistence.sqlite_repository import SQLiteDatasetRepository
...
class TestPostgreSQLRepository(DatabaseTestBase):
    ...
    async def test_postgresql_async_operations(self, sample_dataset):
        ...
        try:
            from pynomaly.infrastructure.persistence.postgresql_repository import PostgreSQLDatasetRepository
...
class TestDatabasePerformance(DatabaseTestBase):
    ...
    def test_query_optimization(self, sample_detection_result):
        ...
        assert detector_query_time < 1, f"Detector query took too long: {detector_query_time}s"
        ...
        assert recent_query_time < 0.1, f"Recent query took too long: {recent_query_time}s"
    ...
    def test_memory_usage_optimization(self):
        ...
        for i in range(50):
            large_data = pd.DataFrame({
                f'feature_{j}': np.random.normal(0, 1, 10000)
                for j in range(10)
            })
    ...
    def test_transaction_failure_recovery(self, sample_dataset):
        ...
        with temp_database() as db_path:
            try:
                ...
                # Verify rollback
                found = repo.find_by_id(sample_dataset.id)
                assert found.name == original_name, "Transaction should have been rolled back"
    ...
    def test_data_corruption_detection(self, sample_dataset):
        ...
        with temp_database() as db_path:
            try:
                ...
                conn = sqlite3.connect(db_path)
                conn.execute("UPDATE datasets SET features = 'invalid_json' WHERE id = ?", (str(sample_dataset.id),))
                conn.commit()
                conn.close()
