...
class TestObservabilityMiddleware:
    ...
    def test_metrics_middleware_registration(self):
        """Test metrics middleware registration without raising."""
        app = FastAPI()
        ...
        try:
            ...
            # Test middleware addition to app
            app.add_middleware(MetricsMiddleware, app=app)
            
        except Exception as e:
            pytest.fail(f"Middleware registration failed: {e}")
    
    def test_request_metrics_middleware(self):
        """Test request metrics middleware function."""
        try:
            # Test the middleware function exists and is callable
            assert callable(track_request_metrics)
            
            # Test middleware decorator registration
            app = FastAPI()
            decorated_middleware = app.middleware("http")(track_request_metrics)
            assert decorated_middleware is not None
            
        except Exception as e:
            pytest.fail(f"Request metrics middleware registration failed: {e}")


class TestAuthMiddleware:
    """Test authentication middleware registration."""
    
    def test_auth_middleware_components(self):
        """Test auth middleware components can be imported and used."""
    ...
    def test_rate_limiter_creation(self):
        ...
        try:
            from pynomaly.infrastructure.auth.middleware import RateLimiter
...
class TestInfrastructureIntegration:
    ...
    def test_middleware_integration(self):
        ...
        try:
            # Test multiple middleware can be registered
            app.add_middleware(MetricsMiddleware, app=app)
