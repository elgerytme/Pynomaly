...
class TestTaskQueue:
    ...
    async def test_queue_full_behavior(self):
        ...
        task1 = DistributedTask(
            task_type=TaskType.ANOMALY_DETECTION,
            function_name="detect1"
        )
        
        task2 = DistributedTask(
            task_type=TaskType.ANOMALY_DETECTION,
            function_name="detect2"
        )
        
        task3 = DistributedTask(
            task_type=TaskType.ANOMALY_DETECTION,
            function_name="detect3"
        )
...
class TestTaskDistributor:
    ...
    def distributor(self, config):
        ...
        with patch('pynomaly.infrastructure.distributed.task_distributor.get_distributed_config_manager') as mock_manager:
