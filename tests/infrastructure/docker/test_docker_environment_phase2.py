...
class TestDockerInfrastructurePhase2:
    ...
    def test_dockerfile_testing_test_dependencies(self, dockerfile_testing_path):
        ...
        test_dependencies = ["pytest", "pytest-asyncio", "pytest-mock", "hypothesis", "pytest-benchmark"]
    ...
    def test_docker_compose_testing_exists(self, docker_compose_testing_path):
        ...
        assert docker_compose_testing_path.exists(), "docker-compose.testing.yml should exist"
        
        with open(docker_compose_testing_path, 'r') as f:
    ...
    def test_docker_compose_testing_services(self, docker_compose_testing_path):
        ...
        assert "depends_on" in test_service, "Test service should depend on other services"
    ...
    def test_docker_compose_testing_database_config(self, docker_compose_testing_path):
        ...
        assert "environment" in postgres_service, "PostgreSQL should set environment variables"
    ...
    def test_docker_compose_testing_networking(self, docker_compose_testing_path):
        ...
        for service_name in ["pynomaly-test", "postgres-test", "redis-test"]:
            ...
            assert "pynomaly-test-network" in service_networks, f"{service_name} should be on test network"
    ...
    def test_docker_compose_testing_health_checks(self, docker_compose_testing_path):
        ...
        assert "test" in postgres_healthcheck, "PostgreSQL health check should have test command"
        assert "pg_isready" in str(postgres_healthcheck.get("test", "")), "Should use pg_isready"
        ...
        assert "test" in redis_healthcheck, "Redis health check should have test command"
        assert "redis-cli" in str(redis_healthcheck.get("test", "")), "Should use redis-cli"
...
class TestDockerContainerOperationsPhase2:
    ...
    def test_docker_image_layers_optimization(self):
        ...
        assert "rm -rf /var/lib/apt/lists/*" in dockerfile_content, "Should clean apt cache"
        assert "--no-cache-dir" in dockerfile_content, "Should use no-cache-dir for pip"
        assert "COPY requirements.txt" in dockerfile_content, "Should copy requirements first for layer caching"
...
class TestDockerTestExecutionPhase2:
    ...
    def test_docker_parallel_testing(self):
        ...
        assert any("--cov-report=html" in item for item in command), "Should generate HTML coverage report"
        assert any("--junit-xml" in item for item in command), "Should generate JUnit XML report"
...
class TestDockerDependencyManagementPhase2:
    ...
    def test_docker_testing_framework_installation(self):
        ...
        for framework, version in test_dependencies.items():
            assert isinstance(framework, str), f"{framework} should be string"
            assert isinstance(version, str), f"{version} should be string"
            assert framework.startswith("pytest") or framework in ["hypothesis"], f"{framework} should be testing-related"
...
class TestDockerIntegrationWorkflowPhase2:
    ...
    def test_docker_development_workflow(self):
        ...
        for command_name, command in dev_commands.items():
            ...
            assert "docker-compose" in command, f"{command_name} should use docker-compose"
            assert "docker-compose.testing.yml" in command, f"{command_name} should use testing config"
    ...
    def test_docker_phase2_completion_validation(self):
        ...
        assert len(phase2_requirements) >= 10, "Should have comprehensive Phase 2 coverage"
