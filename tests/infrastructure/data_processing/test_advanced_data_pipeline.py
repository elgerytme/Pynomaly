...
from pynomaly.infrastructure.data_processing.advanced_data_pipeline import (
    ...
    PreprocessingStep, ImputationStrategy, ScalingMethod, EncodingMethod
)
...
class TestAdvancedDataPipeline:
    ...
    def test_high_cardinality_categorical_skip(self):
        ...
        assert "normal_cat_A" in result.data.columns or "normal_cat" not in result.data.columns
    ...
    def test_processing_error_handling(self):
        ...
        with patch.object(pipeline, '_step_validate_data', side_effect=Exception("Test error")):
    ...
    def test_ordinal_encoding(self):
        ...
        df = pd.DataFrame({
            "ordinal_cat": ["low", "medium", "high", "low", "high"]
        })
