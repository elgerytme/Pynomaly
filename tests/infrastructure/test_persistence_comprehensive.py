...
from pynomaly.infrastructure.persistence.database import DatabaseConnection, init_database
...
class TestDatabaseDetectorRepository:
    ...
    async def test_save_detector_success(self, detector_repository, sample_detector):
        ...
        assert retrieved.contamination_rate.value == sample_detector.contamination_rate.value
        assert retrieved.hyperparameters == sample_detector.hyperparameters
    
    @pytest.mark.asyncio
    async def test_save_detector_update_existing(self, detector_repository, sample_detector):
    ...
    async def test_find_by_algorithm(self, detector_repository):
        ...
        detector1 = Detector(name="det1", algorithm="isolation_forest", contamination=ContaminationRate(0.1))
        detector2 = Detector(name="det2", algorithm="lof", contamination=ContaminationRate(0.1))
        detector3 = Detector(name="det3", algorithm="isolation_forest", contamination=ContaminationRate(0.1))
        ...
        # Find by algorithm
        isolation_detectors = await detector_repository.find_by_algorithm("isolation_forest")
    ...
    async def test_repository_error_handling(self, detector_repository):
        ...
        # Mock database error
        with patch.object(detector_repository.db_connection, 'get_session') as mock_session:
...
class TestDatabaseDatasetRepository:
    ...
    async def test_dataset_large_data_handling(self, dataset_repository):
        ...
        large_dataset = Dataset(name="large_dataset", features=features, targets=targets)
...
class TestDatabaseDetectionResultRepository:
    ...
    def sample_detection_result(self):
        ...
        detector = Detector(name="test_detector", algorithm="test", contamination=ContaminationRate(0.1))
    ...
    @pytest.mark.asyncio
    async def test_save_detection_result_success(self, result_repository, sample_detection_result):
    ...
    async def test_find_by_detector(self, result_repository):
        ...
        for i in range(3):
            detector = Detector(name=f"detector_{i}", algorithm="test", contamination=ContaminationRate(0.1))
    ...
    @pytest.mark.asyncio
    async def test_delete_detection_result(self, result_repository, sample_detection_result):
