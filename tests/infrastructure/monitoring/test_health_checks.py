...
from pynomaly.infrastructure.monitoring.health_checks import (
    HealthChecker, HealthCheckResult, SystemHealth, HealthStatus, ComponentType,
    check_system_resources, check_memory_health, check_filesystem_health,
    check_model_repository, check_detector_service, check_streaming_service,
    get_health_checker, register_health_check, liveness_probe, readiness_probe
)
...
class TestHealthChecker:
    ...
    async def test_check_all_components(self):
        ...
        assert any(r.component == "service1" and r.status == HealthStatus.HEALTHY for r in results)
        assert any(r.component == "service2" and r.status == HealthStatus.DEGRADED for r in results)
...
class TestPredefinedHealthChecks:
    ...
    async def test_check_system_resources(self):
        ...
        with patch('psutil.virtual_memory') as mock_memory, \
            ...
            mock_memory.return_value = Mock(total=8*1024**3, available=4*1024**3, percent=50.0, used=4*1024**3)
            mock_cpu.return_value = 25.0
            mock_disk.return_value = Mock(total=100*1024**3, used=30*1024**3, free=70*1024**3)
    ...
    async def test_check_system_resources_high_usage(self):
        ...
        with patch('psutil.virtual_memory') as mock_memory, \
            ...
            mock_memory.return_value = Mock(total=8*1024**3, available=1*1024**3, percent=95.0, used=7*1024**3)
            mock_cpu.return_value = 98.0
            mock_disk.return_value = Mock(total=100*1024**3, used=97*1024**3, free=3*1024**3)
    ...
    async def test_check_filesystem_health(self):
        ...
        with patch('psutil.disk_usage') as mock_disk, \
            ...
            mock_open.return_value.__enter__.return_value.read.return_value = "health_check_test"
