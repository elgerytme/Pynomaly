...
class TestAdvancedBehaviorAnalyzer:
    ...
    def analyzer(self):
        ...
        with patch('pynomaly.infrastructure.security.advanced_threat_detection.get_threat_detection_manager') as mock_manager:
...
class TestThreatIntelligenceDetector:
    ...
    def test_add_threat_intelligence(self, detector):
        ...
        initial_count = len(detector.threat_feeds[ThreatIntelligenceSource.KNOWN_BAD_IPS])
        detector.add_threat_intelligence(threat_intel)
        
        assert len(detector.threat_feeds[ThreatIntelligenceSource.KNOWN_BAD_IPS]) == initial_count + 1
        assert threat_intel in detector.threat_feeds[ThreatIntelligenceSource.KNOWN_BAD_IPS]
