...
from pynomaly.domain.exceptions import MonitoringError, TelemetryError
...
class TestTelemetryService:
    ...
    def test_telemetry_service_span_creation(self, sample_telemetry_service):
        ...
        with patch('opentelemetry.trace.get_tracer') as mock_get_tracer:
            ...
            mock_span.set_attribute.assert_any_call("operation.type", "anomaly_detection")
    ...
    def test_telemetry_service_custom_attributes(self, sample_telemetry_service):
        ...
        with patch('opentelemetry.trace.get_tracer') as mock_get_tracer:
            ...
            with sample_telemetry_service.create_span("model_training", attributes=custom_attrs):
                pass
    ...
    def test_telemetry_service_metrics_integration(self, sample_telemetry_service):
        ...
        with patch('opentelemetry.metrics.get_meter') as mock_get_meter:
            ...
            # Record metrics
            sample_telemetry_service.record_counter("operations_total", 1, {"status": "success"})
            sample_telemetry_service.record_histogram("operation_duration", 0.5, {"operation": "detection"})
...
class TestMetricsCollector:
    ...
    def test_metrics_collector_counter_operations(self, sample_metrics_collector, mock_prometheus_client):
        ...
        sample_metrics_collector.increment_counter(
            counter_name, 
            1, 
            {"algorithm": "isolation_forest", "status": "success"}
        )
        
        sample_metrics_collector.increment_counter(
            counter_name, 
            2, 
            {"algorithm": "lof", "status": "error"}
        )
    ...
    def test_metrics_collector_histogram_operations(self, sample_metrics_collector, mock_prometheus_client):
        ...
        sample_metrics_collector.observe_histogram(
            histogram_name,
            0.3,
            {"algorithm": "lof", "dataset_size": "small"}
        )
    ...
    def test_metrics_collector_gauge_operations(self, sample_metrics_collector, mock_prometheus_client):
        ...
        sample_metrics_collector.increment_gauge(
            gauge_name,
            2,
            {"model_type": "lof"}
        )
    ...
    def test_metrics_collector_custom_metrics(self, sample_metrics_collector, mock_prometheus_client):
    ...
    def test_metrics_collector_batch_operations(self, sample_metrics_collector, mock_prometheus_client):
        """Test batch metric operations."""
        # Create metrics for batch operations
        sample_metrics_collector.create_counter("batch_operations_total", "Batch operations", ["status"])
        sample_metrics_collector.create_histogram("batch_processing_time", "Batch processing time", ["batch_size"])
        
        # Record batch metrics
        batch_data = [
            {"metric": "batch_operations_total", "value": 1, "labels": {"status": "success"}},
            {"metric": "batch_operations_total", "value": 1, "labels": {"status": "success"}},
            {"metric": "batch_processing_time", "value": 2.5, "labels": {"batch_size": "100"}},
            {"metric": "batch_processing_time", "value": 1.8, "labels": {"batch_size": "50"}}
        ]
    ...
    def test_metrics_collector_export_functionality(self, sample_metrics_collector, mock_prometheus_client):
...
class TestOpenTelemetryIntegration:
    ...
    def test_otel_custom_span_processors(self, mock_otlp_exporter):
        ...
        with patch('opentelemetry.sdk.trace.export.BatchSpanProcessor') as mock_batch_processor, \
             patch('opentelemetry.sdk.trace.export.SimpleSpanProcessor') as mock_simple_processor:
    ...
    def test_otel_sampling_configuration(self, mock_otlp_exporter):
        ...
        for config in sampling_configs:
            with patch('opentelemetry.sdk.trace.sampling.TraceIdRatioBasedSampler') as mock_sampler:
                integration = OpenTelemetryIntegration(
                    service_name="test_service",
...
class TestPrometheusExporter:
    ...
    def test_prometheus_exporter_custom_collectors(self, mock_prometheus_client):
        ...
        mock_prometheus_client['registry'].return_value.register.assert_called_with(custom_collector)
...
class TestAlertManager:
    ...
    def test_alert_severity_filtering(self):
        ...
        alerts = [
            {"id": "1", "severity": "info", "handlers": ["critical_only", "warning_and_up"]},
            {"id": "2", "severity": "warning", "handlers": ["critical_only", "warning_and_up"]},
            {"id": "3", "severity": "critical", "handlers": ["critical_only", "warning_and_up"]}
        ]
...
class TestPerformanceProfiler:
    ...
    def test_performance_profiler_function_timing(self):
        ...
        assert results["slow_function"]["avg_duration"] > results["fast_function"]["avg_duration"]
