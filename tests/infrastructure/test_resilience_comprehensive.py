...
class TestCircuitBreaker:
    ...
    def test_circuit_breaker_failure_threshold(self, sample_circuit_breaker):
        ...
        assert sample_circuit_breaker.failure_count == sample_circuit_breaker.failure_threshold
    ...
    def test_circuit_breaker_half_open_transition(self, sample_circuit_breaker):
        ...
        with patch('time.time') as mock_time:
            mock_time.return_value = time.time() + sample_circuit_breaker.recovery_timeout + 1
    ...
    def test_bulkhead_concurrent_execution(self):
        ...
        for i in range(2):  # Within limit
            thread = threading.Thread(
                target=lambda i=i: results.update({i: bulkhead.execute(lambda: slow_operation(i))})
            )
