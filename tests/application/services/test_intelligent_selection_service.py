...
from pynomaly.application.dto.selection_dto import (
    DatasetCharacteristicsDTO, AlgorithmPerformanceDTO, OptimizationConstraintsDTO,
    MetaLearningConfigDTO, SelectionRecommendationDTO
)
...
class TestIntelligentSelectionService:
    ...
    async def test_extract_dataset_characteristics(self):
        ...
        characteristics = await self.service._extract_dataset_characteristics(self.test_dataset)
    ...
    def test_filter_algorithm_candidates(self):
        ...
        constrained_candidates = self.service._filter_algorithm_candidates(characteristics, constraints)
    ...
    async def test_recommend_algorithm_basic(self):
        ...
        assert isinstance(recommendation.dataset_characteristics, DatasetCharacteristicsDTO)
    ...
    async def test_benchmark_algorithms(self):
        ...
        with patch.object(self.service, '_benchmark_single_algorithm') as mock_benchmark:
    ...
    async def test_get_learning_insights(self):
        ...
        performance1 = AlgorithmPerformanceDTO(primary_metric=0.8, training_time_seconds=30, memory_usage_mb=100)
        performance2 = AlgorithmPerformanceDTO(primary_metric=0.75, training_time_seconds=45, memory_usage_mb=150)
        
        await self.service.learn_from_result(self.test_dataset, "algo1", performance1, {})
        await self.service.learn_from_result(self.test_dataset, "algo2", performance2, {})
    ...
    def test_compute_dataset_hash(self):
        ...
        different_dataset = Dataset(name="different", data=different_data, features=["col1"])
    ...
    async def test_rule_based_recommendation(self):
        ...
        scores = await self.service._rule_based_recommendation(characteristics, candidates)
    ...
    async def test_historical_similarity_recommendation(self):
        ...
        scores = await self.service._historical_similarity_recommendation(characteristics, candidates)
        assert isinstance(scores, dict)
        assert len(scores) == len(candidates)
        
        # Add some history and test again
        performance = AlgorithmPerformanceDTO(primary_metric=0.8, training_time_seconds=30, memory_usage_mb=100)
        await self.service.learn_from_result(self.test_dataset, "isolation_forest", performance, {})
        
        scores_with_history = await self.service._historical_similarity_recommendation(characteristics, candidates)
    ...
    def test_analyze_algorithm_performance(self):
        ...
        characteristics = DatasetCharacteristicsDTO(
            n_samples=1000, n_features=4, n_numeric_features=3, n_categorical_features=1,
            feature_density=0.9, missing_value_ratio=0.0, outlier_ratio=0.05,
            mean_feature_correlation=0.1, feature_variance_ratio=0.2,
            data_dimensionality_ratio=0.004, skewness_mean=0.1,
            kurtosis_mean=0.1, class_imbalance=0.0
        )
    ...
    async def test_meta_learning_recommendation_no_model(self):
        ...
        scores = await self.service._meta_learning_recommendation(characteristics, candidates, None)
    ...
    async def test_predict_performance_no_predictor(self):
        ...
        result = await self.service._predict_performance(recommendation, characteristics)
    ...
    def test_characteristics_validation(self):
        ...
        with pytest.raises(ValueError):
            DatasetCharacteristicsDTO(
                n_samples=1000, n_features=10, n_numeric_features=8, n_categorical_features=2,
                feature_density=1.5,  # Invalid: > 1
                missing_value_ratio=0.02, outlier_ratio=0.05, mean_feature_correlation=0.15,
                feature_variance_ratio=0.3, data_dimensionality_ratio=0.01,
                skewness_mean=0.1, kurtosis_mean=0.2, class_imbalance=0.1
            )
        ...
        with pytest.raises(ValueError):
            DatasetCharacteristicsDTO(
                ...
                n_features=10, n_numeric_features=8, n_categorical_features=2,
                feature_density=0.95, missing_value_ratio=0.02, outlier_ratio=0.05,
                mean_feature_correlation=0.15, feature_variance_ratio=0.3,
                data_dimensionality_ratio=0.01, skewness_mean=0.1,
                kurtosis_mean=0.2, class_imbalance=0.1
            )
