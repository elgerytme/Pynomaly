...
class TestUnifiedDataService:
    ...
    async def test_load_and_process_dataframe(self):
        ...
        with patch.object(service.data_pipeline, 'process_dataset', return_value=mock_processed_dataset):
    ...
    async def test_load_and_process_with_report(self):
        ...
        with patch.object(service.data_pipeline, 'process_dataset', return_value=(mock_dataset, mock_report)):
            result, report = await service.load_and_process(
                df,
                return_report=True
            )
    ...
    async def test_load_and_process_with_caching(self):
        ...
        with patch.object(service.data_pipeline, 'process_dataset', return_value=(mock_dataset, mock_report)) as mock_process:
    ...
    async def test_load_and_process_auto_detect_target(self):
        ...
        mock_dataset = Dataset(name="test", data=df, target_column="target", metadata={})
        
        with patch.object(service.data_pipeline, 'process_dataset', return_value=mock_dataset):
            result = await service.load_and_process(
                df,
    ...
    async def test_load_and_process_file_source(self):
        ...
        with patch.object(service.smart_loader, 'load', return_value=mock_dataset), \
             patch.object(service.data_pipeline, 'process_dataset', return_value=mock_dataset):
    ...
    async def test_load_and_process_database_source(self):
        ...
        with patch.object(service.database_loader, 'load_query', return_value=mock_dataset), \
             patch.object(service.data_pipeline, 'process_dataset', return_value=mock_dataset):
    ...
    async def test_load_multiple_sources_parallel(self):
        ...
        dataset1 = Dataset(name="dataset1", data=pd.DataFrame({"a": [1, 2]}), metadata={})
        dataset2 = Dataset(name="dataset2", data=pd.DataFrame({"b": [3, 4]}), metadata={})
        
        with patch.object(service, 'load_and_process', side_effect=[dataset1, dataset2]):
    ...
    async def test_load_multiple_sources_combine(self):
        ...
        dataset1 = Dataset(name="dataset1", data=pd.DataFrame({"a": [1, 2]}), metadata={})
        dataset2 = Dataset(name="dataset2", data=pd.DataFrame({"a": [3, 4]}), metadata={})
    ...
    async def test_load_multiple_sources_with_failures(self):
        ...
        with patch.object(service, 'load_and_process', side_effect=[dataset1, Exception("Load failed")]):
            result = await service.load_multiple_sources(
                ["file1.csv", "file2.csv"],
                parallel=True
            )
    ...
    async def test_load_multiple_sources_all_fail(self):
        ...
        with patch.object(service, 'load_and_process', side_effect=Exception("Load failed")):
            with pytest.raises(DataValidationError, match="No sources could be loaded successfully"):
    ...
    def test_create_processing_config_classification_use_case(self):
        ...
        config = service.create_processing_config(
            ...
            performance_preference="thorough"
        )
    ...
    def test_validate_dataset_quality_poor(self):
        ...
        is_valid, quality_report = service.validate_dataset_quality(dataset, requirements)
        
        assert is_valid is False
        assert quality_report["overall_quality"] == "poor"
        assert len(quality_report["issues"]) > 0
        assert any("Insufficient samples" in issue for issue in quality_report["issues"])
        assert any("Too many missing values" in issue for issue in quality_report["issues"])
    ...
    def test_load_from_database_with_table(self):
        ...
        with patch.object(service.database_loader, 'load_table', return_value=mock_dataset):
            result = service._load_from_database(
                "postgresql://user:pass@host/db",
                "test_table",
                table_name="users"
    ...
    def test_load_from_database_no_query_or_table(self):
        ...
        with pytest.raises(DataValidationError, match="Either 'query' or 'table_name' must be provided"):
            service._load_from_database(
                "postgresql://user:pass@host/db",
                "test"
            )
