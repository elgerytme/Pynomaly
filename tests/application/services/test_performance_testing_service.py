...
from pynomaly.application.services.performance_testing_service import (
    PerformanceTestingService, PerformanceMetrics, BenchmarkResult, 
    BenchmarkSuite, StressTestConfig, SystemMonitor
)
...
class TestPerformanceTestingService:
    ...
    def mock_detector(self):
        ...
        detector.decision_function = Mock(return_value=np.array([0.1, -0.5, 0.3, -0.2, 0.4]))
        return detector
    ...
    @pytest.mark.asyncio
    async def test_run_single_benchmark_with_labels(self, service, mock_detector, sample_dataset):
    ...
    def test_aggregate_benchmark_results(self, service):
        ...
        aggregated = service._aggregate_benchmark_results(results, confidence_level=0.95)
    ...
    async def test_run_benchmark_suite(self, service):
        ...
        # Mock detectors
        detectors = {
            "MockAlg1": Mock(spec=Detector),
            "MockAlg2": Mock(spec=Detector)
        }
        ...
        with patch.object(service, '_generate_test_datasets') as mock_gen:
            ...
            with patch.object(service, '_run_single_benchmark') as mock_bench:
                mock_result = BenchmarkResult(algorithm_name="MockAlg", dataset_name="test")
    ...
    async def test_compare_algorithms(self, service):
        ...
        # Mock detectors
        detectors = {
            "Alg1": Mock(spec=Detector),
            "Alg2": Mock(spec=Detector)
        }
        ...
        # Mock datasets
        datasets = [Mock(spec=Dataset, name="dataset1"), Mock(spec=Dataset, name="dataset2")]
    ...
    def test_analyze_algorithmic_complexity(self, service):
        ...
        analysis = service._analyze_algorithmic_complexity(size_scaling, feature_scaling)
    ...
    def test_generate_scalability_recommendations(self, service):
        ...
        recommendations = service._generate_scalability_recommendations(complexity_analysis)
...
class TestPerformanceTestingIntegration:
    ...
    async def test_performance_comparison_workflow(self, tmp_path):
        ...
        for i, alg_name in enumerate(["Alg1", "Alg2"]):
            ...
            detector.fit = Mock()
            detector.predict = Mock(return_value=np.array([1, -1, 1]))
            detector.decision_function = Mock(return_value=np.array([0.1 + i*0.1, -0.5, 0.3]))
