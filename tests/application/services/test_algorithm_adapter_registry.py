...
class TestAlgorithmAdapterRegistry:
    ...
    def test_algorithm_comparison(self, registry, dataset):
        ...
        for algorithm_name, result in results.items():
            assert (
                result["outliers_detected"] > 0
            ), f"{algorithm_name} detected no outliers"
            assert (
                0.0 < result["avg_score"] < 1.0
            ), f"{algorithm_name} avg score out of range"
    ...
    def test_large_dataset_performance(self, registry):
        ...
        outlier_count = sum(predictions)
        assert (
            200 <= outlier_count <= 1000
        )  # Reasonable range for 500 injected outliers
