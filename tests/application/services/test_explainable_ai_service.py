...
class TestExplainableAIService:
    ...
    async def test_generate_counterfactual_explanations(
        ...
        counterfactuals = await explainable_ai_service.generate_counterfactual_explanations(
            model=model,
            instance=instance,
            feature_names=feature_names,
    ...
    @pytest.mark.asyncio
    async def test_detect_explanation_bias(
        self, explainable_ai_service, sample_data
    ):
...
class TestSHAPExplainer:
    ...
    async def test_shap_explain_instance_with_mock(self, sample_data):
        ...
        with patch('pynomaly.application.services.explainable_ai_service.SHAP_AVAILABLE', True):
            with patch('pynomaly.application.services.explainable_ai_service.shap') as mock_shap:
...
class TestLIMEExplainer:
    ...
    async def test_lime_explainer_with_mock(self, sample_data):
        ...
        with patch('pynomaly.application.services.explainable_ai_service.LIME_AVAILABLE', True):
            with patch('pynomaly.application.services.explainable_ai_service.lime') as mock_lime:
                # Mock LIME explainer
                mock_explainer = MagicMock()
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_mismatched_feature_names(self, explainable_ai_service, trained_model, sample_data):
    ...
    for explanation in explanations[1:]:
        ...
        for feature_name in first_importances:
            if feature_name in curr_importances:
                ...
                ) < 1e-6, f"Inconsistent importance for {feature_name}"