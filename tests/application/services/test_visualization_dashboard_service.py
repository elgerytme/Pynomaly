...
from pynomaly.application.services.visualization_dashboard_service import (
    VisualizationDashboardService, DashboardType, DashboardConfig,
    ChartConfig, ChartType, VisualizationEngine, RealTimeMetrics,
    DashboardData
)
...
class TestVisualizationDashboardService:
    ...
    async def test_metrics_history_size_limit(self, service):
        ...
        assert service.metrics_history[0].anomalies_detected == 2  # Should start from index 2
        assert service.metrics_history[-1].anomalies_detected == 4  # Should end at index 4
