...
class TestGovernanceFrameworkService:
    ...
    def service(self, tmp_path):
        ...
        return GovernanceFrameworkService(storage_path=tmp_path, retention_years=7)
    ...
    async def test_track_compliance_metric(self, service):
        ...
        assert (
            metric.get_compliance_status() == "compliant"
        )  # 95.5 >= target (100) - threshold
        ...
        assert (
            metric2.get_compliance_status() == "non_compliant"
    ...
    async def test_get_governance_dashboard(self, service):
        ...
        assert (
            dashboard["risk_management"]["high_risk_assessments"] == 1
    ...
    def test_governance_policy_validation(self):
        ...
        assert policy.is_active()
        assert policy.applies_to_resource(
            "dataset"
        )  # No specific resources, applies to all
