...
from pynomaly.domain.entities.drift_detection import (
    DriftDetectionResult, ModelMonitoringConfig, DriftType, DriftSeverity, 
    DriftDetectionMethod, MonitoringStatus
)
...
class TestDriftDetectionService:
    ...
    @pytest.mark.asyncio
    async def test_detect_data_drift_basic(self, drift_service, sample_data, feature_names):
    ...
    async def test_detect_data_drift_no_drift(self, drift_service, feature_names):
        ...
        assert result.drift_detected in [True, False]  # Could go either way with random data
        assert len(result.affected_features) <= 3
    
    @pytest.mark.asyncio
    async def test_detect_data_drift_with_methods(self, drift_service, sample_data, feature_names):
...
class TestStatisticalDriftDetector:
    ...
    def detector(self):
        ...
        from pynomaly.application.services.drift_detection_service import StatisticalDriftDetector
    ...
    def test_kolmogorov_smirnov_test(self, detector):
        ...
        ks_stat, p_value = detector.kolmogorov_smirnov_test(reference_data, current_data)
...
class TestPerformanceDriftDetector:
    ...
    def detector(self):
        ...
        from pynomaly.application.services.drift_detection_service import PerformanceDriftDetector
    ...
    def test_calculate_prediction_drift_binary(self, detector):
        ...
        drift_score = detector.calculate_prediction_drift(reference_preds, current_preds)
