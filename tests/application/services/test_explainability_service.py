...
class TestExplainabilityService:
    ...
    async def test_explain_instance_with_instance_data(
        self, explainability_service, sample_detector, mock_domain_service, sample_local_explanation
    ):
        """Test explaining instance with direct instance data."""
        # Setup mocks
        explainability_service.detector_repository.get_by_id.return_value = sample_detector
    ...
    async def test_explain_instance_with_dataset_index(
        self, explainability_service, sample_detector, sample_dataset, mock_domain_service, sample_local_explanation
    ):
        """Test explaining instance from dataset by index."""
        # Setup mocks
        explainability_service.detector_repository.get_by_id.return_value = sample_detector
        explainability_service.dataset_repository.get_by_id.return_value = sample_dataset
        mock_domain_service.explain_instance.return_value = sample_local_explanation
    ...
    async def test_explain_instance_detector_not_trained(self, explainability_service):
        ...
        explainability_service.detector_repository.get_by_id.return_value = untrained_detector
        
        # Create request
        request = ExplanationRequest(
            detector_id="untrained_detector",
            instance_data={"feature_1": 1.0}
        )
    ...
    @pytest.mark.asyncio
    async def test_explain_instance_insufficient_data(self, explainability_service, sample_detector):
    ...
    async def test_explain_model_success(
        self, explainability_service, sample_detector, sample_dataset, mock_domain_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_explain_model_missing_dataset(self, explainability_service, sample_detector):
    ...
    async def test_get_feature_statistics(
        ...
        async def mock_explain_instance(request):
            return ExplanationResponse(
                success=True,
                explanation=sample_local_explanation,
                message="Success"
            )
    ...
    async def test_exception_handling_in_explain_instance(
        ...
        # Setup mocks
        explainability_service.detector_repository.get_by_id.return_value = sample_detector
        mock_domain_service.explain_instance.side_effect = Exception("Domain service error")
    ...
    async def test_exception_handling_in_explain_model(
        ...
        explainability_service.detector_repository.get_by_id.return_value = sample_detector
        explainability_service.dataset_repository.get_by_id.return_value = sample_dataset
        mock_domain_service.explain_model.side_effect = Exception("Model explanation error")
