...
from pynomaly.application.services.multi_tenant_service import (
    MultiTenantService, TenantResourceManager, TenantIsolationService
)
from pynomaly.domain.entities.tenant import (
    Tenant, TenantStatus, SubscriptionTier, ResourceQuotaType, TenantConfiguration
)
...
class TestTenantIsolationService:
    ...
    async def test_encryption_key_management(self, isolation_service):
        ...
        tenant = Tenant(tenant_id=tenant_id, name="test", contact_email="test@example.com")
...
class TestMultiTenantService:
    ...
    async def test_list_tenants_with_filters(self, tenant_service):
        ...
        tenant1 = await tenant_service.create_tenant(
            name="tenant1", display_name="Tenant 1", contact_email="1@example.com",
            subscription_tier=SubscriptionTier.FREE
        )
        tenant2 = await tenant_service.create_tenant(
            name="tenant2", display_name="Tenant 2", contact_email="2@example.com",
            subscription_tier=SubscriptionTier.BASIC
        )
        tenant3 = await tenant_service.create_tenant(
            name="tenant3", display_name="Tenant 3", contact_email="3@example.com",
            subscription_tier=SubscriptionTier.FREE
        )
        ...
        # Filter by status
        pending_tenants = await tenant_service.list_tenants(status=TenantStatus.PENDING_ACTIVATION)
        ...
        free_tenants = await tenant_service.list_tenants(subscription_tier=SubscriptionTier.FREE)
        assert len(free_tenants) >= 2
        
        basic_tenants = await tenant_service.list_tenants(subscription_tier=SubscriptionTier.BASIC)
