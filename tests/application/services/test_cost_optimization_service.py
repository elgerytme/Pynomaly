...
class TestCostAnalysisEngine:
    ...
    def sample_resources(self):
        ...
        storage_resource.last_accessed = datetime.utcnow() - timedelta(
            days=5
        )  # Idle for 5 days
        ...
        dev_resource.cost_info = ResourceCost(
            monthly_cost=120.0, hourly_cost=0.077, compute_cost=120.0
        )
        dev_resource.usage_metrics = ResourceUsageMetrics(
            cpu_utilization_avg=0.15, memory_utilization_avg=0.25
        )
    ...
    def test_analyze_cost_trends(self, analysis_engine, sample_resources):
        ...
        assert (
            inefficiency["underutilized_resources"] >= 1
        ...
        assert (
            analysis["top_cost_drivers"][0]["monthly_cost"] == 150.0
        )  # Highest cost first

    def test_predict_future_costs_with_limited_data(
        self, analysis_engine, sample_resources
    ):
        """Test cost prediction with limited data (trend extrapolation)."""
        predictions = analysis_engine.predict_future_costs(
            sample_resources[:2], months_ahead=6
        )
        ...
        assert any(
            "trend extrapolation" in assumption
            for assumption in predictions["key_assumptions"]
        )
...
class TestRecommendationEngine:
    ...
    def underutilized_resource(self):
        ...
        resource.cost_info = ResourceCost(monthly_cost=300.0, hourly_cost=0.192)
    ...
    def overutilized_resource(self):
        ...
        resource.cost_info = ResourceCost(monthly_cost=7.0, hourly_cost=0.0104)
        resource.usage_metrics = ResourceUsageMetrics(
    ...
    def idle_resource(self):
        ...
        resource.cost_info = ResourceCost(monthly_cost=150.0, hourly_cost=0.096)
        resource.usage_metrics = ResourceUsageMetrics(
            cpu_utilization_avg=0.0, memory_utilization_avg=0.0
        )
        resource.last_accessed = datetime.utcnow() - timedelta(days=5)
        return resource

    @pytest.mark.asyncio
    async def test_generate_rightsizing_recommendations_downsize(
        self, recommendation_engine, underutilized_resource
    ):
        """Test generating rightsizing recommendations for downsize."""
        recommendations = (
            await recommendation_engine._generate_rightsizing_recommendations(
    ...
    @pytest.mark.asyncio
    async def test_generate_rightsizing_recommendations_upsize(
        self, recommendation_engine, overutilized_resource
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_scheduling_recommendations(
        self, recommendation_engine, idle_resource
    ):
        """Test generating scheduling recommendations."""
        recommendations = (
            await recommendation_engine._generate_scheduling_recommendations(
    ...
    @pytest.mark.asyncio
    async def test_generate_instance_type_recommendations_spot(
        self, recommendation_engine, idle_resource
    ):
        """Test generating spot instance recommendations."""
        recommendations = (
            await recommendation_engine._generate_instance_type_recommendations(
        ...
        spot_recs = [
            ...
            if r.recommendation_type == RecommendationType.SPOT_INSTANCES
        ]
    ...
    @pytest.mark.asyncio
    async def test_generate_idle_cleanup_recommendations(
        self, recommendation_engine, idle_resource
    ):
        """Test generating idle resource cleanup recommendations."""
        recommendations = (
            await recommendation_engine._generate_idle_cleanup_recommendations(
                idle_resource, OptimizationStrategy.BALANCED
            )
        )
        ...
        assert (
            rec.monthly_savings == idle_resource.cost_info.monthly_cost
        )  # Full cost savings
    ...
    async def test_generate_recommendations_integration(
        self, recommendation_engine, underutilized_resource, idle_resource
    ):
        ...
        for i in range(len(recommendations) - 1):
            assert (
                recommendations[i].get_priority_score()
                >= recommendations[i + 1].get_priority_score()
            )
...
class TestCostOptimizationService:
    ...
    async def test_update_resource_metrics(self, cost_service, sample_resource):
        ...
        success = await cost_service.update_resource_metrics(
            sample_resource.resource_id, new_metrics
        )
    ...
    async def test_generate_optimization_plan(self, cost_service, sample_resource):
        ...
        plan = await cost_service.generate_optimization_plan(
            strategy=OptimizationStrategy.BALANCED, target_savings_percent=0.2
        )
    ...
    async def test_analyze_costs_with_tenant_filter(self, cost_service):
        ...
        resource2 = CloudResource(
            name="resource2", tenant_id=uuid4()
        )  # Different tenant
