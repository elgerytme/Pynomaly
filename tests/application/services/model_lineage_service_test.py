...
class TestLineageTracking:
    ...
    @pytest.mark.asyncio
    async def test_add_model_dataset_relationship(self, lineage_service, sample_model, sample_dataset):
        ...
        for edge in relationships.edges:
            if (edge.source_id == sample_model.id and 
                edge.target_id == sample_dataset.id and 
                edge.relationship_type == "training"):
                edge_found = True
                break
    ...
    async def test_add_model_model_relationship(self, lineage_service):
        ...
        for edge in lineage.edges:
            if (edge.source_id == parent_model_id and 
                edge.target_id == child_model_id and 
                edge.relationship_type == "ensemble_member"):
                parent_found = True
                break
    ...
    async def test_circular_dependency_detection(self, lineage_service):
        ...
        with pytest.raises(CircularDependencyError):
            await lineage_service.add_model_relationship(model_c, model_a, "derived_from")
    ...
    async def test_remove_relationship(self, lineage_service):
        ...
        for edge in lineage.edges:
            if (edge.source_id == model_id and 
                edge.target_id == dataset_id and 
                edge.relationship_type == "training"):
...
class TestLineageSearch:
    ...
    async def test_search_by_relationship_type(self, lineage_service):
        ...
        for edge in training_relationships:
            if (edge.source_id == model_id and 
                edge.target_id == training_dataset and 
                edge.relationship_type == "training"):
                training_found = True
                break
    ...
    async def test_search_by_metadata(self, lineage_service):
        ...
        high_accuracy_relationships = await lineage_service.find_relationships_by_metadata(
            {"accuracy": {"$gte": 0.9}}
        )
        ...
        for edge in high_accuracy_relationships:
            if (edge.source_id == model_id and 
                edge.target_id == dataset_id and 
                edge.metadata.get("accuracy", 0) >= 0.9):
