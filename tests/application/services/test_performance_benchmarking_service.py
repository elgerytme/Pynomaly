...
class TestPerformanceBenchmarkingService:
    ...
    async def test_benchmark_single_run(self, service, sample_dataset):
        ...
        with patch.object(service, '_run_detection_algorithm', new_callable=AsyncMock) as mock_run:
    ...
    async def test_scalability_test(self, service):
        ...
        with patch.object(service, '_benchmark_single_run', new_callable=AsyncMock) as mock_benchmark:
            # Mock different execution times for different scales
            def mock_benchmark_response(algorithm_name, dataset, dataset_size, **kwargs):
    ...
    async def test_throughput_benchmark(self, service):
        ...
        with patch.object(service, '_measure_throughput', new_callable=AsyncMock) as mock_throughput:
    ...
    async def test_generate_benchmark_datasets(self, service, sample_config):
        ...
        expected_count = len(sample_config.dataset_sizes) * len(sample_config.feature_dimensions)
