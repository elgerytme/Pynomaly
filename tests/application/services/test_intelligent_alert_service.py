...
class TestAlertCorrelationEngine:
    ...
    def test_find_temporal_correlations(self, correlation_engine):
        ...
        correlations = correlation_engine.find_correlations(alert1, time_window_minutes=60)
    ...
    def test_find_pattern_correlations(self, correlation_engine):
        ...
        pattern_correlations = [c for c in correlations if c.correlation_type == "pattern"]
    ...
    def test_find_causal_correlations(self, correlation_engine):
        ...
        causal_correlations = [c for c in correlations if c.correlation_type == "causal"]
...
class TestIntelligentAlertService:
    ...
    async def test_process_alert_intelligence(self, alert_service, sample_metadata):
        ...
        assert alert_service.metrics["noise_classifications"][processed_alert.noise_classification.value] > 0
    ...
    async def test_list_alerts_filtering(self, alert_service, sample_metadata):
        ...
        assert all(alert.metadata.tenant_id == sample_metadata.tenant_id for alert in tenant_alerts)
    ...
    async def test_count_similar_alerts(self, alert_service):
        ...
        similar_count = alert_service._count_similar_alerts(base_alert, timedelta(hours=1))
