...
class TestAutoMLService:
    ...
    def test_algorithm_scoring(self, automl_service):
        ...
        score = automl_service._calculate_algorithm_score(isolation_forest_config, profile)
    ...
    def test_small_dataset_recommendations(self, automl_service):
        ...
        recommended = automl_service.recommend_algorithms(small_profile, max_algorithms=5)
    ...
    def test_large_dataset_recommendations(self, automl_service):
        ...
        recommended = automl_service.recommend_algorithms(large_profile, max_algorithms=5)
        
        # Should prefer scalable algorithms for large datasets
        assert 'IsolationForest' in recommended or 'COPOD' in recommended

    @pytest.mark.asyncio
    async def test_hyperparameter_optimization_without_optuna(self, automl_service, sample_dataset):
        """Test hyperparameter optimization when Optuna is not available."""
        automl_service.dataset_repository.get = AsyncMock(return_value=sample_dataset)
        
        # Mock Optuna as unavailable
        with patch('pynomaly.application.services.automl_service.OPTUNA_AVAILABLE', False):
            with pytest.raises(AutoMLError, match="Optuna is required"):
                await automl_service.optimize_hyperparameters(
                    "test_id", 
                    "IsolationForest", 
                    OptimizationObjective.AUC
                )

    @pytest.mark.asyncio
    async def test_hyperparameter_optimization_with_mocked_optuna(self, automl_service, sample_dataset):
        ...
        with patch('pynomaly.application.services.automl_service.OPTUNA_AVAILABLE', True):
            with patch('pynomaly.application.services.automl_service.optuna') as mock_optuna:
    ...
    def test_real_automl_workflow_simulation(self, automl_service, sample_dataset):
        ...
        async def run_workflow():
            ...
            recommendations = automl_service.recommend_algorithms(profile, max_algorithms=3)
