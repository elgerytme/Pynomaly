...
class TestDetectAnomaliesUseCase:
    ...
    async def test_execute_invalid_features(self, detector_repository, feature_validator, sample_detector):
        ...
        feature_validator.check_data_quality = Mock(return_value={"quality_score": 0.5, "missing_values": ["feature_1"], "constant_features": [], "low_variance_features": [], "infinite_values": ["feature_0"], "duplicate_rows": 0})
...
class TestTrainDetectorUseCase:
    ...
    async def test_execute_success(self, detector_repository, feature_validator, sample_detector, sample_dataset):
        ...
        # Mock feature validator
        feature_validator.validate_numeric_features = Mock(return_value=["feature_0", "feature_1", "feature_2", "feature_3", "feature_4"])
        feature_validator.check_data_quality = Mock(return_value={
            "quality_score": 0.9,
            "missing_values": [],
            "constant_features": []
        })
    ...
    async def test_execute_hyperparameter_tuning(self, detector_repository, feature_validator, sample_detector, sample_dataset):
        ...
        with patch.object(sample_detector, 'fit') as mock_fit, \
            ...
            mock_update_params.assert_called_once_with(n_estimators=50, contamination=0.1)
            mock_fit.assert_called_once_with(sample_dataset)

    @pytest.mark.asyncio
    async def test_execute_comprehensive_request_response(self, detector_repository, feature_validator, sample_detector, sample_dataset):
    ...
    @pytest.mark.asyncio
    async def test_execute_detector_not_found_pattern(self, detector_repository, feature_validator, sample_dataset):
    ...
    @pytest.mark.asyncio
    async def test_execute_with_quality_warnings(self, detector_repository, feature_validator, sample_detector):
        ...
        feature_validator.validate_numeric_features = Mock(return_value=["feature_0", "feature_2", "feature_3"])
        feature_validator.check_data_quality = Mock(return_value={
            "quality_score": 0.6,
        ...
        with patch.object(sample_detector, 'fit') as mock_fit:
            ...
            assert response.training_warnings is not None
            assert len(response.training_warnings) >= 2  # Missing values + constant features
    ...
    @pytest.mark.asyncio 
    async def test_execute_batch_training_simulation(self, detector_repository, feature_validator, sample_detector):
        ...
        assert len(responses) == 3
        assert all(response.trained_detector.id == sample_detector.id for response in responses)
        assert all(response.training_metrics is not None for response in responses)

    @pytest.mark.asyncio
    async def test_execute_minimal_vs_comprehensive_response(self, detector_repository, feature_validator, sample_detector, sample_dataset):
        ...
        with patch.object(sample_detector, 'fit') as mock_fit:
            ...
            assert minimal_response.training_warnings is None or len(minimal_response.training_warnings) == 0
...
class TestEvaluateModelUseCase:
    ...
    async def test_execute_success(self, detector_repository, sample_detector, sample_dataset):
        ...
        target_labels = np.array([0 if i < 90 else 1 for i in range(len(sample_dataset.data))])
    ...
    async def test_execute_with_ground_truth(self, detector_repository, sample_detector, sample_dataset):
        ...
        # Add target labels to dataset for evaluation
        target_labels = np.array([0 if i < 85 else 1 for i in range(len(sample_dataset.data))])
        sample_dataset.data['target'] = target_labels
        sample_dataset.target_column = 'target'
...
class TestExplainAnomalyUseCase:
    ...
    @pytest.mark.asyncio
    async def test_execute_multiple_anomalies(self, detector_repository, sample_detector, sample_dataset):
