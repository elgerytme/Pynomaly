...
from src.pynomaly.application.use_cases.drift_monitoring_use_case import DriftMonitoringUseCase
...
class TestDriftMonitoringUseCase:
    ...
    async def test_run_concept_drift_detection(self):
        ...
        self.mock_drift_service.detect_concept_drift = AsyncMock(return_value=mock_result)
    ...
    async def test_get_monitoring_status(self):
        ...
        if self.mock_detector_repository:
            self.mock_detector_repository.get_monitoring_status = AsyncMock(return_value=mock_status)
        
        status = await self.use_case.get_monitoring_status(model_id=self.model_id)
        
        # If no repository, should return default status
        if self.mock_detector_repository and hasattr(self.mock_detector_repository, 'get_monitoring_status'):
    ...
    async def test_get_drift_metrics(self):
        ...
        with patch.object(self.use_case, '_calculate_drift_metrics', return_value=mock_metrics):
