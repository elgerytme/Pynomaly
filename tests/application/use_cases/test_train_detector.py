...
from pynomaly.domain.exceptions import InsufficientDataError, FittingError
...
class TestTrainDetectorUseCase:
    ...
    async def test_execute_orchestrates_complete_training_workflow(
        ...
        mock_feature_validator.validate_numeric_features.return_value = ['feature1', 'feature2']
        ...
        mock_feature_validator.validate_numeric_features.assert_called_once_with(sample_dataset)
        mock_feature_validator.check_data_quality.assert_called_once_with(sample_dataset)
        
        # Assert - Execute training
        mock_adapter_registry.fit_detector.assert_called_once_with(sample_detector, sample_dataset)
    ...
    async def test_execute_validates_data_quality(
        ...
        assert any('Missing values' in warning for warning in response.training_warnings)
        assert any('constant features' in warning for warning in response.training_warnings)
