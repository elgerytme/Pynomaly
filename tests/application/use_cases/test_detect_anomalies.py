...
class TestDetectAnomaliesUseCase:
    ...
    async def test_execute_orchestrates_complete_detection_workflow(
        ...
        mock_scores = [AnomalyScore(0.1), AnomalyScore(0.15), AnomalyScore(0.12), 
                      AnomalyScore(0.18), AnomalyScore(0.14), AnomalyScore(0.95)]
        ...
        mock_feature_validator.check_data_quality.assert_called_once_with(sample_dataset)
        
        # Assert - Detect anomalies
        mock_adapter_registry.score_with_detector.assert_called_once_with(fitted_detector, sample_dataset)
        mock_adapter_registry.predict_with_detector.assert_called_once_with(fitted_detector, sample_dataset)
    ...
    async def test_execute_handles_detection_errors(
        ...
        mock_adapter_registry.score_with_detector.side_effect = RuntimeError("Detection failed")
    ...
    async def test_execute_validates_data_quality_when_enabled(
        ...
        mock_feature_validator.suggest_preprocessing.return_value = ["Remove missing values"]
        ...
        assert any('Data quality score is low' in warning for warning in response.warnings)
