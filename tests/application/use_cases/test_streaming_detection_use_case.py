...
class TestStreamingDetectionUseCase:
    ...
    def streaming_request(self, sample_detector, streaming_config):
        ...
        return StreamingRequest(
            ...
            configuration=streaming_config
        )
    ...
    async def test_start_streaming_success(
        ...
        # Setup mocks
        streaming_use_case.detector_repository.get = AsyncMock(return_value=sample_detector)
    ...
    async def test_stop_streaming_success(
        ...
        assert stream_id in streaming_use_case._stream_metrics  # Metrics kept for analysis
    ...
    async def test_processing_batch_adaptive_strategy(
        ...
        for i in range(3):
            sample = StreamingSample(data=np.array([float(i), float(i+1), float(i+2)]))
            await streaming_use_case.add_sample(stream_id, sample)
    ...
    @pytest.mark.asyncio
    async def test_sample_data_formats(
        self, streaming_use_case, sample_detector
    ):
        ...
        sample2 = StreamingSample(data={"feature1": 1.0, "feature2": 2.0, "feature3": 3.0})
    ...
    async def test_ensemble_streaming_missing_detector(
        ...
        streaming_use_case.detector_repository.get = AsyncMock(
            side_effect=lambda detector_id: sample_detector if detector_id == sample_detector.id else None
        )
    ...
    async def test_streaming_configuration_validation(
        ...
        invalid_config = StreamingConfiguration(
            min_batch_size=10,
            max_batch_size=5  # Invalid: min > max
        )
        ...
        response = await streaming_use_case.start_streaming(request)
        assert response.success is False
        assert "min_batch_size cannot be greater than max_batch_size" in response.error_message
    ...
    async def test_streaming_configuration_validation_watermarks(
        ...
        invalid_config = StreamingConfiguration(
            high_watermark=0.3,
            low_watermark=0.8  # Invalid: high < low
        )
        ...
        response = await streaming_use_case.start_streaming(request)
        assert response.success is False
        assert "high_watermark must be greater than low_watermark" in response.error_message
