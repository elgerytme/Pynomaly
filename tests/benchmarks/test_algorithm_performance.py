...
class BenchmarkDataGenerator:
    ...
    @staticmethod
    def generate_dataset(n_samples: int, n_features: int, contamination: float = 0.1, random_state: int = 42):
...
class TestAlgorithmPerformance:
    ...
    def test_isolation_forest_performance(self, benchmark, n_samples, n_features, performance_tracker):
    ...
    def test_algorithm_comparison_medium_dataset(self, benchmark, algorithm_name, performance_tracker):
    ...
    def test_large_dataset_scalability(self, benchmark, performance_tracker):
        ...
        try:
            ...
            assert benchmark.stats.mean < 60.0, "Large dataset should complete within 60 seconds"
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_scaling(self, n_samples, performance_tracker):
        ...
        try:
            ...
            assert memory_per_sample < 10.0, f"Memory usage too high: {memory_per_sample:.2f} KB per sample"
    ...
    def test_parallel_execution(self, benchmark, performance_tracker):
        ...
        try:
            ...
            for i in range(4):  # 4 parallel tasks
                X, y = BenchmarkDataGenerator.generate_dataset(1000, 10, random_state=42 + i)
                datasets.append(X)
            ...
            def run_parallel():
                with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                    futures = [executor.submit(run_single_algorithm, X) for X in datasets]
            ...
            # Record performance
            performance_tracker.record_result(
                f"{algorithm_name}_parallel", "4x1000x10", "execution_time", benchmark.stats.mean
            )
...
class TestPerformanceRegression:
    ...
    def test_baseline_performance(self, benchmark, performance_tracker):
        ...
        try:
            ...
            assert benchmark.stats.mean < max_time_seconds, \
                f"Performance regression: took {benchmark.stats.mean:.2f}s, expected < {max_time_seconds}s"
            
            # Record baseline
            performance_tracker.record_result(
                f"{algorithm_name}_baseline", f"{n_samples}x{n_features}", 
...
def test_performance_summary(performance_tracker):
    ...
    if not execution_times.empty:
        ...
        if max_time > 30.0:
            pytest.warns(UserWarning, f"Some tests took longer than 30s: {max_time:.3f}s")