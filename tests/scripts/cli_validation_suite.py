...
4.0,5.0,0"""
    ...
    def test_cli_structure(self) -> bool:
        ...
        try:
            # Test CLI app import
            from pynomaly.presentation.cli.app import main
            self.results["structure"]["main_import"] = True
            print("✅ Main CLI app imports successfully")
            
            # Test command modules
            cli_modules = [
                "autonomous", "datasets", "detection", "detectors", 
                "export", "preprocessing", "server"
            ]
    ...
    def test_detection_workflow(self, test_file: Path) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"✅ Autonomous detection completed in {execution_time:.2f} seconds")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if "startup_time" in self.results.get("performance", {}):
            ...
            if startup_time > 5.0:
                recommendations.append(f"Consider optimizing startup time (current: {startup_time:.2f}s)")
