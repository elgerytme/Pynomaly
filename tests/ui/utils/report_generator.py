...
class UITestReportGenerator:
    ...
    def _analyze_test_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "accessibility" in results:
            analysis["category_scores"]["accessibility"] = self._analyze_accessibility_results(
                results["accessibility"], analysis
            )
            
        if "responsive_design" in results:
            analysis["category_scores"]["responsive"] = self._analyze_responsive_results(
                results["responsive_design"], analysis
            )
    ...
    def _analyze_layout_results(self, results: Dict, analysis: Dict) -> float:
        ...
        if not results.get("responsive_navigation", True):
            analysis["warnings"].append(
                "Mobile navigation may not be fully functional"
            )
            score -= 10
        ...
        if not results.get("semantic_html", True):
            analysis["warnings"].append(
                "Missing or improper semantic HTML elements"
            )
            score -= 5
    ...
    def _analyze_ux_results(self, results: Dict, analysis: Dict) -> float:
        ...
        if results.get("slow_interactions", False):
            analysis["warnings"].append(
                "Some interactions are slower than optimal"
            )
            score -= 5
    ...
    def _analyze_visual_results(self, results: Dict, analysis: Dict) -> float:
        ...
        if baseline_coverage < 80:
            analysis["warnings"].append(
                "Visual test baseline coverage is below 80%"
            )
            score -= 5
    ...
    def _analyze_accessibility_results(self, results: Dict, analysis: Dict) -> float:
        ...
        critical_a11y = [issue for issue in a11y_issues if issue.get("severity") == "critical"]
        ...
        # Check for warnings
        warning_a11y = [issue for issue in a11y_issues if issue.get("severity") == "warning"]
        if warning_a11y:
            analysis["warnings"].append(
    ...
    def _generate_overall_recommendations(self, analysis: Dict):
        ...
        if overall_score >= 90:
            ...
            analysis["recommendations"].insert(0,
                "👍 **Good**: Solid UI foundation with room for refinement."
            )
            ...
            analysis["recommendations"].insert(0,
                "🚨 **Critical**: Significant UI/UX issues need immediate attention."
            )
