...
class PerformanceMonitor:
    ...
    def _evaluate_performance(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if lcp > 0:
            if lcp <= PERFORMANCE_THRESHOLDS["core_web_vitals"]["largest_contentful_paint"]:
                checks.append({"name": "LCP", "passed": True, "value": lcp, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["largest_contentful_paint"]})
            else:
                checks.append({"name": "LCP", "passed": False, "value": lcp, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["largest_contentful_paint"]})
                evaluation["critical_issues"].append(f"Largest Contentful Paint ({lcp}ms) exceeds threshold")
        
        fid = core_vitals.get("firstInputDelay", 0)
        if fid > 0:
            if fid <= PERFORMANCE_THRESHOLDS["core_web_vitals"]["first_input_delay"]:
                checks.append({"name": "FID", "passed": True, "value": fid, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["first_input_delay"]})
            else:
                checks.append({"name": "FID", "passed": False, "value": fid, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["first_input_delay"]})
                evaluation["critical_issues"].append(f"First Input Delay ({fid}ms) exceeds threshold")
        
        cls = core_vitals.get("cumulativeLayoutShift", 0)
        if cls <= PERFORMANCE_THRESHOLDS["core_web_vitals"]["cumulative_layout_shift"]:
            checks.append({"name": "CLS", "passed": True, "value": cls, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["cumulative_layout_shift"]})
        else:
            checks.append({"name": "CLS", "passed": False, "value": cls, "threshold": PERFORMANCE_THRESHOLDS["core_web_vitals"]["cumulative_layout_shift"]})
            evaluation["critical_issues"].append(f"Cumulative Layout Shift ({cls}) exceeds threshold")
        ...
        if dom_loaded <= PERFORMANCE_THRESHOLDS["custom_metrics"]["dom_content_loaded"]:
            checks.append({"name": "DOM Content Loaded", "passed": True, "value": dom_loaded})
        else:
            checks.append({"name": "DOM Content Loaded", "passed": False, "value": dom_loaded})
            evaluation["warnings"].append(f"DOM Content Loaded ({dom_loaded}ms) is slow")
        
        load_complete = nav_timing.get("loadComplete", 0)
        if load_complete <= PERFORMANCE_THRESHOLDS["custom_metrics"]["load_complete"]:
            checks.append({"name": "Load Complete", "passed": True, "value": load_complete})
        else:
            checks.append({"name": "Load Complete", "passed": False, "value": load_complete})
        ...
        if resource_count <= PERFORMANCE_THRESHOLDS["custom_metrics"]["resource_count"]:
            checks.append({"name": "Resource Count", "passed": True, "value": resource_count})
        else:
            checks.append({"name": "Resource Count", "passed": False, "value": resource_count})
        ...
        if len(checks) > 0:
            evaluation["overall_score"] = (evaluation["passed_checks"] / len(checks)) * 100
        ...
        if evaluation["critical_issues"]:
            evaluation["recommendations"].append("Address critical Core Web Vitals issues immediately")
        if evaluation["warnings"]:
            evaluation["recommendations"].append("Optimize resource loading and rendering performance")
        if resource_count > 30:
            evaluation["recommendations"].append("Consider bundling or reducing the number of resources")
        
        evaluation["checks"] = checks
        return evaluation
    
    async def run_lighthouse_audit(self, url: str, device_config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            lighthouse_config = {
                ...
                "settings": {
                    ...
                    "throttling": {
                        ...
                        "requestLatencyMs": 40 if device_config.get("network") == "fast3g" else 150,
                        "downloadThroughputKbps": 1600 if device_config.get("network") == "fast3g" else 400,
                        "uploadThroughputKbps": 750 if device_config.get("network") == "fast3g" else 400
                    },
            ...
            # Run Lighthouse
            output_path = LIGHTHOUSE_REPORTS_DIR / f"lighthouse_{device_config['name']}_{int(time.time())}.json"
            ...
            if result.returncode == 0 and output_path.exists():
                ...
                processed_results = {
                    "scores": {
                        category: data.get("score", 0) * 100 if data.get("score") is not None else 0
                    ...
                    "opportunities": [
                        ...
                        if audit.get("scoreDisplayMode") == "numeric" and audit.get("numericValue", 0) > 0
                    ],
    ...
    async def save_performance_report(self, metrics: Dict[str, Any], test_name: str = "performance_test"):
...
@pytest.mark.skipif(not PERFORMANCE_TESTING_ENABLED, reason="Performance testing disabled")
class TestPerformanceComprehensive:
    """Comprehensive performance testing with Core Web Vitals and Lighthouse."""
    
    async def test_homepage_performance(self, performance_monitor: PerformanceMonitor, page: Page):
        ...
        if lcp > 0:  # Only test if LCP was measured
            assert lcp <= PERFORMANCE_THRESHOLDS["core_web_vitals"]["largest_contentful_paint"], \
                f"LCP ({lcp}ms) exceeds threshold ({PERFORMANCE_THRESHOLDS['core_web_vitals']['largest_contentful_paint']}ms)"
        
        # CLS should be under 0.1
        cls = core_vitals.get("cumulativeLayoutShift", 0)
        assert cls <= PERFORMANCE_THRESHOLDS["core_web_vitals"]["cumulative_layout_shift"], \
        ...
        assert evaluation.get("overall_score", 0) >= 70, \
            f"Performance score ({evaluation.get('overall_score', 0)}) is too low"
        
        # Save report
        await performance_monitor.save_performance_report(metrics, "homepage_performance")
    
    async def test_detectors_page_performance(self, performance_monitor: PerformanceMonitor, page: Page):
        ...
        assert dom_loaded <= PERFORMANCE_THRESHOLDS["custom_metrics"]["dom_content_loaded"], \
        ...
        assert resource_count <= PERFORMANCE_THRESHOLDS["custom_metrics"]["resource_count"], \
            f"Too many resources loaded ({resource_count})"
        
        await performance_monitor.save_performance_report(metrics, "detectors_performance")
    
    async def test_performance_across_devices(self, performance_monitor: PerformanceMonitor, page: Page, device_config: Dict[str, Any]):
        ...
        if device_config["network"] == "slow3g":
            ...
            await page.route("**/*", lambda route: (
                asyncio.sleep(0.1) if asyncio.iscoroutinefunction(asyncio.sleep) else None,
                route.continue_()
            ))
        ...
        if lcp > 0:
            assert lcp <= device_thresholds["largest_contentful_paint"], \
                f"LCP on {device_config['name']} ({lcp}ms) exceeds threshold"
        
        await performance_monitor.save_performance_report(
            metrics, 
            f"device_performance_{device_config['name']}"
        )
    
    @pytest.mark.skipif(not LIGHTHOUSE_ENABLED, reason="Lighthouse not enabled")
    async def test_lighthouse_audit_comprehensive(self, performance_monitor: PerformanceMonitor, page: Page):
        """Test comprehensive Lighthouse audit."""
        url = page.url if page.url and page.url != "about:blank" else "http://localhost:8000"
        
        # Test different device configurations
        for device_config in DEVICE_CONFIGS:
            lighthouse_results = await performance_monitor.run_lighthouse_audit(url, device_config)
            ...
            for category, threshold in PERFORMANCE_THRESHOLDS["lighthouse_scores"].items():
                if category in scores:
                    score = scores[category]
                    assert score >= threshold, \
                        f"Lighthouse {category} score ({score}) below threshold ({threshold}) on {device_config['name']}"
    
    async def test_resource_optimization(self, performance_monitor: PerformanceMonitor, page: Page):
        ...
        if total_bundle > 0:
            assert total_bundle <= PERFORMANCE_THRESHOLDS["custom_metrics"]["bundle_size"], \
        ...
        assert script_count <= 10, f"Too many script resources ({script_count}), consider bundling"
        
        await performance_monitor.save_performance_report(metrics, "resource_optimization")
    
    async def test_core_web_vitals_monitoring(self, performance_monitor: PerformanceMonitor, page: Page):
        ...
        for url, page_name in pages_to_test:
            ...
            evaluation = metrics.get("evaluation", {})
            assert evaluation.get("overall_score", 0) >= 60, \
        ...
        await performance_monitor.save_performance_report(comprehensive_report, "comprehensive_cwv_monitoring")


# Utility functions
def generate_performance_summary_report(reports_dir: Path = PERFORMANCE_REPORTS_DIR) -> Dict[str, Any]:
