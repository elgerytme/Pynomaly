...
class UITestRunner:
    ...
    async def _analyze_and_critique(self):
        ...
        print(
            f"🎯 Analysis complete. Found {len(all_issues)} issues across all categories"
        )
    ...
    def print_summary(self):
        ...
        if self.issues_detected:
            ...
            for i, issue in enumerate(self.issues_detected[:5], 1):
                print(
                    f"  {i}. {issue.get('description', 'Unknown issue')} "
...
async def main():
    ...
    try:
        ...
        print("📸 Screenshots available in 'screenshots' directory")
