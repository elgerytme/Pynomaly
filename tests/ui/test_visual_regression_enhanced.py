...
for directory in [VISUAL_TEST_CONFIG["baseline_directory"], 
...
class VisualRegressionTester:
    ...
    async def _compare_screenshots(self, baseline_path: Path, current_path: Path, name: str):
        ...
        try:
            ...
            if diff_percentage > 1.0:  # 1% threshold
                # Save diff image
                diff_path = VISUAL_TEST_CONFIG["diff_directory"] / f"{name}_{self.browser_name}_diff.png"
                    ...
                    for y in range(baseline.size[1]):
                        if x < current.size[0] and y < current.size[1]:
                            ...
                            if baseline_pixel != current_pixel:
                                diff_highlighted.putpixel((x, y), (255, 0, 0))  # Red for differences
                            else:
    ...
    async def _wait_for_page_stability(self):
        ...
        try:
            await self.page.wait_for_function(
                "() => !document.body.hasAttribute('hx-request')",
                timeout=5000
            )
        except:
            pass
...
class CrossBrowserVisualTester:
    ...
    async def run_cross_browser_test(self, test_name: str, test_function, browsers: List[str] = None):
...
class TestVisualRegression:
    ...
    async def test_dashboard_visual_consistency(self, visual_tester: VisualRegressionTester, page: Page):
    ...
    async def test_detectors_page_visual(self, visual_tester: VisualRegressionTester, page: Page):
    ...
    async def test_datasets_page_visual(self, visual_tester: VisualRegressionTester, page: Page):
        """Test datasets page visual consistency."""
        await page.goto("/datasets")
        await visual_tester.take_percy_screenshot("datasets_list")
    
    async def test_detection_page_visual(self, visual_tester: VisualRegressionTester, page: Page):
        """Test detection page visual consistency."""
        await page.goto("/detection")
        await visual_tester.take_percy_screenshot("detection_form")
    
    async def test_visualizations_page_visual(self, visual_tester: VisualRegressionTester, page: Page):
    ...
    async def test_responsive_design_visual(self, visual_tester: VisualRegressionTester, page: Page):
        ...
        for url, name in pages_to_test:
            ...
            for viewport, result in results.items():
                if isinstance(result, dict) and result.get("status") == "error":
                    pytest.fail(f"Screenshot failed for {name} at {viewport}: {result.get('error')}")
    
    async def test_dark_mode_visual(self, visual_tester: VisualRegressionTester, page: Page):
        """Test dark mode visual consistency (if implemented)."""
        # Check if dark mode toggle exists
        await page.goto("/")
        
        dark_mode_toggle = await page.query_selector("[data-theme-toggle], .dark-mode-toggle")
        if dark_mode_toggle:
            # Test light mode
    ...
    async def test_form_states_visual(self, visual_tester: VisualRegressionTester, page: Page):
    ...
    async def test_loading_states_visual(self, visual_tester: VisualRegressionTester, page: Page):
...
class TestPercyIntegration:
    ...
    async def test_percy_cross_browser_comparison(self, cross_browser_tester: CrossBrowserVisualTester):
    ...
    async def test_percy_component_isolation(self, visual_tester: VisualRegressionTester, page: Page):
...
def generate_visual_test_report(results: Dict[str, Any]) -> str:
    ...
    report = {
        "summary": {
            ...
            "differences_found": sum(r.get("differences_found", 0) for r in results.values())
        },
