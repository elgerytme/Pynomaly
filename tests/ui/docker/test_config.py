...
def capture_test_artifacts(request, page):
    ...
    if hasattr(request.node, "rep_call") and request.node.rep_call.failed:
        ...
        if TEST_CONFIG["take_screenshots"]:
            screenshot_path = SCREENSHOTS_DIR / f"{browser_name}_{test_name}_{timestamp}.png"
        ...
        if TEST_CONFIG["record_videos"] and page.video:
            ...
            if video_path:
                final_video_path = VIDEOS_DIR / f"{browser_name}_{test_name}_{timestamp}.webm"
                import shutil
...
def pytest_configure(config):
    ...
    config.option.json_report_file = REPORTS_DIR / f"report_{TEST_CONFIG['browser']}.json"
