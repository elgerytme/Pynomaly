...
def run_tests_in_docker(browser="chrome", parallel=True, verbose=True):
    ...
    cmd = [
        ...
        "-e", f"BROWSER={browser}",
        "-e", "HEADLESS=true",
        "-e", "TAKE_SCREENSHOTS=true",
        "-e", "RECORD_VIDEOS=true",
        "-e", f"PYNOMALY_BASE_URL=http://host.docker.internal:8000",
        "-v", f"{reports_dir.absolute()}:/app/test_reports",
        "--network", "host",
        "pynomaly-ui-tests"
    ]
...
def run_docker_compose_tests():
    ...
    result = run_command(["docker-compose", "up", "--build", "--abort-on-container-exit"])
        ...
        if report_file.exists():
            with open(report_file, 'r') as f:
                ...
                if "summary" in browser_results:
                    ...
                    combined_results["summary"]["total_tests"] += summary.get("total", 0)
                    ...
                    combined_results["summary"]["duration"] += summary.get("duration", 0)
