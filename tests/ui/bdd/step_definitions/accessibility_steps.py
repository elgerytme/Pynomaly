...
async def then_content_accessible_screen_reader(
    ...
    assert len(screen_reader_violations) == 0, f"Found {len(screen_reader_violations)} screen reader violations"
...
async def then_landmarks_properly_labeled(page: Page):
    ...
    assert landmarks_check["percentage"] >= 80, f"Only {landmarks_check['percentage']:.1f}% of landmarks are properly labeled"
...
async def then_heading_structure_logical(page: Page):
    ...
    assert heading_structure["logical_order"], f"Heading structure is not logical: {heading_structure['levels']}"
...
async def then_interactive_elements_keyboard_accessible(page: Page):
    ...
    assert keyboard_test["percentage"] >= 95, f"Only {keyboard_test['percentage']:.1f}% of interactive elements are keyboard accessible"
...
async def then_tab_order_logical(page: Page):
    ...
    assert len(visible_elements) >= 5, "Should be able to tab through at least 5 visible elements"
...
async def then_focus_indicators_visible(page: Page):
    ...
    assert focus_test["percentage"] >= 90, f"Only {focus_test['percentage']:.1f}% of elements have visible focus indicators"
...
async def then_sufficient_color_contrast(
    ...
    assert len(contrast_violations) == 0, f"Found {len(contrast_violations)} serious color contrast violations"
...
async def then_layout_not_break_larger_text(page: Page):
    ...
    assert not layout_check["hasOverflow"], "Text scaling should not cause overflow issues"
...
async def then_form_fields_have_labels(page: Page):
    ...
    assert form_accessibility["percentage"] >= 90, f"Only {form_accessibility['percentage']:.1f}% of form elements are properly labeled"
...
async def then_required_fields_indicated(page: Page):
    ...
    assert required_fields_check["percentage"] >= 100, f"Only {required_fields_check['percentage']:.1f}% of required fields are clearly indicated"
...
async def given_encounter_modal(page: Page):
    ...
    modal_triggers = await page.query_selector_all("button[data-modal], [data-toggle='modal'], .modal-trigger")
    ...
    try:
        ...
        if modal:
            ...
            modal_contains_focus = await modal.evaluate(
                "(modal, focusedEl) => modal.contains(focusedEl)",
                focused_element
            )
...
async def then_wcag_compliance_achieved(
    ...
    wcag_violations = [
        v for v in compliance_results.get("violations", [])
        if any(tag in v.get("tags", []) for tag in ["wcag2a", "wcag2aa", "wcag21aa"])
        and v.get("impact") in ["serious", "critical"]
    ]
    
    assert len(wcag_violations) == 0, f"Found {len(wcag_violations)} serious WCAG compliance violations"


@then("automated accessibility tests should pass")
async def then_automated_tests_pass(
    accessibility_context: AccessibilityContext
):
    ...
    assert accessibility_context.accessibility_tester is not None, "Accessibility testing should be available"
    ...
    if test_results:
        ...
        assert len(failed_tests) == 0, f"Found {len(failed_tests)} failed accessibility tests"