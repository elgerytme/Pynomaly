...
async def then_page_loads_within_2_seconds(performance_context: PerformanceContext):
    ...
    assert performance_context.load_time <= 2.0, f"Page load time {performance_context.load_time:.2f}s exceeds 2 second limit"


@then("the Largest Contentful Paint (LCP) should be under 2.5 seconds")
async def then_lcp_under_2_5_seconds(page: Page, performance_context: PerformanceContext):
...
async def then_critical_resources_prioritized(page: Page):
    ...
    resource_timing = await page.evaluate("window.performanceMetrics?.resourceTiming || []")
    
    # Check that CSS and critical JS load early
    css_loads = [r for r in resource_timing if '.css' in r['name']]
    js_loads = [r for r in resource_timing if '.js' in r['name'] and 'critical' in r['name']]
...
async def then_cached_content_loads_instantly(page: Page, performance_context: PerformanceContext):
...
async def then_essential_functionality_available(page: Page, performance_context: PerformanceContext):
    ...
    if performance_context.offline_mode:
        ...
        try:
            ...
            assert "pynomaly" in content.lower(), "Basic app content should be available"


@then("data should sync when connection is restored")
async def then_data_syncs_when_online(page: Page, performance_context: PerformanceContext):
...
async def when_upload_large_dataset(page: Page, ui_helper: UITestHelper, performance_context: PerformanceContext):
    ...
    upload_selectors = ["input[type='file']", "[data-testid='file-upload']", ".file-upload"]
...
async def then_progress_indicators_accurate(page: Page):
    ...
    progress_selectors = [
        ...
        ".loading", ".spinner", "[data-testid='progress']"
    ]
...
async def then_interface_remains_responsive(page: Page):
    ...
    try:
        ...
        assert click_response_time <= 0.5, f"Click response took {click_response_time:.2f}s"
        
        # Try scrolling
        await page.evaluate("window.scrollBy(0, 100)")
        scroll_response_time = time.time() - start_time
        assert scroll_response_time <= 1.0, f"Scroll response took {scroll_response_time:.2f}s"
...
async def then_memory_usage_acceptable(page: Page, performance_context: PerformanceContext):
    ...
    try:
        ...
        if memory_info:
            ...
            assert usage_percentage <= 80, f"Memory usage {usage_percentage:.1f}% exceeds 80% limit"
    except:
...
async def then_content_loads_adaptively(page: Page):
    ...
    if responsive_images['total'] > 0:
        ...
        assert responsiveness >= 0.5, f"Only {responsiveness*100:.1f}% of images are responsive"
...
async def when_interact_with_charts(page: Page):
    ...
    chart_selectors = [
        ".chart", ".graph", "svg", "canvas",
        "[data-chart]", ".visualization", ".d3-chart"
    ]
...
async def then_touch_responds_immediately(page: Page):
    ...
    assert response_time <= 0.1, f"Touch response time {response_time:.3f}s exceeds 100ms"
...
async def then_api_responses_under_200ms(page: Page):
    ...
    for response in api_responses:
        if response['timing']:
            total_time = response['timing']['responseEnd'] - response['timing']['requestStart']
            if 'simple' in response['url'] or response['status'] == 200:
                assert total_time <= 200, f"API response {response['url']} took {total_time}ms"
...
async def then_no_performance_regression(performance_context: PerformanceContext):
    ...
    if performance_context.load_time:
        assert performance_context.load_time <= 5.0, f"Load time {performance_context.load_time:.2f}s indicates regression"
...
async def then_app_shell_loads_quickly(page: Page):
    ...
    if paint_timing > 0:
        ...
        assert fcp_seconds <= 2.0, f"First Contentful Paint {fcp_seconds:.2f}s exceeds 2s limit"
