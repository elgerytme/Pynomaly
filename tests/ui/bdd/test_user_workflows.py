...
async def given_dataset_with_anomalies(
    workflow_context: UserWorkflowContext, 
    sample_anomaly_dataset: str
):
...
async def when_upload_dataset(
    ...
    if file_input:
        ...
        name_selectors = ["#dataset-name", "[name='name']", "input[placeholder*='name' i]"]
...
async def when_create_detector(
    ...
    form_fields = [
        ...
        ("#detector-description", "[name='description']", detector_config["description"]),
    ]
...
async def then_see_dataset_in_list(
    ...
    assert "dataset" in page_content.lower() or dataset_found, "Dataset should be visible or page should contain dataset-related content"
...
async def then_see_training_success(
    ...
    has_training_content = any(keyword in page_content.lower() for keyword in training_keywords)
    
    assert training_success or has_training_content, "Training success should be indicated"
...
async def then_see_detection_results(
    ...
    has_results_content = any(keyword in page_content.lower() for keyword in results_keywords)
...
async def then_export_results(
    ...
    has_export_content = any(keyword in page_content.lower() for keyword in export_keywords)
    
    assert export_found or has_export_content, "Export functionality should be available"
...
async def then_see_error_message(
    ...
    has_error_content = any(keyword in page_content.lower() for keyword in error_keywords)
    
    assert error_found or has_error_content, "Error message should be displayed for invalid input"