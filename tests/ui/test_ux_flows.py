...
class TestUXFlows:
    ...
    def test_complete_detector_creation_flow(self, detectors_page, sample_detector_data):
        ...
        assert layout_check["has_create_form"], "Detector creation form should be available"
    ...
    def test_navigation_flow_between_pages(self, dashboard_page):
        ...
        for path, expected_title_part in navigation_flow:
            ...
            if expected_title_part != "Dashboard":  # Dashboard doesn't always have title in content
    ...
    def test_form_validation_flow(self, detectors_page):
        ...
        if detectors_page.page.locator(detectors_page.CREATE_BUTTON).count() > 0:
            ...
            if detectors_page.page.locator(detectors_page.DETECTOR_NAME_INPUT).count() > 0:
                detectors_page.fill_input(detectors_page.DETECTOR_NAME_INPUT, "Test Detector")
    ...
    def test_error_recovery_flow(self, detection_page):
        ...
        if detection_page.page.locator(detection_page.RUN_DETECTION_BUTTON).count() > 0:
            ...
            detector_select = detection_page.page.locator(detection_page.DETECTOR_SELECT)
            dataset_select = detection_page.page.locator(detection_page.DATASET_SELECT)
            
            if detector_select.count() > 0:
                assert detector_select.is_enabled(), "Form should remain usable after error"
                
            if dataset_select.count() > 0:
                assert dataset_select.is_enabled(), "Form should remain usable after error"
    ...
    def test_accessibility_navigation_flow(self, page: Page):
        ...
        assert tab_count < max_tabs, "Should be able to navigate to interactive elements"
    ...
    def test_user_feedback_flow(self, page: Page):
        ...
        if forms.count() > 0:
            ...
            if submit_button.count() > 0:
                ...
                feedback_provided = (
                    updated_messages.count() > message_areas.count() or
                    loading_indicators.count() > 0
                )
