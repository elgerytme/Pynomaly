...
class BDDTestRunner:
    ...
    def generate_bdd_report(self, feature_analysis: Dict[str, Any], 
                           execution_result: Dict[str, Any],
                           junit_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not execution_result["success"]:
            report["recommendations"].append("Review failed tests and fix implementation issues")
        
        if junit_results.get("failures", 0) > 0:
            report["recommendations"].append(f"Address {junit_results['failures']} test failures")
        
        if junit_results.get("errors", 0) > 0:
            report["recommendations"].append(f"Fix {junit_results['errors']} test errors")
        
        if feature_analysis["total_scenarios"] == 0:
            report["recommendations"].append("Add more BDD scenarios to improve test coverage")
        
        # Calculate success rate
        total_tests = junit_results.get("total_tests", 0)
        if total_tests > 0:
            passed_tests = total_tests - junit_results.get("failures", 0) - junit_results.get("errors", 0)
    ...
    def save_results(self, report: Dict[str, Any]):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Total Features: {report['execution_summary']['total_features']}\n")
            f.write(f"Total Scenarios: {report['execution_summary']['total_scenarios']}\n")
            f.write(f"Total Steps: {report['execution_summary']['total_steps']}\n")
            
            if 'success_rate' in report['execution_summary']:
                f.write(f"Success Rate: {report['execution_summary']['success_rate']}%\n")
            
            f.write("\nTest Results:\n")
            test_results = report.get('test_results', {})
            f.write(f"  Total Tests: {test_results.get('total_tests', 0)}\n")
            f.write(f"  Passed: {test_results.get('total_tests', 0) - test_results.get('failures', 0) - test_results.get('errors', 0)}\n")
    ...
    def run_comprehensive_bdd_tests(self, test_categories: List[str] = None) -> Dict[str, Any]:
        ...
        print(f"Analysis complete: {feature_analysis['total_features']} features, "
        ...
        report = self.generate_bdd_report(feature_analysis, execution_result, junit_results)
