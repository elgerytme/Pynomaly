...
# Configuration
ACCESSIBILITY_TESTING_ENABLED = os.getenv("ACCESSIBILITY_TESTING", "true").lower() == "true"
...
class AccessibilityTester:
    ...
    def _process_axe_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_tests = (processed["summary"]["violations_count"] + 
        ...
        processed["recommendations"] = self._generate_recommendations(processed["violations"])
    ...
    def _categorize_violations(self, violations: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for violation in violations:
            ...
            elif any(keyword in rule_id for keyword in ["keyboard", "focus", "tabindex"]):
                categories["keyboard_navigation"].append(violation)
            elif any(keyword in rule_id for keyword in ["aria", "screen-reader", "label"]):
            ...
            elif any(keyword in rule_id for keyword in ["heading", "landmark", "region"]):
    ...
    def _generate_recommendations(self, violations: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, str]]:
    ...
    async def _save_accessibility_report(self, results: Dict[str, Any]):
        ...
        filename = f"accessibility_report_{url_safe}_{timestamp[:19].replace(':', '-')}.json"
    ...
    async def test_keyboard_navigation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, element in enumerate(visible_elements[:10]):  # Test first 10 elements
...
@pytest.mark.skipif(not ACCESSIBILITY_TESTING_ENABLED, reason="Accessibility testing disabled")
class TestAccessibilityCompliance:
    """Comprehensive WCAG 2.1 AA accessibility compliance tests."""
    
    async def test_homepage_accessibility(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        assert results["summary"]["overall_score"] >= 90, f"Accessibility score too low: {results['summary']['overall_score']}%"
    
    async def test_detectors_page_accessibility(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        assert len(form_violations) <= 2, f"Too many form accessibility issues: {form_violations}"
        ...
        elements_without_focus = [
            el for el in keyboard_results.get("tab_order", [])
            if not el.get("has_focus_indicator", False)
        ]
        assert len(elements_without_focus) <= 1, f"Elements missing focus indicators: {elements_without_focus}"
    
    async def test_datasets_page_accessibility(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        if await page.query_selector("table"):
            ...
            for table in table_results:
                assert table["hasTheadTbody"], "Tables should have thead and tbody elements"
                assert table["hasThElements"], "Tables should have th elements for headers"
    
    async def test_detection_page_accessibility(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        screen_reader_results = await accessibility_tester.test_screen_reader_compatibility()
        
        # Check form labels
        form_elements = screen_reader_results.get("form_labels", [])
        unlabeled_elements = [
            el for el in form_elements 
            if not (el.get("hasLabel") or el.get("hasAriaLabel") or el.get("hasAriaLabelledBy"))
        ]
        
        assert len(unlabeled_elements) == 0, f"Form elements without labels: {unlabeled_elements}"
    
    async def test_visualizations_page_accessibility(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        if images:
            images_without_alt = [img for img in images if not img.get("hasAlt") and not img.get("ariaLabel")]
            assert len(images_without_alt) == 0, f"Images without alternative text: {images_without_alt}"
    
    async def test_keyboard_navigation_comprehensive(self, accessibility_tester: AccessibilityTester, page: Page):
        """Test comprehensive keyboard navigation across the application."""
        pages_to_test = ["/", "/detectors", "/datasets", "/detection", "/visualizations"]
        ...
        for url in pages_to_test:
            ...
            assert len(keyboard_results.get("tab_order", [])) > 0, f"No keyboard navigation possible on {url}"
            
            # Check for keyboard traps
            traps = keyboard_results.get("keyboard_traps", [])
            assert len(traps) == 0, f"Keyboard traps found on {url}: {traps}"
    
    async def test_color_contrast_compliance(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        for url in pages_to_test:
            ...
            assert len(contrast_violations) == 0, f"Color contrast violations on {url}: {contrast_violations}"
    
    async def test_screen_reader_compatibility_comprehensive(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        if await page.query_selector("nav"):
            assert "navigation" in landmark_roles or "nav" in landmark_roles, "Navigation should be marked as landmark"
    
    async def test_wcag_aaa_compliance(self, accessibility_tester: AccessibilityTester, page: Page):
        ...
        assert total_violations <= 5, f"Too many WCAG AAA violations: {total_violations}"


# Utility functions for accessibility testing
def generate_accessibility_report_summary(results_dir: Path = ACCESSIBILITY_REPORTS_DIR) -> Dict[str, Any]:
    ...
    for report_file in results_dir.glob("*.json"):
        try:
            ...
            summary["total_violations"] += data.get("summary", {}).get("violations_count", 0)
            
            # Aggregate violation categories
            for category, violations in data.get("violations", {}).items():
                if violations:
                    summary["violation_categories"][category] = summary["violation_categories"].get(category, 0) + len(violations)
