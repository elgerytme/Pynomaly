...
from tests.ui.conftest import UITestHelper, TEST_CONFIG, ACCESSIBILITY_REPORTS_DIR
...
class AccessibilityTester:
    ...
    async def inject_axe(self):
        ...
        try:
            ...
            await self.page.wait_for_function("() => typeof axe !== 'undefined'", timeout=10000)
            ...
            if len(interactive_elements) > 0:
                ...
                for i, element in enumerate(interactive_elements[:10]):
                    try:
                        ...
                        navigation_results["issues"].append(f"Element {i} not keyboard accessible")
    ...
    def analyze_violations(self, results: dict) -> dict:
        ...
        for violation in results["violations"]:
            ...
            analysis["by_type"][violation_id] = analysis["by_type"].get(violation_id, 0) + 1
            
            # Categorize violations
            if any(tag in violation.get("tags", []) for tag in ["wcag2a", "wcag2aa", "wcag21aa"]):
    ...
    def assert_no_violations(self, results: dict, severity: str = "serious"):
        ...
        if filtered_violations:
            ...
            raise AssertionError(
                ...
                + (f"\n... and {len(filtered_violations) - 10} more" if len(filtered_violations) > 10 else "")
            )
...
class TestAccessibilityEnhanced:
    ...
    async def test_homepage_accessibility_comprehensive(
        ...
        if "keyboard_navigation" in manual_results and "error" not in manual_results["keyboard_navigation"]:
            ...
            if total_elements > 0:
                ...
                assert accessibility_ratio >= 0.8, f"At least 80% of interactive elements should be keyboard accessible. Got {accessibility_ratio:.2%}"

        print(f"Homepage accessibility summary:")
        print(f"  - Total violations: {violation_analysis['total']}")
        print(f"  - By severity: {violation_analysis['by_severity']}")
        print(f"  - Manual checks completed: {len([k for k, v in manual_results.items() if 'error' not in v])}")
    ...
    async def test_form_accessibility_comprehensive(
        ...
        form_elements = await page.query_selector_all("form input, form select, form textarea, form button")
        ...
        if form_elements:
            ...
            for i in range(min(len(form_elements), 5)):
                ...
                assert focused_element is not None, f"Focus should be maintained during form navigation"
    ...
    async def test_color_contrast_comprehensive(
        ...
        color_violations = [v for v in results.get("violations", []) if "color" in v.get("id", "")]
        ...
        serious_color_violations = [
            v for v in color_violations 
            if v.get("impact") in ["serious", "critical"]
        ]
        
        assert len(serious_color_violations) == 0, f"Found {len(serious_color_violations)} serious color contrast violations"
    ...
    async def test_screen_reader_compatibility(
        ...
        assert "H1" in semantic_check["headingStructure"], "Page should have an H1 heading"

        accessibility_tester.assert_no_violations(results, "serious")

    @pytest.mark.asyncio
    async def test_responsive_accessibility(
        self, 
        page: Page, 
        ui_helper: UITestHelper
    ):
        ...
        for viewport in viewports:
            ...
            results = await accessibility_tester.run_axe_scan()
            await accessibility_tester.save_results(results, f"responsive_{viewport['name']}")
            ...
            print(f"Responsive accessibility ({viewport['name']}): {len(serious_violations)} serious violations")
    ...
    def generate_accessibility_report(self) -> dict:
        ...
        if ACCESSIBILITY_REPORTS_DIR.exists():
            result_files = list(ACCESSIBILITY_REPORTS_DIR.glob("*_accessibility_*.json"))
            ...
            for result_file in result_files:
                try:
                    ...
                    wcag_violations = [v for v in violations if any(tag in v.get("tags", []) for tag in ["wcag2a", "wcag2aa", "wcag21aa"])]
        ...
        if total_violations > 0:
            if report["violation_summary"]["critical"] > 0:
                report["recommendations"].append("Address critical accessibility violations immediately")
            if report["violation_summary"]["serious"] > 5:
                report["recommendations"].append("Focus on serious violations - consider accessibility audit")
            if report["violation_summary"]["color_contrast"] > 0:
                report["recommendations"].append("Review color contrast ratios for WCAG AA compliance")
