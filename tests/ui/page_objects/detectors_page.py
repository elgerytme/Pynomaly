...
class DetectorsPage(BasePage):
    ...
    DETECTOR_DESCRIPTION_INPUT = "textarea[name='description'], input[name='description']"
    ...
    def create_detector(self, detector_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.page.locator(self.DETECTOR_ALGORITHM_SELECT).count() > 0:
                self.select_option(self.DETECTOR_ALGORITHM_SELECT, detector_data["algorithm"])
                
            if self.page.locator(self.DETECTOR_DESCRIPTION_INPUT).count() > 0:
                self.fill_input(self.DETECTOR_DESCRIPTION_INPUT, detector_data.get("description", ""))
                
            if self.page.locator(self.CONTAMINATION_INPUT).count() > 0:
                self.fill_input(self.CONTAMINATION_INPUT, str(detector_data.get("contamination", 0.1)))
    ...
    def get_detector_list(self) -> List[Dict[str, str]]:
        ...
        for i in range(detector_items.count()):
            ...
            for line in lines:
                ...
                elif len(line) > 10 and not any(x in line for x in ["Name:", "Algorithm:", "Status:"]):
    ...
    def verify_detectors_page_layout(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "has_algorithm_select": self.page.locator(self.DETECTOR_ALGORITHM_SELECT).count() > 0,
    ...
    def test_form_validation(self) -> Dict[str, bool]:
        ...
        if self.page.locator(self.CREATE_BUTTON).count() > 0:
            ...
            validation_messages = self.page.locator(".error, .invalid, [class*='error'], [class*='invalid']")
    ...
    def get_form_errors(self) -> List[str]:
        ...
        errors = []
        error_elements = self.page.locator(".error, .invalid, [class*='error'], [class*='invalid']")
