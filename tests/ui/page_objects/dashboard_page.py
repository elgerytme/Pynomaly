...
class DashboardPage(BasePage):
            ...
            for i in range(rows.count()):
                ...
                if cells.count() >= 3:
                    results.append({
                        ...
                        "anomalies": cells.nth(3).text_content() or "" if cells.count() > 3 else ""
                    })
    ...
    def click_quick_action(self, action_title: str) -> None:
        ...
        for i in range(action_cards.count()):
            ...
            if title_elem.count() > 0 and action_title in (title_elem.text_content() or ""):
                card.click()
                break
    ...
    def verify_dashboard_layout(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "has_recent_results": self.page.locator(self.RECENT_RESULTS_TABLE).count() > 0,
    ...
    def test_htmx_functionality(self) -> Dict[str, bool]:
        ...
        if self.page.locator(self.REFRESH_BUTTON).count() > 0:
            initial_content = self.page.locator(self.RECENT_RESULTS_TABLE).text_content()
            self.refresh_results()
            updated_content = self.page.locator(self.RECENT_RESULTS_TABLE).text_content()
