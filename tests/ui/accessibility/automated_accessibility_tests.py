...
class AccessibilityTestRunner:
    ...
    async def run_accessibility_test_suite(
        ...
        async with async_playwright() as p:
            for browser_name in config["browsers"]:
                ...
                browser_results = await self._test_browser(
                    browser, browser_name, config, base_url
                )
        ...
        suite_results["summary"] = self._generate_suite_summary(
            suite_results["browser_results"]
        )
        suite_results["compliance_scores"] = self._calculate_suite_compliance(
            suite_results["browser_results"]
        )
        suite_results["critical_issues"] = self._extract_critical_issues(
            suite_results["browser_results"]
        )
        ...
        print(
            f"✅ Accessibility test suite completed in {suite_results['execution_time']:.1f}s"
        )
        print(
            f"📊 Overall Compliance: {suite_results['compliance_scores']['average']:.1f}%"
        )
    ...
    async def _test_browser(
        ...
        for viewport in config["viewports"]:
            ...
            viewport_results = await self._test_viewport(
                page, config["pages"], base_url
            )
        ...
        browser_results["summary"] = self._aggregate_browser_results(
            browser_results["viewport_results"]
        )
        browser_results["compliance_score"] = self._calculate_browser_compliance(
            browser_results["viewport_results"]
        )
    ...
    async def _test_viewport(
        ...
        for page_path in pages:
            ...
            try:
                ...
                pynomaly_result = await self._run_pynomaly_specific_tests(
                    page, page_path
                )

                # Combine results
                combined_result = self._combine_test_results(
                    wcag_result, pynomaly_result
                )
        ...
        viewport_results["summary"] = self._calculate_viewport_summary(
            viewport_results["page_results"]
        )
        viewport_results["compliance_score"] = self._calculate_viewport_compliance(
            viewport_results["page_results"]
        )
    ...
    async def _test_chart_accessibility(self, page: Page) -> dict[str, Any]:
        ...
        charts = await page.query_selector_all(
            "svg, canvas, [data-component='anomaly-chart'], .chart"
        )
    ...
    async def _test_pwa_accessibility(self, page: Page) -> dict[str, Any]:
        ...
        if page.url.endswith("/offline"):
            ...
            offline_message = await page.query_selector(
                ".offline-message, .offline-animation"
            )
            if offline_message:
                test_result["passes"].append(
                    {
        ...
        install_buttons = await page.query_selector_all(
    ...
    def _combine_test_results(self, wcag_result, pynomaly_result) -> dict[str, Any]:
        ...
        return {
            ...
            "compliance_score": max(
                0, 100 - (len(combined_violations) * 5)
            ),  # Simple scoring
    ...
    def _calculate_suite_compliance(
        ...
        average_score = (
            sum(browser_scores.values()) / len(browser_scores) if browser_scores else 0
        )
    ...
    def _extract_critical_issues(
        ...
        for browser, browser_data in browser_results.items():
            for viewport, viewport_data in browser_data.get(
                "viewport_results", {}
            ).items():
                for page_result in viewport_data.get("page_results", []):
                    for violation in page_result.get("violations", []):
                        if (
                            hasattr(violation, "impact")
                            and violation.impact == "critical"
                        ):
    ...
    async def _save_suite_report(self, suite_results: dict[str, Any]):
        ...
        # Save JSON report
        json_path = (
            self.reports_dir / f"accessibility_suite_{scenario}_{timestamp}.json"
        )
        ...
        # Save HTML report
        html_path = (
            self.reports_dir / f"accessibility_suite_{scenario}_{timestamp}.html"
        )
        ...
        summary_path = (
            self.reports_dir / f"accessibility_summary_{scenario}_{timestamp}.txt"
        )
...
class TestAutomatedAccessibility:
    ...
    async def test_accessibility_smoke(self, page: Page):
        ...
        assert (
            results["compliance_scores"]["average"] >= 70.0
        ), f"Accessibility smoke test failed with {results['compliance_scores']['average']:.1f}% compliance"
        assert (
            len(results["critical_issues"]) == 0
        ), f"Found {len(results['critical_issues'])} critical accessibility issues"
    ...
    async def test_accessibility_comprehensive(self, page: Page):
        ...
        assert (
            results["compliance_scores"]["average"] >= 80.0
        ), f"Comprehensive accessibility test failed with {results['compliance_scores']['average']:.1f}% compliance"
        assert (
            len(results["critical_issues"]) == 0
        ), "Critical accessibility issues found"
        assert (
            results["summary"]["total_violations"] <= 20
        ), f"Too many accessibility violations: {results['summary']['total_violations']}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:8000", help="Base URL to test"
    )
    parser.add_argument(
        "--ci",
        action="store_true",
    ...
    async def main():
        runner = AccessibilityTestRunner()
        results = await runner.run_accessibility_test_suite(
