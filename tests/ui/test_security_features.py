...
class TestWebApplicationFirewall:
    ...
    def test_request_analysis(self, mock_request):
        ...
        assert blocked
        assert len(events) >= 2  # Should detect both SQL injection and blocked user agent
        
        # Check event details
        sql_events = [e for e in events if e.event_type == SecurityEventType.SQL_INJECTION_ATTEMPT]
        agent_events = [e for e in events if e.event_type == SecurityEventType.SUSPICIOUS_PATTERN]
...
class TestSecurityIntegration:
    ...
    def test_malicious_request_blocking(self):
        ...
        response = client.get("/test?id=1' OR 1=1 --", headers={"User-Agent": "sqlmap/1.0"})
        assert response.status_code == 403
