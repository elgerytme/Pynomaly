...
class UITestRunner:
    ...
    def test_dashboard_loads(self, page):
        ...
        try:
            ...
            # Take screenshot
            page.screenshot(
                path=str(self.screenshots_dir / "dashboard_test.png"), full_page=True
            )
        ...
        try:
            ...
            for url_path, _expected_title in pages:
                try:
                    ...
                    page.screenshot(
                        path=str(self.screenshots_dir / f"{page_name}_test.png"),
                        full_page=True,
                    )
            ...
            if success_rate >= 0.8:  # 80% success rate
                self.log_test(
                    "Navigation", "PASS", f"{nav_success}/{len(pages)} pages", duration
                )
                return True
            else:
                self.log_test(
                    "Navigation",
                    "FAIL",
                    f"Only {nav_success}/{len(pages)} pages",
                    duration,
                )
                return False
        ...
        try:
            ...
            for device_name, viewport in viewports:
                try:
                    ...
                    page.screenshot(
                        path=str(
                            self.screenshots_dir / f"responsive_{device_name}.png"
                        ),
                        full_page=True,
                    )
            ...
            if responsive_success == len(viewports):
                ...
                self.log_test(
                    "Responsive Design",
                    "PARTIAL",
                    f"{responsive_success}/{len(viewports)} viewports",
                    duration,
                )
                return False
    ...
    def test_mobile_menu(self, page):
        ...
        try:
            ...
            page.screenshot(
                path=str(self.screenshots_dir / "mobile_menu_open.png"), full_page=True
            )
        ...
        try:
            ...
            if interactive_success >= 2:  # At least 2 out of 3 buttons working
                self.log_test(
                    ...
                    "PASS",
                    f"{interactive_success}/{len(buttons)} elements",
                    duration,
                )
                return True
            else:
                self.log_test(
                    "Interactive Elements",
                    "FAIL",
                    f"Only {interactive_success}/{len(buttons)} elements",
                    duration,
                )
                return False
    ...
    def test_performance(self, page):
        ...
        try:
            ...
            if load_time < 5.0:  # Reasonable load time
                self.log_test(
                    "Performance", "PASS", f"Load time: {load_time:.2f}s", duration
                )
                return True
            else:
                self.log_test(
                    "Performance", "FAIL", f"Slow load time: {load_time:.2f}s", duration
                )
                return False
