...
class UITestRunner:
    ...
    def __init__(self, base_url: str = "http://localhost:8000", headless: bool = True):
        ...
        print(f"🚀 Pynomaly UI Test Suite Runner")
    ...
    def check_server_availability(self) -> bool:
        ...
        try:
            ...
            print("   poetry run uvicorn pynomaly.presentation.api:app --host 127.0.0.1 --port 8000")
            return False
            ...
            with sync_playwright() as p:
                ...
                for url_path, name in pages_to_capture:
                    try:
                        ...
                        screenshot_path = self.screenshots_dir / f"app_{name}_viewport.png"
    ...
    def analyze_screenshots(self) -> Dict[str, Any]:
        ...
        for screenshot in screenshot_files:
            ...
            if "mobile" in screenshot.name:
                analysis["screenshots_by_type"]["mobile"] = analysis["screenshots_by_type"].get("mobile", 0) + 1
            elif "tablet" in screenshot.name:
                analysis["screenshots_by_type"]["tablet"] = analysis["screenshots_by_type"].get("tablet", 0) + 1
            elif "desktop" in screenshot.name or "app_" in screenshot.name:
                analysis["screenshots_by_type"]["desktop"] = analysis["screenshots_by_type"].get("desktop", 0) + 1
            else:
                analysis["screenshots_by_type"]["other"] = analysis["screenshots_by_type"].get("other", 0) + 1
        
        if file_sizes:
            file_sizes.sort(key=lambda x: x[1])
            analysis["smallest_screenshot"] = {"name": file_sizes[0][0], "size": file_sizes[0][1]}
            analysis["largest_screenshot"] = {"name": file_sizes[-1][0], "size": file_sizes[-1][1]}
    ...
    def generate_html_report(self) -> str:
        ...
        report_path = self.reports_dir / f"ui_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
