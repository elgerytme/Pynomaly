...
class TestWebAppAutomation:
    ...
    def test_navigation_menu(self, dashboard_page):
        ...
        for link_text, expected_url in nav_links:
            ...
            if nav_link.count() > 0:
                ...
                assert expected_url in current_url, f"Expected {expected_url} in {current_url}"
    ...
    def test_responsive_design(self, browser):
        ...
        for viewport in viewports:
            ...
            try:
                ...
                screenshot_path = Path("screenshots") / f"responsive_{viewport['name']}_dashboard.png"
                ...
                if viewport["width"] <= 768:
                    ...
                    if mobile_menu_btn.count() > 0:
                        ...
                        page.screenshot(path=f"screenshots/responsive_{viewport['name']}_menu_open.png")
    ...
    def test_detectors_page_functionality(self, detectors_page):
        ...
        detector_table = detectors_page.page.locator('table, .empty-state, [data-testid="detector-list"]')
        expect(detector_table).to_be_visible()
        
        # Test create detector button if present
        create_btn = detectors_page.page.locator('button:has-text("Create"), a:has-text("Create")')
    ...
    def test_datasets_page_functionality(self, datasets_page):
        ...
        dataset_content = datasets_page.page.locator('table, .empty-state, [data-testid="dataset-list"]')
        expect(dataset_content).to_be_visible()
        
        # Test upload functionality if present
        upload_btn = datasets_page.page.locator('button:has-text("Upload"), input[type="file"]')
    ...
    def test_detection_workflow(self, detection_page):
        ...
        detection_form = detection_page.page.locator('form, .detection-form, [data-testid="detection-form"]')
        if detection_form.count() > 0:
            detection_page.take_screenshot("detection_form_available")
        
        # Check for recent results
        results_section = detection_page.page.locator('.results, [data-testid="results"]')
    ...
    def test_visualizations_page(self, visualizations_page):
        ...
        chart_containers = visualizations_page.page.locator('[id*="chart"], .chart, canvas, svg')
