...
class TestPerformanceMonitoring:
    ...
    def test_page_load_performance(self, page: Page):
        ...
        assert metrics["domContentLoaded"] < 1000, f"DOM content loaded too slow: {metrics['domContentLoaded']}ms"
        assert metrics["loadComplete"] < 3000, f"Page load too slow: {metrics['loadComplete']}ms"
        assert metrics["firstContentfulPaint"] < 2000, f"FCP too slow: {metrics['firstContentfulPaint']}ms"
    ...
    def test_interactive_performance(self, dashboard_page):
        ...
        if form_input.count() > 0:
            ...
            assert input_response < 100, f"Form input response too slow: {input_response}ms"
    ...
    def test_chart_rendering_performance(self, visualizations_page):
        ...
        if charts_loaded:
            assert chart_load_time < 5000, f"Chart rendering too slow: {chart_load_time}ms"
            ...
            if charts:
                ...
                if first_chart.count() > 0:
                    ...
                    assert hover_response < 200, f"Chart hover response too slow: {hover_response}ms"
    ...
    def test_htmx_performance(self, dashboard_page):
        ...
        if refresh_button.count() > 0:
            ...
            assert htmx_response_time < 2000, f"HTMX response too slow: {htmx_response_time}ms"
    ...
    def test_memory_usage(self, page: Page):
        ...
        if initial_memory:
            ...
            if final_memory:
                ...
                assert memory_increase_mb < 20, f"Excessive memory usage increase: {memory_increase_mb:.2f}MB"
    ...
    def test_resource_loading_efficiency(self, page: Page):
        ...
        external_cdn_requests = [r for r in requests if "cdn." in r["url"] or "unpkg.com" in r["url"]]
    ...
    def test_core_web_vitals(self, page: Page):
        ...
        if "lcp" in vitals and vitals["lcp"] > 0:
            assert vitals["lcp"] < 2500, f"LCP too slow: {vitals['lcp']}ms (should be < 2.5s)"
            
        if "cls" in vitals:
            assert vitals["cls"] < 0.1, f"CLS too high: {vitals['cls']} (should be < 0.1)"
