...
class TestLayoutValidation:
    ...
    def test_dashboard_layout_elements(self, dashboard_page):
        ...
        assert layout_check["has_recent_results"], "Dashboard should have recent results section"
    ...
    def test_navigation_consistency(self, page: Page):
        ...
        nav_counts = [result["nav_links_count"] for result in navigation_results.values()]
    ...
    def test_responsive_navigation(self, mobile_page: Page):
        ...
        assert not desktop_nav.is_visible(), "Desktop navigation should be hidden on mobile"
    ...
    def test_form_elements_validation(self, detectors_page):
        ...
        assert name_input.get_attribute("name") is not None, "Name input should have name attribute"
        
        algorithm_select = detectors_page.page.locator(detectors_page.DETECTOR_ALGORITHM_SELECT)
        assert algorithm_select.get_attribute("name") is not None, "Algorithm select should have name attribute"
    ...
    def test_button_states(self, page: Page):
        ...
        for i in range(buttons.count()):
            ...
            if button_text and "submit" in button_text.lower():
                assert button_type == "submit", "Submit buttons should have type='submit'"
    ...
    def test_loading_states(self, detection_page):
        ...
        if htmx_indicators.count() > 0:
            ...
            for i in range(htmx_indicators.count()):
                ...
                assert "htmx-indicator" in indicator_class, "Should have htmx-indicator class"
    ...
    def test_layout_consistency_viewport_sizes(self, page: Page):
        ...
        for viewport in viewports:
            ...
            page.screenshot(path=f"screenshots/layout_{viewport['name']}.png", full_page=True)
            
        # All viewports should have main content and navigation
        for result in layout_results.values():
            assert result["main_visible"], "Main content should be visible on all viewports"
            assert result["nav_visible"], "Navigation should be visible on all viewports"