...
from pynomaly.application.dto.configuration_dto import (
    DatasetCharacteristicsDTO, ConfigurationLevel, ConfigurationSource,
    ExperimentConfigurationDTO, AlgorithmConfigurationDTO, DatasetConfigurationDTO,
    ConfigurationMetadataDTO, PerformanceResultsDTO
)
...
class TestConfigurationRecommendationService:
    ...
    async def test_train_recommendation_models_insufficient_data(
        ...
        assert "available_configurations" in results or "configurations_processed" in results
    ...
    async def test_train_recommendation_models_with_data(
        ...
        if "error" not in results:
            ...
            assert "configurations_processed" in results or "available_configurations" in results
    ...
    def test_get_recommendation_statistics(self, recommendation_service):
        ...
        expected_keys = [
            "total_recommendations", "ml_recommendations", 
            "similarity_recommendations", "performance_predictions",
            "successful_predictions", "model_training_count"
        ]
    ...
    async def test_similarity_based_recommendations(
        self, recommendation_service, repository, sample_configurations, sample_dataset_characteristics
    ):
        ...
        recommendations = await recommendation_service._generate_similarity_recommendations(
    ...
    @pytest.mark.asyncio
    async def test_rule_based_recommendations_edge_cases(
        self, recommendation_service
    ):
        ...
        recommendations = await recommendation_service._generate_rule_based_recommendations(
        ...
        sparse_recommendations = await recommendation_service._generate_rule_based_recommendations(
    ...
    async def test_preprocessing_suggestions(self, recommendation_service):
        ...
        suggestions = recommendation_service._get_preprocessing_suggestions(high_dim_dataset)
    ...
    async def test_recommendation_deduplication(
        ...
        from pynomaly.application.dto.configuration_dto import ConfigurationRecommendationDTO
        ...
        assert isolation_rec.confidence_score >= 0.8  # Should be the higher confidence one
