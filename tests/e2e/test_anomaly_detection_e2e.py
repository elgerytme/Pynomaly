...
class TestEndToEndAnomalyDetection:
    ...
    def test_complete_detection_workflow_via_api(self, client, sample_data):
        ...
        with patch('pynomaly.application.services.anomaly_detection_service.AnomalyDetectionService.detect_anomalies') as mock_detect:
            mock_detect.return_value = DetectionResult(
                detector_id=detector_id,
                dataset_id=dataset_id,
                scores=[AnomalyScore(np.random.random()) for _ in range(len(sample_data))],
    ...
    def test_model_training_and_inference_workflow(self, client, sample_data):
        ...
        with patch('pynomaly.application.services.model_service.ModelService.train_model') as mock_train:
        ...
        with patch('pynomaly.application.services.model_service.ModelService.predict') as mock_predict:
            ...
            inference_response = client.post("/api/v1/models/predict", json=inference_request)
    ...
    def test_error_recovery_workflow(self, client, sample_data):
        ...
        invalid_detector_response = client.post("/api/v1/detectors", json=invalid_detector)
    ...
    def test_data_persistence_workflow(self, client, sample_data):
        ...
        assert updated_dataset["description"] == "Updated dataset for persistence testing"
    ...
    async def test_async_workflow(self, client, sample_data):
        ...
        async_detection_data = {
            "dataset_id": dataset_id,
            "detector_id": detector_id
        }
        
        with patch('pynomaly.application.services.anomaly_detection_service.AnomalyDetectionService.detect_anomalies_async') as mock_async_detect:
            # Mock async detection
            mock_async_detect.return_value = asyncio.Future()
            mock_async_detect.return_value.set_result(DetectionResult(
                detector_id=detector_id,
                dataset_id=dataset_id,
                scores=[AnomalyScore(np.random.random()) for _ in range(len(sample_data))],
            ...
            async_response = client.post("/api/v1/detect/async", json=async_detection_data)
            ...
            with patch('pynomaly.application.services.task_service.TaskService.get_task_result') as mock_get_result:
...
class TestRealWorldScenarios:
    ...
    def test_fraud_detection_scenario(self, client):
        ...
        fraud_data.loc[fraud_indices, 'transaction_amount'] *= 10  # Unusually high amounts
        fraud_data.loc[fraud_indices, 'num_transactions_today'] *= 5  # Unusually high frequency
    ...
    def test_network_intrusion_scenario(self, client):
        ...
        network_data.loc[intrusion_indices, 'packet_size'] *= 0.1  # Unusually small packets
    ...
    def test_large_scale_scenario(self, client):
        ...
        large_data = pd.DataFrame({
            f'feature_{i}': np.random.normal(0, 1, 50000)
            for i in range(20)
        })
