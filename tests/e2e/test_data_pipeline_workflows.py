...
class TestDataPipelineWorkflows:
    ...
    def test_complete_data_pipeline_workflow(self, app_client, raw_datasets):
        ...
        try:
            ...
            quality_response = app_client.post("/api/data/analyze-quality", json={
            ...
            preprocessing_response = app_client.post("/api/data/preprocess", json=preprocessing_config)
            ...
            report_response = app_client.post("/api/pipeline/generate-report", json={
    ...
    def test_categorical_data_pipeline(self, app_client, raw_datasets):
        ...
        try:
            ...
            analysis_response = app_client.post("/api/data/analyze-categorical", json={
            ...
            encoding_response = app_client.post("/api/data/encode-categorical", json=categorical_config)
    ...
    def test_time_series_pipeline(self, app_client, raw_datasets):
        ...
        try:
            ...
            ts_analysis_response = app_client.post("/api/data/analyze-timeseries", json={
                "dataset_id": dataset_id,
            ...
            ts_preprocessing_response = app_client.post("/api/data/preprocess-timeseries", json=ts_preprocessing)
            ...
            temporal_analysis_response = app_client.post("/api/analysis/temporal-anomalies", json={
                "dataset_id": processed_ts_id,
                "detector_id": detector_id,
                "timestamp_column": "timestamp",
        ...
        try:
            ...
            chunked_config = {
                ...
                "operations": [
                    ...
                    {"operation": "dimensionality_reduction", "method": "pca", "n_components": 5}
                ],
                "output_name": "Processed Large Dataset"
            }
            
            chunked_response = app_client.post("/api/data/process-chunked", json=chunked_config)
            ...
            batch_train_response = app_client.post(f"/api/detectors/{detector_id}/train-batch", json=batch_train_config)
            ...
            batch_detect_response = app_client.post(f"/api/detectors/{detector_id}/detect-batch", json=batch_detect_config)
    ...
    def test_data_validation_pipeline(self, app_client, raw_datasets):
        ...
        try:
                ...
                "validation_rules": [
                    {
                        ...
                        "expected_types": {"normal_feature": "float", "extreme_feature": "float", "bounded_feature": "float"}
                    },
                    {
                        "rule": "check_ranges",
                        "feature_ranges": {
            ...
            validation_response = app_client.post("/api/data/validate", json=validation_config)
            ...
            outlier_results = next((r for r in rule_results if r["rule"] == "detect_outliers"), None)
            ...
            revalidation_response = app_client.post("/api/data/validate", json={
                **validation_config,
