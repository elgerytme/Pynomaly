...
class TestSecurityAuthenticationWorkflows:
    ...
    def test_user_registration_login_workflow(self, app_client):
        ...
        register_response = app_client.post("/api/auth/register", json=registration_data)
    ...
    def test_api_key_authentication_workflow(self, app_client):
        ...
        api_key_response = app_client.post("/api/auth/api-keys", json=api_key_request, headers=headers)
        ...
        create_response = app_client.post("/api/detectors/", json=detector_data, headers=api_headers)
        assert create_response.status_code == 200  # Has write permission
        
        # Revoke API key
        revoke_response = app_client.delete(f"/api/auth/api-keys/{api_key_result['key_id']}", headers=headers)
    ...
    def test_role_based_access_control_workflow(self, app_client):
        ...
        # Login as admin
        admin_login = app_client.post("/api/auth/login", data={
        ...
        user_login = app_client.post("/api/auth/login", data={
            "username": "regularuser", "password": "UserPassword123!"
        })
        ...
        create_response = app_client.post("/api/detectors/", json=detector_data, headers=user_headers)
        assert create_response.status_code == 200
        detector_id = create_response.json()["id"]
        
        # User can access their own detector
        get_response = app_client.get(f"/api/detectors/{detector_id}", headers=user_headers)
        assert get_response.status_code == 200
        
        # Admin can access any detector
        admin_get_response = app_client.get(f"/api/detectors/{detector_id}", headers=admin_headers)
    ...
    def test_secure_data_upload_workflow(self, app_client, test_dataset):
        ...
        # Login
        login_response = app_client.post("/api/auth/login", data={
            "username": "datauser", "password": "DataPassword123!"
        })
        assert login_response.status_code == 200
        ...
        try:
            ...
            get_response = app_client.get(f"/api/datasets/{dataset_id}", headers=headers)
            assert get_response.status_code == 200
            ...
            download_response = app_client.get(f"/api/datasets/{dataset_id}/download", headers=headers)
    ...
    def test_session_management_workflow(self, app_client):
        ...
        # Login and get token
        login_response = app_client.post("/api/auth/login", data={
            "username": "sessionuser", "password": "SessionPassword123!"
        })
        assert login_response.status_code == 200
        login_result = login_response.json()
        ...
        login1 = app_client.post("/api/auth/login", data={
            "username": "sessionuser", "password": "SessionPassword123!"
        })
        login2 = app_client.post("/api/auth/login", data={
            "username": "sessionuser", "password": "SessionPassword123!"
        })
        ...
        logout_all_response = app_client.post("/api/auth/logout-all", 
                                            headers={"Authorization": f"Bearer {token1}"})
        assert logout_all_response.status_code == 200
        
        # Both tokens should be invalid
        for token in [token1, token2]:
            invalid_response = app_client.get("/api/auth/profile", 
                                            headers={"Authorization": f"Bearer {token}"})
            assert invalid_response.status_code == 401
    ...
    def test_password_security_workflow(self, app_client):
        ...
        login_response = app_client.post("/api/auth/login", data={
            "username": "stronguser", "password": "StrongPassword123!@#"
        })
        assert login_response.status_code == 200
        token = login_response.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        ...
        change_response = app_client.post("/api/auth/change-password", 
                                        json=change_password_data, headers=headers)
        assert change_response.status_code == 200
        
        # Test login with new password
        new_login_response = app_client.post("/api/auth/login", data={
            "username": "stronguser", "password": "NewStrongPassword456$%^"
        })
        assert new_login_response.status_code == 200
        
        # Test old password rejection
        old_login_response = app_client.post("/api/auth/login", data={
            "username": "stronguser", "password": "StrongPassword123!@#"
        })
        assert old_login_response.status_code == 401
        
        # Test password reset workflow
        reset_request_response = app_client.post("/api/auth/password-reset-request", json={
            "email": "strong@example.com"
        })
    ...
    def test_brute_force_protection_workflow(self, app_client):
        ...
        for i in range(6):  # Exceed rate limit threshold
            failed_login = app_client.post("/api/auth/login", data={
                "username": "bruteuser", "password": "WrongPassword"
            })
        ...
        lockout_response = app_client.post("/api/auth/login", data={
            "username": "bruteuser", "password": "BrutePassword123!"  # Correct password
        })
        ...
        legitimate_response = app_client.post("/api/auth/login", data={
            "username": "bruteuser", "password": "BrutePassword123!"
        })
    ...
    def test_data_encryption_workflow(self, app_client, test_dataset):
        ...
        # Login
        login_response = app_client.post("/api/auth/login", data={
            "username": "cryptouser", "password": "CryptoPassword123!"
        })
        assert login_response.status_code == 200
        token = login_response.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        ...
        try:
            ...
            dataset_info_response = app_client.get(f"/api/datasets/{dataset_id}/info", headers=headers)
            ...
            create_response = app_client.post("/api/detectors/", json=detector_data, headers=headers)
            assert create_response.status_code == 200
            detector_id = create_response.json()["id"]
            ...
            export_response = app_client.post(f"/api/datasets/{dataset_id}/export", json={
