...
class TestPerformanceScalingWorkflows:
    ...
    def test_load_testing_workflow(self, app_client, performance_datasets):
        ...
        try:
            ...
            def make_detection_request():
                ...
                response = app_client.post(f"/api/detectors/{detector_id}/detect", json={
                    "dataset_id": dataset_id
                })
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(make_detection_request) for _ in range(concurrent_requests)]
            ...
            max_response_time = np.max([r["response_time"] for r in successful_requests])
            ...
            report_response = app_client.post("/api/performance/load-test-report", json=load_report_request)
    ...
    def test_scalability_benchmarking_workflow(self, app_client, performance_datasets):
        ...
        for size_name in dataset_sizes:
            ...
            try:
                ...
                with open(dataset_file, 'rb') as file:
                    upload_response = app_client.post(
                        "/api/datasets/upload",
                        files={"file": (f"benchmark_{size_name}.csv", file, "text/csv")},
                ...
                # Benchmark detection
                detect_start = time.time()
                detect_response = app_client.post(f"/api/detectors/{detector_id}/detect", json={
                    "dataset_id": dataset_id
                })
                detect_time = time.time() - detect_start
        ...
        if len(benchmark_results) > 1:
            scalability_analysis = {
                "dataset_sizes": {name: results["dataset_size"] for name, results in benchmark_results.items()},
                "training_times": {name: results["train_time"] for name, results in benchmark_results.items()},
                "detection_times": {name: results["detect_time"] for name, results in benchmark_results.items()}
            }
            
            # Test scalability report generation
            scalability_report_response = app_client.post("/api/performance/scalability-report", 
                                                         json=scalability_analysis)
    ...
    def test_memory_optimization_workflow(self, app_client, performance_datasets):
        ...
        try:
            ...
            post_upload_memory_response = app_client.get("/api/performance/memory-usage")
            if post_upload_memory_response.status_code == 200:
                post_upload_memory = post_upload_memory_response.json()
                upload_memory_usage = post_upload_memory.get("used_memory_mb", 0) - initial_used
            ...
            train_start_memory_response = app_client.get("/api/performance/memory-usage")
            ...
            if (train_start_memory_response.status_code == 200 and 
                train_end_memory_response.status_code == 200):
                
                start_memory = train_start_memory_response.json().get("used_memory_mb", 0)
            ...
            # Test memory cleanup
            cleanup_response = app_client.post("/api/performance/memory-cleanup", json={
            ...
            profiling_response = app_client.post("/api/performance/memory-profile", json={
    ...
    def test_cpu_optimization_workflow(self, app_client, performance_datasets):
        ...
        try:
            ...
            create_response = app_client.post("/api/detectors/", json=single_thread_detector)
            assert create_response.status_code == 200
            single_detector_id = create_response.json()["id"]
            
            # Train single-threaded
            single_train_start = time.time()
            train_response = app_client.post(f"/api/detectors/{single_detector_id}/train", json={
                "dataset_id": dataset_id
            })
            ...
            create_response = app_client.post("/api/detectors/", json=multi_thread_detector)
            assert create_response.status_code == 200
            multi_detector_id = create_response.json()["id"]
            
            # Train multi-threaded
            multi_train_start = time.time()
            train_response = app_client.post(f"/api/detectors/{multi_detector_id}/train", json={
                "dataset_id": dataset_id
            })
            ...
            batch_response = app_client.post("/api/detection/batch-optimized", json=batch_request)
            ...
            profiling_response = app_client.post("/api/performance/cpu-profile", json=profiling_request)
            if profiling_response.status_code == 200:
                profiling_result = profiling_response.json()
    ...
    def test_high_dimensional_performance_workflow(self, app_client, performance_datasets):
        ...
        try:
            ...
            dim_reduction_response = app_client.post("/api/preprocessing/dimensionality-reduction", 
            ...
            feature_importance_response = app_client.post(f"/api/detectors/{detector_id}/feature-importance", 
            ...
            streaming_response = app_client.post("/api/detection/stream-high-dim", json=streaming_request)
    ...
    def test_auto_scaling_workflow(self, app_client):
        ...
        if scaling_status_response.status_code == 200:
            ...
            policy_response = app_client.post("/api/scaling/configure", json=scaling_policy)
            ...
            load_response = app_client.post("/api/scaling/simulate-load", json=load_simulation_request)
            ...
            if load_response.status_code == 200:
                ...
                scaling_events_response = app_client.get(f"/api/scaling/events?simulation_id={simulation_id}")
                ...
                if scaling_events_response.status_code == 200:
                    ...
                    scale_up_events = [e for e in events["events"] if e.get("action") == "scale_up"]
                    if scale_up_events:
            ...
            cooldown_response = app_client.post("/api/scaling/cooldown", json=cooldown_request)
            if cooldown_response.status_code == 200:
                ...
                if final_status_response.status_code == 200:
                    ...
                    assert final_status["current_instances"] >= scaling_policy["min_instances"]
    ...
    def test_caching_optimization_workflow(self, app_client, performance_datasets):
        ...
        try:
            ...
            cache_setup_response = app_client.post("/api/cache/configure", json=cache_config)
            if cache_setup_response.status_code == 200:
                # First detection (cache miss)
                first_detect_start = time.time()
                first_detect_response = app_client.post(f"/api/detectors/{detector_id}/detect", json={
                    "dataset_id": dataset_id
                })
                ...
                second_detect_response = app_client.post(f"/api/detectors/{detector_id}/detect", json={
                    "dataset_id": dataset_id
                })
            ...
            invalidation_response = app_client.post("/api/cache/invalidate", json={
