...
class TestMonitoringAlertingWorkflows:
    ...
    def streaming_data_generator(self):
        ...
        def generate_batch(batch_size=100, anomaly_rate=0.05):
            ...
            normal_data = np.random.multivariate_normal([0, 0], [[1, 0.3], [0.3, 1]], normal_size)
            
            # Generate anomalous data
            anomaly_size = batch_size - normal_size
            anomaly_data = np.random.multivariate_normal([3, 3], [[0.5, 0], [0, 0.5]], anomaly_size)
            ...
            df = pd.DataFrame({
                ...
                'batch_id': [f"batch_{int(time.time())}" for _ in range(batch_size)]
            })
    ...
    def test_real_time_monitoring_setup(self, app_client, streaming_data_generator):
        ...
        try:
            ...
            monitor_setup_response = app_client.post("/api/monitoring/setup", json=monitoring_config)
    ...
    def test_streaming_anomaly_detection(self, app_client, streaming_data_generator):
        ...
        try:
            ...
            session_response = app_client.post("/api/streaming/start", json=streaming_config)
            ...
            for i in range(5):
                ...
                batch_response = app_client.post("/api/streaming/process-batch", json=batch_request)
    ...
    def test_alert_system_workflow(self, app_client, streaming_data_generator):
        ...
        try:
            ...
            alert_setup_response = app_client.post("/api/alerts/setup", json=alert_config)
            ...
            for scenario in test_scenarios:
                ...
                detection_response = app_client.post("/api/detection/process-with-alerts", json=detection_request)
            ...
            assert len(history_result["alerts"]) > 0  # Should have some alerts from scenarios
    ...
    def test_model_drift_monitoring(self, app_client, streaming_data_generator):
        ...
        try:
            ...
            drift_setup_response = app_client.post("/api/monitoring/drift/setup", json=drift_config)
            ...
            for period in time_periods:
                ...
                drift_analysis_response = app_client.post("/api/monitoring/drift/analyze", json=drift_analysis_request)
                assert drift_analysis_response.status_code == 200
                drift_analysis_result = drift_analysis_response.json()
                
                drift_reports.append({
                    "period": period["name"],
                    "results": drift_analysis_result
                })
            ...
            significant_score = drift_reports[-1]["results"]["drift_scores"]["jensen_shannon"]
            assert significant_score > stable_score  # Drift should increase
            
            # Get comprehensive drift report
            report_response = app_client.get(f"/api/monitoring/drift/{drift_monitor_id}/report")
    ...
    def test_performance_monitoring_workflow(self, app_client, streaming_data_generator):
        ...
        try:
            ...
            perf_setup_response = app_client.post("/api/monitoring/performance/setup", json=perf_config)
            ...
            benchmark_response = app_client.post("/api/monitoring/performance/benchmark", json=benchmark_config)
            ...
            realtime_response = app_client.get(f"/api/monitoring/performance/{perf_monitor_id}/current")
