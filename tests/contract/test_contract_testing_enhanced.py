...
from pynomaly.domain.protocols import DetectorProtocol, AdapterProtocol, RepositoryProtocol
from pynomaly.application.dto import DetectionRequestDTO, TrainingRequestDTO, EvaluationResponseDTO
from pynomaly.presentation.api.schemas import DatasetSchema, DetectorSchema, PredictionSchema
    ...
    def api_schemas(self):
        ...
        return {
            ...
            "prediction_response": {
                ...
                "properties": {
                    "predictions": {"type": "array", "items": {"type": "integer", "enum": [0, 1]}},
                    "anomaly_scores": {"type": "array", "items": {"type": "number", "minimum": 0, "maximum": 1}},
                    "confidence_intervals": {"type": "array", "items": {"type": "array", "items": {"type": "number"}}},
                    "detector_id": {"type": "string"},
...
class TestServiceContractTesting:
    ...
    def test_detector_protocol_contract(self):
        ...
        for adapter in adapters:
            ...
            assert callable(getattr(adapter, 'fit'))
            assert callable(getattr(adapter, 'predict'))
            
            # Check method signatures match protocol
            fit_method = getattr(adapter, 'fit')
            predict_method = getattr(adapter, 'predict')
    ...
    def test_adapter_protocol_contract(self):
        ...
        assert isinstance(adapter, AdapterProtocol) or hasattr(adapter, 'validate_parameters')
        ...
        valid_events = [
            {
                ...
                "timestamp": "2024-01-01T12:00:00Z",
                "payload": {"detector_id": "detector_123", "algorithm": "IsolationForest"}
            },
            {
...
class TestInterfaceContractTesting:
    ...
    def test_database_interface_contract(self):
        ...
        from pynomaly.infrastructure.persistence.database import DatabaseInterface
    ...
    def test_monitoring_interface_contract(self):
        ...
        # Test interface compliance
        monitoring_methods = [
            'record_metric', 'increment_counter', 'record_histogram',
    ...
    def test_notification_interface_contract(self):
        ...
        # Test interface compliance
        notification_methods = ['send_notification', 'send_alert', 'subscribe', 'unsubscribe']
