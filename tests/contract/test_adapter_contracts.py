...
from pynomaly.infrastructure.adapters import SklearnAdapter, PyODAdapter
...
class AdapterContractTest(ABC):
    ...
    def test_adapter_provides_algorithms(self):
        ...
        assert all(isinstance(algo, str) for algo in algorithms), "Algorithm names must be strings"
    ...
    def test_adapter_creates_valid_detector(self):
        ...
        assert detector is not None, "create_detector() must handle contamination parameter"
        
        # Test with additional parameters
        detector = adapter.create_detector(algorithm, contamination=0.05, random_state=42)
        assert detector is not None, "create_detector() must handle additional parameters"
    ...
    def test_detector_has_required_methods(self):
        ...
        assert hasattr(detector, 'decision_function'), "Detector must have decision_function() method"
        
        # Methods should be callable
        assert callable(detector.fit), "fit() must be callable"
        assert callable(detector.predict), "predict() must be callable"
        assert callable(detector.decision_function), "decision_function() must be callable"
    ...
    def test_fit_predict_workflow(self):
        ...
        assert fit_result is None or fit_result is detector, "fit() must return None or self"
        
        # Predict on the same data
        predictions = detector.predict(data)
        assert isinstance(predictions, np.ndarray), "predict() must return numpy array"
        assert predictions.shape == (len(data),), f"predictions shape must be ({len(data)},)"
        assert predictions.dtype in [np.int32, np.int64], "predictions must be integer type"
        
        # Predictions should be binary (-1 for anomaly, 1 for normal in sklearn convention)
        unique_predictions = np.unique(predictions)
        assert len(unique_predictions) <= 2, "Should have at most 2 unique prediction values"
        assert all(pred in [-1, 1] for pred in unique_predictions), "Predictions must be -1 or 1"
    ...
    def test_decision_function_workflow(self):
        ...
        assert isinstance(scores, np.ndarray), "decision_function() must return numpy array"
    ...
    def test_deterministic_behavior(self):
        ...
        detector1 = adapter.create_detector(algorithm, contamination=0.1, random_state=42)
        detector2 = adapter.create_detector(algorithm, contamination=0.1, random_state=42)
        ...
        assert np.allclose(scores1, scores2, rtol=1e-10), "Scores should be deterministic"
    ...
    def test_contamination_parameter_effect(self):
        ...
        for contamination in contamination_rates:
            detector = adapter.create_detector(algorithm, contamination=contamination, random_state=42)
            detector.fit(data)
            predictions = detector.predict(data)
...
class TestAdapterInteroperability:
    ...
    def test_adapter_interface_consistency(self):
        ...
        for name, adapter in adapters:
            assert hasattr(adapter, 'list_algorithms'), f"{name} adapter missing list_algorithms()"
            assert hasattr(adapter, 'create_detector'), f"{name} adapter missing create_detector()"
            
            # Methods should return expected types
            algorithms = adapter.list_algorithms()
            assert isinstance(algorithms, list), f"{name} adapter list_algorithms() should return list"
            ...
            if algorithms:  # If algorithms are available
                ...
                assert hasattr(detector, 'predict'), f"{name} detector missing predict()"
                assert hasattr(detector, 'decision_function'), f"{name} detector missing decision_function()"
    ...
    def test_cross_adapter_result_correlation(self):
        ...
        common_algorithm_map = {
            "sklearn": "local_outlier_factor",
            "pyod": "pyod_lof"
        }
            ...
            if name in common_algorithm_map:
                try:
                    ...
                    if algorithm in available_algorithms:
                        detector = adapter.create_detector(algorithm, contamination=0.1, random_state=42)
                        detector.fit(data)
                        scores = detector.decision_function(data)
                        results[name] = scores
                except Exception:
        ...
        if len(results) >= 2:
            ...
            assert correlation > 0.3, f"Adapter results should be correlated: {correlation:.3f}"
