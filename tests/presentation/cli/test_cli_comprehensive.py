...
from pynomaly.presentation.cli import autonomous, detectors, datasets, detection, server, preprocessing
...
from pynomaly.infrastructure.config import Config
...
class TestDetectorsCLI:
    ...
    def test_detector_list_with_algorithm_filter(self, runner, mock_container):
        ...
        result = runner.invoke(detectors.app, ["list", "--algorithm", "IsolationForest"])
        
        assert result.exit_code == 0

    def test_detector_create_success(self, runner, mock_container):
        """Test successful detector creation."""
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.PyODAdapter') as mock_adapter:
            mock_instance = Mock()
            mock_adapter.return_value = mock_instance
            
            result = runner.invoke(detectors.app, [
                "create",
                "--name", "New Detector",
    ...
    def test_detector_algorithms_list(self, runner):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.PyODAdapter') as mock_pyod:
            with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_sklearn:
                mock_pyod.ALGORITHM_MAPPING = {"IsolationForest": "IF", "LOF": "LocalOutlierFactor"}
    ...
    def test_detector_tune_hyperparameters(self, runner, mock_container):
        ...
        with patch('pynomaly.application.services.hyperparameter_tuning_service.HyperparameterTuningService') as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.tune_hyperparameters.return_value = {"n_estimators": 150}
...
class TestDatasetsCLI:
    ...
    def mock_container(self):
        ...
        with patch('pynomaly.presentation.cli.container.get_cli_container') as mock:
            ...
            sample_dataset.features = ["feature1", "feature2", "feature3", "feature4", "feature5"]
    ...
    def test_dataset_load_csv(self, runner, mock_container, sample_csv_file):
        ...
        result = runner.invoke(datasets.app, [
    ...
    def test_dataset_validate(self, runner, mock_container):
        ...
        with patch('pynomaly.application.services.data_validation_service.DataValidationService') as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
...
class TestDetectionCLI:
    ...
    def test_detection_train(self, runner, mock_container):
        ...
        with patch('pynomaly.application.use_cases.train_detector.TrainDetectorUseCase') as mock_use_case:
            mock_use_case_instance = Mock()
            mock_use_case.return_value = mock_use_case_instance
            
            result = runner.invoke(detection.app, [
                "train",
    ...
    def test_detection_run(self, runner, mock_container):
        ...
        with patch('pynomaly.application.use_cases.detect_anomalies.DetectAnomaliesUseCase') as mock_use_case:
            mock_use_case_instance = Mock()
            mock_use_case.return_value = mock_use_case_instance
    ...
    def test_detection_evaluate(self, runner, mock_container):
        ...
        with patch('pynomaly.application.use_cases.evaluate_detector.EvaluateDetectorUseCase') as mock_use_case:
            mock_use_case_instance = Mock()
            mock_use_case.return_value = mock_use_case_instance
    ...
    def test_detection_batch_multiple_detectors(self, runner, mock_container):
        ...
        with patch('pynomaly.application.use_cases.batch_detection.BatchDetectionUseCase') as mock_use_case:
            mock_use_case_instance = Mock()
            mock_use_case.return_value = mock_use_case_instance
...
class TestAutonomousCLI:
    ...
    def test_autonomous_detect(self, runner, sample_csv_file):
        ...
        with patch('pynomaly.application.services.autonomous_service.AutonomousDetectionService') as mock_service:
            ...
            try:
                result = runner.invoke(autonomous.app, [
                    "detect", sample_csv_file,
                    "--output", output_path
                ])
    ...
    def test_autonomous_profile(self, runner, sample_csv_file):
        ...
        with patch('pynomaly.application.services.autonomous_service.AutonomousDetectionService') as mock_service:
            ...
            mock_service_instance.profile_data.return_value = {
                "shape": (100, 3),
                "data_types": {"feature1": "numeric", "feature2": "numeric", "feature3": "numeric"},
                "missing_values": 0,
...
class TestPreprocessingCLI:
    ...
    def test_preprocessing_clean(self, runner, mock_container):
        """Test data cleaning."""
        with patch('pynomaly.application.services.preprocessing_service.PreprocessingService') as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            
            result = runner.invoke(preprocessing.app, [
                "clean", "Test Dataset",
                "--missing", "drop_rows",
                "--outliers", "clip",
                "--duplicates", "remove"
...
class TestCLIIntegration:
    ...
    def test_complete_detection_workflow(self, runner):
        ...
        with patch('pynomaly.presentation.cli.container.get_cli_container') as mock_container:
            container = Mock()
            ...
            try:
                # 1. Load dataset
                load_result = runner.invoke(datasets.app, [
                    "load", temp_path,
                    "--name", "workflow_data"
                ])
                
                # 2. Create detector
                create_result = runner.invoke(detectors.app, [
                    "create",
    ...
    def test_help_system_completeness(self, runner):
        ...
        subcommands = ["detector", "dataset", "detect", "auto", "data", "export", "server"]
