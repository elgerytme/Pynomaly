...
class TestAutonomousCLI:
    ...
    def sample_data_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'cpu_usage', 'memory_usage', 'network_io', 'disk_io'])
    ...
    def mock_autonomous_service(self):
        ...
        with patch('pynomaly.application.services.autonomous_service.AutonomousDetectionService') as mock:
            service = Mock()
            ...
            service.profile_data.return_value = {
                ...
                "recommended_algorithms": ["IsolationForest", "LOF", "EllipticEnvelope"],
                "seasonality_detected": False,
                "time_series_features": ["timestamp"],
                "numerical_features": ["cpu_usage", "memory_usage", "network_io", "disk_io"],
    ...
    def test_profile_save_report(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_quick_detect_basic(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_quick_detect_with_contamination(self, runner, mock_autonomous_service, sample_data_file):
        """Test quick detection with custom contamination rate."""
        result = runner.invoke(app, [
            "quick", sample_data_file,
            "--contamination", "0.1"
        ])
    ...
    def test_quick_detect_verbose(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_quick_detect_save_results(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_detect_with_custom_options(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_detect_verbose_output(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_detect_with_config_file(self, runner, mock_autonomous_service, sample_data_file):
    ...
    def test_detect_large_dataset_warning(self, runner, mock_autonomous_service):
        ...
        try:
            ...
            assert result.exit_code == 0
            assert "large dataset" in result.stdout.lower() or "1000000" in result.stdout
            
        finally:
            Path(large_data_path).unlink(missing_ok=True)

    def test_detect_performance_monitoring(self, runner, mock_autonomous_service, sample_data_file):
