...
from pynomaly.application.services.preprocessing_service import PreprocessingService
...
class TestPreprocessingCLI:
    ...
    def mock_container(self):
        ...
        with patch('pynomaly.presentation.cli.container.get_cli_container') as mock:
            ...
            mock_dataset.features = ["feature1", "feature2", "feature3", "feature4", "target"]
    ...
    def test_analyze_all(self, runner, mock_container):
        ...
        result = runner.invoke(app, [
            ...
            "--missing", "--outliers", "--correlations", "--distribution"
        ])
    ...
    def test_preprocessing_service_error(self, runner, mock_container):
        ...
        preprocessing_service = mock_container.preprocessing_service.return_value
        preprocessing_service.clean_missing_values.side_effect = Exception("Processing error")
    ...
    def test_complete_preprocessing_workflow(self, runner, mock_container):
        ...
        # 1. Analyze dataset
        analyze_result = runner.invoke(app, [
            "analyze", "Test Dataset", "--missing", "--outliers"
        ])
        
        # 2. Clean dataset
        clean_result = runner.invoke(app, [
            "clean", "Test Dataset",
            "--missing", "fill_mean",
            "--outliers", "clip"
        ])
        ...
        # 3. Transform dataset
        transform_result = runner.invoke(app, [
            "transform", "Test Dataset",
            "--scaling", "standard",
            "--encoding", "onehot"
        ])
    ...
    def test_file_preprocessing_workflow(self, runner, sample_dirty_data_file):
        ...
        with patch('pandas.read_csv') as mock_read_csv:
            ...
            with patch('pynomaly.application.services.preprocessing_service.PreprocessingService') as mock_service:
                ...
                try:
                    result = runner.invoke(app, [
                        "file", "clean", sample_dirty_data_file,
                        "--output", output_path,
                        "--missing", "fill_mean",
                        "--outliers", "clip"
                    ])
