...
from pynomaly.presentation.web.routes import (
    dashboard_routes,
    detector_routes,
    dataset_routes,
    detection_routes,
...
from pynomaly.presentation.web.htmx_endpoints import (
    detector_htmx,
    dataset_htmx,
    detection_htmx,
    visualization_htmx
...
class TestDashboardRoutes:
    ...
    def test_dashboard_stats_endpoint(self, test_client, mock_services):
        ...
        with patch.multiple(
            'pynomaly.presentation.web.routes.dashboard_routes',
            **mock_services
        ):
...
class TestDetectorRoutes:
    ...
    def test_detectors_list_page(self, test_client, mock_detector_service):
        ...
        with patch('pynomaly.presentation.web.routes.detector_routes.get_detector_service') as mock_get:
    ...
    def test_detector_detail_page(self, test_client, mock_detector_service, sample_detector):
    ...
    def test_detector_create_form_submission(self, test_client, mock_detector_service, sample_detector):
        ...
        with patch('pynomaly.presentation.web.routes.detector_routes.get_detector_service') as mock_get:
            ...
            response = test_client.post("/detectors/create", data={
                "name": "test_detector",
                "algorithm": "isolation_forest",
    ...
    def test_detector_edit_page(self, test_client, mock_detector_service, sample_detector):
    ...
    def test_detector_delete_action(self, test_client, mock_detector_service, sample_detector):
...
class TestDatasetRoutes:
    ...
    def test_datasets_list_page(self, test_client, mock_dataset_service):
        ...
        with patch('pynomaly.presentation.web.routes.dataset_routes.get_dataset_service') as mock_get:
    ...
    def test_dataset_detail_page(self, test_client, mock_dataset_service, sample_dataset):
    ...
    def test_dataset_upload_form_submission(self, test_client, mock_dataset_service, sample_dataset):
    ...
    def test_dataset_statistics_page(self, test_client, mock_dataset_service, sample_dataset):
...
class TestDetectionRoutes:
    ...
    def test_detection_results_page(self, test_client, mock_detection_service):
        ...
        with patch('pynomaly.presentation.web.routes.detection_routes.get_detection_service') as mock_get:
    ...
    def test_detection_result_detail(self, test_client, mock_detection_service, sample_detection_result):
        """Test detection result detail page."""
        mock_detection_service.get_detection_result.return_value = sample_detection_result
        
        with patch('pynomaly.presentation.web.routes.detection_routes.get_detection_service') as mock_get:
            mock_get.return_value = mock_detection_service
            
            response = test_client.get(f"/detection/results/{sample_detection_result.id}")
            
            assert response.status_code == 200
            assert "result" in response.text.lower()
    
    def test_detection_form_submission(self, test_client, mock_detection_service, sample_detection_result):
        ...
        with patch('pynomaly.presentation.web.routes.detection_routes.get_detection_service') as mock_get:
            ...
            response = test_client.post("/detection/run", data={
...
class TestHTMXEndpoints:
    ...
    def test_detection_visualization_htmx(self, test_client, htmx_headers):
        ...
        response = test_client.get("/htmx/detection/visualization/test_id", headers=htmx_headers)
...
class TestVisualizationComponents:
    ...
    def test_anomaly_scatter_plot(self, test_client, sample_detection_result):
        ...
        with patch('pynomaly.presentation.web.routes.visualization_routes.get_detection_service') as mock_get:
            mock_service = Mock()
            mock_service.get_detection_result.return_value = sample_detection_result
            mock_get.return_value = mock_service
            
            response = test_client.get(f"/visualizations/scatter/{sample_detection_result.id}")
            
            assert response.status_code == 200
            assert "scatter" in response.text.lower() or "visualization" in response.text.lower()
    ...
    def test_anomaly_histogram(self, test_client, sample_detection_result):
        ...
        with patch('pynomaly.presentation.web.routes.visualization_routes.get_detection_service') as mock_get:
            ...
            response = test_client.get(f"/visualizations/histogram/{sample_detection_result.id}")
            
            assert response.status_code == 200
            assert "histogram" in response.text.lower() or "chart" in response.text.lower()
...
class TestResponsiveDesign:
    ...
    def test_mobile_dashboard(self, test_client, mobile_headers):
        ...
        assert "responsive" in response.text.lower() or "mobile" in response.text.lower()
...
class TestAccessibility:
    ...
    def test_keyboard_navigation_support(self, test_client):
        ...
        assert "tabindex" in response.text or "focus" in response.text or "keyboard" in response.text
...
class TestPerformanceOptimization:
    ...
    def test_static_asset_compression(self, test_client):
        ...
        response = test_client.get("/static/css/main.css", headers={"Accept-Encoding": "gzip"})
