...
from pynomaly.application.dto.dataset_dto import (
    DatasetRequestDTO, DatasetResponseDTO, DatasetUploadResponseDTO
)
...
15.0,25.0,35.0,1"""
    ...
    def test_create_dataset_successful(self, client, mock_auth, auth_headers, sample_dataset_request):
        """Test successful dataset creation."""
        with patch('pynomaly.application.use_cases.dataset_use_case.CreateDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            ...
            response = client.post("/datasets", json=sample_dataset_request, headers=auth_headers)
    ...
    def test_get_dataset_by_id(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.GetDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = DatasetResponseDTO(
                id="dataset123",
                name="test_dataset",
    ...
    def test_list_datasets(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.ListDatasetsUseCase') as mock_use_case:
            mock_instance = Mock()
    ...
    def test_update_dataset(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.UpdateDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = DatasetResponseDTO(
                id="dataset123",
                name="test_dataset",
            ...
            response = client.put("/datasets/dataset123", json=update_data, headers=auth_headers)
    ...
    def test_delete_dataset(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.DeleteDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
    ...
    def test_upload_dataset_csv(self, client, mock_auth, auth_headers, sample_csv_data):
        ...
        files = {
            "file": ("test_data.csv", io.StringIO(sample_csv_data), "text/csv")
        }
        data = {
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.UploadDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            ...
            response = client.post("/datasets/upload", files=files, data=data, headers=auth_headers)
    ...
    def test_upload_dataset_invalid_format(self, client, mock_auth, auth_headers):
        ...
        files = {
            "file": ("test_data.txt", io.StringIO("invalid data"), "text/plain")
        }
    ...
    def test_upload_dataset_too_large(self, client, mock_auth, auth_headers):
        ...
        files = {
            "file": ("large_data.csv", io.StringIO(large_data), "text/csv")
        }
    ...
    def test_upload_status_check(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.GetUploadStatusUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = {
                "id": "upload123",
                "status": "completed",
            ...
            response = client.get("/datasets/upload/upload123/status", headers=auth_headers)
    ...
    def test_validate_dataset(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.ValidateDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = {
                "is_valid": True,
                "validation_results": {
            ...
            response = client.post("/datasets/dataset123/validate", headers=auth_headers)
    ...
    def test_dataset_preview(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.GetDatasetPreviewUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = {
    ...
    def test_dataset_statistics(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.GetDatasetStatisticsUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = {
                "row_count": 1000,
                "column_count": 4,
            ...
            response = client.get("/datasets/dataset123/statistics", headers=auth_headers)
    ...
    def test_export_dataset(self, client, mock_auth, auth_headers):
        ...
        response = client.post("/datasets/dataset123/export", json=export_params, headers=auth_headers)
    ...
    def test_download_dataset(self, client, mock_auth, auth_headers):
        ...
        with patch('pynomaly.application.use_cases.dataset_use_case.DownloadDatasetUseCase') as mock_use_case:
            mock_instance = Mock()
            mock_instance.execute.return_value = {
    ...
    def test_share_dataset(self, client, mock_auth, auth_headers):
        ...
        response = client.post("/datasets/dataset123/share", json=share_data, headers=auth_headers)
    ...
    def test_dataset_operations_with_processing_status(self, client, mock_auth, auth_headers):
...
class TestDatasetEndpointsIntegration:
    ...
    def test_complete_dataset_lifecycle(self, authenticated_client, sample_csv_data):
        ...
        upload_response = authenticated_client.post("/datasets/upload", files=files, data=data)
        assert upload_response.status_code == 202
        
        # 2. Check upload status (simulate polling)
        upload_id = upload_response.json()["id"]
        status_response = authenticated_client.get(f"/datasets/upload/{upload_id}/status")
        ...
        update_response = authenticated_client.put(f"/datasets/{dataset_id}", json=update_data)
        ...
        export_response = authenticated_client.post(f"/datasets/{dataset_id}/export", json=export_data)
    ...
    def test_dataset_permission_workflow(self, authenticated_client):
        ...
        permissions_response = authenticated_client.get(f"/datasets/{dataset_id}/permissions")
        ...
        share_response = authenticated_client.post(f"/datasets/{dataset_id}/share", json=share_data)
        assert share_response.status_code in [200, 404]
        
        # 3. Check updated permissions
        updated_permissions = authenticated_client.get(f"/datasets/{dataset_id}/permissions")
