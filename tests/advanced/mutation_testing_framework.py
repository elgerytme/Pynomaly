...
class UnaryOperatorMutation(MutationOperator):
    ...
    def apply(self, node: ast.UnaryOp) -> List[ast.AST]:
        ...
        if original_op == ast.Not:
            ...
            for new_op_class in self.mutations.get(original_op, []):
                mutant = ast.copy_location(
                    ast.UnaryOp(op=new_op_class(), operand=node.operand),
                    node
                )
                mutants.append(mutant)
...
class MutationGenerator:
    ...
    def generate_mutations(self, source_code: str, target_file: str) -> List[Tuple[str, str, str]]:
                    ...
                    for mutant in mutants:
                        ...
                        if mutated_tree:
                            try:
                                ...
                                location = f"{target_file}:{node.lineno}:{node.col_offset}"
                                original_snippet = ast.unparse(node) if hasattr(ast, 'unparse') else str(node)
                                mutated_snippet = ast.unparse(mutant) if hasattr(ast, 'unparse') else str(mutant)
                                ...
                                logger.debug(f"Failed to generate mutant {mutation_id}: {e}")
        
        return mutations
    
    def _apply_mutation(self, tree: ast.AST, original_node: ast.AST, mutant_node: ast.AST) -> Optional[ast.AST]:
...
class MutationTester:
    ...
    def run_mutation_testing(self, target_files: List[str] = None, max_mutations: int = None) -> MutationTestSummary:
        ...
        if max_mutations and len(all_mutations) > max_mutations:
            logger.info(f"Limiting to {max_mutations} mutations (from {len(all_mutations)})")
        ...
        if not self._run_baseline_tests():
            raise RuntimeError("Baseline tests failed - cannot proceed with mutation testing")
        ...
        killed_mutations = sum(1 for r in results if not r.test_passed and r.error is None)
        survived_mutations = sum(1 for r in results if r.test_passed and r.error is None)
    ...
    def _test_mutation(self, mutation: Dict) -> MutationResult:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Apply mutation
            target_file = temp_source_dir / Path(mutation['file']).relative_to(self.source_dir)
        ...
        return MutationResult(
            ...
            original_code=mutation['original_code'][:200] + "..." if len(mutation['original_code']) > 200 else mutation['original_code'],
            mutated_code=mutation['code'][:200] + "..." if len(mutation['code']) > 200 else mutation['code'],
...
def main():
    ...
    parser.add_argument("--source-dir", type=Path, default="src/pynomaly", help="Source directory to mutate")
    parser.add_argument("--test-dir", type=Path, default="tests", help="Test directory")
    parser.add_argument("--test-command", default="python -m pytest", help="Test command to run")
    parser.add_argument("--max-mutations", type=int, help="Maximum number of mutations to test")
