...
class PropertyTestGenerator:
    ...
    def __init__(self):
        if not HYPOTHESIS_AVAILABLE:
            raise ImportError(
                "Hypothesis is required for property-based testing. Install with: pip install hypothesis"
            )
    ...
    def _build_strategy_map(self) -> dict[type, Callable]:
        ...
        return {
            ...
            list: lambda element_type=int: st.lists(
                self._get_strategy(element_type), min_size=0, max_size=10
            ),
            dict: lambda: st.dictionaries(
                st.text(min_size=1, max_size=10), st.integers(), min_size=0, max_size=5
            ),
            tuple: lambda *element_types: st.tuples(
                *[self._get_strategy(t) for t in element_types]
            ),
            # NumPy types
        ...
        if type_hint in self.strategies:
            ...
            if origin is list and args:
                ...
                if len(non_none_types) == 1:
                    ...
                    # General Union
                    return st.one_of(
                        *[self._get_strategy(arg) for arg in non_none_types]
                    )
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module):
                if (
                    ...
                    and self._is_testable_function(obj)
                ):
    ...
    def _generate_domain_properties(
        ...
        if any(
            keyword in func_name for keyword in ["preprocess", "clean", "transform"]
        ):
...
class PropertyTester:
    ...
    def run_property_testing(
        ...
        for file_path in target_files:
            ...
            if functions:
                ...
                logger.info(
                    f"Generated {len(properties)} property tests for {file_path}"
                )
        ...
        logger.info(
            f"Results: {passed_properties} passed, {failed_properties} failed, {error_properties} errors"
        )
    ...
    def _test_property(self, property_def: dict) -> PropertyTestResult:
        ...
        try:
            ...
            for param_name, param in sig.parameters.items():
                if param.annotation != param.empty:
                    strategies[param_name] = self.generator._get_strategy(
                        param.annotation
                    )
    ...
    def _test_determinism_property(self, func: Callable, strategies: dict) -> dict:
        ...
        @given(**strategies)
        @settings(
            max_examples=self.max_examples // 2,
            deadline=self.timeout * 1000,
            suppress_health_check=[HealthCheck.return_value],
        )
        def test_function(**kwargs):
            ...
            try:
                ...
                elif isinstance(result1, pd.DataFrame) and isinstance(
        ...
        def test_function(**kwargs):
            ...
            try:
                ...
                if isinstance(result, (int, float, np.number)):
                    return (
                        0 <= result <= 1 or -1 <= result <= 1
                    )  # Accept common score ranges
                elif isinstance(result, np.ndarray):
                    return np.all((0 <= result) & (result <= 1)) or np.all(
                        (-1 <= result) & (result <= 1)
                    )
                else:
        ...
        def test_function(**kwargs):
            ...
            try:
                if len(kwargs) == 1:
                    ...
                    if isinstance(input_val, np.ndarray) and isinstance(
                        result, np.ndarray
                    ):
                        # Check if shapes are compatible (same or broadcast-compatible)
                        return input_val.shape == result.shape or len(
                            result.shape
                        ) <= len(input_val.shape)
                    elif isinstance(input_val, pd.DataFrame) and isinstance(
                        result, pd.DataFrame
                    ):
                        return (
                            input_val.shape[0] == result.shape[0]
                        )  # Same number of rows
    ...
    def _test_symmetry_property(self, func: Callable, strategies: dict) -> dict:
        ...
        def test_function(arg1, arg2):
            ...
            try:
                ...
                if isinstance(result1, (int, float, np.number)) and isinstance(
                    result2, (int, float, np.number)
                ):
                    return (
    ...
    def _analyze_coverage(self, results: list[PropertyTestResult]) -> dict:
        ...
        coverage = {
            ...
            "total_examples": sum(r.examples_tested for r in results),
            "average_examples_per_property": (
                sum(r.examples_tested for r in results) / len(results) if results else 0
            ),
            "coverage_by_type": {},
        }
    ...
    def print_summary(self, summary: PropertyTestSummary):
        ...
        if summary.coverage_analysis:
            ...
            print(
                f"Avg examples per property: {summary.coverage_analysis['average_examples_per_property']:.1f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--timeout", type=int, default=60, help="Timeout per property test (seconds)"
    )
    ...
    try:
        ...
        if summary.failed_properties > 0 or summary.error_properties > 0:
            logger.warning(
                f"Property testing found {summary.failed_properties} failures and {summary.error_properties} errors"
            )
            sys.exit(1)
        else:
