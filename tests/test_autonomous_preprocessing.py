...
def sample_dataset_with_issues():
    ...
    data = {
        ...
        'feature_outliers': np.concatenate([
            ...
            np.array([100, -100, 200, -200, 300, -300, 400, -400, 500, -500])  # 10 extreme outliers
        ]),
...
class TestAutonomousQualityAnalyzer:
    ...
    def test_outlier_detection(self):
        ...
        data = pd.DataFrame({
            'col1': np.concatenate([normal_data, outliers])
        })
...
class TestAutonomousPreprocessingOrchestrator:
    ...
    def test_should_preprocess_clean_data(self, clean_dataset):
        ...
        should_preprocess, quality_report = orchestrator.should_preprocess(clean_dataset, 0.8)
    ...
    def test_should_preprocess_problematic_data(self, sample_dataset_with_issues):
        ...
        should_preprocess, quality_report = orchestrator.should_preprocess(sample_dataset_with_issues, 0.8)
        
        assert should_preprocess
        assert quality_report.overall_score < 0.8 or quality_report.preprocessing_required
    ...
    def test_preprocess_for_autonomous_detection(self, sample_dataset_with_issues):
        ...
        assert processed_dataset.data.shape[0] <= sample_dataset_with_issues.data.shape[0]  # May remove rows
        
        # Check that some issues were fixed
        new_quality_report = orchestrator.quality_analyzer.analyze_data_quality(processed_dataset)
    ...
    def test_preprocessing_error_handling(self, sample_dataset_with_issues):
        ...
        corrupted_data.loc[0, 'feature_normal'] = "not_a_number"  # String in numeric column
        ...
        should_preprocess, quality_report = orchestrator.should_preprocess(corrupted_dataset, 0.8)
...
class TestAutonomousServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_assess_and_preprocess_data_disabled(self, mock_autonomous_service, clean_dataset):
        """Test preprocessing disabled configuration."""
        config = AutonomousConfig(enable_preprocessing=False)
        
        processed_dataset, profile = await mock_autonomous_service._assess_and_preprocess_data(
    ...
    @pytest.mark.asyncio
    async def test_assess_and_preprocess_data_enabled(self, mock_autonomous_service, sample_dataset_with_issues):
    ...
    @pytest.mark.asyncio
    async def test_profile_data_with_preprocessing_info(self, mock_autonomous_service, clean_dataset):
