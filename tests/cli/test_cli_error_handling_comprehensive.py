...
class TestCLIErrorHandling:
    ...
    def test_network_errors(self, runner):
        ...
        with patch('pynomaly.presentation.cli.server.get_cli_container') as mock_get_container:
            container = Mock()
            config = Mock()
            ...
            with patch('requests.get', side_effect=Exception("Network error")):
                ...
                assert result.exit_code == 0
                assert "Cannot connect to API" in result.stdout or "Network error" in result.stdout
    ...
    def test_timeout_handling(self, runner):
        ...
        with patch('pynomaly.presentation.cli.server.get_cli_container') as mock_get_container:
            ...
            with patch('requests.get', side_effect=requests.exceptions.Timeout("Request timeout")):
                ...
                assert result.exit_code == 1
                assert "Timeout" in result.stdout or "Some services are not available" in result.stdout

    def test_database_connection_errors(self, runner, mock_container):
        """Test handling of database connection errors."""
        # Mock repository that fails
        mock_container.detector_repository.side_effect = Exception("Database connection failed")
    ...
    def test_service_unavailable_errors(self, runner):
        ...
        with patch('pynomaly.presentation.cli.app.get_cli_container', side_effect=Exception("Service unavailable")):
            result = runner.invoke(app, ["version"])
            assert result.exit_code != 0

    def test_memory_errors(self, runner):
        """Test handling of memory-related errors."""
        with patch('pynomaly.presentation.cli.app.get_cli_container', side_effect=MemoryError("Out of memory")):
            result = runner.invoke(app, ["version"])
            assert result.exit_code != 0

    def test_keyboard_interrupt_handling(self, runner):
        """Test handling of keyboard interrupts."""
        with patch('pynomaly.presentation.cli.app.get_cli_container', side_effect=KeyboardInterrupt()):
            result = runner.invoke(app, ["version"])
            assert result.exit_code != 0
...
class TestCLIEdgeCases:
    ...
    def test_float_precision(self, runner):
        ...
        result = runner.invoke(app, [
            "generate-config", "test",
            "--contamination", "0.123456789012345"
        ])
        assert result.exit_code == 0
...
class TestCLIRecovery:
    ...
    def test_graceful_degradation(self, runner):
        ...
        with patch('pynomaly.presentation.cli.app.get_cli_container') as mock_get_container:
            ...
            container.config.return_value = config
            container.optional_feature.side_effect = ImportError("Feature not available")
    ...
    def test_partial_failure_handling(self, runner):
        ...
        with patch('pynomaly.presentation.cli.app.get_cli_container') as mock_get_container:
            ...
            container.dataset_repository.return_value.count.side_effect = Exception("Partial failure")
...
class TestCLISecurityEdgeCases:
    ...
    def test_buffer_overflow_prevention(self, runner):
        ...
        result = runner.invoke(app, [
            "generate-config", "test",
            "--description", extremely_long_input
        ])
