...
class TestServerCommands:
    ...
    def test_start_server_success(self, mock_subprocess, mock_socket, runner, mock_container):
    ...
    @patch('socket.socket')
    @patch('subprocess.run')
    def test_start_server_with_options(self, mock_subprocess, mock_socket, runner, mock_container):
    ...
    @patch('socket.socket')
    @patch('subprocess.run')
    def test_start_server_keyboard_interrupt(self, mock_subprocess, mock_socket, runner, mock_container):
    ...
    @patch('socket.socket')
    @patch('subprocess.run')
    def test_start_server_subprocess_error(self, mock_subprocess, mock_socket, runner, mock_container):
    ...
    @patch('subprocess.run')
    @patch('os.fork')
    def test_start_server_daemon_mode(self, mock_fork, mock_subprocess, mock_socket, runner, mock_container):
    ...
    @patch('socket.socket')
    @patch('os.fork')
    def test_start_server_daemon_fork_error(self, mock_fork, mock_socket, runner, mock_container):
    ...
    @patch('subprocess.run')
    def test_logs_follow_keyboard_interrupt(self, mock_subprocess, runner, mock_container):
...
class TestServerCommandsEdgeCases:
    ...
    def test_status_process_check_exception(self, mock_kill, runner, mock_container):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert result.exit_code == 0
            assert "Error reading PID file" in result.stdout or "Cannot connect to API" in result.stdout