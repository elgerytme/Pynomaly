...
from pynomaly.infrastructure.adapters.enhanced_sklearn_adapter import EnhancedSklearnAdapter  
from pynomaly.infrastructure.adapters.ensemble_meta_adapter import EnsembleMetaAdapter, AggregationMethod
from pynomaly.infrastructure.adapters.algorithm_factory import AlgorithmFactory, DatasetCharacteristics, AlgorithmLibrary
...
class TestEnhancedPyODAdapter:
    ...
    def test_detect_method(self, mock_iforest, sample_dataset):
        ...
        # Mock the PyOD model
        mock_model = Mock()
        mock_model.predict.return_value = np.array([0, 0, 1, 0, 1])  # Sample predictions
        mock_model.decision_function.return_value = np.array([0.1, 0.2, 0.8, 0.3, 0.9])  # Sample scores
        mock_iforest.return_value = mock_model
...
class TestEnhancedSklearnAdapter:
    ...
    def test_detect_with_scaling(self, mock_iforest, sample_dataset):
        ...
        mock_model.decision_function.return_value = np.array([0.1, 0.2, -0.8, 0.3, -0.9])
        mock_iforest.return_value = mock_model
...
class TestAlgorithmFactory:
    ...
    @patch('pynomaly.infrastructure.adapters.enhanced_sklearn_adapter.EnhancedSklearnAdapter')
    ...
    def test_end_to_end_detection(self, real_dataset):
        ...
        if best_rec.library != AlgorithmLibrary.ENSEMBLE:
            try:
                ...
                assert 0 < n_anomalies < len(real_dataset.data) * 0.2  # Reasonable range
