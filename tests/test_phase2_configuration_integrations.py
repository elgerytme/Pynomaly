...
from pynomaly.application.dto.configuration_dto import (
    ConfigurationSource, PerformanceResultsDTO
)
...
from pynomaly.application.services.automl_configuration_integration import (
    AutoMLConfigurationIntegration, AutoMLConfigurationManager
)
from pynomaly.application.services.autonomous_configuration_integration import (
    AutonomousConfigurationIntegration, AutonomousConfigurationManager
)
...
from pynomaly.infrastructure.monitoring.cli_parameter_interceptor import (
    CLIParameterInterceptor, initialize_cli_interceptor, capture_detection_command
)
...
class TestAutoMLConfigurationIntegration:
    ...
    async def test_optimize_with_configuration_capture_success(
        ...
        integration_service.automl_service.optimize.return_value = (mock_detector, optimization_report)
        
        # Run optimization with capture
        detector, report, config_id = await integration_service.optimize_with_configuration_capture(
    ...
    async def test_optimize_with_configuration_capture_failure(
        ...
        integration_service.automl_service.optimize.side_effect = RuntimeError("Optimization failed")
...
class TestAutonomousConfigurationIntegration:
    ...
    def sample_detection_result(self):
        ...
        scores = [AnomalyScore(0.1), AnomalyScore(0.8), AnomalyScore(0.2), AnomalyScore(0.9)]
    ...
    async def test_detect_with_configuration_capture_success(
        ...
        integration_service.autonomous_service.detect_anomalies.return_value = sample_detection_result
        ...
        result, config_id = await integration_service.detect_with_configuration_capture(
            ...
            config=config,
            capture_metadata={"test_run": True}
        )
    ...
    async def test_analyze_autonomous_configurations(
        ...
        analysis = await integration_service.analyze_autonomous_configurations(days_back=1)
...
class TestCLIParameterInterceptor:
    ...
    async def test_capture_cli_command_decorator_async(self, interceptor):
        ...
        async def async_test_function(dataset_path: str, algorithm: str = "isolation_forest"):
            """Async test function."""
        ...
        stats = interceptor.get_capture_statistics()
        assert stats["capture_stats"]["total_commands"] == 1
        assert stats["capture_stats"]["captured_commands"] == 1  # Should capture due to execution time
    ...
    async def test_capture_cli_command_with_exception(self, interceptor):
        ...
        stats = interceptor.get_capture_statistics()
        assert stats["capture_stats"]["total_commands"] == 1
        assert stats["capture_stats"]["captured_commands"] == 1  # Should capture failed command
...
class TestConfigurationManagers:
    ...
    async def test_automl_configuration_manager(self, capture_service):
        ...
        assert all(config["algorithm_config"]["algorithm_name"] == "isolation_forest" for config in configs)
