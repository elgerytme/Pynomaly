...
class TestSDKStreamingDetection:
        ...
        class MockStreamingSDK:
            ...
            def create_streaming_detector(self, algorithm: str, **parameters) -> str:
                ...
                try:
                    ...
                    self.data_buffers[detector_id] = deque(maxlen=self.models[detector_id]["buffer_size"])
            ...
            def train_streaming_detector(self, detector_id: str, initial_data: pd.DataFrame) -> Dict[str, Any]:
                ...
                try:
                    ...
                    return {
                        ...
                        "initial_threshold": self.adaptive_thresholds[detector_id]["current_threshold"],
            ...
            def process_streaming_sample(self, detector_id: str, sample: Dict[str, float]) -> Dict[str, Any]:
            ...
            def process_streaming_batch(self, detector_id: str, batch: List[Dict[str, float]]) -> List[Dict[str, Any]]:
            ...
            def _update_adaptive_threshold(self, detector_id: str, new_score: float):
                ...
                if len(recent_scores) >= 10:  # Need minimum samples
                    ...
                    updated_threshold = (1 - adaptation_rate) * current_threshold + adaptation_rate * new_threshold
                    
                    # Apply bounds
                    updated_threshold = max(threshold_info["min_threshold"], 
            ...
            def get_streaming_stats(self, detector_id: str) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "anomaly_rate": stats["anomalies_detected"] / max(1, stats["samples_processed"]),
                    ...
                    "samples_per_second": 1.0 / np.mean(processing_times) if processing_times else 0,
                    "last_update": stats["last_update"].isoformat() if stats["last_update"] else None
                }
            
            def create_streaming_pipeline(self, detector_id: str, data_source: Iterator[Dict[str, float]]) -> str:
            ...
            def get_streaming_results(self, pipeline_id: str, limit: int = None) -> List[Dict[str, Any]]:
    ...
    def test_incremental_model_updates(self, mock_streaming_sdk):
        ...
        try:
            ...
            # Initial training
            training_data = pd.DataFrame({
                'sensor': np.random.normal(0, 1, 200)
            })
            ...
            for expected, actual in zip(expected_updates, samples_with_updates):
