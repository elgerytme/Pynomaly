...
from pynomaly.infrastructure.preprocessing.data_cleaner import MissingValueStrategy, OutlierStrategy
from pynomaly.infrastructure.preprocessing.preprocessing_pipeline import PreprocessingPipeline
...
class TestDataCleaningCLI:
    ...
    def test_clean_missing_values_dry_run(self, mock_get_container, runner, mock_container):
        """Test cleaning missing values in dry run mode."""
        mock_get_container.return_value = mock_container
        
        result = runner.invoke(app, [
            "clean", "test-dataset-123",
            "--missing", "drop_rows",
            "--dry-run"
        ])
...
class TestDataTransformationCLI:
    ...
    @patch('pynomaly.presentation.cli.preprocessing.get_cli_container')
    def test_transform_feature_selection(self, mock_get_container, runner, mock_container):
    ...
    @patch('pynomaly.presentation.cli.preprocessing.get_cli_container')
    def test_transform_polynomial_features(self, mock_get_container, runner, mock_container):
...
class TestPreprocessingWorkflow:
    ...
    @patch('pynomaly.presentation.cli.preprocessing.get_cli_container')
    def test_complete_preprocessing_workflow(self, mock_get_container, runner, mock_container):
        ...
        assert "cleaning" in result1.output.lower() or "missing" in result1.output.lower()
        assert "transform" in result2.output.lower() or "scaling" in result2.output.lower()
