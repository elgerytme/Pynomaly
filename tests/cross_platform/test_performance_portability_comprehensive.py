    ...
    def test_multi_core_performance_scaling(self):
        ...
        for thread_count in thread_counts:
            if thread_count <= cpu_count:
                ...
                assert (
                    abs(parallel_result - sequential_result) / sequential_result < 0.01
                )
...
class TestMemoryPerformancePortability:
    ...
    def test_memory_pressure_handling(self):
        ...
        try:
            ...
            safe_memory_usage = min(
                available_memory // 4, 500 * 1024 * 1024
            )  # 500MB max
    ...
    def test_threading_performance_characteristics(self):
        ...
        with ThreadPoolExecutor(max_workers=num_tasks) as executor:
            threaded_results = list(
                executor.map(io_bound_task, [task_duration] * num_tasks)
            )
    ...
    def test_multiprocessing_performance_characteristics(self):
        ...
        with ProcessPoolExecutor(max_workers=num_tasks) as executor:
            parallel_results = list(
                executor.map(cpu_bound_task, [task_size] * num_tasks)
            )
        parallel_time = time.time() - start_time
    ...
    def test_concurrent_user_scalability(self):
        ...
        for user_count in user_counts:
            ...
            with ThreadPoolExecutor(max_workers=user_count) as executor:
                futures = [
                    executor.submit(simulate_user_operation) for _ in range(user_count)
                ]
