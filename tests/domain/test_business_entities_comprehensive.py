...
from pynomaly.domain.entities.tenant import Tenant
from pynomaly.domain.value_objects import (
    AnomalyScore, ContaminationRate, PerformanceMetrics
)
from pynomaly.domain.exceptions import ValidationError, BusinessRuleViolation
...
class TestExperiment:
    ...
    def test_create_experiment(self, sample_experiment):
        ...
        assert sample_experiment.hypothesis == "IsolationForest will show better precision on high-dimensional data"
    ...
    def test_experiment_analysis(self, sample_experiment):
        ...
        runs_data = [
            {"algorithm": "IsolationForest", "contamination": 0.05, "precision": 0.95, "recall": 0.82},
            {"algorithm": "IsolationForest", "contamination": 0.10, "precision": 0.92, "recall": 0.87},
            {"algorithm": "IsolationForest", "contamination": 0.15, "precision": 0.88, "recall": 0.91},
            {"algorithm": "LocalOutlierFactor", "n_neighbors": 10, "precision": 0.89, "recall": 0.85},
            {"algorithm": "LocalOutlierFactor", "n_neighbors": 20, "precision": 0.91, "recall": 0.88},
            {"algorithm": "LocalOutlierFactor", "n_neighbors": 30, "precision": 0.87, "recall": 0.92}
        ]
        
        for run_data in runs_data:
            sample_experiment.add_run({
                "algorithm": run_data["algorithm"],
                "parameters": {k: v for k, v in run_data.items() if k not in ["algorithm", "precision", "recall"]},
                "results": {"precision": run_data["precision"], "recall": run_data["recall"]}
            })
...
class TestPipeline:
    ...
    def test_pipeline_execution(self, sample_pipeline):
        ...
        steps = [
            ...
            {"name": "step2", "type": "processor", "configuration": {}, "depends_on": ["step1"]},
            {"name": "step3", "type": "detector", "configuration": {}, "depends_on": ["step2"]}
        ]
        sample_pipeline.configure_steps(steps)
        ...
        sample_pipeline.update_step_status(run_id, "step1", "completed", {"records_processed": 10000})
        sample_pipeline.update_step_status(run_id, "step2", "completed", {"features_selected": 25})
        sample_pipeline.update_step_status(run_id, "step3", "completed", {"anomalies_detected": 150})
        
        # Mark run as completed
        sample_pipeline.complete_run(run_id, {"total_anomalies": 150, "execution_time_minutes": 15})
    ...
    def test_pipeline_monitoring(self, sample_pipeline):
        ...
        metrics_data = [
            {"run_id": "run_1", "execution_time": 900, "success": True, "data_quality_score": 0.97},
            {"run_id": "run_2", "execution_time": 950, "success": True, "data_quality_score": 0.96},
            {"run_id": "run_3", "execution_time": 1200, "success": False, "error": "data_validation_failed"},
            {"run_id": "run_4", "execution_time": 1250, "success": False, "error": "connection_timeout"}
        ]
    ...
    def test_pipeline_versioning(self, sample_pipeline):
        ...
        initial_version = sample_pipeline.create_version("v1.0.0", "Initial pipeline version")
        ...
        # Create new version
        updated_version = sample_pipeline.create_version("v1.1.0", "Added enhanced processing")
        ...
        assert current_config != sample_pipeline.get_configuration()  # Should be different from v1.1.0
...
class TestAlert:
    ...
    def test_alert_evaluation(self, sample_alert):
        ...
        sample_alert.add_condition("high_rate", {
            "metric": "anomaly_rate",
            "operator": ">",
            "threshold": 0.1,
            "evaluation_window": "5m"
        ...
        escalation_policy = {
            "levels": [
                ...
                },
                {
                    "level": 2,
                    "delay_minutes": 15,
                    "recipients": ["secondary-oncall@company.com", "manager@company.com"],
...
class TestGovernance:
    ...
    def test_governance_policies(self, governance_framework):
        ...
        assert governance_framework.get_policy("Model Documentation Policy")["category"] == "documentation"
    ...
    def test_governance_workflow(self, governance_framework):
        ...
        updated_instance = governance_framework.get_workflow_instance(workflow_instance["id"])
...
class TestCostOptimization:
    ...
    def test_optimization_implementation(self, cost_optimizer):
        ...
        cost_optimizer.complete_implementation(
            implementation["id"],
            {"actual_savings": 0.32, "implementation_cost": 5000}
        )
    ...
    def test_cost_monitoring(self, cost_optimizer):
        ...
        # Set baseline costs
        baseline_costs = {"total": 10000, "compute": 7000, "storage": 2000, "other": 1000}
