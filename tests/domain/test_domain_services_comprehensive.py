...
from pynomaly.domain.entities import Dataset, Anomaly, DetectionResult
from pynomaly.domain.exceptions import ValidationError, InvalidValueError
...
class TestThresholdCalculator:
    ...
    def sample_scores(self):
        ...
        return [
            AnomalyScore(0.1), AnomalyScore(0.2), AnomalyScore(0.3),
            AnomalyScore(0.4), AnomalyScore(0.5), AnomalyScore(0.6),
            AnomalyScore(0.7), AnomalyScore(0.8), AnomalyScore(0.9),
            AnomalyScore(0.95)
        ]
    ...
    def test_calculate_contamination_threshold(self, threshold_calculator, sample_scores):
        ...
        expected_threshold = sorted([s.value for s in sample_scores], reverse=True)[0]  # 90th percentile
    ...
    def test_calculate_threshold_with_confidence(self, threshold_calculator, sample_scores):
...
class TestFeatureValidator:
    ...
    def test_validate_feature_correlation(self, feature_validator, sample_dataset):
        ...
        sample_dataset.data['correlated_feature'] = sample_dataset.data['numeric_feature'] * 2
    ...
    def test_feature_validation_with_remediation(self, feature_validator, sample_dataset):
        ...
        constant_suggestions = [
            s for s in validation_result["remediation_suggestions"]
...
class TestEnsembleAggregator:
    ...
    def test_stacking_aggregation(self, ensemble_aggregator, sample_predictions):
        ...
        training_predictions = {
            "detector_1": [AnomalyScore(0.6), AnomalyScore(0.4), AnomalyScore(0.8), AnomalyScore(0.3)],
            "detector_2": [AnomalyScore(0.5), AnomalyScore(0.5), AnomalyScore(0.7), AnomalyScore(0.2)],
            "detector_3": [AnomalyScore(0.7), AnomalyScore(0.3), AnomalyScore(0.9), AnomalyScore(0.4)]
        }
...
class TestAnomalyScorer:
    ...
    def test_contextual_scoring(self, anomaly_scorer, sample_data):
        ...
        sample_data['timestamp'] = pd.date_range('2023-01-01', periods=len(sample_data), freq='1H')
...
class TestExplainabilityService:
    ...
    def test_feature_importance_explanation(self, explainability_service, sample_anomaly_data):
    ...
    def test_counterfactual_explanation(self, explainability_service, sample_anomaly_data):
    ...
    def test_explanation_quality_assessment(self, explainability_service, sample_anomaly_data):
