...
from pynomaly.domain.exceptions.entity_exceptions import (
    EntityNotFoundError, EntityValidationError, EntityStateError,
    EntityConstraintViolationError
)
from pynomaly.domain.exceptions.detector_exceptions import (
    DetectorNotTrainedError, DetectorConfigurationError, 
    DetectorValidationError, DetectorStateError
)
from pynomaly.domain.exceptions.dataset_exceptions import (
    DatasetValidationError, DatasetFormatError, DatasetSizeError,
    DatasetSchemaError, DatasetCorruptionError
)
from pynomaly.domain.exceptions.result_exceptions import (
    ResultValidationError, ResultProcessingError, ResultStorageError,
...
class TestEntityExceptions:
    ...
    def test_entity_validation_error(self):
        ...
        validation_errors = [
            ...
            {"field": "parameters", "error": "missing_required_parameter_contamination"}
        ]
        ...
        assert entity_validation_error.validation_errors[0]["error"] == "cannot_be_empty"
    ...
    def test_entity_constraint_violation_error(self):
        ...
        assert constraint_error.violated_constraints[0]["constraint"] == "unique_name_per_user"
...
class TestDetectorExceptions:
    ...
    def test_detector_configuration_error(self):
        ...
        config_errors = [
            {"parameter": "contamination", "error": "value_out_of_range", "valid_range": [0.0, 1.0]},
            {"parameter": "n_estimators", "error": "must_be_positive_integer", "minimum": 1},
            {"parameter": "unknown_param", "error": "unknown_parameter", "valid_params": ["contamination", "n_estimators"]}
        ]
    ...
    def test_detector_validation_error(self):
        ...
        assert detector_validation_error.validation_issues[0]["category"] == "algorithm_compatibility"
    ...
    def test_detector_state_error(self):
        ...
        assert detector_state_error.state_transition_error == "cannot_delete_while_training"
    ...
    def test_dataset_validation_error(self):
        ...
        dataset_validation_error = DatasetValidationError(
            ...
            context={
                "total_rows": 1000,
                "total_columns": 5,
                "validation_rules": ["no_null_values", "no_infinite_values", "min_variance"]
            }
        )
...
class TestResultExceptions:
    ...
    def test_result_validation_error(self):
        ...
        assert result_validation_error.validation_failures[0]["field"] == "anomaly_scores"
...
class TestExceptionHandling:
    ...
    def test_exception_context_propagation(self):
        ...
        wrapped_exception = DetectorConfigurationError(
            ...
            configuration_errors=[{"parameter": "contamination", "error": "out_of_range"}],
    ...
    def test_exception_aggregation(self):
        ...
        assert all(isinstance(exc, ValidationError) for exc in aggregated_exception.underlying_exceptions)
