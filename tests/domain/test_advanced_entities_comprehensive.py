...
from pynomaly.domain.value_objects import (
    SemanticVersion, ModelStorageInfo, PerformanceMetrics,
...
class TestModelRegistry:
    ...
    def test_model_promotion(self, model_registry, sample_model_versions):
        ...
        with pytest.raises(ValidationError):
            model_registry.promote_version(version.id, "staging", "production")  # Already in production
...
class TestDriftDetection:
    ...
    def drift_detection(self):
        ...
        return DriftDetection(
            ...
            model_id=uuid4(),
            detection_methods=["ks_test", "jensen_shannon", "population_stability_index"],
...
class TestExplainableAI:
    ...
    def test_explanation_bias_detection(self, explainable_ai):
        ...
        biased_data = pd.DataFrame({
            'feature1': np.random.normal(0, 1, 1000),
            'feature2': np.random.normal(0, 1, 1000),
            'protected_attr': np.random.choice([0, 1], 1000)  # Binary protected attribute
        })
        ...
        for i in range(100):
            if biased_data.iloc[i]['protected_attr'] == 0:
                explanation = explainable_ai.generate_local_explanation(
                    instance=biased_data.iloc[i].to_dict(),
                    method="shap"
                )
...
class TestSecurityCompliance:
    ...
    def test_audit_trail_management(self, security_compliance):
        ...
        # Log various events
        events = [
            {"event_type": "model_training", "user": "data_scientist_1", "timestamp": datetime.now()},
            {"event_type": "model_deployment", "user": "ml_engineer_1", "timestamp": datetime.now()},
            {"event_type": "data_access", "user": "analyst_1", "timestamp": datetime.now()},
            {"event_type": "prediction_request", "user": "api_client", "timestamp": datetime.now()}
        ]
    ...
    def test_privacy_compliance(self, security_compliance):
        ...
        deletion_response = security_compliance.handle_data_subject_request(deletion_request)
