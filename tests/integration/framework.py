...
class IntegrationTestRunner:
    ...
    async def run_suite(
        ...
        try:
            ...
            suite.passed_steps = len(
                [s for s in suite.steps if s.result == TestResult.PASSED]
            )
            suite.failed_steps = len(
                [s for s in suite.steps if s.result == TestResult.FAILED]
            )
            suite.skipped_steps = len(
                [s for s in suite.steps if s.result == TestResult.SKIPPED]
            )
            suite.error_steps = len(
                [s for s in suite.steps if s.result == TestResult.ERROR]
            )
            suite.total_execution_time = sum(
                step.execution_time for step in suite.steps
            )
    ...
    async def _run_steps_sequential(self, suite: IntegrationTestSuite, fail_fast: bool):
    ...
    async def _run_steps_parallel(self, suite: IntegrationTestSuite, fail_fast: bool):
            ...
            for attempt in range(step.retry_count + 1):
                try:
                    step.actual_result = await self._execute_action(
                        step.action, step.name, step.timeout_seconds
                    )
            ...
            if step.cleanup_action:
                try:
                    await self._execute_action(
                        step.cleanup_action, f"{step.name}_cleanup"
                    )
                except Exception as e:
                    self.logger.warning(f"Cleanup failed for step {step.name}: {e}")

    async def _execute_action(
        self, action: Callable, action_name: str, timeout_seconds: int = 30
    ) -> Any:
        ...
        if asyncio.iscoroutinefunction(action):
            ...
            return await asyncio.wait_for(
                loop.run_in_executor(None, action), timeout=timeout_seconds
            )
    ...
    def generate_report(self, output_file: str = "integration_test_report.html") -> str:
        ...
        for suite_name, suite in self.test_results.items():
            success_rate = (
                (suite.passed_steps / suite.total_steps * 100)
                if suite.total_steps > 0
                else 0
            )
            ...
            for step in suite.steps:
                ...
                if step.actual_result is not None:
                    html_content += (
                        f"<div>Result: <pre>{step.actual_result}</pre></div>"
                    )
        ...
        async def wrapper(*args, **kwargs):
            ...
            if result_suite.failed_steps > 0 or result_suite.error_steps > 0:
                ...
                report_path = runner.generate_report(
                    f"failed_integration_test_{suite.name}.html"
                )
                pytest.fail(
