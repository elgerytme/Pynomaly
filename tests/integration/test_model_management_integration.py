...
class TestModelRegistryIntegration:
    ...
    async def test_model_registry_lifecycle(self, sample_model, temp_model_dir):
        ...
        # Get model
        retrieved_model = await registry.get_model("test_model_v1", SemanticVersion("1.0.0"))
        ...
        # Verify update
        updated_metadata = await registry.get_model_metadata("test_model_v1", SemanticVersion("1.0.0"))
    ...
    async def test_model_versioning_integration(self, sample_model, temp_model_dir):
        ...
        updated_model = Model(
            name="versioned_model",
            algorithm="isolation_forest",
            parameters={"contamination": 0.05, "n_estimators": 100},  # Updated parameter
    ...
    async def test_model_deployment_integration(self, sample_model, sample_detector, temp_model_dir):
...
class TestModelMonitoringIntegration:
    ...
    async def test_model_monitoring_lifecycle(self, sample_model, sample_dataset, temp_model_dir):
    ...
    async def test_drift_detection_integration(self, sample_model, sample_dataset, temp_model_dir):
        ...
        feature_drift_result = await monitoring.detect_drift(monitor_id, feature_drift_dataset)
        ...
        concept_drift_data['feature_1'] = concept_drift_data['feature_1'] + 0.5 * concept_drift_data['feature_2']
        ...
        concept_drift_result = await monitoring.detect_drift(monitor_id, concept_drift_dataset)
        assert isinstance(concept_drift_result, dict)
        
        # 3. No drift (similar data)
        no_drift_data = sample_dataset.data + np.random.normal(0, 0.01, sample_dataset.data.shape)  # Minimal noise
        no_drift_data['timestamp'] = pd.date_range(start='2023-03-01', periods=100, freq='H')
...
class TestAutoMLPipelineIntegration:
    ...
    async def test_automl_pipeline_lifecycle(self, sample_dataset, temp_model_dir):
        ...
        pipeline_id = await automl.run_pipeline(
            sample_dataset,
            config,
            experiment_name="test_automl_experiment"
        )
        ...
        experiment_found = any(exp["name"] == "test_automl_experiment" for exp in experiments)
        assert experiment_found
        
        # Get specific experiment details
        experiment_details = await automl.get_experiment_details("test_automl_experiment")
...
class TestModelManagementEndToEnd:
    ...
    async def test_complete_model_lifecycle(self, sample_dataset, sample_detector, temp_model_dir):
        ...
        improved_model = Model(
            name="end_to_end_model",
            algorithm=sample_detector.algorithm,
            parameters={**sample_detector.model.get_params(), "param1": 2.0},  # Improved parameter
