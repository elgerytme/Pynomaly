...
class TestDetectionFlow:
    ...
    async def test_experiment_tracking(self, container: Container):
        ...
        from pynomaly.application.use_cases import TrainDetectorRequest, EvaluateModelRequest
        ...
        # Train
        await train_use_case.execute(TrainDetectorRequest(
            detector_id=detector.id,
            dataset=dataset,
            validate_data=True,
            save_model=True
        ...
        # Evaluate
        eval_response = await evaluate_use_case.execute(EvaluateModelRequest(
            detector_id=detector.id,
