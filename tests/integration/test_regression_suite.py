...
class TestRegressionSuite:
    ...
    async def test_api_backward_compatibility(
        ...
        required_dataset_fields = ["id", "name", "status", "rows", "columns", "created_at"]
        ...
        required_detector_fields = ["id", "name", "algorithm", "status", "parameters", "feature_columns"]
        for field in required_detector_fields:
            assert field in detector, f"Detector response missing required field: {field}"
        ...
        for field in required_training_fields:
            assert field in training_result, f"Training response missing required field: {field}"
        ...
        for metric in metrics_fields:
            assert metric in training_result["metrics"], f"Training metrics missing: {metric}"
        ...
        for field in required_prediction_fields:
            assert field in prediction, f"Prediction response missing required field: {field}"
    ...
    async def test_model_serialization_compatibility(
        ...
        for field in required_export_fields:
            assert field in export_result, f"Export response missing required field: {field}"
        ...
        required_model_fields = ["id", "version", "algorithm", "parameters", "metrics", "created_at"]
    ...
    async def test_streaming_api_stability(
        ...
        required_session_fields = ["id", "name", "status", "detector_id", "data_source", "configuration"]
        for field in required_session_fields:
            assert field in session, f"Streaming session missing required field: {field}"
        
        # Test session lifecycle operations
        session_id = session["id"]
        
        # Start session
        response = await async_test_client.post(f"/api/streaming/sessions/{session_id}/start")
        response.raise_for_status()
        ...
        required_metrics_fields = ["messages_processed", "messages_per_second", "anomalies_detected", "anomaly_rate"]
        for field in required_metrics_fields:
            assert field in metrics, f"Streaming metrics missing required field: {field}"
        ...
        required_processing_fields = ["session_id", "anomaly_score", "is_anomaly", "timestamp"]
        for field in required_processing_fields:
            assert field in processing_result, f"Processing result missing required field: {field}"
        
        # Stop session
        response = await async_test_client.post(f"/api/streaming/sessions/{session_id}/stop")
        response.raise_for_status()
    ...
    async def test_experiment_api_consistency(
        ...
        required_experiment_fields = ["id", "name", "status", "algorithm_configurations", "evaluation_metrics"]
        for field in required_experiment_fields:
            assert field in experiment, f"Experiment response missing required field: {field}"
        ...
        required_results_fields = ["experiment_id", "algorithm_results", "best_algorithm"]
        for field in required_results_fields:
            assert field in results, f"Experiment results missing required field: {field}"
        ...
        for algo_result in results["algorithm_results"]:
            required_algo_fields = ["algorithm_name", "metrics", "cross_validation_scores"]
            for field in required_algo_fields:
                assert field in algo_result, f"Algorithm result missing required field: {field}"
        ...
        for field in required_comparison_fields:
            assert field in comparison, f"Experiment comparison missing required field: {field}"
    ...
    async def test_error_response_format_consistency(
        ...
        response = await async_test_client.post(
            "/api/detectors/create?dataset_id=invalid-id",
            json=invalid_detector_config
        )
    ...
    async def test_pagination_and_filtering_consistency(
        ...
        for field in required_pagination_fields:
            assert field in page1, f"Pagination response missing required field: {field}"
        ...
        # Test filtering
        response = await async_test_client.get("/api/datasets?name_contains=pagination_test")
        response.raise_for_status()
        ...
        pagination_datasets = [name for name in dataset_names if "pagination_test" in name]
    ...
    async def test_authentication_and_authorization_compatibility(
        ...
        if dataset_upload_response.status_code in [200, 201]:
            ...
            cleanup_response = await async_test_client.delete(f"/api/datasets/{dataset_data['id']}")
    ...
    async def test_performance_regression_baseline(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv,
            "performance_regression_dataset"
        )
        ...
        assert prediction_time < 2.0, f"Prediction took too long: {prediction_time:.2f}s"
        
        # Measure batch prediction time
        batch_data = {"data": [{"feature1": i * 0.1, "feature2": i * 0.2, "feature3": i * 0.05} for i in range(100)]}
        ...
        assert throughput > 10, f"Throughput too low: {throughput:.2f} predictions/second"
