...
class TestEnvironmentManager:
    ...
    def _install_packages(
        ...
        try:
            ...
            if requirements_file and Path(requirements_file).exists():
                self.logger.info(
                    f"Installing from requirements file: {requirements_file}"
                )
                subprocess.run(
    ...
    def create_test_matrix(self, config_file: str) -> List[Dict[str, Any]]:
        ...
        for env_config in config.get("environments", []):
            ...
            try:
                ...
                environments.append({"name": name, "error": str(e), "success": False})
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for env_config in environments:
                    ...
                    future = executor.submit(
                        self.run_tests_in_environment, env_name, test_command
                    )
                ...
                for future in concurrent.futures.as_completed(future_to_env):
                    ...
                    try:
                        ...
                        results["environments"].append(
                            {"environment": env_name, "error": str(e), "success": False}
                        )
        else:
    ...
    def generate_report(
        ...
        output_file: str = "test_matrix_report.html",
    ) -> str:
...
@click.option(
    "--command", "-cmd", default="python -m pytest tests/", help="Test command"
)
@click.option("--parallel", is_flag=True, help="Run tests in parallel")
@click.option(
    "--report", "-r", default="test_matrix_report.html", help="Report output file"
)
