...
class TestServiceIntegration(ServiceIntegrationTest):
    ...
    async def test_detection_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            result = await detection_service.detect_anomalies(
                detector_id=test_detector.id, dataset_id=test_dataset.id
            )
            ...
            communication_test = await self.test_service_communication(
                "detection_service", "result_repository", "sync"
            )
    ...
    async def test_dataset_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            update_config = {"description": "Updated description via service"}

            updated_dataset = await dataset_service.update_dataset(
                created_dataset.id, update_config
            )
    ...
    async def test_detector_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            test_dataset = await env.test_data_manager.create_dataset(
                name="detector_fitting_dataset", size=500, anomaly_rate=0.1, features=5
            )

            fitted_detector = await detector_service.fit_detector(
                created_detector.id, test_dataset.id
            )
            ...
            optimized_detector = await detector_service.optimize_parameters(
                created_detector.id, test_dataset.id, optimization_metric="f1_score"
            )
    ...
    async def test_caching_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            # Create test data
            test_dataset = await env.test_data_manager.create_dataset(
                name="cache_test_dataset", size=200, anomaly_rate=0.1, features=5
            )
    ...
    async def test_audit_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            # Create test data
            test_dataset = await env.test_data_manager.create_dataset(
                name="audit_test_dataset", size=100, anomaly_rate=0.1, features=3
            )
            ...
            await audit_service.log_event(
                action="detection_started", context=audit_context
            )
            ...
            audit_logs = await audit_service.get_audit_logs(
                filters={"user_id": "test_user"}, limit=10
            )
    ...
    async def test_configuration_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            config_updates = {
                ...
                "performance": {"max_concurrent_detections": 5, "timeout_seconds": 300},
            }
    ...
    async def test_cross_service_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            # Create test data
            test_dataset = await env.test_data_manager.create_dataset(
                name="cross_service_dataset", size=300, anomaly_rate=0.15, features=8
            )
            ...
            # Log start
            await audit_service.log_event(
                action="cross_service_detection_started", context=audit_context
            )
            ...
            # Verify audit logs
            audit_logs = await audit_service.get_audit_logs(
                filters={"user_id": "integration_test_user"}, limit=10
            )
