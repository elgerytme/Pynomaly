...
class TestDatabaseIntegration:
    ...
    async def test_detector_persistence_and_versioning(
        ...
        assert lineage_v1["training_dataset"] == lineage_v2["training_dataset"]  # Same dataset
        ...
        response = await async_test_client.get(f"/api/models/comparisons/{comparison_id}")
        response.raise_for_status()
    ...
    async def test_experiment_result_persistence(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv,
            "experiment_persistence_dataset"
        )
        ...
        response = await async_test_client.get(f"/api/experiments/{experiment_id}/results")
        response.raise_for_status()
        ...
        response = await async_test_client.get(f"/api/experiments/{experiment_id}/history")
        response.raise_for_status()
        ...
        assert "experiment_started" in event_types or "experiment_completed" in event_types
        ...
        response = await async_test_client.put(
            f"/api/experiments/{experiment_id}/metadata",
    ...
    async def test_streaming_session_state_persistence(
        ...
        test_data_points = [
            {"timestamp": f"2024-12-25T10:{i:02d}:00Z", "feature1": i * 0.1, "feature2": i * 0.2, "feature3": i * 0.05}
            for i in range(10)
        ]
    ...
    async def test_transaction_integrity_and_rollback(
        ...
        try:
            ...
            if response.status_code == 400:
                error = response.json()
                assert "dependent" in str(error).lower() or "constraint" in str(error).lower()
        except Exception:
