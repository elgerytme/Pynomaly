...
class MockedExternalDependencies:
    ...
    def setup_pyod_mocks(self):
        ...
        with patch.dict('sys.modules', {
            ...
            mock_iforest.decision_function = Mock(return_value=np.array([0.1, 0.8, 0.2, 0.9, 0.1]))
            ...
            mock_lof.decision_function = Mock(return_value=np.array([0.2, 0.7, 0.3, 0.8, 0.2]))
...
class TestEnhancedIntegrationIsolation:
    ...
    def test_data_pipeline_isolated(self, mocked_dependencies, isolated_filesystem):
        ...
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.IsolationForest') as mock_if:
            ...
            # Mock data loader
            with patch('pynomaly.infrastructure.data_loaders.csv_loader.CSVLoader') as mock_loader:
    ...
    @pytest.mark.asyncio
    async def test_async_service_integration_isolated(self, mocked_dependencies, isolated_database):
        """Test async service integration with database isolation."""
        
        # Mock async database session
        with patch('pynomaly.infrastructure.persistence.async_session_maker') as mock_session_maker:
            mock_session_maker.return_value = mocked_dependencies.database_mocks['session']
            
            # Mock repository
            with patch('pynomaly.infrastructure.repositories.detector_repository.DetectorRepository') as mock_repo:
                mock_detector = Mock()
                ...
                mock_repo.return_value.find_by_id = AsyncMock(return_value=mock_detector)
                ...
                with patch.object(DetectionService, '__init__', return_value=None):
                    ...
                    created_detector = await service.detector_repository.create(mock_detector)
                    found_detector = await service.detector_repository.find_by_id(mock_detector.id)
                    
                    assert created_detector.id == mock_detector.id
                    assert found_detector.name == mock_detector.name
    
    @pytest.mark.asyncio
    async def test_caching_integration_isolated(self, mocked_dependencies, isolated_redis):
        """Test caching integration with Redis isolation."""
        
        # Mock Redis client
        with patch('pynomaly.infrastructure.caching.redis_cache.RedisCache') as mock_cache:
            ...
            mock_cache.return_value.delete = mocked_dependencies.redis_mocks['client'].delete
    ...
    def test_external_api_integration_isolated(self, mocked_dependencies):
        ...
        with patch('requests.get') as mock_get, \
            ...
            from pynomaly.infrastructure.external_services.ml_model_service import MLModelService
            ...
            with patch.object(MLModelService, '__init__', return_value=None):
                ...
                service.get_model_info = Mock(return_value={"model_id": "test", "status": "ready"})
    ...
    def test_end_to_end_workflow_isolated(self, mocked_dependencies, isolated_filesystem, isolated_database):
        """Test complete end-to-end workflow with full isolation."""
        
        # Setup comprehensive mocking
        with patch('pynomaly.infrastructure.adapters.pyod_adapter.IsolationForest') as mock_if, \
             patch('pynomaly.infrastructure.persistence.async_session_maker') as mock_session, \
             patch('pynomaly.infrastructure.data_loaders.csv_loader.CSVLoader') as mock_loader:
    ...
    def test_error_recovery_isolated(self, mocked_dependencies):
        ...
        with patch('pynomaly.infrastructure.persistence.get_database_session') as mock_db:
    ...
    def test_concurrent_operations_isolated(self, mocked_dependencies):
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_adapter:
            ...
            def create_mock_detector(algorithm_name):
                ...
                mock.decision_function = Mock(return_value=np.array([0.1, 0.8, 0.2, 0.9]))
                return mock
    ...
    def test_resource_cleanup_isolated(self, mocked_dependencies, isolated_filesystem):
        ...
        try:
            ...
            with patch('pynomaly.infrastructure.persistence.create_connection') as mock_conn:
            ...
            for temp_file in cleanup_tracker["files_created"]:
                if temp_file.exists():
                    temp_file.unlink()
                    cleanup_tracker["resources_cleaned"].append(f"file:{temp_file.name}")
            
            for connection in cleanup_tracker["connections_opened"]:
                connection.close()
                cleanup_tracker["resources_cleaned"].append(f"connection:{id(connection)}")
        ...
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_adapter:
            ...
            for size in data_sizes:
                ...
                performance_results[size] = {
                    ...
                    "samples_per_second": size / execution_time if execution_time > 0 else float('inf'),
            ...
            assert performance_results[1000]["execution_time"] > performance_results[100]["execution_time"]
...
class TestContainerIntegration:
    ...
    async def test_real_database_integration(self, isolated_database):
        ...
        try:
            ...
            # Query test data
            cursor.execute("SELECT * FROM test_detectors WHERE name = %s", ("Test Detector",))
