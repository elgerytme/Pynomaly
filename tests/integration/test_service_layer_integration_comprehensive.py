...
class TestDetectionServiceIntegration:
    ...
    def mock_detection_service(self):
        ...
        class MockDetectionService:
            ...
            def train_detector(
                self, adapter_name: str, dataset: Dataset, parameters: dict[str, Any]
            ):
            ...
            def detect_anomalies(
                ...
                if threshold_config:
                    ...
                    if method == "contamination":
                        ...
                        threshold = (
                            self.threshold_calculator.calculate_contamination_threshold(
                                scores=scores,
                        ...
                        threshold = (
                            self.threshold_calculator.calculate_percentile_threshold(
                                scores=scores, percentile=percentile
                            )
                        )
                    else:
                ...
                labels = np.array(
                    [1 if score.value > threshold else 0 for score in scores]
                )
            ...
            def evaluate_performance(
                self, result: DetectionResult, true_labels: np.ndarray
            ):
    ...
    def test_detection_service_workflow_integration(
        self, mock_detection_service, sample_detection_data
    ):
        ...
        try:
            ...
            mock_detection_service.register_adapter(
                "isolation_forest", isolation_forest_adapter
            )
            ...
            detection_result = mock_detection_service.detect_anomalies(
                adapter_name="isolation_forest",
                dataset=sample_detection_data,
                threshold_config={"method": "contamination", "contamination": 0.1},
            )
            ...
            performance = mock_detection_service.evaluate_performance(
                result=detection_result, true_labels=true_labels
            )
    ...
    def test_detection_service_multiple_adapters_integration(
        self, mock_detection_service, sample_detection_data
    ):
    ...
    def test_detection_service_error_handling_integration(self, mock_detection_service):
        ...
        with pytest.raises(ValueError, match="not registered"):
            mock_detection_service.train_detector(
                adapter_name="nonexistent", dataset=Mock(), parameters={}
            )
...
class TestEnsembleServiceIntegration:
    ...
    def ensemble_test_data(self):
        ...
        normal_data = np.random.multivariate_normal(
            mean=[0, 0], cov=[[1, 0.3], [0.3, 1]], size=800
        )

        anomaly_data = np.random.multivariate_normal(
            mean=[4, 4], cov=[[0.5, 0], [0, 0.5]], size=200
        )
    ...
    def mock_ensemble_service(self):
        ...
        class MockEnsembleService:
            ...
            def ensemble_predict(
                self, dataset: Dataset, aggregation_method: str = "voting"
            ):
                ...
                if aggregation_method == "voting":
                    ...
                    weights = {
                        name: 1.0 / len(detector_predictions)
                        for name in detector_predictions
                    }
                    result = self.ensemble_aggregator.aggregate_weighted_average(
                        predictions=detector_predictions, weights=weights
                    )
                else:
                    raise ValueError(
    ...
    def test_ensemble_service_training_integration(
        self, mock_ensemble_service, ensemble_test_data
    ):
        ...
        try:
            ...
            training_results = mock_ensemble_service.train_ensemble(
                dataset=ensemble_test_data, detector_configs=detector_configs
            )
            ...
            successful_trainings = [
                name for name, result in training_results.items() if result["success"]
            ]
    ...
    def test_ensemble_service_prediction_integration(
        self, mock_ensemble_service, ensemble_test_data
    ):
        ...
        try:
            ...
            for method in aggregation_methods:
                ensemble_result = mock_ensemble_service.ensemble_predict(
                    dataset=ensemble_test_data, aggregation_method=method
                )
    ...
    def test_ensemble_service_evaluation_integration(
        self, mock_ensemble_service, ensemble_test_data
    ):
