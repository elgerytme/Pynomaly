...
class TestAPIIntegration(ServiceIntegrationTest):
    ...
    async def test_detector_crud_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            found_detector = next(
                (d for d in detectors_list if d["id"] == detector_id), None
            )
    ...
    async def test_detection_workflow_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            response = client.post("/api/v1/detection/predict", json=detection_request)
    ...
    async def test_error_handling_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            response = client.post("/api/v1/detection/predict", json=invalid_detection)
            assert response.status_code == 404
    ...
    async def test_streaming_endpoint_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            response = client.post("/api/v1/streaming/configure", json=config_data)
            assert response.status_code == 200
    ...
    async def test_export_integration(self):
        ...
        async with self.setup_test_environment() as env:
            ...
            # Create test data
            test_dataset = await env.test_data_manager.create_dataset(
                name="export_test_dataset", size=100, anomaly_rate=0.1, features=5
            )
