...
class TestSecurityIntegration:
    ...
    async def test_input_validation_and_sanitization(
        ...
        for payload in sql_injection_payloads:
            ...
            response = await async_test_client.get(f"/api/datasets?name_contains={payload}")
            ...
            if response.status_code in [200, 201]:
                ...
                assert payload not in created_detector["name"] or created_detector["name"] == payload
                integration_helper.created_resources["detectors"].append(created_detector["id"])
        ...
        for payload in xss_payloads:
            ...
            if response.status_code == 200:
                ...
                assert "<script>" not in description or payload == description  # Either sanitized or stored as-is
    ...
    async def test_authorization_and_access_control(
        ...
        for role_info in test_roles:
            headers = {
                "X-User-Role": role_info["role"],
                "X-User-ID": role_info["user"]
            }
            ...
            response = await async_test_client.delete(f"/api/datasets/{dataset['id']}", headers=headers)
...
admin@company.com,Administrator,admin_secret"""
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                integration_helper.created_resources["datasets"].append(sensitive_dataset["id"])
                
                # Test data access controls
                response = await async_test_client.get(f"/api/datasets/{sensitive_dataset['id']}/data")
                ...
                if response.status_code == 200:
                    ...
                    sensitive_indicators = ["john.doe@email.com", "secret123", "password456"]
                    
                    # In a secure system, sensitive data should be masked
                    exposed_count = sum(1 for indicator in sensitive_indicators if indicator in data_str)
                    # This test documents current behavior rather than enforcing encryption
                    print(f"Sensitive data exposure check: {exposed_count}/{len(sensitive_indicators)} indicators found")
    ...
    async def test_rate_limiting_and_ddos_protection(
        ...
        for endpoint in endpoints_to_test:
            ...
            rate_limited_count = sum(1 for r in rapid_results if r.get("rate_limited", False))
            successful_count = sum(1 for r in rapid_results if r.get("status_code") == 200)
            
            # Rate limiting might not be implemented, so we document behavior
            print(f"Endpoint {endpoint}: {successful_count}/30 successful, {rate_limited_count} rate limited")
        ...
        async def concurrent_request_flood(endpoint: str, concurrent_count: int):
            ...
            tasks = [
                async_test_client.get(endpoint)
        ...
        assert flood_result["successful"] + flood_result["rate_limited"] >= flood_result["total_requests"] * 0.5
        
        print(f"Concurrent flood test: {flood_result['successful']}/{flood_result['total_requests']} successful")
        ...
        successful_predictions = sum(1 for r in intensive_results if r.get("success", False))
    ...
    async def test_secure_file_upload_handling(
        ...
        malicious_files = [
            ...
            ("virus.bat", b"@echo off\ndel /f /q C:\\*.*", "application/x-msdos-program"),
        ...
        assert response.status_code in [200, 201, 400, 413, 422]  # Success or file too large
        ...
        invalid_csv_files = [
            ...
            ("xml_injection.csv", b"<?xml version='1.0'?><root><![CDATA[malicious]]></root>", "text/csv"),
    ...
    async def test_api_security_headers(
        ...
        for endpoint in endpoints_to_test:
            ...
            print(f"Endpoint {endpoint}: {len(headers_present)}/{len(expected_security_headers)} security headers present")
            ...
            for sensitive_header in sensitive_header_patterns:
                if sensitive_header in response.headers:
                    ...
                    print(f"Sensitive header found - {sensitive_header}: {header_value}")
        ...
        if cors_response.status_code in [200, 204]:
            ...
            cors_present = sum(1 for header in cors_headers if header in cors_response.headers)
        ...
        disclosed_info = [pattern for pattern in sensitive_patterns if pattern in error_content]
