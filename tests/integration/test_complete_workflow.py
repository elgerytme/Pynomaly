...
class TestCompleteWorkflowIntegration:
    ...
    def real_test_data(self):
        ...
        df = pd.DataFrame(shuffled_data, columns=['feature_1', 'feature_2', 'feature_3'])
    ...
    @pytest.mark.integration
    def test_step2_api_endpoints_with_real_algorithms(self, client, mock_container_with_real_services, real_test_data):
        """Step 2: Test API endpoints work with real algorithms."""
        with patch('pynomaly.presentation.api.app.get_container', return_value=mock_container_with_real_services):
            ...
            detector = Detector(
                ...
                contamination_rate=ContaminationRate(created_detector["contamination_rate"]),
    ...
    def test_step3_pwa_functionality(self, client, mock_container_with_real_services):
        ...
        with patch('pynomaly.presentation.web.app.get_container', return_value=mock_container_with_real_services):
            # Test PWA manifest
    ...
    @pytest.mark.integration
    def test_step4_complete_end_to_end_workflow(self, client, mock_container_with_real_services, real_test_data):
        ...
        with patch('pynomaly.presentation.web.app.get_container', return_value=mock_container_with_real_services):
            ...
            detector = Detector(
                ...
                contamination_rate=ContaminationRate(detector_info["contamination_rate"]),
            ...
            assert "anomalies" in response_text.lower() or "detected" in response_text.lower()
    ...
    def test_real_algorithms_comparison(self):
        ...
        for algorithm in algorithms:
            try:
                detector = Detector(
                    ...
                    contamination_rate=ContaminationRate(0.1),
                    parameters={"random_state": 42} if algorithm != "LOF" else {"n_neighbors": 20}
                )
                ...
                results[algorithm] = {
                    ...
                    "score_range": (min(s.value for s in scores), max(s.value for s in scores))
                }
        ...
        for algorithm, result in results.items():
            ...
            assert 0.0 < result["anomaly_rate"] < 0.5, f"{algorithm} anomaly rate out of reasonable range"
    ...
    def test_scalability_with_larger_dataset(self):
        ...
        assert expected_range[0] <= anomaly_count <= expected_range[1], \
            f"Anomaly count {anomaly_count} outside expected range {expected_range}"

    @pytest.mark.integration
    def test_feature_integration_completeness(self, client, mock_container_with_real_services):
        """Test that all 4 major features are properly integrated."""
        with patch('pynomaly.presentation.api.app.get_container', return_value=mock_container_with_real_services), \
             patch('pynomaly.presentation.web.app.get_container', return_value=mock_container_with_real_services):
