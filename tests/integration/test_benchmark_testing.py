...
class PerformanceBenchmark:
    ...
    def generate_benchmark_dataset(self, n_samples: int, n_features: int, contamination: float = 0.1, random_state: int = 42) -> pd.DataFrame:
    ...
    def benchmark_algorithm(
        ...
        n_samples, n_features = dataset.shape[0], dataset.shape[1] - 1  # Exclude label column
        ...
        try:
            ...
            accuracy_metrics = self._calculate_accuracy_metrics(y_true, predictions, scores)
    ...
    def _calculate_accuracy_metrics(self, y_true: np.ndarray, predictions: np.ndarray, scores: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            metrics['detection_rate'] = np.sum((y_true == 1) & (predictions == 1)) / max(n_anomalies_true, 1)
    ...
    def run_comprehensive_benchmark(self, algorithms: List[str], dataset_sizes: List[Tuple[int, int]], n_runs: int = 3) -> Dict[str, PerformanceProfile]:
        ...
        for algorithm in algorithms:
            ...
            for n_samples, n_features in dataset_sizes:
                for run in range(n_runs):
                    print(f"Benchmarking {algorithm} on {n_samples}x{n_features} dataset (run {run + 1}/{n_runs})")
                    
                    # Generate dataset for this run
                    dataset = self.generate_benchmark_dataset(n_samples, n_features, random_state=42 + run)
                    
                    # Determine adapter class
                    if algorithm in ["IsolationForest", "LocalOutlierFactor", "OneClassSVM"]:
    ...
    def _create_performance_profile(self, algorithm: str, results: List[BenchmarkResult]) -> PerformanceProfile:
        ...
        # Calculate averages
        avg_training_time = statistics.mean([r.training_time_ms for r in successful_results])
        avg_prediction_time = statistics.mean([r.prediction_time_ms for r in successful_results])
        
        # Calculate time complexity factor (time increase per sample)
        if len(successful_results) > 1:
            time_complexity_factor = self._calculate_time_complexity_factor(successful_results)
        else:
            time_complexity_factor = 1.0
        
        # Calculate memory efficiency (MB per 1000 samples)
        memory_efficiency = statistics.mean([
            r.memory_usage_mb / (r.dataset_size[0] / 1000) for r in successful_results
        ...
        if len(successful_results) > 1:
            ...
            cv = statistics.stdev(training_times) / statistics.mean(training_times) if statistics.mean(training_times) > 0 else 1
        ...
        memory_score = 1 / (1 + memory_efficiency / 10)  # Normalize by 10MB per 1K samples
        accuracy_score = avg_accuracy
        
        overall_score = (time_score * 0.3 + memory_score * 0.2 + accuracy_score * 0.3 + stability_score * 0.2)
    ...
    def _calculate_time_complexity_factor(self, results: List[BenchmarkResult]) -> float:
    ...
    def generate_performance_report(self) -> str:
        ...
        sorted_algorithms = sorted(self.profiles.items(), key=lambda x: x[1].overall_score, reverse=True)
        
        report.append("| Rank | Algorithm | Overall Score | Training Time (ms) | Memory Efficiency | Accuracy | Stability |")
        report.append("|------|-----------|---------------|-------------------|-------------------|----------|-----------|")
        ...
        for algorithm, profile in sorted_algorithms:
            ...
            report.append(f"- **Average Training Time**: {profile.avg_training_time_ms:.1f} ms")
            report.append(f"- **Average Prediction Time**: {profile.avg_prediction_time_ms:.1f} ms") 
            report.append(f"- **Time Complexity Factor**: {profile.time_complexity_factor:.2f}")
            report.append(f"- **Memory Efficiency**: {profile.memory_efficiency:.2f} MB per 1000 samples")
        ...
        report.append(f"- **Best Overall Performance**: {best_overall[0]} (score: {best_overall[1].overall_score:.3f})")
        report.append(f"- **Fastest Training**: {best_speed[0]} ({best_speed[1].avg_training_time_ms:.1f} ms)")
        report.append(f"- **Most Memory Efficient**: {best_memory[0]} ({best_memory[1].memory_efficiency:.2f} MB/1K)")
        report.append(f"- **Highest Accuracy**: {best_accuracy[0]} ({best_accuracy[1].accuracy_score:.3f})")
        report.append("")
        
        # Usage recommendations
        report.append("### Usage Recommendations")
        report.append("")
        report.append("- **For real-time applications**: Choose algorithms with low training and prediction times")
        report.append("- **For large datasets**: Prioritize memory efficiency and time complexity")
        report.append("- **For high accuracy requirements**: Focus on accuracy score while considering computational cost")
        report.append("- **For production stability**: Consider stability score for consistent performance")
        report.append("")
...
class TestPerformanceBenchmarking:
    ...
    def test_algorithm_benchmark_small_dataset(self):
        ...
        assert result.total_time_ms == result.training_time_ms + result.prediction_time_ms
    ...
    def test_multiple_algorithm_comparison(self):
        ...
        # Run benchmark
        profiles = benchmark.run_comprehensive_benchmark(algorithms, dataset_sizes, n_runs=2)
    ...
    def test_dataset_generation(self):
        ...
        for n_samples, n_features, contamination in configs:
            dataset = benchmark.generate_benchmark_dataset(n_samples, n_features, contamination)
            ...
            assert abs(actual_contamination - contamination) < 0.05  # Allow 5% tolerance
    ...
    def test_performance_report_generation(self):
        ...
        profiles = benchmark.run_comprehensive_benchmark(algorithms, dataset_sizes, n_runs=1)
    ...
    def test_memory_usage_tracking(self):
        ...
        assert result.memory_usage_mb < 100  # Should not use more than 100MB for this test
    ...
    def test_accuracy_metrics_calculation(self):
        ...
        for metric_name, value in metrics.items():
            assert 0 <= value <= 1, f"Metric {metric_name} = {value} is out of range [0, 1]"
    ...
    def test_concurrent_benchmarking(self):
        ...
        def run_single_benchmark(algorithm):
            dataset = benchmark.generate_benchmark_dataset(100, 5, random_state=hash(algorithm) % 1000)
...
class TestPerformanceRegression:
    ...
    def test_performance_baseline_establishment(self):
        ...
        dataset = benchmark.generate_benchmark_dataset(1000, 10, contamination=0.1, random_state=42)
        
        # Benchmark standard algorithms
        baseline_algorithms = [
            ("IsolationForest", SklearnAdapter, {"n_estimators": 100, "contamination": 0.1, "random_state": 42}),
            ("LocalOutlierFactor", SklearnAdapter, {"n_neighbors": 20, "contamination": 0.1}),
        ]
        
        baselines = {}
        
        for algorithm, adapter_class, hyperparams in baseline_algorithms:
            result = benchmark.benchmark_algorithm(algorithm, dataset, adapter_class, hyperparams)
    ...
    def test_performance_consistency(self):
        ...
        dataset = benchmark.generate_benchmark_dataset(500, 8, contamination=0.1, random_state=42)
        ...
        for run in range(n_runs):
            result = benchmark.benchmark_algorithm(
                ...
                dataset,
                SklearnAdapter,
                {"n_estimators": 50, "contamination": 0.1, "random_state": 42}  # Fixed seed for consistency
            )
        ...
        if len(results) > 1:
            ...
            training_cv = statistics.stdev(training_times) / statistics.mean(training_times)
            prediction_cv = statistics.stdev(prediction_times) / statistics.mean(prediction_times)
    ...
    def test_scalability_characteristics(self):
        ...
        for n_samples, n_features in sizes:
            dataset = benchmark.generate_benchmark_dataset(n_samples, n_features, random_state=42)
            result = benchmark.benchmark_algorithm(
                algorithm,
                dataset,
                SklearnAdapter,
        ...
        assert len(results) >= 2, "Need at least 2 successful results to test scalability"
        ...
        for i in range(1, len(results)):
            ...
            memory_factor = curr_result.memory_usage_mb / max(prev_result.memory_usage_mb, 1)
            
            print(f"Size factor: {size_factor:.2f}x, Time factor: {time_factor:.2f}x, Memory factor: {memory_factor:.2f}x")
            
            # Time scaling should be reasonable (not exponential)
            assert time_factor < size_factor ** 2, f"Time scaling too poor: {time_factor} vs {size_factor}^2"
            
            # Memory scaling should be reasonable
            assert memory_factor < size_factor * 2, f"Memory scaling too poor: {memory_factor} vs {size_factor}*2"
