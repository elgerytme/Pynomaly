...
class ExternalDependencyStabilizer:
    ...
    def configure_retry(self, dependency_name: str, max_retries: int = 3, 
                       retry_delay: float = 1.0, exponential_backoff: bool = True):
    ...
    def stabilized_dependency(self, dependency_name: str):
        ...
        if is_healthy:
            ...
            if dependency_name in self.mock_registry:
                ...
                pytest.skip(f"Dependency {dependency_name} unavailable and no fallback configured")
...
stabilizer.register_health_check("database", lambda: os.getenv("DATABASE_URL") is not None)
    ...
    async def test_cache_integration_stable(self):
        ...
        with stabilizer.stabilized_dependency("redis") as dependency_type:
            ...
            if dependency_type == "real":
                ...
                try:
                    import aioredis
                    redis = await aioredis.from_url(os.getenv("REDIS_URL", "redis://localhost"))
    ...
    def test_error_recovery_mechanisms(self):
        ...
        try:
            with patch('sklearn.ensemble.IsolationForest') as mock_if:
                ...
                try:
                    detector = mock_if()
                    error_scenarios.append("ml_library_first_success")  # Should not reach here
                except ImportError:
        ...
        for scenario in expected_scenarios:
            assert scenario in error_scenarios, f"Expected scenario '{scenario}' not found in {error_scenarios}"
