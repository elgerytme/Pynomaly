...
""")
            ...
            todo_violations = [v for v in validator.result.violations if "TODO" in v.message]
            assert len(todo_violations) == 1
            assert todo_violations[0].severity == ViolationSeverity.LOW
            
            # Check for print statement violation
            print_violations = [v for v in validator.result.violations if "print" in v.message.lower()]
...
""")
            ...
            assert len(validator.result.violations) >= 1
            eval_violations = [v for v in validator.result.violations if "eval" in v.message]
...
class TestValidationResult:
    ...
    def test_group_by_severity(self):
        ...
        violations = [
            ...
            ValidationViolation("Another Critical", ViolationSeverity.CRITICAL, "test.py"),
        ]
