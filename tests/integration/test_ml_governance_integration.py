...
@pytest.mark.asyncio
async def test_model_onboarding_complete_workflow(governance_service, sample_model, sample_validation_data):
    ...
    validation_check = next(
        (c for c in record.compliance_checks if c["type"] == "validation"), 
        None
    )
...
@pytest.mark.asyncio
async def test_model_approval_workflow(governance_service, sample_model, sample_validation_data):
...
@pytest.mark.asyncio
async def test_model_deployment_stages(governance_service, sample_model, sample_validation_data):
...
@pytest.mark.asyncio
async def test_model_promotion_through_stages(governance_service, sample_model, sample_validation_data):
...
@pytest.mark.asyncio
async def test_governance_audit(governance_service, sample_model, sample_validation_data):
...
async def test_bulk_compliance_check(governance_service, sample_validation_data):
    ...
    assert compliance_results["compliant_models"] + compliance_results["non_compliant_models"] >= 3
...
@pytest.mark.asyncio
async def test_governance_dashboard(governance_service, sample_model, sample_validation_data):
...
@pytest.mark.asyncio
async def test_governance_record_retrieval(governance_framework, sample_model, sample_validation_data):
    ...
    retrieved_record = governance_framework.get_model_governance_record(record1.record_id)
    ...
    dev_records = governance_framework.list_governance_records(stage=ModelStage.DEVELOPMENT)
    assert len(dev_records) >= 2
    
    # Test filtering by status
    pending_records = governance_framework.list_governance_records(status=GovernanceStatus.PENDING)
