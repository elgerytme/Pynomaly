...
class TestStreamingIntegration:
    ...
    async def test_websocket_streaming_monitoring(
        ...
        test_data_points = [
            ...
            {"timestamp": "2024-12-25T10:02:00Z", "value": 150.0, "cpu_usage": 95.2},  # Anomaly
        ]
        ...
        assert processing_results[2]["is_anomaly"] is True  # Last point should be anomaly
        assert processing_results[2]["anomaly_score"] > processing_results[0]["anomaly_score"]
    ...
    async def test_streaming_session_lifecycle(
        ...
        response = await async_test_client.post(f"/api/streaming/sessions/{session_id}/pause")
        response.raise_for_status()
        paused_session = response.json()["data"]
        assert paused_session["status"] == "paused"
        
        # Step 5: Resume session
        response = await async_test_client.post(f"/api/streaming/sessions/{session_id}/resume")
        response.raise_for_status()
        resumed_session = response.json()["data"]
        assert resumed_session["status"] == "active"
        
        # Step 6: Get session summary
        response = await async_test_client.get(f"/api/streaming/sessions/{session_id}/summary")
        response.raise_for_status()
    ...
    async def test_streaming_session_scaling(
        ...
        tasks = [
            process_data_for_session(session["id"], 10)
            for session in sessions
        ]
    ...
    async def test_streaming_error_handling(
        ...
        invalid_data_cases = [
            ...
            # Invalid data types
            {"data": {"timestamp": "invalid", "feature1": "not_a_number", "feature2": None, "feature3": []}},
            # Out of range values
            {"data": {"timestamp": "2024-12-25T10:02:00Z", "feature1": float('inf'), "feature2": float('nan'), "feature3": 1e10}}
        ]
        ...
        assert metrics["failed_messages"] >= 0  # Might be 0 if errors are handled gracefully
        
        # Step 6: Test session recovery after errors
        # Send more valid data to ensure session can continue
        recovery_data = [
            {"timestamp": "2024-12-25T10:03:00Z", "feature1": 0.5, "feature2": 1.5, "feature3": 2.5},
            {"timestamp": "2024-12-25T10:04:00Z", "feature1": 0.8, "feature2": 1.2, "feature3": 1.8}
        ]
    ...
    async def test_streaming_data_sink_integration(
        ...
        test_data_points = [
            {"timestamp": "2024-12-25T10:00:00Z", "feature1": 0.1, "feature2": 0.2, "feature3": 0.3},  # Normal
            {"timestamp": "2024-12-25T10:01:00Z", "feature1": 5.0, "feature2": 4.8, "feature3": 3.2},  # Anomaly
            {"timestamp": "2024-12-25T10:02:00Z", "feature1": 0.2, "feature2": 0.3, "feature3": 0.1},  # Normal
        ]
