...
def sample_datasets():
    ...
    datasets['normal'] = pd.DataFrame(normal_data, columns=[f'feature_{i}' for i in range(5)])
    ...
    datasets['with_anomalies'] = pd.DataFrame(anomaly_data, columns=[f'feature_{i}' for i in range(5)])
    
    # Large dataset
    large_data = np.random.normal(0, 1, (10000, 20))
    datasets['large'] = pd.DataFrame(large_data, columns=[f'feature_{i}' for i in range(20)])
    
    # Time series data
    time_data = np.sin(np.linspace(0, 20*np.pi, 1000)) + np.random.normal(0, 0.1, 1000)
    ...
    class PerformanceMonitor:
        ...
        def get_stats(self):
            return {
                ...
                'memory_usage_mb': self.peak_memory - self.start_memory if self.start_memory else 0,
...
class TestEndToEndAPIWorkflows:
    ...
    async def test_complete_anomaly_detection_pipeline(
        ...
        while time.time() - start_time < timeout:
            status_response = await async_api_client.get(f"/api/experiments/{experiment_id}")
            ...
            if experiment_status["status"] == "completed":
                ...
                pytest.fail(f"Experiment failed: {experiment_status.get('error_message')}")
        ...
        results_response = await async_api_client.get(f"/api/experiments/{experiment_id}/results")
    ...
    @pytest.mark.asyncio
    async def test_streaming_detection_workflow(self, async_api_client: AsyncClient, sample_datasets):
        ...
        streaming_points = sample_datasets['with_anomalies'].iloc[:100].to_dict('records')
    ...
    @pytest.mark.asyncio
    async def test_concurrent_detection_operations(self, async_api_client: AsyncClient, sample_datasets):
        ...
        assert successful_trainings >= len(training_tasks) * 0.8  # At least 80% success rate
        ...
        assert successful_predictions >= len(prediction_tasks) * 0.8  # At least 80% success rate
...
class TestCrossLayerIntegration:
    ...
    def test_api_cli_web_consistency(self, api_client: TestClient, web_client: TestClient, cli_runner: CliRunner):
        ...
        with patch('pynomaly.presentation.web.app.get_detector_details') as mock_get_details:
            ...
            web_response = web_client.get(
                f"/htmx/detectors/{detector_id}/details",
    ...
    def test_error_propagation_across_layers(self, integration_container):
        ...
        with pytest.raises(Exception):  # Could be FileNotFoundError or custom domain exception
        ...
        with pytest.raises(Exception):  # Could be ValidationError or custom domain exception
...
class TestPerformanceAndScalability:
    ...
    def test_large_dataset_processing_performance(self, api_client: TestClient, performance_monitor):
        ...
        assert stats['memory_usage_mb'] < 1000  # Should not use more than 1GB additional memory
        
        print(f"Performance Stats - Upload: {upload_time:.2f}s, Train: {train_time:.2f}s, "
    ...
    def test_concurrent_user_simulation(self, api_client: TestClient):
        ...
        def user_workflow(user_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                upload_response = api_client.post(
                    "/api/datasets/",
                    files={"file": (f"user_{user_id}_data.csv", csv_data.encode(), "text/csv")},
                ...
                detector_response = api_client.post(
                    ...
                    json={
                        ...
                        "algorithm": "IsolationForest",
                        "hyperparameters": {"contamination": 0.1, "random_state": user_id}
                    }
                )
                if detector_response.status_code != 201:
                    return {"user_id": user_id, "success": False, "error": "Detector creation failed"}
                ...
                if train_response.status_code != 200:
                    return {"user_id": user_id, "success": False, "error": "Training failed"}
                ...
                if predict_response.status_code != 200:
                    return {"user_id": user_id, "success": False, "error": "Prediction failed"}
        ...
        print(f"Concurrent users test: {successful_users}/{num_users} successful in {total_time:.2f}s")
        if failed_users:
...
class TestRealWorldScenarios:
    ...
    def test_production_monitoring_scenario(self, api_client: TestClient, sample_datasets):
        ...
        drift_batch = pd.DataFrame(drift_data, columns=[f'feature_{i}' for i in range(5)])
        ...
        for batch_name, batch_data in production_batches:
            ...
            upload_response = api_client.post(
                "/api/datasets/",
                files={"file": (f"prod_batch_{batch_name}.csv", batch_csv.encode(), "text/csv")},
            ...
            anomaly_rate = prediction_result["anomaly_count"] / len(prediction_result["predictions"])
        ...
        normal_result = next(r for r in monitoring_results if r["batch_name"] == "normal")
        assert normal_result["anomaly_rate"] < 0.1
        
        # Anomaly batch should have higher anomaly rate
        anomaly_result = next(r for r in monitoring_results if r["batch_name"] == "with_anomalies")
        ...
        high_alert_batches = [r for r in monitoring_results if r["anomaly_rate"] > alert_threshold]
        ...
        for result in monitoring_results:
            print(f"  {result['batch_name']}: {result['anomaly_rate']:.3f} anomaly rate, "
...
class TestSystemReliability:
    ...
    def test_graceful_error_handling_and_recovery(self, api_client: TestClient):
        ...
        invalid_response = api_client.post(
            "/api/datasets/",
            files={"file": ("invalid.csv", b"invalid,csv,content\nwith,missing\n", "text/csv")},
    ...
    def test_resource_cleanup_and_management(self, api_client: TestClient):
        ...
        try:
            ...
            for i in range(5):
                ...
                upload_response = api_client.post(
                    "/api/datasets/",
                    files={"file": (f"cleanup_test_{i}.csv", data.encode(), "text/csv")},
            ...
            for dataset_id, detector_id in created_resources:
                ...
                delete_detector_response = api_client.delete(f"/api/detectors/{detector_id}")
                assert delete_detector_response.status_code in [200, 204]
                
                # Delete dataset
                delete_dataset_response = api_client.delete(f"/api/datasets/{dataset_id}")
