...
class TestDatabaseIntegration:
    ...
    async def test_detection_result_storage(self, database_manager):
        ...
        scores = [AnomalyScore(0.1), AnomalyScore(0.9), AnomalyScore(0.2), 
                 AnomalyScore(0.3), AnomalyScore(0.8)]
        ...
        results = await database_manager.list_detection_results(detector_id="detector_001")
    ...
    async def test_concurrent_database_operations(self, database_manager):
        ...
        retrieval_tasks = [
            database_manager.get_dataset(d.id) for d in datasets
        ]
...
class TestAPIIntegration:
    ...
    def test_detector_training_api_workflow(self, test_client):
        ...
        response = test_client.post(f"/api/v1/detectors/{detector_id}/train", json=training_data)
    ...
    def test_api_authentication_integration(self, test_client):
        ...
        if response.status_code == 401:
            ...
            with patch('pynomaly.infrastructure.auth.jwt_auth.verify_token') as mock_verify:
...
class TestDataFlowIntegration:
    ...
    async def test_batch_processing_pipeline(self):
        ...
        from pynomaly.application.services.batch_service import BatchService
        
        with patch('pynomaly.application.services.batch_service.BatchService') as mock_service:
    ...
    async def test_model_versioning_pipeline(self):
        ...
        with patch('pynomaly.application.services.model_persistence_service.ModelPersistenceService') as mock_service:
            service = mock_service.return_value
...
class TestCrossLayerIntegration:
    ...
    async def test_full_stack_integration(self):
        ...
        # Mock all layers
        with patch('pynomaly.infrastructure.adapters.sklearn_adapter.SklearnAdapter') as mock_adapter, \
             patch('pynomaly.infrastructure.persistence.database.DatabaseManager') as mock_db, \
             patch('pynomaly.application.services.detection_service.DetectionService') as mock_service:
    ...
    async def test_monitoring_integration(self):
        ...
        with patch('pynomaly.infrastructure.monitoring.telemetry.MetricsCollector') as mock_metrics, \
             patch('pynomaly.infrastructure.monitoring.health_service.HealthService') as mock_health:
            ...
            metrics_collector.record_metric("domain.entity.created", 1, tags={"type": "dataset"})
            
            # Application layer metrics
            metrics_collector.record_metric("application.service.called", 1, tags={"service": "detection"})
            
            # Infrastructure layer metrics
            metrics_collector.record_metric("infrastructure.adapter.prediction", 1, tags={"algorithm": "isolation_forest"})
            
            # API layer metrics
            metrics_collector.record_metric("api.request.processed", 1, tags={"endpoint": "/detect"})
...
class TestPerformanceIntegration:
    ...
    async def test_database_performance_integration(self):
        ...
        with patch('pynomaly.infrastructure.persistence.database.DatabaseManager') as mock_db:
            ...
            db_manager.list_datasets.return_value = [Mock(id=f"dataset_{i}") for i in range(100)]
            
            # Execute timed operations
            await timed_operation("save_dataset", lambda: db_manager.save_dataset(Mock()))
            await timed_operation("get_dataset", lambda: db_manager.get_dataset("test_id"))
        ...
        save_time = next(op["duration"] for op in operations if op["operation"] == "save_dataset")
        get_time = next(op["duration"] for op in operations if op["operation"] == "get_dataset")
        list_time = next(op["duration"] for op in operations if op["operation"] == "list_datasets")
