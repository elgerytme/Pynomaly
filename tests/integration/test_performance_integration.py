...
class TestPerformanceIntegration:
    ...
    async def test_api_performance_under_load(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "performance_test_dataset"
        )
        ...
        batch_tasks = [
            make_prediction_batch(i, batch_size) for i in range(num_concurrent_batches)
        ]
        ...
        assert (
            len(successful_batches) >= num_concurrent_batches * 0.9
        )  # 90% success rate
    ...
    async def test_streaming_performance_scalability(
        ...
        # Setup
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "streaming_scale_dataset"
        )
        ...
        for i in range(num_sessions):
            ...
            response = await async_test_client.post(
                "/api/streaming/sessions?created_by=test_user", json=session_config
            )
            response.raise_for_status()
        ...
        async def process_data_load(session_id: str, num_messages: int):
            ...
            return {
                ...
                "error_rate": (
                    error_count / (processed_count + error_count)
        ...
        assert (
            len(successful_sessions) >= num_sessions * 0.8
        )  # 80% of sessions successful
    ...
    async def test_database_performance_under_load(
        ...
        async def concurrent_dataset_operations(operation_id: int):
            ...
            try:
                # Upload dataset
                dataset = await integration_helper.upload_dataset(
                    sample_dataset_csv, f"concurrent_dataset_{operation_id}"
                )
                operations.append(
                    {
                        "operation": "upload",
                ...
                # Get dataset stats
                response = await async_test_client.get(
                    f"/api/datasets/{dataset['id']}/stats"
                )
                response.raise_for_status()
                operations.append(
                    {
        ...
        db_tasks = [concurrent_dataset_operations(i) for i in range(num_concurrent_ops)]
        ...
        successful_ops = [
            r for r in db_results if not isinstance(r, Exception) and r["success"]
        ]
        assert len(successful_ops) >= num_concurrent_ops * 0.8  # 80% success rate

        total_db_time = end_db_test - start_db_test
        avg_operation_time = sum(r["total_time"] for r in successful_ops) / len(
            successful_ops
        )
            ...
            if "dataset_id" in result:
                try:
                    await async_test_client.delete(
                        f"/api/datasets/{result['dataset_id']}"
                    )
                except:
    ...
    async def test_memory_and_resource_usage(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "resource_test_dataset"
        )
        ...
        response = await async_test_client.post(
            f"/api/detection/predict/{detector['id']}", json=large_batch_data
        )
        response.raise_for_status()
        ...
        first_half_avg = sum(sustained_times[: sustained_batches // 2]) / (
            sustained_batches // 2
        )
        second_half_avg = sum(sustained_times[sustained_batches // 2 :]) / (
            sustained_batches // 2
        )
            ...
            operations = [
                ...
                async_test_client.post(
                    ...
                    json={
                        "data": [
                            {
                                "feature1": task_id * 0.1,
                                "feature2": task_id * 0.2,
                                "feature3": task_id * 0.05,
                            }
                        ]
                    },
                ),
        ...
        resource_tasks = [
            resource_intensive_task(i) for i in range(num_concurrent_tasks)
        ]
    ...
    async def test_api_rate_limiting_and_throttling(
        ...
        # Setup
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "rate_limit_dataset"
        )
        ...
        async def rapid_requests(request_count: int, delay: float = 0.01):
            ...
            return {
                ...
                "rate_limited_count": sum(
                    1 for r in results if r.get("rate_limited", False)
                ),
            }
        ...
        total_requests = (
            rapid_result["success_count"]
            + burst_result["success_count"]
            + normal_result["success_count"]
        )
        total_rate_limited = (
            rapid_result["rate_limited_count"]
            + burst_result["rate_limited_count"]
            + normal_result["rate_limited_count"]
        )
        ...
        assert (
            rapid_result["request_rate"] > 10
        )  # Should achieve reasonable request rate

        # If rate limiting is implemented, we might see 429 responses
        if total_rate_limited > 0:
            assert (
                burst_result["rate_limited_count"] >= rapid_result["rate_limited_count"]
            )  # Burst should trigger more limiting
            assert (
        ...
        print(
            f"  Burst requests: {burst_result['success_count']}/{len(burst_result['results'])} success, rate: {burst_result['request_rate']:.2f} req/s"
        )
        print(
            f"  Normal requests: {normal_result['success_count']}/{len(normal_result['results'])} success, rate: {normal_result['request_rate']:.2f} req/s"
        )
        ...
        async def sustained_client_load(client_id: int, duration_seconds: int):
            ...
            while time.time() - start_time < duration_seconds:
                test_data = {
                    "data": [
                        {
                            "feature1": client_id * 0.1,
                            "feature2": request_count * 0.01,
                            "feature3": 0.1,
                        }
                    ]
                }
        ...
        client_tasks = [
            sustained_client_load(i, test_duration) for i in range(num_clients)
        ]
        ...
        aggregate_success_rate = (
            total_sustained_successes / total_sustained_requests
            if total_sustained_requests > 0
            else 0
        )
