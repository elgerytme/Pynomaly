...
class TestCompleteAnomalyDetectionWorkflow:
    ...
    async def test_basic_anomaly_detection_pipeline(self):
        ...
        async def create_detector():
            ...
            try:
                ...
                detector = adapter.create_algorithm("IsolationForest", {
                    "contamination": 0.1,
                    "n_estimators": 50,
                    "random_state": 42
                })
        ...
        async def detect_anomalies():
            ...
            assert len(predictions) == len(data), "Should have prediction for each sample"
        ...
        async def evaluate_results():
            ...
            assert detected_anomalies < len(predictions), "Should not classify all as anomalies"
            ...
            return {
                ...
                'true': true_anomalies,
                'precision_estimate': min(detected_anomalies, true_anomalies) / max(detected_anomalies, 1)
            }
        
        # Build and run integration test
        suite = (IntegrationTestBuilder("basic_anomaly_detection_pipeline", "Complete end-to-end workflow")
                .add_step("load_data", "Load test dataset", load_data, expected_result=500)
                .add_step("validate_data", "Validate data quality", validate_data, 
                         expected_result=True, dependencies=["load_data"])
                .add_step("create_detector", "Create anomaly detector", create_detector,
                         expected_result=True, dependencies=["validate_data"])
                .add_step("train_detector", "Train detector on data", train_detector,
                         expected_result=True, dependencies=["create_detector"])
                .add_step("detect_anomalies", "Detect anomalies", detect_anomalies,
                         dependencies=["train_detector"])
                .add_step("evaluate_results", "Evaluate detection results", evaluate_results,
        ...
        assert result_suite.failed_steps == 0, f"Pipeline failed: {result_suite.failed_steps} failures"
        assert result_suite.error_steps == 0, f"Pipeline errors: {result_suite.error_steps} errors"
    ...
    async def test_streaming_anomaly_detection_workflow(self):
        ...
        async def validate_streaming_results():
            ...
            assert len(scores) == processed, "Should have score for each processed sample"
            assert len(predictions) == processed, "Should have prediction for each processed sample"
        ...
        suite = (IntegrationTestBuilder("streaming_anomaly_detection", "Streaming detection workflow")
                .add_step("setup_detector", "Setup streaming detector", setup_streaming_detector,
                         expected_result=True)
                .add_step("initialize", "Initialize with baseline data", initialize_detector,
                         expected_result=True, dependencies=["setup_detector"])
                .add_step("process_batches", "Process streaming batches", process_streaming_batches,
                         dependencies=["initialize"])
                .add_step("validate_results", "Validate streaming results", validate_streaming_results,
    ...
    async def test_multi_algorithm_comparison_workflow(self):
        ...
        async def run_algorithm_comparison():
            ...
            try:
                ...
                for algorithm in algorithms:
                    ...
                    detector = adapter.create_algorithm(algorithm, {
                        "contamination": 0.15,
                        "random_state": 42,
        ...
        suite = (IntegrationTestBuilder("multi_algorithm_comparison", "Compare multiple algorithms")
                .add_step("prepare_data", "Prepare comparison data", prepare_comparison_data,
                         expected_result=300)
                .add_step("run_comparison", "Run algorithm comparison", run_algorithm_comparison,
                         dependencies=["prepare_data"])
                .add_step("analyze_results", "Analyze comparison results", analyze_comparison_results,
...
class TestDataPipelineIntegration:
    ...
    async def test_data_preprocessing_pipeline(self):
        ...
        mixed_data = pd.DataFrame({
            ...
            'text_feature': ['hello', 'world', 'test', '', 'data', 'science'],  # Has empty string
        ...
        async def validate_cleaned_data():
            ...
            assert len(cleaned_data) == len(original_data), "Row count should be preserved"
            
            # Should have no missing values
            assert not cleaned_data.isnull().any().any(), "Should have no missing values"
            
            # Should have fewer columns (constant removed)
            assert len(cleaned_data.columns) <= len(original_data.columns), "Should not add columns"
        ...
        async def prepare_for_modeling():
            ...
            # Basic normalization
            normalized_data = (numeric_features - numeric_features.mean()) / numeric_features.std()
        ...
        suite = (IntegrationTestBuilder("data_preprocessing_pipeline", "Complete data preprocessing workflow")
                .add_step("load_data", "Load raw data", load_raw_data, expected_result=6)
                .add_step("detect_issues", "Detect data quality issues", detect_data_quality_issues,
                         dependencies=["load_data"])
                .add_step("clean_data", "Clean the data", clean_data,
                         dependencies=["detect_issues"])
                .add_step("validate_cleaned", "Validate cleaned data", validate_cleaned_data,
                         expected_result=True, dependencies=["clean_data"])
                .add_step("prepare_modeling", "Prepare for modeling", prepare_for_modeling,
