...
class TestPyGODAdapterValidation:
    ...
    def test_graph_data_preparation_edge_list(self, mock_data, mock_torch, sample_graph_dataset):
    ...
    @patch('pynomaly.infrastructure.adapters.pygod_adapter.torch')
    @patch('pynomaly.infrastructure.adapters.pygod_adapter.Data')
    def test_fit_and_predict_workflow(self, mock_data, mock_torch, sample_graph_dataset):
...
class TestPyGODAdapterRealDependencies:
    ...
    @pytest.mark.skipif(
        not _pygod_available(),
        reason="PyGOD dependencies not installed"
    )
