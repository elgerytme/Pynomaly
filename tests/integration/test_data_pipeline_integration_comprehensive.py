...
class TestDataPreprocessingPipelineIntegration:
    ...
    def test_feature_selection_pipeline(self, raw_dataset):
        ...
        if len(high_variance_cols) >= 2:
            ...
            selected_features_dataset = Dataset(name="Selected Features", data=selected_data)
        ...
        try:
            ...
            if len(results) == 2:
                ...
                selected_features_variance = results["Selected Features"]["score_variance"]
...
class TestCompleteDataPipelineIntegration:
    ...
    def test_streaming_data_pipeline_simulation(self):
        ...
        try:
            ...
            if len(later_batches) > 0:
                avg_later_contamination = np.mean([b["contamination_rate"] for b in later_batches])
                assert avg_later_contamination > cumulative_results[0]["contamination_rate"]
    ...
    def test_pipeline_error_recovery_integration(self):
        ...
        datasets.append(("different", Dataset(name="Different", data=different_structure)))
        ...
        try:
            ...
            assert missing_result["status"] in ["success_with_preprocessing", "expected_failure"]
            
            # Different structure should either fail gracefully or handle unexpectedly
            different_result = next(r for r in pipeline_results if r["type"] == "different")
            assert different_result["status"] in ["expected_failure", "unexpected_success"]
