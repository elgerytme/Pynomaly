...
def production_dataset():
    ...
    # Create main metric
    main_metric = (
        100 + trend + daily_seasonal + weekly_seasonal + yearly_seasonal + noise
    )
    ...
    main_metric[anomaly_indices] += np.random.choice(
        [-1, 1], len(anomaly_indices)
    ) * np.random.uniform(20, 50, len(anomaly_indices))
    ...
    data = pd.DataFrame(
        {
            "timestamp": dates,
            "cpu_usage": np.clip(
                main_metric * 0.8 + np.random.normal(0, 5, n_samples), 0, 100
            ),
            "memory_usage": np.clip(
                main_metric * 0.7 + np.random.normal(0, 8, n_samples), 0, 100
            ),
            "network_io": np.maximum(
                0, main_metric * 0.5 + np.random.normal(0, 10, n_samples)
            ),
            "disk_io": np.maximum(
                0, main_metric * 0.6 + np.random.normal(0, 7, n_samples)
            ),
            "response_time": np.maximum(
                0, main_metric * 0.02 + np.random.normal(0, 0.5, n_samples)
            ),
            "error_rate": np.clip(np.random.exponential(0.5, n_samples), 0, 10),
            "service_id": np.random.choice(
                ["web-1", "web-2", "api-1", "api-2", "db-1"], n_samples
            ),
            "datacenter": np.random.choice(
                ["us-east-1", "us-west-2", "eu-west-1"], n_samples
            ),
            "environment": np.random.choice(
                ["production", "staging"], n_samples, p=[0.8, 0.2]
            ),
        }
    )
...
class TestEndToEndDataPipeline:
    ...
    async def test_complete_data_pipeline(self, production_dataset, temp_system_dir):
        ...
        engineered_dataset = await feature_engineer.engineer_features(
            production_dataset, feature_config
        )

        # Verify feature engineering
        assert len(engineered_dataset.data.columns) > len(
            production_dataset.data.columns
        )
        ...
        model_metadata = await model_registry.register_model(
            model, name="prod_detector_v1", version=SemanticVersion("1.0.0")
        )
        ...
        analytics_report = await analytics_engine.analyze_dataset(
            engineered_dataset, [validation_result]
        )
        ...
        feature_report = await feature_engineer.generate_feature_report(
            engineered_dataset, production_dataset
        )

        assert feature_report["original_features"] == len(
            production_dataset.data.columns
        )
        assert (
            feature_report["engineered_features"] >= feature_report["original_features"]
        )
        ...
        pipeline_results = {
            ...
            "validation": {
                ...
                "detection_rate": len(validation_result.anomalies)
...
class TestEndToEndRealTimeSystem:
    ...
    async def test_real_time_anomaly_detection_system(
        self, production_dataset, temp_system_dir
    ):
        ...
        pipeline_created = await stream_processor.create_pipeline(
            "production_anomaly_pipeline", detector, streaming_config
        )
        assert pipeline_created

        # Start pipeline
        pipeline_started = await stream_processor.start_pipeline(
        ...
        for idx, row in real_time_data.iterrows():
            ...
            success = await stream_processor.submit_data(
                "production_anomaly_pipeline", data_point, source="production_system"
            )
        ...
        production_pipeline_stats = pipeline_status["pipelines"][
            "production_anomaly_pipeline"
        ]
        assert production_pipeline_stats["is_running"]
        assert (
            production_pipeline_stats["buffer_stats"]["total_received"]
            >= successful_submissions * 0.8
        )
        ...
        real_time_analytics = await analytics_engine.analyze_dataset(
            Dataset(
                name="real_time_data", data=real_time_data, description="Real-time data"
            ),
            [batch_result],
        )

        # Step 7: Stop streaming pipeline
        pipeline_stopped = await stream_processor.stop_pipeline(
            "production_anomaly_pipeline"
        )
        ...
        system_performance = {
            "streaming": {
                ...
                "data_processed": production_pipeline_stats["buffer_stats"][
                    "total_processed"
                ],
                "detections_performed": production_pipeline_stats["detector_stats"][
                    "total_detections"
                ],
                "processing_rate": successful_submissions
                / 3.0,  # submissions per second
            },
            "api_gateway": {
            ...
            "analytics": {
                "real_time_analysis": len(real_time_analytics["analyses_performed"])
                > 0,
        ...
        assert (
            system_performance["streaming"]["processing_rate"] > 10
        )  # >10 samples/second
...
class TestEndToEndModelLifecycle:
    ...
    async def test_complete_model_lifecycle(self, production_dataset, temp_system_dir):
        ...
        v1_metadata = await model_registry.register_model(
            model_v1, name="system_monitor", version=SemanticVersion("1.0.0")
        )
        ...
        if len(v1_results.anomalies) > 0:
            await model_registry.update_model_metadata(
                "system_monitor", SemanticVersion("1.0.0"), status=ModelStatus.VALIDATED
            )
        ...
        v1_deployment_id = await deployment_service.deploy_model(
            "system_monitor", SemanticVersion("1.0.0"), deployment_config
        )
        ...
        monitor_id = await monitoring_service.start_monitoring(
            "system_monitor", SemanticVersion("1.0.0"), validation_dataset
        )
        ...
        v2_metadata = await model_registry.register_model(
            model_v2, name="system_monitor", version=SemanticVersion("1.1.0")
        )
        ...
        v1_avg_score = (
            np.mean([a.score.value for a in v1_results.anomalies])
            if v1_results.anomalies
            else 0
        )
        v2_avg_score = (
            np.mean([a.score.value for a in v2_results.anomalies])
            if v2_results.anomalies
            else 0
        )
        ...
        if v2_avg_score >= v1_avg_score:
            await model_registry.update_model_metadata(
                "system_monitor", SemanticVersion("1.1.0"), status=ModelStatus.VALIDATED
            )
            ...
            v2_deployment_id = await deployment_service.deploy_model(
                "system_monitor", SemanticVersion("1.1.0"), prod_config
            )

            # Start monitoring v2
            v2_monitor_id = await monitoring_service.start_monitoring(
                "system_monitor", SemanticVersion("1.1.0"), validation_dataset
            )
        ...
        version_comparison = await versioning_service.compare_versions(
            "system_monitor", SemanticVersion("1.0.0"), SemanticVersion("1.1.0")
        )
        ...
        v1_analytics = await analytics_engine.analyze_dataset(
            validation_dataset, [v1_results]
        )
        v2_analytics = await analytics_engine.analyze_dataset(
            validation_dataset, [v2_results]
        )
        ...
        lifecycle_report = {
            "model_versions": {
                ...
                "v1.1.0": {
                    ...
                    "deployment_id": v2_deployment_id
                    if v2_avg_score >= v1_avg_score
                    else None,
            ...
            "monitoring": {
                ...
                "v2_monitor_id": v2_monitor_id
                if v2_avg_score >= v1_avg_score
                else None,
            },
            "performance_improvement": {
                "score_improvement": v2_avg_score - v1_avg_score,
                "detection_rate_change": (v2_anomaly_count - v1_anomaly_count)
                / max(v1_anomaly_count, 1),
            },
        }
        ...
        assert (
            lifecycle_report["version_comparison"]["parameter_changes"][
                "contamination"
            ]["old"]
            != lifecycle_report["version_comparison"]["parameter_changes"][
                "contamination"
...
class TestEndToEndScalabilityAndPerformance:
    ...
    async def test_system_under_load(self, production_dataset):
        ...
        for i, detector in enumerate(detectors):
            ...
            created = await stream_processor.create_pipeline(
                pipeline_id, detector, config
            )
            assert created
        ...
        async def stream_data_to_pipeline(pipeline_id: str, data_chunk: pd.DataFrame):
            ...
            for idx, row in data_chunk.iterrows():
                ...
                success = await stream_processor.submit_data(
                    pipeline_id, data_point, source=f"load_test_{pipeline_id}"
                )
            ...
            return {
                ...
                "throughput": successful_submissions / processing_time
        ...
        streaming_tasks = [
            ...
            for pipeline_id, chunk in zip(pipeline_ids, data_chunks, strict=False)
        ]
        ...
        async def make_api_request(request_id: int):
            ...
            request = APIRequest(
                ...
                headers={"Content-Type": "application/json"},
                query_params={},
                body={"data": sample_data.to_dict("records"), "algorithm": "load_test"},
                timestamp=datetime.now(),
        ...
        average_api_response_time = np.mean(
            [result["response_time"] for result in api_results]
        )
        ...
        load_test_results = {
            ...
            "system_performance": {
                ...
                "overall_processing_time": max(
                    concurrent_processing_time, api_processing_time
                ),
                "system_utilization": final_status["active_pipelines"]
                / final_status["total_pipelines"],
            },
        }

        # Performance assertions
        assert (
            load_test_results["streaming_performance"]["total_throughput"] > 50
        )  # >50 samples/second total
        assert (
            load_test_results["api_performance"]["success_rate"] > 0.95
        )  # >95% success rate
        assert (
            load_test_results["api_performance"]["average_response_time"] < 2.0
        )  # <2 seconds avg
        assert (
            load_test_results["api_performance"]["requests_per_second"] > 10
        )  # >10 requests/second
...
class TestEndToEndErrorRecovery:
    ...
    async def test_system_error_recovery(self, production_dataset):
        ...
        created = await stream_processor.create_pipeline(
            "error_recovery_test", detector, config
        )
        assert created
        ...
        for idx, row in test_data.iterrows():
            ...
            success = await stream_processor.submit_data(
                "error_recovery_test", data_point, source="error_recovery_test"
            )
        ...
        try:
            ...
            await model_registry.register_model(
                invalid_model, name="invalid_model", version=SemanticVersion("1.0.0")
            )
        ...
        nonexistent_model = await model_registry.get_model(
            "nonexistent", SemanticVersion("1.0.0")
        )
        ...
        for idx, row in normal_data.iterrows():
            success = await stream_processor.submit_data(
                "error_recovery_test", row.to_dict(), source="recovery_test"
            )
            if success:
        ...
        error_recovery_results = {
            "streaming_recovery": {
                ...
                "error_handling_effective": detector.failure_count
                > detector.max_failures,
            },
            "api_error_handling": {
                "total_error_scenarios": len(error_test_requests),
                "gracefully_handled": sum(
                    1 for r in api_error_responses if r["handled_gracefully"]
                ),
            ...
            "analytics_recovery": {"handled_empty_dataset": analytics_handled_empty},
        }

        # Verify error recovery
        assert error_recovery_results["streaming_recovery"]["pipeline_survived_errors"]
        assert error_recovery_results["streaming_recovery"][
            "normal_operations_after_errors"
        ]
        assert error_recovery_results["api_error_handling"]["gracefully_handled"] >= 2
        assert error_recovery_results["api_error_handling"][
            "normal_operation_after_errors"
        ]
        assert error_recovery_results["model_registry_recovery"][
            "handled_invalid_input"
        ]
        assert error_recovery_results["model_registry_recovery"][
            "handled_nonexistent_queries"
        ]
