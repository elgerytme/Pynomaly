...
from src.pynomaly.infrastructure.data_processing import (
    MemoryOptimizedDataLoader, DataValidator, StreamingDataProcessor
)
from src.pynomaly.infrastructure.monitoring import (
    ProductionMonitor, PerformanceMonitor, get_monitor, init_monitor
)
from src.pynomaly.application.services import DetectionService
from src.pynomaly.infrastructure.repositories import InMemoryDetectorRepository, InMemoryDatasetRepository
...
class TestComprehensiveIntegration:
    ...
    def setup_method(self):
        ...
        # Add some anomalies
        self.test_data.iloc[950:, :] = self.test_data.iloc[950:, :] * 5  # Make last 50 rows anomalous
    ...
    def test_end_to_end_detection_workflow(self):
        ...
        with monitor.monitor_operation("e2e_detection_test", "integration_test"):
            ...
            assert anomalies_in_region > 0, "Should detect some anomalies in the anomalous region"
            ...
            class MockDataProcessor:
                ...
                def finalize(self):
                    return {
                        ...
                        'all_valid': all(chunk.metadata.get('validation_result', False) 
    ...
    def test_monitoring_integration(self):
        ...
        with monitor.monitor_operation("outer_operation", "integration_test") as outer_request_id:
            
            # Simulate data loading with monitoring
            with monitor.monitor_operation("data_loading", "data_processor") as load_request_id:
                dataset = Dataset(id="test", name="Test", data=self.test_data)
                assert dataset.data is not None
            
            # Simulate validation with monitoring
            with monitor.monitor_operation("validation", "data_validator") as val_request_id:
                validator = DataValidator()
    ...
    def test_file_processing_integration(self):
        ...
        try:
            ...
            with monitor.monitor_operation("file_processing_test", "integration_test"):
                # 1. File validation
                from src.pynomaly.infrastructure.data_processing import validate_file_format
    ...
    def test_algorithm_adapter_integration(self):
        ...
        for algorithm_name in algorithms_to_test:
            with monitor.monitor_operation(f"test_{algorithm_name}", "algorithm_integration"):
                try:
                    # Create adapter
                    ...
                    assert set(scores) <= {-1, 1}, f"Scores should be -1 or 1 for {algorithm_name}"
        ...
        log_entries = monitor.log_entries
        tested_algorithms = [entry.operation for entry in log_entries 
                           if entry.operation.startswith("test_")]
        
        assert len(tested_algorithms) >= len(algorithms_to_test) * 2  # Start and end logs
    
    def test_production_readiness_checklist(self):
        """Test production readiness checklist."""
        monitor = get_monitor()
        
        with monitor.monitor_operation("production_readiness_check", "integration_test"):
            checklist_results = {}
            ...
            try:
                ...
                error_id = monitor.report_error(test_error, "production_check", "error_handling")
            ...
            assert readiness_score >= 80.0, f"Production readiness score too low: {readiness_score}%"
...
async def test_async_integration():
    ...
    async with monitor.monitor_async_operation("async_integration_test", "integration_test"):
