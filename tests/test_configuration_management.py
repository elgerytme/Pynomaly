...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationCaptureRequestDTO, ConfigurationExportRequestDTO,
    ConfigurationSearchRequestDTO, ConfigurationSource, ConfigurationStatus, ExportFormat,
    DatasetConfigDTO, AlgorithmConfigDTO, EvaluationConfigDTO, ConfigurationMetadataDTO,
...
class TestConfigurationCaptureService:
    ...
    async def test_get_configuration_statistics(self, capture_service):
        ...
        for i in range(5):
            request = ConfigurationCaptureRequestDTO(
                source=ConfigurationSource.AUTOML if i % 2 == 0 else ConfigurationSource.CLI,
                raw_parameters={
                    "algorithm": "isolation_forest" if i < 3 else "local_outlier_factor",
                    "contamination": 0.1,
                    "dataset_path": f"/test/data_{i}.csv"
                },
                auto_save=True
    ...
    async def test_export_import_configurations(self, repository):
        ...
        with tempfile.TemporaryDirectory() as temp_dir2:
            ...
            for original_config in original_configs:
                imported_config = await new_repository.load_configuration(original_config.id)
...
class TestConfigurationIntegration:
    ...
    async def test_search_and_export_workflow(self, integrated_service):
        ...
        assert search_results[0].performance_results.accuracy >= search_results[1].performance_results.accuracy
