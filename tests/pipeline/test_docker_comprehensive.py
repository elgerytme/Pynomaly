...
"""
            ...
            def build_image(self, dockerfile_content: str, image_name: str, 
                ...
                for line in dockerfile_lines:
                    ...
                    if line and not line.startswith('#'):
                        if any(line.startswith(cmd) for cmd in ['FROM', 'RUN', 'COPY', 'ADD']):
                            layer_id = f"sha256:{str(uuid.uuid4()).replace('-', '')}"
                            layer_size = 10 * 1024 * 1024  # 10MB mock layer size
                            
                            build_result["layers"].append({
                                "layer_id": layer_id,
                                "instruction": line[:50] + "..." if len(line) > 50 else line,
                                "size": layer_size
                            })
                            layer_count += 1
                
                # Simulate potential build issues
                if "apt-get update" in dockerfile_content and "--no-install-recommends" not in dockerfile_content:
                    build_result["warnings"].append("Consider using --no-install-recommends to reduce image size")
                
                if "pip install" in dockerfile_content and "--no-cache-dir" not in dockerfile_content:
                    build_result["warnings"].append("Consider using --no-cache-dir to reduce image size")
                
                if "USER" not in dockerfile_content and dockerfile_content.count("FROM") == 1:
                    build_result["warnings"].append("Running as root user may pose security risks")
                
                # Calculate total image size
                build_result["image_size"] = sum(layer["size"] for layer in build_result["layers"])
            ...
            def scan_image_security(self, image_id: str, scanner: str = "trivy") -> Dict[str, Any]:
            ...
            def _generate_mock_vulnerabilities(self, scanner: str) -> List[Dict[str, Any]]:
            ...
            def run_container(self, image_id: str, command: List[str] = None, 
                            environment: Dict[str, str] = None,
                            ports: Dict[str, str] = None) -> Dict[str, Any]:
                ...
                if command[0] == "python":
                    if "--help" in command:
                        container_result["logs"].append("Pynomaly - Anomaly Detection Library")
                        container_result["logs"].append("Usage: python -m pynomaly [OPTIONS]")
                        ...
                        container_result["logs"].append("245 passed, 3 failed, 2 skipped")
                        container_result["exit_code"] = 1 if "failed" in " ".join(command) else 0
                        container_result["status"] = "exited"
                    else:
            ...
            def push_image_to_registry(self, image_id: str, registry_url: str, 
                ...
                push_result = {
                    ...
                    "compressed_size": image["image_size"] * 0.7,  # Assume 30% compression
            ...
            def validate_docker_best_practices(self, dockerfile_content: str) -> Dict[str, Any]:
            ...
            def optimize_image_size(self, dockerfile_content: str) -> Dict[str, Any]:
                ...
                if dockerfile_content.count("FROM") == 1 and "gcc" in dockerfile_content:
                    optimizations.append({
                        "type": "multi_stage",
                ...
                if "python:3.11" in dockerfile_content and "slim" not in dockerfile_content:
                    optimizations.append({
                        "type": "base_image",
                ...
                return {
                    ...
                    "estimated_total_savings": sum(
                        ...
                        for opt in optimizations if "MB" in opt["potential_savings"]
                    )
                }
    ...
    def test_container_execution(self, mock_docker_environment):
        ...
        api_run_result = docker_env.run_container(
            image_id=image_id,
            command=["python", "-m", "uvicorn", "pynomaly.api:app", "--host", "0.0.0.0"],
            ports={"8000": "8000"}
        )
    ...
    def test_security_scanning(self, mock_docker_environment):
        ...
        dockerfile_content = docker_env.create_dockerfile("production")
        build_result = docker_env.build_image(dockerfile_content, "pynomaly:security-test")
        image_id = build_result["image_id"]
        ...
        for scanner in scanners:
            ...
            individual_total = summary["critical"] + summary["high"] + summary["medium"] + summary["low"]
    ...
    def test_image_registry_operations(self, mock_docker_environment):
        ...
        dockerfile_content = docker_env.create_dockerfile("production")
        build_result = docker_env.build_image(dockerfile_content, "pynomaly:registry-test")
        image_id = build_result["image_id"]
        ...
        assert push_result["compressed_size"] < build_result["image_size"]  # Should be compressed
    ...
    def test_dockerfile_best_practices_validation(self, mock_docker_environment):
        ...
        production_validation = docker_env.validate_docker_best_practices(production_dockerfile)
        ...
        high_severity_violations = [
            v for v in production_validation["violations"] 
            if v["severity"] == "HIGH"
        ]
...
"""
        ...
        assert len(bad_validation["violations"]) > len(production_validation["violations"])
        ...
        user_violations = [
            v for v in bad_validation["violations"] 
            if v["rule"] == "USER_REQUIRED"
        ]
...
""",
        ...
        partially_optimized_count = len(optimization_results["partially_optimized"]["optimizations"])
        well_optimized_count = len(optimization_results["well_optimized"]["optimizations"])
        
        assert unoptimized_count >= partially_optimized_count
        assert partially_optimized_count >= well_optimized_count
        
        # Check specific optimizations
        unoptimized_types = [opt["type"] for opt in optimization_results["unoptimized"]["optimizations"]]
    ...
    def test_container_health_checks(self, mock_docker_environment):
        ...
        build_result = docker_env.build_image(dockerfile_with_healthcheck, "pynomaly:health-test")
        image_id = build_result["image_id"]
        ...
        if container_result["status"] == "running":
            assert container_result["health_status"] in ["starting", "healthy", "unhealthy"]
