...
class PerformanceMonitor:
    ...
    def measure(self):
        ...
        self.gc_start_collections = sum(gc.get_stats()[i]['collections'] for i in range(len(gc.get_stats())))
        ...
        try:
            ...
            gc_end_collections = sum(gc.get_stats()[i]['collections'] for i in range(len(gc.get_stats())))
            
            # Store results
            self.execution_time = end_time - self.start_time
            self.memory_usage_mb = end_memory - self.start_memory
            self.cpu_usage_percent = ((end_cpu_time.user - self.start_cpu_time.user) / 
                                    max(self.execution_time, 0.001)) * 100
...
class OptimizedDataGenerator:
    ...
    def generate_dataset(self, n_samples: int, n_features: int, 
                        contamination: float = 0.1, random_state: int = 42) -> np.ndarray:
...
class OptimizedAlgorithmRunner:
    ...
    def run_single_algorithm(self, algorithm_name: str, data: np.ndarray, 
                           parameters: Dict[str, Any] = None) -> Tuple[np.ndarray, np.ndarray, PerformanceMetrics]:
    ...
    def run_algorithm_scaling_test(self, algorithm_name: str, 
                                 data_sizes: List[int],
                                 n_features: int = 10) -> Dict[int, PerformanceMetrics]:
    ...
    def run_parallel_algorithms(self, algorithms: List[str], 
                              data: np.ndarray) -> Dict[str, Tuple[np.ndarray, np.ndarray, PerformanceMetrics]]:
        ...
        for algorithm in algorithms:
            try:
                predictions, scores, metrics = self.run_single_algorithm(algorithm, data)
                results[algorithm] = (predictions, scores, metrics)
            except Exception as e:
                # Store error information
                results[algorithm] = (None, None, PerformanceMetrics(
                    execution_time=0.0,
                    memory_usage_mb=0.0,
...
class TestOptimizedPerformance:
    ...
    def test_algorithm_performance_scaling(self, algorithm_runner, algorithm, data_size):
        ...
        # Run algorithm
        predictions, scores, metrics = algorithm_runner.run_single_algorithm(algorithm, data)
    ...
    def test_memory_efficiency(self, algorithm_runner):
        ...
        for size in data_sizes:
            ...
            _, _, metrics = algorithm_runner.run_single_algorithm("IsolationForest", data)
    ...
    def test_algorithm_comparison_benchmark(self, algorithm_runner):
        ...
        successful_algorithms = [alg for alg in algorithms if "error" not in benchmark_results[alg]]
        
        if len(successful_algorithms) >= 2:
            # Find fastest algorithm
            fastest = min(successful_algorithms, 
                         key=lambda alg: benchmark_results[alg]["execution_time"])
            
            # Find most memory efficient
            most_efficient = min(successful_algorithms,
            ...
            for algorithm in successful_algorithms:
                ...
                assert metrics["throughput_samples_per_second"] > 5  # Reasonable throughput
    ...
    def test_data_loading_performance(self, data_generator):
        ...
        assert cache_stats["cached_datasets"] > 0 or cache_stats["hits"] == 0  # Either cached or no repeats
    ...
    def test_dimensionality_performance_impact(self, algorithm_runner, n_features):
        ...
        expected_max_time = 5.0 + (n_features * 0.5)  # Allow more time for higher dimensions
        assert metrics.execution_time < expected_max_time
        
        # Throughput should remain reasonable
        min_throughput = max(10, 100 - n_features)  # Lower expectation for high dimensions
...
class TestPerformanceRegression:
    ...
    def test_performance_regression_detection(self, baseline_results_file):
        ...
        predictions, scores, current_metrics = runner.run_single_algorithm("IsolationForest", data)
        
        current_results = {
            "isolation_forest_1000x10": current_metrics.to_dict()
        }
        ...
        if test_name in baseline:
            ...
            time_regression = (current_metrics_dict["execution_time"] / 
                             baseline_metrics["execution_time"])
            
            if time_regression > 1.5:
                pytest.fail(f"Performance regression detected: {time_regression:.2f}x slower than baseline")
            
            # Check for memory regression (>2x memory usage)
            memory_regression = (current_metrics_dict["peak_memory_mb"] / 
                               max(baseline_metrics["peak_memory_mb"], 1.0))
            
            if memory_regression > 2.0:
                pytest.fail(f"Memory regression detected: {memory_regression:.2f}x more memory than baseline")
