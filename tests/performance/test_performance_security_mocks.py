...
class MockThreatDetector:
    ...
    def analyze_input(self, input_data: str) -> float:
        ...
        if any(pattern in input_data.lower() for pattern in ["drop", "delete", "insert", "update"]):
            threat_level += 0.8
        
        if any(pattern in input_data.lower() for pattern in ["<script>", "javascript:", "onerror"]):
            threat_level += 0.7
...
class MockInputSanitizer:
    ...
    def validate_input(self, input_type: str, value: str) -> bool:
        ...
        if input_type == "email":
            ...
            return value.startswith(("http://", "https://")) and "javascript:" not in value
        return True
...
class MockJWTAuthService:
    ...
    def create_access_token(self, user: dict, ip_address: str = None, user_agent: str = None):
...
class MockDetector:
    ...
    def __init__(self, id: str, name: str, algorithm: str, parameters: dict, trained: bool = False):
        self.id = id
        self.name = name
...
def patch_imports():
    ...
    # Create mock modules
    mock_threat_detection = ModuleType("pynomaly.infrastructure.security.threat_detection")
    mock_threat_detection.ThreatDetector = MockThreatDetector
    
    mock_input_validation = ModuleType("pynomaly.infrastructure.security.input_validation")
    ...
    mock_performance_metrics = ModuleType("pynomaly.domain.value_objects.performance_metrics")
    mock_performance_metrics.PerformanceMetrics = MockPerformanceMetrics
    
    # Add to sys.modules
    sys.modules["pynomaly.infrastructure.security.threat_detection"] = mock_threat_detection
    sys.modules["pynomaly.infrastructure.security.input_validation"] = mock_input_validation
    ...
    sys.modules["pynomaly.domain.value_objects.performance_metrics"] = mock_performance_metrics
