...
class PerformanceMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if failed_metrics:
            summary['error_messages'] = [m.error_message for m in failed_metrics if m.error_message]
...
def generate_test_dataset(n_samples: int, n_features: int, with_anomalies: bool = False) -> pd.DataFrame:
...
class TestAPILoadTesting:
    ...
    def test_concurrent_dataset_uploads(self, api_client: TestClient, performance_monitor: PerformanceMonitor):
        ...
        dataset_ids = [data["id"] for status_code, data in results if data and status_code == 201]
        ...
        print(f"Concurrent uploads: {successful_uploads}/{len(datasets)} successful in {total_time:.2f}s")
    ...
    def test_detector_creation_load(self, api_client: TestClient, performance_monitor: PerformanceMonitor):
        ...
        def create_detector(config):
            with performance_monitor.measure_operation(f"create_detector_{config['name']}"):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_detector, config) for config in detector_configs]
            results = [future.result() for future in as_completed(futures)]
        
        total_time = time.time() - start_time
        
        # Analyze results
        successful_creations = sum(1 for status_code, _ in results if status_code == 201)
        detector_ids = [data["id"] for status_code, data in results if data and status_code == 201]
        ...
        print(f"Detector creation: {successful_creations}/{len(detector_configs)} successful in {total_time:.2f}s")
    ...
    async def test_high_throughput_health_checks(self, async_api_client: AsyncClient):
        ...
        response_times = [result['duration_ms'] for result in results if result['success']]
        ...
        print(f"Health check throughput: {successful_requests}/{num_requests} successful")
        print(f"RPS: {requests_per_second:.1f}, Avg: {avg_response_time:.1f}ms, P95: {p95_response_time:.1f}ms")
    
    def test_mixed_workload_simulation(self, api_client: TestClient, performance_monitor: PerformanceMonitor):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(execute_operations, chunk) for chunk in operation_chunks]
            all_results = []
        ...
        print(f"Mixed workload: {successful_operations}/{total_operations} successful in {total_time:.2f}s")
...
class TestMemoryAndResourceTesting:
    ...
    def test_memory_leak_detection(self, api_client: TestClient):
        ...
        try:
            for iteration in range(50):
                ...
                upload_response = api_client.post(
                    "/api/datasets/",
                    files={"file": (f"leak_test_{iteration}.csv", csv_data.encode(), "text/csv")},
                ...
                if upload_response.status_code == 201:
                    ...
                    detector_response = api_client.post(
                        ...
                        json={
                            ...
                            "algorithm": "IsolationForest",
                            "hyperparameters": {"contamination": 0.1, "random_state": iteration}
                        }
                    )
    ...
    def test_concurrent_resource_exhaustion(self, api_client: TestClient):
        ...
        def resource_intensive_operation(thread_id: int):
            try:
                ...
                upload_response = api_client.post(
                    "/api/datasets/",
                    files={"file": (f"intensive_{thread_id}.csv", csv_data.encode(), "text/csv")},
                    data={"name": f"Resource Intensive Dataset {thread_id}"}
                )
                
                if upload_response.status_code != 201:
                    return {"success": False, "error": "Upload failed", "thread_id": thread_id}
                ...
                if detector_response.status_code != 201:
                    api_client.delete(f"/api/datasets/{dataset_id}")
                    return {"success": False, "error": "Detector creation failed", "thread_id": thread_id}
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            results = [future.result(timeout=120) for future in as_completed(futures, timeout=120)]
        ...
        if successful_operations > 0:
            ...
            avg_train_time = statistics.mean([r["train_time"] for r in successful_results])
            max_train_time = max([r["train_time"] for r in successful_results])
        else:
            avg_train_time = max_train_time = 0
        
        print(f"Resource exhaustion test: {successful_operations}/{num_threads} successful")
    ...
    def test_garbage_collection_effectiveness(self, api_client: TestClient):
        ...
        print(f"Memory: Initial: {initial_memory:.1f}MB, After creation: {memory_after_creation:.1f}MB")
        print(f"Memory: After cleanup: {memory_after_cleanup:.1f}MB")
        print(f"Memory increase: {memory_increase:.1f}MB, Recovered: {memory_recovered:.1f}MB")
        print(f"Recovery rate: {recovery_rate:.1%}")
        
        # Assertions
        assert memory_increase < 500  # Creating resources shouldn't use excessive memory
        assert recovery_rate > 0.5    # Should recover at least 50% of memory
        assert memory_after_cleanup - initial_memory < 100  # Final increase should be minimal
...
class TestStressAndReliability:
    ...
    def test_sustained_load_stability(self, api_client: TestClient):
        ...
        upload_response = api_client.post(
            "/api/datasets/",
            files={"file": ("sustained_load_dataset.csv", csv_data.encode(), "text/csv")},
        ...
        print(f"Avg response: {avg_response_time:.1f}ms, P95: {p95_response_time:.1f}ms")
    ...
    def test_rapid_scaling_scenario(self, api_client: TestClient):
        ...
        for phase_name, duration, ops_per_second in phases:
            print(f"Starting phase: {phase_name} ({ops_per_second} ops/sec for {duration}s)")
            ...
            phase_success_rate = (phase_operations - phase_errors) / phase_operations if phase_operations > 0 else 0
            phase_avg_response = statistics.mean(phase_response_times) if phase_response_times else 0
            ...
            print(f"Phase {phase_name}: {phase_operations} ops, {phase_success_rate:.1%} success")
        ...
        for phase_name, results in phase_results.items():
            print(f"{phase_name}: {results['actual_ops_per_sec']:.1f} ops/sec, "
        ...
        assert peak_results["actual_ops_per_sec"] > ramp_up_results["actual_ops_per_sec"]
