...
def test_config_loading():
    ...
    if config:
        ...
        print(
            f"  - Found {len(config.get('allowed_root_files', []))} allowed root files"
        )
        print(
            f"  - Found {len(config.get('allowed_root_directories', []))} allowed root directories"
        )
    else:
            ...
            for file_name in test_files:
                ...
                if file_name in [
                    ...
                    if category != "artifacts_for_deletion":
                        print(
                            f"✗ {file_name} should be categorized as artifacts_for_deletion, got {category}"
                        )
                        return False
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for file_name in test_files:
                if file_name.endswith("/"):
                    file_path = temp_path / file_name.rstrip("/")
                    should_delete = should_delete_item(
                        file_path,
                        config.get("delete_patterns", []),
