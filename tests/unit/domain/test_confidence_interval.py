...
class TestConfidenceInterval:
    ...
    def test_confidence_interval_validation(self):
        ...
        with pytest.raises(InvalidValueError, match="Lower bound.*cannot be greater than upper bound"):
            ConfidenceInterval(lower=0.9, upper=0.1, method="test")
        
        # Test invalid confidence level
        with pytest.raises(InvalidValueError, match="Confidence level must be between 0 and 1"):
            ConfidenceInterval(lower=0.1, upper=0.9, confidence_level=1.5, method="test")
        
        with pytest.raises(InvalidValueError, match="Confidence level must be between 0 and 1"):
            ConfidenceInterval(lower=0.1, upper=0.9, confidence_level=-0.1, method="test")
        
        # Test empty method
        with pytest.raises(InvalidValueError, match="Method must be a non-empty string"):
    ...
    def test_intersection_method(self):
        ...
        ci1 = ConfidenceInterval(lower=10.0, upper=20.0, confidence_level=0.95, method="test1")
        ci2 = ConfidenceInterval(lower=15.0, upper=25.0, confidence_level=0.90, method="test2")
        ci3 = ConfidenceInterval(lower=25.0, upper=35.0, method="test3")
    ...
    def test_from_samples_empty_error(self):
        ...
        with pytest.raises(InvalidValueError, match="Cannot create confidence interval from empty samples"):
    ...
    def test_confidence_levels_boundary(self):
        ...
        # Test 0% confidence
        ci_0 = ConfidenceInterval(lower=10.0, upper=20.0, confidence_level=0.0, method="test")
        assert ci_0.confidence_level == 0.0
        
        # Test 100% confidence
        ci_100 = ConfidenceInterval(lower=10.0, upper=20.0, confidence_level=1.0, method="test")
