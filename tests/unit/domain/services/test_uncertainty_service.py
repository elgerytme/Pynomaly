...
class TestUncertaintyQuantificationService:
    ...
    def test_bootstrap_confidence_interval_mean(self, uncertainty_service, sample_scores):
    ...
    def test_bootstrap_confidence_interval_median(self, uncertainty_service, sample_scores):
    ...
    def test_bootstrap_confidence_interval_std(self, uncertainty_service, sample_scores):
    ...
    def test_bootstrap_invalid_statistic(self, uncertainty_service, sample_scores):
        ...
        with pytest.raises(ValueError, match="Unknown statistic function"):
            uncertainty_service.calculate_bootstrap_confidence_interval(
                scores=sample_scores,
                statistic_function="invalid_stat"
            )
    
    def test_bootstrap_empty_scores(self, uncertainty_service):
        """Test bootstrap with empty scores."""
        with pytest.raises(ValueError, match="Cannot calculate confidence interval from empty scores"):
    ...
    def test_prediction_uncertainty_bootstrap(self, uncertainty_service, sample_detection_results):
        ...
        assert isinstance(uncertainty_metrics["confidence_interval_mean"], ConfidenceInterval)
        
        # Check additional metrics
        assert "prediction_interval" in uncertainty_metrics
        assert "entropy" in uncertainty_metrics
    
    def test_prediction_uncertainty_normal(self, uncertainty_service, sample_detection_results):
        ...
        assert isinstance(uncertainty_metrics, dict)
        assert "confidence_interval_mean" in uncertainty_metrics
        assert "confidence_interval_std" not in uncertainty_metrics  # Not available for normal method
    
    def test_prediction_uncertainty_bayesian(self, uncertainty_service, sample_detection_results):
    ...
    def test_prediction_uncertainty_invalid_method(self, uncertainty_service, sample_detection_results):
    ...
    def test_prediction_uncertainty_empty_results(self, uncertainty_service):
        ...
        with pytest.raises(ValueError, match="Cannot calculate uncertainty from empty results"):
    ...
    def test_ensemble_uncertainty_empty_scores(self, uncertainty_service):
        ...
        with pytest.raises(ValueError, match="Cannot calculate ensemble uncertainty from empty scores"):
    ...
    def test_highest_density_interval(self, uncertainty_service):
        ...
        assert ci.width() <= percentile_ci.width() + 1e-6  # Small tolerance for numerical precision
