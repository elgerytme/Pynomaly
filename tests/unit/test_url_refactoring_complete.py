...
def test_template_directory():
    ...
    if total_web_refs == 0:
        ...
        print(f"❌ Found {total_web_refs} /web references in {files_with_refs} template files")
        return False
...
def test_configuration_files():
    ...
    for file_path_str in config_files:
        ...
        for line_num, line_content in web_refs:
            ...
            if line_content.strip().startswith('#') and ('comment' in line_content.lower() or 'note' in line_content.lower()):
                continue
            filtered_refs.append((line_num, line_content))

        if filtered_refs:
            print(f"⚠️  {file_path_str} contains {len(filtered_refs)} potentially problematic /web references:")
