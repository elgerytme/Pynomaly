...
class TestPipelineService:
    ...
    def test_default_templates_created(self, pipeline_service):
        ...
        template_types = [t.template_type for t in pipeline_service.pipeline_templates.values()]
        assert "python_package" in template_types
        assert "ml_model" in template_types

    async def test_create_pipeline_from_template(self, pipeline_service, sample_template):
        """Test creating pipeline from template."""
        # Add template to service
        pipeline_service.pipeline_templates[sample_template.template_id] = sample_template
    ...
    async def test_trigger_already_running_pipeline(self, pipeline_service, sample_template):
    ...
    async def test_execute_pipeline_success(self, mock_subprocess, pipeline_service, sample_template):
    ...
    @patch("pynomaly.infrastructure.cicd.pipeline_service.asyncio.create_subprocess_exec")
    async def test_execute_pipeline_failure(self, mock_subprocess, pipeline_service, sample_template):
    ...
    async def test_execute_stage_success(self, mock_subprocess, pipeline_service, sample_template):
    ...
    @patch("pynomaly.infrastructure.cicd.pipeline_service.asyncio.create_subprocess_shell")
    async def test_execute_stage_failure(self, mock_subprocess, pipeline_service, sample_template):
    ...
    async def test_get_pipeline_logs(self, pipeline_service, sample_template):
        ...
        stage_logs = await pipeline_service.get_pipeline_logs(pipeline.pipeline_id, "test")
    ...
    async def test_get_logs_nonexistent_stage(self, pipeline_service, sample_template):
        ...
        logs = await pipeline_service.get_pipeline_logs(pipeline.pipeline_id, "nonexistent")
        assert logs is None
