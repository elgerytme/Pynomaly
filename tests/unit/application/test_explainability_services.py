...
class TestExplainabilityService:
    ...
    async def test_shap_explanation_generation(
        self, explainability_service, sample_detector, sample_anomaly
    ):
    ...
    @pytest.mark.asyncio
    async def test_lime_explanation_generation(
        self, explainability_service, sample_detector, sample_anomaly
    ):
    ...
    @pytest.mark.asyncio
    async def test_cohort_explanation_analysis(
        self, explainability_service, sample_detector, sample_dataset
    ):
        ...
        assert (
            "temperature_spikes"
            in response.cohort_explanations["high_temperature"]["anomaly_patterns"]
        )

    @pytest.mark.asyncio
    async def test_explanation_method_comparison(
        self, explainability_service, sample_detector, sample_anomaly
    ):
        ...
        with patch.object(
            explainability_service, "_compare_explanations"
        ) as mock_compare:
            ...
            assert (
                "vibration" in response.disagreement_analysis["disagreement_features"]
            )

    @pytest.mark.asyncio
    async def test_temporal_explanation_analysis(
    ...
    async def test_model_agnostic_explanations(self, explainability_service):
        ...
        for model_type in model_types:
            ...
            assert (
                explainer_result["explainer_type"]
                in response.explanation_metadata["explainer_type"]
            )

    @pytest.mark.asyncio
    async def test_explanation_confidence_assessment(
        self, explainability_service, sample_detector, sample_anomaly
    ):
    ...
    async def test_explanation_caching(
        ...
        with patch.object(
            explainability_service, "_get_cached_explanation"
        ) as mock_cache:
...
class TestExplainabilityIntegration:
    ...
    async def test_end_to_end_explanation_workflow(self, explainability_system):
        ...
        importance_response = await service.analyze_feature_importance(
            importance_request
        )
        ...
        historical_explanations = [
            {
                ...
                "feature_importance": {"f1": 0.4, "f2": 0.3, "f3": 0.3},
            },
            {
                "timestamp": "2024-01-02",
                "feature_importance": {"f1": 0.42, "f2": 0.28, "f3": 0.3},
            },
            {
                "timestamp": "2024-01-03",
                "feature_importance": {"f1": 0.6, "f2": 0.2, "f3": 0.2},
            },  # Drift
        ]
    ...
    async def test_cross_model_explanation_comparison(self, explainability_system):
        ...
        cross_model_explanations = await service.compare_explanations_across_models(
            models=models, anomaly_data=anomaly_data, explanation_method="shap"
        )
    ...
    async def test_explanation_interactive_exploration(self, explainability_system):
        ...
        base_explanation = await service.explain_anomaly(
            ExplanationRequestDTO(
                detector_id=detector.id,
                anomaly_data={"f1": 2.5, "f2": -1.8, "f3": 0.9},
                explanation_method="shap",
