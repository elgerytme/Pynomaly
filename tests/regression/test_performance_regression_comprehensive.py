...
class TestTrainingPerformanceRegression:
        ...
        for dataset_name, dataset in performance_datasets.items():
            try:
                ...
                if dataset_name == "small":
                    assert training_time < 5.0, f"Small dataset training too slow: {training_time}s"
                elif dataset_name == "medium":
                    assert training_time < 30.0, f"Medium dataset training too slow: {training_time}s"
                elif dataset_name == "large":
                    assert training_time < 120.0, f"Large dataset training too slow: {training_time}s"
                elif dataset_name == "wide":
                    assert training_time < 60.0, f"Wide dataset training too slow: {training_time}s"
                
                # Throughput should be reasonable
                samples_per_second = dataset.n_samples / training_time
                assert samples_per_second > 100, f"Training throughput too low: {samples_per_second} samples/s"
                
            except ImportError:
                pytest.skip("scikit-learn not available")
        
        # Log performance results for monitoring
        print(f"Training performance results: {json.dumps(performance_results, indent=2)}")
    ...
    def test_local_outlier_factor_training_performance(self, performance_datasets):
        ...
        test_datasets = {k: v for k, v in performance_datasets.items() if k in ["small", "medium"]}
        ...
        for dataset_name, dataset in test_datasets.items():
            try:
                ...
                if dataset_name == "small":
                    assert training_time < 10.0, f"LOF small dataset training too slow: {training_time}s"
                elif dataset_name == "medium":
                    assert training_time < 60.0, f"LOF medium dataset training too slow: {training_time}s"
...
class TestInferencePerformanceRegression:
    ...
    def test_single_sample_inference_performance(self, trained_models, performance_datasets):
        ...
        for model_name, model in trained_models.items():
            ...
            assert inference_time < 1.0, f"{model_name} single sample inference too slow: {inference_time}s"
            assert len(scores) == 1
        ...
        for model_name, model in trained_models.items():
            for batch_size in batch_sizes:
                ...
                if batch_size <= len(performance_datasets['medium'].data):
                    ...
                    if batch_size == 100:
                        assert inference_time < 2.0, f"{model_name} batch 100 inference too slow: {inference_time}s"
                    elif batch_size == 1000:
                        assert inference_time < 10.0, f"{model_name} batch 1000 inference too slow: {inference_time}s"
                    elif batch_size == 5000:
                        assert inference_time < 30.0, f"{model_name} batch 5000 inference too slow: {inference_time}s"
                    
                    # Throughput should be reasonable
                    assert throughput > 50, f"{model_name} throughput too low: {throughput} samples/s"
...
class TestMemoryPerformanceRegression:
    ...
    def test_training_memory_usage(self, performance_datasets):
        ...
        for dataset_name, dataset in performance_datasets.items():
            ...
            try:
                ...
                if dataset_name == "small":
                    assert training_mb < 100, f"Small dataset training uses too much memory: {training_mb} MB"
                elif dataset_name == "medium":
                    assert training_mb < 500, f"Medium dataset training uses too much memory: {training_mb} MB"
                elif dataset_name == "wide":
                    assert training_mb < 200, f"Wide dataset training uses too much memory: {training_mb} MB"
                
                # Scoring should not significantly increase memory
                assert scoring_mb < 50, f"Scoring increases memory too much: {scoring_mb} MB"
    ...
    def test_concurrent_memory_usage(self):
        ...
        assert concurrent_mb < 300, f"Concurrent operations use too much memory: {concurrent_mb} MB"
...
class TestConcurrencyPerformanceRegression:
    ...
    def test_thread_safety_performance(self):
        ...
        try:
            ...
            assert slowdown_ratio < 3.0, f"Concurrent inference too slow: {slowdown_ratio}x slower"
    ...
    def test_multiprocessing_performance(self):
        ...
        assert parallel_time < sequential_time * 1.5, "Multiprocessing significantly slower than sequential"
...
class TestIOPerformanceRegression:
    ...
    def test_model_serialization_performance(self):
        ...
        try:
            ...
            # Performance thresholds
            assert serialization_time < 5.0, f"Model serialization too slow: {serialization_time}s"
            assert deserialization_time < 3.0, f"Model deserialization too slow: {deserialization_time}s"
            ...
            try:
                ...
                assert joblib_serialize_time < 5.0, f"Joblib serialization too slow: {joblib_serialize_time}s"
                assert joblib_deserialize_time < 3.0, f"Joblib deserialization too slow: {joblib_deserialize_time}s"
    ...
    def test_data_loading_performance(self):
        ...
        for size in data_sizes:
            ...
            if size <= 1000:
                assert csv_write_time < 2.0, f"CSV write too slow for {size} rows: {csv_write_time}s"
                assert csv_read_time < 1.0, f"CSV read too slow for {size} rows: {csv_read_time}s"
            elif size <= 10000:
                assert csv_write_time < 5.0, f"CSV write too slow for {size} rows: {csv_write_time}s"
                assert csv_read_time < 3.0, f"CSV read too slow for {size} rows: {csv_read_time}s"
            elif size <= 50000:
                assert csv_write_time < 15.0, f"CSV write too slow for {size} rows: {csv_write_time}s"
                assert csv_read_time < 10.0, f"CSV read too slow for {size} rows: {csv_read_time}s"
            ...
            try:
                ...
                if size >= 10000:
                    assert parquet_read_time <= csv_read_time, "Parquet not faster than CSV for large data"
