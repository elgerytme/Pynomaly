...
class TestAPIResponseFormatRegression:
    ...
    def test_error_response_format_consistency(self, client):
        ...
        assert len(present_fields) >= 1, "At least one standard error field should be present"
...
class TestAPIPerformanceRegression:
    ...
    def test_health_endpoint_response_time(self, client):
        ...
        assert max_response_time < 2.0, f"Maximum response time too slow: {max_response_time}s"
...
class TestAPISecurityRegression:
    ...
    def test_cors_configuration_stability(self, client):
        ...
        response = client.get("/api/v1/info", headers={"Origin": "http://localhost:3000"})
        ...
        for endpoint in deprecated_endpoints:
            ...
            if response.status_code == 200:
                ...
                deprecation_indicators = [
                    ...
                    "deprecated" in response.json().get("warnings", []) if response.headers.get("content-type", "").startswith("application/json") else False
                ]
                
                # At least one deprecation indicator should be present
                assert any(deprecation_indicators), f"No deprecation warning for {endpoint}"
    ...
    def test_concurrent_data_operations_consistency(self, client):
        ...
        def create_dataset(index):
            ...
            payload = {
                ...
                "data": {
                    "values": list(range(index, index + 5))
                }
            }
    ...
    def test_special_character_handling(self, client):
        ...
        if response.status_code == 201:
            ...
            if get_response.status_code == 200:
                ...
                assert "測試" in retrieved_data["name"] or "test" in retrieved_data["name"].lower()
                assert "café" in retrieved_data["description"] or "cafe" in retrieved_data["description"].lower()
