...
from pynomaly.domain.entities.deployment import (
    ...
    Environment, StrategyType, HealthMetrics, RollbackCriteria
)
from pynomaly.application.services.deployment_orchestration_service import (
    DeploymentOrchestrationService, DeploymentOrchestrationError,
    DeploymentNotFoundError, EnvironmentNotReadyError
)
...
class TestDeploymentOrchestrationService:
    ...
    async def test_deploy_model_success(self, deployment_service):
        ...
        deployment_files = list(deployment_service.storage_path.glob("deployment_*.json"))
    ...
    async def test_promote_to_production(self, deployment_service):
        ...
        production_deployment = await deployment_service.promote_to_production(
            ...
            approval_metadata={"approved_by": "manager", "notes": "Performance validated"},
            user="test-user"
        )
        
        assert production_deployment.environment == Environment.PRODUCTION
        assert production_deployment.model_version_id == staging_deployment.model_version_id
        assert "promoted_from" in production_deployment.metadata
        assert production_deployment.metadata["promoted_from"] == str(staging_deployment.id)
    ...
    async def test_get_environment_status(self, deployment_service):
        ...
        status = await deployment_service.get_environment_status(Environment.DEVELOPMENT)
...
class TestDeploymentIntegration:
    ...
    async def test_end_to_end_deployment_workflow(self, temp_storage_path):
        ...
        # Initialize services
        model_registry_service = ModelRegistryService(storage_path=temp_storage_path / "registry")
        ...
        await deployment_service.update_deployment_health(staging_deployment.id, good_metrics)
        ...
        bad_metrics = HealthMetrics(
            ...
            memory_usage=90.0,
            error_rate=15.0,
            response_time_p95=3000.0
        )
        
        await deployment_service.update_deployment_health(production_deployment.id, bad_metrics)
        
        # Check if automatic rollback was triggered
        updated_deployment = await deployment_service.get_deployment(production_deployment.id)
        ...
        prod_status = await deployment_service.get_environment_status(Environment.PRODUCTION)
        assert prod_status["has_active_deployment"]
        
        staging_status = await deployment_service.get_environment_status(Environment.STAGING)
