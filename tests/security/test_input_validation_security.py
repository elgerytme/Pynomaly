...
from pynomaly.infrastructure.validation import (
    InputValidator, DataSanitizer, SecurityValidator
)
...
class TestInputValidationSecurity:
        ...
        for payload in redos_payloads:
            for pattern in vulnerable_patterns:
                ...
                with pytest.raises((ValidationError, DomainValidationError, TimeoutError)):
                    validator.validate_regex_pattern(payload, pattern, timeout_seconds=1)
    ...
    def test_file_content_validation(self, validator):
        ...
        for content in malicious_contents:
            with pytest.raises((ValidationError, DomainValidationError)):
                validator.validate_file_content_security(content, allowed_types=["image/jpeg", "image/png"])
    ...
    def test_encoding_confusion_prevention(self, validator):
        ...
        encoding_variants = [
            "admin",  # ASCII
            "admin".encode('utf-8').decode('utf-8'),  # UTF-8
        ...
        canonical_forms = [validator.canonicalize_string(variant) for variant in encoding_variants]
    ...
    def test_json_sanitization_security(self, sanitizer):
        ...
        dangerous_keys = ["__proto__", "constructor", "toString", "valueOf", "eval", "Function", "process"]
    ...
    def test_computational_dos_prevention(self, validator):
        ...
        for expensive_input in expensive_inputs:
            with pytest.raises((ValidationError, DomainValidationError, TimeoutError)):
                validator.validate_with_resource_limits(expensive_input, max_size=10000, timeout_seconds=1)