...
class VulnerabilityScanner:
    ...
    def scan_for_information_disclosure(self, response: str) -> list[str]:
        ...
        if any(
            ...
            for error in ["mysql", "postgresql", "oracle", "sql server"]
        ):
...
class TestProductionSecurity:
    ...
    def test_sql_injection_protection(self, security_harness):
        ...
        for attack_pattern in security_harness.attack_patterns["sql_injection"]:
            ...
            assert (
                attack_pattern != sanitized
            ), f"SQL injection not sanitized: {attack_pattern}"

            # Test threat detection
            threat_level = security_harness.threat_detector.analyze_input(
                attack_pattern
            )
    ...
    def test_command_injection_protection(self, security_harness):
        ...
        for attack_pattern in security_harness.attack_patterns["command_injection"]:
            # Test input sanitization
            sanitized = security_harness.input_sanitizer.sanitize_command(
                attack_pattern
            )
            ...
            for char in dangerous_chars:
                ...
                ), f"Command injection char not removed: {char}"
            ...
            assert (
                threat_level > 0.8
            ), f"Command injection not detected: {attack_pattern}"
    ...
    def test_jwt_security_implementation(self, mock_auth_service):
        ...
        with pytest.raises(jwt.InvalidTokenError):
            jwt.decode(
                tampered_token, mock_auth_service.secret_key, algorithms=["HS256"]
            )
    ...
    def test_input_validation_security(self, security_harness):
        ...
        for input_type, value, expected_valid in test_cases:
            is_valid = security_harness.input_sanitizer.validate_input(
                input_type, value
            )
            assert (
                is_valid == expected_valid
            ), f"Validation failed for {input_type}: {value}"
    ...
    def test_vulnerability_scanning(self, security_harness):
        ...
        vulnerabilities = security_harness.vulnerability_scanner.scan_for_weak_crypto(
            vulnerable_code
        )
    ...
    def test_security_monitoring_and_alerting(self):
        ...
        monitor.log_security_event(
            "failed_login", {"user": "admin", "ip": "192.168.1.100"}
        )
        monitor.log_security_event(
            "sql_injection_attempt", {"query": "'; DROP TABLE users; --"}
        )
