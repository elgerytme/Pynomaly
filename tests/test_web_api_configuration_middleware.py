...
from pynomaly.application.dto.configuration_dto import (
    ConfigurationSource, RequestConfigurationDTO, ResponseConfigurationDTO,
    WebAPIContextDTO
)
...
from pynomaly.infrastructure.middleware.configuration_middleware import (
    ConfigurationCaptureMiddleware, ConfigurationAPIMiddleware
)
from pynomaly.presentation.api.middleware_integration import (
    setup_configuration_middleware, add_configuration_endpoints
)
...
class TestConfigurationCaptureMiddleware:
    ...
    def test_should_capture_request(self, middleware, mock_response):
        ...
        assert not middleware._should_capture_request(mock_response, 50.0)  # Below threshold
        
        # Slow successful request
        assert middleware._should_capture_request(mock_response, 200.0)  # Above threshold
    ...
    async def test_extract_response_data(self, middleware, mock_response):
        ...
        duration_ms = 150.5
        response_data = await middleware._extract_response_data(mock_response, duration_ms)
    ...
    def test_parse_platform(self, middleware):
        ...
        assert middleware._parse_platform("Mozilla/5.0 (Macintosh; Intel Mac OS X)") == "macos"
    ...
    def test_determine_client_type(self, middleware):
        ...
        assert middleware._determine_client_type("Mozilla/5.0 Firefox/89.0") == "browser"
...
class TestConfigurationAPIMiddleware:
    ...
    def test_create_production_middleware(self, capture_service):
        ...
        with patch('pynomaly.infrastructure.config.feature_flags.require_feature'):
            middleware_factory = ConfigurationAPIMiddleware.create_production_middleware(
                configuration_service=capture_service
            )
...
class TestWebAPIConfigurationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_get_endpoint_performance_metrics_not_found(self, integration_service):
    ...
    async def test_track_api_configuration_quality_not_found(self, integration_service):
        ...
        config_id = uuid4()
        quality_metrics = await integration_service.track_api_configuration_quality(config_id)
    ...
    def test_get_integration_statistics(self, integration_service):
        ...
        expected_keys = [
            "total_api_requests", "configurations_captured", "unique_endpoints",
            "unique_clients", "total_processing_time_ms", "last_analysis_time", "error_count"
        ]
...
class TestMiddlewareIntegration:
    ...
    def test_setup_configuration_middleware_disabled(self, app, capture_service):
        ...
        with patch('pynomaly.infrastructure.config.feature_flags.is_feature_enabled', return_value=False):
    ...
    def test_setup_configuration_middleware_development(self, app, capture_service):
        ...
        with patch('pynomaly.infrastructure.config.feature_flags.is_feature_enabled', return_value=True):
            setup_configuration_middleware(app, capture_service, "development")
    ...
    def test_add_configuration_endpoints_with_integration(self, app, capture_service):
        ...
        integration_service.analyze_api_usage_patterns = AsyncMock(return_value={
            "total_configurations": 0,
            "endpoint_analysis": {"total_unique_endpoints": 0}
        })
        integration_service.get_endpoint_performance_metrics = AsyncMock(return_value={
            "endpoint": "/test",
            "total_requests": 0
        })
        integration_service.generate_api_configuration_report = AsyncMock(return_value={
        ...
        response = client.get("/api/v1/configurations/endpoints/test/performance?days=7")
        assert response.status_code == 200
