"""
Performance tests with statistical baseline assertions.
...
class PerformanceBaselineManager:
    ...
    def __init__(self, baseline_path: str = "perf/baselines.json"):
    ...
    def assert_performance_regression(
        self, test_name: str, current_stats: dict[str, float]
    ):
        ...
        if "samples" in current_stats:
            ...
            assert (
                p_value < p_threshold
                or current_median <= baseline_median * self.get_threshold("median")
            ), (
...
def test_anomaly_detection_small_performance(benchmark, sample_data_small):
    ...
    baseline_manager.assert_performance_regression(
        "anomaly_detection_small", stats_dict
    )
...
def test_anomaly_detection_medium_performance(benchmark, sample_data_medium):
    ...
    baseline_manager.assert_performance_regression(
        "anomaly_detection_medium", stats_dict
    )
...
def test_anomaly_detection_large_performance(benchmark, sample_data_large):
    ...
    baseline_manager.assert_performance_regression(
        "anomaly_detection_large", stats_dict
    )
...
def test_ensemble_prediction_performance(benchmark, sample_data_medium):
    ...
    def ensemble_predict():
        ...
        detectors = [IsolationForest(random_state=42), OneClassSVM(nu=0.1)]
...
@pytest.mark.performance
@pytest.mark.memory
def test_memory_usage_monitoring(benchmark, sample_data_large):
    ...
    def memory_intensive_task():
        ...
        detector = IsolationForest(random_state=42)
        result = detector.fit_predict(sample_data_large)
    ...
    result = benchmark(memory_intensive_task)
    ...
    # Use anomaly_detection_large baseline for memory test
    baseline_manager.assert_performance_regression(
        "anomaly_detection_large", stats_dict
    )
