...
class FinancialDataPreprocessor:
    ...
    def _validate_financial_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.columns:
            if 'amount' in col.lower() or 'value' in col.lower() or 'balance' in col.lower():
    ...
    def _handle_missing_values(self, df: pd.DataFrame, target_column: str = None) -> pd.DataFrame:
    ...
    def _engineer_financial_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            if df[col].dtype.name.startswith('datetime'):
                ...
                new_features.extend([
                    f'{base_name}_year', f'{base_name}_month', f'{base_name}_day',
                    f'{base_name}_hour', f'{base_name}_dayofweek', f'{base_name}_is_weekend'
                ])
        ...
        for col in amount_columns:
            if df[col].dtype in ['int64', 'float64']:
                ...
                # Amount ranges
                df[f'{col}_range'] = pd.cut(df[col], bins=5, labels=['very_low', 'low', 'medium', 'high', 'very_high'])
        ...
        if 'account_id' in df.columns and len(amount_columns) > 0:
            for amount_col in amount_columns:
                ...
                df['transaction_count'] = df.groupby('account_id')[amount_col].transform('count')
                df['avg_transaction_amount'] = df.groupby('account_id')[amount_col].transform('mean')
                df['total_transaction_amount'] = df.groupby('account_id')[amount_col].transform('sum')
                
                new_features.extend(['transaction_count', 'avg_transaction_amount', 'total_transaction_amount'])
        
        self.preprocessing_steps.append({
            'step': 'financial_feature_engineering',
            'new_features': new_features
        })
        
        return df
    
    def _encode_categorical_variables(self, df: pd.DataFrame, target_column: str = None) -> pd.DataFrame:
    ...
    def _scale_features(self, df: pd.DataFrame, target_column: str = None) -> pd.DataFrame:
    ...
    def _select_features(self, df: pd.DataFrame, target_column: str = None) -> pd.DataFrame:
...
def main():
    ...
    data = {
        ...
        'transaction_type': np.random.choice(['debit', 'credit', 'transfer'], n_samples),
        'merchant_category': np.random.choice(['grocery', 'gas', 'restaurant', 'retail', 'other'], n_samples),
        'timestamp': pd.date_range('2023-01-01', periods=n_samples, freq='H'),
        'balance_after': np.random.normal(5000, 2000, n_samples),
        'currency': np.random.choice(['USD', 'EUR', 'GBP'], n_samples, p=[0.7, 0.2, 0.1])
    }
    
    # Add some missing values and outliers
    df = pd.DataFrame(data)
    df.loc[np.random.choice(df.index, 500), 'amount'] = np.nan
    df.loc[np.random.choice(df.index, 100), 'amount'] = np.random.uniform(50000, 100000, 100)
