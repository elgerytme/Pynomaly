...
class IoTSensorPreprocessor:
    ...
    def preprocess(self, 
                  ...
                  sensor_columns: List[str] = None) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
    ...
    def _prepare_temporal_data(self, df: pd.DataFrame, timestamp_column: str) -> pd.DataFrame:
    ...
    def _assess_sensor_quality(self, df: pd.DataFrame, sensor_columns: List[str]) -> Dict[str, Any]:
        """Assess quality of sensor data."""
        quality_results = {
            'total_sensors': len(sensor_columns),
            'time_range': (df.index.min(), df.index.max()),
            'sampling_rate_seconds': (df.index[1] - df.index[0]).total_seconds() if len(df) > 1 else None,
            'sensor_analysis': {}
        }
        ...
        for sensor in sensor_columns:
            if sensor in df.columns:
                ...
                IQR = Q3 - Q1
                outliers = ((sensor_data < (Q1 - 1.5 * IQR)) | 
                           (sensor_data > (Q3 + 1.5 * IQR))).sum()
                ...
                if sensor_stats['std'] > 0:
                    sensor_stats['noise_level'] = sensor_stats['std'] / abs(sensor_stats['mean'])
                ...
                if len(sensor_data) > 100:
                    ...
                    sensor_stats['drift_detected'] = abs(last_quarter_mean - first_quarter_mean) > drift_threshold
    ...
    def _handle_missing_values(self, df: pd.DataFrame, sensor_columns: List[str], timestamp_column: str) -> pd.DataFrame:
        ...
        if sensors_to_drop:
            ...
            sensor_columns = [col for col in sensor_columns if col not in sensors_to_drop]
            ...
            if sensor in df.columns:
                ...
                if missing_mask.any():
                    if strategy == 'interpolation':
                        ...
                        gaps = missing_mask.astype(int).groupby((~missing_mask).cumsum()).cumsum()
                        ...
                        interpolation_info[sensor] = {
                            'method': method,
                            'interpolated_points': (missing_mask & interpolate_mask).sum(),
                        ...
                        knn_columns = [col for col in sensor_columns if col in df.columns]
    ...
    def _handle_outliers(self, df: pd.DataFrame, sensor_columns: List[str]) -> pd.DataFrame:
        ...
        for sensor in sensor_columns:
            if sensor in df.columns:
                ...
                if method == 'iqr_per_sensor':
                    ...
                    rolling_q25 = sensor_data.rolling(window=window_size, center=True).quantile(0.25)
                    rolling_q75 = sensor_data.rolling(window=window_size, center=True).quantile(0.75)
                    ...
                    # Rolling z-score
                    rolling_mean = sensor_data.rolling(window=window_size, center=True).mean()
                    rolling_std = sensor_data.rolling(window=window_size, center=True).std()
    ...
    def _calibrate_sensors(self, df: pd.DataFrame, sensor_columns: List[str]) -> pd.DataFrame:
        ...
        for sensor in sensor_columns:
            if sensor in df.columns:
                ...
                if self.config['quality_checks']['noise_analysis']:
                    noise_level = sensor_data.std() / abs(sensor_data.mean()) if sensor_data.mean() != 0 else 0
                    if noise_level > 0.1:  # High noise threshold
                        ...
                        calibration_info[sensor] = calibration_info.get(sensor, '') + '_noise_reduction'
        
        if calibration_info:
            self.preprocessing_steps.append({
                'step': 'sensor_calibration',
                'calibrations_applied': calibration_info
            })
        
        return df
    
    def _engineer_iot_features(self, df: pd.DataFrame, sensor_columns: List[str], timestamp_column: str = None) -> pd.DataFrame:
            ...
            for sensor in sensor_columns:
                if sensor in df.columns:
                    for window in window_sizes:
                        ...
                        df[f'{sensor}_rolling_mean_{window}'] = df[sensor].rolling(window=window).mean()
                        df[f'{sensor}_rolling_std_{window}'] = df[sensor].rolling(window=window).std()
                        df[f'{sensor}_rolling_min_{window}'] = df[sensor].rolling(window=window).min()
                        df[f'{sensor}_rolling_max_{window}'] = df[sensor].rolling(window=window).max()
                        
                        new_features.extend([
                            f'{sensor}_rolling_mean_{window}', f'{sensor}_rolling_std_{window}',
                            f'{sensor}_rolling_min_{window}', f'{sensor}_rolling_max_{window}'
                        ])
        ...
        # Sensor interactions
        if self.config['feature_engineering']['sensor_interactions'] and len(sensor_columns) > 1:
            for i, sensor1 in enumerate(sensor_columns):
                for sensor2 in sensor_columns[i+1:]:
                    if sensor1 in df.columns and sensor2 in df.columns:
                        ...
                        new_features.extend([
                            f'{sensor1}_{sensor2}_ratio',
                            f'{sensor1}_{sensor2}_diff'
                        ])
        
        # Seasonal decomposition features
        if self.config['feature_engineering']['seasonal_features']:
            for sensor in sensor_columns[:3]:  # Limit to first 3 sensors for performance
                if sensor in df.columns and len(df) > 100:
                    try:
                        ...
                        new_features.extend([
                            f'{sensor}_trend', f'{sensor}_seasonal', f'{sensor}_residual'
                        ])
                    except:
        ...
        self.preprocessing_steps.append({
            ...
            'feature_types': ['temporal', 'rolling_stats', 'lags', 'interactions', 'seasonal']
        })
        
        return df
    
    def _fuse_sensors(self, df: pd.DataFrame, sensor_columns: List[str]) -> pd.DataFrame:
        ...
        if len(sensor_columns) > 1:
            ...
            for i, sensor1 in enumerate(sensor_columns):
                for sensor2 in sensor_columns[i+1:]:
                    if sensor1 in corr_matrix.columns and sensor2 in corr_matrix.columns:
        ...
        if fusion_info:
            self.preprocessing_steps.append({
                'step': 'sensor_fusion',
                'fused_sensors': fusion_info
            })
        
        return df
    
    def _scale_features(self, df: pd.DataFrame, original_sensor_columns: List[str]) -> pd.DataFrame:
    ...
    def _final_validation(self, df: pd.DataFrame, original_shape: Tuple[int, int]) -> Dict[str, Any]:
    ...
    def _check_temporal_continuity(self, df: pd.DataFrame) -> bool:
        ...
        if isinstance(df.index, pd.DatetimeIndex) and len(df) > 1:
            ...
            irregular_intervals = ((time_diffs < acceptable_range[0]) | 
                                 (time_diffs > acceptable_range[1])).sum()
...
def main():
    ...
    data = {
        ...
        'temperature': 20 + 5 * np.sin(np.arange(n_samples) * 2 * np.pi / 1440) + np.random.normal(0, 1, n_samples),  # Daily pattern
        'humidity': 60 + 10 * np.cos(np.arange(n_samples) * 2 * np.pi / 1440) + np.random.normal(0, 2, n_samples),     # Daily pattern
        'pressure': 1013 + np.random.normal(0, 5, n_samples),  # Random walk
        'light_level': np.maximum(0, 100 * (np.sin(np.arange(n_samples) * 2 * np.pi / 1440) + 1) + np.random.normal(0, 10, n_samples)),  # Day/night
        'co2_level': 400 + 100 * np.random.exponential(0.1, n_samples)  # Exponential distribution
    }
    ...
    df.loc[np.random.choice(df.index, 50), 'pressure'] = df['pressure'].iloc[np.random.choice(df.index, 50)] + 200  # Outliers
    ...
    for sensor, stats in metadata['quality_results']['sensor_analysis'].items():
        print(f"- {sensor}: Missing={stats['missing_ratio']:.3f}, Outliers={stats['outlier_ratio']:.3f}")
