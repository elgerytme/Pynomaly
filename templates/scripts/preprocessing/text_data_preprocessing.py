...
class TextDataPreprocessor:
    ...
    def preprocess(self, 
                  ...
                  metadata_columns: List[str] = None) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
    ...
    def _validate_text_data(self, df: pd.DataFrame, text_columns: List[str]) -> Dict[str, Any]:
        ...
        for col in text_columns:
            if col in df.columns:
                ...
                if text_stats['avg_length'] < self.config['text_cleaning']['min_length']:
                    validation_results['issues'].append({
    ...
    def _clean_text_data(self, df: pd.DataFrame, text_columns: List[str]) -> pd.DataFrame:
    ...
    def _preprocess_text(self, df: pd.DataFrame, text_columns: List[str]) -> pd.DataFrame:
    ...
    def _preprocess_document(self, text: str, tokenization_config: Dict, normalization_config: Dict) -> str:
        ...
        for token in tokens:
            ...
            # Stopword filter
            if tokenization_config['remove_stopwords'] and token.lower() in self.stop_words:
                continue
        ...
        for col in text_columns:
            ...
            if processed_col in df.columns:
                ...
                if len(non_empty_docs) < self.config['validation']['min_documents']:
                    logger.warning(f"Too few non-empty documents for {col}, skipping feature extraction")
                    continue
                ...
                if feature_config['tfidf']['enable']:
                    ...
                    documents_for_tfidf = [doc if doc.strip() else 'empty_document' for doc in documents]
                ...
                if feature_config['count_vectorizer']['enable']:
                    ...
                    documents_for_count = [doc if doc.strip() else 'empty_document' for doc in documents]
                    count_features = vectorizer.fit_transform(documents_for_count)
                    
                    feature_names = [f"{col}_count_{i}" for i in range(count_features.shape[1])]
                ...
                if feature_config['hashing_vectorizer']['enable']:
                    ...
                    vectorizer = HashingVectorizer(
                        n_features=hash_config['n_features'],
                        stop_words='english'
                    )
                    
                    documents_for_hash = [doc if doc.strip() else 'empty_document' for doc in documents]
                    hash_features = vectorizer.transform(documents_for_hash)
                    
                    feature_names = [f"{col}_hash_{i}" for i in range(hash_features.shape[1])]
                    hash_df = pd.DataFrame(
                        hash_features.toarray(), 
                        columns=feature_names, 
                        index=df.index
                    )
    ...
    def _engineer_linguistic_features(self, df: pd.DataFrame, text_columns: List[str]) -> pd.DataFrame:
            ...
            if col in df.columns:
                ...
                if linguistic_config['readability_metrics']:
                    ...
                    df[f"{col}_automated_readability_index"] = original_text.apply(
                        lambda x: textstat.automated_readability_index(x) if x.strip() else 0
                    )
                ...
                if linguistic_config['complexity_metrics']:
                    ...
                    df[f"{col}_avg_sentence_length"] = original_text.apply(
                        lambda x: np.mean([len(word_tokenize(sent)) for sent in sent_tokenize(x)]) 
                        if x.strip() and sent_tokenize(x) else 0
                    )
                ...
                if linguistic_config['pos_tag_features']:
                    ...
                    pos_df = pd.DataFrame(
                        pos_features.tolist(), 
                        index=df.index,
                        columns=[f"{col}_pos_{tag}" for tag in ['NN', 'VB', 'JJ', 'RB', 'PRP']]
                    )
                    df = pd.concat([df, pos_df], axis=1)
                
                # Named entity features
                if linguistic_config['named_entity_features']:
                    df[f"{col}_named_entities_count"] = original_text.apply(self._count_named_entities)
        
        self.preprocessing_steps.append({
            'step': 'linguistic_feature_engineering',
            'features_added': ['readability', 'complexity', 'pos', 'named_entities']
        })
    ...
    def _engineer_anomaly_features(self, df: pd.DataFrame, text_columns: List[str]) -> pd.DataFrame:
        ...
        if anomaly_config['cluster_features']:
            ...
            text_feature_columns = [col for col in df.columns 
                                  if any(text_type in col for text_type in ['_tfidf_', '_count_', '_svd_', '_lda_'])]
        ...
        if anomaly_config['statistical_features']:
            for col in text_columns:
                ...
                df[f"{col}_vocabulary_diversity"] = df[f"{col}_unique_words"] / (df[f"{col}_word_count"] + 1)
    ...
    def _scale_features(self, df: pd.DataFrame, metadata_columns: List[str]) -> pd.DataFrame:
        """Scale numerical features for anomaly detection."""
        # Identify numerical columns to scale (exclude metadata and original text)
        exclude_columns = set(metadata_columns + [col for col in df.columns if col.endswith('_original')])
    ...
    def _final_validation(self, df: pd.DataFrame, original_shape: Tuple[int, int]) -> Dict[str, Any]:
        ...
        validation_results = {
            ...
            'text_features_created': len([col for col in df.columns if any(
                feat_type in col for feat_type in ['_tfidf_', '_count_', '_svd_', '_lda_', '_similarity', '_cluster']
            )]),
            'linguistic_features_created': len([col for col in df.columns if any(
                feat_type in col for feat_type in ['_flesch_', '_sentence_', '_word_', '_pos_', '_named_']
            )]),
    ...
    def _extract_pos_features(self, text: str) -> List[float]:
        ...
        # Return ratios
        return [pos_counts[key] / total_words for key in ['NN', 'VB', 'JJ', 'RB', 'PRP']]
    ...
    def _update_vocabulary_stats(self, processed_texts: pd.Series, column_name: str):
        ...
        if all_words:
            vocab_stats = {
                ...
                'most_common_words': pd.Series(all_words).value_counts().head(10).to_dict()
            }
...
def main():
    ...
    for col, stats in metadata['vocabulary_stats'].items():
        print(f"- {col}: {stats['vocabulary_size']} unique words, {stats['avg_words_per_doc']:.1f} avg words/doc")
    
    print(f"\nText Features Created: {metadata['final_validation']['text_features_created']}")
    print(f"Linguistic Features Created: {metadata['final_validation']['linguistic_features_created']}")
    ...
    feature_cols = [col for col in processed_df.columns if any(
        feat_type in col for feat_type in ['_tfidf_', '_flesch_', '_word_', '_similarity', '_cluster']
