...
class TimeSeriesAnomalyDetector:
    ...
    def analyze_time_series_properties(self, df=None):
        ...
        analysis['frequency'] = {
            ...
            'missing_timestamps': len(pd.date_range(df.index.min(), df.index.max(), 
                                                  freq=most_common_freq)) - len(df) if most_common_freq else None
        }
        
        print(f"\n⏰ Frequency Analysis:")
        print(f"  • Most common interval: {most_common_freq}")
        print(f"  • Irregular intervals: {analysis['frequency']['irregular_intervals']}")
    ...
    def decompose_time_series(self, df=None, model='additive'):
        ...
        try:
            ...
            print(f"  • Trend component: {len(self.decomposition.trend.dropna())} points")
            print(f"  • Seasonal component: {len(self.decomposition.seasonal.dropna())} points")
            print(f"  • Residual component: {len(self.decomposition.resid.dropna())} points")
    ...
    def engineer_features(self, df=None, include_decomposition=True):
        ...
        for window in windows:
            if window < len(df):
                ...
                features_df[f'rolling_median_{window}'] = series.rolling(window).median()
                
                # Deviation from rolling statistics
                features_df[f'dev_from_mean_{window}'] = (series - features_df[f'rolling_mean_{window}']).abs()
                features_df[f'zscore_{window}'] = (series - features_df[f'rolling_mean_{window}']) / features_df[f'rolling_std_{window}']
