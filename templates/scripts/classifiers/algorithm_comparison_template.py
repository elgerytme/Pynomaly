...
class AlgorithmComparator:
    ...
    def _evaluate_algorithm(self, 
        ...
        if y is not None:
            if scores is not None:
                metrics['roc_auc'] = roc_auc_score(y, -scores)  # Negative because higher scores = more normal
        ...
        performance = {
            ...
            'predictions_per_second': len(X) / predict_time if predict_time > 0 else float('inf'),
            'scalability_score': self._calculate_scalability_score(X.shape, fit_time, predict_time)
        }
    ...
    def _cross_validate_algorithm(self, 
                                 algorithm_class, 
                                 params: Dict[str, Any],
                                 X: np.ndarray, 
    ...
    def _analyze_algorithm_characteristics(self, 
                                         algo_name: str,
                                         model: Any,
                                         X: np.ndarray,
    ...
    def _perform_comparative_analysis(self, 
        ...
        if len(successful_results) < 2:
            return {'status': 'insufficient_algorithms', 'message': 'Need at least 2 successful algorithms for comparison'}
    ...
    def _compare_performance(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if performance_data:
            ...
            fit_times = {name: perf.get('fit_time', float('inf')) 
                        for name, perf in performance_data.items()}
            performance_rankings['fit_time'] = sorted(fit_times.items(), key=lambda x: x[1])
            
            # Predict time ranking (lower is better)
            predict_times = {name: perf.get('predict_time', float('inf')) 
                           for name, perf in performance_data.items()}
            performance_rankings['predict_time'] = sorted(predict_times.items(), key=lambda x: x[1])
            
            # Predictions per second ranking (higher is better)
            throughput = {name: perf.get('predictions_per_second', 0) 
                         for name, perf in performance_data.items()}
            performance_rankings['throughput'] = sorted(throughput.items(), key=lambda x: x[1], reverse=True)
            
            # Scalability score ranking (higher is better)
            scalability = {name: perf.get('scalability_score', 0) 
                          for name, perf in performance_data.items()}
            performance_rankings['scalability'] = sorted(scalability.items(), key=lambda x: x[1], reverse=True)
        ...
        for metric_name, algo_scores in cv_scores.items():
            if len(algo_scores) >= 2:
                        ...
                        if len(scores1) == len(scores2) and len(scores1) > 0:
                            try:
                                ...
                                pairwise_tests[f"{algo1}_vs_{algo2}"] = {
                                    'statistic': statistic,
                                    'p_value': p_value,
                                    'significant': p_value < self.config['evaluation']['statistical_tests']['significance_level'],
                                    'winner': algo1 if np.mean(scores1) > np.mean(scores2) else algo2
                                }
                            except Exception as e:
                                pairwise_tests[f"{algo1}_vs_{algo2}"] = {
                                    'error': str(e)
                ...
                if len(algorithms) > 2:
                    ...
                    friedman_result = {'status': 'skipped', 'reason': 'Less than 3 algorithms'}
    ...
    def _rank_algorithms(self, results: Dict[str, Any], data_shape: Tuple[int, int]) -> Dict[str, Any]:
        ...
        for algo_name in algorithms:
            ...
            performance_scores = [metrics.get(m, 0) for m in key_metrics if m in metrics]
            scores['overall_performance'] = np.mean(performance_scores) if performance_scores else 0
            
            # Speed score (based on predictions per second)
            performance = result.get('performance', {})
            throughput = performance.get('predictions_per_second', 0)
            scores['speed'] = min(1.0, throughput / 1000)  # Normalize to 1000 predictions/sec
            ...
            interpretability_scores = {'high': 1.0, 'medium': 0.6, 'low': 0.3, 'unknown': 0.0}
            scores['interpretability'] = interpretability_scores.get(interpretability, 0)
        ...
        return {
            ...
            'recommended_algorithm': ranked_algorithms[0][0] if ranked_algorithms else None
        }
    ...
    def _analyze_algorithm_similarity(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_similarity': np.mean([s[2] for s in similarities]) if similarities else 0
        }
    ...
    def _generate_comparison_report(self, 
        ...
        report['detailed_analysis'] = {
            ...
            'statistical_significance': self._summarize_statistical_tests(comparison_results),
            'algorithm_characteristics': self._summarize_characteristics(algorithm_results)
        }
    ...
    def _get_alternative_recommendations(self, 
        ...
        performance_ranking = comparison_results.get('performance_comparison', {}).get('rankings', {})
    ...
    def _get_use_case_recommendations(self, 
                                    algorithm_results: Dict[str, Any],
                                    comparison_results: Dict[str, Any]) -> Dict[str, str]:
    ...
    def _summarize_performance(self, algorithm_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_metric_trends(self, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _summarize_statistical_tests(self, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_name, results in statistical_results.items():
            ...
            significant_pairs = [
                pair for pair, test in pairwise_tests.items()
    ...
    def _summarize_characteristics(self, algorithm_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def save_results(self, filepath: str):
        ...
        for dataset_name, results in self.results.items():
            ...
            for algo_name, result in algorithm_results.items():
                if result.get('status') == 'success':
                    serializable_results[dataset_name]['algorithm_summary'][algo_name] = {
                        'metrics': result.get('metrics', {}),
                        'performance': result.get('performance', {}),
                        'characteristics': result.get('characteristics', {}),
                        'cv_results': result.get('cv_results', {}),
...
def main():
    ...
    if recommendations:
        ...
        if alternatives:
            ...
            for choice_type, choice_info in alternatives.items():
                print(f"  - {choice_type}: {choice_info['algorithm']} ({choice_info['reason']})")
    ...
    if characteristics:
        ...
        for algo, chars in characteristics.items():
            print(f"  {algo}: {chars['type']}, {chars['complexity']}, interpretability: {chars['interpretability']}")
