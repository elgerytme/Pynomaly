...
class TechnicalReportData:
    ...
    duplicate_rows_pct: Optional[float] = None
...
class TechnicalReportGenerator:
    ...
    def generate_report(
        ...
        try:
            ...
            return self._generate_fallback_report(data, output_path, format_type, context)
    
    def _perform_statistical_analysis(self, data: TechnicalReportData) -> Dict[str, Any]:
                ...
                for alg2 in algorithm_names[i+1:]:
                    try:
                        ...
                        pairwise_tests[f"{alg1}_vs_{alg2}"] = {
                            ...
                            'p_value': p_value,
                            'significant': p_value < 0.05,
                            'better_algorithm': alg1 if np.mean(cv_results[alg1]) > np.mean(cv_results[alg2]) else alg2
                        }
                    except Exception as e:
    ...
    def _generate_performance_comparison(self, data: TechnicalReportData) -> Dict[str, Any]:
        ...
        for metric in metrics:
            metric_values = [(alg.name, getattr(alg, metric)) for alg in data.algorithms]
    ...
    def _identify_best_algorithm(self, data: TechnicalReportData) -> Dict[str, Any]:
        ...
        best_algorithm = next(alg for alg in data.algorithms if alg.name == best_algorithm_name)
    ...
    def _generate_technical_charts(self, data: TechnicalReportData, output_path: str) -> List[str]:
        ...
        if len(data.algorithms) <= 5:  # Radar chart works best with few algorithms
            ...
            for i, algorithm in enumerate(data.algorithms):
                ...
                ax.plot(angles, values, 'o-', linewidth=2, label=algorithm.name, color=colors[i])
                ax.fill(angles, values, alpha=0.25, color=colors[i])
            
            ax.set_xticks(angles[:-1])
            ax.set_xticklabels([metric.replace('_', ' ').title() for metric in metrics])
            ax.set_ylim(0, 1)
            ax.set_title('Algorithm Performance Comparison', size=16, fontweight='bold', pad=20)
        ...
        for idx, (metric, title) in enumerate(metrics_with_names):
            ...
            if metric == 'accuracy' and any(getattr(alg, metric) is None for alg in data.algorithms):
                # Skip accuracy if not available
                ax.text(0.5, 0.5, 'Accuracy\nNot Available', ha='center', va='center', 
        ...
        algorithms_with_cm = [alg for alg in data.algorithms if alg.confusion_matrix is not None]
        ...
        if any(alg.training_time_seconds is not None for alg in data.algorithms):
            ...
            for i, alg in enumerate(data.algorithms):
                ax.annotate(alg.name, (training_times[i], f1_scores[i]),
    ...
    def _generate_technical_recommendations(self, data: TechnicalReportData) -> List[str]:
        ...
        memory_intensive = [alg for alg in data.algorithms 
        ...
        slow_algorithms = [alg for alg in data.algorithms 
        ...
        if data.cross_validation_results:
            cv_std = {name: np.std(scores) for name, scores in data.cross_validation_results.items()}
    ...
    def _generate_methodology_notes(self, data: TechnicalReportData) -> List[str]:
        ...
        if data.target_distribution:
            ...
            normal_pct = (data.target_distribution.get('normal', 0) / total_samples) * 100
            anomaly_pct = (data.target_distribution.get('anomaly', 0) / total_samples) * 100
            notes.append(
        ...
        if data.preprocessing_steps:
            notes.append(
                f"🔧 **Preprocessing**: {', '.join(data.preprocessing_steps)}"
            )
        
        # Validation method
        if data.validation_method:
            notes.append(
                f"✅ **Validation**: {data.validation_method}"
            )
    ...
    def _generate_validation_summary(self, data: TechnicalReportData) -> Dict[str, Any]:
        ...
        if data.cross_validation_results:
            ...
            summary['cross_validation'] = {
                ...
                'mean_scores': {name: np.mean(scores) for name, scores in cv_results.items()},
                'std_scores': {name: np.std(scores) for name, scores in cv_results.items()}
            }
    ...
    def _calculate_cohens_d(self, mean1: float, mean2: float, std1: float = 0.1, std2: float = 0.1) -> float:
        ...
        if context['statistical_analysis']:
            ...
            if 'friedman_test' in stat_analysis:
                ...
                if 'error' not in friedman:
                    ...
                    report_content += f"**Interpretation:** {friedman['interpretation']}\n\n"
...
if __name__ == "__main__":
    ...
    algorithms = [
        AlgorithmMetrics(
            ...
            feature_importance={"feature_1": 0.25, "feature_2": 0.18, "feature_3": 0.15}
        ),
        AlgorithmMetrics(
    ...
    sample_data = TechnicalReportData(
        ...
        dataset_features=["amount", "hour", "day_of_week", "merchant_category", "customer_age"],
