...
class TemplateValidator:
        ...
        return {
            'validation_categories': {
                ...
                'functionality': {
                    'enabled': True,
                    'weight': 0.25,
                    'checks': ['mock_data_testing', 'method_execution', 'error_handling']
                },
                'performance': {
                    'enabled': True,
                    'weight': 0.20,
                ...
                'documentation': {
                    'enabled': True,
                    'weight': 0.15,
                    'checks': ['docstring_coverage', 'parameter_documentation', 'examples']
                },
                'best_practices': {
                    'enabled': True,
                    'weight': 0.15,
                ...
                'integration': {
                    'enabled': True,
                    'weight': 0.10,
                    'checks': ['dependency_compatibility', 'api_consistency', 'data_compatibility']
                }
            },
    ...
    def discover_templates(self, templates_root: str) -> Dict[str, Dict[str, Any]]:
        ...
        for category, patterns in template_categories.items():
            ...
            for pattern in patterns:
                for template_path in templates_root.glob(pattern):
                    if template_path.is_file() and not template_path.name.startswith('_'):
                        template_name = template_path.stem
                        
                        # Skip test files and __pycache__
                        if 'test_' in template_name or '__pycache__' in str(template_path):
                            continue
                        ...
                        template_info = {
                            ...
                            'relative_path': str(template_path.relative_to(templates_root))
                        }
        ...
        logger.info(f"Discovered {total_templates} templates across {len(template_registry)} categories")
        
        return template_registry
    
    def validate_template(self, 
                         template_path: str,
                         template_name: str = None) -> TemplateValidationResult:
        """
        ...
        for category_name, category_config in validation_categories.items():
            if category_config['enabled']:
                ...
                try:
                    ...
                    result_data['validation_categories'][category_name] = category_result
                    
                    # Aggregate errors and warnings
                    result_data['errors'].extend(category_result.get('errors', []))
                    result_data['warnings'].extend(category_result.get('warnings', []))
                    result_data['recommendations'].extend(category_result.get('recommendations', []))
        ...
        # Performance metrics
        result_data['performance_metrics']['validation_time'] = time.time() - validation_start
    ...
    def _validate_category(self, 
        ...
        for check_name in checks:
            try:
                ...
                category_result['recommendations'].extend(check_result.get('recommendations', []))
        ...
        total_checks = category_result['checks_passed'] + category_result['checks_failed']
        if total_checks > 0:
            category_result['score'] = (category_result['checks_passed'] / total_checks) * 100
        
        # Update category status
        if category_result['checks_failed'] > 0:
            category_result['status'] = 'failed' if category_result['errors'] else 'warning'
    ...
    def _run_validation_check(self, 
        ...
        if hasattr(self, validation_method):
            ...
            return self._generic_validation_check(template_path, category_name, check_name)
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if not has_init:
                        result['warnings'].append(f"Class {node.name} missing __init__ method")
            ...
            template_classes = [cls for cls in classes if 'Template' in cls or 'Processor' in cls or 'Comparator' in cls]
            if not template_classes:
                result['warnings'].append("No main template class found (should contain 'Template', 'Processor', or 'Comparator')")
    ...
    def _validate_functionality_mock_data_testing(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for name, obj in vars(template_module).items():
                if (isinstance(obj, type) and 
                    any(keyword in name for keyword in ['Template', 'Processor', 'Comparator', 'Selector'])):
            ...
            for class_name, template_class in template_classes[:1]:  # Test first class
                try:
                    # Generate mock data
                    n_samples = mock_data_config['sample_sizes'][0]  # Use smallest size for testing
                    ...
                    try:
                        ...
                        try:
                            ...
                            result['metrics'][f'{class_name}_instantiation'] = 'success_with_config'
                        except Exception as e:
                            result['errors'].append(f"Failed to instantiate {class_name}: {str(e)}")
                            continue
                    
                    # Test main processing method
                    main_methods = ['preprocess', 'process', 'compare_algorithms', 'select_ensemble']
                        ...
                        if hasattr(instance, method_name):
                            try:
                                ...
                                if method_name in ['preprocess', 'process']:
                                    method(X)
                                elif method_name in ['compare_algorithms', 'select_ensemble']:
                                    method(X, y)
                                
                                result['metrics'][f'{class_name}_{method_name}'] = 'success'
                                break
                            except Exception as e:
                                result['warnings'].append(f"Method {method_name} failed with mock data: {str(e)}")
                
                except Exception as e:
                    result['warnings'].append(f"Error testing class {class_name}: {str(e)}")
    ...
    def _validate_functionality_method_execution(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    try_blocks = len([n for n in ast.walk(node) if isinstance(n, ast.Try)])
            ...
            result['metrics']['average_try_blocks'] = np.mean([m['try_blocks'] for m in method_analysis.values()]) if method_analysis else 0
            
            # Check for methods without error handling
            methods_without_error_handling = [
                name for name, analysis in method_analysis.items()
                if analysis['try_blocks'] == 0 and analysis['lines'] > 5 and not name.startswith('_')
            ]
            
            if methods_without_error_handling:
                result['warnings'].append(f"Methods without error handling: {', '.join(methods_without_error_handling)}")
    ...
    def _validate_functionality_error_handling(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_patterns = [
                'try:', 'except:', 'raise', 'finally:', 'logging.error', 'logger.error'
            ]
            ...
            if 'except:' in source_code:
                ...
                result['warnings'].append(f"Found {bare_except_count} bare except clause(s) - should specify exception types")
            
            # Check for proper logging
            if pattern_counts.get('logging.error', 0) + pattern_counts.get('logger.error', 0) == 0:
                if pattern_counts.get('except:', 0) > 0:
                    result['recommendations'].append("Consider adding error logging for better debugging")
    ...
    def _validate_performance_memory_usage(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_concerns = {
                'large_data_structures': source_code.count('np.zeros(') + source_code.count('np.ones('),
                'deep_copies': source_code.count('.copy()') + source_code.count('copy.deepcopy'),
                'list_comprehensions': source_code.count('[') - source_code.count('[]'),
                'dataframe_operations': source_code.count('pd.DataFrame') + source_code.count('.merge(') + source_code.count('.join(')
            }
            ...
            if memory_concerns['large_data_structures'] > 5:
                result['recommendations'].append("Consider using memory-efficient data structures or chunking for large arrays")
            
            if memory_concerns['deep_copies'] > 3:
                result['warnings'].append("High number of copy operations detected - may impact memory usage")
    ...
    def _validate_performance_execution_time(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_patterns = {
                ...
                'nested_loops': len([line for line in source_code.split('\n') 
                                   if '    for ' in line or '        for ' in line]),
                'vectorized_operations': source_code.count('np.') + source_code.count('pd.'),
            ...
            if performance_patterns['nested_loops'] > 2:
                result['warnings'].append("Multiple nested loops detected - consider vectorization")
            
            if performance_patterns['vectorized_operations'] < performance_patterns['loops']:
                result['recommendations'].append("Consider using more vectorized operations for better performance")
    ...
    def _validate_performance_scalability(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            scalability_indicators = {
                'parallel_processing': source_code.count('multiprocessing') + source_code.count('concurrent.futures'),
                'chunking': source_code.count('chunk') + source_code.count('batch'),
                'streaming': source_code.count('stream') + source_code.count('iterative'),
                'memory_management': source_code.count('del ') + source_code.count('gc.collect')
            }
            ...
            if total_scalability_features == 0:
                result['recommendations'].append("Consider adding scalability features like chunking or parallel processing")
    ...
    def _validate_documentation_docstring_coverage(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if function_coverage < 80:
                result['warnings'].append(f"Low function docstring coverage: {function_coverage:.1f}%")
            
            if class_coverage < 90:
                result['warnings'].append(f"Low class docstring coverage: {class_coverage:.1f}%")
    ...
    def _validate_documentation_parameter_documentation(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if doc_patterns['args_sections'] < doc_patterns['returns_sections']:
                result['recommendations'].append("Consider documenting function arguments more consistently")
            
            if doc_patterns['examples_sections'] == 0:
                result['recommendations'].append("Consider adding usage examples to documentation")
        
        except Exception as e:
            result['status'] = 'failed'
            result['errors'].append(f"Error analyzing parameter documentation: {str(e)}")
    ...
    def _validate_documentation_examples(self, template_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not example_indicators['main_function'] and not example_indicators['if_name_main']:
    ...
    def _generic_validation_check(self, 
                                 template_path: str,
                                 category_name: str,
                                 check_name: str) -> Dict[str, Any]:
    ...
    def _calculate_overall_score(self, 
                               validation_categories: Dict[str, Any],
                               categories_config: Dict[str, Any]) -> float:
    ...
    def validate_all_templates(self, templates_root: str) -> Dict[str, Any]:
        ...
        for category, templates in template_registry.items():
            ...
            for template_name, template_info in templates.items():
                current_template += 1
                logger.info(f"Validating template {current_template}/{total_templates}: {template_name}")
                ...
                try:
                    ...
                    result = self.validate_template(template_info['path'], template_name)
                    ...
                    logger.error(f"Failed to validate template {template_name}: {str(e)}")
                    failed_templates.append(template_name)
        
        # Generate summary statistics
        summary_stats = {
            'total_templates': total_templates,
            'templates_validated': len(passed_templates) + len(warning_templates) + len(failed_templates),
            'passed_templates': len(passed_templates),
            'warning_templates': len(warning_templates),
            'failed_templates': len(failed_templates),
            'success_rate': len(passed_templates) / max(1, len(passed_templates) + len(warning_templates) + len(failed_templates)) * 100,
            'validation_timestamp': datetime.now(),
        ...
        logger.info(f"Template validation complete: {summary_stats['success_rate']:.1f}% success rate")
    ...
    def generate_validation_report(self, 
                                 output_path: str = "template_validation_report.html") -> str:
        """
...
def main():
    ...
    print(f"\nReports generated:")
    print(f"  - HTML Report: template_validation_report.html")
    print(f"  - JSON Results: template_validation_results.json")
