...
class QualityMetrics:
    ...
    feature_importance_consistency: Optional[float] = None
...
class ErrorAnalysis:
    ...
    misclassified_samples: Optional[List[Dict[str, Any]]] = None
    confidence_distribution: Optional[Dict[str, float]] = None
    
    # Error severity analysis
    critical_errors: Optional[int] = None
    warning_errors: Optional[int] = None
    minor_errors: Optional[int] = None
...
class TestResultsManager:
    ...
    def save_test_result(
        ...
        if format_type == "json":
            ...
            with open(filepath, 'w') as f:
                json.dump(self._serialize_test_result(test_result), f, indent=2, default=str)
        
        elif format_type == "yaml":
            filepath = self.results_dir / "raw" / f"{filename}.yaml"
            with open(filepath, 'w') as f:
                yaml.dump(self._serialize_test_result(test_result), f, default_flow_style=False)
        ...
        if include_artifacts and test_result.visualizations:
            ...
            test_result.visualizations = [str(artifacts_dir / Path(viz).name) 
    ...
    def generate_test_report(
        ...
        if report_type == "summary":
            ...
            return self._generate_detailed_report(test_results, output_path, include_charts)
        elif report_type == "comparison":
            return self._generate_comparison_report(test_results, output_path, include_charts)
        else:
    ...
    def validate_test_result(self, test_result: TestResult) -> Dict[str, Any]:
        ...
        if test_result.quality_metrics:
            ...
            if test_result.error_analysis:
                ...
                total_predicted = ea.true_positives + ea.false_positives + ea.true_negatives + ea.false_negatives
                
                if total_predicted == 0:
                    issues.append("Empty confusion matrix")
                else:
                    calculated_precision = ea.true_positives / (ea.true_positives + ea.false_positives) if (ea.true_positives + ea.false_positives) > 0 else 0
                    calculated_recall = ea.true_positives / (ea.true_positives + ea.false_negatives) if (ea.true_positives + ea.false_negatives) > 0 else 0
                    
                    if abs(calculated_precision - qm.precision) > 0.01:
                        warnings.append(f"Precision mismatch: reported {qm.precision:.3f}, calculated {calculated_precision:.3f}")
                    
                    if abs(calculated_recall - qm.recall) > 0.01:
                        warnings.append(f"Recall mismatch: reported {qm.recall:.3f}, calculated {calculated_recall:.3f}")
        ...
        if test_result.environment.random_seed is None:
            warnings.append("No random seed specified - results may not be reproducible")
    ...
    def compare_test_results(
        ...
        for result in comparison_results:
            ...
            for metric in metrics:
                if hasattr(result.quality_metrics, metric):
                    ...
                    result_comparison["differences"][metric] = current_value - baseline_value
        ...
        if comparison["comparisons"]:
            for metric in metrics:
                differences = [comp["differences"].get(metric, 0) for comp in comparison["comparisons"]]
    ...
    def _generate_summary_report(
        ...
        if any(r.quality_metrics for r in test_results):
            f1_scores = [r.quality_metrics.f1_score for r in test_results 
...
"""
        ...
        for result in test_results:
            ...
            f1_score = result.quality_metrics.f1_score if result.quality_metrics else "N/A"
            duration = f"{result.duration_seconds:.1f}s" if result.duration_seconds else "N/A"
            algorithm = result.configuration.algorithm if result.configuration else "N/A"
