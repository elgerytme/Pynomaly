    ...
    for n_samples in data_sizes:
        ...
        for algorithm in algorithms:
            ...
            try:
                ...
                print(f"  ✓ Fit: {fit_time:.4f}s, Detect: {detect_time:.4f}s, Total: {fit_time + detect_time:.4f}s")
...
def print_performance_summary(results):
    ...
    if successful_results:
        ...
        for n_samples in sorted(df['n_samples'].unique()):
            ...
            print(f"{n_samples:6d} samples: {avg_time:.4f}s avg ({num_algorithms} algorithms)")
        
        # Fastest algorithms
        print("\n🏃 Fastest Algorithms:")
        print("-" * 40)
        
        avg_times = df.groupby(['algorithm', 'library'])['total_time'].mean().sort_values()
        ...
        avg_rates = df.groupby(['algorithm', 'library'])['anomaly_rate'].mean().sort_values(ascending=False)
    ...
    if failed_results:
        ...
        for result in failed_results:
            print(f"{result['algorithm']} ({result['library']}) - {result['n_samples']} samples: {result.get('error', 'Unknown error')}")
