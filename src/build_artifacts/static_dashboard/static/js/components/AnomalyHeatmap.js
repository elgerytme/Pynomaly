...
class AnomalyHeatmap {
  ...
  createAxes() {
    ...
    this.xAxisGroup = this.svg
      ...
      .attr(
        "transform",
        `translate(${this.options.margin.left}, ${this.options.margin.top + this.height})`,
      );
  ...
  createLegend() {
    const legend = this.svg
      ...
      .attr(
        "transform",
        `translate(${this.fullWidth - this.options.margin.right + 20}, ${this.options.margin.top})`,
      );
    ...
    // Legend scale
    const legendScale = d3.scaleLinear().domain([0, 1]).range([200, 0]);
  ...
  setupAccessibility() {
    ...
    this.container
      ...
      .attr("class", "sr-only")
      .text(
        "Interactive heatmap showing anomaly scores across features and time periods. Use keyboard navigation to explore cells.",
      );
  ...
  renderAxes(transition) {
    const xAxis = d3.axisBottom(this.xScale).tickSize(0).tickPadding(8);

    const yAxis = d3.axisLeft(this.yScale).tickSize(0).tickPadding(8);
  ...
  removeHighlights() {
    this.mainGroup.selectAll(".heatmap-cell").style("opacity", 1);
  }
    ...
    switch (event.key) {
      case "ArrowLeft":
        ...
        if (currentFeatureIndex > 0) {
          this.focusCell(
            this.xDomain[currentFeatureIndex - 1],
            this.focusedCell.timePeriod,
          );
        }
        break;
      case "ArrowRight":
      ...
      case "ArrowUp":
        ...
        if (currentTimeIndex > 0) {
          this.focusCell(
            this.focusedCell.feature,
            this.yDomain[currentTimeIndex - 1],
          );
        }
        break;
      case "ArrowDown":
      ...
      case "End":
        event.preventDefault();
        this.focusCell(
          this.xDomain[this.xDomain.length - 1],
          this.focusedCell.timePeriod,
        );
        break;
      case "Enter":
  ...
  focusCell(feature, timePeriod) {
    const cellData = this.data.find(
      (d) => d.feature === feature && d.timePeriod === timePeriod,
    );
    if (cellData) {
  ...
  updateCellFocus() {
    ...
    this.mainGroup
      ...
      .filter(
        (d) =>
          d.feature === this.focusedCell.feature &&
          d.timePeriod === this.focusedCell.timePeriod,
      )
  ...
  updateAccessibilityData() {
    ...
    const anomalyCells = scores.filter(
      (s) => s > this.options.thresholds.low,
    ).length;
  ...
  exportData() {
    return {
      ...
      summary: {
        ...
        anomalyCells: this.data.filter(
          (d) => d.anomalyScore > this.options.thresholds.low,
        ).length,
      },
    };
  }
