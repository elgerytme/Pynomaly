...
} from "./components/dashboard-layout.js";
...
class PynomAlyApp {
    ...
    if ("serviceWorker" in navigator) {
      try {
        ...
        dashboardState.dispatch(
          dashboardState.actions.setPreference("serviceWorkerEnabled", true),
        );
      } catch (error) {
  ...
  async initializeComponents() {
    ...
    const advancedDashboardContainer = document.querySelector(
      '[data-component="advanced-dashboard"]',
    );
    if (advancedDashboardContainer) {
      const advancedDashboard = new AdvancedDashboard(
        advancedDashboardContainer,
        {
    ...
    const dashboardContainer = document.querySelector(
      '[data-component="real-time-dashboard"]',
    );
    ...
    document
      .querySelectorAll('[data-component="anomaly-timeline"]')
      .forEach((container) => {
    ...
    document
      .querySelectorAll('[data-component="anomaly-heatmap"]')
      .forEach((container) => {
  ...
  initializeFilters() {
    ...
    dateRangeInputs.forEach((input) => {
      input.addEventListener("change", () => {
        ...
        if (startInput && endInput) {
          ...
          dashboardState.dispatch(
            dashboardState.actions.setDateRange(start, end),
          );
        }
      });
    });
    ...
    const confidenceRange = document.querySelector(
      '[data-filter="confidence-range"]',
    );
    if (confidenceRange) {
      confidenceRange.addEventListener("input", (e) => {
        const value = parseFloat(e.target.value);
        dashboardState.dispatch(
          dashboardState.actions.setConfidenceRange(value, 1),
        );
      });
    }
    ...
    if (statusFilter) {
      statusFilter.addEventListener("change", (e) => {
        dashboardState.dispatch(
          dashboardState.actions.setStatusFilter(e.target.value),
        );
      });
    }
  }
  ...
  setupPeriodicTasks() {
    ...
    setInterval(
      () => {
        const autoRefresh = dashboardState.getStateSlice(
          "preferences.autoRefresh",
        );
        const interval = dashboardState.getStateSlice(
  ...
  handlePointClick(detail) {
    ...
    dashboardState.dispatch(
      dashboardState.actions.setTimelineSelection({
        point: data,
  ...
  handleCellClick(detail) {
    ...
    dashboardState.dispatch(
      dashboardState.actions.setDateRange(timeRange.start, timeRange.end),
    );
  }
  ...
  handleTimeRangeSelection(detail) {
    ...
    dashboardState.dispatch(
      dashboardState.actions.setDateRange(startTime, endTime),
    );
  }
  ...
  async handleFormSubmit(detail) {
    ...
    try {
      ...
      dashboardState.dispatch(
        dashboardState.actions.addNotification({
          type: "success",
          title: "Analysis Started",
          message:
            "Your anomaly detection analysis has been queued for processing.",
        }),
      );
  ...
  handleFileUpload(detail) {
    ...
    dashboardState.dispatch(
      dashboardState.actions.setDatasets([
  ...
  updateMetrics() {
    ...
    const anomalyRate =
      totalDataPoints > 0 ? anomalyCount / totalDataPoints : 0;
  ...
  showNotification(notification) {
    ...
    notificationElement
      ...
      .addEventListener("click", () => {
        dashboardState.dispatch(
          dashboardState.actions.removeNotification(notification.id),
        );
  ...
  handleError(error, context = "Application Error") {
    ...
    // Add error to state
    dashboardState.dispatch(
      dashboardState.actions.addError({
  ...
  getTimeRangeForCell(cellData) {
    ...
    const aggregation = dashboardState.getStateSlice(
      "charts.heatmap.aggregation",
    );
    switch (aggregation) {
      case "5min":
