...
class ViolationDetector:
    ...
    def _check_file_organization(self):
        ...
        for dir_node in root_dir.subdirectories:
            if dir_node.name.startswith('.') and dir_node.name not in {'.git', '.github'}:
                continue
        ...
        for layer_name, layer_config in EXPECTED_STRUCTURE.items():
            if layer_config["required"] and layer_name not in self.model.layers:
                self.violations.append(Violation(
                    ...
                    directory_path=self.model.root_path / "src" / "pynomaly" / layer_name,
                ))
            ...
            for dep in layer_deps:
                if dep not in allowed_dependencies:
                    self.violations.append(Violation(
                        ...
                        directory_path=self.model.layers.get(layer, {}).path if layer in self.model.layers else None,
                    ))
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    if (module not in ALLOWED_DOMAIN_IMPORTS and 
                        not node.module.startswith("pynomaly.domain") and
                        not node.module.startswith("pynomaly.shared")):
                        self.violations.append(Violation(
                            type=ViolationType.DOMAIN_PURITY,
                            severity=Severity.ERROR,
