...
from pynomaly.shared.exceptions import ValidationError, AuthorizationError, ReportNotFoundError
...
@router.post("/tenants/{tenant_id}/reports", response_model=ReportResponse, status_code=status.HTTP_201_CREATED)
                    ...
                    "metrics": [
                        {
                            ...
                            "formatted_value": metric.latest_value.format_value() if metric.latest_value else "N/A",
...
@router.post("/tenants/{tenant_id}/dashboards", response_model=DashboardResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/tenants/{tenant_id}/dashboards/{dashboard_id}", response_model=DashboardResponse)
...
@router.put("/tenants/{tenant_id}/dashboards/{dashboard_id}", response_model=DashboardResponse)
async def update_dashboard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND if isinstance(e, ReportNotFoundError) else status.HTTP_403_FORBIDDEN,
            detail=str(e)
        )


@router.post("/tenants/{tenant_id}/dashboards/standard", response_model=DashboardResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/tenants/{tenant_id}/alerts", response_model=AlertResponse, status_code=status.HTTP_201_CREATED)
