...
def setup_configuration_middleware(
    ...
    if not is_feature_enabled("advanced_automl"):
        logger.info("Configuration middleware disabled - advanced_automl feature not enabled")
        return
    ...
    try:
        ...
        app.add_middleware(
            ...
            **(middleware_factory.__dict__ if hasattr(middleware_factory, '__dict__') else {})
        )
        
        logger.info(f"Configuration capture middleware enabled for {environment} environment")
...
def add_configuration_endpoints(
    ...
    async def get_api_usage_analysis(days: int = 7) -> Dict[str, Any]:
        ...
        if not integration_service:
            raise HTTPException(status_code=404, detail="Integration service not available")
