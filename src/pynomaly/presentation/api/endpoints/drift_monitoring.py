...
from pynomaly.domain.entities.drift_detection import (
    DriftDetectionResult, DriftAlert, ModelMonitoringConfig, DriftMonitoringStatus,
    DriftReport, DriftType, DriftSeverity, DriftDetectionMethod, MonitoringStatus
)
...
class DriftCheckRequest(BaseModel):
    ...
    current_data: List[List[float]] = Field(..., description="Current dataset to compare")
...
class PerformanceDriftRequest(BaseModel):
    ...
    reference_metrics: Dict[str, float] = Field(..., description="Reference performance metrics")
    current_metrics: Dict[str, float] = Field(..., description="Current performance metrics")
    threshold: float = Field(default=0.05, description="Minimum change to consider drift")
...
class MonitoringConfigRequest(BaseModel):
    ...
    reference_window_days: int = Field(default=30, description="Reference data window in days")
    comparison_window_days: int = Field(default=7, description="Comparison data window in days")
    min_sample_size: int = Field(default=100, description="Minimum sample size for checks")
    ...
    alert_on_severity: List[str] = Field(
        ...
        description="Severity levels that trigger alerts"
    )
    ...
    features_to_monitor: List[str] = Field(default=[], description="Specific features to monitor")
    exclude_features: List[str] = Field(default=[], description="Features to exclude from monitoring")
...
async def perform_drift_check(
    ...
    try:
        ...
        return SuccessResponse(
            data=result,
            message=f"Drift check completed for detector {detector_id}"
        )
...
async def check_performance_drift(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Performance drift check failed: {str(e)}")
...
async def configure_monitoring(
    ...
    try:
        ...
        config = ModelMonitoringConfig(
            ...
            enabled_methods=[DriftDetectionMethod(method) for method in request.enabled_methods],
            drift_thresholds=request.drift_thresholds,
            alert_on_severity=[DriftSeverity(severity) for severity in request.alert_on_severity],
...
async def pause_monitoring(
    ...
    try:
        ...
        return SuccessResponse(
            data="paused",
            message=f"Monitoring paused for detector {detector_id}"
        )
        
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to pause monitoring: {str(e)}")
...
async def resume_monitoring(
    ...
    try:
        ...
        return SuccessResponse(
            data="resumed",
            message=f"Monitoring resumed for detector {detector_id}"
        )
        
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to resume monitoring: {str(e)}")
...
async def stop_monitoring(
    ...
    try:
        ...
        return SuccessResponse(
            data="stopped",
            message=f"Monitoring stopped for detector {detector_id}"
        )
...
async def list_active_monitors(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list active monitors: {str(e)}")
...
async def get_drift_alerts(
    ...
    try:
        ...
        return SuccessResponse(
            data=alerts,
            message=f"Retrieved {len(alerts)} drift alerts"
        )
...
async def resolve_alert(
    ...
    try:
        ...
        return SuccessResponse(
            data="resolved",
            message=f"Alert {alert_id} resolved by {request.user}"
        )
...
async def generate_drift_report(
    ...
    try:
        if period_days < 1 or period_days > 365:
            raise HTTPException(
                status_code=400,
                detail="Report period must be between 1 and 365 days"
            )
        ...
        return SuccessResponse(
            data=report,
            message=f"Drift report generated for detector {detector_id}"
        )
