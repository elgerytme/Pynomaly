...
from pynomaly.infrastructure.monitoring.health_service import HealthService, HealthStatus, SystemMetrics
...
async def health_check(
    ...
    include_database: bool = Query(True, description="Include database connectivity checks"),
...
async def readiness_check(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={"status": "not_ready", "error": str(e), "timestamp": datetime.utcnow().isoformat()}
        )
...
def _check_adapters(container: Container) -> Dict[str, Any]:
    ...
    try:
        ...
        available_count = sum(1 for status in adapters.values() if status == "available")
...
def _check_configuration(settings) -> Dict[str, Any]:
    ...
    try:
        ...
        message = "Configuration OK" if not issues else f"Configuration issues: {', '.join(issues)}"
