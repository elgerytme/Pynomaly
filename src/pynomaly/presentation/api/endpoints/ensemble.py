...
async def detect_anomalies_ensemble(
    ...
    **Response Fields:**
    ...
    """
    try:
        ensemble_use_case: EnsembleDetectionUseCase = container.ensemble_detection_use_case()
    ...
    **Response Fields:**
    ...
    try:
        ...
        use_case_request = EnsembleOptimizationRequest(
            ...
            optimization_objective=EnsembleOptimizationObjective(request.optimization_objective),
            target_voting_strategies=[VotingStrategy(s) for s in request.target_voting_strategies],
        ...
        return EnsembleOptimizationResponseDTO(
            ...
            optimal_voting_strategy=response.optimal_voting_strategy.value if response.optimal_voting_strategy else "",
    ...
    try:
        ...
        ensemble_stats = {
            ...
            "average_f1_score": sum(
                m.f1_score for m in ensemble_use_case._performance_tracker.values()
            ) / len(ensemble_use_case._performance_tracker) if ensemble_use_case._performance_tracker else 0.0
        }
        
        # Get recent optimization history
        recent_optimizations = ensemble_use_case._optimization_history[-10:] if ensemble_use_case._optimization_history else []
            ...
            recent_optimizations=[
                {
                    ...
                    "detector_count": len(opt["request"].detector_ids) if hasattr(opt["request"], "detector_ids") else 0,
                    "success": opt["response"].success if hasattr(opt["response"], "success") else False
                }
