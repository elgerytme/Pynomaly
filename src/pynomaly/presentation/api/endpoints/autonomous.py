...
class AutonomousDetectionRequest(BaseModel):
    ...
    data_source: str | None = None  # File path or URL
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "preprocessing_applied": results.get("autonomous_detection_results", {})
                .get("data_profile", {})
                .get("preprocessing_applied", False),
            },
        }
        ...
        raise HTTPException(
            status_code=400, detail=f"Autonomous detection failed: {str(e)}"
        )
...
async def automl_optimize(
    ...
    try:
        ...
        detector_id = await automl_service.create_optimized_detector(
            automl_result, detector_name=f"AutoML_{automl_result.best_algorithm}"
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"AutoML optimization failed: {str(e)}"
        )
...
async def create_ensemble(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ensemble creation failed: {str(e)}"
        )
...
async def create_family_ensemble(
    ...
    if invalid_families:
        raise HTTPException(
            status_code=400, detail=f"Invalid algorithm families: {invalid_families}"
        )
    ...
    try:
        ...
        for family in request.families:
            ...
            if request.enable_family_ensembles and family_result.ensemble_config:
                ...
                detector_id = await automl_service.create_optimized_detector(
                    family_result, detector_name=f"Ensemble_{family}"
                )
        ...
        return {
            ...
            "total_algorithms_tested": sum(
                len(family_algorithms[f]) for f in request.families
            ),
        }

    except Exception as e:
        raise HTTPException(
            status_code=400, detail=f"Family ensemble creation failed: {str(e)}"
        )
...
async def explain_algorithm_choices(
    ...
    try:
        ...
        if request.dataset_id:
            ...
            with tempfile.NamedTemporaryFile(
                delete=False, suffix=f".{data_file.filename.split('.')[-1]}"
            ) as tmp:
        ...
        raise HTTPException(
            status_code=400, detail=f"Algorithm choice explanation failed: {str(e)}"
        )
