...
async def get_executive_summary(
    ...
    try:
        ...
        logger.info(f"Executive summary requested by {current_user.get('username', 'unknown')}")
        return summary
...
async def get_realtime_dashboard_data(
    ...
    try:
        ...
        logger.debug(f"Real-time dashboard data requested by {current_user.get('username', 'unknown')}")
        return data
...
async def get_business_metrics(
    ...
    try:
        ...
        logger.info(f"Business metrics requested by {current_user.get('username', 'unknown')}")
        return metrics
...
async def get_operational_metrics(
    ...
    try:
        if not dashboard_service.enable_operational_monitoring:
            raise HTTPException(status_code=503, detail="Operational monitoring not enabled")
        
        metrics = dashboard_service.operational_metrics
        
        logger.debug(f"Operational metrics requested by {current_user.get('username', 'unknown')}")
        return metrics
...
async def get_active_alerts(
    ...
    metric_type: Optional[DashboardMetricType] = Query(None, description="Filter by metric type"),
    ...
    try:
        ...
        alerts.sort(key=lambda x: (priority_order.get(x.priority, 99), x.timestamp), reverse=True)
        
        # Apply limit
        alerts = alerts[:limit]
        
        logger.debug(f"Active alerts requested by {current_user.get('username', 'unknown')}")
        return alerts
...
async def get_compliance_report(
    current_user: dict = Depends(get_current_user),
    _: bool = Depends(require_permission("compliance:read")) if AUTH_AVAILABLE else None,
    dashboard_service: EnterpriseDashboardService = Depends(get_dashboard_service)
) -> Dict[str, Any]:
    ...
    try:
        if not dashboard_service.enable_compliance_tracking:
            raise HTTPException(status_code=503, detail="Compliance tracking not enabled")
        
        report = dashboard_service.get_compliance_report()
        
        logger.info(f"Compliance report requested by {current_user.get('username', 'unknown')}")
        return report
...
async def get_algorithm_performance(
    ...
    limit: int = Query(10, ge=1, le=50, description="Maximum number of algorithms to return"),
    current_user: dict = Depends(get_current_user),
    dashboard_service: EnterpriseDashboardService = Depends(get_dashboard_service)
    ...
    try:
        ...
        logger.debug(f"Algorithm performance requested by {current_user.get('username', 'unknown')}")
        return result
        
    except Exception as e:
        logger.error(f"Failed to get algorithm performance: {e}")
        raise HTTPException(status_code=500, detail="Failed to retrieve algorithm performance")
    ...
    try:
        ...
        cost_analysis = {
            **cost_metrics,
            ...
            "cost_per_detection": processing_cost / max(dashboard_service.detection_stats["today"]["total"], 1),
            "analysis_timestamp": datetime.now().isoformat()
        }
        
        logger.info(f"Cost analysis requested by {current_user.get('username', 'unknown')}")
...
async def record_detection_event(
    ...
    try:
        ...
        required_fields = ["detection_id", "success", "execution_time", "algorithm_used", "anomalies_found", "dataset_size"]
...
async def export_dashboard_data(
    ...
    try:
        ...
        logger.info(f"Dashboard data exported by {current_user.get('username', 'unknown')} in {format} format")
        return JSONResponse(
    ...
    try:
        ...
        health_status = {
            ...
            "services": {
                ...
                "business_metrics": "enabled" if dashboard_service.enable_business_metrics else "disabled",
                "operational_monitoring": "enabled" if dashboard_service.enable_operational_monitoring else "disabled",
                "compliance_tracking": "enabled" if dashboard_service.enable_compliance_tracking else "disabled"
            },
            "metrics": {
                ...
                "total_detections_today": dashboard_service.detection_stats["today"]["total"]
            }
        }
