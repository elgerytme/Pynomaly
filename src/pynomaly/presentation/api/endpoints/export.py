...
from ....application.dto.export_options import ExportOptions, ExportFormat, ExportDestination
...
class ExportRequest(BaseModel):
    ...
    notification_emails: Optional[List[str]] = Field(None, description="Notification email addresses")
...
class MultiExportRequest(BaseModel):
    ...
    base_filename: str = Field("anomaly_export", description="Base filename for outputs")
    
    # Format-specific configurations
    excel_options: Optional[Dict[str, Any]] = Field(None, description="Excel-specific options")
    powerbi_options: Optional[Dict[str, Any]] = Field(None, description="Power BI-specific options")
    gsheets_options: Optional[Dict[str, Any]] = Field(None, description="Google Sheets-specific options")
    smartsheet_options: Optional[Dict[str, Any]] = Field(None, description="Smartsheet-specific options")


class ExportStatusResponse(BaseModel):
    """Response model for export status."""
    export_id: str = Field(..., description="Export operation ID")
    status: str = Field(..., description="Export status: pending, running, completed, failed")
    format: str = Field(..., description="Export format")
    progress: float = Field(..., description="Progress percentage (0-100)")
    message: Optional[str] = Field(None, description="Status message")
    result: Optional[Dict[str, Any]] = Field(None, description="Export result when completed")
...
async def get_supported_formats(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get supported formats: {e}")
...
async def start_multi_export(
    ...
    try:
        ...
        for format_api in request.formats:
            ...
            if format_api == ExportFormatAPI.EXCEL and request.excel_options:
                ...
                for key, value in request.gsheets_options.items():
                    setattr(export_request, key, value)
            elif format_api == ExportFormatAPI.SMARTSHEET and request.smartsheet_options:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start multi-export: {e}")


@router.get("/history")
async def get_export_history(
    limit: int = 50,
    format_filter: Optional[ExportFormatAPI] = None
) -> List[ExportStatusResponse]:
    ...
    templates = {
        ...
        ExportFormatAPI.POWERBI: {
            "streaming": {
                ...
                "table_name": "AnomalyResults"
            },
            "batch": {
                "streaming_dataset": False,
                "table_name": "AnomalyAnalysis"
            }
        },
...
def _create_export_options(request: ExportRequest, export_format: ExportFormat) -> ExportOptions:
