...
from pynomaly.presentation.api.middleware.security_middleware import SecurityMiddleware
...
async def get_security_overview(request: Request):
    ...
    try:
        ...
        return SecurityOverview(
            ...
            blocked_ips=waf_stats.get("blocked_ips", 0) + rate_limiter_stats.get("blocked_ips", 0),
            total_security_events=waf_stats.get("waf_stats", {}).get("total_requests", 0),
...
async def get_security_stats(request: Request, hours: int = 24):
    ...
    try:
        ...
        return SecurityStats(
            ...
            unique_attackers=len(waf_stats.get("waf_stats", {}).get("top_attackers", {})),
...
async def block_ip(request: Request, block_request: IPBlockRequest):
    ...
    try:
        ...
        audit_logger.log_security_event(
            ...
            message=f"IP {block_request.ip} manually blocked: {block_request.reason}",
            level="INFO",
            details={
                "ip": block_request.ip,
...
async def update_security_config(request: Request, config: SecurityConfigUpdate):
    ...
    try:
        ...
        return {"message": "Security configuration updated successfully", "updated": updated_settings}
    except Exception as e:
...
@router.get("/events")
async def get_security_events(request: Request, limit: int = 100, event_type: Optional[str] = None):
    ...
    try:
        ...
        if hasattr(request.app.state, 'rate_limiter'):
            ...
            components["rate_limiter"] = {
                "status": "healthy",
                "active_clients": rate_limiter.get_statistics().get("active_clients", 0),
        ...
        raise HTTPException(status_code=500, detail="Failed to perform security health check")
    ...
    try:
        ...
        if hasattr(request.app.state, 'waf_middleware'):
            ...
            dashboard_data["overview"]["waf"] = {
                "total_requests": waf_stats.get("waf_stats", {}).get("total_requests", 0),
                "blocked_requests": waf_stats.get("waf_stats", {}).get("blocked_requests", 0),
                "attacks_detected": waf_stats.get("waf_stats", {}).get("attacks_detected", 0),
                "attack_rate": waf_stats.get("waf_stats", {}).get("attack_rate", 0)
            }
            
            dashboard_data["threat_intelligence"]["attack_types"] = waf_stats.get("waf_stats", {}).get("attack_types", {})
            dashboard_data["threat_intelligence"]["top_attackers"] = waf_stats.get("waf_stats", {}).get("top_attackers", {})
        ...
        if hasattr(request.app.state, 'rate_limiter'):
            ...
            dashboard_data["real_time_metrics"]["threat_distribution"] = rl_stats.get("threat_distribution", {})
            dashboard_data["real_time_metrics"]["adaptive_limits"] = rl_stats.get("adaptive_limits", {})
