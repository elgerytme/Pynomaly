...
class DetectionResultModel(BaseModel):
    ...
    score: float = Field(..., ge=0.0, le=1.0, description="Anomaly score between 0 and 1")
...
class UncertaintyRequestModel(BaseModel):
    ...
    include_prediction_intervals: bool = Field(
        True, 
        description="Whether to include prediction intervals"
    )
    include_entropy: bool = Field(
        True, 
        description="Whether to include entropy-based uncertainty"
    )
...
class EnsembleUncertaintyRequestModel(BaseModel):
    ...
    include_disagreement: bool = Field(
        True, 
        description="Whether to include disagreement metrics"
    )
...
class BootstrapRequestModel(BaseModel):
    ...
    n_bootstrap: int = Field(1000, ge=100, le=10000, description="Number of bootstrap samples")
    statistic: str = Field(
        "mean", 
        description="Statistic to calculate",
        regex="^(mean|median|std|var)$"
    )
...
class BayesianRequestModel(BaseModel):
    ...
    binary_scores: List[int] = Field(..., min_items=1, description="Binary anomaly indicators")
    confidence_level: float = Field(0.95, ge=0.0, le=1.0, description="Confidence level")
...
class UncertaintyResponseModel(BaseModel):
    ...
    summary: Dict[str, Union[str, float]] = Field(..., description="Summary of key metrics")
...
def _convert_confidence_interval_to_model(ci: ConfidenceInterval) -> ConfidenceIntervalModel:
...
async def quantify_uncertainty(
    ...
    try:
        ...
        detection_results = [
            _convert_to_detection_result(result) 
            for result in request.detection_results
        ]
