...
class PerformanceMetricsPayload(BaseModel):
    ...
    metrics: List[Dict[str, Any]] = Field(..., description="Array of performance metrics")
...
class PerformanceAnalyzer:
    ...
    @staticmethod
    def generate_recommendations(cwv: CoreWebVitals, metrics: List[PerformanceMetricData]) -> List[str]:
    ...
    def analyze_trends(recent_hours: int = 24) -> Dict[str, Any]:
        ...
        for cwv_type in ['lcp', 'fid', 'cls', 'fcp']:
            if cwv_type in metric_groups:
                values = [m['data'].get('value') for m in metric_groups[cwv_type] if 'value' in m.get('data', {})]
                if values:
                    trends[cwv_type] = {
                        'count': len(values),
        ...
        if 'api_response' in metric_groups:
            api_durations = [m['data'].get('duration') for m in metric_groups['api_response'] if 'duration' in m.get('data', {})]
            if api_durations:
                trends['api_response'] = {
...
async def submit_performance_metrics(
    ...
    try:
        ...
        logger.info(f"Received {len(metrics)} performance metrics for session {payload.session}")
...
async def get_performance_analysis(
    ...
    try:
        ...
        summary = {
            ...
            "session_count": len(set(m.get('session') for m in session_metrics if m.get('session'))),
        ...
        return PerformanceAnalysis(
            core_web_vitals={
                ...
                "fcp": cwv.fcp,
                "scores": {
                    "lcp": "good" if cwv.lcp and cwv.lcp <= PerformanceThresholds.LCP_GOOD else "needs_improvement" if cwv.lcp and cwv.lcp <= PerformanceThresholds.LCP_POOR else "poor",
                    "fid": "good" if cwv.fid and cwv.fid <= PerformanceThresholds.FID_GOOD else "needs_improvement" if cwv.fid and cwv.fid <= PerformanceThresholds.FID_POOR else "poor",
                    "cls": "good" if cwv.cls and cwv.cls <= PerformanceThresholds.CLS_GOOD else "needs_improvement" if cwv.cls and cwv.cls <= PerformanceThresholds.CLS_POOR else "poor"
                }
            },
    ...
    try:
        ...
        for page, metrics in page_metrics.items():
            ...
            for metric in metrics:
                ...
                if metric_type in ['lcp', 'fid', 'cls', 'fcp'] and 'value' in metric.get('data', {}):
        ...
        return {
            ...
            "unique_sessions": len(set(m.get('session') for m in recent_metrics if m.get('session'))),
...
async def process_metrics_async(metrics: List[Dict[str, Any]], session_id: str):
    ...
    try:
        ...
        for metric in metrics:
            if metric.get('type') == 'lcp' and metric.get('data', {}).get('value', 0) > 4000:
                logger.warning(f"High LCP detected: {metric['data']['value']}ms for page {metric.get('page')}")
            
            if metric.get('type') == 'api_response' and metric.get('data', {}).get('duration', 0) > 2000:
                logger.warning(f"Slow API response detected: {metric['data']['duration']}ms for {metric['data'].get('url')}")
...
async def performance_monitoring_health() -> Dict[str, Any]:
    ...
    return {
        ...
        "last_metric_time": max([m.get('timestamp', 0) for m in performance_metrics_store]) if performance_metrics_store else None,
