...
from pynomaly.infrastructure.adapters.model_trainer_adapter import create_model_trainer_adapter
...
class TrainingConfigurationRequest(BaseModel):
    ...
    max_trials: int = Field(default=100, ge=10, le=1000, description="Maximum number of optimization trials")
    timeout_minutes: Optional[int] = Field(default=60, ge=1, le=1440, description="Timeout in minutes")
    optimization_strategy: OptimizationStrategy = Field(default=OptimizationStrategy.TPE, description="Optimization strategy")
    pruning_strategy: PruningStrategy = Field(default=PruningStrategy.MEDIAN, description="Pruning strategy for early stopping")
    
    # Optimization objectives
    primary_metric: str = Field(default="roc_auc", description="Primary optimization metric")
    secondary_metrics: List[str] = Field(default=["precision", "recall", "f1"], description="Secondary metrics to track")
    optimization_direction: str = Field(default="maximize", regex="^(maximize|minimize)$", description="Optimization direction")
    ...
    min_improvement_threshold: float = Field(default=0.001, ge=0.0, le=0.1, description="Minimum improvement threshold")
    
    # Resource constraints
    max_memory_gb: Optional[float] = Field(default=None, ge=0.1, le=1024, description="Maximum memory usage in GB")
    max_cpu_cores: Optional[int] = Field(default=None, ge=1, le=64, description="Maximum CPU cores")
    ...
    # Algorithm selection
    algorithm_whitelist: Optional[List[str]] = Field(default=None, description="Allowed algorithms")
    algorithm_blacklist: Optional[List[str]] = Field(default=None, description="Excluded algorithms")
    ensemble_methods: List[str] = Field(default=["voting", "stacking"], description="Ensemble methods to try")
...
class CreateTrainingJobRequest(BaseModel):
    ...
    target_algorithms: Optional[List[str]] = Field(default=None, description="Target algorithms")
    configuration: TrainingConfigurationRequest = Field(..., description="Training configuration")
...
class QuickOptimizeRequest(BaseModel):
    ...
    algorithms: Optional[List[str]] = Field(default=None, description="Target algorithms")
    max_trials: int = Field(default=50, ge=10, le=200, description="Maximum trials")
    timeout_minutes: int = Field(default=30, ge=5, le=120, description="Timeout in minutes")
...
async def list_training_jobs(
    status_filter: Optional[TrainingStatus] = Query(None, alias="status", description="Filter by status"),
...
@router.post("/quick-optimize", response_model=TrainingJobResponse, status_code=status.HTTP_201_CREATED)
...
async def get_training_system_status(
    ...
    try:
        ...
        status_info = {
            ...
            "supported_algorithms": len(create_model_trainer_adapter().get_supported_algorithms()),
