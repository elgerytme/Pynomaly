...
class TrainingWebSocketHandler:
    ...
    def __init__(self, training_service: AutomatedTrainingService):
        ...
        self.client_subscriptions: dict[str, set[str]] = {}  # client_id -> set of training_ids
    ...
    async def handle_message(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            if message_type == "start_training":
                ...
                await self._handle_subscribe_training_updates(websocket, client_id, message)
            else:
    ...
    async def _handle_start_training(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            objective = objective_map.get(msg.optimization_objective.lower(), OptimizationObjective.AUC)
            ...
            # Start training
            training_id = await self.training_service.schedule_training(config, TriggerType.MANUAL)
    ...
    async def _handle_cancel_training(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            if success:
                ...
                await self._send_error(websocket, f"Failed to cancel training {msg.training_id}")
                
        except Exception as e:
            await self._send_error(websocket, f"Failed to cancel training: {str(e)}")
    
    async def _handle_get_training_status(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            if status:
                ...
                await self._send_error(websocket, f"Training {msg.training_id} not found")
                
        except Exception as e:
            await self._send_error(websocket, f"Failed to get training status: {str(e)}")
    
    async def _handle_get_active_trainings(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            await self._send_error(websocket, f"Failed to get active trainings: {str(e)}")
    
    async def _handle_get_training_history(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            history = await self.training_service.get_training_history(detector_id, msg.limit)
            ...
            await self._send_error(websocket, f"Failed to get training history: {str(e)}")
    
    async def _handle_subscribe_training_updates(self, websocket: WebSocket, client_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            await self._send_error(websocket, f"Failed to subscribe to updates: {str(e)}")
    ...
    async def broadcast_training_update(self, training_id: str, update_data: dict[str, Any]):
        ...
        for client_id in subscribed_clients:
            ...
            try:
                ...
                logger.error(f"Failed to send training update to client {client_id}: {str(e)}")
