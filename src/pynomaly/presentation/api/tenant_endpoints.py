...
@router.post("/", response_model=TenantDetailResponse, status_code=status.HTTP_201_CREATED)
...
async def list_tenants(
    ...
    tier_filter: Optional[SubscriptionTier] = Query(None, alias="tier"),
...
async def update_tenant_status(
    ...
    try:
        if request.status == TenantStatus.ACTIVE:
            ...
            success = await tenant_service.suspend_tenant(tenant_id, request.reason or "")
...
async def get_tenant_usage(
    ...
    for quota_type, quota_info in usage_summary["quotas"].items():
        quota_responses[quota_type] = QuotaResponse(
            ...
            remaining=quota_info["remaining"] if quota_info["remaining"] != "unlimited" else -1,
