...
class SecurityAnalyzer:
    ...
    def analyze_request(self, request: Request, content: str = "") -> list[SecurityEvent]:
            ...
            for pattern in patterns:
                ...
                if re.search(pattern, analysis_data, re.IGNORECASE):
                    event = self._create_security_event(
                        ...
                        request=request,
                        details={"pattern_matched": pattern, "content_sample": analysis_data[:200]},
    ...
    def _check_heuristics(self, request: Request, data: str) -> list[SecurityEvent]:
        ...
        if self._is_suspicious_user_agent(user_agent):
            events.append(self._create_security_event(
    ...
    def _is_suspicious_user_agent(self, user_agent: str) -> bool:
        ...
        suspicious_patterns = [
            "nikto", "nmap", "dirb", "dirbuster", "gobuster",
            "sqlmap", "burp", "wpscan", "masscan", "zap",
    ...
    def _create_security_event(
        ...
        return SecurityEvent(
            ...
            blocked=response_action in [SecurityResponse.BLOCK_REQUEST, SecurityResponse.BLOCK_IP]
        )
    ...
    def _determine_response(
        ...
        if threat_level == SecurityThreatLevel.CRITICAL:
            if event_type in [SecurityEventType.SQL_INJECTION, SecurityEventType.MALWARE_UPLOAD]:
        ...
        # Medium threats - monitor and potentially block
            ...
            if event_type in [SecurityEventType.BRUTE_FORCE, SecurityEventType.DDOS_ATTEMPT]:
                return SecurityResponse.BLOCK_REQUEST
...
class SecurityAlertManager:
    ...
    def _generate_alert_message(self, event: SecurityEvent) -> str:
        ...
        return messages.get(
            event.event_type,
            f"Security event {event.event_type.value} detected from {event.source_ip}"
        )
    ...
    def _get_recommended_actions(self, event: SecurityEvent) -> list[str]:
        ...
        return actions.get(event.event_type, ["Review security logs", "Investigate the incident"])
...
class SecurityMonitor:
    ...
    async def process_request(self, request: Request, content: str = "") -> list[SecurityEvent]:
    ...
    def _check_rate_limit(self, client_ip: str) -> bool:
        ...
        for ip in list(self.request_counts.keys()):
            self.request_counts[ip] = {
                m: count for m, count in self.request_counts[ip].items()
                if m > cutoff
            }
            ...
            try:
                ...
                for ip in list(self.events_by_ip.keys()):
                    self.events_by_ip[ip] = [
                        event for event in self.events_by_ip[ip]
        ...
        for key, events in attack_groups.items():
            if len(events) >= 5:  # 5 or more similar events in same minute
                ...
                if unique_ips >= 3:  # From 3+ different IPs
                    ...
                    logger.warning(f"Distributed attack detected: {key} from {unique_ips} IPs")
    ...
    async def _analyze_traffic_patterns(self):
        ...
        if len(recent_events) > 100:  # Only analyze if we have enough data
            ...
            if events_per_minute:
                avg_per_minute = sum(events_per_minute.values()) / len(events_per_minute)
                max_per_minute = max(events_per_minute.values())
                
                # If max is 3x average, consider it suspicious
                if max_per_minute > avg_per_minute * 3:
                    logger.warning(f"Traffic spike detected: {max_per_minute} events/minute (avg: {avg_per_minute:.1f})")
