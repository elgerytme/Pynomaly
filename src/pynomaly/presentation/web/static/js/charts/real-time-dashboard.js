...
export class RealTimeDashboard {
  ...
  initializeCharts() {
    ...
    this.charts.set(
      "timeline",
      new AnomalyTimelineChart(timelineContainer, {
    ...
    this.charts.set(
      "heatmap",
      new AnomalyHeatmapChart(heatmapContainer, {
  ...
  setupEventHandlers() {
    ...
    this.container
      .querySelector("#timeline-interval")
    ...
    this.container
      .querySelector("#heatmap-aggregation")
      .addEventListener("change", (e) => {
  ...
  processHeatmapData(point) {
    ...
    if (existing) {
      ...
      existing.anomalyScore = Math.max(
  ...
  getTimeInterval(timestamp) {
    const aggregation = this.container.querySelector(
      "#heatmap-aggregation",
    ).value;
  ...
  updateMetrics() {
    ...
    const anomalyRate =
      timelineData.length > 0
        ? ((anomalyCount / timelineData.length) * 100).toFixed(1)
        : "0.0";
  ...
  updateDataRate() {
    ...
    const recentPoints = timelineData.filter(
      (d) => d.timestamp.getTime() > oneSecondAgo,
    ).length;
