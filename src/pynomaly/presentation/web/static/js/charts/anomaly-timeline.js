...
export class AnomalyTimelineChart {
  ...
  createSVG() {
    ...
    this.svg = this.container
      ...
      .attr('role', 'img')
      .attr('aria-label', 'Anomaly detection timeline showing normal and anomalous data points over time');
  ...
  renderData() {
    ...
    const pointsEnter = points.enter()
      ...
      .attr('fill', d => d.isAnomaly ? this.scales.color('anomaly') : this.scales.color('normal'))
      .attr('opacity', 0);
  ...
  renderAnomalyAnnotations() {
    ...
    const annotationsEnter = annotations.enter()
      ...
      .attr('transform', d => `translate(${this.scales.x(d.timestamp)}, ${this.scales.y(d.value)})`);
  ...
  renderLegend() {
    ...
    const legend = this.svg.selectAll('.legend')
      .data([
        ...
        { label: 'Anomaly', color: this.scales.color('anomaly'), shape: 'circle' }
      ]);
    
    const legendEnter = legend.enter()
      .append('g')
      .attr('class', 'legend')
      .attr('transform', (d, i) => `translate(${this.options.width - 70}, ${30 + i * 20})`);
  ...
  renderInsights() {
    ...
    // Add insights text
    const insights = this.svg.selectAll('.insights')
      .data([{
  ...
  handleZoom(event) {
    ...
    this.dataGroup.selectAll('.baseline')
      .attr('d', line);
    
    // Update annotations
    this.dataGroup.selectAll('.anomaly-annotation')
      .attr('transform', d => `translate(${newXScale(d.timestamp)}, ${this.scales.y(d.value)})`);
  }
  ...
  handlePointClick(event, data) {
    ...
    this.container.node().dispatchEvent(new CustomEvent('pointClicked', {
      detail: { data, event }
    }));
  }
