...
export class AnomalyHeatmapChart {
  ...
  createSVG() {
    ...
    this.svg = this.container
      ...
      .attr('role', 'img')
      .attr('aria-label', 'Anomaly heatmap showing anomaly scores across different features and time periods');
  ...
  renderCells() {
    ...
    cellsEnter.merge(cells)
      ...
      .attr('x', d => this.scales.x(d.x))
      .attr('y', d => this.scales.y(d.y))
      .attr('width', d => this.scales.x.bandwidth() * this.scales.size(d.anomalyScore))
      .attr('height', d => this.scales.y.bandwidth() * this.scales.size(d.anomalyScore))
      .attr('fill', d => this.scales.color(d.anomalyScore))
  ...
  renderAnomalyIndicators() {
    ...
    const anomalyCells = this.processedData.filter(d => d.anomalyScore >= highAnomalyThreshold);
    ...
    const indicatorsEnter = indicators.enter()
      ...
      .attr('transform', d => `translate(${this.scales.x(d.x) + this.scales.x.bandwidth()/2}, ${this.scales.y(d.y) + this.scales.y.bandwidth()/2})`);
    ...
    // Update indicators
    indicatorsEnter.merge(indicators)
      .transition()
      .duration(this.options.animationDuration)
      .attr('transform', d => `translate(${this.scales.x(d.x) + this.scales.x.bandwidth()/2}, ${this.scales.y(d.y) + this.scales.y.bandwidth()/2})`)
  ...
  renderLegend() {
    ...
    const stops = [
      ...
      { offset: '100%', color: this.scales.color(this.scales.color.domain()[0]) }
    ];
  ...
  renderStatistics() {
    ...
    const statsPanel = this.svg.append('g')
    ...
    statsPanel.selectAll('.stat-text')
      .data(statsText)
      .enter()
      .append('text')
  ...
  calculateStatistics() {
    ...
    const highAnomalies = this.processedData.filter(d => d.anomalyScore >= 0.8).length;
    const averageScore = d3.mean(this.processedData, d => d.anomalyScore) || 0;
  ...
  handleCellClick(event, data) {
    ...
    this.container.node().dispatchEvent(new CustomEvent('cellClicked', {
      detail: { data, event }
    }));
  }
  ...
  filterByScore(minScore, maxScore) {
    const filteredData = this.data.filter(d => 
      d.anomalyScore >= minScore && d.anomalyScore <= maxScore
    );
...
export function initializeHeatmap() {
  const container = document.querySelector('[data-component="anomaly-heatmap"]');
  if (!container) return;
...
function generateSampleHeatmapData() {
  const data = [];
  const features = ['Temperature', 'Pressure', 'Humidity', 'Vibration', 'Current', 'Voltage'];
