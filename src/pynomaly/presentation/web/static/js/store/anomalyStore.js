...
const createAnomalyStore = (set, get) => ({
  ...
  removeSelectedFeature: (feature) => set((state) => ({
    selectedFeatures: state.selectedFeatures.filter(f => f !== feature)
  })),
  
  setSelectedAnomalies: (anomalies) => set({ selectedAnomalies: anomalies }),
  
  addSelectedAnomaly: (anomaly) => set((state) => ({
    selectedAnomalies: [...state.selectedAnomalies, anomaly]
  })),
  
  removeSelectedAnomaly: (anomaly) => set((state) => ({
    selectedAnomalies: state.selectedAnomalies.filter(a => a.id !== anomaly.id)
  })),
  ...
  setSeverityThreshold: (threshold) => set((state) => ({
    filters: { ...state.filters, severityThreshold: threshold }
  })),
  
  setAnomalyTypes: (types) => set((state) => ({
    filters: { ...state.filters, anomalyTypes: types }
  })),
  
  setFeatureFilters: (featureFilters) => set((state) => ({
  ...
  addRealTimeError: (error) => set((state) => ({
    realTime: {
      ...state.realTime,
      errors: [...state.realTime.errors.slice(-9), { // Keep last 10 errors
  ...
  setAccessibilityPreference: (key, value) => set((state) => ({
    preferences: {
      ...state.preferences,
  ...
  setDashboardPreference: (key, value) => set((state) => ({
    preferences: {
      ...state.preferences,
  ...
  recordRenderTime: (component, time) => set((state) => ({
    performance: {
      ...state.performance,
  ...
  recordDataUpdateTime: (component, time) => set((state) => ({
    performance: {
      ...state.performance,
      dataUpdateTimes: { ...state.performance.dataUpdateTimes, [component]: time }
    }
  })),
  ...
  getFilteredData: () => {
    ...
    if (state.filters.severityThreshold > 0) {
      filtered = filtered.filter(point => 
        !point.isAnomaly || point.anomalyScore >= state.filters.severityThreshold
      );
    }
    ...
    if (state.filters.dateRange.start && state.filters.dateRange.end) {
      filtered = filtered.filter(point => {
        ...
        return timestamp >= state.filters.dateRange.start && 
    ...
    if (state.filters.anomalyTypes.length > 0) {
      filtered = filtered.filter(point => 
        !point.isAnomaly || state.filters.anomalyTypes.includes(point.anomalyType || 'unknown')
      );
    }
  ...
  getSelectedData: () => {
    ...
    if (state.selectedTimeRange.start && state.selectedTimeRange.end) {
      return state.timeSeries.filter(point => {
        const timestamp = new Date(point.timestamp);
        return timestamp >= state.selectedTimeRange.start && 
  ...
  getAnomalyStatistics: () => {
    ...
    return {
      ...
      severityDistribution: anomalies.reduce((acc, anomaly) => {
        const severity = anomaly.anomalyScore > 0.8 ? 'high' : 
                        anomaly.anomalyScore > 0.6 ? 'medium' : 'low';
  ...
  getPerformanceMetrics: () => {
    ...
    return {
      ...state.performance,
      averageRenderTime: Object.values(state.performance.renderTimes).length > 0 
        ? Object.values(state.performance.renderTimes).reduce((a, b) => a + b, 0) / Object.values(state.performance.renderTimes).length
        : 0,
      averageDataUpdateTime: Object.values(state.performance.dataUpdateTimes).length > 0
        ? Object.values(state.performance.dataUpdateTimes).reduce((a, b) => a + b, 0) / Object.values(state.performance.dataUpdateTimes).length
        : 0
    };
  }
});
...
class StoreDevTools {
  constructor(store, options = {}) {
    this.store = store;
    this.options = {
      enabled: typeof window !== 'undefined' && window.__REDUX_DEVTOOLS_EXTENSION__,
      name: 'Pynomaly Store',
      ...options
    };
...
const useAnomalyStore = {
  ...
  reset: () => {
    anomalyStore.setState(createAnomalyStore(() => {}, () => {}), true);
  },
