...
class ProgressTracker {
    ...
    updateProgressBar(element, progress, status) {
        ...
        if (progressBar && progressPercentage) {
            ...
            if (status === 'completed') {
                ...
                progressBar.className = 'progress-bar bg-blue-600 h-2 rounded-full transition-all duration-300';
            }
        }
    }
    ...
    updateTaskDetails(element, taskData) {
        ...
        if (!detailsElement) {
            ...
            detailsElement.className = 'task-details mt-4 p-3 bg-gray-50 rounded text-sm';
...
window.startAutoMLOptimization = function(formData) {
    ...
    const modalId = progressTracker.createProgressModal(taskId, 'AutoML Optimization');
