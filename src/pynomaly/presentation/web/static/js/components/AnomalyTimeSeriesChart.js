...
class AnomalyTimeSeriesChart {
  ...
  setupAccessibility() {
    ...
    this.container
      ...
      .attr("class", "sr-only")
      .text(
        "Interactive time series chart showing data points and detected anomalies over time. Use keyboard navigation to explore data points.",
      );
  ...
  updateScales() {
    ...
    this.yScale.domain([valueExtent[0] - padding, valueExtent[1] + padding]);
  }
  ...
  render() {
    ...
    // Update axes
    this.chartGroup.select(".x-axis").transition(t).call(this.xAxis);

    this.chartGroup.select(".y-axis").transition(t).call(this.yAxis);

    // Update grid
    this.chartGroup
      .select(".grid-horizontal")
      .transition(t)
      .call(d3.axisBottom(this.xScale).tickSize(-this.height).tickFormat(""));

    this.chartGroup
      .select(".grid-vertical")
      .transition(t)
      .call(d3.axisLeft(this.yScale).tickSize(-this.width).tickFormat(""));
  ...
  renderDataLine(transition) {
    const line = this.chartGroup.selectAll(".data-line").data([this.data]);
  ...
  renderAnomalies(transition) {
    const anomalyPoints = this.chartGroup
    ...
    anomalyPoints.exit().transition(transition).attr("r", 0).remove();
  }
  ...
  renderDataPoints(transition) {
    ...
    dataPoints.exit().transition(transition).attr("r", 0).remove();
  }
  ...
  handleZoom(event) {
    ...
    this.chartGroup.select(".data-line").attr("d", zoomedLine);
  ...
  navigateDataPoint(direction) {
    ...
    this.focusedPointIndex = Math.max(
      0,
      Math.min(this.data.length - 1, (this.focusedPointIndex || 0) + direction),
    );
  ...
  focusDataPoint(index) {
    ...
    this.chartGroup
      .selectAll(".data-point")
      .style("stroke", (d, i) =>
        i === index ? "var(--color-primary-500)" : "none",
      )
