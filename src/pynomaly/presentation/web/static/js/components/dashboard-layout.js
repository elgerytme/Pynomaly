...
export class DashboardLayoutEngine {
  ...
  createLayoutContainer() {
    this.container.className =
      `${this.container.className} dashboard-layout`.trim();
  ...
  setupEventHandlers() {
    // Add widget button
    this.container
      .querySelector('[data-action="add-widget"]')
    ...
    // Layout menu button
    this.container
      .querySelector('[data-action="layout-menu"]')
      .addEventListener("click", () => {
        this.showLayoutMenu();
      });
  ...
  findOptimalPosition(width, height) {
    ...
    const maxY = Math.max(
      0,
      ...this.layout.map((item) => item.y + item.height),
    );
    return { x: 0, y: maxY };
  }
  ...
  async loadWidgetContent(widget) {
    const contentContainer = widget.element.querySelector(
  ...
  handleDragMove(clientX, clientY) {
    ...
    const newX = Math.max(
      ...
      Math.min(
        ...
        Math.round(
          (startLayout.x * this.columnWidth + deltaX) / this.columnWidth,
        ),
      ),
    );
    ...
    const newY = Math.max(
      0,
      Math.round(
        (startLayout.y * this.options.gridSize + deltaY) /
    ...
    if (
      this.isPositionAvailable(
        ...
        this.getOccupiedCells().filter(
          (cell) => !this.isInWidget(cell, widget.layout),
        ),
      )
    ) {
  ...
  handleResizeMove(clientX, clientY) {
    ...
    const newWidth = Math.max(
      ...
      Math.min(
        ...
        Math.round(
          (startLayout.width * this.columnWidth + deltaX) / this.columnWidth,
        ),
      ),
    );
    ...
    const newHeight = Math.max(
      2,
      Math.round(
        (startLayout.height * this.options.gridSize + deltaY) /
          this.options.gridSize,
      ),
    );
...
export function initializeDashboardLayouts() {
  document
    .querySelectorAll('[data-component="dashboard-layout"]')
    .forEach((container) => {
