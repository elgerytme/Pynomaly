...
export class MultiStepForm {
  ...
  parseSteps() {
    this.steps = Array.from(this.container.querySelectorAll("[data-step]")).map(
      (stepElement, index) => ({
        element: stepElement,
        index,
  ...
  parseValidationRules(field) {
    ...
    if (field.type === "email" || field.hasAttribute("data-email")) {
      rules.push({
        type: "email",
        message:
          field.dataset.emailMessage || "Please enter a valid email address",
      });
    }
    ...
    if (field.type === "number") {
      if (field.min !== "") {
        rules.push({
          ...
          message:
            field.dataset.minMessage || `Value must be at least ${field.min}`,
        });
      }
      ...
      if (field.max !== "") {
        rules.push({
          ...
          message:
            field.dataset.maxMessage || `Value must be at most ${field.max}`,
        });
      }
    }
    ...
    if (field.pattern) {
      rules.push({
        ...
        message:
          field.dataset.patternMessage || "Please match the requested format",
      });
    }
      ...
      this.progressContainer.addEventListener("click", (e) => {
        ...
        if (stepElement) {
          ...
          if (
            ...
            this.isStepAccessible(stepIndex)
          ) {
  ...
  showStep(stepIndex) {
    ...
    const firstInput = this.steps[stepIndex].element.querySelector(
  ...
  updateProgress() {
    ...
    const progressSteps =
      this.progressContainer.querySelectorAll(".progress-step");
  ...
  updateNavigation() {
    ...
    if (prevButton) {
      prevButton.style.display =
        this.currentStep > 0 && this.options.allowBack
          ? "inline-block"
          : "none";
    }
...
export class FileUploadComponent {
  ...
  validateFile(file) {
    ...
    if (!this.options.allowedTypes.includes(extension)) {
      this.showError(
        `File type "${extension}" is not allowed. Supported formats: ${this.options.allowedTypes.join(", ")}.`,
      );
      return false;
    }
...
export function initializeInteractiveForms() {
  ...
  document
    .querySelectorAll('[data-component="multi-step-form"]')
    .forEach((container) => {
  ...
  document
    .querySelectorAll('[data-component="file-upload"]')
    .forEach((container) => {
