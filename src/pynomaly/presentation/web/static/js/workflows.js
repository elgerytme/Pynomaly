...
window.WorkflowManager = {
        ...
        const templates = [
            {
                ...
                steps: [
                    { id: 'load-data', type: 'data_input', name: 'Load Dataset', x: 100, y: 100 },
                    { id: 'train', type: 'training', name: 'Train Detector', x: 300, y: 100 },
                    { id: 'detect', type: 'detection', name: 'Detect Anomalies', x: 500, y: 100 },
                    { id: 'visualize', type: 'visualization', name: 'Visualize Results', x: 700, y: 100 }
                ],
                connections: [
            ...
            },
            {
                ...
                steps: [
                    { id: 'load-data', type: 'data_input', name: 'Load Dataset', x: 100, y: 200 },
                    { id: 'train1', type: 'training', name: 'Train Detector 1', x: 300, y: 150 },
                    { id: 'train2', type: 'training', name: 'Train Detector 2', x: 300, y: 250 },
                    { id: 'ensemble', type: 'ensemble', name: 'Ensemble Aggregation', x: 500, y: 200 },
                    { id: 'evaluate', type: 'evaluation', name: 'Evaluate Results', x: 700, y: 200 }
                ],
                connections: [
            ...
            },
            {
                ...
                steps: [
                    { id: 'load-data', type: 'data_input', name: 'Load Dataset', x: 100, y: 100 },
                    { id: 'preprocess', type: 'preprocessing', name: 'Preprocess Data', x: 300, y: 100 },
                    { id: 'automl', type: 'automl', name: 'AutoML Optimization', x: 500, y: 100 },
                    { id: 'validate', type: 'validation', name: 'Cross Validation', x: 700, y: 100 },
                    { id: 'deploy', type: 'deployment', name: 'Deploy Model', x: 900, y: 100 }
                ],
                connections: [
    ...
    setupEventListeners() {
        ...
        document.addEventListener('mousedown', (e) => {
            if (e.target.matches('.workflow-step') || e.target.closest('.workflow-step')) {
                this.startDrag(e);
            }
        });
    ...
    renderConnection(connection) {
        ...
        const fromElement = document.querySelector(`[data-step-id="${connection.from}"]`);
        const toElement = document.querySelector(`[data-step-id="${connection.to}"]`);
    ...
    getStepIcon(type) {
        const icons = {
            data_input: '<svg class="w-6 h-6 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z"/></svg>',
            training: '<svg class="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
            detection: '<svg class="w-6 h-6 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M12.432 0c1.34 0 2.01.912 2.01 1.957 0 1.305-1.164 2.512-2.679 2.512-1.269 0-2.009-.75-1.974-1.99C9.789 1.436 10.67 0 12.432 0zM8.309 20c-1.058 0-1.833-.652-1.093-3.524l1.214-5.092c.211-.814.246-1.141 0-1.141-.317 0-1.689.562-2.502 1.117l-.528-.88c2.572-2.186 5.531-3.467 6.801-3.467 1.057 0 1.233 1.273.705 3.23l-1.391 5.352c-.246.945-.141 1.271.106 1.271.317 0 1.357-.392 2.379-1.207l.6.814C12.098 19.02 9.365 20 8.309 20z"/></svg>',
            visualization: '<svg class="w-6 h-6 text-purple-500" fill="currentColor" viewBox="0 0 20 20"><path d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z"/></svg>',
            ensemble: '<svg class="w-6 h-6 text-red-500" fill="currentColor" viewBox="0 0 20 20"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"/></svg>',
            evaluation: '<svg class="w-6 h-6 text-indigo-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>',
            preprocessing: '<svg class="w-6 h-6 text-cyan-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"/></svg>',
            automl: '<svg class="w-6 h-6 text-orange-500" fill="currentColor" viewBox="0 0 20 20"><path d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z"/></svg>',
            validation: '<svg class="w-6 h-6 text-pink-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>',
            deployment: '<svg class="w-6 h-6 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M2 5a2 2 0 012-2h8a2 2 0 012 2v10a2 2 0 002 2H4a2 2 0 01-2-2V5zm3 1h6v4H5V6zm6 6H5v2h6v-2z" clip-rule="evenodd"/><path d="M15 7h1a2 2 0 012 2v5.5a1.5 1.5 0 01-3 0V9a1 1 0 00-1-1h-1v-1z"/></svg>'
        };
    ...
    saveCurrentWorkflow() {
        ...
        localStorage.setItem('pynomaly-workflows', JSON.stringify(Array.from(this.workflows.entries())));
