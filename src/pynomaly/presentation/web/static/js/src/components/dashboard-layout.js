...
export class DashboardLayout {
    ...
    detectBreakpoint() {
        ...
        for (const [breakpoint, minWidth] of Object.entries(this.options.breakpoints)) {
            if (width >= minWidth) {
    ...
    render() {
        ...
        const existingWidgets = this.container.querySelectorAll('.dashboard-widget');
        existingWidgets.forEach(w => w.remove());
        
        // Calculate grid dimensions
        const containerWidth = this.container.clientWidth - (this.options.containerPadding[0] * 2);
        const colWidth = (containerWidth - (this.options.margin[0] * (this.options.columns - 1))) / this.options.columns;
        ...
        if (this.options.autoSize) {
            const maxY = Math.max(0, ...this.layout.map(w => w.y + w.h));
            const containerHeight = (maxY * (this.options.rowHeight + this.options.margin[1])) + 
    ...
    createWidgetElement(widget, colWidth) {
        ...
        const height = (widget.h * this.options.rowHeight) + ((widget.h - 1) * this.options.margin[1]);
        ...
        // Add action buttons
        if (!widget.static) {
            const removeBtn = this.createActionButton('×', () => this.removeWidget(widget.id));
    ...
    startDrag(e, widget) {
        ...
        const widgetElement = this.container.querySelector(`[data-widget-id="${widget.id}"]`);
    ...
    handleDrag(e) {
        ...
        const containerRect = this.container.getBoundingClientRect();
        const colWidth = (this.container.clientWidth - (this.options.containerPadding[0] * 2) - 
                         (this.options.margin[0] * (this.options.columns - 1))) / this.options.columns;
        
        // Calculate grid position
        const x = Math.round((e.clientX - containerRect.left - this.dragOffset.x) / (colWidth + this.options.margin[0]));
        const y = Math.round((e.clientY - containerRect.top - this.dragOffset.y) / (this.options.rowHeight + this.options.margin[1]));
        
        // Constrain to grid bounds
        const constrainedX = Math.max(0, Math.min(x, this.options.columns - this.draggedWidget.w));
    ...
    handleResize(e) {
        ...
        if (direction.includes('e')) {
            const newWidth = e.clientX - containerRect.left - (widget.x * (colWidth + this.options.margin[0]));
            newW = Math.max(widget.minW, Math.min(widget.maxW, 
                Math.round(newWidth / (colWidth + this.options.margin[0]))));
        }
        
        if (direction.includes('s')) {
            const newHeight = e.clientY - containerRect.top - (widget.y * (this.options.rowHeight + this.options.margin[1]));
            newH = Math.max(widget.minH, Math.min(widget.maxH,
                Math.round(newHeight / (this.options.rowHeight + this.options.margin[1]))));
        }
        
        // Update widget size temporarily for visual feedback
        const widgetElement = this.container.querySelector(`[data-widget-id="${widget.id}"]`);
        const width = (newW * colWidth) + ((newW - 1) * this.options.margin[0]);
        const height = (newH * this.options.rowHeight) + ((newH - 1) * this.options.margin[1]);
    ...
    endResize() {
        ...
        widget.h = Math.round(elementHeight / (this.options.rowHeight + this.options.margin[1]));
    ...
    resolveCollisions(movedWidget) {
        ...
        const collisions = this.layout.filter(w => 
            w.id !== movedWidget.id && 
            !w.static && 
    ...
    updatePlaceholderPosition() {
        ...
        const y = (this.placeholder.y * (this.options.rowHeight + this.options.margin[1]));
        const width = (this.placeholder.w * colWidth) + ((this.placeholder.w - 1) * this.options.margin[0]);
        const height = (this.placeholder.h * this.options.rowHeight) + ((this.placeholder.h - 1) * this.options.margin[1]);
