...
class EChartsDemo {
  ...
  createPerformanceDemo() {
    ...
    const chart = new PerformanceMetricsChart('#performance-chart', {
      ...
      description: 'Real-time monitoring of system CPU, memory, and network utilization',
  ...
  setupPerformanceControls() {
    document.getElementById('performance-animation')?.addEventListener('change', (e) => {
    ...
    document.getElementById('performance-max-points')?.addEventListener('change', (e) => {
      const chart = this.charts.get('performance');
      if (chart) {
        chart.options.maxDataPoints = parseInt(e.target.value);
        chart.setData(this.demoData.performance.slice(-chart.options.maxDataPoints));
      }
    });
    ...
    document.getElementById('performance-metrics')?.addEventListener('change', (e) => {
  ...
  createAnomalyDistributionDemo() {
    ...
    const chart = new AnomalyDistributionChart('#anomaly-distribution-chart', {
      ...
      description: 'Pie chart showing distribution of anomaly types detected in the system',
      chartType: 'pie',
      animation: true
    });
  ...
  setupDistributionControls() {
    ...
    document.getElementById('distribution-animation')?.addEventListener('change', (e) => {
      const chart = this.charts.get('distribution');
      if (chart) {
        chart.options.animation = e.target.checked;
        chart.updateChart();
    ...
    document.getElementById('distribution-randomize')?.addEventListener('click', () => {
  ...
  updateDistributionStats() {
    ...
    document.getElementById('avg-confidence-value').textContent = (avgConfidence * 100).toFixed(1) + '%';
  ...
  createTimelineDemo() {
    ...
    const chart = new DetectionTimelineChart('#detection-timeline-chart', {
      ...
      description: 'Stacked bar chart showing anomaly detection events over time by severity level',
      timeRange: '24h',
  ...
  setupTimelineControls() {
    document.getElementById('timeline-time-range')?.addEventListener('change', (e) => {
    ...
    document.getElementById('timeline-severity')?.addEventListener('change', (e) => {
      const chart = this.charts.get('timeline');
      if (chart) {
    ...
    document.getElementById('timeline-refresh')?.addEventListener('click', () => {
  ...
  setupDashboardControls() {
    ...
    document.getElementById('dashboard-realtime')?.addEventListener('change', (e) => {
      if (e.target.checked) {
  ...
  updateDashboardMetrics() {
    ...
    const avgConfidence = this.demoData.anomalies.reduce((sum, d) => sum + d.confidence, 0) / totalAnomalies;
    const criticalAlerts = this.demoData.anomalies.filter(d => d.confidence > 0.95).length;
    const systemHealth = criticalAlerts > 5 ? 'Critical' : criticalAlerts > 2 ? 'Warning' : 'Good';

    document.getElementById('dashboard-total-anomalies').textContent = totalAnomalies;
    document.getElementById('dashboard-avg-confidence').textContent = (avgConfidence * 100).toFixed(1) + '%';
    document.getElementById('dashboard-critical-alerts').textContent = criticalAlerts;
    document.getElementById('dashboard-system-health').textContent = systemHealth;
  }
  ...
  startDashboardRealTime() {
    ...
    this.dashboardTimer = setInterval(() => {
      ...
      if (Math.random() < 0.1) {
        const newAnomaly = {
          id: Date.now(),
          type: ['Statistical Outlier', 'Temporal Anomaly', 'Pattern Deviation'][Math.floor(Math.random() * 3)],
  ...
  exportDashboardData() {
    const exportData = {
      ...
      metrics: {
        ...
        avgConfidence: this.demoData.anomalies.reduce((sum, d) => sum + d.confidence, 0) / this.demoData.anomalies.length,
        criticalAlerts: this.demoData.anomalies.filter(d => d.confidence > 0.95).length
      }
    };
  ...
  setupEventListeners() {
    // Real-time toggle
    document.getElementById('echarts-realtime-toggle')?.addEventListener('click', (e) => {
      this.toggleRealTime();
      e.target.textContent = this.isRealTimeEnabled ? 'Stop Real-Time' : 'Start Real-Time';
    });

    // Theme toggle
    document.getElementById('echarts-theme-toggle')?.addEventListener('click', (e) => {
      const newTheme = echartsManager.currentTheme === 'light' ? 'dark' : 'light';
      this.switchTheme(newTheme);
      e.target.textContent = `Switch to ${newTheme === 'light' ? 'Dark' : 'Light'}`;
    });

    // Refresh data
    document.getElementById('echarts-refresh-data')?.addEventListener('click', () => {
      this.refreshAllData();
    });

    // Export charts
    document.getElementById('echarts-export-charts')?.addEventListener('click', () => {
      this.exportCharts();
    });
  }
  ...
  toggleRealTime() {
    if (this.isRealTimeEnabled) {
      ...
      const interval = parseInt(document.getElementById('echarts-update-interval')?.value || '2000');
  ...
  updateRealTimeData() {
    ...
    if (Math.random() < 0.05) {
      ...
      if (newAnomaly.confidence > 0.9) {
        this.announceToScreenReader(`High confidence anomaly detected: ${newAnomaly.type}, confidence ${(newAnomaly.confidence * 100).toFixed(1)}%`);
      }
    }
  }
