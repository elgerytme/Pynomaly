  ...
  constructor() {
    ...
    this.themes = {
      light: {
        ...
        colorBy: 'series',
        color: ['#0ea5e9', '#22c55e', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4']
      },
      dark: {
        ...
        colorBy: 'series',
        color: ['#38bdf8', '#4ade80', '#fbbf24', '#f87171', '#a78bfa', '#22d3ee']
      }
    };
...
class BaseEChart {
  ...
  setupAccessibility() {
    ...
    this.container.setAttribute('aria-label', this.options.title || 'Chart visualization');
    ...
    if (this.options.description) {
      ...
      if (!descElement) {
        ...
        this.container.parentNode.insertBefore(descElement, this.container.nextSibling);
      }
  ...
  setupEventListeners() {
    ...
    this.chart.on('click', (params) => {
      this.container.dispatchEvent(new CustomEvent('chart-click', {
    ...
    this.chart.on('mouseover', (params) => {
      this.container.dispatchEvent(new CustomEvent('chart-hover', {
        detail: { params, chart: this }
      }));
    });
...
class PerformanceMetricsChart extends BaseEChart {
  constructor(container, options = {}) {
    super(container, {
      ...
      description: 'Real-time visualization of CPU, memory, and network utilization',
  ...
  addRandomDataPoint() {
    ...
    this.options.metrics.forEach(metric => {
      switch (metric) {
        case 'cpu':
          newPoint[metric] = Math.max(0, Math.min(100, 
            (this.data.length > 0 ? this.data[this.data.length - 1][metric] : 50) + 
            (Math.random() - 0.5) * 20
          ));
          break;
        case 'memory':
          newPoint[metric] = Math.max(0, Math.min(100,
            (this.data.length > 0 ? this.data[this.data.length - 1][metric] : 60) + 
            (Math.random() - 0.5) * 10
          ));
          break;
        case 'network':
          newPoint[metric] = Math.max(0, Math.min(100, Math.random() * 30 + 10));
          break;
        default:
    ...
    if (this.options.accessibility) {
      const highUsage = this.options.metrics.find(metric => newPoint[metric] > 90);
      if (highUsage) {
  ...
  generateDataAnnouncement() {
    ...
    const metrics = this.options.metrics.map(metric => 
      `${metric}: ${(latest[metric] || 0).toFixed(1)}%`
    ).join(', ');
...
class AnomalyDistributionChart extends BaseEChart {
  constructor(container, options = {}) {
    super(container, {
      ...
      description: 'Statistical distribution of detected anomalies by type and confidence',
  ...
  updatePieChart() {
    ...
    const option = {
      ...
      tooltip: {
        trigger: 'item',
        formatter: (params) => {
          const percentage = ((params.value / this.data.length) * 100).toFixed(1);
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          const percentage = ((param.value / this.data.length) * 100).toFixed(1);
  ...
  generateDataAnnouncement() {
    ...
    const avgConfidence = this.data.reduce((sum, d) => sum + (d.confidence || 0), 0) / this.data.length;
...
class DetectionTimelineChart extends BaseEChart {
  constructor(container, options = {}) {
    super(container, {
      ...
      description: 'Chronological view of anomaly detection events with severity indicators',
    ...
    const option = {
      ...
      xAxis: {
        ...
        axisLabel: {
          formatter: (value) => {
            const date = new Date(value);
            if (this.options.timeRange === '1h' || this.options.timeRange === '6h') {
              return date.toLocaleTimeString();
