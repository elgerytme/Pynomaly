    ...
    reduce(state, action) {
        ...
        switch (action.type) {
            ...
            case '@@STATE/SET_PROPERTY':
                return this.setNestedProperty(state, action.payload.path, action.payload.value);
...
export class StateHooks {
    ...
    useState(component, selector = (state) => state) {
        ...
        if (!componentData.unsubscribe) {
            componentData.unsubscribe = this.stateManager.subscribe((state, prevState, action) => {
