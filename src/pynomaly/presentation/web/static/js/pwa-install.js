...
class PWAManager {
  ...
  createInstallBanner() {
    ...
    banner.className = 'fixed bottom-4 left-4 right-4 bg-white border border-gray-200 rounded-lg shadow-lg p-4 hidden z-50';
    banner.innerHTML = `
   ...
   */
  showOfflineMessage() {
    this.showToast('You\'re now offline. The app will continue to work with cached data.', 'warning');
  }
  ...
  showUpdateAvailable() {
    ...
    updateBanner.className = 'fixed top-4 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50';
  ...
  showInstallSuccessMessage() {
    this.showToast('Pynomaly has been installed! You can now access it from your home screen.', 'success');
  }
  ...
  showNotificationPermissionBanner() {
    const banner = document.createElement('div');
    banner.className = 'fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg shadow-lg p-4 max-w-sm z-50';
    banner.innerHTML = `
      <div class="flex items-start gap-3">
        <div class="flex-shrink-0">
  ...
  async requestNotificationPermission() {
    try {
      ...
      if (permission === 'granted') {
        ...
        this.showToast('Notifications enabled! You\'ll receive updates about your analysis.', 'success');
        
        // Subscribe to push notifications if supported
        await this.subscribeToPushNotifications();
      } else {
        console.log('[PWA] Notification permission denied');
        this.showToast('Notification permission denied. You can enable it later in settings.', 'info');
      }
    } catch (error) {
  ...
  handleServiceWorkerMessage(data) {
    switch (data.type) {
      ...
      case 'UPLOAD_COMPLETE':
        ...
        break;
      case 'SYNC_ERROR':
        this.showToast('Failed to sync data. Will retry when connection improves.', 'warning');
        break;
    }
  }
