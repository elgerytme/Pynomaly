...
function createScatterPlot(element, data) {
  ...
  g.selectAll(".point")
    ...
    .on("mouseover", function (event, d) {
      ...
      tooltip
        .html(
          `X: ${d.x.toFixed(2)}<br>Y: ${d.y.toFixed(2)}<br>Score: ${(d.score || 0).toFixed(3)}`,
        )
...
function showLoading(element) {
  ...
  loadingDiv.className =
    "htmx-indicator absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center";
