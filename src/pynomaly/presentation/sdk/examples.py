...
from . import AsyncPynomaliClient, PynomaliClient
...
def basic_synchronous_example():
    ...
    try:
        ...
        print(
            f"Generated dataset with {len(all_data)} samples and {all_data.shape[1]} features"
        )
        ...
        print(
            f"Detection results: {result.num_anomalies}/{result.num_samples} anomalies"
        )
        ...
        sorted_scores = sorted(
            enumerate(result.anomaly_scores), key=lambda x: x[1].value, reverse=True
        )
...
async def basic_asynchronous_example():
    ...
    async with AsyncPynomaliClient(
        ...
        results = await client.batch_detect_concurrent(
            detector_id=detectors[0].id, data_sources=test_batches, max_concurrent=3
        )

        print(f"Completed {len(results)} detections concurrently")
        for i, result in enumerate(results):
            print(
                f"  Batch {i + 1}: {result.num_anomalies}/{result.num_samples} anomalies"
            )
...
async def streaming_detection_example():
    ...
    async with AsyncPynomaliClient(
        ...
        async for result in client.stream_detection(
            ...
            print(
                f"Batch: {result.num_anomalies}/{result.num_samples} anomalies "
                f"(Total: {anomaly_count}/{total_samples})"
            )

        print(
            f"Streaming completed. Overall anomaly rate: {anomaly_count / total_samples:.2%}"
        )
...
def configuration_example():
    ...
    client1 = PynomaliClient(base_url="http://localhost:8000", api_key="direct-api-key")
    print("Client 1: Direct configuration")

    # Method 2: Configuration object
    config = SDKConfig(
        base_url="http://api.example.com", api_key="config-api-key", auth_type="api_key"
    )
...
def batch_processing_example():
    ...
    try:
        ...
        results = client.batch_detect(
            detector_id=detector.id, data_sources=test_datasets, return_scores=True
        )

        print(f"Batch processing completed for {len(results)} datasets:")
        for i, result in enumerate(results):
            print(
                f"  Dataset {i + 1}: {result.num_anomalies}/{result.num_samples} anomalies "
