...
class AsyncPynomaliClient:
    ...
    async def _make_request(
        ...
        try:
            ...
            async with session.request(
                method=method, url=url, json=data, params=params, timeout=timeout_obj
            ) as response:
    ...
    async def detect_anomalies(
        ...
        response = await self._make_request(
            "POST", "detection/detect", data=request_data
        )
    ...
    async def batch_detect_concurrent(
        self,
        detector_id: str,
        ...
        async for data_batch in data_stream:
            ...
            if len(buffer) >= buffer_size:
                ...
                result = await self.detect_anomalies(
                    detector_id=detector_id, data=buffer, return_scores=return_scores
                )
                yield result
                buffer = []
