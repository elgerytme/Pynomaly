    ...
    async def run_audit_report():
        ...
        with Progress(
            ...
            if actions:
                for action in actions:
                    try:
                        ...
                        console.print(f"[yellow]Warning: Unknown action '{action}' ignored[/yellow]")
            ...
            try:
                ...
                if output_file:
                    task3 = progress.add_task(f"Saving report as {report_format}...", total=None)
...
@click.option("--policy-type", type=click.Choice([
    "data_governance", "access_control", "change_management", 
    "risk_management", "compliance", "quality_assurance", "security", "privacy"
]), required=True, help="Type of governance policy")
...
@click.option("--compliance-frameworks", multiple=True, help="Applicable compliance frameworks")
@click.option("--enforcement-level", type=click.Choice([
    "mandatory", "recommended", "optional"
]), default="mandatory", help="Policy enforcement level")
def create_policy(
    policy_name: str,
    policy_type: str,
    description: str,
        ...
        with Progress(
            ...
            for framework in compliance_frameworks:
                try:
                    ...
                    console.print(f"[yellow]Warning: Unknown framework '{framework}' ignored[/yellow]")
...
@click.option("--likelihood", type=click.Choice([
    "very_low", "low", "medium", "high", "very_high", "critical"
]), required=True, help="Probability of risk occurrence")
@click.option("--impact", type=click.Choice([
    "very_low", "low", "medium", "high", "very_high", "critical"
]), required=True, help="Potential impact if risk occurs")
        ...
        with Progress(
            ...
            try:
                assessment_id = await governance_service.conduct_risk_assessment(
                    ...
                    existing_controls=list(existing_controls) if existing_controls else None
                )
...
@click.option("--change-type", type=click.Choice([
    "configuration", "policy", "system", "data", "process"
]), required=True, help="Type of change being requested")
@click.option("--approvers", multiple=True, required=True, help="Required approvers for the change")
@click.option("--urgency", type=click.Choice([
    "low", "normal", "high", "emergency"
]), default="normal", help="Urgency level of the change")
...
@click.option("--decision", type=click.Choice([
    "approved", "rejected"
]), required=True, help="Approval decision")
@click.option("--comments", help="Comments for the approval decision")
def approve_change(request_id: str, decision: str, comments: Optional[str]):
    ...
    async def run_approve_change():
        ...
        with Progress(
            ...
            try:
                ...
                final_status = "Fully Approved" if is_approved else f"Decision: {decision.title()}"
...
@click.option("--responsible-party", required=True, help="Person responsible for the metric")
...
@click.option("--action", type=click.Choice([
    ...
    "escalate", "audit", "monitor", "report"
]), required=True, help="Governance action performed")
...
@click.option("--risk-level", type=click.Choice([
    "very_low", "low", "medium", "high", "very_high", "critical"
...
def _display_governance_dashboard(dashboard_data: dict):
    ...
    rate_color = "green" if compliance_rate >= 95 else "yellow" if compliance_rate >= 80 else "red"
    
    compliance_table.add_row("Compliance Rate", f"[{rate_color}]{compliance_rate:.1f}%[/{rate_color}]")
    compliance_table.add_row("Total Metrics", str(compliance_data.get("total_metrics", 0)))
...
def _save_audit_report_csv(report: dict, output_file: str):
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        # Write header
        writer.writerow([
            'Entry ID', 'Timestamp', 'User ID', 'Action', 'Resource Type', 
            'Resource ID', 'Details', 'Risk Level', 'Compliance Frameworks'
        ])
