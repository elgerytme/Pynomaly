...
def start_server(
    ...
    # Start server
    console.print(f"[green]Starting Pynomaly API server...[/green]")
    ...
    console.print(f"Health check: http://{settings.api_host}:{settings.api_port}/health")
    ...
    if pid_file.exists():
        try:
            ...
            try:
                ...
                console.print("[yellow]⚠[/yellow] PID file exists but process not found")
                pid_file.unlink()
    ...
    for endpoint, name in endpoints:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console.print(f"[yellow]⚠[/yellow] {name}: Status {response.status_code}")
                all_healthy = False
        except requests.exceptions.ConnectionError:
