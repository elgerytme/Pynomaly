...
from pynomaly.application.services.performance_benchmarking_service import (
    PerformanceBenchmarkingService, BenchmarkConfig, PerformanceMetrics,
    BenchmarkSuite
)
...
@click.option("--feature-dimensions", multiple=True, type=int, help="Feature dimensions to test")
@click.option("--contamination-rates", multiple=True, type=float, help="Contamination rates to test")
...
@click.option("--export-format", type=click.Choice(["json", "csv", "html"]), 
              default="html", help="Export format for results")
def comprehensive(
    suite_name: str,
    ...
    async def run_comprehensive_benchmark():
        ...
        with Progress(
            ...
            config = BenchmarkConfig(
                ...
                dataset_sizes=list(dataset_sizes) if dataset_sizes else [1000, 5000, 10000],
                feature_dimensions=list(feature_dimensions) if feature_dimensions else [10, 50, 100],
                contamination_rates=list(contamination_rates) if contamination_rates else [0.05, 0.1, 0.15],
                algorithms=list(algorithms) if algorithms else ["IsolationForest", "LOF", "OneClassSVM"],
            ...
            task3 = progress.add_task("Running comprehensive benchmarks...", total=len(config.algorithms))
            ...
            try:
                ...
                # Generate report
                if output_dir:
                    task4 = progress.add_task(f"Generating {export_format} report...", total=None)
                    ...
                    console.print(f"[green]Benchmark report saved to: {report_path}[/green]")
        ...
        with Progress(
            ...
            try:
                ...
                if output_file:
                    ...
                    console.print(f"[green]Scalability results saved to: {output_file}[/green]")
...
@click.option("--max-size", type=int, default=1000000, help="Maximum dataset size to test")
        ...
        with Progress(
            ...
            try:
                ...
                if output_file:
                    with open(output_file, 'w') as f:
                        json.dump(results, f, indent=2, default=str)
                    console.print(f"[green]Memory stress results saved to: {output_file}[/green]")
...
def throughput(
    ...
    async def run_throughput_test():
        ...
        with Progress(
            ...
            algorithm_list = list(algorithms) if algorithms else ["IsolationForest", "LOF", "OneClassSVM"]
            size_list = list(dataset_sizes) if dataset_sizes else [1000, 5000, 10000, 25000]
            ...
            try:
                ...
                if output_file:
                    with open(output_file, 'w') as f:
                        json.dump(results, f, indent=2, default=str)
                    console.print(f"[green]Throughput results saved to: {output_file}[/green]")
...
@click.option("--dataset-sizes", multiple=True, type=int, help="Dataset sizes for comparison")
...
def compare(
    ...
    async def run_algorithm_comparison():
        ...
        with Progress(
            ...
            metric_list = list(metrics) if metrics else ["execution_time", "memory_usage", "accuracy", "throughput"]
...
def list_results(benchmark_dir: Optional[str]):
    """List available benchmark results."""
    
    benchmark_path = Path(benchmark_dir) if benchmark_dir else Path("./benchmarks")
    
    if not benchmark_path.exists():
        console.print(f"[yellow]Benchmark directory not found: {benchmark_path}[/yellow]")
        return
    ...
    if json_files or html_files or csv_files:
        ...
        all_files = [(f, "JSON") for f in json_files] + [(f, "HTML") for f in html_files] + [(f, "CSV") for f in csv_files]
...
def _display_benchmark_summary(suite: BenchmarkSuite):
    ...
    if suite.comparative_analysis:
        ...
        console.print(f"• Fastest Algorithm: [green]{analysis.get('fastest_algorithm', 'N/A')}[/green]")
        console.print(f"• Most Memory Efficient: [green]{analysis.get('most_memory_efficient', 'N/A')}[/green]")
        console.print(f"• Highest Throughput: [green]{analysis.get('highest_throughput', 'N/A')}[/green]")
        console.print(f"• Most Accurate: [green]{analysis.get('most_accurate', 'N/A')}[/green]")
...
def _display_scalability_results(results: dict):
    ...
    if 'scalability_summary' in results:
        ...
        console.print(f"• Linear Scalability Score: {summary.get('linear_scalability_score', 0):.3f}")
        console.print(f"• Time Complexity Estimate: {summary.get('time_complexity_estimate', 'unknown')}")
        console.print(f"• Scalability Grade: {summary.get('scalability_grade', 'N/A')}")
        console.print(f"• Max Efficient Scale: {summary.get('max_efficient_scale', 'N/A')}")
...
def _display_memory_stress_results(results: dict):
    ...
    if 'memory_analysis' in results:
        ...
        console.print(f"• Memory Efficiency: {analysis.get('memory_efficiency', 0):.3f} MB per 1000 samples")
        console.print(f"• Average Memory Growth: {analysis.get('memory_growth_rate', 0):.1f} MB")
        console.print(f"• Max Memory Tested: {analysis.get('max_memory_tested', 0):.1f} MB")
        console.print(f"• Memory Scalability: {analysis.get('memory_scalability', 'unknown')}")
...
def _display_throughput_results(results: dict):
    ...
    for algorithm, stats in analysis.items():
        if algorithm != 'overall_best':
            console.print(f"\n{algorithm}:")
            console.print(f"  • Average Throughput: {stats.get('avg_throughput', 0):.1f} samples/s")
            console.print(f"  • Max Throughput: {stats.get('max_throughput', 0):.1f} samples/s")
            console.print(f"  • Throughput Stability: {stats.get('throughput_stability', 0):.3f}")
            console.print(f"  • Best Dataset Size: {stats.get('best_dataset_size', 'N/A')}")
