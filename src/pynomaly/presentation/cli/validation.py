...
class RichOutputFormatter:
    ...
    def display_results(self, result: ValidationResult):
        ...
        for severity in [
            ...
            self.console.print(
                f"[{color}]{severity.value.upper()} VIOLATIONS ({len(violations)})[/{color}]"
            )
    ...
    def _display_violation(
        self, violation: ValidationViolation, index: int, color: str
    ):
...
"""
    ...
    def _generate_failure_comment(self, result: ValidationResult) -> str:
        ...
        if all_violations:
            sections.append(
                f"### 🔍 Top {min(self.max_violations, len(all_violations))} Violations"
            )
            sections.append("")

            for i, violation in enumerate(all_violations[: self.max_violations], 1):
                sections.append(
                    f"#### {i}. {violation.severity.value.upper()}: {violation.message}"
                )
    ...
    def post_to_github(self, comment: str) -> bool:
        ...
        if not all([github_token, github_repository, pr_number]):
            console.print(
                "⚠️ GitHub environment variables not found. Skipping PR comment."
            )
            return False
        ...
        try:
            ...
            console.print(
                "⚠️ requests library not available. Install with: pip install requests"
            )
            return False
        except Exception as e:
...
def run_validation(
    ...
    github_comment: bool = typer.Option(
        False, "--github-comment", help="Post results to GitHub PR"
    ),
    fail_fast: bool = typer.Option(
        False, "--fail-fast", help="Stop on first critical violation"
    ),
    output_format: str = typer.Option(
        "rich", "--format", help="Output format: rich, json, markdown"
    ),
    save_report: Path | None = typer.Option(None, "--save", help="Save report to file"),
) -> None:
