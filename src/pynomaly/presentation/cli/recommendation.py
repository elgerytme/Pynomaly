...
from pynomaly.application.dto.configuration_dto import (
    DatasetCharacteristicsDTO, ConfigurationLevel
)
...
def recommend_for_dataset(
    ...
    samples: Optional[int] = typer.Option(None, "--samples", help="Number of samples (auto-detected if not provided)"),
    features: Optional[int] = typer.Option(None, "--features", help="Number of features (auto-detected if not provided)"),
    min_accuracy: Optional[float] = typer.Option(0.7, "--min-accuracy", help="Minimum required accuracy"),
    use_case: Optional[str] = typer.Option(None, "--use-case", help="Specific use case (e.g., fraud_detection)"),
    difficulty: str = typer.Option("intermediate", "--difficulty", help="Difficulty level (beginner/intermediate/advanced)"),
    max_results: int = typer.Option(5, "--max-results", help="Maximum number of recommendations"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Save recommendations to file"),
    ...
    if difficulty not in difficulty_mapping:
        console.print(f"[red]Error:[/red] Invalid difficulty level. Use: {list(difficulty_mapping.keys())}")
        raise typer.Exit(1)
    ...
    async def run_recommendation():
        ...
        with Progress(
            ...
            performance_requirements = {"min_accuracy": min_accuracy} if min_accuracy else None
...
def recommend_similar(
    config_id: str = typer.Argument(..., help="Configuration ID to find similar configurations for"),
    max_results: int = typer.Option(5, "--max-results", help="Maximum number of recommendations"),
    similarity_threshold: float = typer.Option(0.7, "--threshold", help="Minimum similarity threshold"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Save recommendations to file"),
    format: str = typer.Option("table", "--format", help="Output format (table/json/yaml)")
):
    ...
    async def run_similar_recommendation():
        ...
        try:
            ...
            console.print(f"[red]Error:[/red] Invalid configuration ID format")
            raise typer.Exit(1)
        ...
        with Progress(
            ...
            console.print("[yellow]Similar configuration search not yet implemented[/yellow]")
...
def predict_performance(
    config_id: str = typer.Argument(..., help="Configuration ID to predict performance for"),
    ...
    features: Optional[int] = typer.Option(None, "--features", help="Number of features"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Save predictions to file"),
    format: str = typer.Option("table", "--format", help="Output format (table/json/yaml)")
):
...
@require_feature("advanced_automl")
def train_models(
    min_configs: int = typer.Option(20, "--min-configs", help="Minimum configurations needed for training"),
    test_size: float = typer.Option(0.2, "--test-size", help="Fraction of data for testing"),
    force: bool = typer.Option(False, "--force", help="Force retraining even if models exist")
):
    ...
    async def run_training():
        ...
        with Progress(
            ...
            results = await recommendation_service.train_recommendation_models(
                min_configurations=min_configs,
                test_size=test_size
            )
        ...
        if "error" in results:
            ...
            if "available_configurations" in results:
                console.print(f"Available configurations: {results['available_configurations']}")
        else:
            ...
            table.add_row("Performance RMSE", f"{results['performance_predictor_rmse']:.4f}")
            table.add_row("Algorithm Accuracy", f"{results['algorithm_selector_accuracy']:.3f}")
...
def show_statistics():
    ...
    async def get_stats():
        ...
        rec_table.add_row("Total Recommendations", str(rec_stats["total_recommendations"]))
        rec_table.add_row("ML Recommendations", str(rec_stats["ml_recommendations"]))
        rec_table.add_row("Similarity Recommendations", str(rec_stats["similarity_recommendations"]))
        rec_table.add_row("Performance Predictions", str(rec_stats["performance_predictions"]))
        rec_table.add_row("Successful Predictions", str(rec_stats["successful_predictions"]))
        rec_table.add_row("Model Training Count", str(rec_stats["model_training_count"]))
        ...
        status_table.add_row(
            ...
            "✓ Trained" if model_status["performance_predictor_trained"] else "✗ Not trained"
        )
        status_table.add_row(
            "Algorithm Selector",
            "✓ Trained" if model_status["algorithm_selector_trained"] else "✗ Not trained"
        )
        status_table.add_row(
            "Feature Scaler",
            "✓ Available" if model_status["feature_scaler_available"] else "✗ Not available"
        )
        ...
        cache_table.add_row("Cached Configurations", str(cache_info["cached_configurations"]))
...
async def _analyze_dataset(
    ...
    try:
        ...
        if len(numeric_cols) > 0:
            ...
            sparsity = zero_count / total_numeric_values if total_numeric_values > 0 else 0.0
    ...
    if format == "json":
        ...
        for rec in recommendations:
            ...
            table.add_row(
                ...
                accuracy,
                training_time,
                rec.recommendation_reason[:50] + "..." if len(rec.recommendation_reason) > 50 else rec.recommendation_reason
            )
...
def _display_dataset_characteristics(characteristics: DatasetCharacteristicsDTO):
    ...
    if characteristics.feature_types:
        ...
        char_table.add_row("Feature Types", ", ".join(f"{k}: {v}" for k, v in type_counts.items()))
...
def _display_pattern_analysis(
    ...
    if format == "json":
        ...
        summary_table.add_row("Total Configurations", str(analysis["total_configurations"]))
        ...
        if "performance_summary" in analysis:
            ...
            summary_table.add_row("High Performance Rate", f"{perf['high_performance_rate']:.1%}")
