...
from pynomaly.application.dto.configuration_dto import (
    ConfigurationCaptureRequestDTO, ConfigurationExportRequestDTO,
    ConfigurationSearchRequestDTO, ConfigurationSource, ExportFormat
)
...
def capture_configuration(
    source: str = typer.Argument(..., help="Configuration source (automl, cli, web_api, etc.)"),
    parameters_file: Path = typer.Option(..., "--params", "-p", help="JSON file with raw parameters"),
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file for captured configuration"),
    auto_save: bool = typer.Option(True, "--auto-save/--no-auto-save", help="Automatically save configuration"),
    generate_name: bool = typer.Option(True, "--generate-name/--no-generate-name", help="Auto-generate configuration name"),
    tags: Optional[List[str]] = typer.Option(None, "--tag", "-t", help="Tags for configuration"),
    user_id: Optional[str] = typer.Option(None, "--user", "-u", help="User identifier"),
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Configuration description")
):
    ...
    try:
        ...
        if response.success:
            ...
            rprint(f"[green]✓[/green] Configuration captured successfully")
            ...
            if config.is_valid:
                rprint(f"[green]✓[/green] Configuration is valid")
            else:
                rprint(f"[yellow]⚠[/yellow] Configuration has validation issues:")
...
def export_configurations(
    ...
    export_format: str = typer.Option("json", "--format", "-f", help="Export format (json, yaml, python, notebook, docker)"),
    include_metadata: bool = typer.Option(True, "--metadata/--no-metadata", help="Include metadata in export"),
    include_performance: bool = typer.Option(True, "--performance/--no-performance", help="Include performance results"),
    include_lineage: bool = typer.Option(False, "--lineage/--no-lineage", help="Include lineage information"),
    template_name: Optional[str] = typer.Option(None, "--template", help="Export as template with given name"),
    compress: bool = typer.Option(False, "--compress/--no-compress", help="Compress output file")
):
    ...
    try:
        ...
        if response.success:
            ...
            if response.export_files:
                rprint(f"[blue]Files created:[/blue]")
...
def import_configurations(
    ...
    storage_path: Optional[Path] = typer.Option(None, "--storage", "-s", help="Configuration storage path"),
    overwrite: bool = typer.Option(False, "--overwrite/--no-overwrite", help="Overwrite existing configurations"),
    ...
    try:
        ...
        if dry_run:
            ...
            rprint(f"[blue]Import Preview:[/blue]")
            ...
            for i, config_data in enumerate(configurations[:5]):
                ...
                algorithm = config_data.get("algorithm_config", {}).get("algorithm_name", "Unknown")
            ...
            # Perform import
            import asyncio
            imported_count = asyncio.run(repository.import_configurations(import_file, overwrite))
            
            if imported_count > 0:
                rprint(f"[green]✓[/green] Successfully imported {imported_count} configurations")
                rprint(f"[blue]Storage:[/blue] {repo_path}")
            else:
                rprint(f"[yellow]⚠[/yellow] No configurations were imported")
...
def list_configurations(
    ...
    algorithm: Optional[str] = typer.Option(None, "--algorithm", help="Filter by algorithm"),
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum number of configurations to show"),
    show_details: bool = typer.Option(False, "--details/--no-details", help="Show detailed information")
):
    ...
    try:
        ...
        # Load configurations
        import asyncio
        configurations = asyncio.run(repository.list_configurations(
...
"""
                if config.metadata.tags:
                    panel_content += f"[blue]Tags:[/blue] {', '.join(config.metadata.tags)}\n"
...
def search_configurations(
    ...
    source: Optional[str] = typer.Option(None, "--source", help="Filter by source"),
    algorithm: Optional[str] = typer.Option(None, "--algorithm", help="Filter by algorithm"),
    min_accuracy: Optional[float] = typer.Option(None, "--min-accuracy", help="Minimum accuracy filter"),
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum results to return"),
    sort_by: str = typer.Option("created_at", "--sort", help="Sort field (created_at, name, algorithm, accuracy)"),
    ...
    try:
        ...
        if not results:
            rprint(f"[yellow]No configurations found matching query: '{query}'[/yellow]")
            return
        
        # Display results
        rprint(f"[green]Found {len(results)} configurations matching '{query}'[/green]\n")
        ...
        for config in results:
            ...
            tags_str = ", ".join(config.metadata.tags[:3]) if config.metadata.tags else ""
...
def show_configuration(
    ...
    storage_path: Optional[Path] = typer.Option(None, "--storage", "-s", help="Configuration storage path"),
    include_performance: bool = typer.Option(True, "--performance/--no-performance", help="Include performance details"),
    include_lineage: bool = typer.Option(False, "--lineage/--no-lineage", help="Include lineage information"),
    ...
    try:
        ...
        if output_format == "json":
            ...
            if config.dataset_config.feature_columns:
                dataset_tree.add(f"Features: {len(config.dataset_config.feature_columns)} columns")
            ...
            if include_performance and config.performance_results:
                ...
                if config.performance_results.accuracy:
                    perf_tree.add(f"Accuracy: {config.performance_results.accuracy:.4f}")
                if config.performance_results.precision:
                    perf_tree.add(f"Precision: {config.performance_results.precision:.4f}")
                ...
                if config.performance_results.f1_score:
                    perf_tree.add(f"F1 Score: {config.performance_results.f1_score:.4f}")
                if config.performance_results.training_time_seconds:
                    perf_tree.add(f"Training Time: {config.performance_results.training_time_seconds:.2f}s")
            ...
            if include_lineage and config.lineage:
                ...
                if config.lineage.parent_configurations:
                    lineage_tree.add(f"Parent Configs: {len(config.lineage.parent_configurations)}")
...
"""
        ...
        rprint(Panel(panel_content, title="Configuration Repository Statistics", border_style="blue"))
