...
from pynomaly.domain.entities.alert import (
    AlertSeverity, AlertStatus, AlertCategory, AlertSource, 
    NoiseClassification, AlertMetadata
)
...
@click.option("--category", type=click.Choice([
    "anomaly_detection", "system_performance", "security", "data_quality", 
    "model_drift", "resource_usage", "tenant_quota", "infrastructure", 
    "authentication", "compliance"
]), default="anomaly_detection", help="Alert category")
@click.option("--source", type=click.Choice([
    "detector", "system_monitor", "tenant_service", "security_service",
    "performance_monitor", "data_pipeline", "model_service", "external_webhook"
]), default="detector", help="Alert source")
...
@click.option("--business-impact", type=click.Choice(["low", "medium", "high", "critical"]), 
        ...
        with Progress(
            ...
            if detector_id:
                try:
                    ...
                    console.print(f"[red]Error: Invalid detector ID: {detector_id}[/red]")
                    return
            ...
            # Create alert
            task3 = progress.add_task("Creating alert with intelligent processing...", total=None)
            ...
            try:
                ...
                task4 = progress.add_task("Processing alert intelligence...", total=None)
                ...
                if processed_alert.is_suppressed():
                    console.print(f"[yellow]⚠ Alert was automatically suppressed: {processed_alert.suppression.suppression_reason}[/yellow]")
                
                if processed_alert.correlation:
                    console.print(f"[blue]🔗 Alert correlated with {len(processed_alert.correlation.related_alerts)} other alerts[/blue]")
                
                if processed_alert.noise_classification != NoiseClassification.UNKNOWN:
                    console.print(f"[cyan]🤖 ML Classification: {processed_alert.noise_classification.value} "
...
@alert_commands.command()
@click.option("--status", type=click.Choice(["open", "acknowledged", "in_progress", "resolved", "suppressed", "escalated"]), 
...
@click.option("--limit", type=int, default=50, help="Maximum number of alerts to display")
...
def acknowledge(alert_id: str, user: str, note: Optional[str]):
...
def resolve(alert_id: str, user: str, note: Optional[str], quality: Optional[float]):
    """Resolve an alert."""
    ...
    async def resolve_alert():
        ...
        if quality is not None and (quality < 0.0 or quality > 1.0):
            console.print(f"[red]Error: Quality score must be between 0.0 and 1.0[/red]")
            return
        
        success = await alert_service.resolve_alert(alert_uuid, user, note or "", quality)
...
def suppress(alert_id: str, user: str, reason: Optional[str], duration: Optional[int]):
    """Suppress an alert."""
    ...
    async def suppress_alert():
        ...
        success = await alert_service.suppress_alert(alert_uuid, user, reason or "", duration)
...
def escalate(alert_id: str, user: str, reason: Optional[str]):
    """Escalate an alert."""
...
def analytics(days: int, output_format: str, output_file: Optional[str]):
...
@alert_commands.command()
@click.option("--status", type=click.Choice(["open", "acknowledged", "in_progress"]), 
              default="open", help="Status of alerts to check for escalation")
def check_escalations(status: str):
    ...
    async def check_alert_escalations():
        ...
        alerts = await alert_service.list_alerts(status_filter=status_filter, limit=1000)
        ...
        console.print(f"[yellow]Found {len(escalation_candidates)} alerts needing escalation:[/yellow]")
...
def _display_analytics(analytics_data: dict, days: int):
    ...
    for classification, count in analytics_data['alert_distribution']['by_noise_classification'].items():
        percentage = (count / total_alerts) * 100 if total_alerts > 0 else 0
