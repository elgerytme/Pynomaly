...
from pynomaly.infrastructure.preprocessing.data_cleaner import (
    DataCleaner, MissingValueStrategy, OutlierStrategy
)
from pynomaly.infrastructure.preprocessing.data_transformer import (
    DataTransformer, ScalingStrategy, EncodingStrategy, FeatureSelectionStrategy
)
...
def clean_data(
    ...
    missing_strategy: Optional[str] = typer.Option("drop_rows", "--missing", help="Missing value strategy"),
    outlier_strategy: Optional[str] = typer.Option("clip", "--outliers", help="Outlier handling strategy"),
    remove_duplicates: bool = typer.Option(True, "--duplicates/--keep-duplicates", help="Remove duplicate rows"),
    handle_zeros: Optional[str] = typer.Option("keep", "--zeros", help="Zero value handling: keep, remove, replace"),
    handle_infinite: Optional[str] = typer.Option("remove", "--infinite", help="Infinite value handling: remove, replace, clip"),
    fill_value: Optional[str] = typer.Option(None, "--fill-value", help="Value for constant fill strategy"),
    outlier_threshold: Optional[float] = typer.Option(3.0, "--outlier-threshold", help="Z-score threshold for outliers"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be done without applying changes"),
    save_as: Optional[str] = typer.Option(None, "--save-as", help="Save cleaned data as new dataset"),
    output_format: str = typer.Option("csv", "--format", help="Output format: csv, parquet, json")
):
    ...
    console.print(f"Original shape: {dataset.n_samples:,} rows × {dataset.n_features} columns")
    ...
    try:
        ...
        if missing_strategy and missing_strategy != "none":
            try:
                ...
                if dry_run:
                    ...
                    console.print(f"Would handle {missing_count:,} missing values using {strategy.value}")
                ...
                console.print(f"[red]Error:[/red] Invalid missing value strategy: {missing_strategy}")
                raise typer.Exit(1)
            ...
            try:
                ...
                if dry_run:
                    ...
                    if len(numeric_cols) > 0:
                        ...
                        console.print(f"Would handle ~{outlier_count:,} outliers using {strategy.value}")
                ...
                console.print(f"[red]Error:[/red] Invalid outlier strategy: {outlier_strategy}")
                raise typer.Exit(1)
        ...
        if handle_zeros and handle_zeros != "keep":
            if dry_run:
                ...
                console.print(f"Would handle {zero_count:,} zero values using {handle_zeros}")
        ...
        if handle_infinite and handle_infinite != "keep":
            if dry_run:
                ...
                console.print(f"Would handle {inf_count:,} infinite values using {handle_infinite}")
        ...
        console.print(f"Final shape: {final_shape[0]:,} rows × {final_shape[1]} columns")
        ...
        if not dry_run:
            ...
            if rows_removed > 0:
                console.print(f"Rows removed: {rows_removed:,} ({rows_removed/original_shape[0]*100:.1f}%)")
            if cols_removed > 0:
            ...
            if save_as:
                ...
                console.print(f"✓ Saved cleaned dataset as '{save_as}' (ID: {new_dataset.id[:8]})")
            else:
...
def transform_data(
    ...
    scaling: Optional[str] = typer.Option(None, "--scaling", help="Feature scaling strategy"),
    encoding: Optional[str] = typer.Option(None, "--encoding", help="Categorical encoding strategy"),
    feature_selection: Optional[str] = typer.Option(None, "--feature-selection", help="Feature selection strategy"),
    polynomial_degree: Optional[int] = typer.Option(None, "--polynomial", help="Generate polynomial features"),
    normalize_column_names: bool = typer.Option(False, "--normalize-names", help="Normalize column names"),
    optimize_dtypes: bool = typer.Option(False, "--optimize-dtypes", help="Optimize data types"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be done without applying changes"),
    save_as: Optional[str] = typer.Option(None, "--save-as", help="Save transformed data as new dataset"),
    exclude_columns: Optional[str] = typer.Option(None, "--exclude", help="Comma-separated columns to exclude")
):
    ...
    try:
        ...
        if scaling and scaling != "none":
            try:
                ...
                if dry_run:
                    console.print(f"Would apply {strategy.value} scaling to {len(target_cols)} numeric columns")
                    transformation_steps.append(f"Scaling: {strategy.value} on {len(target_cols)} columns")
                else:
                    with console.status(f"Applying {strategy.value} scaling..."):
                        df = transformer.scale_features(df, strategy=strategy, columns=target_cols)
                    console.print(f"✓ Applied {strategy.value} scaling to {len(target_cols)} columns")
            except ValueError:
        ...
        if encoding and encoding != "none":
            try:
                ...
                target_cols = [col for col in categorical_cols if col not in excluded_cols]
                
                if dry_run:
                    console.print(f"Would apply {strategy.value} encoding to {len(target_cols)} categorical columns")
                    transformation_steps.append(f"Encoding: {strategy.value} on {len(target_cols)} columns")
                else:
                    with console.status(f"Applying {strategy.value} encoding..."):
                        df = transformer.encode_categorical(df, strategy=strategy, columns=target_cols)
                    console.print(f"✓ Applied {strategy.value} encoding to {len(target_cols)} columns")
            except ValueError:
                console.print(f"[red]Error:[/red] Invalid encoding strategy: {encoding}")
                raise typer.Exit(1)
        ...
        if feature_selection and feature_selection != "none":
            try:
                ...
                if dry_run:
                    ...
                    with console.status(f"Applying {strategy.value} feature selection..."):
                        df = transformer.select_features(df, strategy=strategy)
                    selected_features = len(df.columns)
                    console.print(f"✓ Selected {selected_features}/{original_features} features using {strategy.value}")
            except ValueError:
                console.print(f"[red]Error:[/red] Invalid feature selection strategy: {feature_selection}")
                raise typer.Exit(1)
        ...
        if polynomial_degree and polynomial_degree > 1:
            if dry_run:
                ...
                console.print(f"Would generate polynomial features (degree {polynomial_degree})")
                console.print(f"Estimated new feature count: ~{estimated_features}")
                transformation_steps.append(f"Polynomial features: degree {polynomial_degree}")
            else:
                original_features = len(df.columns)
                with console.status(f"Generating polynomial features (degree {polynomial_degree})..."):
                    df = transformer.create_polynomial_features(df, degree=polynomial_degree)
                new_features = len(df.columns)
                console.print(f"✓ Generated {new_features - original_features} polynomial features")
        
        # Normalize column names
        if normalize_column_names:
            if dry_run:
                console.print("Would normalize column names (lowercase, underscore-separated)")
        ...
        if optimize_dtypes:
            if dry_run:
                ...
                console.print(f"✓ Optimized data types (saved {memory_saved:.1f}% memory)")
        ...
        if not dry_run:
            ...
            if save_as:
                ...
                new_dataset = Dataset(
                    ...
                    data=df,
                    target_column=dataset.target_column if dataset.target_column in df.columns else None,
                    created_at=datetime.now()
                )
                
                dataset_repo.save(new_dataset)
                console.print(f"✓ Saved transformed dataset as '{save_as}' (ID: {new_dataset.id[:8]})")
            else:
                # Update existing dataset
                dataset.data = df
                dataset.n_samples = len(df)
...
def manage_pipeline(
    action: str = typer.Argument(..., help="Action: create, list, show, apply, save, load, delete"),
    ...
    config_file: Optional[Path] = typer.Option(None, "--config", help="Pipeline configuration file"),
    output_file: Optional[Path] = typer.Option(None, "--output", help="Output file for save operations"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be done without applying changes")
):
    ...
    if action == "create":
        ...
        console.print("Available actions: create, list, show, apply, save, load, delete")
        raise typer.Exit(1)


def _create_pipeline(name: Optional[str], config_file: Optional[Path]):
...
def _list_pipelines():
    ...
    for pipeline in pipelines:
        ...
        table.add_row(
            pipeline.name,
            f"{enabled_steps}/{len(pipeline.steps)}",
            status
        )
...
def _show_pipeline(name: Optional[str]):
...
def _apply_pipeline(name: Optional[str], dataset_id: Optional[str], dry_run: bool):
    ...
    console.print(f"[blue]Applying pipeline '{name}' to dataset '{dataset.name}'[/blue]")
    ...
    if dry_run:
        ...
        try:
            ...
            console.print(f"✓ Pipeline applied successfully")
            console.print(f"Shape: {original_shape[0]:,}×{original_shape[1]} → {len(transformed_df):,}×{len(transformed_df.columns)}")
...
def _save_pipeline(name: Optional[str], output_file: Optional[Path]):
...
def _load_pipeline(config_file: Optional[Path], name: Optional[str]):
...
def _delete_pipeline(name: Optional[str]):
    """Delete a pipeline."""
    if not name:
        console.print("[red]Error:[/red] Pipeline name required")
        raise typer.Exit(1)
...
def _get_pipeline_by_name(name: str) -> Optional[PreprocessingPipeline]:
