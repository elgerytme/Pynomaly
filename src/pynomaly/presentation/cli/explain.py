...
from pynomaly.application.services.explainable_ai_service import (
    ExplainableAIService, ExplanationConfiguration
)
from pynomaly.domain.entities.explainable_ai import (
    ExplanationMethod, ExplanationScope, ExplanationAudience
)
...
@click.option("--instance-index", type=int, help="Index of specific instance to explain")
@click.option("--method", type=click.Choice([
    "shap_tree", "shap_kernel", "lime", "permutation_importance", "feature_ablation"
]), default="shap_tree", help="Explanation method to use")
@click.option("--num-features", type=int, default=10, help="Number of top features to show")
@click.option("--output-path", help="Path to save explanation results")
@click.option("--format", "output_format", type=click.Choice(["json", "html", "csv"]), 
              default="json", help="Output format")
@click.option("--audience", type=click.Choice([
    "technical", "business", "regulatory", "end_user"
]), default="technical", help="Target audience for explanation")
@click.option("--enable-bias-detection", is_flag=True, help="Enable bias detection analysis")
@click.option("--enable-counterfactuals", is_flag=True, help="Generate counterfactual explanations")
@click.option("--confidence-threshold", type=float, default=0.8, help="Confidence threshold for explanations")
def predict(
    model_path: str,
    data_path: str,
    ...
    async def run_explanation():
        ...
        with Progress(
            ...
            task1 = progress.add_task("Initializing explainable AI service...", total=None)
            ...
            try:
                if data_path.endswith('.csv'):
                    ...
                    feature_names = data_json.get('feature_names', [f"feature_{i}" for i in range(data.shape[1])])
                else:
            ...
            try:
                if instance_index is not None:
                    ...
                    if instance_index >= len(data):
                        console.print(f"[red]Instance index {instance_index} out of range[/red]")
                        return
                ...
                if output_path:
                    if instance_index is not None:
                        await _save_explanation_result(result, output_path, output_format)
                    else:
                        await _save_global_explanation(global_explanation, output_path, output_format)
...
@click.option("--protected-attributes", required=True, 
        ...
        with Progress(
            ...
            try:
                ...
                if output_path:
                    ...
                    console.print(f"[green]Bias analysis saved to {output_path}[/green]")
...
@click.option("--method", type=click.Choice([
    "shap_tree", "shap_kernel", "lime", "permutation_importance", "feature_ablation"
]), default="permutation_importance", help="Feature importance method")
        ...
        with Progress(
            ...
            try:
                ...
                if output_path and not plot:
                    ...
                    console.print(f"[green]Feature importance saved to {output_path}[/green]")
...
@click.option("--method", type=click.Choice([
    "shap_tree", "shap_kernel", "lime"
]), default="shap_tree", help="Explanation method to validate")
@click.option("--num-samples", type=int, default=100, help="Number of samples for validation")
        ...
        with Progress(
            ...
            try:
                ...
                for i, instance in enumerate(sample_data):
                    ...
                    if i % 10 == 0:
                        progress.update(task3, description=f"Validating explanations... {i+1}/{sample_size}")
                ...
                if output_path:
                    ...
                    console.print(f"[green]Validation results saved to {output_path}[/green]")
...
def summary(model_id: Optional[str], time_window: int, output_path: Optional[str]):
    ...
    async def run_summary():
        ...
        with Progress(
            ...
            try:
                model_uuid = UUID(model_id) if model_id else UUID(int=12345)  # Default UUID
                time_delta = timedelta(hours=time_window)
                
                summary_data = await xai_service.get_explanation_summary(model_uuid, time_delta)
...
def _display_instance_explanation(result, instance_index: int, audience: str):
    ...
    for importance in explanation.get_top_features(10):
        direction = "↑" if importance.contribution_direction == "positive" else "↓" if importance.contribution_direction == "negative" else "→"
        table.add_row(
            str(importance.rank),
...
def _display_bias_analysis(bias_analysis, threshold: float):
    ...
    if bias_analysis.fairness_metrics:
        fairness_text = "\n".join([f"{k}: {v:.3f}" for k, v in bias_analysis.fairness_metrics.items()])
...
def _generate_importance_plot(importances, top_k: int, output_path: Optional[str]):
