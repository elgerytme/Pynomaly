...
from pynomaly.domain.entities.tenant import TenantStatus, SubscriptionTier, ResourceQuotaType
...
@click.option("--tier", type=click.Choice(["free", "basic", "professional", "enterprise"]), 
        ...
        with Progress(
            ...
            if admin_user:
                try:
                    ...
                    console.print(f"[red]Error: Invalid admin user UUID: {admin_user}[/red]")
                    return
            ...
            try:
                ...
                if auto_activate:
                    ...
                    console.print(f"[yellow]Use 'pynomaly tenant activate {tenant.tenant_id}' to activate[/yellow]")
...
def suspend(tenant_id: str, reason: Optional[str]):
    """Suspend a tenant."""
...
def deactivate(tenant_id: str):
    ...
    async def deactivate_tenant():
        ...
        if not Confirm.ask(f"Are you sure you want to deactivate tenant {tenant_id}? This action cannot be easily undone."):
        ...
        if success:
            console.print(f"[green]✓ Tenant {tenant_id} deactivated successfully[/green]")
        else:
...
@tenant_commands.command()
@click.option("--status", type=click.Choice(["active", "suspended", "deactivated", "pending_activation"]), 
              help="Filter by tenant status")
@click.option("--tier", type=click.Choice(["free", "basic", "professional", "enterprise"]), 
              help="Filter by subscription tier")
@click.option("--limit", type=int, default=50, help="Maximum number of tenants to display")
@click.option("--format", "output_format", type=click.Choice(["table", "json"]), 
              default="table", help="Output format")
def list(status: Optional[str], tier: Optional[str], limit: int, output_format: str):
...
@tenant_commands.command()
@click.argument("tenant_identifier", type=str)
@click.argument("new_tier", type=click.Choice(["free", "basic", "professional", "enterprise"]))
def upgrade(tenant_identifier: str, new_tier: str):
    ...
    async def upgrade_tenant():
        ...
        if tier_order[new_subscription_tier] <= tier_order[tenant.subscription_tier]:
            console.print(f"[yellow]Warning: This is not an upgrade from {tenant.subscription_tier.value} to {new_tier}[/yellow]")
            if not Confirm.ask("Continue anyway?"):
                return
        
        success = await tenant_service.upgrade_tenant_subscription(tenant.tenant_id, new_subscription_tier)
        
        if success:
            console.print(f"[green]✓ Tenant {tenant.name} upgraded to {new_tier} tier[/green]")
        else:
            console.print(f"[red]Error upgrading tenant subscription[/red]")
...
def reset_quotas(tenant_identifier: str):
    ...
    async def reset_tenant_quotas():
        ...
        if success:
            ...
            console.print(f"[red]Error resetting quotas[/red]")
...
def _display_usage_summary(usage_summary):
    ...
    for quota_type, quota_info in quotas.items():
        ...
        limit_str = str(quota_info["limit"]) if quota_info["limit"] != "unlimited" else "∞"
        
        status_color = "red" if quota_info["is_exceeded"] else "yellow" if usage_pct > 80 else "green"
