...
from pynomaly.domain.entities.deployment import (
    Environment, DeploymentStrategy, StrategyType, DeploymentConfig, RollbackCriteria
)
...
def deploy_model(
    ...
    strategy: str = typer.Option("rolling", "--strategy", "-s", help="Deployment strategy"),
    ...
    memory_request: str = typer.Option("512Mi", "--memory-request", help="Memory request"),
    ...
    async def _deploy_model():
        ...
        try:
            ...
            if not json_output:
                console.print(f"[blue]Deploying model {model_version_id} to {environment}...[/blue]")
            ...
            if json_output:
                print(json.dumps(deployment.get_deployment_info(), indent=2, default=str))
            else:
...
[bold]Created:[/bold] {deployment.created_at}
                """
                console.print(Panel(panel_content, title="Deployment Summary", border_style="green"))
...
def deployment_status(
    ...
    async def _get_status():
        ...
        try:
            ...
            health_color = "green" if deployment.health_score > 0.8 else "yellow" if deployment.health_score > 0.6 else "red"
...
[bold]Timeline:[/bold]
  ...
  • Created By: {deployment.created_by}
            ...
            console.print(Panel(panel_content, title="Deployment Status", border_style="blue"))
            
        except ValueError:
            console.print(f"[red]Error: Invalid deployment ID format[/red]")
            raise typer.Exit(1)
        except Exception as e:
...
def rollback_deployment(
    ...
    reason: str = typer.Option("Manual rollback", "--reason", "-r", help="Rollback reason"),
    wait: bool = typer.Option(True, "--wait/--no-wait", help="Wait for rollback to complete"),
    json_output: bool = typer.Option(False, "--json", help="Output in JSON format")
):
    ...
    async def _rollback():
        ...
        try:
            ...
            if rollback_deployment:
                if json_output:
                    print(json.dumps(rollback_deployment.get_deployment_info(), indent=2, default=str))
                else:
                    console.print(f"[green]✓ Rollback completed! New deployment: {rollback_deployment.id}[/green]")
            else:
                if not json_output:
                    console.print("[yellow]No previous version available for rollback[/yellow]")
...
def promote_to_production(
    ...
    wait: bool = typer.Option(True, "--wait/--no-wait", help="Wait for promotion to complete"),
    json_output: bool = typer.Option(False, "--json", help="Output in JSON format")
):
    ...
    async def _promote():
        ...
        try:
            ...
            if not json_output:
                console.print(f"[blue]Promoting deployment {deployment_id} to production...[/blue]")
            ...
            if json_output:
                print(json.dumps(production_deployment.get_deployment_info(), indent=2, default=str))
            else:
                console.print(f"[green]✓ Promotion completed! Production deployment: {production_deployment.id}[/green]")
