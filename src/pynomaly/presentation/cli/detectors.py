...
def list_detectors(
    ...
    fitted: Optional[bool] = typer.Option(None, "--fitted", help="Filter by fitted status"),
...
def create_detector(
    ...
    algorithm: str = typer.Option("IsolationForest", "--algorithm", "-a", help="Algorithm to use"),
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Detector description"),
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Created detector '{name}' with ID: {detector.id}")
...
def show_detector(
    ...
    if not matching:
        console.print(f"[red]Error:[/red] No detector found with ID starting with '{detector_id}'")
        raise typer.Exit(1)
    ...
    if detector.is_fitted and detector.metadata:
        ...
        if "training_samples" in detector.metadata:
            console.print(f"  Training samples: {detector.metadata['training_samples']}")
...
def delete_detector(
    ...
    if success:
        ...
        console.print(f"[red]Error:[/red] Failed to delete detector")
        raise typer.Exit(1)
...
def clone_detector(
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Cloned detector '{source.name}' to '{new_name}'")
