...
@app.command("list")
def list_datasets(
    has_target: Optional[bool] = typer.Option(None, "--has-target", help="Filter by target presence"),
...
def load_dataset(
    ...
    sample_size: Optional[int] = typer.Option(None, "--sample", "-s", help="Load only N rows")
):
...
def show_dataset(
    ...
    if not matching:
        console.print(f"[red]Error:[/red] No dataset found with ID starting with '{dataset_id}'")
        raise typer.Exit(1)
    ...
    if sample > 0:
        ...
        for col in sample_df.columns:
            table.add_column(col, style="cyan" if col == dataset.target_column else None)
...
def check_quality(
    ...
    if quality_report["missing_values"]:
        console.print(f"  [yellow]Missing values:[/yellow] {len(quality_report['missing_values'])} columns")
        for col, pct in list(quality_report["missing_values"].items())[:3]:
            console.print(f"    - {col}: {pct:.1%}")
    
    if quality_report["constant_features"]:
        console.print(f"  [red]Constant features:[/red] {len(quality_report['constant_features'])} columns")
        for col in quality_report["constant_features"][:3]:
            console.print(f"    - {col}")
    
    if quality_report["low_variance_features"]:
        console.print(f"  [yellow]Low variance:[/yellow] {len(quality_report['low_variance_features'])} columns")
        for col in quality_report["low_variance_features"][:3]:
            console.print(f"    - {col}")
    
    if quality_report["infinite_values"]:
        console.print(f"  [red]Infinite values:[/red] {len(quality_report['infinite_values'])} columns")
    
    if quality_report["duplicate_rows"] > 0:
        console.print(f"  [yellow]Duplicate rows:[/yellow] {quality_report['duplicate_rows']:,}")
...
def split_dataset(
    ...
    random_state: Optional[int] = typer.Option(42, "--seed", "-s", help="Random seed")
):
...
def delete_dataset(
    ...
    if success:
        ...
        console.print(f"[red]Error:[/red] Failed to delete dataset")
        raise typer.Exit(1)
...
def export_dataset(
    ...
    format: str = typer.Option("csv", "--format", "-f", help="Export format (csv, parquet)")
):
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Exported dataset '{dataset.name}' to {output_path}")
