...
from pynomaly.application.services.security_compliance_service import (
    SecurityComplianceService, SecurityConfiguration, DataSubject, EncryptionContext
)
from pynomaly.domain.entities.security_compliance import (
    ComplianceFramework, SecurityLevel, PrivacyControl, DataClassification,
    AuditLevel, IncidentSeverity
)
...
@click.option("--frameworks", type=click.Choice([
    "soc2", "gdpr", "hipaa", "pci_dss", "iso27001", "nist", "ccpa"
]), multiple=True, default=["soc2", "gdpr"], help="Compliance frameworks to assess")
...
def assess_compliance(
    ...
    async def run_assessment():
        ...
        with Progress(
            ...
            task1 = progress.add_task("Initializing security compliance service...", total=None)
            ...
            try:
                ...
                reports = await security_service.generate_compliance_report(framework_enums)
                ...
                if output_path:
                    ...
                    console.print(f"[green]Compliance reports saved to {output_path}[/green]")
...
@click.option("--classification", type=click.Choice([
...
]), multiple=True, help="Applicable compliance frameworks")
def encrypt_data(
    data_file: str,
        ...
        with Progress(
            ...
            try:
                ...
                context = EncryptionContext(
                    ...
                    compliance_frameworks=[ComplianceFramework(fw) for fw in frameworks]
                )
                
                # Encrypt data
                encrypted_data, encryption_context = await security_service.encrypt_data(data, context)
...
@security_commands.command()
@click.option("--subject-data", required=True, help="JSON file with data subject information")
@click.option("--processing-purposes", required=True, help="Comma-separated processing purposes")
...
def register_data_subject(
    ...
    async def run_registration():
        ...
        with Progress(
            ...
            # Initialize security service
            task1 = progress.add_task("Initializing GDPR compliance service...", total=None)
            storage_path = Path("./security")
            security_service = SecurityComplianceService(storage_path)
            progress.update(task1, completed=True)
...
@click.option("--request-type", type=click.Choice([
    "access", "rectification", "erasure", "portability"
]), required=True, help="Type of GDPR request")
...
def gdpr_request(
    ...
    async def run_gdpr_request():
        ...
        with Progress(
            ...
            task2 = progress.add_task(f"Processing {request_type} request...", total=None)
            ...
            try:
                ...
                if output_file:
                    ...
                    console.print(f"[green]Request result saved to {output_file}[/green]")
...
@click.option("--alert-threshold", type=int, default=3, help="Minimum incidents for alert")
def detect_breach(
    access_logs: str,
    output_file: Optional[str],
    alert_threshold: int
    ...
    async def run_breach_detection():
        ...
        with Progress(
            ...
            try:
                ...
                if output_file:
                    incidents_data = [incident.get_incident_summary() for incident in incidents]
                    with open(output_file, 'w') as f:
                        json.dump(incidents_data, f, indent=2)
                    console.print(f"[green]Breach detection results saved to {output_file}[/green]")
...
def status(config_file: Optional[str], output_file: Optional[str]):
        ...
        with Progress(
            ...
            try:
                ...
                if output_file:
                    ...
                    console.print(f"[green]Security status saved to {output_file}[/green]")
...
def cleanup(dry_run: bool, force: bool):
    ...
    async def run_cleanup():
        ...
        with Progress(
            ...
            # Run cleanup
            task2 = progress.add_task("Analyzing data for cleanup..." if dry_run else "Cleaning up expired data...", total=None)
            
            try:
                if dry_run:
                    # Simulate cleanup
                    console.print("[yellow]DRY RUN MODE - No data will be actually deleted[/yellow]")
                    
                    # Show what would be cleaned
                    expired_count = len([s for s in security_service.data_subjects.values() if s.should_be_deleted()])
                    ...
                    # Confirm cleanup
                    if not force:
                        confirm = click.confirm("This will permanently delete expired data. Continue?")
                        if not confirm:
...
def _display_compliance_violations(framework: str, violations: list):
    ...
    for violation in violations:
        severity_color = "red" if violation.severity == "critical" else "yellow" if violation.severity == "high" else "white"
...
def _display_security_incidents(incidents: list, alert_threshold: int):
    ...
    if incidents:
        ...
        for incident in incidents[:10]:  # Show top 10
            severity_color = "red" if incident.severity.value == "critical" else "yellow"
...
def _display_anonymization_comparison(original: dict, anonymized: dict, rules: dict):
    ...
    console.print(Panel(
        "[bold blue]Anonymization Results[/bold blue]",
    ...
    for field, rule in rules.items():
        if field in original:
            orig_val = str(original[field])[:20] + "..." if len(str(original[field])) > 20 else str(original[field])
            anon_val = str(anonymized.get(field, "REMOVED"))[:20] + "..." if len(str(anonymized.get(field, "REMOVED"))) > 20 else str(anonymized.get(field, "REMOVED"))
