...
from pynomaly.application.services.visualization_dashboard_service import (
    VisualizationDashboardService, DashboardType, DashboardConfig, RealTimeMetrics
)
...
@click.option("--type", "dashboard_type", type=click.Choice([
    "executive", "operational", "analytical", "performance", "real_time", "compliance"
]), default="analytical", help="Type of dashboard to generate")
@click.option("--output-path", help="Path to save dashboard files")
@click.option("--format", "export_format", type=click.Choice([
    "html", "png", "pdf", "svg", "json"
]), default="html", help="Export format for dashboard")
@click.option("--theme", type=click.Choice([
    "default", "dark", "light", "corporate"
]), default="default", help="Dashboard theme")
...
def generate(
    ...
    async def run_generation():
        ...
        with Progress(
            ...
            # Generate dashboard
            task2 = progress.add_task(f"Generating {dashboard_type} dashboard...", total=None)
            
            try:
                dashboard_type_enum = DashboardType(dashboard_type)
                
                if dashboard_type_enum == DashboardType.EXECUTIVE:
                    dashboard_data = await dashboard_service.generate_executive_dashboard()
                elif dashboard_type_enum == DashboardType.OPERATIONAL:
                    dashboard_data = await dashboard_service.generate_operational_dashboard(real_time)
                elif dashboard_type_enum == DashboardType.ANALYTICAL:
                    dashboard_data = await dashboard_service.generate_analytical_dashboard()
                elif dashboard_type_enum == DashboardType.PERFORMANCE:
                    dashboard_data = await dashboard_service.generate_performance_dashboard()
                elif dashboard_type_enum == DashboardType.REAL_TIME:
                    if not websocket_endpoint:
                        websocket_endpoint = "ws://localhost:8000/ws"
                    dashboard_data = await dashboard_service.generate_real_time_dashboard(websocket_endpoint)
                else:
                ...
                if output_path:
                    task3 = progress.add_task(f"Exporting dashboard to {export_format}...", total=None)
...
def status(dashboard_id: Optional[str], detailed: bool):
    ...
    async def run_status():
        ...
        with Progress(
            ...
            # Initialize dashboard service
            task1 = progress.add_task("Checking dashboard service status...", total=None)
            storage_path = Path("./dashboards")
...
@click.option("--websocket-endpoint", default="ws://localhost:8000/ws", help="WebSocket endpoint")
@click.option("--duration", type=int, help="Duration to monitor in seconds")
def monitor(interval: int, websocket_endpoint: str, duration: Optional[int]):
...
@dashboard_commands.command()
@click.option("--dashboard-type", type=click.Choice([
    "executive", "operational", "analytical", "performance"
]), default="analytical", help="Dashboard type to compare")
...
def compare(dashboard_type: str, metrics: List[str], time_period: int):
    ...
    async def run_comparison():
        ...
        with Progress(
            ...
            # Display comparison
            _display_metrics_comparison(comparison_data, dashboard_type, metrics or list(comparison_data["current_period"].keys()))
...
def export(dashboard_id: str, export_format: str, output: str, config_file: Optional[str]):
    ...
    async def run_export():
        ...
        with Progress(
            ...
            # Export dashboard
            task2 = progress.add_task(f"Exporting dashboard to {export_format}...", total=None)
            ...
            try:
                ...
                if export_format == "json":
                    ...
                    with open(output_path, 'w') as f:
                        json.dump({"exported_data": exported_data.decode()}, f, indent=2)
                else:
...
def _update_live_layout(layout: Layout, metrics: RealTimeMetrics, iteration: int):
    ...
    metrics_table.add_row(
        "Anomalies Detected:", str(metrics.anomalies_detected),
        "Detection Rate:", f"{metrics.detection_rate:.1f}%"
    )
    metrics_table.add_row(
        "CPU Usage:", f"{metrics.system_cpu_usage:.1f}%",
        "Memory Usage:", f"{metrics.system_memory_usage:.1f}%"
    )
    metrics_table.add_row(
        "Active Detectors:", str(metrics.active_detectors),
        "Processed Samples:", str(metrics.processed_samples)
    )
    metrics_table.add_row(
        "Latency:", f"{metrics.processing_latency_ms:.1f}ms",
        "Throughput:", f"{metrics.throughput_per_second:.1f}/s"
    )
...
def _display_metrics_comparison(comparison_data: dict, dashboard_type: str, metrics: List[str]):
