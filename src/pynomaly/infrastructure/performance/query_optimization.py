...
class QueryAnalyzer:
    ...
    def _check_query_issues(self, query: str, analysis: Dict[str, Any]) -> None:
        ...
        if any(func in query for func in ["upper(", "lower(", "substring("]):
            ...
            recommendations.append("Consider using functional indexes or preprocessing data")
        
        # Check for OR conditions that might benefit from UNION
        if query.count(" or ") > 2:
            recommendations.append("Consider using UNION instead of multiple OR conditions")
            ...
            if len(columns) > 0:
                ...
                if len(columns) > 1:
                    ...
                    recommendations.append(IndexRecommendation(
                        table_name=table,
                        columns=column_list,
                        reason=f"Multiple columns used together in queries",
...
class QueryPerformanceTracker:
    ...
    async def _evict_oldest_metrics(self) -> None:
        ...
        oldest_key = min(
            self.metrics.keys(),
            key=lambda k: self.metrics[k].last_executed
        )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slowest_query": max(self.metrics.values(), key=lambda m: m.avg_time).avg_time if self.metrics else 0,
        }
...
class QueryOptimizer:
    ...
    async def execute_with_optimization(
        ...
        if use_cache:
            ...
            if cached_result is not None:
                await self.performance_tracker.record_query(query, 0.001, params)  # Minimal time for cache hit
        ...
        try:
            if hasattr(self.engine, 'connect'):
                ...
                async with self.engine.connect() as conn:
                    ...
                    result_data = await result.fetchall() if result.returns_rows else result.rowcount
    ...
    def _should_cache_query(self, query: str, execution_time: float) -> bool:
        ...
        if any(pattern in query_lower for pattern in ["count(", "sum(", "avg(", "max(", "min("]):
            return True
    ...
    async def optimize_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get performance summary
            optimization_results["performance_summary"] = self.performance_tracker.get_performance_summary()
