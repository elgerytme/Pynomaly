...
class PerformanceConfig:
    ...
    resource_alert_thresholds: Dict[str, float] = field(default_factory=lambda: {
        'cpu_percent': 80.0,
        'memory_percent': 85.0,
...
class FunctionProfiler:
    ...
    def profile_function(
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def async_wrapper(*args, **kwargs):
                if not self.config.enable_profiling or not self.config.enable_async_profiling:
    ...
    def _profile_with_cprofile(
        self, func: Callable, args: tuple, kwargs: dict, profile_id: str, save_stats: bool
    ) -> Any:
    ...
    def _profile_with_line_profiler(
        ...
        if not LINE_PROFILER_AVAILABLE:
            ...
            return self._profile_with_cprofile(func, args, kwargs, profile_id, save_stats)
    ...
    def _profile_with_memory_profiler(
        ...
        if not MEMORY_PROFILER_AVAILABLE:
            ...
            return self._profile_with_tracemalloc(func, args, kwargs, profile_id, save_stats)
    ...
    def _profile_with_tracemalloc(
        ...
        if save_stats:
            self._save_tracemalloc_stats(
                func.__name__, profile_id, current, peak
            )
    ...
    def _save_memory_profile_stats(
        self, func_name: str, profile_id: str, start_memory: float, 
        end_memory: float, memory_delta: float
    ) -> None:
...
class ResourceMonitor:
    ...
    def _collect_metrics(self) -> Dict[str, float]:
        ...
        metrics['gc_collections'] = sum(gc.get_stats()[i]['collections'] for i in range(3))
            ...
            if metric_name in metrics:
                ...
                if value > threshold:
                    ...
                    if (last_alert is None or 
                        (current_time - last_alert).total_seconds() > 60):  # 1 minute cooldown
    ...
    def profile_context(
        ...
        try:
            ...
            if profiler_type == ProfilerType.TRACEMALLOC:
                current, peak = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                self.function_profiler._save_tracemalloc_stats(name, profile_id, current, peak)
    ...
    def cleanup_old_profiles(self) -> int:
        ...
        try:
            ...
            if output_dir.exists():
                ...
                profile_files = list(output_dir.glob("*.prof")) + list(output_dir.glob("*.txt"))
            ...
            if len(self._profile_results) > self._max_stored_results:
                ...
                self._profile_results = self._profile_results[-self._max_stored_results:]
                cleaned_count += excess_count
            
            if cleaned_count > 0:
                self.logger.info(f"Cleaned up {cleaned_count} old profile files/records")
