...
class AdaptiveMemoryManager:
    ...
    def get_memory_usage(self) -> MemoryUsage:
        ...
        return MemoryUsage(
            ...
            peak_memory_mb=process_memory.peak_wset / (1024 * 1024) if hasattr(process_memory, 'peak_wset') else 0.0,
        )
    
    async def optimize_memory_usage(self, force: bool = False) -> List[MemoryOptimizationResult]:
        ...
        self.logger.info(f"Starting memory optimization (current usage: {current_usage.percent_used:.1f}%)")
        ...
        if current_usage.percent_used >= self.critical_threshold_percent:
            ...
            self.logger.warning("Critical memory usage - applying all optimization strategies")
        elif current_usage.percent_used >= self.warning_threshold_percent:
            strategies_to_use = self._optimization_strategies[:4]  # Use moderate strategies
            self.logger.info("High memory usage - applying moderate optimization strategies")
        else:
            strategies_to_use = self._optimization_strategies[:2]  # Use gentle strategies
            self.logger.info("Moderate memory usage - applying gentle optimization strategies")
        ...
        for strategy in strategies_to_use:
            try:
                ...
                if new_usage.percent_used <= self.target_memory_percent:
                    self.logger.info(f"Target memory usage reached: {new_usage.percent_used:.1f}%")
                    break
                    
            except Exception as e:
                self.logger.error(f"Optimization strategy {strategy.__name__} failed: {e}")
                optimization_results.append(MemoryOptimizationResult(
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if (self.enable_automatic_optimization and 
                    usage.percent_used > self.warning_threshold_percent and
                    time.time() - self._last_optimization_time > self.optimization_interval_seconds):
    ...
    async def _optimize_garbage_collection(self) -> MemoryOptimizationResult:
        ...
        return MemoryOptimizationResult(
            ...
            memory_saved_mb=memory_before.process_memory_mb - memory_after.process_memory_mb,
            success=True,
    ...
    def _update_optimization_metrics(self, results: List[MemoryOptimizationResult]) -> None:
        """Update optimization metrics."""
        successful_results = [r for r in results if r.success]
        
        self._optimization_metrics["total_optimizations"] += len(successful_results)
        self._optimization_metrics["total_memory_saved_mb"] += sum(r.memory_saved_mb for r in successful_results)
        ...
        if successful_results:
            ...
            self._optimization_metrics["average_optimization_time"] = (
                (current_avg * (total_ops - len(successful_results)) + avg_time * len(successful_results)) / total_ops
            )
    ...
    def get_memory_trends(self, hours: int = 1) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_count": len([r for r in self._optimization_history 
                                     if r.memory_before_mb > 0 and 
                                     any(u.timestamp >= cutoff_time for u in self._memory_history)]),
        }
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_optimizations": len([r for r in self._optimization_history 
                                       if time.time() - r.duration_seconds < 3600]),  # Last hour
...
class MemoryProfiler:
    ...
    def profile_function(self, func_name: str):
        ...
        def decorator(func: Callable) -> Callable:
            async def async_wrapper(*args, **kwargs):
                return await self._profile_async_function(func_name, func, *args, **kwargs)
    ...
    async def _profile_async_function(self, func_name: str, func: Callable, *args, **kwargs):
        ...
        try:
            ...
            self._record_profile(func_name, start_memory, end_memory, end_time - start_time, True)
            ...
            self._record_profile(func_name, start_memory, end_memory, end_time - start_time, False)
            raise
    ...
    def _record_profile(self, func_name: str, start_memory: float, end_memory: float, duration: float, success: bool):
    ...
    def get_profile_summary(self) -> Dict[str, Any]:
        ...
        for func_name, profile in self._profiles.items():
            ...
            summary[func_name] = {
                "total_calls": calls,
                "average_duration": profile["total_duration"] / calls if calls > 0 else 0,
                "average_memory_delta_mb": profile["total_memory_delta"] / calls if calls > 0 else 0,
                "max_memory_delta_mb": profile["max_memory_delta"],
                "min_memory_delta_mb": profile["min_memory_delta"] if profile["min_memory_delta"] != float('inf') else 0,
