...
class PerformanceService:
    ...
    def start_monitoring(self) -> None:
        ...
        if self._monitoring_task is None:
            ...
            logger.info(
                "Performance monitoring started", interval=self.monitoring_interval
            )
        ...
        try:
            ...
            for pool_name, stats in pool_stats.items():
                ...
                if (
                    ...
                    > self._alert_thresholds["avg_response_time"]
                ):
                    await self._create_alert(
                        "slow_pool_response",
                        ...
                        {
                            "pool_name": pool_name,
                            "response_time": pool_metrics["avg_response_time"],
                        },
                    )
            ...
            if self.query_optimizer:
                query_summary = (
                    self.query_optimizer.performance_tracker.get_performance_summary()
                )
                metrics["query_performance"] = query_summary

                # Check for slow query alerts
                if (
                    query_summary["slow_queries"]
                    > self._alert_thresholds["slow_query_count"]
                ):
                    await self._create_alert(
            ...
            logger.debug(
                "Performance metrics collected", pools=len(metrics["connection_pools"])
            )
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        if len(self._performance_history) >= 2:
            ...
            for pool_name in latest["connection_pools"]:
                if pool_name in previous["connection_pools"]:
                    current_response_time = latest["connection_pools"][pool_name][
                        "avg_response_time"
                    ]
                    previous_response_time = previous["connection_pools"][pool_name][
                        "avg_response_time"
                    ]
        ...
        return {
            ...
            "monitoring_duration": len(self._performance_history)
            * self.monitoring_interval
            / 3600,  # hours
        }

    def get_performance_history(
        self, hours: float | None = None, limit: int | None = None
    ) -> list[dict[str, Any]]:
    ...
    def get_alerts(
        self, unresolved_only: bool = True, limit: int | None = None
    ) -> list[dict[str, Any]]:
