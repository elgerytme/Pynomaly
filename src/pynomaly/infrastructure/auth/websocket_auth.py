        ...
        try:
            ...
            if "api_key" in query_params:
                ...
                try:
                    ...
                    if user:
                        logger.info(
                            f"WebSocket authenticated user via API key: {user.username}"
                        )
                        return user
                except Exception as e:
                ...
                if auth_header.startswith("Bearer "):
                    ...
                    if token.startswith("pyn_"):
                        try:
                            ...
                            if user:
                                logger.info(
                                    f"WebSocket authenticated user via header API key: {user.username}"
                                )
                                return user
                        except Exception as e:
                            logger.warning(
                                f"WebSocket header API key authentication failed: {e}"
                            )
                    else:
                        # JWT token
                        try:
                            ...
                            if user and user.is_active:
                                logger.info(
                                    f"WebSocket authenticated user via header JWT: {user.username}"
                                )
                                return user
                        except Exception as e:
                            logger.warning(
                                f"WebSocket header JWT authentication failed: {e}"
                            )
