...
class ResourceManager:
    ...
    def get_resource_utilization(self) -> dict[str, float]:
        ...
        return {
            "cpu": (
                (self.current_usage.cpu_cores / self.quota.max_cpu_cores) * 100
                if self.quota.max_cpu_cores > 0
                else 0
            ),
            "memory": (
                (self.current_usage.memory_gb / self.quota.max_memory_gb) * 100
                if self.quota.max_memory_gb > 0
                else 0
            ),
            "workers": (
                (self.current_usage.workers / self.quota.max_workers) * 100
                if self.quota.max_workers > 0
                else 0
            ),
            "gpu": (
                (self.current_usage.gpu_count / self.quota.max_gpu_count) * 100
