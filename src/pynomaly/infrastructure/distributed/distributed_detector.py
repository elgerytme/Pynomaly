...
from .data_partitioner import DataPartitioner, DataPartition, PartitionStrategy
from .task_distributor import TaskDistributor, DistributedTask, TaskType, TaskPriority
...
class DistributedDetectionResult(BaseModel):
    ...
    scores: List[AnomalyScore] = Field(default_factory=list, description="Anomaly scores")
    ...
    parallel_efficiency: float = Field(..., description="Parallel processing efficiency")
    
    # Resource usage
    total_memory_used_mb: float = Field(default=0.0, description="Total memory used in MB")
    peak_memory_usage_mb: float = Field(default=0.0, description="Peak memory usage in MB")
    
    # Quality metrics
    overall_confidence: float = Field(default=0.0, description="Overall detection confidence")
    chunk_consistency: float = Field(default=0.0, description="Consistency between chunks")
    
    # Performance metrics
    throughput_samples_per_second: float = Field(default=0.0, description="Processing throughput")
    speedup_factor: float = Field(default=1.0, description="Speedup vs sequential processing")
    ...
    # Chunk-level details
    chunk_results: List[ChunkResult] = Field(default_factory=list, description="Individual chunk results")
    ...
    def success_rate(self) -> float:
        ...
        successful_chunks = sum(1 for result in self.chunk_results if result.error is None)
...
class DistributedDetector:
    ...
    async def detect_anomalies(self,
                             ...
                             priority: TaskPriority = TaskPriority.NORMAL) -> DistributedDetectionResult:
        ...
        logger.info(f"Starting distributed detection {detection_id} with detector {detector.name}")
        ...
        try:
            ...
            logger.info(f"Completed distributed detection {detection_id}: "
    ...
    async def _partition_data(self,
                            data: Union[pd.DataFrame, np.ndarray],
                            num_partitions: Optional[int],
                            strategy: Optional[PartitionStrategy]) -> List[DataPartition]:
    ...
    async def _create_detection_chunks(self,
                                     partitions: List[DataPartition],
                                     detector: Detector) -> List[DetectionChunk]:
    ...
    async def _execute_distributed_detection(self,
                                           chunks: List[DetectionChunk],
                                           priority: TaskPriority) -> List[ChunkResult]:
        ...
        for chunk in chunks:
            task = DistributedTask(
                ...
                kwargs={
                    "detector_config": chunk.detector_config
                },
                context={
        ...
        for chunk, task_id in task_futures:
            try:
                ...
                if task_result and task_result.is_successful:
                    ...
                    chunk_result = self._convert_task_result_to_chunk_result(chunk, task_result)
    ...
    def _convert_task_result_to_chunk_result(self,
                                           chunk: DetectionChunk,
                                           task_result) -> ChunkResult:
        """Convert task result to chunk result."""
        ...
        if isinstance(result_data, dict):
            ...
            for i, score in enumerate(anomaly_scores):
                if i < len(anomaly_indices):
                    anomaly = Anomaly(
                        index=anomaly_indices[i] if isinstance(anomaly_indices, list) else i,
                        score=float(score),
    ...
    async def _aggregate_detection_results(self,
                                         result: DistributedDetectionResult,
        ...
        for chunk_result in chunk_results:
            if chunk_result.error is None:
                ...
                result.errors.append(f"Chunk {chunk_result.chunk_id}: {chunk_result.error}")
        
        # Update result
        result.anomalies = all_anomalies
        result.scores = all_scores
        result.total_memory_used_mb = total_memory
        result.peak_memory_usage_mb = max([cr.memory_used_mb for cr in successful_chunks], default=0.0)
        ...
        if successful_chunks:
            ...
            anomaly_rates = [len(cr.anomalies) / max(cr.samples_processed, 1) for cr in successful_chunks]
            if len(anomaly_rates) > 1:
                avg_rate = sum(anomaly_rates) / len(anomaly_rates)
                variance = sum((rate - avg_rate) ** 2 for rate in anomaly_rates) / len(anomaly_rates)
                result.chunk_consistency = max(0.0, 1.0 - variance)
            else:
                result.chunk_consistency = 1.0
    
    async def _calculate_final_metrics(self, result: DistributedDetectionResult) -> None:
        """Calculate final performance metrics."""
        # Throughput calculation
        if result.total_processing_time > 0:
            result.throughput_samples_per_second = result.total_samples / result.total_processing_time
        
        # Parallel efficiency calculation
        total_chunk_time = sum(cr.processing_time_seconds for cr in result.chunk_results)
        if total_chunk_time > 0:
            result.parallel_efficiency = min(1.0, total_chunk_time / result.total_processing_time)
        ...
        if estimated_sequential_time > 0:
            result.speedup_factor = estimated_sequential_time / result.total_processing_time
    
    def get_detection_status(self, detection_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "progress_percentage": (len(result.chunk_results) / max(result.chunks_processed, 1)) * 100
        }
    ...
    def get_performance_statistics(self) -> Dict[str, Any]:
        ...
        successful_detections = [d for d in self.detection_history if d.success_rate > 0.5]
        ...
        return {
            ...
            "average_processing_time": sum(d.total_processing_time for d in successful_detections) / len(successful_detections),
            "average_throughput": sum(d.throughput_samples_per_second for d in successful_detections) / len(successful_detections),
            "average_speedup": sum(d.speedup_factor for d in successful_detections) / len(successful_detections),
            "average_efficiency": sum(d.parallel_efficiency for d in successful_detections) / len(successful_detections),
            "average_workers_used": sum(d.workers_used for d in successful_detections) / len(successful_detections),
            "total_samples_processed": sum(d.total_samples for d in successful_detections),
            "total_anomalies_found": sum(len(d.anomalies) for d in successful_detections)
        }