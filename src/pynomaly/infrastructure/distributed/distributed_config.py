...
class NetworkConfig(BaseModel):
    ...
    protocol: CommunicationProtocol = Field(default=CommunicationProtocol.HTTP, description="Communication protocol")
    ...
    cert_file: Optional[str] = Field(default=None, description="SSL certificate file path")
    key_file: Optional[str] = Field(default=None, description="SSL private key file path")
...
class WorkerConfig(BaseModel):
    ...
    max_concurrent_tasks: int = Field(default=4, ge=1, le=100, description="Maximum concurrent tasks")
    task_timeout: float = Field(default=300.0, ge=10.0, le=3600.0, description="Task timeout in seconds")
    
    # Resource limits
    max_memory_mb: int = Field(default=4096, ge=512, le=32768, description="Maximum memory usage in MB")
    max_cpu_cores: int = Field(default=4, ge=1, le=64, description="Maximum CPU cores to use")
    max_gpu_memory_mb: int = Field(default=0, ge=0, le=32768, description="Maximum GPU memory in MB")
    
    # Capabilities
    supported_algorithms: Set[str] = Field(default_factory=set, description="Supported algorithms")
    hardware_acceleration: Set[str] = Field(default_factory=set, description="Available hardware acceleration")
    data_formats: Set[str] = Field(default_factory=set, description="Supported data formats")
    
    # Performance settings
    batch_size: int = Field(default=1000, ge=1, le=100000, description="Default batch size")
    prefetch_batches: int = Field(default=2, ge=1, le=10, description="Number of batches to prefetch")
    enable_caching: bool = Field(default=True, description="Enable result caching")
    cache_size_mb: int = Field(default=512, ge=64, le=8192, description="Cache size in MB")
    
    # Health monitoring
    heartbeat_interval: float = Field(default=30.0, ge=5.0, le=300.0, description="Heartbeat interval in seconds")
    health_check_interval: float = Field(default=60.0, ge=10.0, le=600.0, description="Health check interval")
    metrics_reporting_interval: float = Field(default=60.0, ge=10.0, le=600.0, description="Metrics reporting interval")
...
class FaultToleranceConfig(BaseModel):
    ...
    retry_delay: float = Field(default=1.0, ge=0.1, le=60.0, description="Initial retry delay in seconds")
    retry_exponential_backoff: bool = Field(default=True, description="Use exponential backoff for retries")
    max_retry_delay: float = Field(default=60.0, ge=1.0, le=600.0, description="Maximum retry delay")
    
    # Circuit breaker
    enable_circuit_breaker: bool = Field(default=True, description="Enable circuit breaker pattern")
    failure_threshold: int = Field(default=5, ge=1, le=100, description="Failure threshold for circuit breaker")
    recovery_timeout: float = Field(default=60.0, ge=10.0, le=600.0, description="Circuit breaker recovery timeout")
    
    # Failover
    enable_failover: bool = Field(default=True, description="Enable automatic failover")
    failover_timeout: float = Field(default=30.0, ge=5.0, le=300.0, description="Failover detection timeout")
    backup_nodes: List[str] = Field(default_factory=list, description="Backup node addresses")
    
    # Data redundancy
    replication_factor: int = Field(default=1, ge=1, le=5, description="Data replication factor")
    enable_checkpointing: bool = Field(default=True, description="Enable task checkpointing")
    checkpoint_interval: float = Field(default=300.0, ge=60.0, le=3600.0, description="Checkpoint interval")
...
class ClusterConfig(BaseModel):
    ...
    cluster_mode: ClusterMode = Field(default=ClusterMode.DISTRIBUTED, description="Cluster operation mode")
    
    # Node management
    min_nodes: int = Field(default=1, ge=1, le=100, description="Minimum required nodes")
    max_nodes: int = Field(default=10, ge=1, le=1000, description="Maximum allowed nodes")
    auto_scaling: bool = Field(default=False, description="Enable automatic scaling")
    scale_up_threshold: float = Field(default=0.8, ge=0.1, le=1.0, description="CPU threshold for scaling up")
    scale_down_threshold: float = Field(default=0.3, ge=0.1, le=1.0, description="CPU threshold for scaling down")
    
    # Coordination
    coordinator_nodes: List[str] = Field(default_factory=list, description="Coordinator node addresses")
    election_timeout: float = Field(default=150.0, ge=50.0, le=500.0, description="Leader election timeout")
    ...
    rebalance_interval: float = Field(default=300.0, ge=60.0, le=3600.0, description="Load rebalancing interval")
    
    # Monitoring
    enable_monitoring: bool = Field(default=True, description="Enable cluster monitoring")
    metrics_retention_days: int = Field(default=7, ge=1, le=90, description="Metrics retention period")
...
class DistributedConfig(BaseModel):
    ...
    mode: ClusterMode = Field(default=ClusterMode.STANDALONE, description="Operation mode")
    ...
    partition_strategy: PartitionStrategy = Field(
        default=PartitionStrategy.SIZE_BASED, 
    ...
    # Performance settings
    chunk_size: int = Field(default=10000, ge=100, le=1000000, description="Default chunk size for processing")
    parallel_chunks: int = Field(default=4, ge=1, le=100, description="Number of parallel chunks")
    ...
    # Storage settings
    shared_storage_path: Optional[str] = Field(default=None, description="Shared storage path")
    temp_storage_path: str = Field(default="/tmp/pynomaly", description="Temporary storage path")
    ...
    mock_workers: bool = Field(default=False, description="Use mock workers for testing")
...
class ConfigurationManager:
    ...
    def get_effective_config(self) -> DistributedConfig:
        ...
        if os.getenv('PYNOMALY_MAX_WORKERS'):
            effective_config.worker.max_concurrent_tasks = int(os.getenv('PYNOMALY_MAX_WORKERS'))
    ...
    def import_config(self, config_dict: Dict[str, any]) -> None:
        ...
        if "runtime" in config_dict:
            ...
            self.runtime = RuntimeConfig(
                ...
                total_network_bandwidth=runtime_data.get("total_network_bandwidth", 0.0),
...
def init_distributed_config(config: Optional[DistributedConfig] = None) -> ConfigurationManager:
