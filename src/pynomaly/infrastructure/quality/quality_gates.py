...
from ..monitoring.complexity_monitor import ComplexityMonitor, ComplexityMetrics
...
class QualityGateReport:
    ...
    def overall_percentage(self) -> float:
        ...
        return (self.overall_score / self.max_overall_score * 100) if self.max_overall_score > 0 else 0.0
...
class QualityGateValidator:
    ...
    def _check_code_style(self, feature_path: Path) -> QualityGateResult:
        ...
        try:
            ...
            if long_lines:
                style_issues.append(f"Long lines found: {len(long_lines)} lines exceed 88 characters")
            ...
            if trailing_whitespace_lines:
                style_issues.append(f"Trailing whitespace on {len(trailing_whitespace_lines)} lines")
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    has_param_hints = any(arg.annotation is not None for arg in node.args.args[1:])  # Skip self
                    ...
                    has_return_hint = node.returns is not None
                    has_param_hints = any(arg.annotation is not None for arg in node.args.args[1:])
    ...
    def _check_import_quality(self, feature_path: Path) -> QualityGateResult:
        ...
        try:
            ...
            absolute_imports = [imp for imp in all_imports if imp.startswith('from ') and not imp.startswith('from .')]
            
            if relative_imports and absolute_imports:
                if len(relative_imports) < len(absolute_imports) / 2:
                    issues.append("Inconsistent import style (mix of relative and absolute)")
                    score -= 1.0
    ...
    def _check_algorithmic_complexity(self, feature_path: Path) -> QualityGateResult:
        ...
        try:
            ...
            if complexity_issues:
                ...
                recommendations.append("Consider vectorized operations for data processing")
    ...
    def _check_clean_architecture(self, feature_path: Path) -> QualityGateResult:
        ...
        try:
            ...
            if 'domain' in path_parts:
                ...
                content = feature_path.read_text(encoding='utf-8')
                if 'from ...application' in content or 'from ...infrastructure' in content:
                    architecture_issues.append("Domain layer imports from application/infrastructure")
                    score -= 3.0
                ...
                if 'from ...infrastructure' in content and 'protocols' not in content:
                    ...
                    if not any(allowed in content for allowed in ['protocols', 'adapters', 'config']):
                        architecture_issues.append("Application layer imports from infrastructure")
                        score -= 2.0
            ...
            if architecture_issues:
                ...
                recommendations.append("Use dependency injection for infrastructure dependencies")
    ...
    def _check_interface_design(self, feature_path: Path) -> QualityGateResult:
        ...
        return QualityGateResult(
            ...
            recommendations=["Design clear, focused interfaces following SOLID principles"]
        )
