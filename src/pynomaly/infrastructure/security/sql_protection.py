...
class SQLInjectionProtector:
    ...
    def __init__(self):
        ...
        self.injection_patterns = [
            ...
            # Blind injection
            (r"(?:SUBSTRING|SUBSTR|MID)\s*\(.*,\s*\d+\s*,\s*\d+\s*\)", "Substring injection"),
            ...
            (r"(?:DATABASE|VERSION|USER|CURRENT_USER)\s*\(\s*\)", "Information gathering"),
        ...
        self.dangerous_keywords = {
            ...
            'xp_', 'sp_', 'OPENROWSET', 'OPENDATASOURCE', 'BULK', 'LOAD_FILE',
            'INTO OUTFILE', 'INTO DUMPFILE', 'SCRIPT', 'SHUTDOWN'
        }
        
        # Safe SQL functions (whitelist)
        self.safe_functions = {
            'COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'UPPER', 'LOWER', 'TRIM',
            'LENGTH', 'ROUND', 'ABS', 'CEIL', 'FLOOR', 'NOW', 'CURDATE',
            'CURTIME', 'DATE', 'TIME', 'YEAR', 'MONTH', 'DAY'
        }
    
    def analyze_query(self, query: str, parameters: Optional[Dict[str, Any]] = None) -> QueryAnalysisResult:
    ...
    def _analyze_parameters(self, parameters: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
            ...
            if isinstance(value, str):
                ...
                for pattern, description in self.compiled_patterns:
                    if pattern.search(value):
                        threats.append(f"Parameter '{key}' contains potential injection: {description}")
                        continue
    ...
    def validate_query_safety(self, query: str, parameters: Optional[Dict[str, Any]] = None) -> bool:
...
class QuerySanitizer:
    ...
    def sanitize_query(self, query: str) -> str:
        ...
        dangerous_functions = [
            'xp_cmdshell', 'sp_execute', 'OPENROWSET', 'OPENDATASOURCE',
...
class SafeQueryBuilder:
    ...
    def _build_where_clause(self, conditions: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
...
# Global instances
_protector: Optional[SQLInjectionProtector] = None
