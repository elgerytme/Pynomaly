...
class AuthenticationService:
    ...
    def authenticate_user(
        ...
        if username in self._locked_accounts:
            ...
            if datetime.utcnow() - lock_time < timedelta(minutes=self.config.account_lockout_duration_minutes):
        ...
        if not stored_password_hash or not self.verify_password(password, stored_password_hash):
    ...
    def _record_failed_login(self, username: str, ip_address: str) -> None:
        ...
        cutoff_time = datetime.utcnow() - timedelta(minutes=self.config.account_lockout_duration_minutes)
        self._failed_login_attempts[username] = [
        ...
        if len(self._failed_login_attempts[username]) >= self.config.max_failed_login_attempts:
        ...
        roles = [
            SecurityRole(
                ...
                permissions={
                    ...
                    "model:train", "model:predict", "model:deploy", "user:manage"
                },
            ...
            SecurityRole(
                ...
                permissions={
                    "data:read", "model:train", "model:predict", "model:analyze"
                },
        ...
        for role_id in user_roles:
            ...
            if role:
                ...
                if permission in role.permissions:
                    ...
                    if self._can_access_security_level(role.security_level, resource_security_level):
                        return True
        
        self.logger.warning(f"Permission denied: user={user_id}, permission={permission}")
        return False
...
class SecurityService:
    ...
    def authenticate_request(
        ...
        # Check authorization
        if not self.authz_service.check_permission(user_id, required_permission, resource_security_level):
            self.audit_service.log_event(
    ...
    def _check_rate_limit(self, identifier: str) -> bool:
        ...
        if identifier in self._rate_limit_tracking:
            self._rate_limit_tracking[identifier] = [
                timestamp for timestamp in self._rate_limit_tracking[identifier]
        ...
        # Check limit
        if len(self._rate_limit_tracking[identifier]) >= self.config.rate_limit_requests_per_minute:
            return False
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "encryption_enabled": CRYPTOGRAPHY_AVAILABLE and self.config.enable_encryption_at_rest,
