...
def _async_rate_limited_wrapper(func: Callable, config: RateLimitDecoratorConfig) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if config.algorithm == RateLimitAlgorithm.EXPONENTIAL_BACKOFF:
                ...
                limiter = await manager.get_or_create_limiter(config.limiter_name, rate_config)
                await limiter.record_success(identifier, config.scope, config.operation_name or func.__name__)
            ...
            if config.algorithm == RateLimitAlgorithm.EXPONENTIAL_BACKOFF:
                manager = get_rate_limit_manager()
                limiter = await manager.get_or_create_limiter(config.limiter_name, rate_config)
                await limiter.record_failure(identifier, config.scope, config.operation_name or func.__name__)
...
def _sync_rate_limited_wrapper(func: Callable, config: RateLimitDecoratorConfig) -> Callable:
...
def _extract_identifier(args: tuple, kwargs: dict, config: RateLimitDecoratorConfig) -> str:
