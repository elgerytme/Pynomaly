...
class InputSanitizer:
    ...
    def sanitize_string(self, value: str, field_name: Optional[str] = None) -> str:
        ...
        if not isinstance(value, str):
            raise ValidationError(f"Expected string, got {type(value)}", field_name, str(value))
        
        # Check length
        if len(value) > self.config.max_length:
            raise ValidationError(
                f"Input too long: {len(value)} > {self.config.max_length}",
                field_name, value[:100] + "..."
            )
