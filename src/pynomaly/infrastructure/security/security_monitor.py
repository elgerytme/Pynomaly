...
from .audit_logger import AuditLogger, SecurityEventType, AuditLevel
        ...
        if event_type == SecurityEventType.AUTH_LOGIN_FAILURE:
            ...
            if len(attempts) >= self.max_attempts:
                ...
                return SecurityAlert(
                    ...
                    evidence=[f"Failed login attempts: {len(attempts)} in {self.time_window}s"],
                    recommended_actions=[
                        "Block IP address",
...
class AnomalousAccessDetector(ThreatDetector):
    ...
    async def analyze(self, event_data: Dict[str, Any]) -> Optional[SecurityAlert]:
        ...
        usual_hours = {datetime.fromtimestamp(t).hour for t in user_pattern['login_times'][-50:]}
        ...
        if anomalies:
            return SecurityAlert(
                ...
                threat_level=ThreatLevel.MEDIUM if len(anomalies) == 1 else ThreatLevel.HIGH,
...
class SecurityMonitor:
    ...
    async def _handle_alert(self, alert: SecurityAlert) -> None:
        ...
        self.audit_logger.log_security_event(
            ...
            level=AuditLevel.WARNING if alert.threat_level in [ThreatLevel.LOW, ThreatLevel.MEDIUM] else AuditLevel.ERROR,
            details={
        ...
        logger.warning(f"Security alert generated: {alert.title} (ID: {alert.alert_id})")
    ...
    def get_active_alerts(self, 
                         ...
                         alert_type: Optional[AlertType] = None) -> List[SecurityAlert]:
    ...
    def get_security_metrics(self, 
                           metric_type: SecurityMetricType,
                           time_range: Optional[timedelta] = None) -> List[SecurityMetric]:
...
def init_security_monitor(audit_logger: Optional[AuditLogger] = None) -> SecurityMonitor:
