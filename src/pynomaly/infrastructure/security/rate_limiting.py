...
class SlidingWindowCounter:
    ...
    async def add_request(self, timestamp: Optional[float] = None) -> bool:
...
class ExponentialBackoffLimiter:
    ...
    def __init__(self, base_delay: float = 1.0, multiplier: float = 2.0, max_delay: float = 3600.0):
    ...
    async def check_request(self) -> Tuple[bool, float]:
        ...
        async with self.lock:
            ...
            delay = min(self.base_delay * (self.multiplier ** (self.failure_count - 1)), self.max_delay)
...
class RateLimiter:
        ...
        if key not in self.limiters:
            if self.config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
                ...
                self.limiters[key] = SlidingWindowCounter(
                    ...
                    max_requests=int(self.config.get_limit_for_operation(operation) * self.config.window_size),
                )
    ...
    async def _check_algorithm_limit(self, limiter: Any, tokens: int, operation: str) -> RateLimitStatus:
        """Check rate limit based on algorithm."""
        current_time = time.time()
        ...
        if self.config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
            ...
            return RateLimitStatus(
                ...
                reset_time=current_time + (limiter.capacity - limiter_status["tokens"]) / limiter.refill_rate,
    ...
    async def record_success(self, identifier: str, scope: RateLimitScope = RateLimitScope.GLOBAL, operation: str = "default") -> None:
    ...
    async def record_failure(self, identifier: str, scope: RateLimitScope = RateLimitScope.GLOBAL, operation: str = "default") -> None:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self.lock:
            return {
                ...
                "recent_violations": len([v for v in self.violations if time.time() - v.violation_time < 3600]),
                "violation_rate": self.stats["violations"] / max(1, self.stats["total_requests"]),
            }
    ...
    async def reset_limiter(self, identifier: str, scope: RateLimitScope = RateLimitScope.GLOBAL, operation: str = "default") -> bool:
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        async with self.lock:
            ...
            self.violations = [v for v in self.violations if v.violation_time >= cutoff_time]
...
class RateLimitManager:
    ...
    async def get_or_create_limiter(self, name: str, config: Optional[RateLimitConfig] = None) -> RateLimiter:
...
async def check_rate_limit(
    ...
    return await manager.check_limit(limiter_name, identifier, scope, operation, tokens, config)
...
async def rate_limit_context(
    ...
    status = await check_rate_limit(identifier, limiter_name, scope, operation, tokens, config)
