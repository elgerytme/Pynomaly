...
from .audit_logger import AuditLevel, AuditLogger, SecurityEventType, audit_context
...
class UserActionTracker:
    ...
    async def track_request_end(
        self, request: Request, response: Response, request_info: RequestInfo
    ) -> UserAction:
        ...
        user_action = UserAction(
            ...
            metadata={
                ...
                "is_sensitive_path": self.data_filter.is_sensitive_path(
                    request_info.path
                ),
            },
        )
    ...
    def _classify_action(
        self, request_info: RequestInfo, response_info: ResponseInfo
    ) -> tuple[str, str]:
        ...
        # File downloads
        if "download" in path or request_info.headers.get("accept", "").startswith(
            "application/"
        ):
    ...
    def _extract_resource(self, request_info: RequestInfo) -> str | None:
...
class UserTrackingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, tracker: UserActionTracker | None = None):
...
_user_tracker: UserActionTracker | None = None
