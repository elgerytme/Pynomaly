...
class ParallelCSVLoader:
    ...
    def __init__(self, max_workers: int = 4, optimized_loader_config: Optional[Dict[str, Any]] = None):
    ...
    async def load_multiple_files(self, file_paths: List[Path]) -> List[Dataset]:
        ...
        self.logger.info(f"Loading {len(file_paths)} files in parallel with {self.max_workers} workers")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            self.logger.info(f"Successfully loaded {len(successful_datasets)}/{len(file_paths)} files")
