...
class DVCConfig(BaseModel):
    ...
    remote_name: Optional[str] = Field(default=None, description="DVC remote name")
    cache_dir: Optional[Path] = Field(default=None, description="Custom DVC cache directory")
    auto_stage: bool = Field(default=True, description="Automatically stage files to DVC")
...
class DVCLoader(DataLoaderProtocol):
    ...
    def _dvc_pull(self, file_path: str) -> None:
        ...
        try:
            ...
            if "no remote" not in e.stderr.lower() and "not found" not in e.stderr.lower():
                raise DataLoadingError(f"Failed to pull file from DVC: {e.stderr}") from e
    ...
    def checkout_version(self, file_path: str, version: str) -> None:
        ...
        try:
            ...
            raise DataLoadingError(f"Failed to checkout version {version}: {e.stderr}") from e


def create_dvc_loader(repo_path: Optional[Path] = None, **kwargs: Any) -> DVCLoader:
