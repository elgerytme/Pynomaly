...
class DatabaseLoader(DatabaseLoaderProtocol, BatchDataLoaderProtocol):
        ...
        try:
            ...
            if chunksize:
                ...
                for chunk in pd.read_sql(
                    query, engine, params=params, chunksize=chunksize
                ):
                    chunks.append(chunk)
            ...
            self.logger.info(
                f"Loaded {len(df)} rows, {len(df.columns)} columns from query"
            )
            return dataset

        except SQLAlchemyError as e:
            raise DataValidationError(f"Database query failed: {e}", query=query) from e
        except Exception as e:
            raise DataValidationError(
                f"Failed to load data from query: {e}", query=query
            ) from e
    ...
    def load_table(
        ...
        **kwargs: Any,
        ...
        """
        self.logger.info(
            f"Loading table: {schema}.{table_name}" if schema else table_name
        )
        ...
        try:
            ...
            self.logger.info(
                f"Loaded table {full_table_name}: {len(df)} rows, {len(df.columns)} columns"
            )
            return dataset
            ...
            raise DataValidationError(
                f"Failed to load table data: {e}", table_name=table_name
            ) from e
    ...
    def load_batch(
        ...
        **kwargs: Any,
        ...
        is_query = any(
            keyword in source_str.upper() for keyword in ["SELECT", "FROM", "WHERE"]
        )
        ...
        try:
            ...
            chunk_iter = pd.read_sql(
                query, engine, chunksize=batch_size, params=kwargs.get("params", {})
            )
        ...
        try:
            ...
            if is_query:
                ...
                if isinstance(row_count, int) and row_count > 0:
                    ...
                    estimated_memory_mb = (row_count * column_count * 50) / (
                        1024 * 1024
                    )
                else:
