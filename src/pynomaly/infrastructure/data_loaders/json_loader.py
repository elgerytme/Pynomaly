...
class JSONLoader(BatchDataLoaderProtocol):
        ...
        **kwargs: Any
        ...
        if not self.validate(source_path):
            raise DataValidationError(
                f"Invalid JSON file: {source_path}",
    ...
    def load_batch(
        ...
        **kwargs: Any
        ...
        dataset_name = name or source_path.stem
        target_column = kwargs.get("target_column")
        is_lines = kwargs.get("lines", self.lines) or source_path.suffix.lower() == ".jsonl"
        ...
        if not is_lines:
            ...
            try:
                                ...
                                if self.normalize_nested:
                                    ...
                                    for col in batch_df.columns:
                                        if batch_df[col].dtype == 'object':
                                            sample_val = batch_df[col].dropna().iloc[0] if not batch_df[col].dropna().empty else None
                                            if isinstance(sample_val, dict):
                                                try:
                                                    normalized = pd.json_normalize(batch_df[col])
                                                    normalized.columns = [f"{col}_{subcol}" for subcol in normalized.columns]
                                                    batch_df = batch_df.drop(columns=[col])
                                                    batch_df = pd.concat([batch_df, normalized], axis=1)
                                                except Exception:
                                                    pass
                ...
                raise DataValidationError(
                    f"Failed to load JSON in batches: {e}",
                    file_path=str(source_path)
                ) from e
            ...
            with open(source_path, "r", encoding=self.encoding) as f:
                ...
                if is_lines:
                    ...
                    if sample_lines:
                        ...
                        estimated_rows = max(len(sample_lines), int(file_size_bytes / avg_line_size))
                    ...
                    try:
                        ...
                        if isinstance(sample_data, list):
                            ...
                            if sample_data:
                                sample_df = pd.DataFrame(sample_data[:100])  # First 100 records
                                n_columns = len(sample_df.columns)
                            else:
                                n_columns = 0
