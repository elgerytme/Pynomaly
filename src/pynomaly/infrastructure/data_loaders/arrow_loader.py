...
class ArrowLoader(DataLoaderProtocol):
    ...
    def __init__(self, use_threads: bool = True, memory_pool: Optional[Any] = None):
            ...
            if transform_type == 'filter':
                ...
                if operation == 'normalize':
                    ...
                    zscore = self.pc.divide(
                        self.pc.subtract(col_data, mean_val),
                        std_val
                    )
                ...
                logger.warning("Group by aggregation not fully implemented in this version")
    ...
    def _extract_metadata(self, table: Any, file_path: Path, **kwargs) -> Dict[str, Any]:
        ...
        **kwargs
        ...
        try:
            if file_path.suffix.lower() in ['.parquet', '.pq']:
                async for batch in self._stream_parquet(file_path, batch_size, **kwargs):
                    yield batch
    ...
    async def validate_file(self, file_path: Union[str, Path]) -> bool:
        ...
        supported_formats = {'.parquet', '.pq', '.csv', '.json', '.jsonl', '.ndjson', '.arrow', '.feather'}
        return file_path.suffix.lower() in supported_formats
    
    def get_supported_formats(self) -> List[str]:
        """Get list of supported file formats."""
        return ['.parquet', '.pq', '.csv', '.json', '.jsonl', '.ndjson', '.arrow', '.feather']
    ...
    def get_compute_functions(self) -> List[str]:
        ...
        return [
            ...
            'stddev', 'variance', 'ln', 'log10',
            'abs', 'ceil', 'floor', 'round',
            'is_null', 'is_valid', 'fill_null',
