...
class ExcelLoader(BatchDataLoaderProtocol):
    ...
    def load(
        ...
        if not self.validate(source_path):
            raise DataValidationError(
                f"Invalid Excel file: {source_path}", file_path=str(source_path)
            )
        ...
        read_options = {
            ...
            **{
                ...
                if k not in ["sheet_name", "header", "skiprows", "target_column"]
            },
        }
        ...
        try:
            ...
            if df.empty:
                raise DataValidationError(
                    "Excel file is empty", file_path=str(source_path)
                )
            ...
            raise DataValidationError(
                f"Excel file not found: {source_path}", file_path=str(source_path)
            )
            ...
            if "xlrd" in str(e) or "openpyxl" in str(e):
                raise DataValidationError(
                    "Missing required Excel library. Install with: pip install openpyxl xlrd",
                    file_path=str(source_path),
                )
            raise DataValidationError(
                f"Failed to load Excel: {e}", file_path=str(source_path)
            ) from e
    ...
    def estimate_size(self, source: str | Path) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(main_sheet, int):
                main_sheet = (
                    sheet_names[main_sheet]
                    if main_sheet < len(sheet_names)
                    else sheet_names[0]
                )
            ...
            try:
                sample_df = pd.read_excel(
                    source_path, sheet_name=main_sheet, nrows=10, header=self.header
                )
                ...
                numeric_columns = len(
                    sample_df.select_dtypes(include=["number"]).columns
                )
    ...
    def _get_sheet_info(self, source_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"sheet_names": sheet_names, "sheet_count": len(sheet_names)}

        except Exception:
            return {"sheet_names": [], "sheet_count": 0}

    def list_sheets(self, source: str | Path) -> list[str]:
        ...
        try:
            ...
            raise DataValidationError(
                f"Failed to read Excel sheets: {e}", file_path=str(source_path)
            ) from e
