...
class EnhancedParquetLoader(BatchDataLoaderProtocol):
        ...
        **kwargs: Any
        ...
        if not self.validate(source_path):
            raise DataValidationError(
                f"Invalid Parquet source: {source_path}",
        ...
        read_options = {
            ...
            "use_pandas_metadata": kwargs.pop("use_pandas_metadata", self.use_pandas_metadata),
            **kwargs,
        }
    ...
    def validate(self, source: Union[str, Path]) -> bool:
        ...
        if source_path.is_dir():
            ...
            parquet_files = list(source_path.glob("*.parquet")) + list(source_path.glob("*.pq"))
    ...
    def load_batch(
        ...
        **kwargs: Any,
        ...
        if not self.validate(source_path):
            raise DataValidationError(
                f"Invalid Parquet file: {source_path}",
                file_path=str(source_path)
            )
        ...
        try:
            if self.engine == "pyarrow":
                ...
                row_group_batch_size = max(1, batch_size // 1000)  # Adjust based on typical row group size
                
                for batch_idx in range(0, parquet_file.num_row_groups, row_group_batch_size):
                    end_idx = min(batch_idx + row_group_batch_size, parquet_file.num_row_groups)
                ...
                full_df = pd.read_parquet(source_path, engine=self.engine, columns=self.columns)
            ...
            raise DataValidationError(
                f"Failed to load Parquet in batches: {e}",
                file_path=str(source_path)
            ) from e
        ...
        try:
            ...
            if self.engine == "pyarrow":
                ...
                return {
                    ...
                    "avg_row_group_size": total_rows // num_row_groups if num_row_groups > 0 else 0,
        ...
        try:
            if self.engine == "pyarrow":
                ...
                return {
                    ...
                    "pandas_metadata": schema.pandas_metadata if hasattr(schema, 'pandas_metadata') else None,
                }
