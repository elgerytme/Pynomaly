...
class ValidationIssue:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'column': self.column,
            'row_indices': self.row_indices[:10] if self.row_indices else None,  # Limit sample
            'value_sample': self.value_sample[:5] if self.value_sample else None,  # Limit sample
...
class DataValidator:
        ...
        with monitor_operation("dataset_validation", "data_validator"):
            ...
            if custom_rules:
                for rule_name, rule_config in custom_rules.items():
                    try:
                        custom_issues = self._run_custom_rule(dataset.data, rule_name, rule_config)
            ...
            if self.fail_on_critical and result.has_critical_issues:
                critical_messages = [issue.message for issue in result.get_issues_by_severity(ValidationSeverity.CRITICAL)]
                raise ValidationError(f"Critical validation issues: {'; '.join(critical_messages)}")
        ...
        for col in df.columns:
            ...
            if missing_pct > 50:
                issues.append(ValidationIssue(
                    ...
                    column=col,
                    metadata={'missing_count': missing_count, 'missing_percentage': missing_pct},
    ...
    def _validate_duplicates(self, df: pd.DataFrame) -> List[ValidationIssue]:
        ...
        if duplicate_rows > 0:
            ...
            severity = ValidationSeverity.ERROR if duplicate_pct > 10 else ValidationSeverity.WARNING
            
            issues.append(ValidationIssue(
                category=ValidationCategory.CONSISTENCY,
                severity=severity,
                message=f"{duplicate_rows} ({duplicate_pct:.1f}%) duplicate rows found",
                metadata={'duplicate_count': duplicate_rows, 'duplicate_percentage': duplicate_pct},
    ...
    def _validate_outliers(self, df: pd.DataFrame) -> List[ValidationIssue]:
        ...
        for col in numeric_cols:
            ...
            if IQR > 0:
                ...
                if outlier_pct > 5:  # More than 5% outliers might indicate data quality issues
                    issues.append(ValidationIssue(
                        category=ValidationCategory.QUALITY,
                        severity=ValidationSeverity.WARNING,
    ...
    def _run_custom_rule(self, df: pd.DataFrame, rule_name: str, rule_config: Dict[str, Any]) -> List[ValidationIssue]:
...
def validate_file_format(file_path: Union[str, Path]) -> ValidationResult:
