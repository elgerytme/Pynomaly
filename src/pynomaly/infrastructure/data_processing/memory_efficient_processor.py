...
class MemoryOptimizedDataLoader:
    ...
    def load_csv(
        ...
        with monitor_memory_usage(f"load_csv_{file_path.name}"):
            ...
            for chunk_df in pd.read_csv(
                file_path, chunksize=chunk_size, **pandas_kwargs
            ):
        ...
        with monitor_memory_usage(f"load_parquet_{file_path.name}"):
            ...
            try:
                ...
                for start_row in range(0, total_rows, chunk_size):
                    ...
                    chunk_df = pd.read_parquet(file_path, **pandas_kwargs).iloc[
                        start_row:end_row
                    ]
    ...
    def _determine_optimal_chunk_size(self, file_path: Path) -> int:
        ...
        try:
            ...
            available_memory = (
                get_memory_usage().available_mb * 0.8 * 1024 * 1024
            )  # bytes
...
class StreamingDataProcessor:
    ...
    def __init__(
        self,
        loader: MemoryOptimizedDataLoader | None = None,
...
class LargeDatasetAnalyzer:
    ...
    def __init__(self, loader: MemoryOptimizedDataLoader | None = None):
        """Initialize analyzer."""
        self.loader = loader or MemoryOptimizedDataLoader()
        ...
        with monitor_memory_usage("dataset_analysis"):
            ...
            for chunk in self.loader.load_dataset(dataset):
                ...
                if first_chunk:
                    ...
                    stats["column_info"] = {
                        col: {"dtype": str(chunk_data[col].dtype)}
