...
class AdvancedDataPipeline:
    ...
    def transform_new_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._fitted_transformers:
            raise ValueError("No fitted transformers available. Process training data first.")
        ...
        if "imputer" in self._fitted_transformers:
            ...
            if len(numeric_cols) > 0:
                transformed_data[numeric_cols] = imputer.transform(transformed_data[numeric_cols])
        ...
        if "scaler" in self._fitted_transformers:
            ...
            numeric_cols = transformed_data.select_dtypes(include=[np.number]).columns
            if len(numeric_cols) > 0:
                transformed_data[numeric_cols] = scaler.transform(transformed_data[numeric_cols])
        ...
        if "feature_selector" in self._fitted_transformers:
            ...
            if hasattr(selector, 'transform'):
                ...
                if hasattr(selector, 'get_support'):
                    ...
                    selected_columns = [col for col, selected in zip(transformed_data.columns, mask) if selected]
                    transformed_data = pd.DataFrame(selected_features, columns=selected_columns, index=transformed_data.index)
    ...
    def _step_validate_data(self, data: pd.DataFrame, report: ProcessingReport) -> None:
        ...
        if not is_valid and self.config.strict_validation:
            raise DataValidationError(
                f"Data validation failed: {errors_list}",
                pipeline_step="validate_data"
            )
    
    def _step_remove_duplicates(self, data: pd.DataFrame, report: ProcessingReport) -> pd.DataFrame:
    ...
    def _step_handle_missing(
        ...
        if cols_to_drop:
            ...
            self.logger.info(f"Dropped columns with >{self.config.missing_threshold*100}% missing: {cols_to_drop}")
        ...
        if "imputer" in self._fitted_transformers and len(numeric_cols) > 0:
            data[numeric_cols] = self._fitted_transformers["imputer"].transform(data[numeric_cols])
    ...
    def _step_optimize_dtypes(self, data: pd.DataFrame, report: ProcessingReport) -> pd.DataFrame:
    ...
    def _step_encode_categoricals(
        ...
        for col in categorical_cols:
            ...
            if data[col].nunique() > self.config.max_categories:
                self.logger.warning(f"Skipping {col}: too many categories ({data[col].nunique()})")
                continue
    ...
    def _step_scale_features(
        ...
        if "scaler" in self._fitted_transformers:
            data[numeric_cols] = self._fitted_transformers["scaler"].transform(data[numeric_cols])
    ...
    def _step_remove_low_variance(
        ...
        if "variance_selector" in self._fitted_transformers:
            ...
            selected_df = pd.DataFrame(selected_features, columns=selected_columns, index=data.index)
            ...
            if removed_features > 0:
                ...
                report.warnings.append(f"Removed {removed_features} low variance features")
    ...
    def _step_feature_selection(
        ...
        if "feature_selector" in self._fitted_transformers:
            ...
            removed_features = len(numeric_cols) - len(selected_columns)
            self.logger.info(f"Selected {len(selected_columns)} features, removed {removed_features}")
            report.warnings.append(f"Feature selection: kept {len(selected_columns)}, removed {removed_features}")
    ...
    def _apply_validation_rule(
        ...
        if rule.rule_type == "range":
            ...
            if expected_type and column_data.dtype != expected_type:
                if rule.severity == "error":
                    errors_list.append(f"{message}: expected {expected_type}, got {column_data.dtype}")
                else:
                    warnings_list.append(f"{message}: expected {expected_type}, got {column_data.dtype}")
            ...
            if null_pct > max_null_pct:
                if rule.severity == "error":
                    errors_list.append(f"{message}: {null_pct:.2%} null values (max allowed: {max_null_pct:.2%})")
                else:
                    warnings_list.append(f"{message}: {null_pct:.2%} null values (max allowed: {max_null_pct:.2%})")
