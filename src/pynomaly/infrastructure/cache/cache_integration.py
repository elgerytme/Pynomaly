...
class CacheConfiguration:
    ...
    def from_environment(cls) -> CacheConfiguration:
        ...
        return cls(
            ...
            max_memory_size=int(
                os.getenv("PYNOMALY_CACHE_MEMORY_SIZE", str(100 * 1024 * 1024))
            ),
            compression_threshold=int(
                os.getenv("PYNOMALY_CACHE_COMPRESSION_THRESHOLD", "1024")
            ),
            prefetch_enabled=os.getenv("PYNOMALY_CACHE_PREFETCH", "true").lower()
            == "true",
            adaptive_ttl=os.getenv("PYNOMALY_CACHE_ADAPTIVE_TTL", "true").lower()
            == "true",
            health_check_interval=int(
                os.getenv("PYNOMALY_CACHE_HEALTH_CHECK_INTERVAL", "60")
            ),
        )
...
class CacheHealthMonitor:
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        healthy_count = sum(
            1 for check in recent_checks if check["overall_health"] == "healthy"
        )
        degraded_count = sum(
            1 for check in recent_checks if check["overall_health"] == "degraded"
        )
        unhealthy_count = sum(
            1 for check in recent_checks if check["overall_health"] == "unhealthy"
        )
...
class CacheIntegrationManager:
    ...
    def __init__(self, config: CacheConfiguration):
        ...
        self.intelligent_cache: IntelligentCacheManager | None = None
        self.health_monitor: CacheHealthMonitor | None = None
...
_cache_integration_manager: CacheIntegrationManager | None = None
...
@asynccontextmanager
async def cache_system_context(config: CacheConfiguration | None = None):
