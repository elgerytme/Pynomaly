...
from .redis_cache import RedisCache, CacheKeys
...
class IntelligentCacheManager:
    ...
    async def delete_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            pattern_keys = [
                key for key in self.access_patterns.keys()
                if self._matches_pattern(key, pattern)
            ]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        memory_stats = {
            ...
            "utilization": sum(entry.size for entry in self.memory_cache.values()) / self.max_memory_size,
        }
    ...
    async def _set_in_memory(self, key: str, value: Any, ttl: Optional[int]) -> None:
    ...
    async def _optimize_cache(self) -> None:
        ...
        logger.debug(f"Cache optimization completed, removed {len(keys_to_remove)} old patterns")
    ...
    async def _async_redis_set(self, key: str, value: Any, ttl: Optional[int]) -> None:
...
_intelligent_cache_manager: Optional[IntelligentCacheManager] = None
...
def get_intelligent_cache_manager(
    ...
    if _intelligent_cache_manager is None:
        if redis_cache is None:
            raise ValueError("Redis cache instance required for intelligent cache manager")
