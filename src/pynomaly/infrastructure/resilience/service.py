...
class ResilienceService:
        ...
        def decorator(func: Callable[..., T]) -> Callable[..., T]:
            ...
            if enable_retry:
                if retry_config:
                    ...
                    wrapped_func = self._get_default_retry_decorator(operation_name)(wrapped_func)
            
            # Apply circuit breaker (outermost layer)
            if enable_circuit_breaker:
                breaker = self._get_or_create_circuit_breaker(operation_name, circuit_breaker_config)
    ...
    def get_health_status(self) -> dict:
        ...
        total_calls = sum(stats["total_calls"] for stats in circuit_breaker_stats.values())
        total_failures = sum(stats["failed_calls"] for stats in circuit_breaker_stats.values())
