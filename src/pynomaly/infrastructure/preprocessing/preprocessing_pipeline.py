...
from .data_transformer import DataTransformer, ScalingStrategy, EncodingStrategy, FeatureSelectionStrategy
...
class PreprocessingPipeline:
    ...
    def __init__(self, name: str = "preprocessing_pipeline"):
        ...
        self._fitted = False
        self._fit_dataset_id: Optional[str] = None
    ...
    def add_missing_value_handling(
        self,
        strategy: Union[MissingValueStrategy, str] = MissingValueStrategy.FILL_MEDIAN,
        threshold: float = 0.5,
        fill_value: Any = None,
    ...
    def add_outlier_handling(
        self,
        strategy: Union[OutlierStrategy, str] = OutlierStrategy.CLIP,
        method: str = "iqr",
        threshold: float = 1.5,
        columns: Optional[List[str]] = None,
        enabled: bool = True
    ...
    def _apply_step(self, step: PreprocessingStep, dataset: Dataset, fit: bool = True) -> Dataset:
    ...
    def create_basic_pipeline(cls) -> PreprocessingPipeline:
        ...
        pipeline.add_duplicate_removal(
            description="Remove duplicate rows"
        )
        
        pipeline.add_infinite_value_handling(
            strategy="replace",
            description="Replace infinite values"
        )
