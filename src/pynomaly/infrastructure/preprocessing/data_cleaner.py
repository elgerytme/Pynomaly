...
class DataCleaner:
    ...
    def handle_missing_values(
        self,
        dataset: Dataset,
        strategy: Union[MissingValueStrategy, str] = MissingValueStrategy.DROP_ROWS,
        threshold: float = 0.5,
        fill_value: Any = None,
        ...
        for col in columns:
            ...
            if strategy == OutlierStrategy.REMOVE:
                ...
                if (df[col] > 0).all():
                    ...
                    warnings.warn(f"Cannot apply log transform to {col} with non-positive values")
                ...
                if (df[col] >= 0).all():
                    ...
                    warnings.warn(f"Cannot apply sqrt transform to {col} with negative values")
    ...
    def comprehensive_clean(
        self,
        dataset: Dataset,
        missing_strategy: Union[MissingValueStrategy, str] = MissingValueStrategy.FILL_MEDIAN,
        outlier_strategy: Union[OutlierStrategy, str] = OutlierStrategy.CLIP,
