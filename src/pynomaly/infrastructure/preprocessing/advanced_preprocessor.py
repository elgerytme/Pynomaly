...
class PreprocessingConfig:
    ...
    # Feature selection
    feature_selection_method: str = "variance"  # "variance", "correlation", "mutual_info", "rfe"
...
class AdvancedPreprocessor:
        ...
        try:
            ...
            for step in preprocessing_config.steps:
                ...
                if step == PreprocessingStep.MISSING_VALUE_IMPUTATION:
                    data, step_metadata = await self._handle_missing_values(data, preprocessing_config)
                
                elif step == PreprocessingStep.OUTLIER_DETECTION:
                    data, step_metadata = await self._handle_outliers(data, preprocessing_config)
                    ...
                    data, step_metadata = await self._scale_features(data, preprocessing_config)
                
                elif step == PreprocessingStep.FEATURE_ENCODING:
                    data, step_metadata = await self._encode_features(data, preprocessing_config)
                
                elif step == PreprocessingStep.FEATURE_TRANSFORMATION:
                    data, step_metadata = await self._transform_features(data, preprocessing_config)
                    ...
                    data, step_metadata = await self._filter_by_variance(data, preprocessing_config)
                
                elif step == PreprocessingStep.CORRELATION_FILTERING:
                    data, step_metadata = await self._filter_by_correlation(data, preprocessing_config)
                
                elif step == PreprocessingStep.STATISTICAL_SELECTION:
                    data, step_metadata = await self._select_features_statistical(data, preprocessing_config)
                
                elif step == PreprocessingStep.MODEL_BASED_SELECTION:
                    data, step_metadata = await self._select_features_model_based(data, preprocessing_config)
                
                elif step == PreprocessingStep.PCA_REDUCTION:
                    data, step_metadata = await self._apply_pca(data, preprocessing_config)
                
                elif step == PreprocessingStep.ICA_REDUCTION:
                    data, step_metadata = await self._apply_ica(data, preprocessing_config)
                
                elif step == PreprocessingStep.SVD_REDUCTION:
                    data, step_metadata = await self._apply_svd(data, preprocessing_config)
                ...
                transformation_record = {
                    ...
                    "shape_before": result.processed_shape if result.processed_shape != (0, 0) else result.original_shape,
                ...
                if "missing_values_handled" in step_metadata:
                    result.missing_values_handled += step_metadata["missing_values_handled"]
            ...
            if preprocessing_config.enable_validation:
                validation_results = await self._validate_processed_data(data, preprocessing_config)
            ...
            processed_dataset = Dataset(
                ...
                metadata={
                    **dataset.metadata,
                    ...
                    "transformations_applied": [t["step"] for t in result.transformations],
            ...
            logger.info(f"Preprocessing completed: {result.original_shape} -> {result.processed_shape}")
            return result
    ...
    async def _handle_missing_values(
        ...
        if config.imputation_method == ImputationMethod.MEAN:
            ...
            for col in processed_data.columns:
                processed_data[col] = processed_data[col].fillna(processed_data[col].mode().iloc[0] if not processed_data[col].mode().empty else 0)
        ...
        elif config.imputation_method == ImputationMethod.ITERATIVE and SKLEARN_AVAILABLE:
            numeric_cols = data.select_dtypes(include=[np.number]).columns
            if len(numeric_cols) > 0:
    ...
    async def _handle_outliers(
        ...
        for col in numeric_cols:
            ...
            if config.outlier_detection_method == "iqr":
                ...
                outlier_mask = pd.Series(z_scores > config.outlier_threshold, index=col_data.dropna().index)
                outlier_mask = outlier_mask.reindex(col_data.index, fill_value=False)
                
            elif config.outlier_detection_method == "isolation_forest" and SKLEARN_AVAILABLE:
                from sklearn.ensemble import IsolationForest
                iso_forest = IsolationForest(contamination=0.1, random_state=42)
                outlier_pred = iso_forest.fit_predict(col_data.dropna().values.reshape(-1, 1))
                outlier_mask = pd.Series(outlier_pred == -1, index=col_data.dropna().index)
                outlier_mask = outlier_mask.reindex(col_data.index, fill_value=False)
            ...
            if outliers_count > 0:
                if config.outlier_action == "remove":
                    ...
                    if config.outlier_detection_method == "iqr":
                        processed_data.loc[outlier_mask & (col_data < lower_bound), col] = lower_bound
                        processed_data.loc[outlier_mask & (col_data > upper_bound), col] = upper_bound
                    elif config.outlier_detection_method == "zscore":
    ...
    async def _remove_duplicates(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    async def _convert_data_types(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        for col in processed_data.select_dtypes(include=['float64']).columns:
            ...
            if np.finfo(np.float32).min <= col_min <= col_max <= np.finfo(np.float32).max:
    ...
    async def _encode_features(
        ...
        features_to_encode = [col for col in features_to_encode if col in categorical_cols]
        ...
        for col in features_to_encode:
            ...
            if num_unique > config.max_categories:
                logger.warning(f"Column {col} has {num_unique} categories, skipping encoding")
                continue
            ...
            elif config.encoding_method == EncodingMethod.TARGET and CATEGORY_ENCODERS_AVAILABLE:
                # Target encoding (requires target variable - skip for now)
                logger.warning(f"Target encoding requires target variable, skipping {col}")
                continue
    ...
    async def _transform_features(
        ...
        if config.power_transform_method:
            try:
                power_transformer = PowerTransformer(method=config.power_transform_method)
                processed_data[numeric_cols] = power_transformer.fit_transform(processed_data[numeric_cols])
    ...
    async def _create_features(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        if len(numeric_cols) >= 2:
            ...
            processed_data[interaction_name] = processed_data[col1] * processed_data[col2]
    ...
    async def _select_features_statistical(
        ...
        processed_data = pd.concat([
            ...
            pd.DataFrame(selected_data, columns=selected_features, index=data.index)
        ], axis=1)
    ...
    async def _validate_processed_data(
        ...
        if len(numeric_cols) > 0:
            ...
            if low_variance_cols:
                validation_results["warnings"].append(f"Found {len(low_variance_cols)} low variance features")
    ...
    async def _calculate_data_quality_score(self, data: pd.DataFrame) -> float:
        ...
        numeric_cols = data.select_dtypes(include=[np.number]).columns
        if len(numeric_cols) > 0:
            inf_ratio = np.isinf(data[numeric_cols]).sum().sum() / (len(data) * len(numeric_cols))
            score -= inf_ratio * 0.5
        
        # Penalize low variance features
        if len(numeric_cols) > 0:
            low_variance_count = sum(1 for col in numeric_cols if data[col].var() < 1e-10)
    ...
    async def get_preprocessing_recommendations(
        ...
        if data_analysis["missing_values"] > 0:
            missing_ratio = data_analysis["missing_values"] / (data.shape[0] * data.shape[1])
            recommendations["recommended_steps"].append(PreprocessingStep.MISSING_VALUE_IMPUTATION)
            
            if missing_ratio > 0.3:
                recommendations["step_configs"]["imputation_method"] = ImputationMethod.KNN
                recommendations["reasoning"].append("High missing value ratio - recommend KNN imputation")
            else:
                recommendations["step_configs"]["imputation_method"] = ImputationMethod.MEDIAN
                recommendations["reasoning"].append("Moderate missing values - recommend median imputation")
        
        # Duplicate removal
        if data_analysis["duplicate_rows"] > 0:
            recommendations["recommended_steps"].append(PreprocessingStep.DUPLICATE_REMOVAL)
            recommendations["reasoning"].append(f"Found {data_analysis['duplicate_rows']} duplicate rows")
        
        # Data type optimization
        if data_analysis["memory_usage_mb"] > 100:
            recommendations["recommended_steps"].append(PreprocessingStep.DATA_TYPE_CONVERSION)
            recommendations["reasoning"].append("Large memory usage - recommend data type optimization")
        
        # Feature scaling for numeric features
        if data_analysis["numeric_columns"] > 0:
            recommendations["recommended_steps"].append(PreprocessingStep.FEATURE_SCALING)
            recommendations["step_configs"]["scaling_method"] = ScalingMethod.ROBUST
            recommendations["reasoning"].append("Numeric features present - recommend robust scaling")
        
        # Feature encoding for categorical features
        if data_analysis["categorical_columns"] > 0:
            recommendations["recommended_steps"].append(PreprocessingStep.FEATURE_ENCODING)
            recommendations["step_configs"]["encoding_method"] = EncodingMethod.ONE_HOT
            recommendations["reasoning"].append("Categorical features present - recommend one-hot encoding")
        
        # Outlier detection for numeric features
        if data_analysis["numeric_columns"] > 0:
            recommendations["recommended_steps"].append(PreprocessingStep.OUTLIER_DETECTION)
            recommendations["step_configs"]["outlier_detection_method"] = "iqr"
            recommendations["reasoning"].append("Numeric features present - recommend outlier detection")
        
        # Feature selection for high-dimensional data
        total_features = data_analysis["numeric_columns"] + data_analysis["categorical_columns"]
        if total_features > 50:
            recommendations["recommended_steps"].append(PreprocessingStep.VARIANCE_FILTERING)
            recommendations["recommended_steps"].append(PreprocessingStep.CORRELATION_FILTERING)
            recommendations["reasoning"].append("High-dimensional data - recommend feature selection")
        ...
        if data_analysis["numeric_columns"] > 100:
            ...
            recommendations["reasoning"].append("Very high-dimensional numeric data - recommend PCA")
...
# Factory function
def create_advanced_preprocessor(config: Optional[PreprocessingConfig] = None) -> AdvancedPreprocessor:
