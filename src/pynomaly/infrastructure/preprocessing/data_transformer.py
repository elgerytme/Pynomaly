...
class DataTransformer:
    ...
    def scale_features(
        self,
        dataset: Dataset,
        strategy: Union[ScalingStrategy, str] = ScalingStrategy.STANDARD,
            ...
            try:
                if strategy == ScalingStrategy.POWER_BOX_COX:
                    ...
                    for col in numeric_columns:
                        if (df[col] <= 0).any():
                            raise ValueError(f"Box-Cox transformation requires positive values in {col}")
    ...
    def encode_categorical_features(
        self,
        dataset: Dataset,
        strategy: Union[EncodingStrategy, str] = EncodingStrategy.ONEHOT,
        columns: Optional[List[str]] = None,
        fit: bool = True,
            ...
            try:
                if strategy == EncodingStrategy.LABEL:
                    ...
                    if not dataset.has_target:
                        warnings.warn(f"Target encoding skipped for {col} - no target column available")
                        continue
                ...
                warnings.warn(f"Failed to encode column {col} with {strategy.value}: {e}")
                continue
    ...
    def create_polynomial_features(
        ...
        if columns is None:
            columns = dataset.get_numeric_features()[:5]  # Limit to 5 features to avoid explosion
    ...
    def select_features(
        self,
        dataset: Dataset,
        strategy: Union[FeatureSelectionStrategy, str] = FeatureSelectionStrategy.VARIANCE_THRESHOLD,
        k: int = 10,
        ...
        if strategy == FeatureSelectionStrategy.VARIANCE_THRESHOLD:
            ...
            selected_features.extend([col for col in columns 
        ...
        elif strategy in [FeatureSelectionStrategy.UNIVARIATE_F, 
            ...
            if strategy == FeatureSelectionStrategy.UNIVARIATE_F:
                score_func = f_classif if pd.api.types.is_integer_dtype(y) else f_classif
            else:  # UNIVARIATE_MI
                score_func = mutual_info_classif if pd.api.types.is_integer_dtype(y) else mutual_info_classif
        ...
        return Dataset(
            ...
            metadata={
                **dataset.metadata,
                ...
                "selected_features": len(selected_features) - (1 if dataset.target_column else 0),
                "selected_feature_names": [col for col in selected_features 
