...
class S3Adapter(CloudStorageAdapter):
    ...
    async def upload_file(
        ...
        try:
            ...
            if file_size > self.config.multipart_threshold:
                return await self._multipart_upload_file(
                    file_path, key, metadata, content_type, encrypt
                )
    ...
    async def delete_object(self, key: str) -> bool:
        ...
        try:
            await self._s3_client.delete_object(Bucket=self.config.bucket_name, Key=key)
            return True
    ...
    async def object_exists(self, key: str) -> bool:
        ...
        try:
            await self._s3_client.head_object(Bucket=self.config.bucket_name, Key=key)
            return True
    ...
    async def _multipart_upload_file(
        ...
        try:
            ...
            response = await self._s3_client.create_multipart_upload(
                Bucket=self.config.bucket_name, Key=key, **extra_args
            )
