...
class IntelligentAlertingEngine:
    ...
    async def evaluate_metric(
        ...
        for rule in self.alert_rules.values():
            ...
            # Check tag filters
            if rule.tag_filters:
                if not all(tags.get(key) == value for key, value in rule.tag_filters.items()):
                    continue
            ...
            # Evaluate condition
            if self._evaluate_condition(metric_value, rule.comparison_operator, rule.threshold_value):
    ...
    def _is_in_cooldown(self, rule_id: str) -> bool:
        ...
        cutoff = datetime.now() - timedelta(minutes=self.alert_rules[rule_id].cooldown_minutes)
        
        for alert in self.alert_history:
            if (alert.rule_id == rule_id and 
                hasattr(alert, 'timestamp') and 
    ...
    def _is_rate_limited(self) -> bool:
        ...
        recent_alerts = [
            timestamp for timestamp in self.alert_rate_limiter
            if timestamp > cutoff
        ]
    ...
    async def _process_alert(self, alert: Alert, rule: AlertRule):
        ...
        if self.enable_intelligent_suppression and self._should_suppress_alert(alert, rule):
            ...
            if condition == "similar_alert_exists":
                ...
                for active_alert in self.active_alerts.values():
                    if (active_alert.category == alert.category and
                        active_alert.source_service == alert.source_service and
    ...
    def _correlate_alert(self, alert: Alert) -> Optional[str]:
        ...
        for existing_alert in self.active_alerts.values():
            if (existing_alert.timestamp > correlation_window and
                existing_alert.category == alert.category and
                existing_alert.source_service == alert.source_service):
        ...
        for channel in rule.notification_channels:
            ...
            try:
                ...
                if success:
                    ...
                    self.logger.error(f"Failed to send notification via {channel.value} for alert {alert.id}")
                    
            except Exception as e:
                self.logger.error(f"Exception sending notification via {channel.value}: {e}")
    
    def _get_recipients_for_channel(self, channel: NotificationChannel, severity: AlertSeverity) -> List[str]:
    ...
    def _schedule_escalation(self, alert: Alert, rule: AlertRule):
        ...
        self.logger.info(f"Escalation scheduled for alert {alert.id} in {rule.escalation_delay_minutes} minutes")
    
    def _schedule_auto_resolve(self, alert: Alert, rule: AlertRule):
        """Schedule automatic alert resolution."""
        # In a real implementation, this would use a scheduler
        self.logger.info(f"Auto-resolve scheduled for alert {alert.id} in {rule.auto_resolve_minutes} minutes")
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rate_limit_status": {
                "current_rate": len([t for t in self.alert_rate_limiter if t > last_hour]),
