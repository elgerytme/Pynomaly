...
class CacheMetrics:
    ...
    def overall_hit_rate(self) -> float:
        ...
        total_requests = total_hits + max(self.l1_misses, self.l2_misses, self.l3_misses)
...
class MemoryCache(CacheBackend):
    ...
    async def _ensure_capacity(self, required_bytes: int) -> None:
        ...
        # Evict by size
        while (self._current_size_bytes + required_bytes > self.max_size_bytes and 
               self._access_order):
...
class AdvancedCacheService:
        ...
        try:
            ...
            for level in cache_levels:
                ...
                if not level_success:
                    success = False
                    self.logger.warning(f"Failed to set key {key} in cache level {level}")
    ...
    async def get_metrics(self) -> CacheMetrics:
        ...
        async with self._metrics_lock:
            ...
            if self._operation_times:
                self.metrics.avg_get_latency_ms = sum(self._operation_times) / len(self._operation_times)
    ...
    async def _serialize_data(
        ...
        if (len(serialized) >= self.config.compression_min_size_bytes and
    ...
    async def get_batch(self, keys: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.l2_cache and missing_keys:
                ...
                l2_results = await self._batch_get_l2([normalized_keys[key] for key in missing_keys])
    ...
    async def set_batch(self, items: Dict[str, Any], ttl_seconds: Optional[int] = None) -> Dict[str, bool]:
        ...
        try:
            ...
            for key, value in items.items():
                ...
                serialized_value = await self._serialize_data(value, serialization_format)
                serialized_items[normalized_key] = serialized_value
            
            # Batch set to L1 cache
            if self.l1_cache:
                for key, value in items.items():
                    success = await self.l1_cache.set(normalized_items[key], value, ttl_seconds=ttl)
                    results[key] = success
