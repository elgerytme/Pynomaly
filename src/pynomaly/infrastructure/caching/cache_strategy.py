...
class RedisCache(CacheBackend):
    ...
    def __init__(self, redis_url: str, default_ttl: int = 3600, enable_compression: bool = True):
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # Store in Redis
            result = self._redis.setex(key, ttl, data) if ttl > 0 else self._redis.set(key, data)
...
class CacheKeyGenerator:
    ...
    @staticmethod
    def search_key(entity_type: str, filters: Dict[str, Any], page: int, page_size: int) -> str:
        """Generate key for search result caching."""
        return CacheKeyGenerator.generate_key("search", entity_type, filters=filters, page=page, page_size=page_size)
...
class CacheManager:
    ...
    async def set_detector(self, detector_id: UUID, detector: Any, ttl: Optional[int] = None) -> bool:
        """Cache detector."""
        key = self.key_generator.detector_key(detector_id)
    ...
    async def set_dataset(self, dataset_id: UUID, dataset: Any, ttl: Optional[int] = None) -> bool:
        """Cache dataset."""
        key = self.key_generator.dataset_key(dataset_id)
    ...
    async def set_detection_result(self, result_id: UUID, result: Any, ttl: Optional[int] = None) -> bool:
