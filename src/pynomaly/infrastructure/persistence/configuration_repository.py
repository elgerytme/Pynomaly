...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationSearchRequestDTO, 
    ConfigurationCollectionDTO, ConfigurationTemplateDTO,
...
class ConfigurationRepository:
    ...
    async def save_configuration(
        ...
        try:
            ...
            logger.info(f"Saved configuration {configuration.name} ({configuration.id})")
            return True
    ...
    async def save_collection(
        ...
        """
        try:
            collection_file = self.storage_path / "collections" / f"{collection.id}.json"
    ...
    async def load_collection(
        self,
        collection_id: UUID
    ) -> Optional[ConfigurationCollectionDTO]:
        ...
        """
        try:
            collection_file = self.storage_path / "collections" / f"{collection_id}.json"
    ...
    async def load_template(
        self,
        template_id: UUID
    ) -> Optional[ConfigurationTemplateDTO]:
    ...
    async def export_configurations(
        ...
        try:
            ...
            logger.info(f"Exported {len(export_data['configurations'])} configurations to {export_path}")
            return True
            ...
            for config_data in configurations:
                try:
                    ...
                    if existing and not overwrite_existing:
                        logger.warning(f"Configuration {config.id} already exists, skipping")
                        continue
    ...
    def get_repository_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config_count = len(list((self.storage_path / "configurations").glob("*.json")))
            collection_count = len(list((self.storage_path / "collections").glob("*.json")))
    ...
    async def _create_backup(self, config_id: UUID) -> None:
        ...
        try:
            ...
            if config_file.exists():
                ...
                backup_file = self.storage_path / "backups" / f"{config_id}_{timestamp}.json"
    ...
    async def _update_repository_metadata(self, configuration: ExperimentConfigurationDTO) -> None:
        """Update repository metadata after configuration change."""
        self.metadata["last_modified"] = datetime.now().isoformat()
        self.metadata["total_configurations"] = len(list((self.storage_path / "configurations").glob("*.json")))
        await self._save_repository_metadata()
