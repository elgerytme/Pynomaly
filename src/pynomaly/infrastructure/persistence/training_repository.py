...
from pynomaly.domain.entities.training_job import TrainingJob, TrainingStatus, TrainingPriority
    ...
    async def update_trial(self, trial: OptimizationTrial) -> None:
        ...
        async with self._lock:
            if trial.trial_id in self._trials:
                ...
                raise RepositoryError(f"Optimization trial {trial.trial_id} not found for update")
    
    async def get_trials_by_job(self, job_id: str) -> List[OptimizationTrial]:
        """Get all trials for a training job."""
        trial_ids = self._job_trials.get(job_id, [])
        trials = [self._trials[trial_id] for trial_id in trial_ids if trial_id in self._trials]
...
class SQLTrainingRepository(TrainingRepositoryProtocol):
    ...
    def __init__(self, connection_string: str):
        ...
        logger.warning("SQLTrainingRepository is not implemented, using in-memory fallback")
    ...
    async def list_jobs(
        ...
        """List training jobs with optional filtering."""
        return await self._in_memory_repo.list_jobs(dataset_id, status, priority, limit, offset)
...
class TrainingRepository:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        if jobs:
            ...
            completed_jobs = [j for j in jobs if j.status == TrainingStatus.COMPLETED and j.duration]
            if completed_jobs:
