...
class SQLiteTrainingJobRepository(TrainingJobRepository):
        ...
        try:
            async with self.async_session() as session:
                ...
                if existing:
                    ...
                    existing.best_model = json.dumps(job.best_model) if job.best_model else None
                    existing.best_score = job.best_score
                    existing.best_parameters = json.dumps(job.best_parameters) if job.best_parameters else None
                    ...
                    model = TrainingJobModel(
                        ...
                        best_model=json.dumps(job.best_model) if job.best_model else None,
                        best_score=job.best_score,
                        best_parameters=json.dumps(job.best_parameters) if job.best_parameters else None,
    ...
    def _model_to_job(self, model: TrainingJobModel) -> TrainingJob:
        ...
        from pynomaly.application.services.training_automation_service import TrainingConfiguration
        ...
        best_parameters = json.loads(model.best_parameters) if model.best_parameters else None
        ...
        try:
            async with self.async_session() as session:
                ...
                for status in TrainingStatus:
                    count_query = session.query(TrainingJobModel).filter(
                        TrainingJobModel.status == status.value
                    ).count()
                ...
                avg_time_query = session.query(
                    ...
                    TrainingJobModel.status == TrainingStatus.COMPLETED.value
                )
                avg_time = await session.execute(avg_time_query)
                stats["average_execution_time"] = avg_time.scalar() or 0.0
                
                # Success rate
                total_completed = stats.get("completed_count", 0) + stats.get("failed_count", 0)
                if total_completed > 0:
                    stats["success_rate"] = stats.get("completed_count", 0) / total_completed
                else:
