...
class RepositoryFactory:
        ...
        if self._database_manager is None and self._should_use_database():
            try:
                self._database_manager = DatabaseManager(
                    database_url=self.config.get_connection_url(),
                ...
                if self._should_initialize_database():
                    ...
                    if not migrator.check_tables_exist():
                        ...
                        if not migrator.create_all_tables():
                            logger.warning("Failed to create database tables, falling back to in-memory")
    ...
    def _should_initialize_database(self) -> bool:
        ...
        if self.config.profile in [DatabaseProfile.DEVELOPMENT, DatabaseProfile.TESTING]:
            return True
    ...
    def get_repository_info(self) -> dict:
        ...
        """
        using_database = self._should_use_database() and self.database_manager is not None
        ...
        info = {
            ...
            "connection_url": self.config.get_connection_url() if using_database else None,
...
def get_repository_manager(config: Optional[DatabaseConfig] = None) -> RepositoryManager:
...
def create_detector_repository(config: Optional[DatabaseConfig] = None) -> DetectorRepositoryProtocol:
...
def create_dataset_repository(config: Optional[DatabaseConfig] = None) -> DatasetRepositoryProtocol:
...
def create_result_repository(config: Optional[DatabaseConfig] = None) -> DetectionResultRepositoryProtocol:
...
def get_repository_info(config: Optional[DatabaseConfig] = None) -> dict:
...
def create_testing_repositories() -> tuple[DetectorRepositoryProtocol, 
                                          DatasetRepositoryProtocol, 
                                          DetectionResultRepositoryProtocol]:
...
def create_development_repositories() -> tuple[DetectorRepositoryProtocol, 
                                             DatasetRepositoryProtocol, 
                                             DetectionResultRepositoryProtocol]:
