...
class OptunaOptimizer(HyperparameterOptimizer):
            ...
            try:
                ...
                if hasattr(objective_function, "intermediate_values"):
                    for step, value in enumerate(
                        objective_function.intermediate_values
                    ):
    ...
    def _sample_parameter(
        self, trial, param_name: str, param_range: HyperparameterRange
    ) -> Any:
    ...
    def _calculate_convergence_data(
        ...
        return {
            ...
            "improvement_rate": (
                (scores[-1] - scores[0]) / len(scores) if len(scores) > 1 else 0.0
            ),
    ...
    def _detect_plateau(
        ...
        relative_improvement = (
            improvement / abs(recent_scores[0]) if recent_scores[0] != 0 else 0
        )
...
class GridSearchOptimizer(HyperparameterOptimizer):
    ...
    async def optimize(
        ...
        **kwargs,
        ...
        logger.info(
            f"Starting grid search with {len(param_grid)} parameter combinations"
        )
            ...
            if param_range.type == "categorical":
                ...
                if param_range.type == "float":
                    ...
                    values = np.linspace(
                        ...
                        param_range.high,
                        num=min(
                            param_range.grid_size or 10,
                            param_range.high - param_range.low + 1,
                        ),
                        dtype=int,
                    )
        ...
        for param_name in param_names:
            ...
            for trial in trials:
                if (
                    ...
                    and param_name in trial.parameters.parameters
                ):
...
class RandomSearchOptimizer(HyperparameterOptimizer):
            ...
            if param_range.type == "categorical":
                ...
                if param_range.log:
                    ...
                    params[param_name] = np.random.randint(
                        param_range.low, param_range.high + 1
                    )
    ...
    def _calculate_exploration_efficiency(
        ...
        successful_trials = sum(
            1 for trial in trials if trial.status == TrialStatus.COMPLETED
        )
