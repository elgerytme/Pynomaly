...
PREDICTION_COUNTER = Counter(
    "model_predictions_total", "Total predictions made", ["model_id", "version"]
)
PREDICTION_LATENCY = Histogram(
    "model_prediction_duration_seconds", "Prediction latency", ["model_id", "version"]
)
ERROR_COUNTER = Counter(
    "model_errors_total", "Total errors", ["model_id", "version", "error_type"]
)
ACTIVE_CONNECTIONS = Gauge(
    "model_websocket_connections", "Active WebSocket connections"
)
MODEL_LOAD_TIME = Histogram(
    "model_load_duration_seconds", "Model loading time", ["model_id", "version"]
)
...
class PredictionRequest(BaseModel):
    ...
    version: str | None = Field(None, description="Specific model version to use")
...
class BatchPredictionRequest(BaseModel):
    ...
    data: list[dict[str, Any]] = Field(
        ..., description="Array of input data for prediction"
    )
    model_id: str | None = Field(None, description="Specific model ID to use")
    version: str | None = Field(None, description="Specific model version to use")
...
class ModelServer:
    ...
    def _add_prediction_routes(self, app: FastAPI) -> None:
        ...
        async def predict(request: PredictionRequest) -> PredictionResponse:
            ...
            try:
                ...
                PREDICTION_COUNTER.labels(
                    model_id=model_info["model_id"], version=model_info["version"]
                ).inc()
        ...
        async def predict_batch(
            ...
            try:
                ...
                throughput = (
                    len(request.data) / (total_time_ms / 1000)
                    if total_time_ms > 0
                    else 0
                )
    ...
    def _add_model_management_routes(self, app: FastAPI) -> None:
        ...
        async def list_models() -> list[ModelInfo]:
            ...
            try:
                deployments = await self.deployment_service.list_deployments(
                    environment=self.environment, limit=100
                )
                ...
                for deployment in deployments:
                    if deployment.is_deployed:
                        model_info = ModelInfo(
                            ...
                            performance_metrics=deployment.get_deployment_info()[
                                "health_metrics"
                            ],
                        )
        ...
        async def get_model_info(model_id: str) -> ModelInfo:
            ...
            try:
                ...
                deployments = await self.deployment_service.list_deployments(
                    environment=self.environment, model_version_id=model_uuid
                )
        ...
        async def load_model(model_id: str) -> dict[str, str]:
            ...
            try:
                ...
                MODEL_LOAD_TIME.labels(model_id=model_id, version="1.0.0").observe(
                    load_time
                )
    ...
    def _add_health_routes(self, app: FastAPI) -> None:
        ...
        async def prometheus_metrics():
            ...
            return JSONResponse(content=output.decode("utf-8"), media_type=content_type)
    ...
    async def _load_model(self, model_id: str, version: str | None = None) -> None:
    ...
    async def _evict_least_recently_used_model(self) -> None:
        ...
        lru_model_id = min(
            self.model_access_times.keys(), key=lambda k: self.model_access_times[k]
        )
    ...
    async def _make_prediction(
        ...
        try:
            ...
            result = {"score": float(anomaly_score), "is_anomaly": bool(is_anomaly)}
