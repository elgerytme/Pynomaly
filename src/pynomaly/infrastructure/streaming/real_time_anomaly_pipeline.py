...
class StreamingDetectionResult(BaseModel):
    """Simplified detection result for streaming use case."""
...
class StreamingAlert(BaseModel):
    ...
    detection_result: StreamingDetectionResult | None = None
    metadata: dict[str, Any] = Field(default_factory=dict)
    ...
    async def process_batch(
        ...
        if self.is_trained:
            ...
            for i, (point, pred, score) in enumerate(
                ...
                result = StreamingDetectionResult(
                    detector_name=self.detector_algorithm,
...
class RealTimeAnomalyPipeline:
    ...
    async def _handle_detection_results(
        self, results: list[StreamingDetectionResult], batch: list[DataPoint]
    ) -> None:
    ...
    async def _check_alert_thresholds(self) -> None:
        ...
        if self.metrics.processed_records > 0:
            anomaly_rate = (
                self.metrics.anomalies_detected / self.metrics.processed_records
            )
