...
class PipelineService:
    ...
    def __init__(self, workspace_path: str = "/tmp/pynomaly-ci"):
        ...
        self.logger.info(
            f"Pipeline service initialized with workspace: {workspace_path}"
        )
    ...
    async def create_pipeline_from_template(
        ...
        self.logger.info(
            f"Created pipeline: {pipeline_name} from template {template.name}"
        )
        return pipeline
    ...
    async def trigger_pipeline(
        ...
        self.logger.info(
            f"Queued pipeline {pipeline.name} for execution (priority: {priority})"
        )
        return True
    ...
    async def execute_pipeline(self, pipeline_id: UUID) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Pipeline {pipeline.name} completed with status: {final_status.value}"
            )
            return success
            ...
            try:
                ...
                if pipeline_id:
                    ...
                    self.execution_tasks = {
                        task for task in self.execution_tasks if not task.done()
                    }
    ...
    async def _clone_repository(self, pipeline: Pipeline, workspace: Path) -> None:
        ...
        try:
            ...
            if pipeline.commit_sha:
                ...
                cmd = ["git", "clone", pipeline.repository_url, str(workspace / "repo")]
            ...
            if pipeline.commit_sha:
                ...
                if process.returncode != 0:
                    raise Exception(
                        f"Git checkout failed for commit {pipeline.commit_sha}"
                    )
    ...
    async def _execute_stages(self, pipeline: Pipeline, workspace: Path) -> bool:
        ...
        while len(completed_stages) < len(pipeline.stages):
            ...
            if not ready_stages:
                ...
                remaining_stages = [
                    s.name for s in pipeline.stages if s.name not in completed_stages
                ]
                self.logger.error(
    ...
    async def _calculate_pipeline_metrics(self, pipeline_id: UUID) -> None:
        ...
        related_pipelines = [
            p for p in self.pipelines.values() if p.name == target_pipeline.name
        ]
            ...
            if workspace_dir.is_dir():
                try:
                    ...
                    if pipeline_id in self.pipelines:
                        pipeline = self.pipelines[pipeline_id]
                        if (
                            pipeline.created_at < cutoff_time
                            and pipeline_id not in self.running_pipelines
                        ):
                            # Remove workspace
                            import shutil
    ...
    async def _cleanup_old_pipelines(self) -> None:
        ...
        if pipelines_to_remove:
            self.logger.info(
                f"Cleaned up {len(pipelines_to_remove)} old pipeline records"
            )
