...
class TestRunner:
    ...
    async def execute_test_suite(
        ...
        try:
            ...
            success = await self._execute_tests(test_suite, workspace_path, environment, config)
            
            # Complete test suite
            test_suite.end_time = await self._get_current_time()
            if test_suite.start_time:
                test_suite.duration_seconds = (test_suite.end_time - test_suite.start_time).total_seconds()
            
            test_suite.calculate_metrics()
            
            self.logger.info(f"Test suite {test_suite.name} completed: {test_suite.status.value}")
            return success
    ...
    async def discover_tests(
        ...
        for pattern in patterns:
            try:
                ...
                discovered_tests.extend([str(f.relative_to(workspace_path)) for f in test_files])
            except Exception as e:
                self.logger.warning(f"Error discovering tests with pattern {pattern}: {e}")
        
        # Remove duplicates and sort
        discovered_tests = sorted(list(set(discovered_tests)))
        
        self.logger.debug(f"Discovered {len(discovered_tests)} test files for {test_type.value}")
    ...
    async def validate_test_environment(
        ...
        try:
            ...
            if not workspace_path.exists():
                ...
                validation_result["errors"].append(f"Workspace path does not exist: {workspace_path}")
                return validation_result
            
            # Check test-specific requirements
            if test_type == TestType.UNIT:
                await self._validate_unit_test_environment(workspace_path, validation_result)
            elif test_type == TestType.INTEGRATION:
                await self._validate_integration_test_environment(workspace_path, validation_result)
            elif test_type == TestType.PERFORMANCE:
                await self._validate_performance_test_environment(workspace_path, validation_result)
            elif test_type == TestType.SECURITY:
                await self._validate_security_test_environment(workspace_path, validation_result)
            elif test_type == TestType.END_TO_END:
                await self._validate_e2e_test_environment(workspace_path, validation_result)
    ...
    async def _execute_tests(
        ...
        if test_suite.test_type == TestType.UNIT:
            return await self._execute_unit_tests(test_suite, workspace_path, command, env)
        elif test_suite.test_type == TestType.INTEGRATION:
            return await self._execute_integration_tests(test_suite, workspace_path, command, env)
        elif test_suite.test_type == TestType.PERFORMANCE:
            return await self._execute_performance_tests(test_suite, workspace_path, command, env)
        elif test_suite.test_type == TestType.SECURITY:
            return await self._execute_security_tests(test_suite, workspace_path, command, env)
        else:
            # Generic test execution
            return await self._execute_generic_tests(test_suite, workspace_path, command, env)
    ...
    async def _execute_integration_tests(
        ...
        try:
            ...
            await self._add_test_error(test_suite, "Integration test execution failed", str(e))
            return False
    ...
    async def _execute_performance_tests(
        ...
        try:
            ...
            await self._add_test_error(test_suite, "Performance test execution failed", str(e))
            return False
        ...
        try:
            ...
            for cmd in security_commands:
                ...
                test_result = TestResult(
                    ...
                    status=PipelineStatus.SUCCESS if process.returncode == 0 else PipelineStatus.FAILED,
            ...
            await self._add_test_error(test_suite, "Security test execution failed", str(e))
            return False
        ...
        try:
            ...
            for line in lines:
                ...
                if "::" in line and (" PASSED" in line or " FAILED" in line):
                    ...
                    status = PipelineStatus.SUCCESS if "PASSED" in line else PipelineStatus.FAILED
            ...
            if not test_suite.tests:
                test_result = TestResult(
                    ...
                    test_type=test_suite.test_type,
                    status=PipelineStatus.SUCCESS if return_code == 0 else PipelineStatus.FAILED,
                    output=stdout,
    ...
    async def _parse_coverage_results(self, test_suite: TestSuite, workspace_path: Path) -> None:
    ...
    async def _parse_benchmark_results(self, test_suite: TestSuite, workspace_path: Path) -> None:
    ...
    async def _validate_unit_test_environment(
        ...
        if not test_dir.exists():
            validation_result["warnings"].append("Unit test directory not found: tests/unit/")
        else:
        ...
        config_found = any((workspace_path / config).exists() for config in pytest_configs)
    ...
    async def _validate_integration_test_environment(
        ...
        if not test_dir.exists():
            validation_result["warnings"].append("Integration test directory not found: tests/integration/")
        else:
            validation_result["requirements_met"].append("Integration test directory exists")
        
        # Check for docker-compose or test services
        if (workspace_path / "docker-compose.test.yml").exists():
            validation_result["requirements_met"].append("Docker test environment configuration found")
        elif (workspace_path / "tests" / "conftest.py").exists():
            validation_result["requirements_met"].append("Pytest configuration for fixtures found")
    ...
    async def _validate_performance_test_environment(
        ...
        if not test_dir.exists():
            validation_result["warnings"].append("Performance test directory not found: tests/performance/")
        else:
            validation_result["requirements_met"].append("Performance test directory exists")
        ...
        if requirements_file.exists():
            ...
            if "pytest-benchmark" in content:
                validation_result["requirements_met"].append("pytest-benchmark dependency found")
    ...
    async def _validate_security_test_environment(
        ...
        if requirements_file.exists():
            ...
            if found_tools:
                validation_result["requirements_met"].append(f"Security tools found: {', '.join(found_tools)}")
            else:
                validation_result["warnings"].append("No security scanning tools found in requirements")
    ...
    async def _validate_e2e_test_environment(
        ...
        if e2e_found:
            validation_result["requirements_met"].append("End-to-end test directory exists")
        else:
    ...
    async def _check_python_dependencies(
        ...
        if (workspace_path / "pyproject.toml").exists():
            ...
            validation_result["errors"].append("No dependency file found (requirements.txt or pyproject.toml)")
            validation_result["valid"] = False
    ...
    async def _setup_integration_environment(
        ...
        if docker_compose_test.exists():
            ...
            process = await asyncio.create_subprocess_exec(
                "docker-compose", "-f", str(docker_compose_test), "up", "-d",
                cwd=workspace_path,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
    ...
    async def _cleanup_integration_environment(self, workspace_path: Path) -> None:
        ...
        if docker_compose_test.exists():
            # Stop test services
            process = await asyncio.create_subprocess_exec(
                "docker-compose", "-f", str(docker_compose_test), "down",
                cwd=workspace_path,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
