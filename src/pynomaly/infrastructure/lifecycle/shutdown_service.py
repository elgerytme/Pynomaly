...
class ShutdownService:
    ...
    def register_handler(
        ...
        shutdown_handler = ShutdownHandler(
            name=name, phase=phase, handler=handler, timeout=timeout, is_async=is_async
        )

        self._handlers[phase].append(shutdown_handler)
        logger.info(
            "Registered shutdown handler", name=name, phase=phase.value, timeout=timeout
        )
    ...
    def initiate_shutdown(self) -> None:
        ...
        shutdown_thread = threading.Thread(
            target=self._execute_shutdown, name="shutdown-thread"
        )
    ...
    def _execute_handler(self, handler: ShutdownHandler) -> None:
        ...
        try:
            ...
            logger.info(
                f"Shutdown handler completed: {handler.name}", duration=handler_duration
            )

        except TimeoutError:
            logger.warning(
                f"Shutdown handler timed out: {handler.name}", timeout=handler.timeout
            )
        except Exception as e:
            logger.error(
    ...
    def get_shutdown_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "registered_handlers": {
                phase.value: len(handlers) for phase, handlers in self._handlers.items()
            },
            "uptime_seconds": time.time() - self._start_time,
...
_shutdown_service: ShutdownService | None = None
