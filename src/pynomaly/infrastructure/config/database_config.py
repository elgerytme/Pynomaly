...
class DatabaseConfig(BaseModel):
    ...
    @classmethod
    def ensure_file_path_exists(cls, v: Optional[Path]) -> Optional[Path]:
    ...
    def get_connection_url(self) -> str:
        ...
        if self.db_type == DatabaseType.MEMORY:
            ...
            if password:
                return f"mariadb+pymysql://{username}:{password}@{host}:{port}/{database}"
            else:
    ...
    def get_engine_kwargs(self) -> Dict[str, Any]:
        ...
        elif self.db_type in [DatabaseType.POSTGRESQL, DatabaseType.MYSQL, DatabaseType.MARIADB]:
            kwargs.update({
...
class DatabaseConfigManager:
    ...
    def detect_environment(self) -> DatabaseProfile:
        ...
        if env_profile in [p.value for p in DatabaseProfile]:
            logger.info(f"Using database profile from PYNOMALY_DB_PROFILE: {env_profile}")
    ...
    def _apply_env_overrides(self, config: DatabaseConfig) -> None:
        ...
        db_url = (os.environ.get("DATABASE_URL") or 
                 os.environ.get("PYNOMALY_DATABASE_URL"))
        if db_url:
            config.url = db_url
    ...
    def create_profile(self, 
        ...
        custom_profile = DatabaseProfile(name) if name in [p.value for p in DatabaseProfile] else name
        ...
        try:
            ...
            if config.db_type == DatabaseType.SQLITE and config.file_path:
                if not config.file_path.parent.exists():
                    logger.error(f"SQLite database directory does not exist: {config.file_path.parent}")
                    return False
...
def get_database_config(profile: Optional[DatabaseProfile] = None, 
                       **overrides) -> DatabaseConfig:
    """Get database configuration for the specified profile.
