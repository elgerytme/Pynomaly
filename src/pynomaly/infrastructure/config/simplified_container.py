...
class SimplifiedContainer(containers.DeclarativeContainer):
    ...
    def _setup_factories(self):
        ...
        self._service_group_factory = ServiceGroupFactory(self._service_registry, config_instance)
    ...
    def _setup_repositories(self):
        ...
        self.detector_repository = providers.Singleton(
            lambda: self._repository_factory.create_repository('detector', repository_classes)
        )
        self.dataset_repository = providers.Singleton(
            lambda: self._repository_factory.create_repository('dataset', repository_classes)
        )
        self.result_repository = providers.Singleton(
            lambda: self._repository_factory.create_repository('result', repository_classes)
        )
        
        # Async repository wrappers
        self.async_detector_repository = providers.Singleton(
            AsyncDetectorRepositoryWrapper,
            sync_repository=self.detector_repository
        )
        self.async_dataset_repository = providers.Singleton(
            AsyncDatasetRepositoryWrapper,
            sync_repository=self.dataset_repository
        )
    ...
    def _setup_optional_services(self):
        ...
        # Data loaders
        data_loader_providers = self._service_group_factory.create_data_loader_providers()
        ...
        if self._service_registry.is_available('explainability_service'):
            self.domain_explainability_service = self._service_registry.create_singleton_provider(
                'explainability_service'
            )
            
            # Create explainer providers
            if self._service_registry.is_available('shap_explainer'):
                self.shap_explainer = self._service_registry.create_singleton_provider('shap_explainer')
            
            if self._service_registry.is_available('lime_explainer'):
                self.lime_explainer = self._service_registry.create_singleton_provider('lime_explainer')
    ...
    def get_available_data_loaders(self) -> list:
        ...
        loaders = []
        for loader_name in ['csv_loader', 'parquet_loader', 'polars_loader', 
                          'arrow_loader', 'spark_loader']:
