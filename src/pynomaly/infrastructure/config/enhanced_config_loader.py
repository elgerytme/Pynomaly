...
class ConfigurationContext(BaseValueObject):
    ...
    profile: str | None = Field(None, description="Configuration profile")
    namespace: str | None = Field(None, description="Configuration namespace")
    version: str | None = Field(None, description="Configuration version")
    tenant: str | None = Field(None, description="Tenant identifier")
...
class ConfigurationLoader(ABC, Generic[T]):
    ...
    def load_from_source(
        self, source: ConfigurationSource, context: ConfigurationContext
    ) -> dict[str, Any]:
...
class FileConfigurationLoader(ConfigurationLoader[T]):
    ...
    def __init__(self, config_class: type[T], config_dir: Path | None = None) -> None:
    ...
    def _resolve_file_path(
        ...
        if context.environment and context.environment != "default":
            env_path = base_path.with_suffix(
                f".{context.environment}{base_path.suffix}"
            )
            if env_path.exists():
                return env_path
...
class RemoteConfigurationLoader(ConfigurationLoader[T]):
    ...
    def __init__(
        self, config_class: type[T], base_url: str, auth_token: str | None = None
    ) -> None:
...
class ConfigurationManager:
    ...
    def get_configuration(self, context: ConfigurationContext | None = None) -> T:
        """Get configuration.
    ...
    def reload_configuration(self, context: ConfigurationContext | None = None) -> T:
...
def create_configuration_manager(
    config_class: type[T], config_dir: Path | None = None
