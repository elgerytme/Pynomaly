...
class ServiceRegistry:
    ...
    def create_singleton_provider(self, name: str, **override_kwargs) -> Optional[providers.Singleton]:
    ...
    def create_factory_provider(self, name: str, **override_kwargs) -> Optional[providers.Factory]:
...
class RepositoryFactory:
    ...
    def create_repository(self, repo_type: str, repository_classes: Dict[str, Type]) -> Any:
        ...
        if (self.config.use_database_repositories and 
            self.config.database_configured and 
            f'database_{repo_type}' in repository_classes):
...
class ServiceGroupFactory:
    ...
    def create_auth_providers(self) -> Dict[str, Any]:
        ...
        # Permission Checker
        permission_provider = self.registry.create_singleton_provider('permission_checker')
        ...
        rate_limiter_provider = self.registry.create_singleton_provider(
            'rate_limiter',
            default_requests_per_minute=self.config.api_rate_limit
        )
    ...
    def create_data_loader_providers(self) -> Dict[str, Any]:
        ...
        providers_dict['parquet_loader'] = providers.Factory(
            self.registry.get_service_class('parquet_loader'),
            use_pyarrow=True
        )
        ...
        for loader_name, config in [
            ...
            loader_provider = self.registry.create_factory_provider(loader_name, **config)
            if loader_provider:
    ...
    def create_adapter_providers(self) -> Dict[str, Any]:
        ...
        # Optional adapters
        for adapter_name in ['pygod_adapter', 'pytorch_adapter', 'tensorflow_adapter', 'jax_adapter']:
...
def register_all_services(registry: ServiceRegistry) -> ServiceRegistry:
    ...
    # Core data loaders
    registry.register_service('csv_loader', 'pynomaly.infrastructure.data_loaders.CSVLoader')
    registry.register_service('parquet_loader', 'pynomaly.infrastructure.data_loaders.ParquetLoader')
    
    # Optional data loaders
    registry.register_service('polars_loader', 'pynomaly.infrastructure.data_loaders.PolarsLoader')
    registry.register_service('arrow_loader', 'pynomaly.infrastructure.data_loaders.ArrowLoader')
    registry.register_service('spark_loader', 'pynomaly.infrastructure.data_loaders.SparkLoader')
    
    # Algorithm adapters
    registry.register_service('pyod_adapter', 'pynomaly.infrastructure.adapters.PyODAdapter')
    registry.register_service('sklearn_adapter', 'pynomaly.infrastructure.adapters.SklearnAdapter')
    registry.register_service('pygod_adapter', 'pynomaly.infrastructure.adapters.PyGODAdapter')
    registry.register_service('pytorch_adapter', 'pynomaly.infrastructure.adapters.PyTorchAdapter')
    registry.register_service('tensorflow_adapter', 'pynomaly.infrastructure.adapters.TensorFlowAdapter')
    registry.register_service('jax_adapter', 'pynomaly.infrastructure.adapters.JAXAdapter')
    
    # Authentication services
    registry.register_service('jwt_auth_service', 'pynomaly.infrastructure.auth.JWTAuthService')
    registry.register_service('permission_checker', 'pynomaly.infrastructure.auth.PermissionChecker')
    registry.register_service('rate_limiter', 'pynomaly.infrastructure.auth.RateLimiter')
    
    # Cache services
    registry.register_service('redis_cache', 'pynomaly.infrastructure.cache.RedisCache')
    registry.register_service('detector_cache_decorator', 'pynomaly.infrastructure.cache.DetectorCacheDecorator')
    
    # Monitoring services
    registry.register_service('telemetry_service', 'pynomaly.infrastructure.monitoring.TelemetryService')
    registry.register_service('health_service', 'pynomaly.infrastructure.monitoring.HealthService')
    
    # Database services
    registry.register_service('database_manager', 'pynomaly.infrastructure.persistence.DatabaseManager')
    
    # AutoML services
    registry.register_service('automl_service', 'pynomaly.application.services.AutoMLService')
    
    # Explainability services
    registry.register_service('explainability_service', 'pynomaly.domain.services.ExplainabilityService')
    registry.register_service('shap_explainer', 'pynomaly.infrastructure.explainers.SHAPExplainer')
    registry.register_service('lime_explainer', 'pynomaly.infrastructure.explainers.LIMEExplainer')
