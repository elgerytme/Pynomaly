...
class TDDSettings(BaseModel):
    ...
    auto_validation: bool = Field(
        default=True, description="Automatically validate TDD compliance"
    )

    # Coverage settings
    min_test_coverage: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Minimum required test coverage"
    )
    coverage_fail_under: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Coverage threshold for failing builds"
    )
    branch_coverage_required: bool = Field(
        default=True, description="Require branch coverage analysis"
    )
    ...
    test_naming_convention: str = Field(
        default="test_{function_name}", description="Test function naming convention"
    )
    test_class_naming_convention: str = Field(
        default="Test{ClassName}", description="Test class naming convention"
    )
    require_test_docstrings: bool = Field(
        default=True, description="Require docstrings in test functions"
    )

    # Git integration
    git_hooks_enabled: bool = Field(
        default=True, description="Enable git hook integration"
    )
    pre_commit_validation: bool = Field(
        default=True, description="Validate TDD compliance on pre-commit"
    )
    pre_push_validation: bool = Field(
        default=False, description="Validate TDD compliance on pre-push"
    )

    # CI/CD integration
    ci_validation_enabled: bool = Field(
        default=True, description="Enable CI/CD TDD validation"
    )
    fail_on_violations: bool = Field(
        default=False, description="Fail builds on TDD violations"
    )

    # Reporting and metrics
    generate_compliance_reports: bool = Field(
        default=True, description="Generate TDD compliance reports"
    )
    report_output_path: str = Field(
        default="./reports/tdd", description="Path for TDD reports"
    )
    metrics_retention_days: int = Field(
        default=90, description="Days to retain TDD metrics"
    )

    # Development workflow
    allow_implementation_first: bool = Field(
        default=False, description="Allow implementation before tests in development"
    )
    grace_period_hours: int = Field(
        default=24, description="Grace period for TDD compliance in development"
    )
    require_test_plan: bool = Field(
        default=True, description="Require test plan before implementation"
    )

    # Advanced settings
    mutation_testing_enabled: bool = Field(
        default=False, description="Enable mutation testing for test quality"
    )
    property_testing_enabled: bool = Field(
        default=False, description="Enable property-based testing requirements"
    )
    integration_test_ratio: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Required ratio of integration tests"
    )
    ...
    def validate_naming_convention(cls, v: str) -> str:
        ...
        if "test_" not in v.lower() and "{" not in v:
            raise ValueError(
                "Naming convention must contain 'test_' or use placeholders like {function_name}"
            )
        return v
    ...
    def validate_package_paths(cls, v: list[str]) -> list[str]:
        ...
        for package in v:
            if not package.startswith("src/") and not package.startswith("tests/"):
                raise ValueError(
                    f"Package path '{package}' should start with 'src/' or 'tests/'"
                )
...
class TDDRuleEngine:
    ...
    def validate_test_naming(
        self, test_function_name: str, implementation_function: str
    ) -> bool:
    ...
    def calculate_compliance_score(self, report: TDDComplianceReport) -> float:
        ...
        implementation_score = (
            report.implemented_requirements / report.total_requirements
        )
        validation_score = report.validated_requirements / report.total_requirements
        coverage_score = (
            sum(report.coverage_report.values()) / len(report.coverage_report)
        ...
        # Weighted average
        compliance_score = (
            implementation_score * 0.4 + validation_score * 0.4 + coverage_score * 0.2
        )
...
class TDDConfigManager:
    ...
    def __init__(self, config_path: Path | None = None):
        ...
        self._settings: TDDSettings | None = None
        self._rule_engine: TDDRuleEngine | None = None
...
_tdd_config_manager: TDDConfigManager | None = None
