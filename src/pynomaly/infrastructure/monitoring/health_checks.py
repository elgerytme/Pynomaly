...
class HealthChecker:
    ...
    async def get_system_health(self, version: str = "unknown") -> SystemHealth:
        ...
        if not check_results:
            ...
            unhealthy_count = len([r for r in check_results if r.status == HealthStatus.UNHEALTHY])
...
async def check_system_resources() -> HealthCheckResult:
    ...
    try:
        ...
        if memory.percent > 90 or cpu_percent > 95 or details["disk"]["percent_used"] > 95:
            status = HealthStatus.UNHEALTHY
            message = "Critical resource usage detected"
        elif memory.percent > 80 or cpu_percent > 85 or details["disk"]["percent_used"] > 85:
