...
class AutonomousDetectionMonitor:
    ...
    def record_detection_end(
        ...
        self.record_metric(
            ...
            anomalies_found,
            "count",
            labels={"detection_id": detection_id, "algorithm": best_algorithm or "unknown"}
        )
        ...
        if self.enable_prometheus:
            ...
            for algo in algorithms_used:
                ...
                if success:
                    self.detection_duration.labels(algorithm=algo).observe(execution_time)
                    if algo == best_algorithm:
                        self.anomalies_found.labels(algorithm=algo).observe(anomalies_found)
    ...
    def record_algorithm_selection(
        ...
        self.record_metric(
            ...
            avg_confidence,
            "ratio",
            labels={"top_algorithm": selected_algorithms[0] if selected_algorithms else "none"}
        )
        
        self.record_metric(
            "algorithm_selection_top_confidence",
            top_confidence,
            "ratio"
        )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        avg_execution_time = (
            self.detection_stats["total_execution_time"] / 
            max(1, self.detection_stats["successful_detections"])
        )
...
_global_monitor: Optional[AutonomousDetectionMonitor] = None
