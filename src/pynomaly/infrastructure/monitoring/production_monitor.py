...
class ProductionMonitor:
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        for level in LogLevel:
            log_levels[level.value] = len(
                [log for log in recent_logs if log.level == level]
            )
        ...
        # Recent audit events
        recent_audits = [
            audit for audit in self.audit_events if audit.timestamp >= hour_ago
        ]

        return {
            "timestamp": now.isoformat(),
            "monitoring_period": "last_hour",
            "logs": {"total": len(recent_logs), "by_level": log_levels},
            "errors": {
                "total": len(recent_errors),
                "by_type": error_types,
                "unresolved": len([err for err in recent_errors if not err.resolved]),
            },
            "audit_events": {"total": len(recent_audits)},
            "storage_usage": {
...
_monitor_instance: ProductionMonitor | None = None
...
def init_monitor(
    log_level: LogLevel = LogLevel.INFO, log_file: Path | None = None, **kwargs
) -> ProductionMonitor:
...
def audit_event(
    event_type: str, operation: str, resource: str, action: str, **kwargs
) -> str:
