...
class OnCallSchedule:
    ...
    # Override settings
    overrides: List[Dict[str, Any]] = field(default_factory=list)  # Vacation, holidays, etc.
...
class EmailNotificationProvider(NotificationProvider):
    ...
    async def send_notification(
        ...
        try:
            ...
            # Create email body
            email_template = Template(self.config.get("template", """
            <html>
            <body>
                <h2>{{ alert.title }}</h2>
...
class SMSNotificationProvider(NotificationProvider):
    ...
    async def send_notification(
        ...
        try:
            ...
            # Create SMS message
            sms_template = Template(self.config.get("template", 
            ...
            logger.info(f"Sent SMS notifications for alert {alert.alert_id} to {success_count}/{len(recipients)} recipients")
...
class AdvancedAlertingService:
    ...
    def __init__(self):
        ...
        self.dynamic_thresholds: Dict[str, Dict[str, float]] = {}  # metric_name -> {mean, std, threshold}
    ...
    def add_notification_channel(self, channel: NotificationChannel) -> None:
        ...
        if channel.channel_type not in self.notification_providers:
            provider = self._create_notification_provider(channel.channel_type, channel.config)
            if provider:
                self.notification_providers[channel.channel_type] = provider
        
        logger.info(f"Added notification channel: {channel.name} ({channel.channel_type.value})")
    ...
    def _create_notification_provider(
        ...
        try:
            ...
            logger.error(f"Failed to create notification provider for {channel_type}: {e}")
            return None
    ...
    async def _evaluate_metric_alerts(self, metric_name: str, value: Union[int, float]) -> None:
        ...
        for rule in self.alert_rules.values():
            ...
            relevant_conditions = [
                ...
                if cond.metric_name == metric_name
            ]
            ...
            try:
                ...
                for channel in matching_channels:
                    ...
                    success = await provider.send_notification(
                        alert,
                        recipients,
                        {"channel": channel.name, "rule": rule}
                    )
        ...
        if existing_alert:
            ...
            if rule and rule.auto_resolve_minutes:
                ...
                if time_since_trigger > timedelta(minutes=rule.auto_resolve_minutes):
                    await self.resolve_alert(existing_alert.alert_id, "system", "Auto-resolved")
        ...
        while True:
            try:
                ...
                for alert in self.active_alerts.values():
                    ...
                    if time_since_trigger > timedelta(minutes=rule.escalation_delay_minutes):
    ...
    async def _escalate_alert(self, alert: Alert, rule: AlertRule) -> None:
        ...
        next_level = self._get_next_escalation_level(current_level, rule.escalation_levels)
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "active_alerts": len([a for a in self.active_alerts.values() if a.state == AlertState.TRIGGERED]),
