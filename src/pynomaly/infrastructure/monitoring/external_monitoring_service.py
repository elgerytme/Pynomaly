...
class GrafanaMonitoringProvider(ExternalMonitoringProvider):
    ...
    async def send_metric(self, metric: MetricData) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                influx_url, params=params, headers=headers, data=line
            ) as response:
    ...
    async def send_alert(self, alert: AlertData) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                alert_url, headers=headers, json=alert_payload
            ) as response:
    ...
    async def send_metric(self, metric: MetricData) -> bool:
        ...
        try:
            ...
            if metric.metric_type == MetricType.COUNTER:
                statsd.increment(
                    metric.name, value=metric.value, tags=tags, sample_rate=1.0
                )
...
class NewRelicMonitoringProvider(ExternalMonitoringProvider):
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            newrelic.agent.record_custom_event(
                "PynormalyConnectionTest", {"test": True, "timestamp": time.time()}
            )
            return True
...
class PrometheusMonitoringProvider(ExternalMonitoringProvider):
    ...
    async def send_alert(self, alert: AlertData) -> bool:
        ...
        try:
            ...
            webhook_url = self.config.settings.get(
                "alertmanager_webhook", f"{self.config.endpoint_url}/api/v1/alerts"
            )
            ...
            async with self.session.post(
                ...
                if response.status in [200, 201, 202]:
                    logger.info(
                        f"Sent alert {alert.alert_id} to Prometheus/Alertmanager"
                    )
                    return True
                else:
                    logger.error(
                        f"Failed to send alert to Prometheus: {response.status}"
                    )
                    return False
...
class CustomWebhookProvider(ExternalMonitoringProvider):
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                self.config.endpoint_url, json=test_payload, headers=headers
            ) as response:
...
class ExternalMonitoringService:
    ...
    async def _send_metric_to_providers(
        ...
        for provider_name in target_providers:
            if (
                ...
                and self.configurations[provider_name].enabled
            ):
                try:
    ...
    async def flush_buffers(self) -> None:
        ...
        await asyncio.gather(
            self.flush_metrics(), self.flush_alerts(), return_exceptions=True
        )
...
async def send_anomaly_detection_metrics(
    ...
    await service.send_metric(
        "anomaly.detections.count", anomaly_count, MetricType.COUNTER, tags
    )
    await service.send_metric(
        "anomaly.samples.total", total_samples, MetricType.GAUGE, tags
    )
    await service.send_metric(
        "anomaly.detection.time", detection_time, MetricType.TIMER, tags
    )

    if accuracy_score is not None:
        await service.send_metric(
            "anomaly.accuracy", accuracy_score, MetricType.GAUGE, tags
        )

    # Calculate anomaly rate
    anomaly_rate = (anomaly_count / total_samples) * 100 if total_samples > 0 else 0
    await service.send_metric(
        "anomaly.rate.percent", anomaly_rate, MetricType.GAUGE, tags
    )
...
async def send_training_job_metrics(
    ...
    await service.send_metric(
        "training.trials.count", trial_count, MetricType.COUNTER, tags
    )

    if best_score is not None:
        await service.send_metric(
            "training.best_score", best_score, MetricType.GAUGE, tags
        )

    if execution_time is not None:
        await service.send_metric(
            "training.execution_time", execution_time, MetricType.TIMER, tags
        )
