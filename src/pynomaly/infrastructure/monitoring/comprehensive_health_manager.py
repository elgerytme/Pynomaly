...
class ComprehensiveHealthManager:
    ...
    async def _check_cache_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            elif cache_stats.get("health_monitoring", {}).get("current_status", {}).get("overall_health") == "unhealthy":
                status = HealthStatus.UNHEALTHY
                message = "Cache health monitoring reports unhealthy"
            elif cache_stats.get("health_monitoring", {}).get("current_status", {}).get("overall_health") == "degraded":
                status = HealthStatus.DEGRADED
        ...
        try:
            ...
            for setting_name, setting_value in critical_settings:
                if not setting_value:
                    config_issues.append(f"Critical setting '{setting_name}' not configured")
            
            response_time = (time.time() - start_time) * 1000
            
            # Health assessment
            if config_issues:
                status = HealthStatus.DEGRADED if len(config_issues) < 3 else HealthStatus.UNHEALTHY
    ...
    async def _run_health_checks_with_dependencies(self, check_names: List[str]) -> None:
        ...
        while remaining:
            ...
            tasks = [
                self.health_checker.check_component(name)
    ...
    async def get_comprehensive_health_report(self) -> Dict[str, Any]:
        ...
        if len(recent_history) >= 2:
            recent_unhealthy = sum(1 for h in recent_history[-5:] if h.status == HealthStatus.UNHEALTHY)
            older_unhealthy = sum(1 for h in recent_history[-10:-5] if h.status == HealthStatus.UNHEALTHY)
        ...
        return {
            ...
            "summary": {
                ...
                "healthy_critical": len([
                    s for s in component_status.values()
                    if s["status"] == "healthy"
                ]),
            },
        }
...
_comprehensive_health_manager: Optional[ComprehensiveHealthManager] = None
