    ...
    def capture_cli_command(
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def sync_wrapper(*args, **kwargs):
                if not self.enable_automatic_capture or not feature_flags.is_enabled("advanced_automl"):
                    return func(*args, **kwargs)
                
                # Use async wrapper internally
                return asyncio.run(self._async_wrapper(func, args, kwargs, command_type, capture_results, tags))
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                return await self._async_wrapper(func, args, kwargs, command_type, capture_results, tags)
    ...
    async def _capture_command_configuration(
        ...
        try:
            ...
            capture_request = ConfigurationCaptureRequestDTO(
                ...
                auto_save=True,
                generate_name=True,
                tags=["cli", command_context["command_type"], "intercepted"] + command_context["tags"]
            )
            ...
            if response.success:
                ...
                logger.info(f"Captured CLI command configuration: {command_context['command_type']}")
            else:
                self.capture_stats["failed_captures"] += 1
                logger.warning(f"Failed to capture CLI command configuration: {response.message}")
...
_global_interceptor: Optional[CLIParameterInterceptor] = None


def initialize_cli_interceptor(configuration_service: ConfigurationCaptureService) -> CLIParameterInterceptor:
...
def get_cli_interceptor() -> Optional[CLIParameterInterceptor]:
...
class CLIConfigurationAnalytics:
    ...
    async def analyze_cli_usage_patterns(
        ...
        return {
            ...
            "most_used_command": max(command_types.items(), key=lambda x: x[1])[0] if command_types else None,
    ...
    async def get_command_recommendations(
                ...
                try:
                    ...
                    if numeric_values:
                        recommendations[param] = {
                            "recommended_value": sum(numeric_values) / len(numeric_values),
                            "type": "numeric",
        ...
        return {
            ...
            "based_on_configurations": [str(config.id) for config in configurations[:10]]
        }