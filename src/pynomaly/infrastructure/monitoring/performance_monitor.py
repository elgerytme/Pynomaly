...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        self.operation_statistics: Dict[str, List[PerformanceMetrics]] = defaultdict(list)
    ...
    def get_operation_statistics(
        ...
        throughputs = [m.samples_per_second for m in metrics_list if m.samples_per_second > 0]
    ...
    def export_metrics(
        ...
        if format_type == "json":
            ...
            writer = csv.DictWriter(output, fieldnames=[
                'timestamp', 'operation_name', 'algorithm_name', 'execution_time',
                'memory_usage', 'cpu_usage', 'samples_processed', 'samples_per_second'
            ])
            writer.writeheader()
    ...
    def _check_alerts(self, metrics: PerformanceMetrics) -> None:
        ...
        if (metrics.execution_time > self.alert_thresholds.get('execution_time', float('inf'))):
        ...
        if (metrics.memory_usage > self.alert_thresholds.get('memory_usage', float('inf'))):
            alert = PerformanceAlert(
                alert_type='threshold_exceeded',
                severity='high',
        ...
        if (metrics.samples_per_second > 0 and metrics.samples_per_second < min_throughput):
            alert = PerformanceAlert(
                alert_type='threshold_exceeded',
                severity='medium',
            ...
            with PerformanceTracker(monitor, operation_name, algorithm_name) as tracker:
                ...
                if hasattr(result, 'metadata') and isinstance(result.metadata, dict):
                    if 'samples_processed' in result.metadata:
                        tracker.set_samples_processed(result.metadata['samples_processed'])
