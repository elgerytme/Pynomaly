...
class BaseAnomalyModel(nn.Module):
    ...
    def loss_function(self, x: torch.Tensor, recon: torch.Tensor, **kwargs) -> torch.Tensor:
        raise NotImplementedError
...
class VariationalAutoEncoder(BaseAnomalyModel):
    ...
    def loss_function(self, x: torch.Tensor, recon: torch.Tensor, mu: torch.Tensor, logvar: torch.Tensor, beta: float = 1.0) -> torch.Tensor:
        # Reconstruction loss
...
class DAGMM(BaseAnomalyModel):
    ...
    def __init__(self, input_dim: int, hidden_dims: List[int], latent_dim: int, n_gmm: int = 4):
        super().__init__()
    ...
    def compute_gmm_params(self, z: torch.Tensor, gamma: torch.Tensor):
        ...
        # Mu: means
        self.mu = torch.sum(gamma.unsqueeze(-1) * z.unsqueeze(1), dim=0) / torch.sum(gamma, dim=0).unsqueeze(-1)
    ...
    def loss_function(self, x: torch.Tensor, x_recon: torch.Tensor, z: torch.Tensor, gamma: torch.Tensor) -> torch.Tensor:
        # Reconstruction loss
    ...
    def energy(self, z: torch.Tensor) -> torch.Tensor:
        ...
        for k in range(self.n_gmm):
            ...
            exp_term = -0.5 * torch.sum(torch.sum(diff.unsqueeze(-1) * inv_cov.unsqueeze(0), dim=-2) * diff, dim=-1)
...
class PyTorchAdapter(DetectorProtocol):
    ...
    def __init__(self, detector: Detector):
        ...
        self._model: Optional[BaseAnomalyModel] = None
    ...
    def fit(self, dataset: Dataset) -> None:
        ...
        try:
            ...
            if self.detector.algorithm == "DAGMM":
                ...
                self._model = self._model_class(input_dim, hidden_dims, latent_dim, n_gmm)
            else:
            ...
            train_dataset = TensorDataset(tensor_x, tensor_x)  # Using input as target for reconstruction
            ...
            for epoch in range(epochs):
                ...
                for batch_x, _ in train_loader:
                    ...
                    if isinstance(self._model, AutoEncoder):
                        ...
                        loss = self._model.loss_function(batch_x, recon, mu, logvar, beta)
    ...
    def get_algorithm_info(cls, algorithm: str) -> Dict[str, Any]:
        ...
        info = {
            "AutoEncoder": {
                ...
                "parameters": {
                    "hidden_dims": {"type": "list", "default": [128, 64, 32], "description": "Hidden layer dimensions"},
                    "latent_dim": {"type": "int", "default": 16, "description": "Latent space dimension"},
                    "epochs": {"type": "int", "default": 100, "description": "Training epochs"},
                    "batch_size": {"type": "int", "default": 256, "description": "Batch size"},
                ...
                "suitable_for": ["tabular_data", "high_dimensional", "nonlinear_patterns"],
                "pros": ["Simple architecture", "Fast training", "Interpretable reconstruction"],
            ...
            "VAE": {
                ...
                "parameters": {
                    "hidden_dims": {"type": "list", "default": [128, 64, 32], "description": "Hidden dimensions"},
                    "latent_dim": {"type": "int", "default": 16, "description": "Latent dimension"},
                    "beta": {"type": "float", "default": 1.0, "description": "KL divergence weight"},
                    "epochs": {"type": "int", "default": 100, "description": "Training epochs"},
                ...
                "pros": ["Principled probabilistic approach", "Can generate new samples"],
            ...
            "DeepSVDD": {
                ...
                "parameters": {
                    "hidden_dims": {"type": "list", "default": [128, 64, 32], "description": "Network dimensions"},
                    "latent_dim": {"type": "int", "default": 32, "description": "Output dimension"},
                    "epochs": {"type": "int", "default": 100, "description": "Training epochs"},
                    "contamination": {"type": "float", "default": 0.1, "description": "Anomaly rate"}
                },
                "suitable_for": ["one_class_classification", "compact_representations"],
                "pros": ["Theoretically motivated", "Compact decision boundary"],
                "cons": ["Sensitive to center initialization", "May collapse representations"]
            },
            "DAGMM": {
                ...
                "parameters": {
                    "hidden_dims": {"type": "list", "default": [60, 30, 10], "description": "Compression network"},
                    "latent_dim": {"type": "int", "default": 5, "description": "Encoding dimension"},
                    "n_gmm": {"type": "int", "default": 4, "description": "Number of GMM components"},
                    "epochs": {"type": "int", "default": 100, "description": "Training epochs"},
                    "contamination": {"type": "float", "default": 0.1, "description": "Anomaly rate"}
                },
