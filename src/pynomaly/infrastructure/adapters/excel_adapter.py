...
class ExcelAdapter(ExportProtocol, ImportProtocol):
    ...
    def _export_with_xlsxwriter(
        ...
        try:
            ...
            self._create_results_worksheet(
                workbook, results, header_format, anomaly_format, normal_format, score_format
            )
    ...
    def _create_results_worksheet(
        self, workbook, results: DetectionResult, header_format, anomaly_format, normal_format, score_format
    ):
        """Create the main results worksheet with xlsxwriter."""
        worksheet = workbook.add_worksheet('Results')
        
        # Headers
        headers = ['Index', 'Anomaly Score', 'Is Anomaly', 'Confidence', 'Feature Values']
        for col, header in enumerate(headers):
            worksheet.write(0, col, header, header_format)
        
        # Data rows
        for row, (idx, score) in enumerate(zip(range(len(results.scores)), results.scores), 1):
            # Index
            ...
            if hasattr(results, 'features') and results.features is not None:
                if idx < len(results.features):
                    feature_str = str(results.features[idx])[:50]  # Truncate for display
    ...
    def _create_summary_worksheet(self, workbook, results: DetectionResult, header_format):
    ...
    def _create_metadata_worksheet(self, workbook, results: DetectionResult, options: ExportOptions, header_format):
    ...
    def _populate_results_openpyxl(self, worksheet, results: DetectionResult):
        ...
        # Data
        for row, (idx, score) in enumerate(zip(range(len(results.scores)), results.scores), 2):
            worksheet.cell(row=row, column=1, value=idx)
            worksheet.cell(row=row, column=2, value=float(score.value))
            
            is_anomaly = bool(results.labels[idx])
            is_anomaly_cell = worksheet.cell(row=row, column=3, value='YES' if is_anomaly else 'NO')
            if is_anomaly:
                is_anomaly_cell.fill = PatternFill(start_color="FFE6E6", end_color="FFE6E6", fill_type="solid")
                is_anomaly_cell.font = Font(color="CC0000", bold=True)
            
            confidence = getattr(score, 'confidence', None)
            worksheet.cell(row=row, column=4, value=float(confidence) if confidence is not None else 'N/A')
    ...
    def _populate_summary_openpyxl(self, worksheet, results: DetectionResult):
        ...
        summary_data = [
            ...
            ['Anomaly Rate (%)', f'{(sum(anomalies) / len(anomalies) * 100):.2f}' if anomalies else '0.00'],
            ['Average Score', f'{sum(scores) / len(scores):.4f}' if scores else '0.0000'],
        ]
    ...
    def _populate_metadata_openpyxl(self, worksheet, results: DetectionResult, options: ExportOptions):
    ...
    def import_dataset(
        ...
        try:
            ...
            logger.info(f"Successfully imported dataset from {file_path}: {len(df)} rows, {len(df.columns)} columns")
            return dataset
    ...
    def _validate_and_clean_data(self, df: pd.DataFrame, options: Dict[str, Any]) -> pd.DataFrame:
