...
class ModelTrainerAdapter(ModelTrainer):
    ...
    async def evaluate(
        ...
        try:
            if self.evaluate_model_use_case:
                ...
                from pynomaly.application.use_cases.evaluate_model import EvaluateModelRequest
    ...
    async def _fallback_evaluate(
        ...
        threshold = np.percentile(pred_scores, (1 - detector.parameters.get("contamination", 0.1)) * 100)
        ...
        try:
            ...
            metrics["balanced_accuracy"] = balanced_accuracy_score(true_labels, pred_labels)
