...
class OptimizedPyODAdapter(PyODAdapter):
    ...
    async def train(self, dataset: Dataset) -> Detector:
        ...
        try:
            ...
            if self._selected_features is not None:
                detector.metadata["selected_features_count"] = int(
                    np.sum(self._selected_features)
                )
    ...
    async def _prepare_features_optimized(
    ...
    def _apply_feature_selection(
        ...
        if training:
            ...
            self._feature_selector = VarianceThreshold(
    ...
    async def _train_batch_optimized(self, X: np.ndarray, dataset: Dataset) -> Detector:
        ...
        if (
            len(X) > self.batch_size * 5
        ):  # Only sample if dataset is significantly larger
            ...
            self.logger.info(
                f"Training on representative sample: {len(X_sample)} samples"
            )
        else:
            X_sample = X
    ...
    async def _train_standard_optimized(
        self, X: np.ndarray, dataset: Dataset
    ) -> Detector:
    ...
    async def _detect_batch_optimized(
        self, X: np.ndarray, dataset: Dataset
    ) -> DetectionResult:
    ...
    async def _detect_standard_optimized(
        self, X: np.ndarray, dataset: Dataset
    ) -> DetectionResult:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            scores_future = loop.run_in_executor(
                executor, self._model.decision_function, X
            )
        ...
        return self._create_detection_result_optimized(
            anomaly_indices, anomaly_scores, dataset
        )
        ...
        result.metadata.update(
            {
                ...
                "batch_processing_used": self.enable_batch_processing
                and len(dataset.data) > self.batch_size,
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "feature_selection_stats": {
                ...
                "features_selected": (
                    int(np.sum(self._selected_features))
                    if self._selected_features is not None
                    else None
                ),
                "total_features": (
...
class AsyncAlgorithmExecutor:
    ...
    async def execute_multiple_algorithms(
        ...
        trained_detectors = await asyncio.gather(
        ...
        for i, (algo_name, adapter) in enumerate(adapters):
            if not isinstance(trained_detectors[i], Exception):
                ...
                detection_tasks.append(
                    asyncio.create_task(
                        self._handle_exception(algo_name, trained_detectors[i])
                    )
                )
        ...
        self.logger.info(
            f"Completed: {successful_algos}/{len(algorithms)} algorithms successful"
        )

        return final_results

    async def _execute_algorithm_training(
        self, algo_name: str, adapter: OptimizedPyODAdapter, dataset: Dataset
    ) -> Detector:
            ...
            try:
                if self.timeout:
                    detector = await asyncio.wait_for(
                        adapter.train(dataset), timeout=self.timeout
                    )
                else:
            ...
            try:
                if self.timeout:
                    result = await asyncio.wait_for(
                        adapter.detect(dataset), timeout=self.timeout
                    )
                else:
