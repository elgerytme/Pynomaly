...
class StatisticalTimeSeriesDetector:
    ...
    def fit(self, data: np.ndarray) -> None:
        ...
        self.stats_cache['rolling_mean'] = df['value'].rolling(window=self.window_size).mean().fillna(df['value'].mean())
        self.stats_cache['rolling_std'] = df['value'].rolling(window=self.window_size).std().fillna(df['value'].std())
    ...
    def predict(self, data: np.ndarray) -> np.ndarray:
        ...
        for i, value in enumerate(data):
            ...
            if value > self.stats_cache['p95']:
                percentile_score = (value - self.stats_cache['p95']) / (self.stats_cache['p95'] - self.stats_cache['global_mean'])
            elif value < self.stats_cache['p05']:
                percentile_score = (self.stats_cache['p05'] - value) / (self.stats_cache['global_mean'] - self.stats_cache['p05'])
            else:
                percentile_score = 0
...
class SeasonalDecompositionDetector:
    ...
    def _simple_seasonal_decompose(self, data: np.ndarray, period: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def fit(self, data: np.ndarray) -> None:
        ...
        if self.period > 1 and len(data) >= 2 * self.period:
            self.trend_component, self.seasonal_component, self.baseline_residuals = self._simple_seasonal_decompose(data, self.period)
        else:
    ...
    def predict(self, data: np.ndarray) -> np.ndarray:
        ...
        if len(data) != len(self.baseline_residuals):
            ...
            if self.period > 1 and len(data) >= 2 * self.period:
                trend, seasonal, residuals = self._simple_seasonal_decompose(data, self.period)
            else:
    ...
    def predict(self, data: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.window_size, len(data)):
            ...
            if i >= 2 * self.window_size:
                ...
                prev_window = data[:self.window_size] if len(data) >= self.window_size else data[:i]
...
class TimeSeriesAdapter(DetectorProtocol):
    ...
    def _init_algorithm(self) -> None:
        ...
        try:
            ...
            raise AdapterError(f"Failed to initialize {self.detector.algorithm_name}: {e}")
    ...
    def fit(self, dataset: Dataset) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully fitted time series detector: {self.detector.algorithm_name}")
        ...
        try:
            ...
            for idx in anomaly_indices:
                anomaly = Anomaly(
                    ...
                    data_point={"index": int(idx), "value": float(time_series_data[idx])},
            ...
            return DetectionResult(
                ...
                metadata={
                    ...
                    "detected_period": getattr(self._model, 'period', None) if hasattr(self._model, 'period') else None
                }
            )
    ...
    def get_algorithm_info(cls, algorithm: str) -> Dict[str, Any]:
        ...
        info = {
            "StatisticalTS": {
                ...
                "parameters": {
                    "window_size": {"type": "int", "default": 10, "description": "Rolling window size"},
                    "threshold_factor": {"type": "float", "default": 3.0, "description": "Z-score threshold multiplier"},
                    "contamination": {"type": "float", "default": 0.1, "description": "Expected anomaly rate"}
                },
                "suitable_for": ["univariate_time_series", "real_time_detection", "simple_patterns"],
                "pros": ["Fast computation", "No training required", "Interpretable results"],
            ...
            "SeasonalDecomposition": {
                ...
                "parameters": {
                    "period": {"type": "int", "default": "auto", "description": "Seasonal period (auto-detected if None)"},
                    "model": {"type": "str", "default": "additive", "description": "Decomposition model (additive/multiplicative)"},
                    "contamination": {"type": "float", "default": 0.1, "description": "Expected anomaly rate"}
                },
                "suitable_for": ["seasonal_data", "trend_analysis", "long_term_patterns"],
                "pros": ["Handles seasonality", "Separates trend from noise", "Good for business data"],
            ...
            "ChangePointDetection": {
                ...
                "parameters": {
                    "window_size": {"type": "int", "default": 10, "description": "Comparison window size"},
                    "sensitivity": {"type": "float", "default": 2.0, "description": "Detection sensitivity"},
                    "contamination": {"type": "float", "default": 0.1, "description": "Expected anomaly rate"}
                },
                "suitable_for": ["regime_changes", "structural_breaks", "monitoring_systems"],
                "pros": ["Detects structural changes", "Fast detection", "Good for monitoring"],
