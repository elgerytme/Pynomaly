...
class PyODAlgorithmRegistry:
    ...
    def load_algorithm_class(cls, algorithm: str) -> type[Any]:
        ...
        try:
            ...
            raise ImportError(f"PyOD is required for {algorithm}. Install with: pip install pyod") from e
        except AttributeError as e:
            raise AttributeError(f"Algorithm class {algo_info['class']} not found in {algo_info['module']}") from e
...
class PyODService:
    ...
    def create_model(self, algorithm: str, parameters: dict[str, Any] | None = None) -> Any:
    ...
    def predict_anomalies(self, model: Any, data: pd.DataFrame, threshold: float = 0.5) -> list[Anomaly]:
        ...
        try:
            ...
            logger.info(f"PyOD prediction completed: {len(anomalies)} anomalies from {len(data)} samples")
            return anomalies
