...
class AutoEncoderConfig(BaseModel):
    ...
    hidden_dims: List[int] = Field(default=[64, 32, 16], description="Hidden layer dimensions")
    ...
    contamination: float = Field(default=0.1, gt=0.0, lt=1.0, description="Expected contamination rate")
    threshold_strategy: str = Field(default="percentile", description="Threshold selection strategy")
...
class VAEConfig(BaseModel):
    ...
    beta: float = Field(default=1.0, ge=0.0, description="Beta parameter for KL divergence")
    learning_rate: float = Field(default=0.001, gt=0.0, description="Learning rate")
    ...
    use_reconstruction_error: bool = Field(default=True, description="Use reconstruction error for detection")
    use_kl_divergence: bool = Field(default=True, description="Include KL divergence in anomaly score")
...
class LSTMConfig(BaseModel):
    ...
    sequence_length: int = Field(default=10, ge=1, description="Sequence length for time series")
    ...
    contamination: float = Field(default=0.1, gt=0.0, lt=1.0, description="Contamination rate")
    prediction_window: int = Field(default=1, ge=1, description="Prediction window size")
...
class PyTorchAdapter(DetectorProtocol):
    ...
    def fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> 'PyTorchAdapter':
    ...
    def _fit_vae(self, X_tensor: torch.Tensor):
        ...
        for epoch in range(config.epochs):
            ...
            if (epoch + 1) % 20 == 0:
                avg_loss = epoch_loss / len(dataloader)
                logger.debug(f"VAE Epoch {epoch + 1}/{config.epochs}, Loss: {avg_loss:.6f}")
    ...
    def _fit_lstm(self, X_tensor: torch.Tensor):
        ...
        for epoch in range(config.epochs):
            ...
            if (epoch + 1) % 10 == 0:
                avg_loss = epoch_loss / len(dataloader)
                logger.debug(f"LSTM Epoch {epoch + 1}/{config.epochs}, Loss: {avg_loss:.6f}")
    ...
    async def async_fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> 'PyTorchAdapter':
