...
class JAXAutoEncoderConfig(BaseModel):
    ...
    hidden_dims: List[int] = Field(default=[128, 64, 32], description="Hidden layer dimensions")
...
class JAXGMMConfig(BaseModel):
    ...
    n_components: int = Field(default=5, ge=1, description="Number of mixture components")
    input_dim: int = Field(description="Input dimension")
...
class JAXSVDDConfig(BaseModel):
    ...
    nu: float = Field(default=0.1, gt=0.0, lt=1.0, description="Upper bound on anomaly fraction")
    learning_rate: float = Field(default=0.001, gt=0.0, description="Learning rate")
...
def create_gmm_params(key, config: JAXGMMConfig):
    ...
    if config.covariance_type == "full":
        covariances = jnp.array([jnp.eye(config.input_dim) for _ in range(config.n_components)])
...
class JAXAdapter(DetectorProtocol):
    ...
    def fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> 'JAXAdapter':
        """Train the JAX model."""
        try:
            # Preprocess data
    ...
    def _fit_autoencoder(self, X: jnp.ndarray):
        ...
        def train_step(state, batch):
            def loss_fn(params):
                recon = state.apply_fn(params, batch, training=True, rngs={'dropout': subkey})
        ...
        for epoch in range(config.epochs):
            ...
            if (epoch + 1) % 10 == 0:
                ...
                logger.debug(f"AutoEncoder Epoch {epoch + 1}/{config.epochs}, Loss: {avg_loss:.6f}")
        ...
        for iteration in range(config.max_iterations):
            ...
            for k in range(config.n_components):
                if config.covariance_type == "full":
                    ...
                    log_prob = stats.multivariate_normal.logpdf(X, params['means'][k], cov)
                else:
            ...
            log_probs = jnp.stack(log_probs, axis=0)
            log_resp = log_probs - jax.scipy.special.logsumexp(log_probs, axis=0, keepdims=True)
            ...
            # Update means
            for k in range(config.n_components):
                params['means'] = params['means'].at[k].set(
                    jnp.sum(resp[k:k+1].T * X, axis=0) / resp_sum[k]
                )
            ...
            if (iteration + 1) % 10 == 0:
                ...
                logger.debug(f"GMM Iteration {iteration + 1}/{config.max_iterations}, Log-likelihood: {ll:.6f}")
    ...
    def _fit_svdd(self, X: jnp.ndarray):
        ...
        for epoch in range(config.epochs):
            ...
            if (epoch + 1) % 20 == 0:
                avg_loss = epoch_loss / n_batches
                logger.debug(f"SVDD Epoch {epoch + 1}/{config.epochs}, Loss: {avg_loss:.6f}")
    ...
    async def async_fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> 'JAXAdapter':
