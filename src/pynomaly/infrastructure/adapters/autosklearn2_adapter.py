...
from pynomaly.domain.entities import Detector, DetectionResult
...
class AutoSklearn2Adapter(DetectorProtocol):
    ...
    def train(self, detector: Detector, X: np.ndarray, y: Optional[np.ndarray] = None) -> bool:
        ...
        try:
            ...
            self._preprocessors[detector.id] = None  # auto-sklearn2 handles preprocessing
    ...
    def _train_oneclass(self, X: np.ndarray, parameters: Dict[str, Any]) -> AutoSklearnClassifier:
        ...
        classifier = AutoSklearnClassifier(
            ...
            include={
                'classifier': ['random_forest', 'extra_trees', 'gradient_boosting',
                              'adaboost', 'decision_tree', 'k_nearest_neighbors'],
    ...
    def _train_outlier(self, X: np.ndarray, y: Optional[np.ndarray], 
                      parameters: Dict[str, Any]) -> AutoSklearnClassifier:
    ...
    def _train_ensemble(self, X: np.ndarray, y: Optional[np.ndarray],
                       parameters: Dict[str, Any]) -> List[AutoSklearnClassifier]:
    ...
    def _compute_scores(self, model: Union[AutoSklearnClassifier, List[AutoSklearnClassifier]], 
                       X: np.ndarray, algorithm: str) -> np.ndarray:
    ...
    def predict(self, detector: Detector, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            logger.info(f"Predicted {np.sum(predictions)} anomalies out of {len(X)} samples")
    ...
    def get_feature_importance(self, detector: Detector) -> Optional[np.ndarray]:
                        ...
                        if hasattr(m, 'get_models_with_weights'):
                            ...
                            for model_weight_tuple in models_weights:
                                ...
                                if hasattr(base_model, 'feature_importances_'):
                                    importance_list.append(base_model.feature_importances_)
                    except Exception:
                        continue
