...
class EnsembleMetaAdapter(EnsembleDetectorProtocol):
    ...
    def base_detectors(self) -> List[DetectorProtocol]:
        ...
        return [config.detector for config in self._detector_configs.values() if config.enabled]
    ...
    def fit(self, dataset: Dataset) -> None:
        ...
        try:
            enabled_configs = [
                (name, config) for name, config in self._detector_configs.items()
                if config.enabled
            ]
    ...
    def score(self, dataset: Dataset) -> List[AnomalyScore]:
        ...
        return [
            AnomalyScore(value=float(score), method=f"ensemble_{self._aggregation_method.value}")
    ...
    def _fit_parallel(self, dataset: Dataset, configs: List[Tuple[str, DetectorConfig]]) -> None:
    ...
    def _fit_sequential(self, dataset: Dataset, configs: List[Tuple[str, DetectorConfig]]) -> None:
    ...
    def _aggregate_results(
        ...
        aggregated_labels = self._aggregate_predictions_with_method(all_predictions, weights)
        ...
        confidence_scores = self._calculate_confidence_scores(all_scores, agreement_scores)
        ...
        return DetectionResult(
            ...
            threshold=threshold,
            execution_time_ms=sum(r.execution_time_ms for r in individual_results.values()),
            metadata={
                "ensemble_name": self._name,
                "n_detectors": len(individual_results),
    ...
    def _aggregate_predictions_with_method(
        ...
        weight_array = np.array([weights.get(name, 1.0) for name in all_predictions.keys()])
    ...
    def _aggregate_scores(
        ...
        weights = {name: config.weight for name, config in configs if name in all_scores}
        ...
        for idx in anomaly_indices:
            ...
            for name, result in individual_results.items():
                if idx < len(result.labels):
                    individual_votes[name] = {
                        ...
                        "score": result.scores[idx].value if idx < len(result.scores) else 0.0
                    }
            ...
            anomaly = Anomaly(
                ...
                metadata={
                    ...
                    "n_detectors_agree": sum(1 for votes in individual_votes.values() if votes["prediction"] == 1),
                    "agreement_ratio": sum(1 for votes in individual_votes.values() if votes["prediction"] == 1) / len(individual_votes) if individual_votes else 0.0,
                }
            )
    ...
    def _calculate_agreement_scores(self, all_predictions: Dict[str, List[int]]) -> List[float]:
    ...
    def _calculate_dynamic_weights(self, all_scores: Dict[str, List[float]]) -> np.ndarray:
