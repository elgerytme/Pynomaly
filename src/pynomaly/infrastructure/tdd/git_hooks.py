...
def run_pre_commit_validation() -> int:
    ...
    try:
        ...
        critical_violations = [
            ...
            if v.severity in ["error"] and v.violation_type in [
                "missing_test", "implementation_before_test", "parse_error"
            ]
        ]
...
def run_pre_push_validation() -> int:
    ...
    try:
        ...
        critical_violations = [
            v for v in violations 
            if v.severity == "error" or (
                v.severity == "warning" and v.violation_type in [
                    "low_coverage", "missing_test"
                ]
            )
        ]
...
class PreCommitConfig:
    ...
    def add_tdd_hook(self) -> None:
        ...
        tdd_repo_exists = any(
            ...
            any(hook.get("id") == "tdd-validation" for hook in repo.get("hooks", []))
            for repo in repos
        )
    ...
    def remove_tdd_hook(self) -> None:
        ...
        for repo in repos:
            if repo.get("repo") == "local":
                ...
                hooks = [
                    hook for hook in repo.get("hooks", [])
