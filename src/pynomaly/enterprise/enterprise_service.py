...
class EnterpriseService:
    ...
    async def get_dashboard_data(
        self, tenant: TenantInfo, user: TenantUserInfo
        ...
        try:
            ...
            for event in recent_events:
                ...
                audit_summary["actions_breakdown"][action] = (
                    audit_summary["actions_breakdown"].get(action, 0) + 1
                )

                # Count status
                status = event.status.value
                audit_summary["status_breakdown"][status] = (
                    audit_summary["status_breakdown"].get(status, 0) + 1
                )

                # Count users
                if event.user_id:
                    audit_summary["top_users"][event.user_id] = (
                        audit_summary["top_users"].get(event.user_id, 0) + 1
                    )
    ...
    async def generate_compliance_report(
        self, tenant: TenantInfo, user: TenantUserInfo, request: ComplianceReportRequest
        ...
        try:
            ...
            summary = {
                ...
                "unique_users": len(
                    set(event.user_id for event in audit_events if event.user_id)
                ),
                "actions_performed": len(
                    set(event.action.value for event in audit_events)
                ),
                "resources_accessed": len(
                    set(event.resource_type for event in audit_events)
                ),
            ...
            if request.include_user_activity:
                ...
                for event in audit_events:
                    if event.user_id:
                        ...
                        user_stats[event.user_id]["actions_breakdown"][
                            event.action.value
                        ] = (
                            user_stats[event.user_id]["actions_breakdown"].get(
                                event.action.value, 0
                            )
                            + 1
                        )
            ...
            if request.include_resource_usage:
                ...
                for event in audit_events:
                    ...
                    if event.resource_id:
                        resource_stats[event.resource_type]["unique_resources"].add(
                            event.resource_id
                        )
                    resource_stats[event.resource_type]["actions_breakdown"][
                        event.action.value
                    ] = (
                        resource_stats[event.resource_type]["actions_breakdown"].get(
                            event.action.value, 0
                        )
                        + 1
                    )
            ...
            failed_events = [
                event for event in audit_events if event.status == AuditStatus.FAILURE
            ]
...
async def generate_compliance_report(
    ...
    return await enterprise_service.generate_compliance_report(
        current_tenant, current_user, request
    )
