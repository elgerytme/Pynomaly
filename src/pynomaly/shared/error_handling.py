...
def validate_file_exists(file_path: Union[str, Path]) -> Path:
    ...
    if not path.exists():
        raise ResourceNotFoundError(
            ...
            details={"file_path": str(file_path), "absolute_path": str(path.absolute())}
        )
    
    if not path.is_file():
        raise ResourceNotFoundError(
            f"Path is not a file: {file_path}",
            details={"file_path": str(file_path), "path_type": "directory" if path.is_dir() else "other"}
        )
...
def validate_algorithm_name(algorithm: str, available_algorithms: list[str]) -> str:
    ...
    if algorithm not in available_algorithms:
        ...
        matches = [alg for alg in available_algorithms if alg.lower() == algorithm_lower]
