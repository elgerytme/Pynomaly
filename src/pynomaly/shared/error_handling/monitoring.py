...
from .unified_exceptions import PynamolyError, ErrorSeverity, ErrorCategory
...
class ErrorMetrics:
    ...
    def add_error(self, error: PynamolyError) -> None:
        ...
        recent_errors_count = sum(
            1 for error_data in self.recent_errors
...
class ErrorMonitor:
    ...
    def get_error_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for hour in sorted(hourly_errors.keys()):
            avg_severity = statistics.mean(hourly_severity[hour]) if hourly_severity[hour] else 0
            trends.append({
                "hour": hour,
                "timestamp": hour * 3600,
        ...
        return {
            ...
            "summary": {
                ...
                "avg_errors_per_hour": statistics.mean(hourly_errors.values()) if hourly_errors else 0,
                "peak_hour": max(hourly_errors.items(), key=lambda x: x[1]) if hourly_errors else None,
            }
        }
