        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                if any(isinstance(e, exc_type) for exc_type in self.config.stop_on):
                    logger.info(
                        f"Stopping retry due to non-retryable exception: {type(e).__name__}"
                    )
                    break
                ...
                if not any(
                    ...
                    logger.info(
                        f"Not retrying due to exception type: {type(e).__name__}"
                    )
                    break
...
class ResilienceManager:
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        for name, bulkhead in self.bulkheads.items():
            ...
            utilization = (
                bulkhead_stats["current_calls"]
                / bulkhead_stats["config"]["max_concurrent_calls"]
            )

            if utilization > 0.9:
                bulkhead_health = "stressed"
                health_status["issues"].append(
                    f"Bulkhead '{name}' is highly utilized ({utilization:.1%})"
                )
...
def circuit_breaker(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            cb = get_resilience_manager().get_circuit_breaker(
                name, config, error_context
            )
...
def bulkhead(
    ...
    def decorator(func):
        @functools.wraps(func)
        async def wrapper(*args, **kwargs):
            bulkhead_instance = get_resilience_manager().get_bulkhead(
                name, config, error_context
            )
