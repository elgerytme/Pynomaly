...
from pynomaly.domain.entities.alert import (
    ...
    NoiseClassification, MLNoiseFeatures, AlertCorrelation, AlertMetadata
)
...
class AlertCorrelationEngine:
    ...
    def find_correlations(self, alert: Alert, time_window_minutes: int = 60) -> List[AlertCorrelation]:
        """Find correlations for a given alert."""
        correlations = []
        
        # Time-based correlation
        temporal_correlations = self._find_temporal_correlations(alert, time_window_minutes)
    ...
    def _find_temporal_correlations(self, alert: Alert, time_window_minutes: int) -> List[AlertCorrelation]:
        ...
        for other_alert_id, other_alert in self.alert_history.items():
            ...
            if time_diff <= time_threshold:
                ...
                correlation_strength = 1.0 - (time_diff.total_seconds() / (time_window_minutes * 60))
    ...
    def _find_pattern_correlations(self, alert: Alert) -> List[AlertCorrelation]:
        ...
        if similar_alerts:
            ...
            correlation = AlertCorrelation(
                ...
                correlation_strength=statistics.mean([sim for _, sim in similar_alerts[:5]]),
                correlation_reason=f"Similar patterns detected (avg similarity: {statistics.mean([sim for _, sim in similar_alerts[:5]]):.3f})",
                pattern_similarity=statistics.mean([sim for _, sim in similar_alerts[:5]]),
                feature_overlap=self._calculate_feature_overlap(alert_features, [feat for _, feat in similar_alerts[:5]])
            )
            correlations.append(correlation)
    ...
    def _find_causal_correlations(self, alert: Alert) -> List[AlertCorrelation]:
        ...
        for other_alert_id, other_alert in self.alert_history.items():
            ...
            if other_alert.created_at < alert.created_at:
                ...
                if self._is_potential_cause(other_alert, alert):
                    causal_strength = self._calculate_causal_strength(other_alert, alert, time_diff)
        ...
        if potential_causes:
            ...
            correlation = AlertCorrelation(
                ...
                correlation_strength=statistics.mean([strength for _, strength in potential_causes[:3]]),
    ...
    def _calculate_feature_overlap(self, features1: np.ndarray, features_list: List[np.ndarray]) -> float:
    ...
    def _is_potential_cause(self, potential_cause: Alert, effect_alert: Alert) -> bool:
        ...
        if (potential_cause.category == AlertCategory.INFRASTRUCTURE and 
            effect_alert.category == AlertCategory.ANOMALY_DETECTION):
            return True
        
        # System performance issues can cause anomalies
        if (potential_cause.category == AlertCategory.SYSTEM_PERFORMANCE and 
            effect_alert.category == AlertCategory.ANOMALY_DETECTION):
            return True
        
        # Security issues can cascade
        if (potential_cause.category == AlertCategory.SECURITY and 
            effect_alert.category in [AlertCategory.SYSTEM_PERFORMANCE, AlertCategory.ANOMALY_DETECTION]):
            return True
        
        # Same tenant issues
        if (potential_cause.metadata.tenant_id and effect_alert.metadata.tenant_id and
            potential_cause.metadata.tenant_id == effect_alert.metadata.tenant_id):
            return True
        
        return False
    
    def _calculate_causal_strength(self, cause: Alert, effect: Alert, time_diff: timedelta) -> float:
        ...
        if time_diff <= max_causal_window:
            strength += 0.4 * (1.0 - time_diff.total_seconds() / max_causal_window.total_seconds())
...
class NoiseClassificationModel:
    ...
    def extract_features(self, alert: Alert, alert_history: List[Alert]) -> MLNoiseFeatures:
        ...
        features.alerts_last_hour = len([a for a in alert_history if a.created_at >= one_hour_ago])
        features.alerts_last_day = len([a for a in alert_history if a.created_at >= one_day_ago])
        
        # Similar alerts in the last week
        similar_alerts = [a for a in alert_history 
                         if (a.created_at >= one_week_ago and 
        ...
        features.anomaly_score_percentile = self._calculate_percentile(
            ...
            [a.metadata.anomaly_score for a in alert_history if a.metadata.anomaly_score]
        )
        
        # Historical context features
        resolved_alerts = [a for a in alert_history if a.status == AlertStatus.RESOLVED]
        if resolved_alerts:
            false_positives = [a for a in resolved_alerts if a.resolution_quality and a.resolution_quality < 0.5]
            features.false_positive_rate_7d = len(false_positives) / len(resolved_alerts)
            
            resolution_times = [a.time_to_resolve.total_seconds() / 60 for a in resolved_alerts if a.time_to_resolve]
            features.resolution_time_avg = statistics.mean(resolution_times) if resolution_times else 0.0
    ...
    def predict_noise_probability(self, features: MLNoiseFeatures) -> Tuple[NoiseClassification, float]:
    ...
    def _retrain_model(self):
        ...
        try:
            ...
            feature_names = [
                ...
                "alerts_last_hour", "alerts_last_day", "similar_alerts_last_week",
                "system_load_percentile", "memory_pressure", "cpu_utilization",
                "model_confidence", "anomaly_score_percentile", "feature_stability",
                "false_positive_rate_7d", "resolution_time_avg", "escalation_frequency",
                "has_correlated_alerts", "correlation_strength_max", "num_related_alerts"
            ]
            
            self.feature_importance = dict(zip(feature_names, self.classifier.feature_importances_))
            
            logger.info(f"Retrained noise classification model with {len(self.training_features)} samples")
            
        except Exception as e:
            logger.error(f"Error retraining noise classification model: {e}")
    
    def _heuristic_classification(self, features: MLNoiseFeatures) -> Tuple[NoiseClassification, float]:
...
class IntelligentAlertService:
    ...
    async def process_alert_intelligence(self, alert: Alert) -> Alert:
        ...
        try:
            ...
            if correlations:
                ...
                alert.correlation = max(correlations, key=lambda c: c.correlation_strength)
                self.metrics["correlated_alerts"] += 1
            
            # Step 2: ML-based noise classification
            features = self.noise_classifier.extract_features(alert, list(self.alert_history))
            classification, confidence = self.noise_classifier.predict_noise_probability(features)
            ...
            logger.info(f"Processed alert {alert.alert_id} - Classification: {classification.value}, "
                       f"Confidence: {confidence:.3f}, Suppressed: {should_suppress}")
            
            return alert
            
        except Exception as e:
            logger.error(f"Error processing alert intelligence for {alert.alert_id}: {e}")
            return alert
    ...
    async def _should_suppress_alert(self, alert: Alert) -> Tuple[bool, str]:
        ...
        if (alert.noise_classification == NoiseClassification.NOISE and 
            alert.noise_confidence > 0.8):
            return True, f"ML classified as noise (confidence: {alert.noise_confidence:.3f})"
    ...
    def _are_alerts_duplicate(self, alert1: Alert, alert2: Alert) -> bool:
        ...
        if (alert1.category == alert2.category and
            alert1.severity == alert2.severity and
            alert1.source == alert2.source):
            
            # Same tenant if applicable
            if (alert1.metadata.tenant_id and alert2.metadata.tenant_id and
                alert1.metadata.tenant_id == alert2.metadata.tenant_id):
                return True
            
            # Same affected resources
            if (set(alert1.metadata.affected_resources) == 
                set(alert2.metadata.affected_resources) and
                alert1.metadata.affected_resources):
                return True
    ...
    def _count_similar_alerts(self, alert: Alert, time_window: timedelta) -> int:
        ...
        for other_alert in reversed(list(self.alert_history)):
            ...
            if (other_alert.alert_id != alert.alert_id and
                other_alert.category == alert.category and
                other_alert.severity == alert.severity):
                count += 1
    ...
    async def acknowledge_alert(self, alert_id: UUID, acknowledged_by: str, note: str = "") -> bool:
    ...
    async def resolve_alert(
        ...
        self.noise_classifier.add_training_sample(alert.ml_features, is_signal=is_signal)
    ...
    async def escalate_alert(self, alert_id: UUID, escalated_by: str, reason: str = "") -> bool:
        """Escalate an alert."""
        alert = self.alerts.get(alert_id)
        if not alert:
            return False
    ...
    async def get_alert_analytics(self, days: int = 7) -> Dict[str, Any]:
        ...
        analytics = {
            ...
            "noise_reduction_stats": {
                "total_suppressed": len([a for a in recent_alerts if a.is_suppressed()]),
                "ml_classified_noise": len([a for a in recent_alerts if a.noise_classification == NoiseClassification.NOISE]),
            ...
            "performance_metrics": {
                "avg_processing_time": statistics.mean(self.metrics["processing_times"][-1000:]) if self.metrics["processing_times"] else 0.0,
                "total_processed": self.metrics["total_alerts"],
                "suppression_rate": self.metrics["suppressed_alerts"] / max(1, self.metrics["total_alerts"])
            }
        }
        ...
        for alert in recent_alerts:
            ...
            analytics["alert_distribution"]["by_noise_classification"][alert.noise_classification.value] += 1
        
        # Calculate noise reduction stats
        noise_alerts = [a for a in recent_alerts if a.noise_classification == NoiseClassification.NOISE]
        signal_alerts = [a for a in recent_alerts if a.noise_classification == NoiseClassification.SIGNAL]
        
        if noise_alerts:
            analytics["noise_reduction_stats"]["avg_noise_confidence"] = statistics.mean([a.noise_confidence for a in noise_alerts])
        
        if signal_alerts and noise_alerts:
            analytics["noise_reduction_stats"]["signal_to_noise_ratio"] = len(signal_alerts) / len(noise_alerts)
        
        # Calculate correlation stats
        correlated_alerts = [a for a in recent_alerts if a.correlation]
        if correlated_alerts:
            analytics["correlation_stats"]["avg_correlation_strength"] = statistics.mean([a.correlation.correlation_strength for a in correlated_alerts])
            
            for alert in correlated_alerts:
                analytics["correlation_stats"]["correlation_types"][alert.correlation.correlation_type] += 1
