...
class MemoryOptimizationService:
    ...
    def optimize_dataset_for_detection(
        self,
        dataset: Union[Dataset, str, pd.DataFrame],
        ...
        if hasattr(dataset.data, 'nbytes'):
            ...
            dataset_size_mb = len(dataset.data) * 8 / 1024 / 1024  # Assume 8 bytes per element
    ...
    def stream_anomaly_detection(
        ...
        for chunk_dataset in self.streaming_processor.process_large_dataset(data_source):
            try:
...
class MemoryProfiler:
    ...
    def end_profiling(self, operation_name: str) -> Dict[str, Any]:
        ...
        if start_record is None:
            raise ValueError(f"No matching start record for operation: {operation_name}")
        ...
        start_record.update({
            ...
            "duration_seconds": (end_time - start_record["start_time"]).total_seconds()
        })
