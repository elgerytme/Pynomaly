...
from pynomaly.application.dto.training_dto import (
    TrainingRequestDTO, TrainingConfigDTO, TrainingResultDTO
)
...
from pynomaly.shared.exceptions import (
    TrainingError, ModelValidationError, DataValidationError
)
...
class AutomatedTrainingService:
    ...
    async def _optimize_hyperparameters(
        ...
        if optimization_strategy == 'optuna':
            ...
            raise TrainingError(f"Unknown optimization strategy: {optimization_strategy}")
    ...
    async def _optimize_with_optuna(
        ...
        try:
            ...
            raise TrainingError("Optuna not available. Install with: pip install optuna")
    ...
    async def _evaluate_model(
        ...
        if self._is_anomaly_detection_task(y_val):
            ...
            metrics_dict = self._calculate_classification_metrics(y_val, y_pred, y_pred_proba)
    ...
    def _calculate_classification_metrics(
        ...
        return {
            ...
            'roc_auc': roc_auc_score(y_true, y_pred_proba, multi_class='ovr') if len(np.unique(y_true)) > 1 else 0.0,
            'confusion_matrix': confusion_matrix(y_true, y_pred).tolist(),
            'additional_metrics': {
    ...
    async def _select_best_model(
        ...
        if selection_strategy == 'best_single_metric':
            ...
            best_result = max(
                training_results,
                key=lambda x: x['metrics'].get(primary_metric, 0.0)
            )
    ...
    async def _update_job_status(self, job: TrainingJob, status: TrainingStatus) -> None:
        """Update job status."""
        job.status = status
