...
class ExportService:
    ...
    def export_results(
        ...
        if not adapter.validate_file(file_path):
            raise ValueError(f"Invalid file path for {options.format.value} export: {file_path}")
        
        try:
            # Perform the export
            logger.info(f"Exporting results to {options.format.value} format: {file_path}")
    ...
    def validate_export_request(
        ...
        try:
            ...
            if not adapter.validate_file(file_path):
                ...
                validation_result['errors'].append(f"Invalid file path for {format.value} export")
    ...
    def export_multiple_formats(
        ...
        options_map: Optional[Dict[ExportFormat, ExportOptions]] = None
        ...
        for format in formats:
            try:
                ...
                if format != ExportFormat.EXCEL:
                    ...
                    file_path = base_path.with_name(f"{base_path.stem}_{format.value}{extension}")
