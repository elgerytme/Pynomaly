...
from .interfaces.pipeline_services import FeatureEngineeringResult, IFeatureEngineeringService
...
class FeatureEngineeringService(IFeatureEngineeringService):
    ...
    def _select_features(self, X: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
        ...
        if numeric_cols:
            ...
            try:
                ...
                selected_numeric = [
                    col for col, selected in zip(numeric_cols, selected_mask) 
                    if selected
                ]
                ...
                logger.warning(f"Variance selection failed: {e}, keeping all numeric features")
    ...
    def create_feature_summary(self, result: FeatureEngineeringResult) -> dict[str, Any]:
    ...
    def _generate_feature_recommendations(
        ...
        if final_count > original_count * 2:
            recommendations.append("Consider more aggressive feature selection to reduce dimensionality")
        
        if final_count < original_count * 0.5:
            recommendations.append("Many features were removed - verify important information isn't lost")
        
        # Statistical features recommendations
        stats_count = len(metadata.get("statistical_features", []))
        if stats_count > 0:
            recommendations.append(f"Created {stats_count} statistical features - monitor for overfitting")
        
        # Interaction features recommendations
        interaction_count = len(metadata.get("interaction_features", []))
        if interaction_count > 0:
            recommendations.append(f"Created {interaction_count} interaction features - validate business meaning")
        
        # Data quality recommendations
        if "error" in metadata:
            recommendations.append("Feature engineering encountered errors - review data quality")
