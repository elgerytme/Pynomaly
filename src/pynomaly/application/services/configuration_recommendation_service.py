...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationSource, ConfigurationLevel,
    ConfigurationRecommendationDTO, DatasetCharacteristicsDTO,
    ConfigurationResponseDTO, ConfigurationSearchRequestDTO
)
...
class ConfigurationRecommendationService:
    ...
    async def recommend_configurations(
        ...
        logger.info(f"Generating recommendations for dataset with {dataset_characteristics.n_samples} samples")
        ...
        if self.enable_ml_recommendations and self.performance_predictor:
            ...
            ml_recommendations = await self._generate_ml_recommendations(
                dataset_characteristics, performance_requirements, max_recommendations // 2
            )
        ...
        if self.enable_similarity_recommendations:
            ...
            similarity_recommendations = await self._generate_similarity_recommendations(
                dataset_characteristics, performance_requirements, max_recommendations // 2
            )
        ...
        if not recommendations:
            rule_recommendations = await self._generate_rule_based_recommendations(
                dataset_characteristics, performance_requirements, use_case, 
                difficulty_level, max_recommendations
            )
        ...
        logger.info(f"Generated {len(final_recommendations)} configuration recommendations")
    ...
    async def predict_configuration_performance(
        ...
        logger.debug(f"Predicting performance for {configuration.algorithm_config.algorithm_name}")
        
        # Extract features
        features = self._extract_prediction_features(configuration, dataset_characteristics)
        ...
        try:
            ...
            predicted_f1 = 2 * (predicted_precision * predicted_recall) / (predicted_precision + predicted_recall)
    ...
    async def train_recommendation_models(
        ...
        valid_configs = [
            ...
            if config.performance_results and config.performance_results.accuracy is not None
        ]
        
        if len(valid_configs) < min_configurations:
            logger.warning(f"Insufficient data for training: {len(valid_configs)} < {min_configurations}")
            return {
                "error": f"Need at least {min_configurations} configurations, got {len(valid_configs)}",
                "available_configurations": len(valid_configs)
            }
        
        logger.info(f"Training recommendation models with {len(valid_configs)} configurations")
        ...
        # Split data
        X_train, X_test, y_acc_train, y_acc_test, y_alg_train, y_alg_test = train_test_split(
        ...
        logger.info(f"Model training completed: RMSE={training_results['performance_predictor_rmse']:.3f}, "
    ...
    async def analyze_recommendation_patterns(
        ...
        analysis = {
            ...
            "performance_summary": {
                ...
                "high_performance_rate": len([a for a in accuracies if a > 0.85]) / len(accuracies) if accuracies else 0
            },
            "recommendation_trends": self._analyze_recommendation_trends(recent_configs)
        }
    ...
    async def _generate_ml_recommendations(
        ...
        dummy_config = self._create_dummy_configuration("dummy", dataset_characteristics)
        features = self._extract_prediction_features(dummy_config, dataset_characteristics)
        ...
        for i, idx in enumerate(sorted_indices[:max_recommendations]):
            ...
            if similar_configs:
                ...
                recommendation = ConfigurationRecommendationDTO(
                    ...
                    use_cases=[],
                    difficulty_level=best_config.metadata.difficulty_level or ConfigurationLevel.INTERMEDIATE,
                    estimated_training_time=predicted_performance.get("training_time_seconds", 60),
                    parameter_suggestions=best_config.algorithm_config.hyperparameters or {},
                    preprocessing_suggestions=self._get_preprocessing_suggestions(dataset_characteristics),
    ...
    async def _generate_similarity_recommendations(
        ...
        similar_configs.sort(
            key=lambda x: (x[1], x[0].performance_results.accuracy if x[0].performance_results and x[0].performance_results.accuracy else 0),
            reverse=True
        )
        ...
        for config, similarity in similar_configs[:max_recommendations]:
            ...
            recommendation = ConfigurationRecommendationDTO(
                ...
                confidence_score=similarity * 0.8,  # Scale down similarity-based confidence
                predicted_performance=predicted_performance,
                recommendation_reason=f"Similar dataset characteristics (similarity: {similarity:.1%})",
                use_cases=[],
                difficulty_level=config.metadata.difficulty_level or ConfigurationLevel.INTERMEDIATE,
                estimated_training_time=predicted_performance.get("training_time_seconds", 60),
        ...
        return [
            {
                ...
                "configuration": {"algorithm": "IsolationForest", "parameters": {"n_estimators": 50}},
                "confidence": 0.7,
            ...
            },
            {
                ...
                "configuration": {"algorithm": "LocalOutlierFactor", "parameters": {"n_neighbors": 20}},
                "confidence": 0.8,
            ...
            },
            {
                ...
                "configuration": {"algorithm": "PCA", "parameters": {"n_components": 0.95}},
                "confidence": 0.6,
            ...
            },
            {
                ...
                "configuration": {"algorithm": "IsolationForest", "parameters": {"contamination": 0.001}},
                "confidence": 0.9,
            ...
            },
            {
                ...
                "configuration": {"algorithm": "OneClassSVM", "parameters": {"nu": 0.05, "kernel": "linear"}},
                "confidence": 0.7,
    ...
    def _extract_prediction_features(
        ...
        common_algorithms = ["IsolationForest", "LocalOutlierFactor", "OneClassSVM", "EllipticEnvelope"]
        for algo in common_algorithms:
            features.append(1.0 if configuration.algorithm_config.algorithm_name == algo else 0.0)
    ...
    def _create_dummy_configuration(
        ...
        from pynomaly.application.dto.configuration_dto import (
            AlgorithmConfigurationDTO, DatasetConfigurationDTO, ConfigurationMetadataDTO
        )
    ...
    def _calculate_dataset_similarity(
        ...
        if ds2.n_samples:
            size_ratio = min(ds1.n_samples, ds2.n_samples) / max(ds1.n_samples, ds2.n_samples)
            similarity += 0.3 * size_ratio
        
        # Feature count similarity
        if ds2.n_features:
            feature_ratio = min(ds1.n_features, ds2.n_features) / max(ds1.n_features, ds2.n_features)
    ...
    def _get_preprocessing_suggestions(
        ...
        if dataset_characteristics.missing_values_ratio and dataset_characteristics.missing_values_ratio > 0.01:
    ...
    def _get_feature_importance(self) -> Dict[str, float]:
        ...
        feature_names = [
            "log_n_samples", "log_n_features", "missing_values_ratio", 
            "outlier_ratio", "sparsity", "is_isolation_forest", 
            "is_lof", "is_one_class_svm", "is_elliptic_envelope",
            "contamination", "n_estimators_norm", "n_neighbors_norm",
            "n_hyperparams", "apply_pca", "has_scaling", "handle_missing"
        ]
