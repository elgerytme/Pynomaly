...
class StreamingDetectionService:
    ...
    def __init__(
        ...
        # Processing queues
        self.input_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.max_queue_size)
        self.batch_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.max_concurrent_batches)
        self.result_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.result_buffer_size)
    ...
    async def get_stream_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": metrics,
            "uptime_seconds": (datetime.utcnow() - metrics.stream_start_time).total_seconds(),
        }
    ...
    async def _batch_creation_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                should_create_batch = (
                    len(current_batch_samples) >= self._current_batch_size or
                    (timeout_occurred and current_batch_samples) or
                    len(current_batch_samples) >= self.config.adaptive_batch_max_size
                )
            ...
            try:
                ...
                try:
                    ...
                    if self._latency_history:
                        self.metrics.avg_processing_latency_ms = np.mean(self._latency_history)
                        self.metrics.max_processing_latency_ms = max(self._latency_history)
                        self.metrics.p95_processing_latency_ms = np.percentile(self._latency_history, 95)
                    ...
                    if self.anomaly_callback:
                        for sample_result in result.sample_results:
                            if sample_result.is_anomaly:
                                await asyncio.create_task(
                                    self._safe_call_handler(self.anomaly_callback, sample_result)
                                )
                    
                    # Update anomaly rate
                    if result.batch_size > 0:
                        batch_anomaly_rate = result.anomalies_detected / result.batch_size
                ...
                if time_diff > 0:
                    ...
                    if self.metrics.samples_processed > 0:
                        self.metrics.error_rate = (
                            self.metrics.processing_errors / self.metrics.samples_processed
                        )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if len(self._throughput_samples) >= 3:
                    ...
                    target_throughput = self.config.min_throughput_samples_per_second * 2
    ...
    async def _process_batch(self, batch: StreamBatch) -> StreamingResult:
        ...
        try:
            ...
            for i, (sample, score, prediction) in enumerate(zip(batch.samples, scores, predictions)):
    ...
    async def _check_backpressure(self) -> bool:
        ...
        if queue_utilization >= self.config.backpressure_threshold:
            ...
            if self.state != StreamState.BACKPRESSURE:
                self.logger.warning(f"Backpressure threshold reached: {queue_utilization:.2f}")
                self.state = StreamState.BACKPRESSURE
            
            return True
        
        # Check for recovery from backpressure
        if (self.state == StreamState.BACKPRESSURE and 
            queue_utilization <= self.config.backpressure_recovery_threshold):
        ...
        if strategy == BackpressureStrategy.DROP_NEWEST:
            ...
            try:
                if not self.input_queue.empty():
                    old_sample = await asyncio.wait_for(self.input_queue.get(), timeout=0.01)
                    self.metrics.samples_dropped += 1
                    self.logger.debug(f"Dropped old sample {old_sample.id} due to backpressure")
    ...
    async def _detect_drift(self) -> bool:
        ...
        try:
            ...
            if self._reference_distribution is None:
                self._reference_distribution = current_data[:self.config.drift_window_size // 2]
                return False
            ...
            current_mean = np.mean(current_data[-(self.config.drift_window_size // 2):], axis=0)
            current_std = np.std(current_data[-(self.config.drift_window_size // 2):], axis=0)
