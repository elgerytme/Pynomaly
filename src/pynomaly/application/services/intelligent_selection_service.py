...
from pynomaly.application.dto.selection_dto import (
    DatasetCharacteristicsDTO, AlgorithmPerformanceDTO, SelectionRecommendationDTO,
    LearningInsightsDTO, AlgorithmBenchmarkDTO, SelectionHistoryDTO,
    OptimizationConstraintsDTO, MetaLearningConfigDTO
)
...
class IntelligentSelectionService:
    ...
    def __init__(
        ...
        # Storage paths
        self.selection_history_path = selection_history_path or Path("data/selection_history.json")
    ...
    async def recommend_algorithm(
        self,
        dataset: Dataset,
        constraints: Optional[OptimizationConstraintsDTO] = None,
        config: Optional[MetaLearningConfigDTO] = None
        ...
        if self.enable_meta_learning and self.meta_learner is not None:
            meta_rec = await self._meta_learning_recommendation(characteristics, candidates, config)
            recommendations.append(meta_rec)
        
        # Historical similarity recommendation  
        if self.enable_historical_learning:
            historical_rec = await self._historical_similarity_recommendation(characteristics, candidates)
    ...
    async def _extract_dataset_characteristics(self, dataset: Dataset) -> DatasetCharacteristicsDTO:
        ...
        if len(numeric_features) > 0:
            numeric_data = X if len(categorical_features) == 0 else data[numeric_features].values
            
            # Correlation analysis
            if numeric_data.shape[1] > 1:
                corr_matrix = np.corrcoef(numeric_data.T)
                characteristics["mean_feature_correlation"] = np.mean(np.abs(corr_matrix[np.triu_indices_from(corr_matrix, k=1)]))
            
            # Missing values
            characteristics["missing_value_ratio"] = np.isnan(numeric_data).sum() / numeric_data.size
            ...
            characteristics["feature_variance_ratio"] = np.std(variances) / np.mean(variances) if np.mean(variances) > 0 else 0
            
            # Distribution characteristics
            from scipy import stats
            try:
                skewness_values = [stats.skew(numeric_data[:, i][~np.isnan(numeric_data[:, i])]) for i in range(numeric_data.shape[1])]
                kurtosis_values = [stats.kurtosis(numeric_data[:, i][~np.isnan(numeric_data[:, i])]) for i in range(numeric_data.shape[1])]
    ...
    def _filter_algorithm_candidates(
        ...
        constraints: Optional[OptimizationConstraintsDTO]
    ) -> List[str]:
        ...
        for algo_name, algo_info in self.algorithm_registry.items():
            ...
            # Check constraints
            if constraints:
                if constraints.max_memory_mb and algo_info["memory_usage"] > constraints.max_memory_mb:
                    continue
                    
                if constraints.max_training_time_seconds and algo_info["training_time"] > constraints.max_training_time_seconds:
                    continue
    ...
    async def _meta_learning_recommendation(
        ...
        characteristics: DatasetCharacteristicsDTO,
        candidates: List[str],
        config: Optional[MetaLearningConfigDTO]
    ) -> Dict[str, float]:
        """Generate meta-learning based recommendation."""
        if self.meta_learner is None:
            return {algo: 0.0 for algo in candidates}
    ...
    async def _historical_similarity_recommendation(
        ...
        for history in self.selection_history:
            hist_features = self._characteristics_to_features(history.dataset_characteristics)
    ...
    async def _ensemble_recommendations(
        ...
        sorted_algorithms = sorted(ensemble_scores.items(), key=lambda x: x[1], reverse=True)
        ...
        if top_algo:
            reasoning.append(f"Top recommendation: {top_algo} (score: {sorted_algorithms[0][1]:.3f})")
            
            # Add dataset-specific reasoning
            if characteristics.n_samples < 1000:
                reasoning.append("Small dataset: prioritizing algorithms suitable for limited data")
            elif characteristics.n_samples > 100000:
                reasoning.append("Large dataset: prioritizing scalable algorithms")
            
            if characteristics.n_features > 100:
                reasoning.append("High-dimensional data: considering dimensionality reduction")
            
            if characteristics.outlier_ratio > 0.1:
                reasoning.append("High outlier ratio detected: prioritizing robust algorithms")
    ...
    async def _predict_performance(
        ...
        recommendation.selection_context["predicted_performances"] = predicted_performances
        
        return recommendation
    
    def _characteristics_to_features(self, characteristics: DatasetCharacteristicsDTO) -> List[float]:
    ...
    async def _update_meta_models(self) -> None:
        ...
        for history in self.selection_history:
            features = self._characteristics_to_features(history.dataset_characteristics)
            X.append(features)
    ...
    def _analyze_dataset_preferences(self) -> Dict[str, List[str]]:
        ...
        for category in preferences:
            ...
            preferences[category] = sorted(algo_counts.items(), key=lambda x: x[1], reverse=True)[:3]
    ...
    def _analyze_performance_trends(self) -> Dict[str, List[float]]:
        ...
        for i in range(len(sorted_history)):
            ...
            window_performances = [h.performance.primary_metric for h in sorted_history[start_idx:i+1]]
    ...
    async def _analyze_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            feature_names = [
                "n_samples", "n_features", "n_numeric_features", "n_categorical_features",
                "feature_density", "mean_feature_correlation", "data_dimensionality_ratio",
                "missing_value_ratio", "outlier_ratio", "class_imbalance",
                "feature_variance_ratio", "skewness_mean", "kurtosis_mean"
            ]
    ...
    async def _get_meta_model_accuracy(self) -> Optional[float]:
    ...
    def _calculate_recommendation_confidence(self) -> float:
        ...
        if self.selection_history:
            recent_performances = [h.performance.primary_metric for h in self.selection_history[-10:]]
    ...
    async def _load_selection_history(self) -> None:
        ...
        try:
            if self.selection_history_path.exists():
                ...
                logger.info(f"Loaded {len(self.selection_history)} selection history entries")
        except Exception as e:
    ...
    async def _save_selection_history(self) -> None:
        ...
        try:
            ...
            history_data = [
                history.dict() for history in self.selection_history
            ]
            
            with open(self.selection_history_path, 'w') as f:
                json.dump(history_data, f, indent=2, default=str)
                
            logger.info(f"Saved {len(self.selection_history)} selection history entries")
        except Exception as e:
    ...
    async def _save_meta_models(self) -> None:
        ...
        try:
            ...
            if self.performance_predictor is not None:
                with open(self.meta_model_path / "performance_predictor.pkl", 'wb') as f:
