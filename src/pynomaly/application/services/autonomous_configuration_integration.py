...
from pynomaly.application.dto.configuration_dto import (
    ConfigurationCaptureRequestDTO, ConfigurationSource, ConfigurationStatus,
    AlgorithmConfigDTO, DatasetConfigDTO, EvaluationConfigDTO,
    PerformanceResultsDTO, ConfigurationMetadataDTO, EnvironmentConfigDTO,
...
class AutonomousConfigurationIntegration:
    ...
    async def detect_with_configuration_capture(
        ...
        try:
            logger.info(f"Starting autonomous detection with configuration capture")
            ...
            if self.auto_save_successful and self._should_save_configuration(detection_result):
            ...
            if self.save_preprocessing_configs and hasattr(detection_result, 'preprocessing_report'):
            ...
            if self.save_algorithm_selections and hasattr(detection_result, 'algorithm_explanations'):
                await self._save_algorithm_selection_configuration(
                    detection_result, initial_params, capture_metadata
                )
            
            logger.info(f"Autonomous detection completed. Configuration ID: {configuration_id}")
    ...
    async def capture_autonomous_experiment(
        self,
        experiment_name: str,
        data_sources: List[Union[str, Path, pd.DataFrame]],
        configs: List[AutonomousConfig],
        experiment_metadata: Optional[Dict[str, Any]] = None
    ) -> List[Tuple[DetectionResult, Optional[str]]]:
        ...
        logger.info(f"Starting autonomous experiment '{experiment_name}' with {len(data_sources)} datasets and {len(configs)} configurations")
        ...
        for i, data_source in enumerate(data_sources):
            for j, config in enumerate(configs):
                try:
                    ...
                    logger.warning(f"Experiment run failed for dataset {i}, config {j}: {e}")
                    results.append((None, None))
        
        logger.info(f"Autonomous experiment '{experiment_name}' completed with {len([r for r in results if r[0] is not None])} successful runs")
        return results
    ...
    async def analyze_autonomous_configurations(
        ...
        from pynomaly.application.dto.configuration_dto import ConfigurationSearchRequestDTO
        ...
        configurations = await self.configuration_service.search_configurations(search_request)
        
        if not configurations:
            return {"message": "No autonomous configurations found", "configurations": 0}
        ...
        for config in configurations:
            ...
            if config.preprocessing_config:
                ...
                preprocessing_patterns[strategy] = preprocessing_patterns.get(strategy, 0) + 1
        ...
        if performance_distribution:
            avg_performance = sum(performance_distribution) / len(performance_distribution)
        ...
        return {
            ...
            "most_used_preprocessing": max(preprocessing_patterns.items(), key=lambda x: x[1])[0] if preprocessing_patterns else None,
    ...
    def get_integration_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "autonomous_service_info": {
                "available_algorithms": len(getattr(self.autonomous_service, 'algorithm_adapters', {})),
                "data_loaders": list(getattr(self.autonomous_service, 'data_loaders', {}).keys()),
                "preprocessing_enabled": getattr(self.autonomous_service, 'preprocessing_orchestrator', None) is not None
            },
            "capture_settings": {
    ...
    def _extract_initial_parameters(
        ...
        if isinstance(data_source, (str, Path)):
            ...
            params["dataset_name"] = f"dataframe_{data_source.shape[0]}x{data_source.shape[1]}"
    ...
    def _get_data_source_type(self, data_source: Union[str, Path, pd.DataFrame]) -> str:
    ...
    def _should_save_configuration(self, detection_result: DetectionResult) -> bool:
        ...
        if (hasattr(detection_result, 'anomalies') and 
            len(detection_result.anomalies) > 0 and
            len(detection_result.anomalies) < len(detection_result.scores) * 0.5):  # Not too many anomalies
            return True
    ...
    def _calculate_performance_score(self, detection_result: DetectionResult) -> float:
        ...
        if hasattr(detection_result, 'processing_time') and detection_result.processing_time > 0:
    ...
    async def _capture_autonomous_configuration(
        ...
        try:
            ...
            capture_request = ConfigurationCaptureRequestDTO(
                ...
                execution_results=performance_results.model_dump() if performance_results else None,
                source_context={
                    "autonomous_config": config.__dict__,
                    "detection_summary": self._create_detection_summary(detection_result),
                    "performance_score": self._calculate_performance_score(detection_result),
                    **(capture_metadata or {})
                },
                auto_save=True,
            ...
            response = await self.configuration_service.capture_configuration(capture_request)
            ...
            if response.success:
                ...
                logger.error(f"Autonomous configuration capture failed: {response.message}")
                return None
    ...
    def _extract_performance_results(
        ...
        detection_result: DetectionResult,
        detection_duration: float
    ) -> Optional[PerformanceResultsDTO]:
        ...
        return PerformanceResultsDTO(
            ...
            anomaly_scores=detection_result.scores.tolist() if hasattr(detection_result.scores, 'tolist') else None,
            ...
            interpretability_score=getattr(detection_result, 'interpretability_score', None)
        )
    
    def _create_detection_summary(self, detection_result: DetectionResult) -> Dict[str, Any]:
        """Create summary of detection results."""
        if not detection_result:
            return {}
        
        return {
            "n_samples": len(detection_result.scores) if detection_result.scores is not None else 0,
            "n_anomalies": detection_result.n_anomalies,
            "anomaly_rate": detection_result.anomaly_rate,
            "detector_algorithm": detection_result.detector_name if hasattr(detection_result, 'detector_name') else None,
            "timestamp": detection_result.timestamp.isoformat(),
            "has_explanations": hasattr(detection_result, 'explanations') and detection_result.explanations is not None,
        ...
        try:
            ...
            selection_params = {
                **initial_params,
                ...
                "selected_algorithms": getattr(detection_result, 'selected_algorithms', []),
...
class AutonomousConfigurationManager:
    ...
    async def recommend_autonomous_config(
        ...
        search_request = ConfigurationSearchRequestDTO(
            source=ConfigurationSource.AUTONOMOUS,
            tags=["successful"],
            min_accuracy=performance_requirements.get("min_accuracy", 0.7) if performance_requirements else 0.7,
            limit=50,
            sort_by="accuracy",
            sort_order="desc"
        )
        ...
        for config in configurations:
            if config.source_context:
                ...
                confidence_thresholds.append(autonomous_config.get("confidence_threshold", 0.8))
                max_algorithms_values.append(autonomous_config.get("max_algorithms", 5))
                preprocessing_strategies.append(autonomous_config.get("preprocessing_strategy", "auto"))
        
        # Calculate optimal values
        recommended_config = AutonomousConfig()
        
        if confidence_thresholds:
            recommended_config.confidence_threshold = sum(confidence_thresholds) / len(confidence_thresholds)
        
        if max_algorithms_values:
            # Use mode for max_algorithms
            from collections import Counter
            recommended_config.max_algorithms = Counter(max_algorithms_values).most_common(1)[0][0]
        
        if preprocessing_strategies:
            # Use most common strategy
            from collections import Counter
            recommended_config.preprocessing_strategy = Counter(preprocessing_strategies).most_common(1)[0][0]
    ...
    async def export_autonomous_configurations(
        ...
        """
        from pynomaly.application.dto.configuration_dto import (
            ConfigurationExportRequestDTO, ExportFormat
        )
        ...
        response = await self.configuration_service.export_configurations(export_request)
