...
from pynomaly.domain.entities.explainable_ai import (
    ExplanationMethod, ExplanationType, ExplanationScope, FeatureImportance,
    ModelExplanation, InstanceExplanation, GlobalExplanation, ExplanationRequest,
    ExplanationResult, ExplanationMetadata, TrustScore, BiasAnalysis
)
...
class ExplanationCache:
    ...
    def is_expired(self) -> bool:
        ...
        expiration_time = self.creation_timestamp + timedelta(hours=self.expiration_hours)
        return datetime.utcnow() > expiration_time
    
    def get_explanation(self, key: str) -> Optional[Any]:
...
class ExplainableAIService:
    ...
    def __init__(
        self,
        storage_path: Path,
        default_config: Optional[ExplanationConfiguration] = None
    ):
    ...
    async def _initialize_explanation_methods(self) -> None:
        ...
        try:
            if SHAP_AVAILABLE:
                self.explanation_generators[ExplanationMethod.SHAP_TREE] = SHAPExplainer()
                self.explanation_generators[ExplanationMethod.SHAP_KERNEL] = SHAPKernelExplainer()
                self.explanation_generators[ExplanationMethod.SHAP_DEEP] = SHAPDeepExplainer()
                self.explanation_generators[ExplanationMethod.SHAP_LINEAR] = SHAPLinearExplainer()
            ...
            self.explanation_generators[ExplanationMethod.PERMUTATION_IMPORTANCE] = PermutationImportanceExplainer()
            self.explanation_generators[ExplanationMethod.FEATURE_ABLATION] = FeatureAblationExplainer()
    ...
    async def explain_prediction(
        ...
        config: Optional[ExplanationConfiguration] = None
    ) -> ExplanationResult:
        ...
        try:
            ...
            if config.cache_explanations:
                ...
                if cached_result:
                    logger.debug(f"Retrieved cached explanation for instance")
            ...
            logger.info(f"Generated explanation for instance using {config.explanation_method.value}")
    ...
    async def explain_model_global(
        ...
        try:
            ...
            if training_data is not None and len(training_data) > config.background_sample_size:
            ...
            logger.info(f"Generated global explanation using {config.explanation_method.value}")
        ...
        try:
            ...
            for i in range(num_counterfactuals):
                ...
                if cf_prediction != original_prediction:
                    ...
                    changes = {}
                    for j, (orig, cf) in enumerate(zip(instance, counterfactual)):
    ...
    async def assess_explanation_trust(
        ...
        try:
            ...
            trust_score = TrustScore(
                ...
                confidence_interval=(max(0.0, overall_trust - 0.1), min(1.0, overall_trust + 0.1)),
                trust_level="high" if overall_trust > 0.8 else "medium" if overall_trust > 0.6 else "low"
            )
            
            logger.info(f"Assessed explanation trust: {trust_score.trust_level} ({overall_trust:.3f})")
            return trust_score
        ...
        try:
            ...
            if model_id in self.explanation_cache:
                ...
                if cache.creation_timestamp >= cutoff_time:
                    summary["explanation_stats"]["total_explanations"] = cache.access_count
                    summary["explanation_stats"]["cache_hit_rate"] = min(1.0, cache.access_count / 100)
    ...
    def _generate_cache_key(self, request: ExplanationRequest, instance: np.ndarray) -> str:
        """Generate cache key for explanation."""
        instance_hash = hash(instance.tobytes())
        return f"{request.explanation_method.value}_{instance_hash}"
    
    def _get_cached_explanation(self, model_id: UUID, cache_key: str) -> Optional[ExplanationResult]:
        """Get cached explanation."""
        if model_id in self.explanation_cache:
            return self.explanation_cache[model_id].get_explanation(cache_key)
        return None
    
    def _cache_explanation(self, model_id: UUID, cache_key: str, result: ExplanationResult) -> None:
    ...
    def _calculate_group_bias_score(self, group_importances: Dict[str, List[FeatureImportance]]) -> float:
...
class SHAPExplainer:
    ...
    async def explain_global(
        ...
        try:
            ...
            global_importances = []
            for i, (name, importance) in enumerate(zip(feature_names, mean_importance)):
...
class PermutationImportanceExplainer:
    ...
    async def get_feature_importance(
        ...
        try:
            ...
            importances = []
            for i, (name, importance, std) in enumerate(zip(
                feature_names, perm_importance.importances_mean, perm_importance.importances_std
            )):
                feature_importance = FeatureImportance(
                    feature_name=name,
                    importance_value=float(importance),
