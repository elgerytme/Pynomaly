...
from pynomaly.domain.entities import Model, ModelVersion, ModelType, ModelStage
from pynomaly.domain.exceptions import ModelNotFoundError, InvalidModelStateError
from pynomaly.domain.value_objects import SemanticVersion, PerformanceMetrics, ModelStorageInfo
from pynomaly.shared.protocols import ModelRepositoryProtocol, ModelVersionRepositoryProtocol
...
class ModelManagementService:
    ...
    async def create_model_version(
        ...
        existing_version = await self.model_version_repository.find_by_model_and_version(
            model_id, version
        )
        if existing_version:
            raise ValueError(f"Version {version.version_string} already exists for model {model_id}")
    ...
    async def promote_to_production(
        ...
        if model.current_version_id:
            ...
            if current_version and current_version.is_deployed:
                ...
                current_version.update_metadata("demoted_at", datetime.utcnow().isoformat())
        ...
        if validation_results:
            model.update_metadata("last_validation_results", validation_results, promoted_by)
