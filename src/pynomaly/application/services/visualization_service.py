...
class VisualizationService:
    ...
    async def create_anomaly_scatter_plot(
        ...
        try:
            ...
            return VisualizationResult(
                plot_id=plot_id,
                plot_type=VisualizationType.SCATTER_3D if use_3d else VisualizationType.SCATTER_2D,
                title=title,
                ...
                metadata={
                    ...
                    "n_anomalies": int(np.sum(anomaly_scores > np.median(anomaly_scores))),
                },
            )
        ...
        try:
            if PLOTLY_AVAILABLE:
                ...
                if anomaly_indices:
                    ...
                    anomaly_vals = [values[i] for i in anomaly_indices if i < len(values)]
                ...
                if anomaly_scores is not None:
                    ...
                    # Add anomaly scores
                    fig_subplots.add_trace(
                        go.Scatter(x=timestamps, y=anomaly_scores, name='Anomaly Scores'),
                        row=2, col=1
                    )
    ...
    async def create_correlation_heatmap(
        ...
        try:
            ...
            return VisualizationResult(
                ...
                metadata={
                    "n_features": len(feature_names),
                    "max_correlation": float(np.max(correlation_matrix[correlation_matrix < 1])),
        ...
        try:
            if PLOTLY_AVAILABLE:
                fig = make_subplots(
                    rows=2, cols=2,
                    subplot_titles=[
                        'Score Distributions', 'Method Correlations',
                        'Performance Comparison', 'Score Scatter Matrix'
                    ],
                ...
                if len(ensemble_results) > 1:
                    ...
                    scores_matrix = np.array([ensemble_results[method] for method in methods])
    ...
    async def create_dashboard(
        ...
        try:
            ...
            fig = make_subplots(
                rows=3, cols=3,
                subplot_titles=[
                    'Detection Overview', 'Score Distribution', 'Time Series',
                    'Detector Performance', 'Dataset Summary', 'Alert Trends',
                    'Resource Usage', 'Model Health', 'Recent Activity'
                ],
                specs=[
            ...
            anomaly_rate = total_anomalies / total_detections if total_detections > 0 else 0
            ...
            all_scores = [max(r.scores, key=lambda s: s.value).value for r in detection_results if r.scores]
            if all_scores:
                fig.add_trace(
            ...
            for detector in detectors:
                detector_results = [r for r in detection_results if r.detector_id == detector.id]
                if detector_results:
                    avg_score = np.mean([max(r.scores, key=lambda s: s.value).value for r in detector_results if r.scores])
            ...
            # 5. Dataset Summary
            dataset_sizes = [len(d.data) if hasattr(d, 'data') and d.data is not None else 0 for d in datasets]
            ...
            for r in detection_results[-10:]:  # Last 10 results
                recent_activity.append([
                    ...
                    f"{max(r.scores, key=lambda s: s.value).value:.3f}" if r.scores else "N/A",
                ])
    ...
    async def _prepare_scatter_data(
        ...
        if data.shape[1] > (3 if use_3d else 2):
            if SKLEARN_AVAILABLE:
                ...
                data_reduced = reducer.fit_transform(StandardScaler().fit_transform(data))
            else:
        ...
        plot_data.hover_text = [
            ...
            for score, label in zip(anomaly_scores, labels if labels is not None else [None] * len(anomaly_scores))
        ]
    ...
    async def save_visualization(
        ...
        if filename is None:
            ...
            filename = f"{result.plot_type.value}_{timestamp}.{self.config.output_format}"
    ...
    async def get_cached_visualization(self, cache_key: str) -> Optional[VisualizationResult]:
