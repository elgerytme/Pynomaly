...
class AutoMLConfigurationIntegration:
    ...
    async def optimize_with_configuration_capture(
    ...
    ) -> Tuple[Detector, Dict[str, Any], Optional[str]]:
        ...
        try:
            logger.info(f"Starting AutoML optimization with configuration capture for {algorithm_name}")
            
            # Run AutoML optimization
            optimized_detector, optimization_report = await self.automl_service.optimize(
                dataset=dataset,
            ...
            logger.info(f"AutoML optimization completed successfully. Configuration ID: {configuration_id}")
            ...
            if self.auto_save_failed:
                try:
                    ...
                    logger.error(f"Failed to capture configuration for failed optimization: {capture_error}")
    ...
    async def capture_manual_configuration(
        ...
        try:
            ...
            optimization_params = {
                "dataset_path": getattr(dataset, 'path', None) or f"dataset_{dataset.name}",
            ...
            logger.info(f"Manual configuration captured. Configuration ID: {configuration_id}")
        ...
        try:
            for i, result in enumerate(optimization_results):
                try:
                    ...
                    logger.warning(f"Failed to capture configuration for batch item {i}: {e}")
                    continue
            
            logger.info(f"Batch configuration capture completed. Captured {len(configuration_ids)} configurations")
    ...
    def get_integration_statistics(self) -> Dict[str, Any]:
        ...
        """
        config_stats = asyncio.run(self.configuration_service.get_configuration_statistics())
        
        return {
            "integration_stats": self.integration_stats,
            "configuration_service_stats": config_stats,
            "automl_service_info": {
                "available_algorithms": len(getattr(self.automl_service, 'algorithm_configs', {})),
                "optimization_history_size": len(getattr(self.automl_service, 'optimization_history', [])),
                "enable_distributed": getattr(self.automl_service, 'enable_distributed', False),
    ...
    def _extract_optimization_parameters(
        ...
        params = {
            ...
            "dataset_name": dataset.name,
            "dataset_shape": dataset.data.shape if hasattr(dataset.data, 'shape') else None,
            "algorithm": algorithm_name,
            "objectives": [obj.name if hasattr(obj, 'name') else str(obj) for obj in (objectives or [])],
            "enable_learning": enable_learning,
    ...
    async def _capture_optimization_configuration(
        ...
        performance_results: Optional[PerformanceResultsDTO],
        optimization_report: Dict[str, Any],
        optimized_detector: Optional[Detector],
        success: bool,
        capture_config: Optional[Dict[str, Any]] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            capture_request = ConfigurationCaptureRequestDTO(
                ...
                source_context={
                    "optimization_report": optimization_report,
                    "success": success,
                    "detector_algorithm": optimized_detector.algorithm_name if optimized_detector else None,
                    "detector_params": getattr(optimized_detector, 'params', {}) if optimized_detector else {},
                    **(capture_config or {})
                },
                auto_save=True,
                generate_name=True,
                tags=["automl", "optimization", optimization_params.get("algorithm", "unknown")]
            )
    ...
    async def _save_optimization_history(
        ...
        try:
            history_entry = {
                ...
                "optimization_params": optimization_params,
                "performance_results": performance_results.model_dump() if performance_results else None,
            ...
            logger.debug(f"Optimization history saved for configuration {configuration_id}")
...
class AutoMLConfigurationManager:
        ...
        if max_training_time:
            configurations = [
                ...
                if (config.performance_results and 
                    config.performance_results.training_time_seconds and
                    config.performance_results.training_time_seconds <= max_training_time)
            ]
    ...
    async def analyze_automl_performance_trends(self) -> Dict[str, Any]:
        ...
        for config in configs:
            algorithm = config.get("algorithm_config", {}).get("algorithm_name", "unknown")
            if algorithm not in algorithm_performance:
                algorithm_performance[algorithm] = []
            
            if config.get("performance_results", {}).get("accuracy"):
                algorithm_performance[algorithm].append(config["performance_results"]["accuracy"])
        ...
        if not config_df.empty and "metadata" in config_df.columns:
            ...
            weekly_stats = config_df.groupby("week").agg({
                "performance_results": lambda x: [r.get("accuracy") for r in x if r and r.get("accuracy")]
            }).to_dict()
        else:
            weekly_stats = {}
