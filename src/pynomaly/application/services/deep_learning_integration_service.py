...
try:
    from pynomaly.infrastructure.adapters.deep_learning.pytorch_adapter import PyTorchAdapter, PYTORCH_AVAILABLE
except ImportError:
    PyTorchAdapter = None
    PYTORCH_AVAILABLE = False

try:
    from pynomaly.infrastructure.adapters.deep_learning.tensorflow_adapter import TensorFlowAdapter, TENSORFLOW_AVAILABLE
except ImportError:
    TensorFlowAdapter = None
    TENSORFLOW_AVAILABLE = False

try:
    from pynomaly.infrastructure.adapters.deep_learning.jax_adapter import JAXAdapter, JAX_AVAILABLE
except ImportError:
    JAXAdapter = None
    JAX_AVAILABLE = False
...
class DLOptimizationConfig(BaseModel):
    ...
    target_framework: Optional[str] = Field(None, description="Preferred framework")
    performance_priority: str = Field(default="balanced", description="Performance priority")
    resource_constraints: Dict[str, Any] = Field(default_factory=dict, description="Resource constraints")
    optimization_objectives: List[str] = Field(default=["accuracy", "speed"], description="Optimization objectives")
    auto_select_framework: bool = Field(default=True, description="Auto-select best framework")
...
class DeepLearningIntegrationService:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized DeepLearningIntegrationService with {len(self.available_frameworks)} frameworks")
    ...
    def get_framework_capabilities(self, framework: str) -> Optional[DLFrameworkInfo]:
    ...
    def recommend_framework(
        ...
        dataset: Dataset,
        algorithm: str,
        config: Optional[DLOptimizationConfig] = None
    ) -> str:
        ...
        if config.target_framework and config.target_framework in self.available_frameworks:
    ...
    def _auto_select_framework(
        ...
        logger.info(f"Auto-selected framework: {best_framework} (score: {scores[best_framework]:.3f})")
    ...
    async def create_deep_learning_detector(
        ...
        optimization_config: Optional[DLOptimizationConfig] = None
    ) -> DetectorProtocol:
        """Create deep learning detector with optimal framework."""
        try:
            # Recommend framework if not specified
            if not framework:
                framework = self.recommend_framework(dataset, algorithm, optimization_config)
    ...
    async def _create_framework_detector(
        ...
        optimization_config: Optional[DLOptimizationConfig]
    ) -> DetectorProtocol:
    ...
    async def benchmark_frameworks(
        ...
        if not frameworks:
            frameworks = [
                name for name, info in self.available_frameworks.items()
        ...
        for framework in frameworks:
            try:
                ...
                inference_time = (end_time - start_time).total_seconds() / len(sample_data)
                ...
                logger.info(f"Completed {framework} benchmark: {training_time:.2f}s training")
    ...
    def get_performance_recommendations(
        ...
        if hasattr(dataset.data, 'shape'):
            ...
            if n_samples > 100000:
                ...
                recommendations["primary_framework"] = "tensorflow"  # Good for large scale
            ...
            if n_samples < 1000:
                ...
                recommendations["primary_framework"] = "pytorch"  # Good for experimentation
    ...
    def update_framework_preferences(self, performance_results: List[DLModelPerformance]):
        ...
        for result in performance_results:
            ...
            score = (1.0 / (result.training_time + 1.0)) + (1.0 / (result.inference_time + 1.0))
