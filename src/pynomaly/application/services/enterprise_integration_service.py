...
class EnterpriseIntegrationService:
    ...
    def __init__(self, config: IntegrationConfig = None):
        ...
        # Core services
        self.dashboard_service: Optional[EnterpriseDashboardService] = None
        self.alerting_engine: Optional[IntelligentAlertingEngine] = None
        self.autonomous_service: Optional[AutonomousDetectionService] = None
    ...
    async def _setup_notification_providers(self):
        ...
        # Email notifications
        if (self.config.email_notifications_enabled and 
    ...
    async def _alerting_monitoring_loop(self):
        ...
        while self.integration_active:
            try:
                ...
                self.integration_metrics["alerts_generated"] = stats.get("alerts_last_hour", 0)
                
                # Check for alerting system health issues
                if stats.get("active_alerts", 0) > 50:
                    self.logger.warning(f"High number of active alerts: {stats['active_alerts']}")
    ...
    async def process_autonomous_detection_event(
        ...
        try:
            ...
            self.logger.error(f"Failed to process autonomous detection event {detection_id}: {e}")
        ...
        try:
            ...
            if self.dashboard_service:
                ...
                alert_id = self.dashboard_service.create_alert(
                    ...
                    metric_type=metric_type_map.get(category, DashboardMetricType.OPERATIONAL),
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "notification_providers": (
                ...
                if self.alerting_engine else 0
            )
        }
...
_global_integration_service: Optional[EnterpriseIntegrationService] = None


def get_integration_service() -> Optional[EnterpriseIntegrationService]:
    """Get the global integration service instance."""
    return _global_integration_service


async def initialize_enterprise_integration(config: IntegrationConfig = None) -> EnterpriseIntegrationService:
