...
class DataPreprocessingService:
        ...
        try:
            ...
            if enable_quality_assessment:
                ...
                if result.quality_report.overall_quality_score < 0.3:
                    ...
                    result.error_message = "Data quality too low for reliable processing"
                    return result
    ...
    async def _assess_data_quality(
        ...
        outlier_ratio = outlier_count / (n_samples * len(numerical_cols)) if numerical_cols.size > 0 else 0
    ...
    async def _engineer_features(
        ...
        if target is not None and max_features is not None and len(original_features) > max_features:
            try:
                # Use SelectKBest for feature selection
                selector = SelectKBest(f_classif, k=min(max_features, len(original_features)))
                selected_data = selector.fit_transform(engineered_data, target)

                # Get selected feature names
                selected_mask = selector.get_support()
                selected_features = [
                    feature for feature, selected in zip(original_features, selected_mask, strict=False)
                    if selected
                ]
                ...
                feature_importance = {
                    feature: score for feature, score in zip(original_features, feature_scores, strict=False)
                ...
                selected_features = original_features[:max_features] if max_features else original_features
        ...
        dimensionality_reduction = (
            (len(original_features) - len(selected_features)) / len(original_features) * 100
            if len(original_features) > 0 else 0
        )
    ...
    def get_preprocessing_summary(
        ...
        if result.feature_engineering_result:
            summary["dimensionality_reduction"] = result.feature_engineering_result.dimensionality_reduction
            summary["features_selected"] = len(result.feature_engineering_result.selected_features)
            summary["feature_engineering_time"] = result.feature_engineering_result.processing_time_seconds
