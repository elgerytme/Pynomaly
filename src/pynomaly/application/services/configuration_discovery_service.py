...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationSearchRequestDTO, ConfigurationSource,
    ConfigurationResponseDTO, ConfigurationCollectionDTO, ConfigurationMetadataDTO,
    ConfigurationStatus, ConfigurationLevel
)
...
class ConfigurationDiscoveryService:
    ...
    def __init__(
        ...
        self._clusters: Optional[Dict[int, List[UUID]]] = None
    ...
    async def advanced_search(
        ...
        if similarity_search and self.enable_similarity_analysis:
            ...
            similarity_results = await self._similarity_search(query, filters, max_results // 4)
        ...
        if include_recommendations and configurations:
            ...
            recommendations = await self._generate_search_recommendations(configurations, query)
        
        logger.info(f"Advanced search completed: {len(configurations)} results, {len(recommendations)} recommendations")
    ...
    async def find_similar_configurations(
        ...
        if not reference_config:
            reference_config = await self.repository.load_configuration(reference_config_id)
            if not reference_config:
                return []
    ...
    async def auto_generate_tags(
        ...
        logger.info(f"Auto-generating tags for {len(configs_to_process)} configurations")
        ...
        for config in configs_to_process:
            ...
            if tags:
                ...
                config.metadata.tags = list(set(config.metadata.tags))  # Remove duplicates
    ...
    async def create_smart_collection(
        ...
        collection = ConfigurationCollectionDTO(
            ...
            average_performance=self._calculate_collection_average_performance(matching_configs)
        )
        ...
        logger.info(f"Smart collection '{collection_name}' created with {len(matching_configs)} configurations")
        return collection
    ...
    async def cluster_configurations(
        ...
        feature_matrix = self._extract_clustering_features(configurations, feature_weights)
    ...
    async def get_configuration_trends(
        ...
        recent_configs = [
            config for config in self._configuration_cache.values()
            if config.metadata.created_at >= cutoff_date
        ]
        
        logger.info(f"Analyzing trends for {len(recent_configs)} configurations over {time_period_days} days")
        ...
        for group_name, group_configs in groups.items():
            ...
            if group_configs and group_configs[0].performance_results:
                sorted_configs = sorted(group_configs, key=lambda c: c.metadata.created_at)
            ...
            trends["trends"][group_name] = {
                ...
                "average_performance": sum(performance_trend) / len(performance_trend) if performance_trend else 0,
                "latest_configs": [c.id for c in sorted(group_configs, key=lambda c: c.metadata.created_at, reverse=True)[:5]]
            }
        
        # Summary statistics
        trends["summary"] = {
            "most_active_group": max(groups.keys(), key=lambda k: len(groups[k])) if groups else None,
            "total_groups": len(groups),
            "average_group_size": sum(len(configs) for configs in groups.values()) / len(groups) if groups else 0,
    ...
    async def _rebuild_cache(self) -> None:
        ...
        logger.debug(f"Cache rebuilt with {len(self._configuration_cache)} configurations")
    ...
    async def _keyword_search(
        ...
        for config in results:
            ...
            # Description match
            if config.metadata.description and query_lower in config.metadata.description.lower():
                score += 0.2
    ...
    async def _tag_based_search(
        ...
        for config_id in list(config_ids)[:max_results]:
            ...
            if config:
                ...
                for tag in config.metadata.tags:
                    if query_lower in tag.lower():
                        score += 1.0 / len(config.metadata.tags)  # Normalize by number of tags
    ...
    def _deduplicate_and_score_results(
        self,
        results: List[Tuple[ExperimentConfigurationDTO, float]],
        query: str
    ) -> List[Tuple[ExperimentConfigurationDTO, float]]:
    ...
    async def _generate_search_recommendations(
        self,
        search_results: List[ExperimentConfigurationDTO],
        query: str
    ) -> List[Dict[str, Any]]:
        ...
        algorithms = Counter(config.algorithm_config.algorithm_name for config in search_results)
        ...
        high_perf_configs = [
            config for config in search_results
            if config.performance_results and 
               config.performance_results.accuracy and 
               config.performance_results.accuracy > 0.85
        ]
    ...
    async def _calculate_configuration_similarity(
        ...
        if config1.algorithm_config.hyperparameters and config2.algorithm_config.hyperparameters:
        ...
        if include_performance and config1.performance_results and config2.performance_results:
            if config1.performance_results.accuracy and config2.performance_results.accuracy:
                perf_diff = abs(config1.performance_results.accuracy - config2.performance_results.accuracy)
    ...
    def _generate_tags_for_configuration(self, config: ExperimentConfigurationDTO) -> List[str]:
    ...
    async def _find_configurations_by_criteria(self, criteria: Dict[str, Any]) -> List[ExperimentConfigurationDTO]:
    ...
    def _extract_common_tags(self, configurations: List[ExperimentConfigurationDTO]) -> List[str]:
    ...
    def _find_common_tags(self, configurations: List[ExperimentConfigurationDTO]) -> List[str]:
        """Find common tags across configurations (alias for _extract_common_tags)."""
        return self._extract_common_tags(configurations)
    
    def _determine_collection_category(self, criteria: Dict[str, Any]) -> Optional[str]:
    ...
    def _calculate_collection_success_rate(self, configurations: List[ExperimentConfigurationDTO]) -> Optional[float]:
        ...
        successful_configs = [
            ...
            if config.performance_results and 
               config.performance_results.accuracy and 
               config.performance_results.accuracy > 0.7
        ]
        
        return len(successful_configs) / len(configurations)
    
    def _calculate_collection_average_performance(self, configurations: List[ExperimentConfigurationDTO]) -> Optional[float]:
    ...
    def _extract_clustering_features(
        ...
        all_algorithms = list(set(config.algorithm_config.algorithm_name for config in configurations))
        ...
        # Get all unique tags
        all_tags = list(set(tag for config in configurations for tag in config.metadata.tags))
        ...
        for config in configurations:
            ...
            if config.performance_results and config.performance_results.accuracy:
                feature_vector.append(config.performance_results.accuracy * feature_weights["performance"])
            else:
                feature_vector.append(0.0)
            
            # Hyperparameter count (normalized)
            param_count = len(config.algorithm_config.hyperparameters) if config.algorithm_config.hyperparameters else 0
            feature_vector.append((param_count / 20.0) * feature_weights["hyperparameters"])  # Normalize by max expected params
    ...
    def _calculate_trend_direction(self, configurations: List[ExperimentConfigurationDTO]) -> str:
