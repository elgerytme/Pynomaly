...
class AutomatedTrainingService:
    ...
    async def schedule_training(
        ...
        logger.info(f"Scheduling training {training_id} for detector {config.detector_id}")
    ...
    async def get_training_history(
        ...
        results.sort(
            key=lambda x: x.completion_time or datetime.min,
            reverse=True
        )
    ...
    async def update_performance(self, detector_id: UUID, score: float):
        ...
        self.performance_history[detector_id] = [
            (ts, s) for ts, s in self.performance_history[detector_id]
            if ts > cutoff
        ]
        
        # Check if retraining is needed
        if await self.check_retraining_needed(detector_id):
            logger.info(f"Performance degradation detected for detector {detector_id}, scheduling retraining")
    ...
    async def _execute_training(self, training_id: str, trigger_type: TriggerType):
        ...
        try:
            ...
            if result.previous_score and result.best_score:
                result.performance_improvement = result.best_score - result.previous_score
            ...
            result.training_time_seconds = (result.completion_time - start_time).total_seconds()
                ...
                for detector_id in list(self.performance_history.keys()):
                    if await self.check_retraining_needed(detector_id):
                        ...
                        active_detector_trainings = [
                            t for t in self.active_trainings.values()
                            if self.scheduled_trainings.get(t.training_id, {}).get('detector_id') == detector_id
                            and t.status in [TrainingStatus.RUNNING, TrainingStatus.SCHEDULED]
                        ]
