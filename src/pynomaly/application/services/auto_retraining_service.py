...
from pynomaly.domain.entities.continuous_learning import (
    PerformanceDelta, EvolutionTrigger, ModelEvolution, KnowledgeTransferMetrics
)
from pynomaly.domain.entities.drift_detection import DriftEvent, DriftSeverity
from pynomaly.application.services.continuous_learning_service import ContinuousLearningService
...
class RetrainingPlan:
    ...
    def get_execution_steps(self) -> List[str]:
        ...
        return [
            ...
            "hyperparameter_optimization" if self.hyperparameter_optimization else "skip_hyperopt",
...
class RetrainingResult:
    ...
    plan_id: UUID
    success: bool
    retrained_model_id: Optional[UUID] = None
    performance_improvement: Optional[PerformanceDelta] = None
    ...
    champion_challenger_results: Optional[Dict[str, Any]] = None
...
class CuratedDataset:
    ...
    curation_metrics: Optional[Dict[str, Any]] = None
    ...
    def get_dataset_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_quality": float(np.mean(self.quality_scores)) if self.quality_scores is not None else 0.0,
...
class AutoRetrainingService:
    ...
    def __init__(
        ...
        storage_path: Path,
        default_success_criteria: Optional[Dict[str, float]] = None
    ):
    ...
    async def evaluate_retraining_necessity(
        ...
        try:
            ...
            critical_drift_count = sum(1 for event in drift_events 
                                     if event.severity == DriftSeverity.CRITICAL)
            high_drift_count = sum(1 for event in drift_events 
                                 if event.severity == DriftSeverity.HIGH)
            
            if critical_drift_count > 0:
                triggering_factors.append(f"{critical_drift_count} critical drift events")
                confidence_factors.append(0.85)
                primary_trigger = EvolutionTrigger.DRIFT_DETECTION
            elif high_drift_count > 2:
                triggering_factors.append(f"{high_drift_count} high-severity drift events")
                confidence_factors.append(0.6)
            
            # Evaluate data volume (simplified check)
            data_volume_score = await self._evaluate_data_volume_for_retraining(model_id)
            if data_volume_score > 0.8:
                triggering_factors.append("Sufficient new data available")
                confidence_factors.append(0.5)
            
            # Calculate overall confidence
            overall_confidence = float(np.mean(confidence_factors)) if confidence_factors else 0.0
            
            # Determine if retraining is recommended
            should_retrain = (
                len(triggering_factors) > 0 and 
            ...
            logger.info(f"Retraining evaluation for model {model_id}: "
                       f"recommend={should_retrain}, confidence={overall_confidence:.3f}")
            
            return decision
            
        except Exception as e:
            logger.error(f"Failed to evaluate retraining necessity for model {model_id}: {e}")
    ...
    async def execute_smart_retraining(
        ...
        try:
            ...
            logger.info(f"Step 6: Running champion/challenger comparison for plan {plan_id}")
            ...
            logger.info(f"Completed smart retraining for plan {plan_id}: "
    ...
    def _evaluate_success_criteria(
        ...
        return (performance_improvement.is_improvement() and 
                performance_improvement.overall_improvement >= min_improvement and
