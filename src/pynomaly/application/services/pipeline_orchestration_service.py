...
from pynomaly.domain.entities import Pipeline, PipelineStep, PipelineRun, PipelineType, PipelineStatus, StepType
from pynomaly.domain.exceptions import PipelineNotFoundError, InvalidPipelineStateError
from pynomaly.shared.protocols import PipelineRepositoryProtocol, PipelineRunRepositoryProtocol
...
class PipelineOrchestrationService:
    ...
    async def create_pipeline(
        ...
        existing_pipelines = await self.pipeline_repository.find_by_name_and_environment(name, environment)
        if existing_pipelines:
            raise ValueError(f"Pipeline with name '{name}' already exists in {environment}")
    ...
    async def get_active_pipelines(self, environment: Optional[str] = None) -> List[Pipeline]:
        ...
        """
        all_pipelines = await self.pipeline_repository.find_by_status(PipelineStatus.ACTIVE)
    ...
    async def get_scheduled_pipelines(self) -> List[Pipeline]:
        ...
        return [p for p in all_pipelines if p.is_scheduled and p.status == PipelineStatus.ACTIVE]
    ...
    def _calculate_average_duration(self, runs: List[PipelineRun]) -> Optional[float]:
    ...
    def _analyze_step_failures(self, runs: List[PipelineRun]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_problematic_step": max(step_failures.items(), key=lambda x: x[1])[0] if step_failures else None
        }
    ...
    def _calculate_performance_trend(self, runs: List[PipelineRun]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_performance": "good" if recent_success_rate > 0.8 else "poor" if recent_success_rate < 0.5 else "moderate"
        }
    
    def _get_last_successful_run(self, runs: List[PipelineRun]) -> Optional[Dict[str, Any]]:
    ...
    def _get_last_failed_run(self, runs: List[PipelineRun]) -> Optional[Dict[str, Any]]:
