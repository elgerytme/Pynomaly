...
from pynomaly.domain.entities.deployment import (
    Deployment, DeploymentConfig, DeploymentStrategy, DeploymentStatus, 
    Environment, StrategyType, HealthMetrics
)
...
class DeploymentOrchestrationService:
    ...
    def __init__(
        ...
        kubernetes_config_path: Optional[Path] = None
    ):
    ...
    async def deploy_model(
        ...
        deployment = Deployment(
            ...
            strategy=strategy,
            created_by=user,
            rollback_version_id=existing_deployment.model_version_id if existing_deployment else None
        )
    ...
    async def promote_to_production(
        ...
        if not staging_deployment.is_deployed or not staging_deployment.is_healthy:
            raise DeploymentValidationError(
                "Can only promote healthy, deployed models"
            )
    ...
    async def get_environment_status(self, environment: Environment) -> Dict[str, Any]:
        ...
        """
        active_deployment = await self._get_active_deployment_in_environment(environment)
        ...
        if active_deployment:
            status.update({
                ...
                "deployed_at": active_deployment.deployed_at.isoformat() if active_deployment.deployed_at else None
            })
    ...
    async def update_deployment_health(
        ...
        if deployment.is_deployed and deployment.should_rollback(100):  # Assuming 100 requests
    ...
    async def _execute_blue_green_deployment(
        ...
        await self._create_kubernetes_deployment(deployment, model_version, suffix="green")
        
        # Wait for green deployment to be ready
        await self._wait_for_deployment_ready(deployment, suffix="green")
        
        # Run smoke tests
        smoke_test_duration = deployment.strategy.configuration.get("smoke_test_duration_minutes", 5)
        ...
        switch_delay = deployment.strategy.configuration.get("traffic_switch_delay_seconds", 30)
    ...
    async def _execute_canary_deployment(
        ...
        await self._create_kubernetes_deployment(deployment, model_version, suffix="canary")
        
        # Start with initial traffic percentage
        initial_percentage = deployment.strategy.configuration.get("initial_traffic_percentage", 10)
        await self._set_traffic_percentage(deployment, initial_percentage, suffix="canary")
    ...
    async def _get_model_version(
        ...
        from pynomaly.domain.value_objects.model_storage_info import ModelStorageInfo, StorageBackend, SerializationFormat
    ...
    def _get_default_deployment_config(self, environment: Environment) -> DeploymentConfig:
