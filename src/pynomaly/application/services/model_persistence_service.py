...
class ModelPersistenceService:
    ...
    async def export_model(
        ...
        # Export model using secure serialization
        secure_serializer = get_secure_serializer()
        model_path = export_path / "model.secure"
        secure_serializer.serialize_model(detector, model_path)
...
class {detector.name.replace(" ", "")}Detector:
    ...
    def __init__(self, model_path: str = "model.secure"):
        """Load the detector model."""
        secure_serializer = get_secure_serializer()
        self.detector = secure_serializer.deserialize_model(Path(model_path))
