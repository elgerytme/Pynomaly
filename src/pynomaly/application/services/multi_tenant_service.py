...
from pynomaly.domain.entities.tenant import (
    Tenant, TenantStatus, SubscriptionTier, ResourceQuotaType, ResourceQuota, TenantConfiguration
)
...
class TenantResourceManager:
    ...
    async def allocate_resources(self, tenant_id: UUID, resource_type: str, amount: float) -> bool:
    ...
    async def deallocate_resources(self, tenant_id: UUID, resource_type: str, amount: float) -> bool:
...
class TenantIsolationService:
    ...
    async def get_tenant_encryption_key(self, tenant_id: UUID) -> Optional[str]:
...
class MultiTenantService:
    ...
    async def get_tenant_usage_summary(self, tenant_id: UUID) -> Optional[Dict[str, any]]:
        ...
        base_summary["billing"] = {
            "current_period_start": billing_info.get("current_period_start", "").isoformat() if billing_info.get("current_period_start") else "",
    ...
    async def upgrade_tenant_subscription(
        ...
        # Log the upgrade
        tenant.metadata["subscription_history"] = tenant.metadata.get("subscription_history", [])
    ...
    async def validate_tenant_access(
        ...
        path_valid = await self.isolation_service.validate_tenant_access(tenant_id, resource_path)
