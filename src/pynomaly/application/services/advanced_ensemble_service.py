...
from pynomaly.application.dto import DetectorConfig
...
class AdvancedEnsembleService:
    ...
    async def create_intelligent_ensemble(
        ...
        config: Optional[EnsembleConfiguration] = None
        ...
        if self.enable_meta_learning and algorithms is None:
            ...
            algorithms = algorithms or ["IsolationForest", "LocalOutlierFactor", "OneClassSVM"]
        
        # Step 3: Create and train base detectors
        base_detectors = await self._create_base_detectors(
            dataset, algorithms, config
        )
        ...
        if self.enable_meta_learning:
            await self._store_meta_knowledge(
                dataset_chars, optimized_ensemble, ensemble_weights,
                individual_performance, diversity_metrics
            )
        
        # Step 9: Generate ensemble report
        ensemble_report = self._generate_ensemble_report(
            optimized_ensemble, individual_performance, diversity_metrics,
            ensemble_weights, dataset_chars, config
        )
    ...
    def _analyze_dataset_for_ensemble(self, dataset: Dataset) -> Dict[str, Any]:
        ...
        characteristics = {
            ...
            "sample_efficiency": self._calculate_sample_efficiency(n_samples, n_features)
        }
    ...
    def _calculate_dataset_similarity(
        ...
        key_features = [
            'n_samples', 'n_features', 'size_ratio', 'feature_density',
            'data_complexity', 'noise_level'
        ]
    ...
    def _ensure_algorithm_diversity(
        ...
        for candidate in algorithms[1:]:
            ...
            for selected in diverse_algorithms:
                compatibility = self.algorithm_compatibility.get(
                    selected, {}
                ).get(candidate, 0.5)
    ...
    def _generate_synthetic_anomalies(self, normal_data: np.ndarray, n_anomalies: int) -> np.ndarray:
    ...
    def _calculate_diversity_metrics(self, predictions_matrix: np.ndarray) -> DiversityMetrics:
    ...
    def _calculate_fleiss_kappa(self, predictions_matrix: np.ndarray) -> float:
        ...
        try:
            ...
            for i in range(n_samples):
                ...
                agreement = (max_agreement - 1) / (n_classifiers - 1) if n_classifiers > 1 else 1
    ...
    async def _optimize_ensemble_composition(
        ...
        for i, detector in enumerate(base_detectors):
            ...
            # Performance score
            perf_score = performance.get('roc_auc', 
                        performance.get('accuracy',
                        performance.get('estimated_accuracy', 0.5)))
            ...
            # Combined score
            combined_score = (
                (1 - config.diversity_weighting) * perf_score +
                config.diversity_weighting * diversity_score
            )
    ...
    async def _learn_ensemble_weights(
        ...
        weights = await self._optimize_weights_cv(
            predictions, eval_dataset, config
        )
            ...
            def objective_function(weights):
                ...
                try:
                    if SKLEARN_AVAILABLE and len(np.unique(labels)) > 1:
                        ...
                        accuracy = np.mean(binary_pred == labels) if len(labels) > 0 else 0.5
                        return -accuracy
                except:
        ...
        try:
            ...
            knowledge = MetaLearningKnowledge(
                ...
                algorithm_performance={
                    name: individual_performance.get(f"detector_{i}", {}).get('roc_auc', 0.5)
                ...
                confidence_score=min(1.0, overall_performance + diversity_metrics.overall_diversity * 0.3)
            )
            ...
            if len(self.meta_knowledge) > 100:
                ...
                self.meta_knowledge.sort(key=lambda k: (k.confidence_score, k.timestamp), reverse=True)
    ...
    def _calculate_ensemble_performance_summary(
        ...
        estimated_ensemble_performance = min(1.0, weighted_performance + diversity_bonus)
        
        return {
            "weighted_individual_performance": weighted_performance,
            "diversity_score": diversity_metrics.overall_diversity,
            "estimated_ensemble_performance": estimated_ensemble_performance,
            "performance_improvement": max(0, estimated_ensemble_performance - weighted_performance),
            "confidence_score": min(1.0, diversity_metrics.overall_diversity + weighted_performance * 0.5)
        }
    ...
    def _generate_ensemble_recommendations(
        ...
        if config.ensemble_strategy == "voting" and diversity_metrics.overall_diversity > 0.7:
            recommendations.append(
    ...
    def _generate_meta_learning_insights(self) -> Dict[str, Any]:
        ...
        for knowledge in self.meta_knowledge:
            ...
            for algorithm in knowledge.ensemble_composition:
                algorithm_popularity[algorithm] = algorithm_popularity.get(algorithm, 0) + 1
            
            # Track performance trends
            ensemble_perf = knowledge.performance_metrics.get('ensemble_performance', 0.5)
        ...
        insights = {
            ...
            "learning_confidence": np.mean([k.confidence_score for k in self.meta_knowledge]),
            "diversity_patterns": {
    ...
    async def predict_ensemble_performance(
        ...
        confidence = min(1.0, total_weight / len(similar_cases)) if similar_cases else 0.0
        
        return {
            "predicted_performance": predicted_performance,
            "confidence": confidence,
            "similar_cases_found": len(similar_cases),
            "recommendation_strength": "high" if confidence > 0.7 else "medium" if confidence > 0.4 else "low"
        }