...
from pynomaly.domain.entities.continuous_learning import PerformanceDelta
...
class TestConfiguration:
    ...
    def __post_init__(self):
        ...
        if not (0.0 < self.challenger_traffic_percentage < 1.0):
            raise ValueError("Challenger traffic percentage must be between 0.0 and 1.0")
        if abs(self.champion_traffic_percentage + self.challenger_traffic_percentage - 1.0) > 1e-6:
...
class ABTestResult:
    ...
    def get_winning_variant(self) -> Optional[UUID]:
    ...
    def is_actionable(self) -> bool:
        ...
        return self.conclusion in [
            TestConclusion.CHAMPION_WINS,
            TestConclusion.CHALLENGER_WINS
...
class ABTestingService:
    ...
    def __init__(
        self,
        storage_path: Path,
        auto_retraining_service: Optional[AutoRetrainingService] = None
    ):
    ...
    async def design_ab_test(
        ...
        try:
            ...
            champion_variant = ModelVariant(
                ...
                is_champion=True,
                metadata={"role": "champion", "description": "Current production model"}
            )
            ...
            ab_test = ABTest(
                ...
                required_sample_size=max(required_sample_size, test_config.minimum_sample_size)
            )
    ...
    async def route_prediction(
        ...
        try:
            ...
            if hasattr(variant.model, "predict_proba"):
                ...
                decision = variant.model.decision_function(input_features.reshape(1, -1))[0]
        ...
        try:
            ...
            for record in self.prediction_records[test_id]:
                if record.record_id == prediction_record_id:
                    ...
                    logger.debug(f"Recorded feedback for prediction {prediction_record_id}")
                    return True
    ...
    async def analyze_test_performance(self, test_id: UUID) -> Dict[str, ABTestMetrics]:
        ...
        try:
            ...
            champion_records = [r for r in records if r.variant_id == test.champion_variant.variant_id]
            challenger_records = [r for r in records if r.variant_id == test.challenger_variant.variant_id]
    ...
    async def check_statistical_significance(self, test_id: UUID) -> List[StatisticalTestResult]:
        ...
        try:
            ...
            for metric_name in test.configuration.performance_metrics:
                if metric_name in champion_metrics.performance_metrics and \
                   metric_name in challenger_metrics.performance_metrics:
    ...
    async def evaluate_test_completion(self, test_id: UUID) -> Optional[ABTestResult]:
        ...
        try:
            ...
            conclusion = self._determine_test_conclusion(statistical_tests, test.configuration)
            
            # Calculate confidence
            confidence = self._calculate_conclusion_confidence(statistical_tests)
            
            # Generate recommendation
            recommendation = self._generate_recommendation(conclusion, statistical_tests, metrics)
    ...
    async def get_test_status(self, test_id: UUID) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "duration": str(datetime.utcnow() - test.started_at) if test.started_at else None,
                "total_predictions": len(records),
                "required_sample_size": test.required_sample_size,
                "completion_percentage": min(100, len(records) / test.required_sample_size * 100),
                "traffic_split": {
            ...
            status["variant_stats"] = {
                "champion": {
                    ...
                    "feedback_rate": sum(1 for r in champion_records if r.feedback_received) / max(1, len(champion_records))
                },
                "challenger": {
                    "predictions": len(challenger_records),
                    "feedback_rate": sum(1 for r in challenger_records if r.feedback_received) / max(1, len(challenger_records))
                }
            }
        ...
        if feedback_records:
            ...
            try:
                if "accuracy" in config.performance_metrics:
                    metrics.performance_metrics["accuracy"] = accuracy_score(y_true, y_pred)
                ...
                metrics.correct_predictions = sum(1 for t, p in zip(y_true, y_pred) if t == p)
    ...
    async def _should_complete_test(self, test: ABTest) -> Tuple[bool, str]:
        ...
        if test.configuration.early_stopping_enabled and len(records) > 500:
            try:
                statistical_tests = await self.check_statistical_significance(test.test_id)
                significant_tests = [t for t in statistical_tests if t.is_significant]
                
                if len(significant_tests) >= len(statistical_tests) * 0.8:  # 80% of tests significant
    ...
    def _calculate_conclusion_confidence(self, statistical_tests: List[StatisticalTestResult]) -> float:
    ...
    async def _assess_deployment_risk(
        ...
        if challenger_metrics.confidence_intervals:
            ...
            performance_risk = 0.0
            for metric, (lower, upper) in challenger_metrics.confidence_intervals.items():
    ...
    async def _load_test_from_file(self, test_file: Path) -> ABTest:
        ...
        class ABTest:
            ...
            champion_variant: Optional[ModelVariant] = None
            challenger_variant: Optional[ModelVariant] = None
        ...
        while test_id in self.active_tests:
            ...
            try:
                ...
                if test.configuration.guardrail_metrics:
                    await self.guardrail_monitor.check_guardrails(test_id, test.configuration)
                
                # Wait for next check
                await asyncio.sleep(test.configuration.early_stopping_check_interval_hours * 3600)
    ...
    async def compare_metrics(
        ...
        if is_significant:
            if challenger_value > champion_value:
                interpretation = f"Challenger significantly outperforms champion on {metric_name}"
            else:
                interpretation = f"Champion significantly outperforms challenger on {metric_name}"
        else:
