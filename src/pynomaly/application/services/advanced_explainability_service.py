...
class ExplanationConfig(BaseModel):
    ...
    explanation_type: str = Field(default="local", description="Explanation type (local, global)")
    method: str = Field(default="shap", description="Explanation method")
    n_samples: int = Field(default=1000, ge=10, description="Number of samples for explanation")
...
class BiasAnalysisConfig(BaseModel):
    ...
    fairness_metrics: List[str] = Field(default=["demographic_parity", "equalized_odds"], description="Fairness metrics")
    threshold: float = Field(default=0.5, description="Decision threshold")
    reference_group: Optional[str] = Field(None, description="Reference group for comparison")
    min_group_size: int = Field(default=50, description="Minimum group size for analysis")


class TrustScoreConfig(BaseModel):
    """Configuration for trust scoring."""
    
    consistency_checks: bool = Field(default=True, description="Enable consistency analysis")
    stability_analysis: bool = Field(default=True, description="Enable stability analysis")
    fidelity_assessment: bool = Field(default=True, description="Enable fidelity assessment")
    n_perturbations: int = Field(default=100, description="Number of perturbations for stability")
    perturbation_strength: float = Field(default=0.1, description="Perturbation strength")
...
class LocalExplanation(BaseModel):
    ...
    trust_score: Optional[float] = Field(None, description="Trust score for explanation")
...
class GlobalExplanation(BaseModel):
    ...
    feature_interactions: Dict[str, float] = Field(default_factory=dict, description="Feature interactions")
...
class BiasAnalysisResult(BaseModel):
    ...
    group_statistics: Dict[str, Dict[str, float]] = Field(description="Statistics by group")
...
class ExplanationReport(BaseModel):
    ...
    generation_time: datetime = Field(default_factory=datetime.now, description="Report generation time")
...
class AdvancedExplainabilityService:
    ...
    async def generate_comprehensive_explanation(
        ...
        try:
            ...
            feature_names = config.feature_names or dataset.features or [f"feature_{i}" for i in range(X.shape[1])]
            ...
            if config.explanation_type in ["local", "both"]:
                tasks.append(self._generate_local_explanations(detector, X, feature_names, config))
            
            # Global explanations
            if config.explanation_type in ["global", "both"]:
                tasks.append(self._generate_global_explanation(detector, X, feature_names, config))
    ...
    async def _generate_local_explanations(
        ...
        try:
            ...
            for i, idx in enumerate(indices):
                ...
                if self.enable_shap:
                    try:
                        shap_values = await self._compute_shap_values(detector, sample, X, feature_names)
                ...
                if self.enable_lime and not feature_contributions:
                    try:
                        lime_values = await self._compute_lime_values(detector, sample, X, feature_names)
                ...
                explanation = LocalExplanation(
                    ...
                    explanation_method="shap" if self.enable_shap else "lime" if self.enable_lime else "gradient",
    ...
    async def _generate_global_explanation(
        ...
        try:
            ...
            if self.enable_permutation:
                try:
                    perm_importance = await self._compute_permutation_importance(detector, X, feature_names)
                    if perm_importance:
            ...
            if self.enable_shap and not feature_importance:
                try:
                    shap_importance = await self._compute_shap_global_importance(detector, X, feature_names)
                    if shap_importance:
        ...
        try:
            ...
            for protected_attr in config.protected_attributes:
                ...
                if len(unique_groups) < 2:
                    logger.warning(f"Protected attribute '{protected_attr}' has less than 2 groups")
                    continue
    ...
    async def _analyze_attribute_bias(
        ...
        if len(group_stats) >= 2:
            ...
            fairness_metrics["demographic_parity"] = 1.0 - (max(positive_rates) - min(positive_rates))
            
            # Statistical parity difference
            fairness_metrics["statistical_parity_difference"] = max(positive_rates) - min(positive_rates)
            
            # Equalized odds (simplified)
            mean_scores = [stats["mean_score"] for stats in group_stats.values()]
            fairness_metrics["equalized_odds"] = 1.0 - (max(mean_scores) - min(mean_scores))
        
        # Determine if bias is detected
        bias_detected = any(
            metric < 0.8 for metric in fairness_metrics.values()
    ...
    async def _compute_shap_values(
        ...
        try:
            ...
            if cache_key not in self.explainer_cache:
                ...
                background_sample = background[np.random.choice(len(background), min(100, len(background)), replace=False)]
    ...
    def _compute_gradient_explanation(
        ...
        try:
            ...
            for i, feature_name in enumerate(feature_names):
                ...
                gradients[feature_name] = float(gradient * sample[0, i])  # Gradient * input
    ...
    async def _compute_shap_global_importance(
        ...
        try:
            ...
            if cache_key not in self.explainer_cache:
                ...
                explainer = shap.KernelExplainer(predict_fn, X_sample[:50])  # Small background
    ...
    def _compute_variance_importance(self, X: np.ndarray, feature_names: List[str]) -> Dict[str, float]:
        """Compute variance-based feature importance (fallback)."""
        variances = np.var(X, axis=0)
        normalized_variances = variances / np.sum(variances) if np.sum(variances) > 0 else variances
    ...
    def _compute_feature_interactions(self, X: np.ndarray, feature_names: List[str]) -> Dict[str, float]:
        """Compute feature interactions (simplified)."""
    ...
    async def _assess_consistency(self, detector: DetectorProtocol, X: np.ndarray) -> float:
            ...
            for idx in indices:
                ...
                for noise_level in noise_levels:
                    ...
                    if abs(original_pred) > 1e-10:
                        relative_change = abs(perturbed_pred - original_pred) / abs(original_pred)
    ...
    async def _assess_stability(self, detector: DetectorProtocol, X: np.ndarray, config: TrustScoreConfig) -> float:
        ...
        try:
            ...
            for idx in indices:
                ...
                for _ in range(config.n_perturbations):
                    noise = np.random.normal(0, config.perturbation_strength, sample.shape)
    ...
    async def _assess_fidelity(self, detector: DetectorProtocol, X: np.ndarray) -> float:
    ...
    def _select_representative_samples(self, X: np.ndarray, n_samples: int) -> List[int]:
    ...
    def _calculate_prediction_confidence(self, detector: DetectorProtocol, sample: np.ndarray, X: np.ndarray) -> float:
        ...
        try:
            ...
            max_deviation = max(abs(np.max(all_predictions) - percentile), abs(np.min(all_predictions) - percentile))
    ...
    def _get_dataset_summary(self, dataset: Dataset) -> Dict[str, Any]:
        ...
        return {
            ...
            "features": dataset.features[:10] if dataset.features else []  # First 10 features
        }
    ...
    def _generate_recommendations(self, trust_assessment: TrustScoreResult) -> List[str]:
        ...
        if trust_assessment.risk_assessment == "high":
            recommendations.append("Exercise caution when using model predictions for critical decisions")
