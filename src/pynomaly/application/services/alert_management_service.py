...
from pynomaly.domain.entities import Alert, AlertCondition, AlertNotification, AlertType, AlertSeverity, AlertStatus, NotificationChannel
from pynomaly.domain.exceptions import AlertNotFoundError, InvalidAlertStateError
from pynomaly.shared.protocols import AlertRepositoryProtocol, AlertNotificationRepositoryProtocol
...
class AlertManagementService:
    ...
    async def evaluate_alert_condition(
        ...
        result = {
            ...
            "should_trigger": condition_met and alert.status not in [AlertStatus.SUPPRESSED],
        ...
        if result["should_trigger"]:
            await self.trigger_alert(
                alert_id,
                context={"metric_value": metric_value, "threshold": alert.condition.threshold}
            )
    ...
    async def get_alert_analytics(
        ...
        acknowledged_alerts = [a for a in filtered_alerts if a.response_time_minutes is not None]
        avg_response_time = None
        if acknowledged_alerts:
            avg_response_time = sum(a.response_time_minutes for a in acknowledged_alerts) / len(acknowledged_alerts)
        
        # Resolution time analytics
        resolved_with_time = [a for a in filtered_alerts if a.resolution_time_minutes is not None]
        ...
        return {
            ...
            "alert_frequency": self._calculate_alert_frequency(filtered_alerts, start_date, end_date)
        }
    ...
    async def _escalate_alert(self, alert: Alert) -> None:
        ...
        alert.metadata["escalation_level"] = alert.metadata.get("escalation_level", 0) + 1
