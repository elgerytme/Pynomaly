...
class EventProcessingService:
    ...
    async def acknowledge_event(self, event_id: UUID, user: str, notes: str | None = None) -> AnomalyEvent:
    ...
    async def resolve_event(self, event_id: UUID, user: str, notes: str | None = None) -> AnomalyEvent:
        """Resolve an event.
    ...
    async def ignore_event(self, event_id: UUID, user: str, reason: str | None = None) -> AnomalyEvent:
        """Ignore an event.
    ...
    async def get_event_summary(
        ...
        if anomaly_events:
            scores = [e.anomaly_data.anomaly_score for e in anomaly_events if e.anomaly_data]
        ...
        resolution_rate = len(resolved_events) / total_events if total_events > 0 else 0.0
        ...
        if resolved_events:
            ...
            for event in resolved_events:
                if event.resolved_at and event.event_time:
                    resolution_time = (event.resolved_at - event.event_time).total_seconds()
        ...
        for event in events:
            if event.detector_id:
                detector_counts[str(event.detector_id)] = detector_counts.get(str(event.detector_id), 0) + 1
        
        top_detectors = [
            {"detector_id": detector_id, "event_count": count}
            for detector_id, count in sorted(detector_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
        ...
        for event in events:
            if event.data_source:
                source_counts[event.data_source] = source_counts.get(event.data_source, 0) + 1
    ...
    async def aggregate_events(
        ...
        for group_key, group_events in groups.items():
            ...
            # Unique counts
            unique_detectors = len(set(str(e.detector_id) for e in group_events if e.detector_id))
            unique_sessions = len(set(str(e.source_session_id) for e in group_events if e.source_session_id))
            
            # Status counts
            resolved_count = len([e for e in group_events if e.status == EventStatus.RESOLVED])
            acknowledged_count = len([e for e in group_events if e.status == EventStatus.ACKNOWLEDGED])
    ...
    def register_event_handler(self, event_type: EventType, handler: Callable[[AnomalyEvent], None]) -> None:
    ...
    def unregister_event_handler(self, event_type: EventType, handler: Callable[[AnomalyEvent], None]) -> None:
    ...
    async def _match_pattern(self, pattern: EventPattern, events: list[AnomalyEvent]) -> list[AnomalyEvent]:
    ...
    async def _send_pattern_alert(self, pattern: EventPattern, matching_events: list[AnomalyEvent]) -> None:
