...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationTemplateDTO, ConfigurationLevel,
    ConfigurationCollectionDTO, ConfigurationExportRequestDTO, ExportFormat,
    ConfigurationSearchRequestDTO, ConfigurationSource
)
...
class ConfigurationTemplateService:
    ...
    async def generate_template_from_configuration(
        ...
        logger.info(f"Generating template '{template_name}' from configuration {config_id}")
        
        # Extract variable parameters if parameterizing
        if parameterize:
            variable_parameters, parameter_constraints = self._identify_variable_parameters(base_config)
        else:
            variable_parameters = {}
            parameter_constraints = {}
        
        # Generate template configuration
        template_config = self._create_template_configuration(base_config, variable_parameters)
        ...
        if include_documentation:
            ...
            examples = self._generate_template_examples(template_config, variable_parameters)
    ...
    async def generate_batch_templates(
        ...
        for i, config in enumerate(configurations[:max_templates]):
            try:
                ...
                logger.warning(f"Failed to generate template from config {config.id}: {e}")
                continue
    ...
    async def generate_collection_template(
        ...
        logger.info(f"Generating collection template '{template_name}' from {len(configurations)} configurations")
        ...
        variable_parameters, parameter_constraints = self._identify_variable_parameters_from_multiple(configurations)
        ...
        template = ConfigurationTemplateDTO(
            ...
            documentation=self._generate_collection_documentation(configurations, merge_strategy),
            examples=self._generate_collection_examples(merged_config, variable_parameters)
        )
    ...
    async def export_template(
        ...
        if export_format == ExportFormat.YAML:
            content = self._export_template_yaml(template, include_examples, include_documentation)
        elif export_format == ExportFormat.JSON:
            content = self._export_template_json(template, include_examples, include_documentation)
        elif export_format == ExportFormat.PYTHON:
            content = self._export_template_python(template, include_examples, include_documentation)
        elif export_format == ExportFormat.NOTEBOOK:
            content = self._export_template_notebook(template, include_examples, include_documentation)
        elif export_format == ExportFormat.DOCKER:
            content = self._export_template_docker(template, include_examples, include_documentation)
        else:
    ...
    async def instantiate_template(
        ...
        logger.info(f"Instantiating template '{template.name}' with configuration '{configuration_name}'")
    ...
    async def get_template_recommendations(
        ...
        difficulty_preference: Optional[ConfigurationLevel] = None
        ...
        for template_file in template_files:
            try:
                template = await self.repository.load_template(
                    UUID(template_file.stem)
                )
                if template:
                    templates.append(template)
            except Exception:
        ...
        for template in templates:
            relevance_score = self._calculate_template_relevance(
                template, dataset_characteristics, performance_requirements, difficulty_preference
            )
    ...
    def get_template_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_template_files": len(list(self.template_storage_path.glob("*.json")))
        }
        ...
        if config.algorithm_config.hyperparameters:
            for param, value in config.algorithm_config.hyperparameters.items():
                if self._is_tunable_parameter(param, value):
                    ...
                    parameter_constraints[f"algorithm.{param}"] = self._get_parameter_constraints(param, value)
        ...
        if config.evaluation_config:
            ...
            for param, value in eval_params.items():
                if value is not None:
                    ...
                    parameter_constraints[f"evaluation.{param}"] = self._get_parameter_constraints(param, value)
    ...
    def _get_parameter_constraints(self, param_name: str, value: Any) -> Dict[str, Any]:
        ...
        if param_name == "contamination":
            ...
            if isinstance(value, bool):
                ...
                constraints = {"type": "integer", "min": max(1, int(value * 0.1)), "max": int(value * 10)}
            elif isinstance(value, float):
                constraints = {"type": "float", "min": max(0.001, value * 0.1), "max": value * 10}
            elif isinstance(value, str):
        ...
        if parts[0] == "algorithm":
            if len(parts) == 2:
                ...
                if param_name == "contamination":
                    ...
                    config.algorithm_config.hyperparameters[param_name] = f"{{{{ {param_path} }}}}"
    ...
    def _generate_template_documentation(
        ...
        if variable_parameters:
            doc_parts.extend([
                ...
                f"This template supports the following configurable parameters:\n"
            ])
        ...
        if base_config.performance_results:
            doc_parts.extend([
                ...
                f"Based on the original configuration, you can expect:\n",
                f"- **Accuracy**: {base_config.performance_results.accuracy:.3f}\n" if base_config.performance_results.accuracy else "",
                f"- **Training Time**: {base_config.performance_results.training_time_seconds:.1f}s\n" if base_config.performance_results.training_time_seconds else "",
            ])
        
        doc_parts.extend([
            f"\n## Usage\n",
            f"To use this template:\n",
            f"1. Load the template\n",
            f"2. Provide values for configurable parameters\n",
            f"3. Instantiate the configuration\n",
            f"4. Run anomaly detection\n"
        ])
    ...
    def _generate_template_examples(
        ...
        if len(variable_parameters) > 1:
            ...
            examples.append({
                ...
                "code": self._generate_example_code(template_config, high_perf_params)
            })
    ...
    def _generate_example_code(
        ...
        code_lines = [
            ...
            f"config = template_service.instantiate_template(",
            f"    template_id='{template_config.id}',",
            f"    parameters=parameters,",
            f"    configuration_name='my_detection_config'",
            f")",
            "",
            "# Load your dataset",
            "loader = CSVLoader()",
    ...
    def _infer_use_cases(self, config: ExperimentConfigurationDTO) -> List[str]:
        ...
        if "isolation" in algorithm:
            use_cases.extend(["high_dimensional_data", "large_datasets", "general_purpose"])
        elif "lof" in algorithm or "outlier" in algorithm:
            use_cases.extend(["density_based_detection", "local_anomalies", "small_to_medium_datasets"])
        elif "svm" in algorithm:
            use_cases.extend(["non_linear_boundaries", "robust_detection", "medium_datasets"])
        ...
        if config.performance_results and config.performance_results.accuracy:
            ...
            if config.performance_results.training_time_seconds and config.performance_results.training_time_seconds < 60:
    ...
    def _determine_difficulty_level(self, config: ExperimentConfigurationDTO) -> ConfigurationLevel:
        ...
        if config.algorithm_config.hyperparameters:
            complexity_score += min(len(config.algorithm_config.hyperparameters) // 3, 2)
    ...
    def _estimate_template_runtime(self, config: ExperimentConfigurationDTO) -> Optional[float]:
        """Estimate template runtime based on configuration."""
        if config.performance_results and config.performance_results.training_time_seconds:
    ...
    def _export_template_python(
        ...
        script_lines = [
            ...
            f'    """Create configuration from template."""',
            ...
            f"        parameters=params,",
            f"        configuration_name='template_config'",
            f"    )",
            "",
            "    return config",
            "",
            "def run_detection(dataset_path, **template_params):",
            f'    """Run detection using template configuration."""',
    ...
    def _export_template_notebook(
        ...
        cells.append({
            ...
            "source": [
                ...
                f"\n",
                f"# Instantiate configuration\n",
                f"template_service = ConfigurationTemplateService()\n",
                f"config = template_service.instantiate_template(\n",
                f"    template_id=template_id,\n",
                f"    parameters=parameters,\n",
                f"    configuration_name='notebook_config'\n",
                f")"
            ]
        })
            ...
            "services": {
                f"pynomaly-{template.name.lower().replace(' ', '-')}": {
                    ...
                    "environment": {
                        ...
                        **{f"PYNOMALY_{k.upper().replace('.', '_')}": str(v.get('default', ''))
                    ...
                    "command": [
                        "python", "-m", "pynomaly.cli.template",
                        "--template-id", str(template.id),
                        "--data-path", "/app/data",
                        "--output-path", "/app/results"
                    ]
                }
            }
        }
    ...
    def _calculate_template_relevance(
        ...
        dataset_characteristics: Dict[str, Any],
        performance_requirements: Dict[str, float],
        difficulty_preference: Optional[ConfigurationLevel]
    ) -> float:
        ...
        if difficulty_preference:
            if template.difficulty_level == difficulty_preference:
                score += 0.2
            elif abs(list(ConfigurationLevel).index(template.difficulty_level) - 
                    list(ConfigurationLevel).index(difficulty_preference)) == 1:
                score += 0.1
    ...
    def _create_ensemble_configuration(
        ...
        best_config = max(
            configurations,
            key=lambda c: c.performance_results.accuracy if c.performance_results and c.performance_results.accuracy else 0
        )
    ...
    def _average_configurations(
        ...
        for param, values in numeric_params.items():
            if values:
                averaged_config.algorithm_config.hyperparameters[param] = sum(values) / len(values)
    ...
    def _generate_collection_documentation(
        ...
        doc_parts.extend([
            ...
            f"\n## Expected Benefits\n",
            f"- Combines strengths of multiple approaches\n",
            f"- Robust performance across different data types\n",
            f"- Proven effectiveness on real datasets\n"
        ])
            ...
            if param_path in template.parameter_constraints:
                ...
                if expected_type:
                    if expected_type == "integer" and not isinstance(value, int):
                        errors.append(f"{param_path}: expected integer, got {type(value).__name__}")
                    elif expected_type == "float" and not isinstance(value, (int, float)):
                        errors.append(f"{param_path}: expected float, got {type(value).__name__}")
                    elif expected_type == "boolean" and not isinstance(value, bool):
                        errors.append(f"{param_path}: expected boolean, got {type(value).__name__}")
                    elif expected_type == "string" and not isinstance(value, str):
                        errors.append(f"{param_path}: expected string, got {type(value).__name__}")
                
                # Range validation
                if isinstance(value, (int, float)):
                    if "min" in constraints and value < constraints["min"]:
                        errors.append(f"{param_path}: value {value} below minimum {constraints['min']}")
                    if "max" in constraints and value > constraints["max"]:
                        errors.append(f"{param_path}: value {value} above maximum {constraints['max']}")
                
                # Choice validation
                if "choices" in constraints and value not in constraints["choices"]:
                    errors.append(f"{param_path}: value {value} not in allowed choices {constraints['choices']}")
        ...
        for param_path, param_info in variable_parameters.items():
            if param_path not in parameters:
                ...
                if default_value is not None:
                    self._set_instantiated_value(instantiated, param_path, default_value)
