...
from pynomaly.domain.value_objects.model_storage_info import (
    ModelStorageInfo, StorageBackend, SerializationFormat
)
...
class EnhancedModelPersistenceService:
    ...
    async def serialize_model(
        ...
        try:
            ...
            serialized_data, file_extension = await self._serialize_by_format(detector, format)
        ...
        try:
            ...
            if verify_checksum:
                if not storage_info.verify_checksum(model_data):
                    raise ModelDeserializationError("Model checksum verification failed")
            ...
            detector = await self._deserialize_by_format(model_data, storage_info.format)
            
            return detector
            
        except Exception as e:
            raise ModelDeserializationError(f"Failed to deserialize model: {str(e)}") from e
    ...
    async def compare_versions(
        ...
        """
        performance_diff = version1.performance_metrics.compare_with(version2.performance_metrics)
        ...
        return {
            ...
            "version1_is_better": version1.performance_metrics.is_better_than(version2.performance_metrics),
    ...
    async def create_model_archive(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exported_files = await self.export_model_bundle(
                model_version, temp_path
            )
    ...
    async def _serialize_by_format(
        ...
        if format == SerializationFormat.PICKLE:
            ...
            raise UnsupportedFormatError(f"Serialization format {format.value} not supported")
    ...
    async def _deserialize_by_format(
        ...
        if format == SerializationFormat.PICKLE:
        ...
        elif format in [SerializationFormat.JOBLIB, SerializationFormat.SCIKIT_LEARN_PICKLE]:
            import joblib
            with tempfile.NamedTemporaryFile() as temp_file:
            ...
            raise UnsupportedFormatError(f"Deserialization format {format.value} not supported")
    ...
    async def _create_deployment_config(self, model_version: ModelVersion) -> Dict[str, Any]:
