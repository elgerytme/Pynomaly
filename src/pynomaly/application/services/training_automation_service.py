...
class TrainingAutomationService:
    ...
    def _create_hyperparameter_spaces(self) -> Dict[str, HyperparameterSpace]:
        ...
        spaces["Autoencoder"] = HyperparameterSpace(
            ...
            parameters={
                "hidden_neurons": {"type": "categorical", "choices": [[32, 16, 8], [64, 32, 16], [128, 64, 32]]},
    ...
    async def create_training_job(
        ...
        if configuration.algorithm_whitelist:
            target_algorithms = [
                alg for alg in target_algorithms 
        ...
        logger.info(f"Created training job {job.job_id} for algorithms: {target_algorithms}")
        ...
        try:
            ...
            if MLFLOW_AVAILABLE and job.configuration.experiment_name:
                experiment_id = mlflow.create_experiment(
                    ...
                    artifact_location=str(self.storage_path / "experiments" / job.job_id)
                )
    ...
    async def _optimize_algorithm(
        ...
        try:
            ...
            study.optimize(
                ...
                timeout=job.configuration.timeout_minutes * 60 if job.configuration.timeout_minutes else None
            )
    ...
    async def _objective_function(
        ...
        try:
            ...
            return float('-inf') if job.configuration.optimization_direction == "maximize" else float('inf')
    ...
    async def _train_with_defaults(self, job: TrainingJob, algorithm: str) -> Dict[str, Any]:
...
async def quick_optimize(
    ...
    from pynomaly.infrastructure.persistence.training_job_repository import TrainingJobRepository
    from pynomaly.infrastructure.adapters.model_trainer_adapter import ModelTrainerAdapter
