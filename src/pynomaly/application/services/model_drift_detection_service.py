...
class ModelDriftDetectionService:
    ...
    async def detect_drift(
        ...
        overall_drift_detected = (
            ...
            or multivariate_drift_detected
            or concept_drift_detected
        )
    ...
    async def _detect_concept_drift(
        ...
        try:
            ...
            ref_predictions = await self._get_model_predictions(
                model_id, reference_data
            )
            cur_predictions = await self._get_model_predictions(model_id, current_data)

            # Compare prediction distributions
            drift_score, _ = self._ks_test(
                pd.Series(ref_predictions), pd.Series(cur_predictions)
            )
    ...
    def _generate_recommendations(
        ...
        if severity == DriftSeverity.CRITICAL:
            recommendations.append(
                "URGENT: Stop model serving and investigate immediately"
            )
        ...
        if DriftType.DATA_DRIFT in drift_types and drifted_features:
            recommendations.append(
                f"Focus on features with drift: {', '.join(drifted_features[:5])}"
            )

        return recommendations

    def _ks_test(
        self, ref_values: pd.Series, cur_values: pd.Series
    ) -> tuple[float, float]:
    ...
    def _psi_test(self, ref_values: pd.Series, cur_values: pd.Series) -> float:
        ...
        for i in range(len(ref_props)):
            if ref_props[i] > 0 and cur_props[i] > 0:
                psi += (cur_props[i] - ref_props[i]) * np.log(
                    cur_props[i] / ref_props[i]
                )
    ...
    async def _send_drift_alert(
        self, monitor: DriftMonitor, report: DriftReport
    ) -> None:
