...
class ExplainabilityEngine:
    ...
    def __init__(self, config: Optional[ExplanationConfig] = None):
        ...
        if not SHAP_AVAILABLE:
            self.logger.warning("SHAP not available. SHAP explanations will be disabled.")
        
        if not LIME_AVAILABLE:
            self.logger.warning("LIME not available. LIME explanations will be disabled.")
        
        if not SKLEARN_AVAILABLE:
            self.logger.warning("scikit-learn not available. Some features will be limited.")
    ...
    async def explain_local(
        ...
        """
        self.logger.info(f"Generating local explanations for {len(sample_indices)} samples")
        ...
        try:
            ...
            for idx in sample_indices:
                ...
                explanation = await self._generate_local_explanation(
                    detector, X, sample, idx, sample_score, sample_prediction, explanation_method
                )
    ...
    async def _feature_perturbation_importance(
        ...
        for i in range(n_features):
            ...
            importances.append(FeatureImportance(
                ...
                confidence=0.6,
                method="perturbation",
                metadata={"baseline_value": baseline_value, "original_value": sample[0, i]},
            ))
        ...
        for feature_name, importances_list in feature_groups.items():
            ...
            if total_weight > 0:
                weighted_importance = sum(
                    fi.importance_score * fi.confidence for fi in importances_list
                ) / total_weight
                weighted_contribution = sum(
                    fi.contribution * fi.confidence for fi in importances_list
                ) / total_weight
                avg_confidence = total_weight / len(importances_list)
            else:
                weighted_importance = np.mean([fi.importance_score for fi in importances_list])
                weighted_contribution = np.mean([fi.contribution for fi in importances_list])
                avg_confidence = 0.3
            ...
            combined.append(FeatureImportance(
    ...
    def _select_best_method(self, detector: Detector, X: np.ndarray) -> str:
        ...
        if any(tree_name in model_name for tree_name in ["forest", "tree", "gradient", "xgb"]):
            if SHAP_AVAILABLE:
                return "shap"
        
        # Linear models work well with SHAP linear
        if any(linear_name in model_name for linear_name in ["linear", "logistic", "svm"]):
            if SHAP_AVAILABLE:
                return "shap"
    ...
    def _create_shap_explainer(self, detector: Detector, X: np.ndarray):
        ...
        try:
            ...
            if "tree" in model_name or "forest" in model_name:
                ...
                return shap.KernelExplainer(lambda x: np.random.random(len(x)), background)
            else:
    ...
    async def _global_shap_importance(
        ...
        try:
            ...
            feature_importances = []
            for i, (importance, contribution) in enumerate(zip(mean_abs_shap, mean_shap)):
                feature_importances.append(FeatureImportance(
                    feature_name=f"feature_{i}",
            ...
            for i, feature_name in enumerate(feature_names):
                ...
                if abs(anomaly_mean - normal_mean) > max(anomaly_std, normal_std):
                    pattern = {
                        ...
                        "strength": min(abs(anomaly_mean - normal_mean) / max(anomaly_std, normal_std, 1e-8), 5.0),
                    }
            ...
            if X.shape[1] > 1:
                for i in range(min(5, X.shape[1])):
                    for j in range(i + 1, min(5, X.shape[1])):
                        ...
                        centroid_distance = np.linalg.norm(anomaly_centroid - normal_centroid)
    ...
    async def _analyze_feature_interactions(
        self, X: np.ndarray, scores: np.ndarray, feature_names: Optional[List[str]] = None
        ...
        try:
            ...
            for i in range(min(10, n_features)):
                for j in range(i + 1, min(10, n_features)):
                    ...
                    try:
                        interaction_correlation = abs(np.corrcoef(interaction_feature, scores)[0, 1])
                    ...
                    interaction_strength = max(0, interaction_correlation - individual_max)
    ...
    def _calculate_interpretability_score(
        ...
        if feature_importances:
            ...
            top_5_importance = sum(fi.importance_score for fi in feature_importances[:5])
        ...
        if anomaly_patterns:
            avg_pattern_strength = np.mean([p.get("strength", 0) for p in anomaly_patterns])
