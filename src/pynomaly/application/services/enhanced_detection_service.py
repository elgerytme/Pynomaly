...
class EnhancedDetectionService:
    ...
    async def compare_algorithms(
        ...
        # Analyze results
        comparison = self._analyze_comparison_results(
            detector_names, results, dataset
        )
    ...
    ) -> Union[List[DetectorProtocol], tuple[List[DetectorProtocol], List[AlgorithmRecommendation]]]:
        ...
        for detector in detectors:
            ...
            for dataset in datasets:
                try:
                    ...
                    print(f"Error with detector {detector.name} on dataset {dataset.name}: {e}")
                    continue
    ...
    def _analyze_dataset(
        ...
        has_categorical = any(
            dataset.data[col].dtype == 'object' or dataset.data[col].dtype.name == 'category'
    ...
    async def _run_parallel_detection(
        ...
        detect_tasks = [
            self._detect_with_detector(detector, dataset)
            for detector in detectors
        ]
    ...
    def _analyze_comparison_results(
        ...
        for name, result in zip(detector_names, results):
            if isinstance(result, DetectionResult):
                comparison["individual_results"][name] = {
                    ...
                    "mean_score": sum(s.value for s in result.scores) / len(result.scores),
        ...
        if comparison["individual_results"]:
            all_times = [r["execution_time_ms"] for r in comparison["individual_results"].values()]
            all_rates = [r["anomaly_rate"] for r in comparison["individual_results"].values()]
            
            comparison["summary"] = {
                "fastest_detector": min(comparison["individual_results"].items(), 
                                      key=lambda x: x[1]["execution_time_ms"])[0],
                "slowest_detector": max(comparison["individual_results"].items(),
                                       key=lambda x: x[1]["execution_time_ms"])[0],
    ...
    async def _optimize_ensemble_weights(
        ...
        for detector, result in zip(base_detectors, individual_results):
            ...
            weight = 1.0 / (variance + 1e-8)  # Add small value to avoid division by zero
