...
from pynomaly.application.dto.configuration_dto import (
    ExperimentConfigurationDTO, ConfigurationCaptureRequestDTO, ConfigurationResponseDTO,
    ConfigurationSource, ConfigurationStatus, DatasetConfigDTO, AlgorithmConfigDTO,
    PreprocessingConfigDTO, EvaluationConfigDTO, EnvironmentConfigDTO,
    ConfigurationMetadataDTO, PerformanceResultsDTO, ConfigurationLineageDTO,
    ConfigurationValidationResultDTO, ConfigurationSearchRequestDTO,
...
class ConfigurationCaptureService:
    ...
    def __init__(
        ...
        self.configuration_index: Dict[str, List[UUID]] = {}  # Tag/algorithm -> config IDs
        
        # Capture state
        self.active_captures: Dict[str, Dict[str, Any]] = {}  # session_id -> capture context
        self.capture_stats = {
            "total_captures": 0,
    ...
    async def capture_configuration(
        ...
        try:
            ...
            return ConfigurationResponseDTO(
                success=True,
                message=f"Configuration captured successfully",
    ...
    async def validate_configuration(
        ...
        if not config.dataset_config.dataset_path and not config.dataset_config.dataset_name:
        ...
        if config.algorithm_config.contamination and (
            config.algorithm_config.contamination < 0 or config.algorithm_config.contamination > 0.5
        ):
    ...
    async def _extract_configuration_from_parameters(
        ...
        config_id = uuid4()
        config_name = context.get('name', f"config_{source}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        if any(key.startswith('preprocess') for key in raw_parameters.keys()):
            preprocessing_config = PreprocessingConfigDTO(
                missing_value_strategy=raw_parameters.get('missing_value_strategy', 'mean'),
    ...
    async def _apply_search_filters(
        ...
        if request.query:
            ...
            filtered = [
                config for config in filtered
                if (query_lower in config.name.lower() or
                    query_lower in config.algorithm_config.algorithm_name.lower() or
                    (config.metadata.description and query_lower in config.metadata.description.lower()))
            ]
        ...
        if request.min_accuracy and request.min_accuracy > 0:
            filtered = [
                ...
                if (config.performance_results and
                    ...
                    config.performance_results.accuracy >= request.min_accuracy)
            ]
    ...
    async def _sort_configurations(
        ...
        if request.sort_by == "created_at":
            return sorted(configurations, key=lambda x: x.metadata.created_at, reverse=reverse)
            ...
            return sorted(configurations, key=lambda x: x.algorithm_config.algorithm_name, reverse=reverse)
        elif request.sort_by == "accuracy" and any(c.performance_results for c in configurations):
            return sorted(
                configurations,
                key=lambda x: x.performance_results.accuracy if x.performance_results and x.performance_results.accuracy else 0,
                reverse=reverse
            )
        else:
    ...
    async def _export_to_python(
        ...
        for i, config in enumerate(configurations):
            script_lines.extend([
                ...
                f"    ",
                f"    # Run detection",
                f"    service = DetectionService()",
                f"    result = service.detect_anomalies(dataset, algorithm_config)",
                f"    print(f'Found {{len(result.anomalies)}} anomalies')",
            ])
            ...
            notebook["cells"].extend([
                ...
                },
                {
                    ...
                    "source": [
                        ...
                        f"loader = CSVLoader()\n",
                        f"dataset = loader.load(dataset_path)\n",
                        f"print(f'Dataset shape: {{dataset.shape}}')"
                    ]
                }
            ])
    ...
    async def _estimate_memory_usage(
        ...
        if config.dataset_config.expected_shape:
            ...
            data_memory = (n_samples * n_features * 8) / (1024 * 1024)  # 8 bytes per float64
    ...
    async def _load_existing_configurations(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.configuration_cache)} existing configurations")
