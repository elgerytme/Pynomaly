...
class MLGovernanceApplicationService:
    ...
    async def onboard_model(self, 
                          model: Model, 
                          validation_data: pd.DataFrame,
                          model_info: Dict[str, Any],
                          created_by: str,
                          policy_name: str = "default") -> ModelGovernanceRecord:
        ...
        try:
            ...
            logger.info(f"Model {model.id} validation completed: {validation_results['passed']}")
            
            # Run compliance check if validation passed
            if validation_results["passed"]:
                compliance_results = await self.governance_framework.run_compliance_check(
                    record_id=record.record_id
                )
                logger.info(f"Model {model.id} compliance check completed: {compliance_results['overall_score']}")
    ...
    async def request_model_approval(self, 
                                   record_id: UUID,
                                   requested_by: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if record.status not in [GovernanceStatus.APPROVED, GovernanceStatus.UNDER_REVIEW]:
                raise ValueError(f"Model not ready for approval. Current status: {record.status}")
            ...
            logger.info(f"Approval requests created for model {record.model_id}: {len(approval_requests)} requests")
    ...
    async def approve_model_deployment(self, 
                                     record_id: UUID,
                                     approval_id: str,
                                     approver: str,
    ...
    async def deploy_model_to_stage(self, 
                                  record_id: UUID,
    ...
    async def promote_model_through_stages(self, 
                                         record_id: UUID,
        ...
        try:
            ...
            logger.info(f"Model promotion completed through {len(promotion_results)} stages")
    ...
    async def run_governance_audit(self, record_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            report = await self.governance_framework.generate_governance_report(record_id)
            ...
            if not report["documentation_status"]["model_card_exists"]:
                ...
                recommendations.append("Create comprehensive model card with performance metrics and limitations")
            
            # Check approval status
            if report["approval_summary"]["pending_count"] > 0:
                findings.append(f"{report['approval_summary']['pending_count']} pending approvals")
                recommendations.append("Follow up on pending approvals to complete governance process")
            ...
            if compliance_score < 0.8:
                ...
                recommendations.append("Address compliance violations to improve governance score")
            
            audit_report["audit_findings"] = findings
            audit_report["recommendations"] = recommendations
            audit_report["overall_governance_score"] = self._calculate_governance_score(report)
            
            logger.info(f"Governance audit completed with {len(findings)} findings")
            return audit_report
            
        except Exception as e:
            logger.error(f"Failed to run governance audit for record {record_id}: {str(e)}")
            raise
    ...
    async def get_governance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard_data["governance_health_score"] = self._calculate_overall_governance_health()
            dashboard_data["recommendations"] = self._generate_governance_recommendations(dashboard_data)
    ...
    async def bulk_compliance_check(self, 
                                  stage: Optional[ModelStage] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for record_id, task in tasks:
                try:
                    ...
                    logger.error(f"Compliance check failed for record {record_id}: {str(e)}")
                    compliance_results["results"].append({
                        "record_id": str(record_id),
            ...
            logger.info(f"Bulk compliance check completed: {compliance_results['compliant_models']}/{compliance_results['total_models']} compliant")
    ...
    def _calculate_governance_score(self, report: Dict[str, Any]) -> float:
        ...
        deployment_score = 1.0 if report["deployment_summary"]["deployment_count"] > 0 else 0
    ...
    def _calculate_overall_governance_health(self) -> float:
        ...
        approved_count = len([r for r in records if r.status == GovernanceStatus.APPROVED])
        return approved_count / len(records)
    
    def _generate_governance_recommendations(self, dashboard_data: Dict[str, Any]) -> List[str]:
        ...
        if non_compliant > 0:
            recommendations.append(f"Address compliance issues for {non_compliant} non-compliant models")
        ...
        if health_score < 0.8:
            recommendations.append("Improve overall governance health by completing approval processes")
