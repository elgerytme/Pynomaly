    ...
    def evaluate_compliance(self, context: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for rule in self.rules:
            ...
            if not self._evaluate_rule(rule_type, conditions, context):
                violations.append(rule.get("message", f"Policy rule violation: {rule_type}"))
        
        is_compliant = len(violations) == 0
        return is_compliant, violations
    
    def _evaluate_rule(self, rule_type: str, conditions: Dict[str, Any], context: Dict[str, Any]) -> bool:
...
class GovernanceFrameworkService:
    ...
    async def create_audit_entry(
        ...
        try:
            # Assess risk level
            risk_level = await self._assess_action_risk(action, resource_type, user_role)
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            violation_entries = [entry for entry in period_entries if entry.policy_violations]
            compliance_score = ((total_entries - len(violation_entries)) / max(total_entries, 1)) * 100
            ...
            for policy_id, policy in self.policies.items():
                if policy.is_active():
                    ...
                    policy_compliance[policy_id] = {
                        ...
                        "compliance_rate": ((total_entries - len(policy_violations)) / max(total_entries, 1)) * 100
                    }
            ...
            recommendations = await self._generate_compliance_recommendations(period_entries)
            ...
            report = ComplianceReport(
                ...
                audit_summary={
                    ...
                    "resource_types": len(set(entry.resource_type for entry in period_entries)),
                    "high_risk_actions": len([e for e in period_entries if e.risk_level == RiskLevel.HIGH])
                },
    ...
    async def get_data_lineage(
        ...
        try:
            ...
            if direction in ["upstream", "both"]:
                upstream = [
                    entry for entry in self.data_lineage
    ...
    async def _assess_action_risk(
        ...
        # High-risk actions
        if action in [GovernanceAction.DELETE, GovernanceAction.DEPLOY, GovernanceAction.EXPORT]:
    ...
    async def _process_real_time_alerts(self, entry: AuditTrailEntry) -> None:
        ...
        if entry.policy_violations:
            logger.warning(f"Policy violations detected in entry {entry.entry_id}: {entry.policy_violations}")
        
        if entry.risk_level == RiskLevel.HIGH:
            logger.warning(f"High-risk action detected: {entry.action.value} on {entry.resource_type}")
    ...
    async def _generate_compliance_recommendations(
        ...
        high_risk_count = len([e for e in audit_entries if e.risk_level == RiskLevel.HIGH])
        
        if violation_count > len(audit_entries) * 0.1:  # More than 10% violations
            recommendations.append("Consider reviewing and updating governance policies to reduce violation rate")
        
        if high_risk_count > 0:
            recommendations.append("Implement additional approval workflows for high-risk operations")
        ...
        for resource_type in resource_types:
            ...
            if not applicable_policies:
                recommendations.append(f"Consider creating governance policies for {resource_type} resources")
            ...
            if self.policies_file.exists():
                ...
                for policy_id, policy_dict in policies_data.items():
                    ...
                    policy = GovernancePolicy(
                        ...
                        effective_date=datetime.fromisoformat(policy_dict["effective_date"]),
                    ...
                    if policy_dict.get("expiration_date"):
                        policy.expiration_date = datetime.fromisoformat(policy_dict["expiration_date"])
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            entry = AuditTrailEntry(
                                ...
                                policy_violations=entry_dict.get("policy_violations", [])
                            )
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            entry = DataLineageEntry(
                                ...
                                transformation_details=lineage_dict.get("transformation_details", {}),
