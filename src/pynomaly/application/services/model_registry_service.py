...
from pynomaly.domain.entities.model_registry import (
    ModelRegistry, Model, AccessPolicy, AccessLevel, Environment
)
    ...
    def matches_model(self, model: Model) -> bool:
        ...
        if self.min_accuracy or self.max_inference_time:
            ...
            if latest_version:
                ...
                if self.max_inference_time and metrics.inference_time > self.max_inference_time:
                    return False
...
class ModelRegistryService:
    ...
    async def get_registry(self, name: Optional[str] = None) -> ModelRegistry:
    ...
    async def list_models(
        ...
        filtered_models = [
            ...
            if criteria.matches_model(model)
        ]
    ...
    async def promote_to_production(
        ...
        if not registry.access_policy.is_admin(user, groups):
            raise AccessDeniedError(f"User {user} lacks admin permission for production promotion")
    ...
    async def compare_model_versions(
        ...
        Returns:
            Comparison results
        """
        v1 = await self.get_model_version(model_id, version1, registry_name, user, groups)
        v2 = await self.get_model_version(model_id, version2, registry_name, user, groups)
        ...
        return {
            ...
            "version1_is_better": v1.performance_metrics.is_better_than(v2.performance_metrics),
    ...
    def _generate_version_recommendations(
        ...
        perf_diff = version1.performance_metrics.compare_with(version2.performance_metrics)
        
        if perf_diff["accuracy"] > 0.05:
            recommendations.append(f"Version {version1.version_string} has significantly better accuracy")
        elif perf_diff["accuracy"] < -0.05:
            recommendations.append(f"Version {version2.version_string} has significantly better accuracy")
        
        if perf_diff["inference_time"] < -10:  # ms improvement
            recommendations.append(f"Version {version1.version_string} is faster for inference")
        elif perf_diff["inference_time"] > 10:
            recommendations.append(f"Version {version2.version_string} is faster for inference")
        
        if version1.status == ModelStatus.DEPLOYED:
            recommendations.append(f"Version {version1.version_string} is currently deployed")
        elif version2.status == ModelStatus.DEPLOYED:
            recommendations.append(f"Version {version2.version_string} is currently deployed")
        
        if not recommendations:
            recommendations.append("Both versions have similar performance characteristics")
