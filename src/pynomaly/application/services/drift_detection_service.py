...
from pynomaly.domain.entities.drift_detection import (
    DriftDetectionMethod, DriftScope, DriftThresholds, TimeWindow, FeatureData,
    UnivariateDriftResult, MultivariateDriftResult, FeatureDriftAnalysis,
    ConceptDriftResult, DriftAnalysisResult
)
from pynomaly.domain.entities.continuous_learning import DriftEvent, DriftType, DriftSeverity
...
class PerformanceHistory:
    ...
    def __post_init__(self):
        ...
        lengths = [
            len(self.timestamps), len(self.accuracy_scores),
            len(self.precision_scores), len(self.recall_scores),
            len(self.prediction_confidence), len(self.prediction_counts)
        ]
...
class DriftDetectionService:
        ...
        try:
            ...
            if reference_data is None:
                if model_id not in self.reference_data_cache:
                    raise InsufficientDataError(f"No reference data for model {model_id}")
            ...
            logger.info(f"Data drift analysis completed for model {model_id}: "
    ...
    async def detect_concept_drift(
        ...
        try:
            ...
            if len(performance_history.accuracy_scores) < 10:
                raise InsufficientDataError("Insufficient performance history for concept drift detection")
            ...
            combined_probability = self._combine_drift_probabilities(
                performance_trend["degradation_probability"],
                ai_drift_probability
            )
            
            # Determine if drift is detected
            drift_detected = combined_probability > self.drift_thresholds.neural_drift_threshold
            ...
            label_distribution_shift = self._detect_label_distribution_shift(performance_history)
            decision_boundary_shift = self._detect_decision_boundary_shift(performance_history)
            
            result = ConceptDriftResult(
                detection_method=DriftDetectionMethod.NEURAL_DRIFT_DETECTOR if self.enable_ai_detection 
                ...
                prediction_consistency_score=stability_metrics.get("consistency_score", 1.0),
                label_distribution_shift=label_distribution_shift,
                decision_boundary_shift=decision_boundary_shift
            )
            
            logger.info(f"Concept drift analysis completed for model {model_id}: "
    ...
    async def _detect_univariate_drift(self, feature_data: FeatureData) -> UnivariateDriftResult:
        ...
        return UnivariateDriftResult(
            ...
            additional_metrics={
                "all_methods": [r.to_dict() for r in methods_results]
            }
        )
    ...
    async def _analyze_feature_drift(
        ...
        for method in [DriftDetectionMethod.KOLMOGOROV_SMIRNOV, 
                      DriftDetectionMethod.POPULATION_STABILITY_INDEX,
                      DriftDetectionMethod.JENSEN_SHANNON_DIVERGENCE]:
            result = await self._detect_univariate_drift_with_method(feature_data, method)
    ...
    def _calculate_overall_drift_score(
        ...
        multivariate_result: Optional[MultivariateDriftResult]
    ) -> float:
        """Calculate overall drift score."""
        scores = []
        ...
        if multivariate_result and multivariate_result.drift_detected:
            weighted_scores.append(multivariate_result.drift_score * 2.0)  # Higher weight
...
class StatisticalDriftDetector:
    ...
    def wasserstein_distance(
        ...
        for i in range(reference_data.shape[1]):
            ...
            ref_interp = np.interp(np.linspace(0, 1, n), 
                                 np.linspace(0, 1, len(ref_sorted)), ref_sorted)
            curr_interp = np.interp(np.linspace(0, 1, n), 
                                  np.linspace(0, 1, len(curr_sorted)), curr_sorted)
