...
from pynomaly.domain.exceptions import ExperimentNotFoundError, InvalidExperimentStateError
from pynomaly.shared.protocols import ExperimentRepositoryProtocol, ExperimentRunRepositoryProtocol
...
class ExperimentManagementService:
    ...
    async def create_experiment(
        ...
        objective: str,
        created_by: str,
        optimization_metrics: Optional[List[Dict[str, str]]] = None,
    ...
    async def get_experiment_with_runs(self, experiment_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_run": experiment.get_best_run_info() if experiment.best_run_id else None,
    ...
    def _get_metric_summaries(self, experiment: Experiment, runs: List[ExperimentRun]) -> Dict[str, Dict[str, float]]:
    ...
    async def get_active_experiments(self) -> List[Experiment]:
        ...
        """
        all_experiments = await self.experiment_repository.find_by_status(ExperimentStatus.ACTIVE)
            ...
            "run_timeline": [
                {
                    ...
                    "status": run.status,
                    "key_metrics": {k: v for k, v in run.metrics.items() if k in experiment.optimization_metrics}
                }
