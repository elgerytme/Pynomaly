...
class ApplicationExplainabilityService:
    ...
    async def explain_instance(self, request: ExplanationRequest) -> ExplanationResponse:
        ...
        try:
            ...
            if request.instance_data:
                ...
                instance = np.array([request.instance_data[name] for name in feature_names])
                ...
                if not request.instance_indices or request.instance_indices[0] >= len(dataset.data):
                    return ExplanationResponse(
                        success=False,
    ...
    async def explain_model(self, request: ExplanationRequest) -> ExplanationResponse:
        ...
        try:
            ...
            if len(data) > request.background_samples:
                indices = np.random.choice(len(data), request.background_samples, replace=False)
    ...
    async def explain_cohort(self, request: ExplanationRequest) -> ExplanationResponse:
        ...
        try:
            ...
            # Validate indices
            invalid_indices = [i for i in request.instance_indices if i >= len(dataset.data)]
            if invalid_indices:
                return ExplanationResponse(
                    success=False,
        ...
        try:
            ...
            for idx in indices:
                try:
                    ...
                    logger.warning(f"Failed to generate explanation for instance {idx}: {e}")
                    continue
