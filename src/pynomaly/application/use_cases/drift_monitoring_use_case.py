...
class DriftMonitoringUseCase:
    ...
    async def perform_drift_check(
        ...
        try:
            ...
            if not config:
                raise ValueError(f"No monitoring configuration for detector {detector_id}")
            ...
            if self.metrics_service:
                self.metrics_service.record_error(
                    ...
                    component="drift_monitoring",
                    severity="info" if not result.drift_detected else result.severity.value
                )
    ...
    async def check_performance_drift(
        ...
        try:
            ...
            if result.drift_detected and config and config.should_alert(result.severity):
                await self._handle_drift_alert(result, config)
    ...
    async def get_monitoring_status(self, detector_id: str) -> Optional[DriftMonitoringStatus]:
    ...
    async def resolve_alert(self, alert_id: str, user: str, action: Optional[str] = None) -> bool:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_monitors = len([
                s for s in self.drift_service.monitoring_status.values()
            ...
            health_scores = [
                s.overall_health_score for s in self.drift_service.monitoring_status.values()
            ]
            ...
            recent_drift_count = len([
                s for s in self.drift_service.monitoring_status.values()
                if s.last_drift_detected and 
                (datetime.now() - s.last_drift_detected).days <= 7
            ])
    ...
    async def _update_monitoring_status(
        ...
        if result.drift_detected:
            ...
            health_penalty = 0.1 if result.severity == DriftSeverity.LOW else \
                           0.2 if result.severity == DriftSeverity.MEDIUM else \
                           0.3 if result.severity == DriftSeverity.HIGH else 0.5
            status.overall_health_score = max(0.0, status.overall_health_score - health_penalty)
        else:
