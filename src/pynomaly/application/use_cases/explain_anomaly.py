...
from pynomaly.shared.protocols import ExplainableDetectorProtocol, DetectorRepositoryProtocol
...
class ExplainAnomalyRequest:
    ...
    detector_id: UUID
    dataset: Dataset
    anomaly_indices: Optional[List[int]] = None  # None = explain all anomalies
    ...
    async def _shap_explanation(
        ...
        for i, idx in enumerate(indices):
            feature_contributions = dict(
                zip(dataset.feature_names or [], shap_values[i])
            )
        ...
        global_importance = dict(
            zip(dataset.feature_names or [], np.abs(shap_values).mean(axis=0))
        )
