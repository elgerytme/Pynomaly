...
class ManageActiveLearningUseCase:
    ...
    def select_samples(self, request: SelectSamplesRequest) -> SelectSamplesResponse:
        ...
        if request.sampling_strategy == SamplingStrategy.UNCERTAINTY:
            selected_indices = self.active_learning_service.select_samples_by_uncertainty(
            ...
            if not request.ensemble_results:
                raise ValueError("Ensemble results required for committee disagreement strategy")
            
            selected_indices = self.active_learning_service.select_samples_by_committee_disagreement(
                ensemble_results=request.ensemble_results,
                n_samples=request.n_samples
            )
        
        elif request.sampling_strategy == SamplingStrategy.EXPECTED_MODEL_CHANGE:
            selected_indices = self.active_learning_service.select_samples_by_expected_model_change(
                detection_results=request.detection_results,
                features=request.features,
            ...
            raise ValueError(f"Unsupported sampling strategy: {request.sampling_strategy}")
    ...
    def get_session_status(self, request: SessionStatusRequest) -> SessionStatusResponse:
        """
    ...
    def update_model_with_feedback(self, request: UpdateModelRequest) -> UpdateModelResponse:
        """
        ...
        return UpdateModelResponse(
            ...
            performance_impact={
                ...
                "robustness_increase": min(0.1, update_stats["total_corrections"] * 0.02)
            },
    ...
    def calculate_learning_progress(
        ...
        for session in session_history:
            session_feedback = [
                ...
                if fb.session_id == session.session_id
            ]
        ...
        if len(session_corrections) > 1:
            ...
            improvement_rate = -coefficients[0]  # Negative slope means fewer corrections needed
        else:
        ...
        return {
            ...
            "feedback_consistency": self._calculate_feedback_consistency(feedback_history)
        }
    ...
    def _analyze_feedback_patterns(self, feedback_list: List[HumanFeedback]) -> Dict:
        ...
        time_spent = [fb.time_spent_seconds for fb in feedback_list if fb.time_spent_seconds]
        ...
        return {
            ...
            "high_confidence_rate": confidence_distribution.get("high", 0) / len(feedback_list),
            "expert_feedback_rate": confidence_distribution.get("expert", 0) / len(feedback_list)
        }
    ...
    def _generate_update_recommendations(
        ...
        if update_stats["total_corrections"] > 5:
            recommendations.append("Significant corrections detected - consider retraining")
    ...
    def _calculate_feedback_consistency(self, feedback_history: List[HumanFeedback]) -> float:
