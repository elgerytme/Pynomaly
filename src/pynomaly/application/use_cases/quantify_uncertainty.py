...
class QuantifyUncertaintyUseCase:
    ...
    def execute_ensemble_uncertainty(
        ...
        for i, model_results in enumerate(request.ensemble_results):
            model_uncertainty = self.uncertainty_service.calculate_prediction_uncertainty(
                detection_results=model_results,
                method=request.method
            )
            model_uncertainties.append({
                "model_index": i,
                "uncertainty_metrics": model_uncertainty
            })
    ...
    def _calculate_confidence_intervals(
        ...
        if method == "bootstrap":
            intervals["mean"] = self.uncertainty_service.calculate_bootstrap_confidence_interval(
                scores, confidence_level, statistic_function="mean"
            )
            intervals["std"] = self.uncertainty_service.calculate_bootstrap_confidence_interval(
                scores, confidence_level, statistic_function="std"
            )
        elif method == "normal":
            intervals["mean"] = self.uncertainty_service.calculate_normal_confidence_interval(
                scores, confidence_level
            )
        elif method == "bayesian":
            # Convert scores to binary for Bayesian analysis
            binary_scores = [(score > 0.5) for score in scores]
            intervals["anomaly_rate"] = self.uncertainty_service.calculate_bayesian_confidence_interval(
    ...
    def _calculate_additional_metrics(
        ...
        if include_prediction_intervals:
            prediction_interval = self.uncertainty_service._calculate_prediction_interval(scores)
        ...
        additional_metrics["iqr"] = float(np.percentile(scores, 75) - np.percentile(scores, 25))
