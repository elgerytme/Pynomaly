...
class ExplainabilityResponse:
    ...
    success: bool
    explanation: Optional[Union[LocalExplanation, GlobalExplanation, CohortExplanation]] = None
...
class ExplainabilityUseCase:
    ...
    async def explain_prediction(
        ...
        try:
            ...
            feature_names = await self._get_feature_names(detector, request.background_dataset_id)
            ...
            if request.background_dataset_id:
                background_data = await self._get_background_data(request.background_dataset_id)
            
            # Check cache first
            cache_key = self._generate_cache_key(
                "prediction", request.detector_id, instance_array, request.explanation_method
            )
            ...
            return ExplainabilityResponse(
                success=False,
                error_message=f"Failed to explain prediction: {str(e)}"
            )
    ...
    async def explain_model(
        ...
        try:
            ...
            # Check cache
            cache_key = self._generate_cache_key(
                "model", request.detector_id, request.dataset_id, request.explanation_method
            )
            ...
            return ExplainabilityResponse(
                success=False,
                error_message=f"Failed to explain model: {str(e)}"
            )
    ...
    async def explain_cohort(
        ...
        try:
            ...
            if invalid_indices:
                return ExplainabilityResponse(
                    success=False,
                    error_message=f"Invalid indices: {invalid_indices}"
                )
            
            # Extract cohort data
            cohort_data = dataset.data.iloc[request.instance_indices]
            feature_names = list(dataset.features.columns)
            cohort_id = request.cohort_name or f"cohort_{request.detector_id}_{len(request.instance_indices)}"
            ...
            return ExplainabilityResponse(
                success=False,
                error_message=f"Failed to explain cohort: {str(e)}"
            )
    ...
    async def compare_explanations(
        ...
        try:
            ...
            if request.consistency_analysis and len(explanations) > 1:
                consistency_analysis = await self._analyze_explanation_consistency(explanations)
            ...
            return ExplainabilityResponse(
                success=False,
                error_message=f"Failed to compare explanations: {str(e)}"
            )
    ...
    def _prepare_instance_data(
        self, instance_data: Union[Dict[str, Any], np.ndarray, pd.Series], 
    ...
    async def _explain_with_shap(
        self, detector: Detector, instance: np.ndarray, 
        feature_names: List[str], background_data: Optional[np.ndarray]
    ) -> LocalExplanation:
    ...
    async def _generate_explanation_metadata(
        self, detector: Detector, explanation: LocalExplanation, 
        request: ExplainPredictionRequest
    ) -> Dict[str, Any]:
        """Generate metadata for explanation."""
        return {
            'detector_algorithm': detector.algorithm if hasattr(detector, 'algorithm') else 'unknown',
    ...
    async def _generate_model_metadata(
        ...
        explanation: GlobalExplanation, request: ExplainModelRequest
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            'explanation_coverage': len(explanation.feature_importances) / len(dataset.features.columns)
        }
    
    async def _generate_cohort_metadata(
        self, detector: Detector, cohort_data: pd.DataFrame,
        explanation: CohortExplanation, request: ExplainCohortRequest
    ) -> Dict[str, Any]:
    ...
    async def _generate_counterfactuals(
        self, detector: Detector, instance: np.ndarray, 
        feature_names: List[str], explanation: LocalExplanation
    ) -> List[Dict[str, Any]]:
        ...
        for feature in top_features:
            ...
            if feature.contribution > 0:
                counterfactual_instance[0, feature_idx] *= 0.5  # Reduce positive contribution
            else:
                counterfactual_instance[0, feature_idx] *= -0.5  # Reduce negative contribution
            
            counterfactuals.append({
                'modified_feature': feature.feature_name,
                'original_value': feature.value,
                'counterfactual_value': float(counterfactual_instance[0, feature_idx]),
                'expected_impact': 'Reduced anomaly score' if feature.contribution > 0 else 'Increased anomaly score'
            })
    ...
    async def _analyze_feature_groups(
        ...
        for group_name, features in feature_groups.items():
            ...
            group_analysis[group_name] = {
                ...
                'average_importance': group_importance / len(group_features_found) if group_features_found else 0.0,
    ...
    async def _calculate_cohort_similarity(
        ...
        similarity_matrix = 1 - (distance_matrix / max_distance) if max_distance > 0 else np.ones_like(distance_matrix)
    ...
    async def _analyze_explanation_consistency(
        ...
        for i, method1 in enumerate(methods):
            for method2 in methods[i+1:]:
                # Get common features
                common_features = set(feature_rankings[method1].keys()) & set(feature_rankings[method2].keys())
        ...
        correlation_values = [corr['correlation'] for corr in correlations.values() if not np.isnan(corr['correlation'])]
        ...
        return {
            ...
            'consistency_interpretation': self._interpret_consistency(overall_consistency)
        }
