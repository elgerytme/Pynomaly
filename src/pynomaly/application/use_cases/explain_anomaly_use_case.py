...
class ExplainAnomalyResponse:
    ...
    feature_rankings: Optional[List[Tuple[str, float]]] = None
    cohort_analysis: Optional[Dict[str, Any]] = None
...
class ExplainAnomalyUseCase:
    ...
    async def execute(self, request: ExplainAnomalyRequest) -> ExplainAnomalyResponse:
        ...
        """
        try:
            logger.info(f"Starting anomaly explanation for detector {request.detector_id}")
            ...
            method = method_map.get(request.explanation_method.lower(), ExplanationMethod.SHAP)
            explanations = {}
            ...
            if request.instance_data:
                ...
                response = await self.explainability_service.explain_instance(exp_request)
                if response.success:
            ...
            if request.anomaly_indices and request.dataset_id:
                ...
                for idx in request.anomaly_indices:
                    ...
                    if response.success:
                        ...
                        logger.warning(f"Failed to explain instance {idx}: {response.error}")
            ...
            if request.include_cohort_analysis and request.anomaly_indices and request.dataset_id:
                exp_request = ExplanationRequest(
                    detector_id=request.detector_id,
                    dataset_id=request.dataset_id,
            ...
            if request.compare_methods:
                ...
                comparison_methods = [m for m in available_methods if m != method][:2]  # Limit to 2 additional methods
                
                if comparison_methods and (request.instance_data or request.anomaly_indices):
                    exp_request = ExplanationRequest(
                        detector_id=request.detector_id,
                        dataset_id=request.dataset_id,
                        instance_data=request.instance_data,
                        instance_indices=request.anomaly_indices[:1] if request.anomaly_indices else None,
                        max_features=request.max_features
                    )
                    
                    comparison_results = await self.explainability_service.compare_explanations(
                        exp_request, 
                    ...
                    method_comparison = {
                        method_name: {
                            ...
                            "explanation": result.explanation if result.success else None,
                            "error": result.error,
            ...
            return ExplainAnomalyResponse(
                success=False,
                error=str(e),
                message="Anomaly explanation failed"
            )