...
class EnsembleStrategyDTO(BaseModel):
    ...
    requires_training: bool = Field(default=False, description="Whether strategy requires training")
    supports_weights: bool = Field(default=True, description="Whether strategy supports weights")
    complexity: str = Field(default="medium", description="Computational complexity")
    interpretability: float = Field(default=0.5, ge=0.0, le=1.0, description="Interpretability score")


class DiversityMetricsDTO(BaseModel):
    """DTO for diversity analysis metrics."""
    
    disagreement_measure: float = Field(description="Average disagreement between classifiers")
...
class MetaLearningKnowledgeDTO(BaseModel):
    ...
    dataset_characteristics: Dict[str, Any] = Field(description="Dataset characteristics")
    algorithm_performance: Dict[str, float] = Field(description="Algorithm performance mapping")
    ...
    diversity_requirements: Dict[str, float] = Field(description="Diversity requirements")
    performance_metrics: Dict[str, float] = Field(description="Performance metrics")
    confidence_score: float = Field(description="Confidence in recommendation")
    timestamp: datetime = Field(default_factory=datetime.now, description="Knowledge creation time")
...
class EnsembleConfigurationDTO(BaseModel):
    ...
    ensemble_strategy: str = Field(default="voting", description="Ensemble combination strategy")
    max_ensemble_size: int = Field(default=5, ge=2, le=10, description="Maximum ensemble size")
    min_diversity_threshold: float = Field(default=0.3, ge=0.0, le=1.0, description="Minimum diversity threshold")
    weight_optimization: bool = Field(default=True, description="Enable weight optimization")
    diversity_weighting: float = Field(default=0.3, ge=0.0, le=1.0, description="Diversity vs performance trade-off")
    cross_validation_folds: int = Field(default=3, ge=2, le=10, description="CV folds for validation")
...
class EnsemblePerformanceDTO(BaseModel):
    ...
    weighted_individual_performance: float = Field(description="Weighted average individual performance")
    diversity_score: float = Field(description="Overall diversity score")
    estimated_ensemble_performance: float = Field(description="Estimated ensemble performance")
    performance_improvement: float = Field(description="Performance improvement over individual")
...
class EnsembleReportDTO(BaseModel):
    ...
    dataset_characteristics: Dict[str, Any] = Field(description="Dataset characteristics")
    individual_performance: Dict[str, Dict[str, float]] = Field(description="Individual detector performance")
    diversity_analysis: DiversityMetricsDTO = Field(description="Diversity analysis results")
    ensemble_weights: Dict[str, float] = Field(description="Optimal ensemble weights")
    configuration: EnsembleConfigurationDTO = Field(description="Ensemble configuration")
    performance_summary: EnsemblePerformanceDTO = Field(description="Performance summary")
...
class AlgorithmCompatibilityDTO(BaseModel):
    ...
    compatibility_scores: Dict[str, float] = Field(description="Compatibility scores with other algorithms")
    diversity_potential: float = Field(description="Potential for creating diverse ensembles")
    recommended_combinations: List[str] = Field(description="Recommended algorithm combinations")
...
class EnsembleOptimizationRequestDTO(BaseModel):
    ...
    algorithms: Optional[List[str]] = Field(None, description="Algorithms to consider (None for auto-selection)")
    configuration: EnsembleConfigurationDTO = Field(description="Ensemble configuration")
    meta_learning_enabled: bool = Field(default=True, description="Enable meta-learning")
    optimization_objectives: List[str] = Field(default=["performance", "diversity"], description="Optimization objectives")
...
class EnsembleOptimizationResponseDTO(BaseModel):
    ...
    ensemble_report: EnsembleReportDTO = Field(description="Comprehensive ensemble report")
    optimization_time: float = Field(description="Total optimization time")
    meta_knowledge_updated: bool = Field(description="Whether meta-knowledge was updated")
...
class MetaLearningInsightsDTO(BaseModel):
    ...
    most_popular_algorithms: List[tuple] = Field(description="Most popular algorithms with usage counts")
    average_performance_by_algorithm: Dict[str, float] = Field(description="Average performance by algorithm")
    learning_confidence: float = Field(description="Overall learning confidence")
    diversity_patterns: Dict[str, float] = Field(description="Observed diversity patterns")
    performance_trends: Dict[str, List[float]] = Field(description="Performance trends over time")
    dataset_coverage: Dict[str, int] = Field(description="Coverage of different dataset types")
...
class EnsembleValidationDTO(BaseModel):
    ...
    diversity_validation: DiversityMetricsDTO = Field(description="Diversity validation metrics")
    ...
    generalization_estimate: float = Field(description="Generalization performance estimate")
...
class DynamicEnsembleDTO(BaseModel):
    ...
    selection_strategy: str = Field(default="competence_based", description="Dynamic selection strategy")
    competence_measure: str = Field(default="accuracy", description="Competence measurement method")
    neighborhood_size: int = Field(default=5, description="Neighborhood size for competence estimation")
    competence_threshold: float = Field(default=0.6, description="Minimum competence threshold")
    fallback_strategy: str = Field(default="best_overall", description="Fallback strategy")


class EnsembleEvolutionDTO(BaseModel):
    """DTO for ensemble evolution and adaptation."""
    
    evolution_strategy: str = Field(description="Evolution strategy (genetic, gradient, etc.)")
    mutation_rate: float = Field(default=0.1, description="Mutation rate for evolution")
    population_size: int = Field(default=20, description="Population size for genetic algorithms")
    generations: int = Field(default=50, description="Number of generations")
    fitness_function: str = Field(default="weighted", description="Fitness function definition")
    elitism_ratio: float = Field(default=0.2, description="Elitism ratio for genetic algorithms")


class StakingConfigurationDTO(BaseModel):
    """DTO for stacking ensemble configuration."""
    
    meta_learner_algorithm: str = Field(default="LogisticRegression", description="Meta-learner algorithm")
...
class EnsembleMonitoringDTO(BaseModel):
    ...
    monitoring_enabled: bool = Field(default=True, description="Enable real-time monitoring")
    ...
    alert_thresholds: Dict[str, float] = Field(description="Alert threshold configuration")
    monitoring_interval: int = Field(default=3600, description="Monitoring interval in seconds")
    metric_retention_days: int = Field(default=30, description="Metric retention period")
...
class EnsembleBenchmarkDTO(BaseModel):
    ...
    dataset_characteristics: Dict[str, Any] = Field(description="Benchmark dataset characteristics")
    ensemble_configurations: List[EnsembleConfigurationDTO] = Field(description="Tested configurations")
    ...
    best_configuration: EnsembleConfigurationDTO = Field(description="Best performing configuration")
    improvement_over_single: float = Field(description="Improvement over single best detector")


class MetaFeatureDTO(BaseModel):
    """DTO for meta-features used in meta-learning."""
    
    statistical_features: Dict[str, float] = Field(description="Statistical meta-features")
    information_theoretic_features: Dict[str, float] = Field(description="Information-theoretic features")
    model_based_features: Dict[str, float] = Field(description="Model-based meta-features")
    landmarking_features: Dict[str, float] = Field(description="Landmarking meta-features")
    complexity_features: Dict[str, float] = Field(description="Data complexity features")
...
class EnsembleExplanationDTO(BaseModel):
    ...
    individual_contributions: Dict[str, float] = Field(description="Individual detector contributions")
    weight_explanations: Dict[str, str] = Field(description="Explanation for ensemble weights")
    ...
    alternative_decisions: List[str] = Field(description="Alternative possible decisions")
...
def create_ensemble_performance_from_dict(data: Dict[str, float]) -> EnsemblePerformanceDTO:
    """Create ensemble performance DTO from dictionary."""
    return EnsemblePerformanceDTO(
        weighted_individual_performance=data.get('weighted_individual_performance', 0.0),
