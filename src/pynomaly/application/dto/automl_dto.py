...
class DatasetProfileDTO(BaseModel):
    ...
    contamination_estimate: float = Field(..., description="Estimated contamination rate")
    ...
    time_series_features: List[str] = Field(default_factory=list, description="List of time series features")
    sparsity_ratio: float = Field(..., description="Data sparsity ratio")
    dimensionality_ratio: float = Field(..., description="Dimensionality ratio (features/samples)")
    dataset_size_mb: float = Field(..., description="Dataset size in megabytes")
    has_temporal_structure: bool = Field(default=False, description="Whether dataset has temporal structure")
    has_graph_structure: bool = Field(default=False, description="Whether dataset has graph structure")
...
class AlgorithmRecommendationDTO(BaseModel):
    ...
    recommended_params: Dict[str, Any] = Field(..., description="Recommended hyperparameters")
...
class AutoMLRequestDTO(BaseModel):
    ...
    max_algorithms: int = Field(default=3, ge=1, le=10, description="Maximum number of algorithms to try")
    max_optimization_time: int = Field(default=3600, ge=60, le=86400, description="Maximum optimization time in seconds")
    n_trials: int = Field(default=100, ge=10, le=1000, description="Number of optimization trials")
    enable_ensemble: bool = Field(default=True, description="Whether to enable ensemble creation")
    detector_name: Optional[str] = Field(default=None, description="Optional custom name for the detector")
    cross_validation_folds: int = Field(default=3, ge=2, le=10, description="Number of cross-validation folds")
...
class HyperparameterSpaceDTO(BaseModel):
    ...
    parameter_type: str = Field(..., description="Type of parameter (float, int, categorical)")
    low: Optional[float] = Field(default=None, description="Lower bound for numerical parameters")
    high: Optional[float] = Field(default=None, description="Upper bound for numerical parameters")
    choices: Optional[List[Any]] = Field(default=None, description="Choices for categorical parameters")
...
class EnsembleConfigDTO(BaseModel):
    ...
    algorithms: List[Dict[str, Any]] = Field(..., description="List of algorithms in ensemble")
    ...
    weights: Optional[List[float]] = Field(default=None, description="Algorithm weights")
...
class AutoMLResultDTO(BaseModel):
    ...
    algorithm_rankings: List[tuple] = Field(..., description="Algorithm performance rankings")
    ensemble_config: Optional[EnsembleConfigDTO] = Field(default=None, description="Ensemble configuration if created")
    cross_validation_scores: Optional[List[float]] = Field(default=None, description="Cross-validation scores")
    feature_importance: Optional[Dict[str, float]] = Field(default=None, description="Feature importance scores")
    optimization_history: Optional[List[OptimizationTrialDTO]] = Field(default=None, description="Optimization trial history")
...
class AutoMLResponseDTO(BaseModel):
    ...
    detector_id: Optional[str] = Field(default=None, description="ID of the created detector")
    automl_result: Optional[AutoMLResultDTO] = Field(default=None, description="AutoML optimization result")
    dataset_profile: Optional[DatasetProfileDTO] = Field(default=None, description="Dataset profiling information")
    algorithm_recommendations: Optional[List[AlgorithmRecommendationDTO]] = Field(default=None, description="Algorithm recommendations")
    optimization_summary: Optional[Dict[str, Any]] = Field(default=None, description="Optimization summary")
...
class AutoMLProfileRequestDTO(BaseModel):
    ...
    include_recommendations: bool = Field(default=True, description="Whether to include algorithm recommendations")
    max_recommendations: int = Field(default=5, ge=1, le=10, description="Maximum number of algorithm recommendations")
...
class AutoMLProfileResponseDTO(BaseModel):
    ...
    dataset_profile: Optional[DatasetProfileDTO] = Field(default=None, description="Dataset profile")
    algorithm_recommendations: Optional[List[AlgorithmRecommendationDTO]] = Field(default=None, description="Algorithm recommendations")
    message: str = Field(..., description="Response message")
    error: Optional[str] = Field(default=None, description="Error message if failed")
...
class HyperparameterOptimizationRequestDTO(BaseModel):
    ...
    objective: str = Field(default="auc", description="Optimization objective")
    n_trials: int = Field(default=100, ge=10, le=1000, description="Number of optimization trials")
    timeout: int = Field(default=3600, ge=60, le=86400, description="Optimization timeout in seconds")
    direction: str = Field(default="maximize", description="Optimization direction")
    custom_param_space: Optional[Dict[str, HyperparameterSpaceDTO]] = Field(default=None, description="Custom parameter space")
    cross_validation_folds: int = Field(default=3, ge=2, le=10, description="Number of CV folds")
...
class HyperparameterOptimizationResponseDTO(BaseModel):
    ...
    best_params: Optional[Dict[str, Any]] = Field(default=None, description="Best hyperparameters")
    ...
    optimization_history: Optional[List[OptimizationTrialDTO]] = Field(default=None, description="Trial history")
