...
class UncertaintyRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.n_bootstrap < 100:
            raise ValueError(f"n_bootstrap must be at least 100, got {self.n_bootstrap}")
...
class UncertaintyResponse:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "uncertainty_metrics": {
                key: (value.to_dict() if isinstance(value, ConfidenceInterval) else value)
    ...
    def get_summary(self) -> Dict[str, Union[str, float]]:
        ...
        if "coefficient_of_variation" in self.uncertainty_metrics:
            summary["coefficient_of_variation"] = self.uncertainty_metrics["coefficient_of_variation"]
...
class EnsembleUncertaintyRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(result_counts)) > 1:
            raise ValueError("All models must have the same number of detection results")
...
class EnsembleUncertaintyResponse:
    ...
    def get_summary(self) -> Dict[str, Union[str, float]]:
        ...
        if "aleatoric_uncertainty" in self.ensemble_metrics:
            summary["aleatoric_uncertainty"] = self.ensemble_metrics["aleatoric_uncertainty"]
        
        if "epistemic_uncertainty" in self.ensemble_metrics:
            summary["epistemic_uncertainty"] = self.ensemble_metrics["epistemic_uncertainty"]
...
class PredictionIntervalRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.training_scores) < 10:
            raise ValueError("Need at least 10 training scores for reliable prediction intervals")
