...
class HyperparameterRange:
    ...
    def __post_init__(self):
        ...
        if self.type in [ParameterType.CATEGORICAL, ParameterType.DISCRETE]:
            if not self.choices:
                raise ValueError(f"Choices required for {self.type.value} parameter {self.name}")
        
        elif self.type in [ParameterType.FLOAT, ParameterType.INT]:
            if self.low is None or self.high is None:
                raise ValueError(f"Low and high bounds required for {self.type.value} parameter {self.name}")
            if self.low >= self.high:
                raise ValueError(f"Low bound must be less than high bound for parameter {self.name}")
    ...
    def get_grid_values(self, size: Optional[int] = None) -> List[Any]:
        ...
        if self.type in [ParameterType.CATEGORICAL, ParameterType.DISCRETE, ParameterType.BOOLEAN]:
            return list(self.choices)
        
        elif self.type == ParameterType.FLOAT:
            import numpy as np
            if self.log or self.distribution == DistributionType.LOG_UNIFORM:
                return np.logspace(np.log10(self.low), np.log10(self.high), grid_size).tolist()
            else:
...
class HyperparameterSpace:
    ...
    def __post_init__(self):
        ...
        for param_name, param_range in self.parameters.items():
            if param_range.name != param_name:
                raise ValueError(f"Parameter name mismatch: key '{param_name}' vs name '{param_range.name}'")
    ...
    def validate_values(self, values: Dict[str, Any]) -> Dict[str, bool]:
        ...
        for param_name, param_range in self.parameters.items():
            if param_name in values:
                validation_results[param_name] = param_range.is_valid_value(values[param_name])
            else:
    ...
    def get_grid_combinations(self, max_combinations: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def estimate_grid_size(self) -> int:
        ...
        for param_range in self.parameters.values():
            if param_range.type in [ParameterType.CATEGORICAL, ParameterType.DISCRETE, ParameterType.BOOLEAN]:
    ...
    def get_categorical_parameters(self) -> List[str]:
        ...
        return [
            name for name, param in self.parameters.items()
            if param.type in [ParameterType.CATEGORICAL, ParameterType.DISCRETE, ParameterType.BOOLEAN]
        ]
...
class HyperparameterSet:
    ...
    def __post_init__(self):
        ...
        if self.space:
            if not self.space.are_valid_values(self.parameters):
                invalid_params = [
                    name for name, is_valid in self.space.validate_values(self.parameters).items()
                    if not is_valid
                ]
    ...
    def is_complete_for_space(self, space: HyperparameterSpace) -> bool:
        ...
        return all(param_name in self.parameters for param_name in space.parameters.keys())
    
    def get_missing_parameters(self, space: HyperparameterSpace) -> List[str]:
        """Get list of parameters missing for a given space."""
        return [
            param_name for param_name in space.parameters.keys()
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], space: Optional[HyperparameterSpace] = None) -> 'HyperparameterSet':
        """Create from dictionary representation."""
    ...
    @classmethod
    def from_json(cls, json_str: str, space: Optional[HyperparameterSpace] = None) -> 'HyperparameterSet':
