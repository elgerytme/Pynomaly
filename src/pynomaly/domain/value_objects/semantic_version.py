...
class SemanticVersion:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.major, int) or self.major < 0:
            raise ValueError(f"Major version must be non-negative integer, got {self.major}")
        
        if not isinstance(self.minor, int) or self.minor < 0:
            raise ValueError(f"Minor version must be non-negative integer, got {self.minor}")
        
        if not isinstance(self.patch, int) or self.patch < 0:
            raise ValueError(f"Patch version must be non-negative integer, got {self.patch}")
