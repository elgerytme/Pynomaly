...
class ResourceUsageMetrics:
    ...
    def get_efficiency_score(self) -> float:
        ...
        efficiency = (
            ...
            min(1.0, (self.network_bytes_in + self.network_bytes_out) / (1024**3)) * network_weight +
    ...
    def is_underutilized(self, thresholds: Dict[str, float] = None) -> bool:
        ...
        return (
            self.cpu_utilization_avg < thresholds.get("cpu", 0.2) and
            self.memory_utilization_avg < thresholds.get("memory", 0.3) and
            self.gpu_utilization_avg < thresholds.get("gpu", 0.2)
        )
    ...
    def is_overutilized(self, thresholds: Dict[str, float] = None) -> bool:
        ...
        return (
            self.cpu_utilization_p95 > thresholds.get("cpu", 0.8) or
            self.memory_utilization_p95 > thresholds.get("memory", 0.85) or
            self.gpu_utilization_avg > thresholds.get("gpu", 0.9)
        )
...
class CostBudget:
    ...
    def is_over_budget(self) -> bool:
        ...
        return (
            self.get_monthly_utilization() > 1.0 or
            self.get_annual_utilization() > 1.0
        )
    ...
    def get_triggered_alerts(self) -> List[float]:
        ...
        return [threshold for threshold in self.alert_thresholds if monthly_util >= threshold]
    
    def days_until_budget_exhausted(self) -> Optional[int]:
...
class CostOptimizationPlan:
    ...
    def get_recommendations_by_priority(self) -> List[OptimizationRecommendation]:
        ...
        return sorted(self.recommendations, key=lambda r: r.get_priority_score(), reverse=True)
    ...
    def get_quick_wins(self) -> List[OptimizationRecommendation]:
        ...
        return [
            ...
            if (r.automation_possible and 
                r.annual_savings > 1000 and 
                r.risk_level == "low" and
                r.payback_period_days <= 30)
        ]
    ...
    def get_implementation_phases(self) -> List[List[OptimizationRecommendation]]:
        ...
        phase2 = [r for r in recommendations if r.risk_level == "low" and not r.can_be_automated()]
