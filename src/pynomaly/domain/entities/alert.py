...
class Alert:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.alert_type, AlertType):
            raise TypeError(f"Alert type must be AlertType, got {type(self.alert_type)}")
        ...
        if not isinstance(self.condition, AlertCondition):
            raise TypeError(f"Condition must be AlertCondition, got {type(self.condition)}")
    ...
    def trigger(self, triggered_by: str = "system", context: Optional[Dict[str, Any]] = None) -> None:
        """Trigger the alert."""
    ...
    def suppress(self, suppressed_by: str, reason: str = "", duration_minutes: Optional[int] = None) -> None:
        """Suppress the alert."""
    ...
    def should_escalate(self) -> bool:
        ...
        minutes_since_trigger = (datetime.utcnow() - self.triggered_at).total_seconds() / 60
    ...
    def set_escalation_rules(self, escalation_time_minutes: int, escalation_contacts: List[str]) -> None:
    ...
    def set_suppression_rules(self, duplicate_window_minutes: int = 5, max_notifications_per_hour: int = 10) -> None:
    ...
    def get_timeline(self) -> List[Dict[str, Any]]:
        ...
        if self.acknowledged_at and self.acknowledged_by:
            timeline.append({
                ...
                "details": self.metadata.get("acknowledgment_notes", "Alert acknowledged")
            })
