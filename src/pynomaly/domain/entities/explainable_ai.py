...
class FeatureImportance:
    ...
    importance_type: str = (
        "shap_value"  # shap_value, lime_coefficient, permutation, etc.
    )
...
class ExplanationMetadata:
    ...
    def is_high_quality(self, confidence_threshold: float = 0.8) -> bool:
        ...
        return (
            self.explanation_confidence >= confidence_threshold
            and self.feature_coverage >= 0.9
        )
    ...
    def get_efficiency_score(self) -> float:
        ...
        efficiency = (
            self.feature_coverage * max(1, self.data_sample_size)
...
class GlobalExplanation(ModelExplanation):
    ...
    def __post_init__(self):
        ...
        if self.global_feature_importances:
            self.global_feature_importances.sort(
                key=lambda x: abs(x.importance_value), reverse=True
            )
    ...
    def get_global_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "top_features": [
                f.feature_name for f in self.get_most_important_features(5)
            ],
...
class BiasAnalysis:
    ...
    def get_most_biased_attributes(self, n: int = 3) -> list[tuple[str, float]]:
        ...
        sorted_bias = sorted(
            self.protected_attribute_bias.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def requires_immediate_attention(self) -> bool:
        ...
        return (
            self.overall_bias_score > 0.6
            or any(score > 0.7 for score in self.protected_attribute_bias.values())
            or BiasType.DEMOGRAPHIC_PARITY in self.bias_types_detected
        )
...
class ExplanationResult:
    ...
    explanation_scope: ExplanationScope = ExplanationScope.LOCAL
    success: bool = True
    instance_explanation: InstanceExplanation | None = None
    ...
    def is_high_quality(self) -> bool:
        ...
        if self.instance_explanation:
            quality_indicators.append(
                self.instance_explanation.local_fidelity_score > 0.8
            )
...
class ExplanationValidation:
    ...
    ground_truth_comparison: dict[str, float] | None = None
