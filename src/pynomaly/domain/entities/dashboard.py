...
class VisualizationConfig:
    ...
    def is_real_time(self) -> bool:
        ...
        return self.refresh_interval_seconds is not None and self.refresh_interval_seconds <= 60
...
class DashboardLayout:
    ...
    def get_component_position(self, component_id: str) -> Optional[Dict[str, Any]]:
...
class DashboardFilter:
    ...
    def apply_filter(self, value: Any) -> None:
        """Apply filter value."""
        if self.filter_type == "range" and isinstance(value, (list, tuple)) and len(value) == 2:
    ...
    def is_active(self) -> bool:
        ...
        return self.current_value is not None and self.current_value != self.default_value
...
class Dashboard:
    ...
    title: str = ""
    description: str = ""
    dashboard_type: str = "analytical"  # executive, operational, analytical, performance, compliance
    ...
    last_refresh_duration_seconds: Optional[float] = None
    view_count: int = 0
    ...
    def get_critical_alerts(self) -> List[DashboardAlert]:
        ...
        return [alert for alert in self.alerts if alert.severity == AlertSeverity.CRITICAL and alert.is_active()]
