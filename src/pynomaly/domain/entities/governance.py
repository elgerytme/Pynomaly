...
class GovernanceFramework:
    ...
    def __post_init__(self):
        ...
        if not self.next_review_date:
            self.next_review_date = self.established_date + timedelta(days=30 * self.review_frequency_months)
    ...
    def update_review_date(self) -> None:
        ...
        self.next_review_date = self.last_review_date + timedelta(days=30 * self.review_frequency_months)
...
class AuditFinding:
    ...
    def calculate_risk_score(self) -> float:
        ...
        score = likelihood_scores.get(self.likelihood, 2) * impact_scores.get(self.impact, 2)
    ...
    def is_overdue(self) -> bool:
        ...
        if not self.target_completion_date:
            return False
        return datetime.utcnow() > self.target_completion_date and self.status not in ["resolved", "closed"]
...
class ComplianceAssessment:
    ...
    def calculate_compliance_score(self, findings: List[AuditFinding]) -> float:
        ...
        if max_possible_deductions > 0:
            score = max(0, (max_possible_deductions - total_deductions) / max_possible_deductions * 100)
        else:
            score = 100.0
...
class GovernanceWorkflow:
    ...
    def escalate_workflow(self, escalated_to: str, reason: str = "") -> None:
        ...
        self.add_comment("system", f"Workflow escalated to {escalated_to}. Reason: {reason}")
    ...
    def is_overdue(self) -> bool:
        ...
        if not self.due_date or self.status in [WorkflowStatus.IMPLEMENTED, WorkflowStatus.CLOSED]:
            return False
    ...
    def should_escalate(self) -> bool:
        ...
        return datetime.utcnow() > escalation_due and self.status in [WorkflowStatus.SUBMITTED, WorkflowStatus.UNDER_REVIEW]
