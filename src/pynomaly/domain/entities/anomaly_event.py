...
class AnomalyEventData(BaseModel):
    ...
    model_version: str | None = Field(None, description="Model version that detected the anomaly")
...
class DataQualityEventData(BaseModel):
    ...
    missing_percentage: float | None = Field(None, description="Percentage of missing values")
    outlier_percentage: float | None = Field(None, description="Percentage of outlier values")
    schema_violations: list[str] = Field(default_factory=list, description="Schema violations detected")
    data_drift_score: float | None = Field(None, description="Data drift score if applicable")
...
class PerformanceEventData(BaseModel):
    ...
    degradation_percentage: float = Field(..., description="Performance degradation percentage")
    ...
    potential_causes: list[str] = Field(default_factory=list, description="Potential root causes")
...
class AnomalyEvent(BaseModel):
    ...
    # Source information
    source_session_id: UUID | None = Field(None, description="Source streaming session ID")
    detector_id: UUID | None = Field(None, description="Detector that generated the event")
    data_source: str | None = Field(None, description="Original data source")
    
    # Event data
    title: str = Field(..., description="Event title")
    description: str = Field(..., description="Event description")
    raw_data: dict[str, Any] = Field(..., description="Raw input data that triggered the event")
    
    # Specific event data (polymorphic based on event type)
    anomaly_data: AnomalyEventData | None = Field(None, description="Anomaly-specific data")
    data_quality_data: DataQualityEventData | None = Field(None, description="Data quality-specific data")
    performance_data: PerformanceEventData | None = Field(None, description="Performance-specific data")
    
    # Context information
    business_context: dict[str, Any] = Field(default_factory=dict, description="Business context data")
    technical_context: dict[str, Any] = Field(default_factory=dict, description="Technical context data")
    correlation_id: str | None = Field(None, description="Correlation ID for related events")
    parent_event_id: UUID | None = Field(None, description="Parent event ID for hierarchical events")
    
    # Timing information
    event_time: datetime = Field(..., description="When the event occurred")
    ingestion_time: datetime = Field(default_factory=datetime.utcnow, description="When event was ingested")
    processing_time: datetime | None = Field(None, description="When event was processed")
    
    # Geographic information
    location: dict[str, Any] = Field(default_factory=dict, description="Geographic location data")
    ...
    routing_key: str | None = Field(None, description="Routing key for event processing")
    
    # Acknowledgment and resolution
    acknowledged_by: str | None = Field(None, description="User who acknowledged the event")
    ...
    custom_fields: dict[str, Any] = Field(default_factory=dict, description="Custom application fields")
    ...
    def is_actionable(self) -> bool:
        ...
        return self.severity in [EventSeverity.HIGH, EventSeverity.CRITICAL] and \
               self.status in [EventStatus.PENDING, EventStatus.PROCESSING]
...
class EventFilter(BaseModel):
    ...
    ingestion_time_start: datetime | None = Field(None, description="Ingestion time start")
    ...
    # Geographic filters
    location_bounds: dict[str, Any] | None = Field(None, description="Geographic bounds filter")
...
class EventPattern(BaseModel):
    ...
    # Pattern definition
    pattern_type: str = Field(..., description="Type of pattern (frequency, sequence, correlation)")
    conditions: dict[str, Any] = Field(..., description="Pattern matching conditions")
    time_window: int = Field(..., description="Time window for pattern detection (seconds)")
    ...
    # Actions
    alert_enabled: bool = Field(default=True, description="Whether to alert on pattern match")
...
class EventSummary(BaseModel):
    ...
    avg_resolution_time: float | None = Field(None, description="Average resolution time (seconds)")
    top_detectors: list[dict[str, Any]] = Field(..., description="Top detectors by event count")
    top_data_sources: list[dict[str, Any]] = Field(..., description="Top data sources by event count")
    time_range: dict[str, datetime] = Field(..., description="Time range of events")
    summary_generated_at: datetime = Field(default_factory=datetime.utcnow, description="Summary generation time")