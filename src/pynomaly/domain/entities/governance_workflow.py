...
class ComplianceRule(BaseModel):
    ...
    category: str = Field(..., description="Rule category (security, privacy, performance, etc.)")
    
    # Rule definition
    rule_type: str = Field(..., description="Rule type (metric_threshold, data_requirement, etc.)")
...
class ComplianceViolation(BaseModel):
    ...
    remediation_suggestion: str | None = Field(None, description="Suggested remediation")
...
class ComplianceReport(BaseModel):
    ...
    # Violations
    violations: list[ComplianceViolation] = Field(..., description="Compliance violations")
...
class ApprovalRequest(BaseModel):
    ...
    compliance_report_id: UUID | None = Field(None, description="Associated compliance report")
    ...
    def approve(self, approver: str, comments: str | None = None, conditions: list[str] | None = None) -> None:
        """Approve the request."""
...
class ApprovalWorkflowConfig(BaseModel):
    ...
    # Workflow steps
    required_approvals: list[ApprovalType] = Field(..., description="Required approval types")
    ...
    # Approval rules
    require_all_approvals: bool = Field(True, description="Require all approvals or just majority")
    allow_self_approval: bool = Field(False, description="Allow self-approval")
    require_compliance_check: bool = Field(True, description="Require compliance assessment")
    ...
    notify_on_submission: bool = Field(True, description="Notify on workflow submission")
...
class ApprovalWorkflow(BaseModel):
    ...
    # Approval requests
    approval_requests: list[ApprovalRequest] = Field(..., description="Individual approval requests")
    
    # Compliance
    compliance_report_id: UUID | None = Field(None, description="Associated compliance report")
    compliance_required: bool = Field(True, description="Whether compliance check is required")
    ...
    # Results
    final_decision: ApprovalStatus | None = Field(None, description="Final workflow decision")
    ...
    def submit_for_approval(self, submitter: str) -> None:
        ...
        if self.config.default_due_days > 0:
            self.due_date = datetime.utcnow() + timedelta(days=self.config.default_due_days)

    def process_approval(self, request_id: UUID, status: ApprovalStatus, approver: str, 
                        comments: str | None = None, conditions: list[str] | None = None) -> None:
    ...
    def _check_workflow_completion(self) -> None:
        ...
        rejected_requests = [r for r in self.approval_requests if r.status == ApprovalStatus.REJECTED]
