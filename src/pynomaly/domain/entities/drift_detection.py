...
class TimeWindow:
    ...
    def get_overlap_duration(self) -> timedelta:
        ...
        return timedelta(seconds=self.window_size.total_seconds() * self.overlap_percentage)
    ...
    def _compute_statistical_properties(self) -> Dict[str, float]:
        ...
        if self.data_type == "numerical":
            ...
            return {
                ...
                "ref_mode_frequency": float(self._get_mode_frequency(self.reference_data)),
                "curr_mode_frequency": float(self._get_mode_frequency(self.current_data))
            }
...
class MultivariateDriftResult:
    ...
    spatial_drift_map: Optional[np.ndarray] = None
...
class FeatureDriftAnalysis:
    ...
    temporal_drift_pattern: Optional[Dict[str, Any]] = None
    ...
    stability_score: float = 1.0  # Overall stability (1.0 = very stable, 0.0 = very unstable)
    ...
    def get_strongest_drift_signal(self) -> Optional[UnivariateDriftResult]:
...
class DriftAnalysisResult:
    ...
    analysis_timestamp: datetime = field(default_factory=datetime.utcnow)
    time_window: TimeWindow = field(default_factory=lambda: TimeWindow(
        start_time=datetime.utcnow() - timedelta(days=1),
        end_time=datetime.utcnow(),
        window_size=timedelta(hours=1)
    ))
    data_drift_results: List[UnivariateDriftResult] = field(default_factory=list)
    multivariate_drift_result: Optional[MultivariateDriftResult] = None
    ...
    def _compute_overall_drift_score(self) -> float:
        ...
        if self.multivariate_drift_result and self.multivariate_drift_result.drift_detected:
    ...
    def has_any_drift(self) -> bool:
        ...
        has_multivariate_drift = (self.multivariate_drift_result and 
                                self.multivariate_drift_result.drift_detected)
        has_concept_drift = (self.concept_drift_result and 
                           self.concept_drift_result.drift_detected)
        has_feature_drift = any(a.has_drift() for a in self.feature_analyses.values())
        
        return any([has_data_drift, has_multivariate_drift, has_concept_drift, has_feature_drift])
    ...
    def needs_immediate_action(self) -> bool:
        ...
        return (self.drift_severity in ["high"] or 
                len(self.get_critical_features()) > 0 or
                (self.concept_drift_result and 
                 self.concept_drift_result.drift_probability > 0.8))
    ...
    def get_comprehensive_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_window": {
                ...
                "duration_hours": self.time_window.get_duration().total_seconds() / 3600
            },
            "overall": {
            ...
            "data_drift": {
                ...
                "features_with_drift": sum(1 for r in self.data_drift_results if r.drift_detected),
                "critical_features": self.get_critical_features()
            },
            "multivariate_drift": {
                "detected": self.multivariate_drift_result.drift_detected if self.multivariate_drift_result else False,
                "score": self.multivariate_drift_result.drift_score if self.multivariate_drift_result else 0.0
            },
            "concept_drift": {
                "detected": self.concept_drift_result.drift_detected if self.concept_drift_result else False,
                "probability": self.concept_drift_result.drift_probability if self.concept_drift_result else 0.0
            },
