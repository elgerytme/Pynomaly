...
class Experiment:
    ...
    metrics_to_optimize: List[Dict[str, str]] = field(default_factory=list)  # [{"metric": "f1", "direction": "maximize"}]
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.experiment_type, ExperimentType):
            raise TypeError(f"Experiment type must be ExperimentType, got {type(self.experiment_type)}")
    ...
    def _calculate_run_score(self, run: ExperimentRun) -> Optional[float]:
    ...
    def add_optimization_metric(self, metric_name: str, direction: str = "maximize") -> None:
    ...
    def remove_optimization_metric(self, metric_name: str) -> None:
        ...
        self.metrics_to_optimize = [
            m for m in self.metrics_to_optimize 
        ...
        for metric in all_metrics:
            ...
            if val1 is not None and val2 is not None:
                metric_comparison[metric] = {
                    ...
                    "percent_change": ((val1 - val2) / val2) * 100 if val2 != 0 else None
                }
            else:
                metric_comparison[metric] = {
                    "run1": val1,
