...
class ExplainableAIService:
    ...
    async def explain_anomalies(
        ...
        try:
            ...
            anomaly_indices = [
                anomaly.index
                for anomaly in detection_result.anomalies[:max_explanations]
            ]
            anomaly_scores = [
                anomaly.score.value
                for anomaly in detection_result.anomalies[:max_explanations]
            ...
            result.explanation_quality = await self._calculate_explanation_quality(
                result, X
            )

            logger.info(
                f"Generated {method.value} explanations in {result.computation_time:.2f}s"
            )
    ...
    async def _explain_with_shap(
        ...
        result = ExplanationResult(
            method=ExplanationMethod.SHAP, explanation_type=explanation_type
        )

        try:
            # Create a simple surrogate model for SHAP
            # In practice, you'd use the actual anomaly detection model
            surrogate_model = self._create_surrogate_model(
                X, anomaly_indices, anomaly_scores
            )
            ...
            if explanation_type == ExplanationType.LOCAL:
                ...
                for i, (idx, score) in enumerate(zip(anomaly_indices, anomaly_scores, strict=False)):
                    ...
                    feature_contributions.sort(
                        key=lambda x: abs(x.importance_score), reverse=True
                    )
            ...
            logger.warning(
                f"SHAP explanation failed, falling back to basic method: {e}"
            )
            result = await self._basic_feature_importance(
                X, feature_names, anomaly_indices, anomaly_scores, explanation_type
            )
    ...
    async def _explain_with_lime(
        ...
        result = ExplanationResult(
            method=ExplanationMethod.LIME, explanation_type=explanation_type
        )
        ...
        try:
            ...
            if explanation_type == ExplanationType.LOCAL:
                ...
                for i, (idx, score) in enumerate(zip(anomaly_indices, anomaly_scores, strict=False)):
                    ...
                    for feature_name, importance_score in explanation.as_list():
                        ...
                        original_name = (
                            feature_name.split("<=")[0].split(">")[0].strip()
                        )
            ...
            logger.warning(
                f"LIME explanation failed, falling back to basic method: {e}"
            )
            result = await self._basic_feature_importance(
                X, feature_names, anomaly_indices, anomaly_scores, explanation_type
            )
    ...
    async def _explain_with_permutation_importance(
        ...
        try:
            ...
            perm_importance = permutation_importance(
                surrogate_model, X, y, n_repeats=10, random_state=42, scoring="roc_auc"
            )
            ...
            if explanation_type == ExplanationType.GLOBAL:
                result.global_explanation = GlobalExplanation(
                    ...
                    model_behavior_summary={
                        "total_importance": float(
                            np.sum(perm_importance.importances_mean)
                        ),
                        "importance_std": float(
                            np.mean(perm_importance.importances_std)
                        ),
                ...
                for i, (idx, score) in enumerate(zip(anomaly_indices, anomaly_scores, strict=False)):
                    ...
                    for j, feature_imp in enumerate(global_features):
                        ...
                        feature_value = X[idx][
                            feature_names.index(feature_imp.feature_name)
                        ]
                        local_importance = feature_imp.importance_score * abs(
                            feature_value
                        )
    ...
    async def _explain_with_feature_ablation(
        ...
        result = ExplanationResult(
            method=ExplanationMethod.FEATURE_ABLATION, explanation_type=explanation_type
        )
        ...
        if explanation_type == ExplanationType.GLOBAL:
            result.global_explanation = GlobalExplanation(
                ...
                model_behavior_summary={
                    "total_importance": float(
                        sum(f.importance_score for f in feature_importances)
                    ),
                    "max_importance": float(
                        max(f.importance_score for f in feature_importances)
                    ),
    ...
    async def _basic_feature_importance(
        ...
        for i, feature_name in enumerate(feature_names):
            ...
            normal_values = [
                X[j, i] for j in normal_indices[: len(anomaly_values)]
            ]  # Sample same size
            ...
            if len(anomaly_values) > 0 and len(normal_values) > 0:
                ...
                importance_score = abs(anomaly_mean - normal_mean) / (
                    np.std(X[:, i]) + 1e-8
                )
            else:
        ...
        if explanation_type == ExplanationType.GLOBAL:
            ...
            for i, (idx, score) in enumerate(zip(anomaly_indices, anomaly_scores, strict=False)):
                ...
                for feature_imp in feature_importances:
                    ...
                    normalized_value = abs(feature_value - feature_mean) / (
                        feature_std + 1e-8
                    )
                ...
                local_explanation = LocalExplanation(
                    ...
                    prediction_value=score,
                    feature_values={
                        name: float(X[idx][j]) for j, name in enumerate(feature_names)
                    },
                )
    ...
    async def _calculate_explanation_quality(
            ...
            if len(feature_importance_lists) > 1:
                ...
                for i in range(len(feature_importance_lists)):
                    for j in range(i + 1, len(feature_importance_lists)):
                        common_features = set(feature_importance_lists[i].keys()) & set(
                            feature_importance_lists[j].keys()
                        )
                        if len(common_features) > 1:
                            importance1 = [
                                feature_importance_lists[i][f] for f in common_features
                            ]
                            importance2 = [
                                feature_importance_lists[j][f] for f in common_features
                            ]
        ...
        if explanation_result.global_explanation:
            total_importance = sum(
                abs(f.importance_score)
                for f in explanation_result.global_explanation.global_feature_importance
            )
        ...
        if explanation_result.global_explanation:
            ...
            total_features = len(
                explanation_result.global_explanation.global_feature_importance
            )
    ...
    async def generate_counterfactuals(
        ...
        for i in range(n_counterfactuals):
            ...
            features_to_modify = np.random.choice(
                len(feature_names), size=n_features_to_modify, replace=False
            )
            ...
            counterfactual_dict = {
                ...
                "distance_from_original": float(
                    np.linalg.norm(counterfactual - original_sample)
                ),
            }
    ...
    def _extract_tree_rules(
        ...
        def get_rules(node_id, depth, condition):
            ...
            left_condition = (
                f"{condition} AND {feature_name} <= {threshold:.3f}"
                if condition
                else f"{feature_name} <= {threshold:.3f}"
            )
