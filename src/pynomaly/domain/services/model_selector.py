...
class ModelSelector:
    ...
    def select_best_model(self, candidates) -> dict[str, Any] | None:
        """
        ...
        if hasattr(candidates[0], "model_id"):
            ...
            best_candidate = max(
                candidates, key=lambda x: x.metrics.get(self.primary_metric, 0.0)
            )
            return {
                "selected_model": best_candidate.model_id,
                "algorithm": best_candidate.algorithm,
                "metrics": best_candidate.metrics,
                "primary_metric_value": best_candidate.metrics.get(
            ...
            best_candidate = max(
                candidates,
                key=lambda x: getattr(x, "metrics", {}).get(self.primary_metric, 0.0),
            )
            return {
                ...
                "primary_metric_value": getattr(best_candidate, "metrics", {}).get(
                    self.primary_metric, 0.0
                ),
            }
    ...
    def compare_models(
        self, model1: ModelCandidate, model2: ModelCandidate
    ) -> dict[str, Any]:
        """
        Compare two models.
