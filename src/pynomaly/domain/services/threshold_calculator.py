...
class ThresholdCalculator:
    ...
    def _find_largest_gap(
        ...
        for i in range(min_anomalies - 1, min(max_anomalies - 1, len(sorted_scores) - 1)):
    ...
    def _find_slope_change(
        ...
        for i in range(min_anomalies, max_anomalies - 1):
            ...
            if i > 0 and i < len(sorted_scores) - 1:
                second_deriv = sorted_scores[i - 1] - 2 * sorted_scores[i] + sorted_scores[i + 1]
