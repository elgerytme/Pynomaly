...
class MetricsCalculator:
    ...
    def compute_cross_validation_metrics(
        ...
        for fold_results in cv_results:
            for metric_name in fold_results.keys():
                if (
                    isinstance(fold_results[metric_name], dict)
                    and "value" in fold_results[metric_name]
                ):
    ...
    def compare_models(
        ...
        comparison = {"rankings": {}, "comparisons": {}, "summary": {}}
        ...
        for model_name, results in model_results.items():
            if primary_metric in results:
                if (
                    isinstance(results[primary_metric], dict)
                    and "value" in results[primary_metric]
                ):
        ...
        for i, model1 in enumerate(model_names):
            for j, model2 in enumerate(model_names[i + 1 :], i + 1):
                ...
                common_metrics = set(model_results[model1].keys()) & set(
                    model_results[model2].keys()
                )
                metric_comparisons = {}
