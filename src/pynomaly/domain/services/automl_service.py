...
from .advanced_detection_service import (
    AdvancedDetectionService, DetectionAlgorithm, AlgorithmConfig,
    EnsembleConfig, ScalingMethod, get_detection_service
)
...
class OptimizationConfig:
    ...
    secondary_metrics: List[OptimizationMetric] = field(default_factory=lambda: [OptimizationMetric.AUC])
...
class OptimizationResult:
    ...
    top_k_results: List[Tuple[DetectionAlgorithm, AlgorithmConfig, float]] = field(default_factory=list)
...
class AutoMLService:
    ...
    def _initialize_parameter_spaces(self) -> Dict[DetectionAlgorithm, Dict[str, Any]]:
        ...
        if True:  # Assuming PyOD might be available
            ...
            spaces[DetectionAlgorithm.AUTO_ENCODER] = {
                "hidden_neurons": [[32, 16, 16, 32], [64, 32, 32, 64], [128, 64, 64, 128]],
                "epochs": [50, 100, 200],
                "batch_size": [16, 32, 64],
    ...
    async def optimize_detection(
        ...
        try:
            ...
            available_algorithms = await self.detection_service.get_available_algorithms()

            # Filter algorithms if specified
            if optimization_config.algorithms_to_test:
                algorithms_to_test = [
                    alg for alg in optimization_config.algorithms_to_test
                    if alg in available_algorithms
                ]
            else:
                # Exclude ensemble methods from individual optimization
                algorithms_to_test = [
                    alg for alg in available_algorithms
            ...
            logger.info(f"Starting AutoML optimization with {len(algorithms_to_test)} algorithms")
            ...
            if optimization_config.ensemble_methods and len(algorithms_to_test) >= 2:
                ensemble_result = await self._optimize_ensemble(
                    dataset, algorithms_to_test, optimization_config, ground_truth, result
                )
            ...
            logger.info(f"AutoML optimization completed in {result.optimization_time:.2f}s")
            logger.info(f"Best algorithm: {result.best_algorithm.value} (score: {result.best_score:.4f})")
    ...
    async def _bayesian_optimization(
        ...
        def objective(trial: Trial) -> float:
            ...
            try:
                ...
                score, metrics = loop.run_until_complete(
                    self._evaluate_configuration(dataset, algo_config, config, ground_truth)
                )
        ...
        # Optimize
        study.optimize(objective, n_trials=config.max_trials, timeout=config.timeout_seconds)
    ...
    async def _optimize_ensemble(
        ...
        top_algorithms = [result[0] for result in individual_results.top_k_results[:config.max_ensemble_size]]
        ...
        for ensemble_size in range(2, min(len(top_algorithms) + 1, config.max_ensemble_size + 1)):
            for algorithm_combo in itertools.combinations(top_algorithms, ensemble_size):
                    ...
                    try:
                        ...
                        for alg in algorithm_combo:
                            ...
                            best_config_for_alg = next(
                                (result[1] for result in individual_results.top_k_results if result[0] == alg),
    ...
    async def _quick_algorithm_selection(self, dataset: Dataset) -> Tuple[DetectionAlgorithm, AlgorithmConfig]:
    ...
    async def get_optimization_recommendations(
        self,
        dataset: Dataset,
        current_results: Optional[DetectionResult] = None
    ) -> Dict[str, Any]:
