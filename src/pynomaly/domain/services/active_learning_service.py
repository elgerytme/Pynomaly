...
class ActiveLearningService:
    ...
    def calculate_annotation_value(
        ...
        novelty_value = self._calculate_novelty_value(
    ...
    def update_model_with_feedback(
        ...
        if not feedback_list:
            return {"total_corrections": 0, "average_confidence": 0.0, "update_magnitude": 0.0}
        ...
        if corrections:
            ...
            update_magnitude = np.mean(correction_magnitudes) if correction_magnitudes else 0.0
        else:
    ...
    def _select_by_kmeans_diversity(self, features: np.ndarray, n_samples: int) -> List[int]:
    ...
    def _select_by_max_distance(self, features: np.ndarray, n_samples: int) -> List[int]:
    ...
    def _calculate_diversity_scores(self, features: np.ndarray) -> np.ndarray:
        ...
        if np.max(diversity_scores) > np.min(diversity_scores):
            diversity_scores = (
                (diversity_scores - np.min(diversity_scores)) /
    ...
    def _calculate_diversity_value(
        ...
        for annotated_idx in annotated_indices:
            if annotated_idx < features.shape[0]:
                distance = np.linalg.norm(
                    sample_features - features[annotated_idx]
                )
                min_distance = min(min_distance, distance)
