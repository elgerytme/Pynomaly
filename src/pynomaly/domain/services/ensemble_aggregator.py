...
class EnsembleAggregator:
    ...
    def aggregate_scores(
        ...
        for name, scores in scores_dict.items():
            if len(scores) != n_samples:
                raise ValueError(
                    f"Detector '{name}' has {len(scores)} scores, "
    ...
    @staticmethod
    def _average_scores(scores_dict: Dict[str, List[AnomalyScore]]) -> List[AnomalyScore]:
        ...
        for i in range(n_samples):
            ...
            if all(scores[i].is_confident for scores in scores_dict.values()):
                lower_values = [scores[i].confidence_lower for scores in scores_dict.values()]
                upper_values = [scores[i].confidence_upper for scores in scores_dict.values()]
    ...
    @staticmethod
    def _median_scores(scores_dict: Dict[str, List[AnomalyScore]]) -> List[AnomalyScore]:
    ...
    def _max_scores(scores_dict: Dict[str, List[AnomalyScore]]) -> List[AnomalyScore]:
        ...
        for i in range(n_samples):
            ...
            aggregated.append(
                AnomalyScore(value=max_score, method="ensemble_max")
            )
