...
class StatisticalTester:
    ...
    def _extract_metric_value(self, metrics: Dict[str, Any], metric_name: str) -> Optional[Any]:
    ...
    def _perform_paired_ttest(self, sample_a: np.ndarray, sample_b: np.ndarray) -> TestResult:
    ...
    def _perform_wilcoxon_test(self, sample_a: np.ndarray, sample_b: np.ndarray) -> TestResult:
    ...
    def _perform_mannwhitney_test(self, sample_a: np.ndarray, sample_b: np.ndarray) -> TestResult:
    ...
    def comprehensive_comparison(
        ...
        if all_p_values:
            ...
            for comparison_key in comparison_keys:
                for metric_name in metric_names:
                    original_result = results['pairwise_comparisons'][comparison_key][metric_name]
                    ...
                    results['pairwise_comparisons'][comparison_key][metric_name] = corrected_result
                    idx += 1
        ...
        for metric_name in metric_names:
            ...
            for i, model_a in enumerate(model_names):
                for j, model_b in enumerate(model_names):
                    if i != j:
                        comparison_key = f"{model_a}_vs_{model_b}" if i < j else f"{model_b}_vs_{model_a}"
                        if comparison_key in results['pairwise_comparisons']:
                            significance_matrix[i, j] = int(
                                results['pairwise_comparisons'][comparison_key][metric_name].significant
                            )
