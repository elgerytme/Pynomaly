...
class UncertaintyQuantificationService:
    ...
    def calculate_prediction_uncertainty(
        ...
        uncertainty_metrics["coefficient_of_variation"] = float(np.std(scores) / np.mean(scores))
        ...
        uncertainty_metrics["prediction_interval"] = self._calculate_prediction_interval(
            scores
        )
    ...
    def calculate_highest_density_interval(
        ...
        interval_widths = sorted_samples[n_include-1:] - sorted_samples[:n-n_include+1]
