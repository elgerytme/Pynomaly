...
class AdvancedDetectionService:
    ...
    async def detect_anomalies(
        ...
        try:
            ...
            if config is None:
                ...
                if not config:
                    raise ValueError(f"No default configuration for algorithm: {algorithm}")
            ...
            # Fit and predict
            predictions, scores = await self._fit_and_predict(model, X, algorithm, config)
            ...
            metrics = await self._calculate_metrics(
                algorithm.value, execution_time, memory_usage,
    ...
    async def _prepare_data(self, dataset: Dataset, config: AlgorithmConfig) -> Union[np.ndarray, pd.DataFrame]:
    ...
    async def _create_model(self, algorithm: DetectionAlgorithm, config: AlgorithmConfig):
        ...
        if not PYOD_AVAILABLE and algorithm in [
            DetectionAlgorithm.KNN, DetectionAlgorithm.ABOD, DetectionAlgorithm.AUTO_ENCODER,
            DetectionAlgorithm.VAE, DetectionAlgorithm.OCSVM, DetectionAlgorithm.PCA,
            DetectionAlgorithm.MCD, DetectionAlgorithm.LODA, DetectionAlgorithm.COPOD,
    ...
    def _calculate_dbscan_scores(self, X: np.ndarray, clusters: np.ndarray, model) -> np.ndarray:
    ...
    async def detect_anomalies_ensemble(
        ...
        try:
            ...
            for config in ensemble_config.algorithms:
                result = await self.detect_anomalies(dataset, config.algorithm, config, ground_truth)
            ...
            anomalies = []
            for i, (is_anomaly, score) in enumerate(zip(combined_predictions, combined_scores)):
                if is_anomaly:
                    anomaly = Anomaly(
                        index=i,
                        score=AnomalyScore(score),
            ...
            result = DetectionResult(
                ...
                anomaly_count=len(anomalies),
                contamination_rate=ContaminationRate(len(anomalies) / len(combined_predictions)),
                execution_time=execution_time,
                metadata={
                    "ensemble_config": {
                        "algorithms": [config.algorithm.value for config in ensemble_config.algorithms],
    ...
    def _combine_ensemble_results(
        ...
        if config.combination_method == "voting":
            # Majority voting
            combined_predictions = (np.sum(predictions_array, axis=0) > len(all_predictions) / 2).astype(int)
    ...
    async def benchmark_algorithms(
        ...
        if algorithms is None:
            ...
            algorithms = [alg for alg in algorithms if not alg.value.startswith("ensemble_")]
        ...
        for algorithm in algorithms:
            try:
                ...
                result = await self.detect_anomalies(dataset, algorithm, ground_truth=ground_truth)
