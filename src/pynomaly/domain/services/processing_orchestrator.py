...
from ...infrastructure.streaming.stream_processor import (
    StreamProcessor, StreamConfig, StreamSource, StreamFormat, create_stream_processor
)
from ...infrastructure.batch.batch_processor import (
    BatchProcessor, BatchConfig, BatchEngine, DataFormat, create_batch_processor
)
...
class ProcessingOrchestrator:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        # Global settings
        self.max_concurrent_sessions = self.config.get("processing.max_concurrent_sessions", 10)
        self.enable_auto_scaling = self.config.get("processing.enable_auto_scaling", True)
    ...
    async def start_batch_session(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Started batch session {session_id} ({name}) with job {job_id}")
            return session_id
    ...
    async def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if session.mode == ProcessingMode.STREAMING and session_id in self._stream_processors:
            processor_status = self._stream_processors[session_id].get_status()
            status_data["streaming_details"] = processor_status
        
        elif session.mode == ProcessingMode.BATCH and session_id in self._batch_processors:
            batch_processor = self._batch_processors[session_id]
            job_statuses = []
            for job_id in session.job_ids:
    ...
    async def list_sessions(self, mode: Optional[ProcessingMode] = None) -> List[Dict[str, Any]]:
    ...
    async def get_processing_recommendations(
        ...
        if data_size_mb < 100:  # Small datasets
            ...
            recommendations["reasoning"].append("Small dataset suitable for streaming processing")
            ...
            recommendations["reasoning"].append("Large dataset requires batch processing for efficiency")
            ...
            if latency_requirements == "low":
                ...
                recommendations["reasoning"].append("Low latency requirements favor streaming")
            else:
                recommendations["recommended_mode"] = ProcessingMode.BATCH.value
                recommendations["configuration"] = {
                ...
                recommendations["reasoning"].append("Medium dataset with normal latency requirements")
        ...
        if expected_throughput > 10000:  # High throughput
            if recommendations["recommended_mode"] == ProcessingMode.STREAMING.value:
                ...
                recommendations["reasoning"].append("High throughput requires more workers and backpressure")
            else:
                recommendations["configuration"]["engine"] = BatchEngine.DASK.value
                recommendations["configuration"]["max_workers"] = 12
                recommendations["reasoning"].append("High throughput batch processing with Dask")
        
        # Add alternatives
        if recommendations["recommended_mode"] == ProcessingMode.STREAMING.value:
            recommendations["alternatives"].append({
                "mode": ProcessingMode.BATCH.value,
                "pros": ["Better for large datasets", "More fault tolerant", "Better resource utilization"],
                "cons": ["Higher latency", "More complex setup"]
            })
        else:
            recommendations["alternatives"].append({
                "mode": ProcessingMode.STREAMING.value,
                "pros": ["Lower latency", "Real-time processing", "Simpler for small data"],
        ...
        if data_size_mb > 1000 and expected_throughput > 5000:
            recommendations["alternatives"].append({
                ...
                "pros": ["Combines benefits of both modes", "Flexible processing", "Handles mixed workloads"],
                ...
                if session.mode == ProcessingMode.STREAMING and session_id in self._stream_processors:
                    ...
                    for job_id in session.job_ids:
                        ...
                        if job_status:
                            ...
                            session.total_anomalies = job_status.get("total_anomalies", 0)
        ...
        for session_id, session in self.sessions.items():
            ...
            try:
                ...
                if session.started_at:
                    ...
                    if runtime > 3600 and session.total_records == 0:  # 1 hour with no records
                        logger.warning(f"Session {session_id} appears unhealthy - no records processed in 1 hour")
                        session.status = ProcessingStatus.ERROR
    ...
    async def get_system_metrics(self) -> Dict[str, Any]:
        ...
        running_sessions = len([s for s in self.sessions.values() if s.status == ProcessingStatus.RUNNING])
...
def get_processing_orchestrator(config: Optional[Config] = None) -> ProcessingOrchestrator:
