...
class StashProcessor:
    ...
    def apply_stash(self, stash_ref):
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "stash", "apply", stash_ref], capture_output=True, text=True
            )
            if result.returncode == 0:
                ...
                if "CONFLICT" in result.stderr or "conflict" in result.stderr:
                    ...
                    subprocess.run(
                        ["git", "checkout", "--theirs", "."], capture_output=True
                    )
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                patch_result = subprocess.run(
                    ["git", "apply", "--ignore-whitespace", "--3way"],
                    input=result.stdout,
                    capture_output=True,
                    text=True,
                )
    ...
    def cleanup_branch(self, branch_name):
        ...
        try:
            ...
            subprocess.run(
                ["git", "branch", "-D", branch_name], check=True, capture_output=True
            )
        except subprocess.CalledProcessError as e:
        ...
        try:
            ...
            if not applied:
                self.update_audit_file(
                    index, hash_val, "FAILED", f"Could not apply stash: {apply_notes}"
                )
            ...
            if tests_passed and is_relevant:
                ...
                if self.commit_changes(stash_ref, description):
                    ...
                    self.update_audit_file(
                        index, hash_val, "COMMITTED", f"Tests passed. {test_notes}"
                    )
                    # Keep branch for potential PR
                else:
                    print(f"✗ {stash_ref} failed to commit")
                    self.update_audit_file(
                        index, hash_val, "FAILED", "Could not commit changes"
                    )
                    self.cleanup_branch(branch_name)
            else:
                ...
                self.update_audit_file(
                    ...
                    hash_val,
                    "DROPPED",
                    f"Tests failed or not relevant: {test_notes}",
                )
                self.cleanup_branch(branch_name)
