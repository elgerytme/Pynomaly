#!/bin/bash
"""
Unified CI Pipeline Runner for anomaly_detection.
...
export ANOMALY_DETECTION_ENVIRONMENT="ci"
...
check_prerequisites() {
    ...
    local required_dirs=("src/anomaly_detection" "tests" "scripts/ci")
...
start_test_services() {
    ...
    docker run -d \
        --name anomaly_detection-test-postgres \
    ...
    docker run -d \
        --name anomaly_detection-test-redis \
        -p 6379:6379 \
        redis:7-alpine || true
    ...
    # Health check
    for i in {1..30}; do
        if docker exec anomaly_detection-test-postgres pg_isready -U anomaly_detection &> /dev/null; then
    ...
    for i in {1..30}; do
        if docker exec anomaly_detection-test-redis redis-cli ping &> /dev/null; then
...
stop_test_services() {
    ...
    docker stop anomaly_detection-test-postgres anomaly_detection-test-redis &> /dev/null || true
    docker rm anomaly_detection-test-postgres anomaly_detection-test-redis &> /dev/null || true
}
...
build_docker_images() {
    ...
    docker build \
        ...
        -t anomaly_detection:ci-test \
        "${PROJECT_ROOT}"

    # Test image
    docker run --rm \
        --name anomaly_detection-ci-test \
        -e ANOMALY_DETECTION_ENVIRONMENT=test \
        anomaly_detection:ci-test \
        python -c "import anomaly_detection; print('✅ Docker image test passed')"
...
run_security_scans() {
    ...
    if command -v trivy &> /dev/null; then
        trivy image \
            ...
            anomaly_detection:ci-test || true
...
# 🚀 anomaly_detection CI Pipeline Report
...
Generated by anomaly_detection CI Pipeline on $(date)
EOF
...
main() {
    log "INFO" "🚀 Starting anomaly_detection CI Pipeline..."
...
case "${1:-}" in
    ...
    "security")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "anomaly_detection CI Pipeline Runner"
        echo ""
