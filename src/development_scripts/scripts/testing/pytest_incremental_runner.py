...
class PytestIncrementalRunner:
    ...
    def run_pytest_command(self, command: str) -> PytestResult:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✓ pytest completed ({duration:.2f}s, {tests_passed}/{tests_run} passed)"
                )
            else:
                logger.error(
                    f"✗ pytest failed ({duration:.2f}s, {tests_failed}/{tests_run} failed)"
                )
    ...
    def run_incremental_tests(
        ...
        pytest_commands = self.get_pytest_commands_for_targets(
            change_analysis.test_targets
        )
    ...
    def save_results(
        self, summary: IncrementalPytestSummary, output_file: Path = None
    ) -> Path:
        """Save test results to JSON file."""
        if output_file is None:
            timestamp = int(time.time())
            output_file = (
                self.repo_root / f"pytest_incremental_results_{timestamp}.json"
            )
...
def main():
    ...
    try:
        ...
        summary = runner.run_incremental_tests(
            base_commit=args.base, target_commit=args.target, fail_fast=args.fail_fast
        )
