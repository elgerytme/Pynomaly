...
class AlertingDemo:
        ...
        for rule in rules:
            try:
                response = self.session.post(
                    f"{self.base_url}/alerting/rules", json=rule
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Created rule: {created_rule['name']} (ID: {created_rule['id']})"
                    )
                else:
                    logger.error(
                        f"Failed to create rule {rule['name']}: {response.text}"
                    )
    ...
    def submit_demo_metrics(self) -> dict[str, Any]:
        ...
        for metric in metrics:
            try:
                response = self.session.post(
                    f"{self.base_url}/alerting/metrics", json=metric
                )

                if response.status_code == 200:
                    submitted_metrics.append(metric)
                    logger.info(
                        f"Submitted metric: {metric['metric_name']} = {metric['value']}"
                    )
                else:
                    logger.error(
                        f"Failed to submit metric {metric['metric_name']}: {response.text}"
                    )
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/alerting/metrics/batch", json=metrics
            )
    ...
    def get_active_alerts(self) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for alert in alerts:
                    logger.info(
                        f"Alert: {alert['rule_name']} - {alert['severity']} - {alert['status']}"
                    )
    ...
    def trigger_demo_alert(self) -> dict[str, Any]:
        ...
        try:
            response = self.session.post(
                ...
                params={"metric_name": "demo.cpu.usage", "value": 95.0},
            )
                ...
                async for message in websocket:
                    try:
                        ...
                        if data.get("type") == "initial_alerts":
                            logger.info(
                                f"Received initial alerts: {len(data.get('alerts', []))}"
                            )

                        elif data.get("type") == "alert_update":
                            logger.info(
                                f"Received alert update: {len(data.get('alerts', []))} alerts"
                            )

                        elif data.get("type") == "new_alert":
                            alert = data.get("alert", {})
                            logger.info(
                                f"NEW ALERT: {alert.get('rule_name')} - {alert.get('severity')}"
                            )
