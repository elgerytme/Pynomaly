...
def run_complexity_analysis(
    ...
    """
    try:
        from anomaly_detection.infrastructure.monitoring.complexity_monitor import (
            ComplexityMonitor,
        )
...
def calculate_complexity_trends(current_metrics, baseline_metrics) -> dict:
    ...
    current = (
        ...
        if hasattr(current_metrics, "to_dict")
...
def assess_quality_status(metrics, baseline_metrics) -> dict:
    ...
    status = {"overall": "good", "issues": [], "warnings": [], "critical_issues": []}

    # Check complexity thresholds
    if current.get("cyclomatic_complexity", 0) > 10:
        status["critical_issues"].append(
            f"High cyclomatic complexity: {current.get('cyclomatic_complexity', 0):.1f}"
        )
        status["overall"] = "critical"

    if current.get("cognitive_complexity", 0) > 15:
        status["warnings"].append(
            f"High cognitive complexity: {current.get('cognitive_complexity', 0):.1f}"
        )
    ...
    if current.get("maintainability_index", 100) < 60:
        status["critical_issues"].append(
            f"Low maintainability index: {current.get('maintainability_index', 0):.1f}"
        )
        status["overall"] = "critical"
    ...
    if current.get("total_files", 0) > 1000:
        status["warnings"].append(
            f"Large file count: {current.get('total_files', 0)} files"
        )
        if status["overall"] == "good":
            status["overall"] = "warning"
    ...
    if current.get("total_dependencies", 0) > 100:
        status["warnings"].append(
            f"High dependency count: {current.get('total_dependencies', 0)}"
        )
        if status["overall"] == "good":
            status["overall"] = "warning"
...
def generate_recommendations(metrics, trends) -> list[str]:
    ...
    if current.get("total_files", 0) > 500:
        recommendations.append(
            "Consider modularizing the codebase to manage file count"
        )
    ...
    if trends:
        for _metric_key, trend in trends.items():
            if trend["severity"] == "critical" and trend["trend"] == "increasing":
                ...
                recommendations.append(
                    f"Monitor {trend['name']} trend - consider preventive action"
                )
                ...
                hotspots.append(
                    {
                        ...
                        "estimated_complexity": round(
                            file_info["complexity_estimate"], 1
                        ),
...
def estimate_technical_debt(metrics) -> dict:
    ...
    if complexity > 10:
        ...
        debt_factors.append(
            f"High cyclomatic complexity (+{(complexity - 10) * 10:.0f})"
        )
    ...
    if maintainability < 70:
        ...
        debt_factors.append(
            f"Low maintainability index (+{(70 - maintainability) * 2:.0f})"
        )
...
def print_ci_summary(report: dict):
    ...
    print(
        f"📁 Files: {metrics.get('total_files', 0)} ({metrics.get('python_files', 0)} Python)"
    )
    ...
    print(
        f"🎯 Maintainability: {maintainability['grade']} ({maintainability['overall_score']:.1f}/100)"
    )
...
def main():
    ...
    parser.add_argument("--output-file", "-o", type=Path, help="Output file path")
    parser.add_argument(
        "--trend-analysis", "-t", action="store_true", help="Perform trend analysis"
    )
    parser.add_argument(
        "--baseline-file", "-b", type=Path, help="Baseline file for comparison"
    )
