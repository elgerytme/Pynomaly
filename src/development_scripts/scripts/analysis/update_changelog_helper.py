...
def get_user_input(prompt: str, options: list[str] | None = None) -> str:
...
def collect_changelog_entries():
    ...
    print(
        "For each category, provide entries (one per line). Press Enter twice when done."
    )
...
def main():
    ...
    new_major, new_minor, new_patch = increment_version(
        major, minor, patch, change_type
    )
    ...
    if confirm.lower() == "y":
        if update_changelog_file(changelog_entry):
            ...
            print("\n💡 Don't forget to:")
            print("   1. Update TODO.md to mark completed items")
            print("   2. Commit both CHANGELOG.md and TODO.md together")
