...
def collect_file_structure_metrics() -> PrometheusMetrics:
    ...
    for violation_type, count in violation_types.items():
        metrics.add_counter(
            "file_structure_violations_by_type_total",
            count,
    ...
    # Count test files
    test_files = len(list(project_root.rglob("test_*.py"))) + len(
...
def main(
    format: str = typer.Option(
        "prometheus", "--format", "-f", help="Output format: prometheus, json"
    ),
    ...
    include_timestamp: bool = typer.Option(
        True, "--timestamp/--no-timestamp", help="Include timestamp in output"
    ),
) -> None:
    """
    ...
    if metrics.metrics.get("file_structure_is_valid", 0.0) == 0.0:
        if verbose:
            violations_count = int(
                metrics.metrics.get("file_structure_violations_total", 0)
            )
            console.print(
                f"[red]File structure validation failed with {violations_count} violations[/red]"
            )
        sys.exit(1)
    else:
        if verbose:
