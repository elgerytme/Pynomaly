...
class TestRunCommand:
    ...
    def test_run_command_with_venv_windows(self):
        ...
        with patch(
            "scripts.maintenance.fix_package_issues.os.path.exists"
        ) as mock_exists:
            mock_exists.side_effect = lambda path: path == ".venv" or path.endswith(
                "python.exe"
            )

            with patch("scripts.maintenance.fix_package_issues.sys.platform", "win32"):
                with patch(
                    "scripts.maintenance.fix_package_issues.subprocess.run"
                ) as mock_run:
                    mock_result = MagicMock()
                    ...
                    with patch("builtins.print") as mock_print:
                        ...
                        assert actual_cmd[0].endswith("python.exe") or actual_cmd[
                            0
                        ].endswith("python")
                        mock_print.assert_any_call(
                            f"Using virtual environment: {actual_cmd[0]}"
                        )
    ...
    def test_run_command_externally_managed_environment(self):
        ...
        with patch("scripts.maintenance.fix_package_issues.subprocess.run") as mock_run:
            ...
            error1 = subprocess.CalledProcessError(
                1, cmd, stderr="externally-managed-environment"
            )
            ...
            with patch("builtins.print") as mock_print:
                ...
                assert result is True
                mock_print.assert_any_call(
                    "💡 Note: System Python is externally managed. Using --user flag or virtual environment required."
                )
                mock_print.assert_any_call("🔄 Retrying with --user flag...")
                mock_print.assert_any_call(
                    "✅ Success with --user: success with --user"
                )
    ...
    def test_run_command_externally_managed_environment_user_fails(self):
        ...
        with patch("scripts.maintenance.fix_package_issues.subprocess.run") as mock_run:
            ...
            error2 = subprocess.CalledProcessError(
                1, cmd + ["--user"], stderr="user installation failed"
            )
            ...
            with patch("builtins.print") as mock_print:
                ...
                mock_print.assert_any_call("🔄 Retrying with --user flag...")
                mock_print.assert_any_call(
                    "❌ --user also failed: user installation failed"
                )
...
class TestMainFunction:
    ...
    def test_main_with_installation_failure(self, mock_print, mock_run_command):
        ...
        mock_run_command.side_effect = [
            ...
            True,
            True,
        ]  # Uninstall fails, install fails, but others succeed
        ...
        with patch("builtins.__import__") as mock_import:
            ...
            with patch(
                ...
                mock_subprocess.return_value = MagicMock(
                    returncode=1, stderr="CLI failed"
                )
    ...
    def test_main_cli_test_timeout(self, mock_print, mock_run_command):
        ...
        with patch("builtins.__import__") as mock_import:
            ...
            with patch(
                ...
                mock_print.assert_any_call(
                    "❌ CLI test failed: Command 'cmd' timed out after 10 seconds"
                )
    ...
    @patch("builtins.print")
    def test_main_environment_info_display(
        self, mock_print, mock_exists, mock_run_command
    ):
...
class TestIntegrationFixPackageIssues:
    ...
    def test_directory_structure_checks(self, tmp_path: Path):
        ...
        try:
            ...
            with patch("builtins.print") as mock_print:
                result = run_command(
                    cmd, description, use_venv=False
                )  # Disable venv to avoid path issues
