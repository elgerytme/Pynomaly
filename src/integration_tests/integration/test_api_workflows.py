...
class TestCompleteAPIWorkflows:
    ...
    async def test_streaming_anomaly_detection_workflow(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_time_series_csv, "time_series_dataset"
        )

        # Step 2: Create detector for time series
        detector = await integration_helper.create_detector(
            dataset["id"], "time_series_anomaly"
        )
        ...
        response = await async_test_client.post(
            f"/api/streaming/sessions/{session['id']}/process", json=test_data
        )
        response.raise_for_status()
    ...
    async def test_experiment_management_workflow(
        ...
        # Step 1: Upload dataset
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "experiment_dataset"
        )
        ...
        experiment_config = {
            ...
            "algorithms": [
                ...
                {"name": "one_class_svm", "parameters": {"nu": 0.05, "kernel": "rbf"}},
                {
    ...
    async def test_model_lifecycle_management_workflow(
        ...
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "model_lifecycle_dataset"
        )
        ...
        response = await async_test_client.post(
            f"/api/models/{model_v1_id}/register", json=model_registration
        )
        response.raise_for_status()
        ...
        improved_training = await integration_helper.train_detector(
            improved_detector["id"]
        )
        ...
        response = await async_test_client.get(
            f"/api/models/{model_v1_id}/monitoring/drift"
        )
        response.raise_for_status()
    ...
    async def test_automl_workflow(
        ...
        # Step 1: Upload dataset
        dataset = await integration_helper.upload_dataset(
            sample_dataset_csv, "automl_dataset"
        )
        ...
        automl_config = {
            ...
            "cross_validation": {"folds": 3, "stratified": True},
        ...
        response = await async_test_client.post(
            "/api/autonomous/automl/start?created_by=test_user", json=automl_config
        )
        response.raise_for_status()
        ...
        response = await async_test_client.get(
            f"/api/autonomous/automl/{automl_job['id']}/status"
        )
        response.raise_for_status()
        ...
        response = await async_test_client.get(
            f"/api/autonomous/automl/{automl_job['id']}/results"
        )
        response.raise_for_status()
        results = response.json()["data"]
        ...
        response = await async_test_client.post(
            f"/api/autonomous/automl/{automl_job['id']}/deploy", json=deployment_config
        )
        response.raise_for_status()
    ...
    async def test_event_processing_workflow(
        ...
        for event_data in events_data:
            response = await async_test_client.post(
                "/api/events/create", json=event_data
            )
            response.raise_for_status()
        ...
        response = await async_test_client.post(
            "/api/events/patterns?created_by=test_user", json=pattern_config
        )
        response.raise_for_status()
        ...
        )  # Pattern detection might not trigger immediately
    ...
    async def test_performance_monitoring_workflow(
        ...
        response = await async_test_client.get(
            f"/api/performance/models/{model_id}/metrics"
        )
        response.raise_for_status()
        metrics = response.json()["data"]
        ...
        response = await async_test_client.post(
            "/api/performance/compare", json=comparison_config
        )
        response.raise_for_status()
        comparison = response.json()["data"]
        ...
        response = await async_test_client.get(
            f"/api/performance/models/{model_id}/optimize"
        )
        response.raise_for_status()
