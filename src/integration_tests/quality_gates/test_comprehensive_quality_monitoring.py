        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    metric.memory_usage_mb,
                    metric.cpu_usage_percent,
                    json.dumps(metric.flaky_tests),
                    json.dumps(metric.slow_tests),
                    metric.timestamp.isoformat(),
                ),
            )
...
class QualityGate:
    ...
    def validate(self, metrics: QualityMetrics) -> dict[str, bool]:
        ...
        results["success_rate"] = (
            metrics.success_rate >= self.gates["success_rate"]["threshold"]
        )

        # Average duration gate
        results["average_duration"] = (
            metrics.average_duration <= self.gates["average_duration"]["threshold"]
        )

        # Flaky test rate gate
        flaky_rate = len(metrics.flaky_tests) / max(metrics.total_tests, 1)
        results["flaky_test_rate"] = (
            flaky_rate <= self.gates["flaky_test_rate"]["threshold"]
        )

        # Memory usage gate
        results["memory_usage_mb"] = (
            metrics.memory_usage_mb <= self.gates["memory_usage_mb"]["threshold"]
        )

        # Slow test rate gate
        slow_rate = len(metrics.slow_tests) / max(metrics.total_tests, 1)
        results["slow_test_rate"] = (
            slow_rate <= self.gates["slow_test_rate"]["threshold"]
        )
...
class TestComprehensiveQualityMonitoring:
    ...
    def test_quality_gate_validation(self, quality_gate):
        ...
        poor_metrics = QualityMetrics(
            ...
            flaky_tests=["test_flaky_" + str(i) for i in range(10)],
            slow_tests=["test_slow_" + str(i) for i in range(20)],
            timestamp=datetime.now(),
        )
    ...
    def test_quality_trend_analysis(self, quality_monitor):
        ...
        for day in range(7):
            ...
            metric = QualityMetrics(
                total_tests=100,
                passed_tests=int(100 * success_rate),
                failed_tests=int(100 * (1 - success_rate)),
                skipped_tests=0,
                error_tests=0,
                total_duration=avg_duration * 100,
    ...
    def test_comprehensive_quality_assessment(self, quality_monitor, quality_gate):
        ...
        assessment = {
            "overall_quality": (
                "good" if gate_summary["success_rate"] >= 0.8 else "needs_improvement"
            ),
            "key_metrics": {
