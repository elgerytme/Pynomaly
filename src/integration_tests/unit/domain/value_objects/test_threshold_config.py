...
class TestThresholdConfig:
    ...
    def test_invalid_method(self):
        ...
        with pytest.raises(
            InvalidValueError, match="Invalid threshold method: invalid"
        ):
    ...
    def test_percentile_method_validation(self):
        ...
        with pytest.raises(
            InvalidValueError, match="Percentile value must be between 0 and 100"
        ):
    ...
    def test_threshold_bounds_validation(self):
        ...
        with pytest.raises(
            InvalidValueError, match="min_threshold.*must be less than.*max_threshold"
        ):
    ...
    def test_auto_adjust_combinations(self):
        ...
        bounded_auto = ThresholdConfig(
            method="adaptive", auto_adjust=True, min_threshold=0.1, max_threshold=0.9
        )
    ...
    def test_practical_usage_scenarios(self):
        ...
        fixed_bounded = ThresholdConfig(
            method="fixed", value=0.5, min_threshold=0.1, max_threshold=0.9
        )
        ...
        adaptive_config = ThresholdConfig(
            method="adaptive", auto_adjust=True, min_threshold=0.05, max_threshold=0.95
        )
        assert adaptive_config.method == "adaptive"
        assert adaptive_config.auto_adjust is True
    ...
    def test_method_case_sensitivity(self):
        ...
        with pytest.raises(
            InvalidValueError, match="Invalid threshold method: PERCENTILE"
        ):
            ThresholdConfig(method="PERCENTILE")
    ...
    def test_validation_error_messages(self):
        ...
        assert "min_threshold (0.8) must be less than max_threshold (0.2)" in str(
            exc_info.value
        )
