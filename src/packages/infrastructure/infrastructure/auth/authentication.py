...
class JWTManager:
    ...
    def refresh_access_token(
        self, refresh_token: str, user_store: UserStore
    ) -> str | None:
    ...
    async def _store_session(self, session: Session):
        ...
        if self.redis_client:
            ...
            self.redis_client.setex(
                f"session:{session.id}", ttl, json.dumps(session_data, default=str)
            )
        else:
...
class AuthenticationService:
    ...
    ) -> tuple[User | None, Session | None, str]:
    ...
    async def authenticate_api_key(
        self, api_key: str
    ) -> tuple[User | None, APIKey | None, str]:
    ...
    async def authenticate_jwt(self, token: str) -> tuple[User | None, str]:
    ...
    async def update_user_password(
        ...
        return (
            success,
            "Password updated successfully" if success else "Failed to update password",
        )
    ...
    def get_auth_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "user_roles": {
                role.value: len(
                    [u for u in self.user_store.users.values() if role in u.roles]
                )
...
async def main():
    ...
    if user:
        ...
        auth_user, api_key_obj, message = await auth_service.authenticate_api_key(
            api_key
        )
