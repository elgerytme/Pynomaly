    ...
    async def _analyze_performance(self) -> None:
        ...
        for suite_name, suite_results in self.results["test_results"].items():
            performance_data["suite_performance"][suite_name] = {
                ...
                "tests_per_second": suite_results["total_tests"]
                / suite_results["execution_time"]
                if suite_results["execution_time"] > 0
                else 0,
                "average_test_time": suite_results["execution_time"]
        ...
        slowest_suite = max(
            self.results["test_results"].items(), key=lambda x: x[1]["execution_time"]
        )
