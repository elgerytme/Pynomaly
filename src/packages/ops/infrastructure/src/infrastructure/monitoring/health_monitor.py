...
This module provides comprehensive health monitoring for all packages in the Pynomaly platform.
"""
...
class HealthMonitor:
    ...
    def _discover_packages(self) -> List[str]:
        ...
        packages = []
        base_path = Path("/mnt/c/Users/andre/Pynomaly/src/packages")
    ...
    async def check_package_health(self, package_name: str) -> PackageHealth:
        ...
        try:
            # Get package info
            package_path = Path(f"/mnt/c/Users/andre/Pynomaly/src/packages/{package_name}")
            ...
            # Check package-specific metrics
            metrics = await self._collect_package_metrics(package_name, package_path)
            
            # Check dependencies
            dependencies_status = await self._check_dependencies(package_path)
            
            # Determine overall status
            overall_status = self._determine_overall_status(metrics, dependencies_status)
    ...
    async def _collect_package_metrics(self, package_name: str, package_path: Path) -> List[HealthMetric]:
    ...
    def _determine_overall_status(self, metrics: List[HealthMetric], dependencies: Dict[str, HealthStatus]) -> HealthStatus:
        ...
        if (any(m.status == HealthStatus.DEGRADED for m in metrics) or 
...
if __name__ == "__main__":
    ...
    if args.once:
        if args.package:
            ...
            # Check system health
            metrics = asyncio.run(monitor.check_system_health())
            print(json.dumps({k: asdict(v) for k, v in metrics.items()}, indent=2, default=str))
    else:
        # Start continuous monitoring
