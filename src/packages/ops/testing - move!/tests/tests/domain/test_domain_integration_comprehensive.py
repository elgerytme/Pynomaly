...
class TestDatasetDetectorIntegration:
    ...
    def mock_detector(self):
        ...
        class TestDetector(Detector):
            ...
            def detect(self, dataset: Dataset) -> DetectionResult:
                ...
                scores = [
                    AnomalyScore(np.random.beta(2, 8)) for _ in range(dataset.n_samples)
                ]
                threshold = 0.7
    ...
    def test_dataset_detector_compatibility(self, sample_dataset, mock_detector):
        ...
        incompatible_data = pd.DataFrame({"different_feature": [1, 2, 3, 4, 5]})
...
class TestAnomalyScoreThresholdIntegration:
    ...
    def test_percentile_threshold_integration(
        ...
        threshold_config = ThresholdConfig(
            method="percentile", value=95.0, auto_adjust=False
        )

        # Calculate threshold
        threshold = threshold_calculator.calculate_percentile_threshold(
            scores=sample_scores, percentile=threshold_config.value
        )
    ...
    def test_contamination_threshold_integration(
        self, threshold_calculator, sample_scores
    ):
    ...
    def test_adaptive_threshold_with_confidence(
        self, threshold_calculator, sample_scores
    ):
...
class TestEnsembleDetectionIntegration:
    ...
    def mock_detector_predictions(self):
        ...
        detector_1_scores = [
            AnomalyScore(score) for score in np.random.beta(3, 7, 100)
        ]  # Conservative
        detector_2_scores = [
            AnomalyScore(score) for score in np.random.beta(2, 3, 100)
        ]  # Aggressive
        detector_3_scores = [
            AnomalyScore(score) for score in np.random.beta(4, 6, 100)
        ]  # Balanced
    ...
    def test_voting_ensemble_integration(
        self, ensemble_aggregator, mock_detector_predictions
    ):
        ...
        for i, (_score, label) in enumerate(
            ...
            individual_votes = sum(
                1
                for detector_scores in mock_detector_predictions.values()
    ...
    def test_weighted_ensemble_integration(
        self, ensemble_aggregator, mock_detector_predictions
    ):
        ...
        result = ensemble_aggregator.aggregate_weighted_average(
            predictions=mock_detector_predictions, weights=weights
        )
    ...
    def test_ensemble_diversity_analysis(
        self, ensemble_aggregator, mock_detector_predictions
    ):
...
class TestContinuousLearningIntegration:
    ...
    def test_drift_triggered_retraining(self, drift_detector, continuous_learner):
        ...
        continuous_learner.add_trigger_condition(
            condition_type="data_drift", threshold=0.1, detection_method="ks_test"
        )
...
class TestBusinessRuleEnforcement:
    ...
    def test_data_quality_business_rules(self):
        ...
        for col in dataset.data.select_dtypes(include=[np.number]).columns:
            z_scores = np.abs(
                (dataset.data[col] - dataset.data[col].mean()) / dataset.data[col].std()
            )
            if (z_scores > 3).any():
