...
class TestDatasetEntity:
    ...
    def test_dataset_validation_empty_data(self):
        ...
        with pytest.raises(InsufficientDataError):
            Dataset(name="Empty Dataset", data=[], features=["x"])
...
class TestDetectorEntity:
    ...
    def test_detector_parameter_validation(self):
        ...
        # Valid parameters
        valid_params = {"n_estimators": 100, "contamination": 0.1}
    ...
    def test_detector_hyperparameter_tuning(self, sample_detector_data):
        ...
        # Mock tuning results
        tuned_params = {"n_estimators": 150, "contamination": 0.08}
    ...
    def test_detector_performance_tracking(self, sample_detector_data):
        ...
        better_metrics = PerformanceMetrics(
            accuracy=0.90, precision=0.88, recall=0.92, f1_score=0.90, auc_roc=0.95
        )
...
class TestDomainServices:
    ...
    def test_detection_service_single_detection(
        self, mock_detector_repository, mock_dataset_repository
    ):
        ...
        result = service.detect_anomalies(
            detector_id=DetectorId("det_001"), dataset_id=DatasetId("data_001")
        )
    ...
    def test_model_validation_service_cross_validation(self):
        ...
        with patch.object(service, "_perform_cv_fold") as mock_cv:
            mock_cv.return_value = {"accuracy": 0.85, "precision": 0.80, "recall": 0.90}

            results = service.cross_validate(
                detector=mock_detector, dataset=mock_dataset, cv_folds=5
            )
...
class TestDomainSpecifications:
    ...
    def test_high_quality_dataset_specification(self):
        ...
        spec = HighQualityDatasetSpecification(
            min_completeness=0.95, min_consistency=0.90, max_duplicates_rate=0.05
        )
