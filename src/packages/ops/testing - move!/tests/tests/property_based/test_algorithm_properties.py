...
@st.composite
def valid_numpy_arrays(
    draw, min_samples=10, max_samples=1000, min_features=2, max_features=50
):
        ...
        params.update(
            {
                ...
                "metric": draw(
                    st.sampled_from(["minkowski", "euclidean", "manhattan"])
                ),
...
class TestAlgorithmUniversalProperties:
    ...
    def test_algorithm_score_range(self, dataset, algorithm_name):
        ...
        try:
            ...
            # Fit and predict
            algorithm_instance = adapter.create_algorithm(
                algorithm_name, detector.parameters
            )
            ...
            assert (
                len(np.unique(scores)) > 1 or len(scores) == 1
            ), "Scores should have some variation (unless single sample)"
    ...
    def test_algorithm_determinism(self, dataset, algorithm_name):
        ...
        try:
            ...
            algorithm_instance_1 = adapter.create_algorithm(
                algorithm_name, detector.parameters
            )
            algorithm_instance_1.fit(X)
            scores_1 = algorithm_instance_1.decision_function(X)

            algorithm_instance_2 = adapter.create_algorithm(
                algorithm_name, detector.parameters
            )
    ...
    @given(anomaly_datasets(), st.sampled_from(["IsolationForest"]))
    @settings(max_examples=10, deadline=60000)
    def test_algorithm_monotonicity_with_contamination(self, dataset, algorithm_name):
        ...
        try:
            ...
            assume(
                X.shape[0] >= 20
            )  # Need enough samples for different contamination rates
            assume(not np.any(np.isnan(X)))
            assume(not np.any(np.isinf(X)))
            ...
            assert (
                outlier_counts[0] <= outlier_counts[1] + 2
            ), "Low contamination should detect fewer outliers"
            assert (
                outlier_counts[1] <= outlier_counts[2] + 2
            ), "Medium contamination should detect fewer outliers than high"
...
class TestDataQualityProperties:
    ...
    @settings(max_examples=10, deadline=30000)
    def test_algorithm_with_correlated_features(
        self, n_samples, n_features, correlation
    ):
        ...
        try:
            ...
            assert (
                np.std(scores) > 0
            ), "Should produce varied scores even with correlated features"
...
class TestScalingProperties:
    ...
    def test_algorithm_scale_invariance(self, n_samples, n_features, scale_factor):
        ...
        try:
            ...
            assert (
                agreement >= 0.8
            ), f"Scale invariance violated: only {agreement:.2f} agreement"
...
class TestRobustnessProperties:
    ...
    def test_algorithm_robustness_to_outliers(
        self, n_samples, n_features, outlier_fraction
    ):
        ...
        try:
            ...
            algorithm_instance_outliers = adapter.create_algorithm(
                algorithm_name, params
            )
            algorithm_instance_outliers.fit(X_with_outliers)
            scores_outliers = algorithm_instance_outliers.decision_function(
                X_with_outliers
            )
            ...
            assert (
                detected_outliers >= expected_outliers * 0.5
