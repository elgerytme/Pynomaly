...
def valid_datasets(draw):
    ...
    target_column = draw(st.one_of(st.none(), st.sampled_from(data.columns.tolist())))

    return Dataset(
        name=name, data=data, description=description, target_column=target_column
    )
...
class TestAnomalyProperties:
    ...
    def test_anomaly_invalid_negative_scores(self, invalid_score):
        ...
        with pytest.raises(ValidationError):
            Anomaly(
                score=invalid_score, data_point={"field": "value"}, detector_name="test"
            )
...
class TestDatasetProperties:
    ...
    def test_dataset_feature_names_consistency(self, dataset):
        ...
        if dataset.feature_names:
            ...
            assert len(set(dataset.feature_names)) == len(
...
class TestBusinessRuleProperties:
    ...
    def test_anomaly_attention_requirements(self, anomaly):
        ...
        expected = (
            anomaly.severity in ["critical", "high"] and anomaly.confidence_level > 0.7
        )
...
class TestErrorHandlingProperties:
    ...
    def test_empty_string_validation(self, empty_string):
        ...
        with pytest.raises(ValidationError):
            Anomaly(
                score=0.5, data_point={"field": "value"}, detector_name=empty_string
            )
    ...
    def test_reserved_key_validation(self, field_name):
        ...
        for reserved_key in reserved_keys:
            with pytest.raises(ValidationError):
                Anomaly(
                    score=0.5, data_point={reserved_key: "value"}, detector_name="test"
                )
    ...
    def test_special_character_validation(self, text_input):
        ...
        assume(
            not text_input.replace("_", "").replace("-", "").replace(".", "").isalnum()
        )

        # Should fail for detector names with invalid characters
        with pytest.raises(ValidationError):
            Anomaly(score=0.5, data_point={"field": "value"}, detector_name=text_input)
