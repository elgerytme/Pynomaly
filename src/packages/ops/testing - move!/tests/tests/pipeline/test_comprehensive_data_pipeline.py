...
class TestDataPipelineCore:
    ...
    def sample_pipeline_config(self):
        ...
        return {
            ...
            "error_handling": {"strategy": "continue_on_error", "max_errors": 10},
            "monitoring": {
                "enabled": True,
    ...
    def test_pipeline_error_handling_continue(self):
        ...
        config = {
            "name": "error_test",
            "stages": [],
            "error_handling": {"strategy": "continue_on_error", "max_errors": 2},
        }
...
10.0,11.0,12.0"""
    ...
    def test_json_data_ingestion(self):
        ...
        json_data = {"data": [[1, 2, 3], [4, 5, 6]], "features": ["x", "y", "z"]}
        ...
        try:
            config = {"source_type": "json", "file_path": temp_file}
    ...
    def test_ingestion_error_handling(self):
        ...
        config = {"source_type": "csv", "file_path": "/nonexistent/file.csv"}
...
class TestDataValidationStage:
    ...
    def test_schema_validation_valid(self, sample_schema):
        ...
        config = {"schema": sample_schema, "strict_mode": True}
    ...
    def test_completeness_validation(self):
        ...
        config = {"completeness_threshold": 0.8, "required_features": ["x", "y", "z"]}
    ...
    def test_validation_with_repair(self):
        ...
        assert len(result["repaired_data"]) < len(
...
class TestDataTransformationStage:
    ...
    def test_standard_scaling_transformation(self, sample_numeric_data):
        ...
        config = {
            "transformers": [
                {"type": "StandardScaler", "features": ["small", "medium", "large"]}
            ]
        }
    ...
    def test_robust_scaling_transformation(self, sample_numeric_data):
        ...
        config = {
            "transformers": [
                {"type": "RobustScaler", "features": ["small", "medium", "large"]}
            ]
        }
    ...
    def test_outlier_removal_transformation(self):
        ...
        config = {
            "transformers": [
                {"type": "OutlierRemover", "method": "iqr", "threshold": 1.5}
            ]
        }
    ...
    def test_chained_transformations(self, sample_numeric_data):
        ...
        config = {
            "transformers": [
                {"type": "StandardScaler", "features": ["small", "medium", "large"]},
                {"type": "PCATransformer", "n_components": 2},
            ]
        }
    ...
    def test_transformation_inverse(self, sample_numeric_data):
        ...
        if "transformer_objects" in result["transformation_metadata"]:
            inverse_data = stage.inverse_transform(
                result["transformed_data"], result["transformation_metadata"]
            )
...
class TestFeatureEngineeringStage:
    ...
    def test_polynomial_feature_generation(self):
        ...
        simple_data = {"data": [[1, 2], [3, 4], [5, 6]], "features": ["x1", "x2"]}
            ...
            "feature_generators": [
                {
                    ...
                    "extract_features": ["hour", "day_of_week", "month", "quarter"],
                }
            ]
        }
    ...
    def test_statistical_feature_generation(self):
        ...
        stat_features = [
            ...
            if any(stat in f for stat in ["mean", "std", "min", "max", "skew"])
        ]
    ...
    def test_frequency_domain_features(self):
        ...
        freq_features = [
            f
            for f in new_features
            if any(feat in f for feat in ["frequency", "spectral"])
        ]
    ...
    def test_interaction_feature_generation(self):
        ...
        interaction_features = [
            f
            for f in new_features
            if any(op in f for op in ["multiply", "add", "subtract"])
        ]
    ...
    def test_domain_specific_features(self):
        ...
        anomaly_features = [
            f
            for f in new_features
            if any(feat in f for feat in ["isolation", "outlier", "distance"])
        ]
...
"""
            ...
            pipeline_config = {
                ...
                "stages": [
                    ...
                    },
                    {
                        "name": "validation",
                        "type": "DataValidationStage",
                        "config": {"quality_checks": {"min_completeness": 0.8}},
                    },
                    {
                        "name": "transformation",
                        "type": "DataTransformationStage",
                        "config": {
                            "transformers": [
                                {"type": "StandardScaler", "features": ["x", "y", "z"]}
                            ]
                        },
                    },
                    {
    ...
    def test_pipeline_branching_and_merging(self):
        ...
        numerical_stage.execute.return_value = {
    ...
    async def test_streaming_pipeline_execution(self):
        ...
        config = {
            "name": "streaming_test",
            "stages": [],
            "execution": {"mode": "streaming", "batch_size": 2, "buffer_size": 10},
        }
