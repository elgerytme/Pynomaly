    ...
    try:
        ...
        async with AsyncClient(
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ OpenAPI docs: Available ({len(openapi.get('paths', {}))} endpoints)"
                )
            else:
                print(
                    f"⚠️  OpenAPI docs: Not available (status: {response.status_code})"
                )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"ℹ️  Datasets endpoint: Protected (status: {response.status_code})"
                )
...
async def main():
    ...
    if passed_checks == total_checks:
        ...
        print("\nNext steps:")
        print(
            "  1. Run full integration tests: python scripts/run_integration_tests.py"
        )
        print(
            "  2. Run specific test suite: python scripts/run_integration_tests.py --suite test_api_workflows.py"
        )
        print(
            "  3. Run with coverage: python scripts/run_integration_tests.py --coverage"
        )
        return True
    else:
        print(
            "⚠️  Some checks failed. Please address the issues above before running integration tests."
        )
        return False
