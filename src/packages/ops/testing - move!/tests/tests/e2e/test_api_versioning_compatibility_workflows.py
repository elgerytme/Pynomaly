        ...
        try:
            ...
            for version in api_versions:
                if version in detector_configs:
                    ...
                    if create_response.status_code == 200:
                        ...
                        train_endpoint = (
                            f"/api/{version}/detectors/{detector_ids[version]}/train"
                        )
                        ...
                        train_response = app_client.post(
                            train_endpoint, json=train_request
                        )
            ...
            if "v1" in detector_ids and "v2" in detector_ids:
                ...
                detect_response = app_client.post(
                    v2_detect_endpoint, json={"dataset_id": dataset_id}
                )
                # Should work with compatibility layer
                assert detect_response.status_code in [
                    200,
                    400,
                ]  # 400 if compatibility not implemented
            ...
            if "v2" in detector_ids:
                ...
                migration_response = app_client.post(
                    "/api/migration/detector", json=migration_request
                )
            ...
            if format_type == "csv":
                ...
                with open(file_path, "rb") as file:
                    upload_response = app_client.post(
                        f"/api/{version}/datasets/upload",
                        files={
                            "file": (f"{version}_data.json", file, "application/json")
                        },
        ...
        if uploaded_datasets:
            ...
            for version, dataset_id in uploaded_datasets.items():
                train_response = app_client.post(
                    ...
                    json={"dataset_id": dataset_id, "format_compatibility_mode": True},
                )
    ...
    def test_response_format_evolution_workflow(self, app_client, sample_dataset):
        ...
        try:
            ...
            for version, format_spec in version_response_formats.items():
                ...
                headers = {"Accept": f"application/vnd.monorepo.{version}+json"}
                ...
                if detect_response.status_code == 200:
                    ...
                    for field in expected_fields:
                        if field in result:
                            ...
                            print(
                                f"Field {field} not found in version {version}, checking alternatives..."
                            )

                # Test backward compatibility endpoint
                compat_endpoint = f"/api/{version}/detectors/{detector_id}/detect"
                compat_response = app_client.post(
                    compat_endpoint, json={"dataset_id": dataset_id}
                )
            ...
            if len(detection_results) > 1:
                ...
                conversion_response = app_client.post(
                    "/api/conversion/response-format", json=conversion_request
                )
    ...
    def test_feature_flag_workflow(self, app_client):
        ...
        for feature in experimental_features:
            ...
            if feature_name:
                ...
                enable_response = app_client.post(
                    f"/api/features/{feature_name}/enable",
                    json={"user_consent": True, "feedback_opt_in": True},
                )

                if enable_response.status_code == 200:
                    # Test using experimental feature
                    feature_test_response = app_client.get(
                        f"/api/features/{feature_name}/test"
                    )
                    assert feature_test_response.status_code in [
                        200,
                        501,
                    ]  # 501 if not implemented

                    # Disable experimental feature
                    disable_response = app_client.post(
                        f"/api/features/{feature_name}/disable"
                    )
    ...
    def test_deprecation_workflow(self, app_client, sample_dataset):
        ...
        for old_endpoint, new_endpoint in deprecated_endpoints:
            ...
            if deprecated_response.status_code == 200:
                ...
                deprecation_warning = deprecated_response.headers[
        ...
        migration_response = app_client.post(
            "/api/migration/analyze", json=migration_request
        )
    ...
    def test_api_contract_testing_workflow(self, app_client):
        ...
        if len(schemas) > 1:
            ...
            for i in range(len(versions) - 1):
                ...
                compatibility_response = app_client.post(
                    "/api/validation/schema-compatibility", json=compatibility_request
                )
        ...
        sdk_test_request = {
            ...
            "test_endpoints": ["/detectors", "/datasets/upload", "/detection/detect"],
        }

        sdk_compatibility_response = app_client.post(
            "/api/validation/sdk-compatibility", json=sdk_test_request
        )
