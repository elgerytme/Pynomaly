...
class TestRBACIntegration:
    ...
    def app(self, auth_service):
        ...
        @app.get("/detectors")
        async def detectors_endpoint(
            user=Depends(require_permissions("detectors:read")),
        ):
            return {"message": "Detectors endpoint", "user": user.username}

        @app.get("/manage-users")
        async def manage_users_endpoint(
            user=Depends(require_permissions("users:write")),
        ):
        ...
        async def admin_or_api_key_endpoint(
            user=Depends(require_role_or_api_key("admin")),
        ):
            return {"message": "Admin or API key endpoint", "user": user.username}

        @app.get("/read-or-api-key")
        async def read_or_api_key_endpoint(
            user=Depends(require_permissions_or_api_key("detectors:read")),
        ):
    ...
    def test_protected_endpoint_with_auth(self, client, regular_token):
        ...
        response = client.get(
            "/protected", headers={"Authorization": f"Bearer {regular_token}"}
        )
        assert response.status_code == 200
    ...
    def test_role_based_access_denied(self, client, regular_token):
        ...
        response = client.get(
            "/admin-only", headers={"Authorization": f"Bearer {regular_token}"}
        )
    ...
    def test_permission_based_access_allowed(self, client, regular_token):
        ...
        response = client.get(
            "/detectors", headers={"Authorization": f"Bearer {regular_token}"}
        )
        assert response.status_code == 200
    ...
    def test_permission_based_access_denied(self, client, regular_token):
        ...
        response = client.get(
            "/manage-users", headers={"Authorization": f"Bearer {regular_token}"}
        )
        assert response.status_code == 403
    ...
    def test_superuser_access_allowed(self, client, superuser_token):
        ...
        response = client.get(
            "/superuser-only", headers={"Authorization": f"Bearer {superuser_token}"}
        )
        assert response.status_code == 200
    ...
    def test_superuser_access_denied(self, client, admin_token):
        ...
        response = client.get(
            "/superuser-only", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 403
    ...
    def test_api_key_access(self, client, api_key):
        """Test API key access."""
        response = client.get(
            "/api-key-only", headers={"Authorization": f"Bearer {api_key}"}
        )
        assert response.status_code == 200
    ...
    def test_api_key_access_invalid(self, client):
        ...
        response = client.get(
            "/api-key-only", headers={"Authorization": "Bearer pyn_invalid_key"}
        )
        assert response.status_code == 401
    ...
    def test_role_or_api_key_with_role(self, client, admin_token):
        ...
        response = client.get(
            "/admin-or-api-key", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
    ...
    def test_role_or_api_key_with_api_key(self, client, api_key):
        ...
        response = client.get(
            "/admin-or-api-key", headers={"Authorization": f"Bearer {api_key}"}
        )
        assert response.status_code == 200
        assert response.json()["message"] == "Admin or API key endpoint"
    ...
    def test_role_or_api_key_denied(self, client, regular_token):
        ...
        response = client.get(
            "/admin-or-api-key", headers={"Authorization": f"Bearer {regular_token}"}
        )
        assert response.status_code == 403
    ...
    def test_permission_or_api_key_with_permission(self, client, regular_token):
        ...
        response = client.get(
            "/read-or-api-key", headers={"Authorization": f"Bearer {regular_token}"}
        )
        assert response.status_code == 200
    ...
    def test_permission_or_api_key_with_api_key(self, client, api_key):
        ...
        response = client.get(
            "/read-or-api-key", headers={"Authorization": f"Bearer {api_key}"}
        )
        assert response.status_code == 200
        assert response.json()["message"] == "Read or API key endpoint"
...
class TestJWKSEndpointIntegration:
    ...
    def test_rotate_keys_endpoint(self, client, superuser_token):
        ...
        response = client.post(
            "/auth/rotate-keys", headers={"Authorization": f"Bearer {superuser_token}"}
        )
        assert response.status_code == 200
    ...
    def test_key_info_endpoint(self, client, superuser_token):
        ...
        response = client.get(
            "/auth/keys/info", headers={"Authorization": f"Bearer {superuser_token}"}
        )
        assert response.status_code == 200
