...
class TestResponseProcessorIntegration:
    ...
    async def test_response_processing_lifecycle(self, sample_api_request):
        ...
        assert (
            processed_response.headers["X-Request-ID"] == sample_api_request.request_id
        )
...
class TestEndpointManagerIntegration:
    ...
    async def test_endpoint_registration_lifecycle(self):
        ...
        manager.update_endpoint_stats(
            "/v1/test-endpoint", HTTPMethod.GET, 125.5, success=True
        )
        manager.update_endpoint_stats(
            "/v1/test-endpoint", HTTPMethod.GET, 98.2, success=True
        )
        manager.update_endpoint_stats(
            "/v1/test-endpoint", HTTPMethod.GET, 155.8, success=False
        )
...
class TestAPIVersioningIntegration:
    ...
    async def test_version_management_lifecycle(self):
        ...
        v1_registered = versioning.register_version(
            "v1", description="Initial API version", deprecated=False
        )
        assert v1_registered

        v2_registered = versioning.register_version(
            "v2", description="Second API version with improvements", deprecated=False
        )
        ...
        assert (
            versioning.get_version_from_path("/v1-beta/test") == "v1"
        )  # Extracts v1 from v1-beta
        assert (
            versioning.get_version_from_path("/no-version/endpoint") == "v1"
        )  # Default
    ...
    async def test_version_path_extraction_edge_cases(self):
        ...
        for path, expected_version in test_cases:
            ...
            ), f"Path {path} should extract version {expected_version}, got {extracted_version}"
...
class TestAPIGatewayPerformance:
    ...
    async def test_large_request_handling(self):
        ...
        large_data_metadata = EndpointMetadata(
            path="/v1/large-data", method=HTTPMethod.POST, handler=large_data_handler
        )
