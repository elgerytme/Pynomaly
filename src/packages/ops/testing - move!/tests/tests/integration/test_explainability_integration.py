...
async def test_basic_explainability():
    ...
    try:
        ...
        print(
            f"✅ Available explanation methods: {[method.value for method in available_methods]}"
        )
        ...
        class MockFeatureImportanceExplainer:
            ...
            def explain_global(self, data, model, feature_names, **kwargs):
                feature_importances = {name: np.random.rand() for name in feature_names}
            ...
            def explain_cohort(
                self, instances, model, feature_names, cohort_id, **kwargs
            ):
                from datetime import datetime
        ...
        explainability_service.register_explainer(
            ExplanationMethod.FEATURE_IMPORTANCE, mock_explainer
        )
    ...
    try:
        ...
        class MockExplainer:
            def explain_instance(
                self, instance, model, feature_names, method, **kwargs
            ):
                contributions = []
                for i, feature_name in enumerate(feature_names):
        ...
        if response.success:
            ...
            if response.explanation:
                ...
                print(
                    f"   Feature count: {len(response.explanation.feature_contributions)}"
                )

            if response.metadata and "counterfactuals" in response.metadata:
                print(
                    f"   Counterfactuals generated: {len(response.metadata['counterfactuals'])}"
                )
        else:
    ...
    try:
        ...
        try:
            explanation = shap_explainer.explain_local(
                instance=instance, model=detector.model, feature_names=feature_names
            )
