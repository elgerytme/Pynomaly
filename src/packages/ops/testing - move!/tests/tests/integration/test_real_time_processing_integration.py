        ...
        for i, row in dataset.data.iterrows():
            ...
            if feature_values and (max(feature_values) > 2 or min(feature_values) < -2):
                anomaly = Anomaly(
                    ...
                    score=AnomalyScore(
                        min(0.9, max(abs(v) for v in feature_values) / 3)
                    ),
...
class TestRealTimeDetectorIntegration:
    ...
    async def test_real_time_detector_streaming(self, mock_detector, streaming_config):
        ...
        anomaly_anomaly_count = sum(
            len(r.anomalies) if r else 0 for r in anomaly_results
        )
    ...
    async def test_real_time_detector_performance(
        self, mock_detector, streaming_config
    ):
...
class TestStreamingPipelineIntegration:
    ...
    async def test_streaming_pipeline_lifecycle(
        self, mock_detector, streaming_config, sample_data_points
    ):
    ...
    async def test_streaming_pipeline_backpressure(
        self, mock_detector, sample_data_points
    ):
    ...
    async def test_streaming_pipeline_event_handling(
        self, mock_detector, streaming_config
    ):
        ...
        pipeline.event_processor.register_handler(
            StreamEventType.ANOMALY_DETECTED, custom_anomaly_handler
        )
...
class TestStreamProcessorIntegration:
    ...
    async def test_stream_processor_multiple_pipelines(
        self, mock_detector, streaming_config, sample_data_points
    ):
        ...
        for pipeline_id in pipeline_ids:
            created = await processor.create_pipeline(
                pipeline_id, mock_detector, streaming_config
            )
            assert created
        ...
        for i, data_point in enumerate(sample_data_points[:9]):  # 3 points per pipeline
            ...
            success = await processor.submit_data(
                pipeline_id, data_point, source=f"multi_pipeline_test_{pipeline_id}"
            )
            assert success
    ...
    async def test_stream_processor_pipeline_management(
        self, mock_detector, streaming_config
    ):
        ...
        # Create pipeline
        created = await processor.create_pipeline(
            "mgmt_test_pipeline", mock_detector, streaming_config
        )
        assert created

        # Try to create duplicate pipeline (should fail)
        duplicate_created = await processor.create_pipeline(
            "mgmt_test_pipeline", mock_detector, streaming_config
        )
        ...
        submit_success = await processor.submit_data(
            "mgmt_test_pipeline", test_data, "management_test"
        )
        ...
        submit_to_stopped = await processor.submit_data(
            "mgmt_test_pipeline", test_data, "stopped_test"
        )
        ...
        nonexistent_submit = await processor.submit_data(
            "nonexistent_pipeline", test_data, "nonexistent_test"
        )
        assert not nonexistent_submit

    async def test_global_stream_processor_integration(
        self, mock_detector, streaming_config
    ):
        ...
        created = await processor1.create_pipeline(
            "global_test_pipeline", mock_detector, streaming_config
        )
        assert created
...
class TestRealTimeProcessingErrorHandling:
    ...
    async def test_detector_error_handling(self, streaming_config):
        ...
        data_point = {"feature_1": 1.0, "feature_2": 2.0, "timestamp": datetime.now()}
    ...
    async def test_pipeline_error_recovery(self, mock_detector):
        ...
        pipeline.event_processor.register_handler(
            StreamEventType.DATA_POINT, error_prone_handler
        )
...
class TestRealTimeProcessingPerformance:
    ...
    async def test_high_throughput_processing(self, mock_detector):
        ...
        submission_rate = (
            submitted_count / submission_time if submission_time > 0 else 0
        )
        ...
        if detector_stats["processing_performance"]["samples"] > 0:
            avg_processing_time = detector_stats["processing_performance"][
                "avg_time_ms"
            ]
    ...
    async def test_concurrent_pipeline_performance(
        ...
        for pipeline_id in pipeline_ids:
            await processor.create_pipeline(
                pipeline_id, mock_detector, streaming_config
            )
        ...
        async def submit_data_to_pipeline(pipeline_id: str, data_count: int):
            ...
            for i in range(data_count):
                ...
                success = await processor.submit_data(
                    pipeline_id, data_point, f"concurrent_test_{pipeline_id}"
                )
                if success:
        ...
        concurrent_throughput = (
            total_submissions / concurrent_time if concurrent_time > 0 else 0
        )

        assert (
            total_submissions > pipeline_count * 40
