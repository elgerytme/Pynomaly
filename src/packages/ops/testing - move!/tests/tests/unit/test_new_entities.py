...
def test_new_entities():
    ...
    try:
        ...
        print(
            f"  After promotion: stage={model.stage.value}, in_production={model.is_in_production}"
        )
        ...
        print(
            f"  F1 score summary: mean={f1_summary.get('mean', 0):.3f}, max={f1_summary.get('max', 0):.3f}"
        )
