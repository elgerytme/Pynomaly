...
class TestInsufficientDataError:
    ...
    def test_basic_creation(self):
        ...
        error = InsufficientDataError("test_dataset", 10, 100)
        expected_message = (
            "Dataset 'test_dataset' has insufficient data: 10 samples, but 100 required"
        )
        assert error.message == expected_message
        assert error.details["dataset_name"] == "test_dataset"
    ...
    def test_zero_samples(self):
        ...
        expected_message = (
            "Dataset 'empty_dataset' has insufficient data: 0 samples, but 1 required"
        )
        assert error.message == expected_message
    ...
    def test_string_representation(self):
        ...
        result = str(error)
        assert (
            "Dataset 'test_dataset' has insufficient data: 10 samples, but 100 required for training"
            in result
        )
        assert "dataset_name=test_dataset" in result
...
class TestDataTypeError:
    ...
    def test_none_values(self):
        ...
        error = DataTypeError(
            "Invalid data type", feature=None, expected_type=None, actual_type=None
        )
        assert error.message == "Invalid data type"
    ...
    def test_empty_string_values(self):
        ...
        error = DataTypeError(
            "Invalid data type", feature="", expected_type="", actual_type=""
        )
        assert error.message == "Invalid data type"
...
class TestFeatureMismatchError:
    ...
    def test_creation_with_expected_features(self):
        ...
        error = FeatureMismatchError(
            "Feature mismatch", expected_features=expected_features
        )
        assert error.message == "Feature mismatch"
    ...
    def test_creation_with_actual_features(self):
        ...
        error = FeatureMismatchError(
            "Feature mismatch", actual_features=actual_features
        )
        assert error.message == "Feature mismatch"
    ...
    def test_creation_with_missing_features(self):
        ...
        error = FeatureMismatchError(
            "Feature mismatch", missing_features=missing_features
        )
        assert error.message == "Feature mismatch"
...
class TestDatasetExceptionIntegration:
    ...
    def test_insufficient_data_with_context(self):
        ...
        error = InsufficientDataError(
            "training_data", 50, 1000, operation="model_training"
        )
        error.with_context(
        ...
        result = str(error)
        assert (
            "Dataset 'training_data' has insufficient data: 50 samples, but 1000 required for model_training"
            in result
        )
    ...
    def test_error_message_construction(self):
        ...
        assert (
            "Dataset 'dataset1' has insufficient data: 5 samples, but 10 required"
            in error1.message
        )

        error2 = InsufficientDataError("dataset2", 100, 1000, operation="validation")
        assert (
            "Dataset 'dataset2' has insufficient data: 100 samples, but 1000 required for validation"
            in error2.message
        )
        ...
        assert (
            error4.message
            == "Validation failed for dataset 'test_data': Invalid format"
        )
