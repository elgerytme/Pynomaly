...
class TestValidationError:
    ...
    def test_creation_with_additional_details(self):
        ...
        error = ValidationError(
            "Invalid value", field="age", value=-5, min_value=0, max_value=120
        )
        assert error.details["field"] == "age"
        assert error.details["value"] == -5
...
class TestConfigurationError:
    ...
    def test_none_values(self):
        ...
        error = ConfigurationError(
            "Invalid configuration", parameter=None, expected=None, actual=None
        )
        assert error.message == "Invalid configuration"
...
class TestAuthenticationError:
    ...
    def test_creation_with_all_parameters(self):
        ...
        error = AuthenticationError(
            "Login failed", username="testuser", reason="Invalid password"
        )
    ...
    def test_creation_with_additional_details(self):
        """Test creation with additional details."""
        error = AuthenticationError(
            username="testuser", reason="Invalid password", attempts=3, locked=True
        )
        assert error.details["username"] == "testuser"
        assert error.details["reason"] == "Invalid password"
...
class TestAuthorizationError:
    ...
    def test_none_values(self):
        """Test handling of None values."""
        error = AuthorizationError(
            user_id=None, required_permission=None, required_role=None
        )
...
class TestCacheError:
    ...
    def test_creation_with_all_parameters(self):
        """Test creation with all parameters."""
        error = CacheError("Cache set failed", operation="set", key="user:123")
    ...
    def test_creation_with_additional_details(self):
        """Test creation with additional details."""
        error = CacheError(operation="get", key="user:123", timeout=30, retry_count=3)
        assert error.details["operation"] == "get"
        assert error.details["key"] == "user:123"
...
class TestInfrastructureError:
    ...
    def test_creation_with_all_parameters(self):
        """Test creation with all parameters."""
        error = InfrastructureError(
            "Database connection failed", component="database", operation="connect"
        )
        assert error.message == "Database connection failed"
        assert error.details["component"] == "database"
        assert error.details["operation"] == "connect"
...
class TestExceptionIntegration:
    ...
    def test_exception_with_complex_cause(self):
        ...
        infrastructure_error = InfrastructureError(
            "Service unavailable", component="cache", cause=cache_error
        )
    ...
    def test_error_serialization_compatibility(self):
        ...
        error = ValidationError("Validation failed", field="age", value=-5)
