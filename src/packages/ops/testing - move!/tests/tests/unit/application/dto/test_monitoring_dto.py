...
class TestSystemMetrics:
    ...
    def test_to_dict(self):
        ...
        now = datetime.utcnow()
        metrics = SystemMetrics(
            cpu_usage_percent=75.5, memory_usage_percent=60.2, timestamp=now
        )
...
class TestDetectionMetrics:
    ...
    def test_to_dict(self):
        """Test dictionary conversion."""
        now = datetime.utcnow()
        metrics = DetectionMetrics(active_detections=5, precision=0.95, timestamp=now)
...
class TestRealTimeUpdate:
    ...
    def test_creation(self):
        ...
        update = RealTimeUpdate(update_type="metrics", data=data, timestamp=now)
    ...
    def test_to_dict(self):
        ...
        update = RealTimeUpdate(update_type="detection", data=data, timestamp=now)
