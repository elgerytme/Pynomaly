        ...
        for method, endpoint in endpoints:
            with performance_profiler.profile_operation(f"{method} {endpoint}"):
                ...
                assert response.status_code in [
                    200,
                    404,
                ]  # 404 acceptable for empty resources
            ...
            def stress_worker(worker_id: int):
                ...
                while time.perf_counter() - worker_start < duration:
                    try:
                        ...
                        worker_results.append({"success": False, "response_time": 0})
...
class TestMLPerformance:
    ...
    def test_prediction_performance_scaling(self):
        ...
        assert (
            prediction_results[1000]["throughput"]
            > prediction_results[100]["throughput"]
        )
        assert (
            prediction_results[10000]["throughput"]
            > prediction_results[1000]["throughput"]
        )
    ...
    def test_concurrent_ml_operations(self):
        ...
        for concurrency in concurrency_levels:
            ...
            with ThreadPoolExecutor(max_workers=concurrency) as executor:
                futures = [
                    executor.submit(ml_operation, i) for i in range(concurrency * 2)
                ]
    ...
    def test_memory_efficiency_ml_operations(self):
        ...
        ratio_10k_1k = (
            memory_results[10000]["memory_used_mb"]
            / memory_results[1000]["memory_used_mb"]
        )
    ...
    def test_system_resource_utilization(self):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                # Memory tasks
                memory_futures = [
                    executor.submit(memory_intensive_task) for _ in range(2)
                ]
    ...
    def test_performance_regression_detection(self):
        ...
        for metric, value in current_performance.items():
            ...
            if metric == "prediction_throughput":
                ...
                print(
                    f"  {metric}: {value:.3f} (baseline: {baseline:.3f}, {improvement:+.1f}%)"
                )
            else:
