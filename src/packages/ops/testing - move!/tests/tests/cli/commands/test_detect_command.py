...
class TestDetectCommand:
    ...
    def test_detect_help_command(self):
        ...
        assert (
            "anomaly detection" in result.stdout.lower()
            or "detect" in result.stdout.lower()
        )
    ...
    def test_detect_train_help(self):
        ...
        assert (
            "algorithm" in result.stdout.lower() or "detector" in result.stdout.lower()
        )
    ...
    def test_detect_batch_basic(self, mock_dataset_service, mock_detection_service):
        ...
        result = self.runner.invoke(
            app, ["detect", "batch", "--config", str(batch_config_file)]
        )
    ...
    def test_detect_train_invalid_algorithm(self):
        ...
        assert result.exit_code != 0
        assert (
            "algorithm" in result.stdout.lower() or "invalid" in result.stdout.lower()
        )
    ...
    def test_detect_train_invalid_contamination_rate(self):
        ...
        assert result.exit_code != 0
        assert (
            "contamination" in result.stdout.lower() or "rate" in result.stdout.lower()
        )
    ...
    def test_detect_run_missing_detector(self):
        ...
        assert result.exit_code != 0
        assert (
            "detector" in result.stdout.lower() or "not found" in result.stdout.lower()
        )
