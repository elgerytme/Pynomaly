...
class TestCLIIntegrationWorkflows:
    ...
    def test_complete_anomaly_detection_workflow(
        self, runner, mock_container, sample_dataset_csv
    ):
        ...
        with patch(
            ...
            mock_container.detection_service.return_value.run_detection.return_value = (
                mock_result
            )
            ...
            assert (
                "Dataset loaded successfully" in result.stdout
                or "test_dataset" in result.stdout
            )
            ...
            assert result.exit_code == 0
            assert (
                "Detector created successfully" in result.stdout
                or "test_detector" in result.stdout
            )

            # Step 3: Train detector
            result = runner.invoke(
                app, ["detect", "train", "test_detector", "test_dataset"]
            )

            assert result.exit_code == 0
            assert (
                "Training completed" in result.stdout
                or "trained successfully" in result.stdout
                or "test_detector" in result.stdout
            )
            ...
            assert result.exit_code == 0
            assert (
                "Detection completed" in result.stdout
                or "results" in result.stdout
                or "anomalies" in result.stdout
            )

    def test_configuration_based_workflow(
        self, runner, mock_container, sample_dataset_csv
    ):
    ...
    def test_multi_algorithm_comparison_workflow(
        ...
        with patch(
            ...
            result = runner.invoke(
                app,
                ["dataset", "load", sample_dataset_csv, "--name", "comparison_dataset"],
            )
    ...
    def test_data_preprocessing_workflow(
        ...
        with patch(
            ...
            result = runner.invoke(
                app, ["dataset", "load", sample_dataset_csv, "--name", "raw_dataset"]
            )
    ...
    def test_export_and_sharing_workflow(self, runner, mock_container):
        ...
        with patch(
            ...
            result = runner.invoke(app, ["detect", "results", "--latest", "5"])
            ...
            for format_type in export_formats:
                ...
                result = runner.invoke(
                    app,
                    ["export", format_type, "results.json", f"output.{format_type}"],
                )
    ...
    def test_server_integration_workflow(self, runner, mock_container):
        ...
        with patch(
            ...
            with (
                ...
                result = runner.invoke(
                    app, ["server", "start", "--host", "localhost", "--port", "8000"]
                )
    ...
    def test_monitoring_and_maintenance_workflow(self, runner, mock_container):
        ...
        mock_container.detector_repository.return_value.find_by_id.return_value = (
            mock_detector
        )

        mock_dataset = Mock()
        mock_dataset.name = "Test Dataset"
        mock_container.dataset_repository.return_value.find_by_id.return_value = (
            mock_dataset
        )
    ...
    def test_error_recovery_workflow(self, runner, mock_container):
        ...
        with patch(
            ...
            dataset_service = Mock()
            dataset_service.load_dataset.side_effect = Exception(
            ...
            result = runner.invoke(
                app,
                ["dataset", "load", "nonexistent_file.csv", "--name", "test_dataset"],
            )
            ...
            result = runner.invoke(
                app, ["dataset", "load", valid_file, "--name", "recovered_dataset"]
            )
    ...
    def test_performance_testing_workflow(self, runner, mock_container):
        ...
        with patch(
            ...
            result = runner.invoke(
                app, ["dataset", "load", large_dataset, "--name", "performance_dataset"]
            )
            ...
            start_time = time.time()
            result = runner.invoke(
                app, ["detect", "train", "performance_detector", "performance_dataset"]
            )
...
class TestCLIStressTests:
    ...
    def test_error_handling_under_stress(self, runner, mock_container):
        ...
        mock_container.detector_repository.return_value.count.side_effect = (
            failing_count
        )
        mock_container.dataset_repository.return_value.count.return_value = 3
        mock_container.result_repository.return_value.count.return_value = 12
...
class TestCLICompatibility:
    ...
    def test_configuration_file_compatibility(self, runner):
        ...
        result = runner.invoke(
            app, ["generate-config", "test", "--output", json_file, "--format", "json"]
        )
        ...
        result = runner.invoke(
            app, ["generate-config", "test", "--output", yaml_file, "--format", "yaml"]
        )
