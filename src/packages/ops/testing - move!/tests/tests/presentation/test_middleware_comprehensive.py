...
class TestAuthenticationMiddleware:
    ...
    def test_jwt_token_validation(
        self, client_with_middleware: TestClient, valid_jwt_token
    ):
    ...
    def test_invalid_jwt_token(self, client_with_middleware: TestClient):
        ...
        response = client_with_middleware.get(
            "/api/detectors/", headers=invalid_headers
        )
    ...
    def test_api_key_authentication(self, client_with_middleware: TestClient):
        ...
        with patch("monorepo.infrastructure.auth.validate_api_key") as mock_validate:
            ...
            response = client_with_middleware.get(
                "/api/detectors/", headers=api_key_headers
            )
    ...
    def test_invalid_api_key(self, client_with_middleware: TestClient):
        ...
        with patch("monorepo.infrastructure.auth.validate_api_key") as mock_validate:
            ...
            response = client_with_middleware.get(
                "/api/detectors/", headers=invalid_api_headers
            )
...
class TestRateLimitingMiddleware:
    ...
    def test_rate_limiting_bypass_for_health_checks(
    ...
    def test_rate_limiting_different_endpoints(
        ...
        detector_auth_failures = sum(
            1 for r in detector_responses if r.status_code == 401
        )
...
class TestRequestValidationMiddleware:
    ...
    def test_sql_injection_prevention(self, client_with_middleware: TestClient):
        ...
        for injection_data in injection_attempts:
            response = client_with_middleware.post(
                "/api/detectors/", json=injection_data
            )
...
class TestMiddlewareIntegration:
    ...
    def test_concurrent_request_handling_with_middleware(
        self, client_with_middleware: TestClient
    ):
