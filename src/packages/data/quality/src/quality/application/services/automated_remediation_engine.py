...
from ...domain.entities.quality_issue import QualityIssue, IssueStatus, RemediationAction as DomainRemediationAction
from ...domain.entities.quality_lineage import QualityLineageGraph
from ...domain.entities.validation_rule import ValidationResult
...
class RemediationHandler(ABC):
    ...
    @abstractmethod
    async def can_handle(self, issue: QualityIssue) -> bool:
        """Check if this handler can handle the given issue."""
        pass
    
    @abstractmethod
    async def create_plan(self, issue: QualityIssue) -> RemediationPlan:
...
class AutomatedRemediationEngine:
    ...
    async def analyze_and_remediate(self, issue: QualityIssue, data: Any) -> Optional[RemediationResult]:
    ...
    async def _find_handler(self, issue: QualityIssue) -> Optional[RemediationHandler]:
