...
class ModelGovernanceService:
    ...
    async def submit_workflow_for_approval(
        ...
        if workflow.compliance_required:
            ...
            if (
                not compliance_report.is_compliant()
                and compliance_report.has_critical_violations()
            ):
                raise ValueError(
                    "Cannot submit workflow with critical compliance violations"
                )
    ...
    async def process_approval(
        ...
        if status == ApprovalStatus.APPROVED and workflow.config.notify_on_approval:
            ...
            await self._send_rejection_notifications(
                workflow, request_id, approver, comments
            )
    ...
    async def run_compliance_check(
        ...
        report = ComplianceReport(
            ...
            assessment_duration=(
                assessment_end_time - assessment_start_time
            ).total_seconds(),
            created_by="system",
        )

        # Store report
        stored_report = await self.governance_repository.create_compliance_report(
            report
        )
    ...
    async def get_pending_approvals(
        ...
        """
        return await self.governance_repository.get_pending_approvals(
    ...
    async def escalate_approval(
        ...
        # Find the request
        request = next(
            (r for r in workflow.approval_requests if r.id == request_id), None
        )
        if not request:
        ...
        await self._send_escalation_notifications(
            workflow, request, escalated_by, reason
        )
