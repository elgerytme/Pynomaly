...
from ....core.domain_entities import Dataset
from ....core.security_configuration import get_security_config
...
async def require_tenant_admin(tenant_id: UUID, current_user=Depends(get_current_user)):
    """Require tenant admin permissions."""
    if not (
        current_user.is_super_admin()
        or current_user.has_role_in_tenant(
            TenantId(str(tenant_id)), UserRole.TENANT_ADMIN
        )
    ):
        raise HTTPException(
...
async def get_user(
    ...
    try:
        ...
        if not current_user.is_super_admin():
            common_tenants = set(current_user.get_tenant_ids()) & set(
                user.get_tenant_ids()
            )
        ...
        audit_logger.log_event(
            ...
            outcome="success",
            severity=AuditSeverity.LOW,
            details={"action": "view_user", "target_user_id": str(user_id)},
        )
...
async def update_user(
    ...
    try:
        user = await user_service.update_user(
            user_id=UserId(str(user_id)), update_data=request.dict(exclude_unset=True)
        )
...
async def get_tenant(
    ...
    try:
        tenant = await user_service._tenant_repo.get_tenant_by_id(
...
async def get_tenant_usage(
    ...
    try:
        usage_report = await user_service.get_tenant_usage_report(
            TenantId(str(tenant_id))
        )
...
async def get_tenant_users(
    ...
    try:
        users = await user_service._user_repo.get_users_by_tenant(
            TenantId(str(tenant_id))
        )
...
async def remove_user_from_tenant(
    ...
    try:
        success = await user_service._user_repo.remove_user_from_tenant(
            user_id=UserId(str(user_id)), tenant_id=TenantId(str(tenant_id))
        )
