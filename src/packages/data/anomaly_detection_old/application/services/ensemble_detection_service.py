...
class WeightedAverageCombiner(EnsembleCombiner):
    ...
    async def combine_predictions(
        ...
        weighted_agreement = (
            sum(
                member_weights.get(member_id, 1.0)
                * (member_predictions[member_id] == final_prediction)
...
class DynamicSelectionCombiner(EnsembleCombiner):
    ...
    def update_performance(self, member_id: str, performance: float) -> None:
        ...
        if len(self.performance_history[member_id]) > self.window_size:
            self.performance_history[member_id] = self.performance_history[member_id][
                -self.window_size :
            ]
    ...
    async def combine_predictions(
        ...
        for member_id in member_scores:
            if (
                member_id in self.performance_history
                and self.performance_history[member_id]
            ):
...
class EnsembleDetectionService:
    ...
    async def add_member(
        ...
        if len(self.members) >= self.config.max_members:
            raise ValidationError(
                f"Maximum ensemble members ({self.config.max_members}) reached"
            )
    ...
    async def remove_member(self, detector_id: UUID) -> bool:
        ...
        for i, member in enumerate(self.members):
            if member.detector.id == detector_id:
                ...
                self.logger.info(
                    f"Removed ensemble member: {removed_member.detector.name}"
                )
    ...
    async def predict(
        ...
        for member in self.members:
            ...
            try:
                ...
                self.logger.warning(
                    f"Member {member.detector.name} prediction failed: {e}"
                )
                continue
        ...
        if self.config.estimate_uncertainty:
            uncertainty = await self._estimate_uncertainty(
                member_scores, member_predictions
            )
    ...
    async def train_ensemble(
        ...
        performance = await self._evaluate_ensemble_performance(
            validation_data or training_data
        )

        self._performance_history.append(performance)

        self.logger.info(
            f"Ensemble training completed. Performance: {performance.ensemble_score:.3f}"
        )
    ...
    async def _optimize_weights(
        ...
        if self.config.weighting_strategy == WeightingStrategy.EQUAL:
            ...
            total_performance = sum(
                m.performance_score for m in self.members if m.enabled
            )
    ...
    async def _calculate_diversity_scores(self, dataset: Dataset) -> None:
        ...
        for i, id1 in enumerate(member_ids):
            for j, id2 in enumerate(member_ids):
                if i != j:
                    corr = np.corrcoef(
                        member_predictions[id1], member_predictions[id2]
                    )[0, 1]
        ...
        if total_diversity > 0:
            for member in self.members:
                if member.enabled:
                    ...
                    combined_score = (
                        member.performance_score * 0.7 + member.diversity_score * 0.3
                    )
    ...
    async def _calculate_adaptive_weights(self) -> None:
        ...
        for member in self.members:
            if member.enabled:
                ...
                recent_performance = member.performance_score * np.random.uniform(
                    0.8, 1.2
                )
    ...
    async def _evaluate_ensemble_performance(
        ...
        best_individual_score = (
            max(individual_scores.values()) if individual_scores else 0.0
        )
        ...
        weight_distribution = {
            str(m.detector.id): m.weight for m in self.members if m.enabled
        }
        ...
        return EnsemblePerformance(
            ...
            diversity_score=np.mean(
                [m.diversity_score for m in self.members if m.enabled]
            ),
    ...
    async def prune_weak_members(self) -> list[str]:
        ...
        for member in self.members[:]:  # Copy list to allow modification
            if (
                member.enabled
                and member.performance_score < self.config.pruning_threshold
