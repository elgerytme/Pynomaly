...
class PerformanceBenchmark:
    ...
    async def benchmark_algorithm(
        ...
        try:
            ...
            true_positives = sum(
                1 for i in range(len(predictions)) if predictions[i] and true_labels[i]
            )
            false_positives = sum(
                1
                for i in range(len(predictions))
                if predictions[i] and not true_labels[i]
            )
            true_negatives = sum(
                1
                for i in range(len(predictions))
                if not predictions[i] and not true_labels[i]
            )
            false_negatives = sum(
                1
                for i in range(len(predictions))
                if not predictions[i] and true_labels[i]
            )
    ...
    async def run_scalability_benchmark(self):
        ...
        print(
            f"Testing {len(algorithms)} algorithms on {len(dataset_sizes)} dataset sizes"
        )
        ...
        for size in dataset_sizes:
            ...
            for algorithm in algorithms:
                ...
                print(
                    f"Train: {result.training_time_ms:.1f}ms, "
    ...
    async def run_feature_dimensionality_benchmark(self):
        ...
        print(
            f"Testing {len(algorithms)} algorithms on {len(feature_counts)} feature dimensions"
        )
    ...
    async def run_parameter_sensitivity_benchmark(self):
        ...
        for n_est in estimator_counts:
            ...
            result = await self.benchmark_algorithm(
                "IsolationForest", data, labels, params
            )
            self.results.append(result)
    ...
    def generate_performance_report(self):
        ...
        df = pd.DataFrame(
            [
                ...
                if r.training_time_ms != float("inf")
            ]
        )
        ...
        if not base_results.empty:
            summary = (
                base_results.groupby("Algorithm")
                .agg(
                    {
                        "Train_Time_ms": "mean",
        ...
        accuracy_ranking = (
            df.groupby("Algorithm")["F1_Score"].mean().sort_values(ascending=False)
        )
        ...
        scalability_data = (
            df[df["Features"] == 10]
            .groupby(["Algorithm", "Dataset_Size"])["Train_Time_ms"]
            .mean()
            .unstack()
        )
