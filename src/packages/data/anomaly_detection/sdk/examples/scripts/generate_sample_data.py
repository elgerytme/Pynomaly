...
def generate_time_series_anomalies(
    n_samples: int = 1000, contamination: float = 0.05, random_state: int = 42
) -> pd.DataFrame:
    ...
    for i, idx in enumerate(anomaly_indices):
        if i % 3 == 0:
            ...
            # Variance change
            signal[max(0, idx - 5) : min(idx + 5, n_samples)] *= np.random.uniform(
                1.5, 2.5
            )
...
def generate_clustering_anomalies(
    ...
    for i in range(n_clusters):
        cluster_data = (
            np.random.randn(samples_per_cluster, n_features) * 0.5 + centers[i]
        )
    ...
    anomaly_data = np.random.uniform(
        X.min(axis=0) - 5, X.max(axis=0) + 5, size=(n_anomalies, n_features)
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate sample datasets for Pynomaly"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
