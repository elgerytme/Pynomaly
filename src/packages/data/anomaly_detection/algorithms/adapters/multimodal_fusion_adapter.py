...
class MultiModalConfig(BaseModel):
    ...
    time_series_weight: float = Field(
        default=0.25, description="Weight for time series modality"
    )
    tabular_weight: float = Field(
        default=0.25, description="Weight for tabular modality"
    )
    ...
    fusion_method: str = Field(
        default="weighted_average", description="Fusion method for combining scores"
    )
    cross_modal_analysis: bool = Field(
        default=True, description="Enable cross-modal correlation analysis"
    )

    # Thresholds
    consensus_threshold: float = Field(
        default=0.6, description="Consensus threshold for multi-modal detection"
    )
    cross_modal_threshold: float = Field(
        default=0.7, description="Threshold for cross-modal anomalies"
    )
    ...
    def predict(self, data: Any) -> tuple[list[float], list[bool]]:
        ...
        if hasattr(self.detector, "predict"):
            ...
            if isinstance(predictions, np.ndarray):
                ...
                labels = [
                    pred == -1 if pred in [-1, 1] else bool(pred)
...
class MultiModalFusionAdapter(DetectorProtocol):
    ...
    def _has_time_series_structure(self, df: pd.DataFrame) -> bool:
        ...
        has_datetime = any(
            df[col].dtype.name.startswith("datetime") for col in df.columns
        )
        has_time_column = any(
            col.lower() in ["time", "timestamp", "date"] for col in df.columns
        )
    ...
    def _has_graph_structure(self, df: pd.DataFrame, dataset: Dataset) -> bool:
        ...
        ) or "edge_index" in df.columns

        # Check for adjacency matrix in metadata
        has_adjacency = (
            "adjacency_matrix" in dataset.metadata if dataset.metadata else False
        )
    ...
    def _has_text_data(self, df: pd.DataFrame) -> bool:
        ...
        for col in text_columns:
            ...
            if any(
                isinstance(val, str) and len(val.split()) > 3 for val in sample_values
            ):
                return True
        ...
        for modality in self.data_types:
            if modality in self.modality_detectors:
                try:
                    if modality == "text":
                        text_data = self._extract_text_data(dataset)
                        scores, labels = self.modality_detectors[modality].predict(
                            text_data
                        )
                    else:
                        scores, labels = self.modality_detectors[modality].predict(
        ...
        return DetectionResult(
            ...
            metadata={
                ...
                "modality_weights": {
                    ...
                    for mod in self.data_types
                    if mod in self.modality_detectors
                },
                "cross_modal_anomalies": cross_modal_anomalies,
                "n_anomalies": len(anomaly_indices),
                "contamination_rate": len(anomaly_indices) / n_samples
                if n_samples > 0
                else 0,
            },
        )
    ...
    def _fuse_predictions(
        self, modality_results: dict[str, dict], n_samples: int
    ...
    def _detect_cross_modal_anomalies(
        self, modality_results: dict[str, dict]
    ) -> list[int]:
        ...
        if modality_labels:
            ...
            cross_modal_threshold = max(
                2, len(modality_results) * self.config.cross_modal_threshold
            )
