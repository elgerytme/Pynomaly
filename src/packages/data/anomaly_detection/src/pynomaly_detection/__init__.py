...
A production-ready anomaly detection library with clean architecture and multiple algorithms.
    ...
    def detect(self, data: ArrayLike, **kwargs: Any) -> PredictionArray:
        ...
        if self.detection_service:
            ...
            try:
                ...
                if algorithm == 'lof':
                    # LOF requires different approach as it doesn't have separate fit/predict
                    lof = LocalOutlierFactor(**{k: v for k, v in kwargs.items() if k != 'algorithm'})
                    predictions = lof.fit_predict(data)
                elif algorithm == 'ocsvm':
                    # Filter out parameters that OneClassSVM doesn't accept
                    ocsvm_params = {k: v for k, v in kwargs.items() if k not in ['algorithm', 'contamination']}
                    model = OneClassSVM(**ocsvm_params)
                    predictions = model.fit_predict(data)
                else:
                    # Default to IsolationForest
                    model = IsolationForest(**{k: v for k, v in kwargs.items() if k != 'algorithm'}) if not self._model else self._model
                    predictions = model.fit_predict(data)
