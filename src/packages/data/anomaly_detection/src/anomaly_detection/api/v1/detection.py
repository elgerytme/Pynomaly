...
from anomaly_detection.infrastructure.logging import async_log_decorator, get_logger
...
class DetectionResponse(BaseModel):
    ...
    scores: list[float] | None = Field(None, description="Anomaly confidence scores")
...
async def ensemble_detect(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ensemble detection failed: {e!s}"
        )
...
async def train_model(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid training data: {e!s}"
        )
    except Exception as e:
        logger.error("Training processing error", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Model training failed: {e!s}"
        )
