...
class EnsembleService:
    ...
    async def get_ensemble_explanations(
        ...
        for idx in anomaly_indices:
            ...
            # Calculate consensus
            votes_anomaly = sum(
                1 for op in detector_opinions.values() if op["label"] == 1
            )
...
class EnsembleDetector:
    ...
    def detect(self, dataset: Dataset) -> DetectionResult:
        ...
        # Aggregate scores
        weights_by_name = {
            d.name: self._weights.get(d.id, 1.0) for d in self.base_detectors
        }

        aggregated_scores = self.aggregator.aggregate_scores(
            all_scores, weights=weights_by_name, method=self.aggregation_method
        )

        # Aggregate labels
        aggregated_labels = self.aggregator.aggregate_labels(
            all_labels, weights=weights_by_name, method="majority"
        )
    ...
    def get_current_weights(self) -> dict[str, float]:
        ...
        return {d.name: self._weights.get(d.id, 1.0) for d in self.base_detectors}
