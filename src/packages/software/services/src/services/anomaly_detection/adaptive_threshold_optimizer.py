...
class AdaptiveThresholdOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized adaptive threshold optimizer with strategy: {optimization_strategy}"
        )
    ...
    def _initialize_threshold_candidates(self) -> None:
        ...
        thresholds = np.linspace(
            self.threshold_range[0], self.threshold_range[1], num_candidates
        )

        for threshold in thresholds:
            self.threshold_candidates[threshold] = ThresholdCandidate(
                threshold=threshold
            )
    ...
    async def get_adaptive_threshold(
        ...
        if context:
            ...
            if context_key in self.context_thresholds:
                ...
                context_weight = min(
                    0.5, len(self.context_performance[context_key]) / 20.0
                )
                threshold = (
                    1 - context_weight
                ) * threshold + context_weight * context_threshold

                adaptation_info["adaptation_applied"] = True
                adaptation_info["adaptation_reason"] = "context_specific"
                adaptation_info["context_adjustment"] = (
                    context_threshold - self.current_threshold
                )

        # Apply score distribution adaptation
        if len(anomaly_scores) > 10:
            distribution_adjustment = await self._calculate_distribution_adjustment(
                anomaly_scores
            )
    ...
    async def _calculate_distribution_adjustment(
        self, anomaly_scores: np.ndarray
    ) -> float:
    ...
    def _calculate_threshold_confidence(self, threshold: float) -> float:
        ...
        try:
            ...
            feedback_confidence = min(
                1.0, len(self.feedback_history) / self.optimization_window
            )

            # Performance consistency confidence
            if len(self.recent_performance) >= 3:
                recent_f1_scores = [
                    m.f1_score for m in list(self.recent_performance)[-5:]
                ]
            ...
            if self.threshold_candidates:
                ...
                if current_candidate:
                    exploration_confidence = min(
                        1.0, current_candidate.exploration_count / 10.0
                    )
    ...
    async def _update_threshold_candidate_stats(
        self, feedback: ThresholdFeedback
    ) -> None:
    ...
    async def _update_context_performance(
        self, context_key: str, feedback: ThresholdFeedback
    ) -> None:
    ...
    async def _check_optimization_trigger(self) -> bool:
        ...
        feedback_trigger = (
            len(self.feedback_history) >= self.min_feedback_for_optimization
        )
    ...
    async def _optimize_threshold(self) -> dict[str, Any]:
        ...
        try:
            if (
                ...
                == ThresholdOptimizationStrategy.BAYESIAN_OPTIMIZATION
            ):
                new_threshold = await self._bayesian_optimization()
            elif (
                self.optimization_strategy
                == ThresholdOptimizationStrategy.GRADIENT_DESCENT
            ):
                new_threshold = await self._gradient_descent_optimization()
            elif (
                self.optimization_strategy
                == ThresholdOptimizationStrategy.BINARY_SEARCH
            ):
                new_threshold = await self._binary_search_optimization()
            elif (
                self.optimization_strategy == ThresholdOptimizationStrategy.GRID_SEARCH
            ):
                new_threshold = await self._grid_search_optimization()
            elif (
                self.optimization_strategy == ThresholdOptimizationStrategy.EVOLUTIONARY
            ):
                new_threshold = await self._evolutionary_optimization()
            elif (
                self.optimization_strategy
                == ThresholdOptimizationStrategy.ADAPTIVE_BANDITS
            ):
            ...
            if abs(new_threshold - self.current_threshold) > 0.01:
                old_performance = await self._evaluate_threshold_performance(
                    self.current_threshold
                )
                new_performance = await self._evaluate_threshold_performance(
                    new_threshold
                )
                ...
                if new_performance > old_performance:
                    ...
                    logger.info(
                        f"Threshold optimized: {self.current_threshold:.4f} (improvement: {optimization_result['improvement']:.4f})"
                    )
                else:
    ...
    async def _bayesian_optimization(self) -> float:
        ...
        if len(self.bayesian_state["observations"]) > 100:
            self.bayesian_state["observations"] = self.bayesian_state["observations"][
                -100:
            ]
        ...
        if candidate_performances:
            ...
            sorted_candidates = sorted(
                candidate_performances.items(), key=lambda x: x[1], reverse=True
            )
    ...
    async def _gradient_descent_optimization(self) -> float:
        ...
        new_threshold = np.clip(
            new_threshold, self.threshold_range[0], self.threshold_range[1]
        )
    ...
    async def _binary_search_optimization(self) -> float:
        ...
        best_performance = await self._evaluate_threshold_performance(
            self.current_threshold
        )
    ...
    async def _evolutionary_optimization(self) -> float:
        ...
        population = np.random.normal(self.current_threshold, 0.1, population_size)
        population = np.clip(
            population, self.threshold_range[0], self.threshold_range[1]
        )
        ...
        for generation in range(generations):
            ...
            for i in range(len(elite), population_size):
                ...
                child = np.clip(
                    parent + mutation, self.threshold_range[0], self.threshold_range[1]
                )
        ...
        for threshold, candidate in self.threshold_candidates.items():
            ...
            if n_evaluations == 0:
                ...
                confidence_bonus = np.sqrt(
                    2 * np.log(total_evaluations) / n_evaluations
                )
    ...
    async def _evaluate_threshold_performance(self, threshold: float) -> float:
        ...
        candidate = None
        closest_threshold = min(
            self.threshold_candidates.keys(), key=lambda t: abs(t - threshold)
        )
        ...
        for feedback in self.feedback_history:
            ...
            actual_anomaly = feedback.feedback_type in [
    ...
    def _get_feedback_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "false_positive_rate": feedback_counts[FeedbackType.FALSE_POSITIVE.value]
            / total_feedback,
            "false_negative_rate": feedback_counts[FeedbackType.FALSE_NEGATIVE.value]
            / total_feedback,
            "accuracy_estimate": (
    ...
    async def get_optimization_report(self) -> dict[str, Any]:
        ...
        report = {
            "optimizer_status": {
                ...
                "optimization_interval": self.optimization_interval.total_seconds()
                / 60,
            },
    ...
    def _generate_optimization_recommendations(self) -> list[str]:
        ...
        if not feedback_summary.get("no_feedback"):
            ...
            if fpr < 0.05 and fnr < 0.05:
                recommendations.append(
                    "Excellent threshold performance - consider fine-tuning for marginal improvements"
                )

        # Check exploration vs exploitation
        total_explorations = sum(
            c.exploration_count for c in self.threshold_candidates.values()
        )
        total_exploitations = sum(
            c.exploitation_count for c in self.threshold_candidates.values()
        )

        if total_explorations < total_exploitations / 10:
            recommendations.append(
                "Low exploration detected - consider increasing exploration rate"
            )
        ...
        if (
            time_since_optimization
            > self.optimization_interval.total_seconds() / 60 * 2
        ):
            recommendations.append(
                "Long time since last optimization - consider more frequent optimization"
            )
