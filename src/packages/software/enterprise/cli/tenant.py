...
from interfaces.domain.entities.tenant import SubscriptionTier, TenantStatus
        ...
        with Progress(
            ...
            try:
                ...
                if auto_activate:
                    ...
                    console.print(
                        f"[yellow]Use 'software tenant activate {tenant.tenant_id}' to activate[/yellow]"
                    )
...
def usage(tenant_identifier: str):
    ...
    async def show_usage():
        ...
        measurements = await tenant_service.get_tenant_measurements(tenant.tenant_id)

        if not measurements:
            console.print("[red]Could not retrieve tenant measurements[/red]")
            return

        _display_tenant_measurements(measurements)
...
def _display_tenant_detail(tenant):
    ...
    console.print(
        Panel(
            ...
            f"Max Processor Size: {config.max_processor_size_mb} MB\n"
...
def _display_tenant_metrics(metrics):
    """Display comprehensive tenant measurements."""
    tenant_info = measurements.get("tenant_info", {})
    measurements.get("quota_status", {})
    resource_usage = measurements.get("resource_usage", {})
    billing = measurements.get("billing", {})

    # Tenant info
    console.print(
        Panel(
            f"[bold blue]Tenant Measurements[/bold blue]\n\n"
            ...
            f"Active Jobs: {measurements.get('active_jobs', 0)}\n"
