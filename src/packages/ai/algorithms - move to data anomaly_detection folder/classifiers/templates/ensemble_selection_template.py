...
class EnsembleSelector:
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ensemble_strategies": {
                "simple_voting": {"method": "majority_vote", "weights": None},
                "weighted_voting": {
                ...
                "rank_averaging": {"method": "average_ranks", "rank_strategy": "dense"},
                "stacking": {
                    "method": "stacking",
            ...
            "selection_criteria": {
                ...
                "weights": {"performance": 0.6, "diversity": 0.3, "efficiency": 0.1},
            },
            "optimization": {
    ...
    def select_ensemble(
        ...
        strategy_results = self._evaluate_ensemble_strategies(ensemble_candidates, X, y)
        ...
        validation_results = self._validate_selected_ensembles(selection_results, X, y)
    ...
    def _train_base_algorithms(self, X: np.ndarray, y: np.ndarray | None):
        ...
        for algo_name, algo_config in base_config.items():
            ...
            try:
                ...
                self.base_algorithms[algo_name] = {"status": "failed", "error": str(e)}

    def _analyze_algorithm_diversity(
        ...
        successful_algorithms = {
            name: data
            for name, data in self.base_algorithms.items()
        ...
        diversity_measures["disagreement"] = {
            ...
            "average_disagreement": np.mean(
                disagreement_matrix[np.triu_indices_from(disagreement_matrix, k=1)]
            ),
        }
        ...
        if scores_matrix:
            correlation_matrix = np.corrcoef(
                [scores_matrix[algo] for algo in algo_names if algo in scores_matrix]
            )
        ...
        for i, algo1 in enumerate(algo_names):
            for j, algo2 in enumerate(algo_names):
                if i != j:
                    ...
                    p_e = np.mean(pred1) * np.mean(pred2) + (1 - np.mean(pred1)) * (
                        1 - np.mean(pred2)
                    )  # Expected agreement
        ...
        diversity_measures["kappa"] = {
            "matrix": kappa_matrix,
            "algorithm_names": algo_names,
            "average_kappa": np.mean(
                kappa_matrix[np.triu_indices_from(kappa_matrix, k=1)]
            ),
        }

        # Overall diversity score
        avg_disagreement = diversity_measures["disagreement"]["average_disagreement"]
        avg_correlation = diversity_measures.get("correlation", {}).get(
        ...
        diversity_score = (
            avg_disagreement + (1 - abs(avg_correlation)) + (1 - avg_kappa)
        ) / 3
    ...
    def _generate_ensemble_candidates(self) -> list[list[str]]:
        ...
        max_size = min(
            optimization_config["max_ensemble_size"], len(successful_algorithms)
        )
        ...
        for strategy_name, strategy_config in strategy_config.items():
            ...
            for i, ensemble_combo in enumerate(ensemble_candidates):
                try:
                    ...
                    logger.warning(
                        f"Failed to evaluate ensemble {ensemble_combo} with strategy {strategy_name}: {str(e)}"
                    )
            ...
            if ensemble_performances:
                ...
                if y is not None:
                    ...
                    strategy_results[strategy_name]["average_performance"] = np.mean(
                        performances
                    )
    ...
    def _apply_ensemble_strategy(
        ...
        if method == "majority_vote":
            ...
            ensemble_scores = (
                np.average(scores, axis=0, weights=weights)
                if scores is not None
                else None
            )
            ...
            if scores is not None:
                ...
                ranked_scores = np.array(
                    [rankdata(-score) for score in scores]
                )  # Negative for proper ranking
                ensemble_ranks = np.mean(ranked_scores, axis=0)
                ensemble_predictions = (
                    ensemble_ranks <= np.percentile(ensemble_ranks, 10)
                ).astype(int)
        ...
        if y is not None:
            ...
            metrics["precision"] = precision_score(
                y, ensemble_predictions, zero_division=0
            )
    ...
    def _calculate_ensemble_weights(
        self, ensemble_algorithms: list[str], weight_strategy: str
    ) -> np.ndarray:
    ...
    def _stacking_ensemble(
        ...
        if y is None:
            ...
            predictions = np.array(
                [
                    self.base_algorithms[algo]["predictions"]
    ...
    def _dynamic_selection_ensemble(
        self,
        ensemble_algorithms: list[str],
        strategy_config: dict[str, Any],
        X: np.ndarray,
        ...
        predictions = np.array(
            [self.base_algorithms[algo]["predictions"] for algo in ensemble_algorithms]
        )
        ...
        for i in range(len(X)):
            ...
            neighbor_indices = np.random.choice(
                len(X), min(k_nearest, len(X)), replace=False
            )
            ...
            for j, _algo_name in enumerate(ensemble_algorithms):
                ...
                competence = np.mean(
                    algo_predictions[neighbor_indices] == y[neighbor_indices]
                )
    ...
    def _optimize_ensemble_composition(
        self, strategy_results: dict[str, Any], X: np.ndarray, y: np.ndarray | None
    ) -> dict[str, Any]:
    ...
    def _exhaustive_search_optimization(
    ...
    def _greedy_search_optimization(
        ...
        current_performance = (
            self.base_algorithms[best_individual].get("metrics", {}).get("roc_auc", 0)
        )
        ...
        remaining_algorithms = [
            algo for algo in successful_algorithms if algo != best_individual
        ]
        ...
        while len(current_ensemble) < max_size and remaining_algorithms:
            ...
            if (
                best_addition and best_improvement > 0.001
    ...
    def _evaluate_greedy_ensemble(
        self, ensemble_algorithms: list[str], X: np.ndarray, y: np.ndarray
    ) -> float:
    ...
    def _select_best_ensembles(
        self, optimization_results: dict[str, Any], diversity_results: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for ensemble_name, ensemble_data in ensemble_candidates.items():
            ...
            # Diversity score
            diversity_score = self._calculate_ensemble_diversity_score(
                algorithms, diversity_results
            )

            # Efficiency score (simplified - based on ensemble size)
            max_size = self.config["optimization"]["max_ensemble_size"]
            efficiency_score = (
                1 - (len(algorithms) - 2) / (max_size - 2) if max_size > 2 else 1
            )
    ...
    def _calculate_ensemble_diversity_score(
        self, algorithms: list[str], diversity_results: dict[str, Any]
    ) -> float:
        ...
        for i in range(len(ensemble_indices)):
            for j in range(i + 1, len(ensemble_indices)):
                ...
                if (
                    idx1 < disagreement_matrix.shape[0]
                    and idx2 < disagreement_matrix.shape[1]
                ):
                    diversity_values.append(disagreement_matrix[idx1, idx2])

        return np.mean(diversity_values) if diversity_values else 0.5

    def _validate_selected_ensembles(
        self, selection_results: dict[str, Any], X: np.ndarray, y: np.ndarray | None
    ) -> dict[str, Any]:
        ...
        for train_idx, test_idx in cv.split(X, y):
            ...
            cv_scores["precision"].append(
                precision_score(y_test, final_predictions, zero_division=0)
            )
            cv_scores["recall"].append(
                recall_score(y_test, final_predictions, zero_division=0)
            )
    ...
    def _generate_ensemble_recommendations(
        self, selection_results: dict[str, Any], validation_results: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for i, (ensemble_name, ensemble_info) in enumerate(
            ranked_ensembles[1:4]
        ):  # Next 3 best
    ...
    def _generate_recommendation_reasons(
        self, ensemble_info: dict[str, Any]
    ) -> list[str]:
        ...
        if not reasons:
            reasons.append(
                "Best overall combination of performance, diversity, and efficiency"
            )
    ...
    def _generate_implementation_notes(
        self, selected_best: tuple | None, validation_results: dict[str, Any]
    ) -> list[str]:
        ...
        if selected_best:
            ...
            if "OneClassSVM" in algorithms:
                notes.append(
                    "OneClassSVM requires feature scaling - ensure data is standardized"
                )

            if "LocalOutlierFactor" in algorithms:
                notes.append(
                    "LOF is sensitive to the choice of k neighbors - consider tuning"
                )

            if len(algorithms) > 3:
                notes.append(
                    "Large ensemble - consider performance impact in production"
                )
            ...
            if validation_stats:
                ...
                if roc_auc_std > 0.1:
                    notes.append(
                        "High performance variance - consider more stable ensemble"
                    )
...
def main():
    ...
    if "overall_diversity_score" in diversity_analysis:
        ...
        print(
            f"Average Disagreement: {diversity_analysis['disagreement']['average_disagreement']:.3f}"
        )
    ...
    if alternatives:
        ...
        for alt in alternatives[:2]:  # Show top 2 alternatives
            print(
                f"- Rank {alt['rank']}: {', '.join(alt['algorithms'])} "
