...
class PerformanceMetrics:
    ...
    def __post_init__(self) -> None:
        ...
        if self.roc_auc is not None:
            ...
            if not 0.0 <= self.roc_auc <= 1.0:
                raise ValueError(
                    f"ROC AUC must be between 0.0 and 1.0, got {self.roc_auc}"
                )
        ...
        if self.pr_auc is not None:
            ...
            if not 0.0 <= self.pr_auc <= 1.0:
                raise ValueError(
                    f"PR AUC must be between 0.0 and 1.0, got {self.pr_auc}"
                )
        ...
        if not isinstance(self.model_size, int) or self.model_size < 0:
            raise ValueError(
                f"Model size must be non-negative integer, got {self.model_size}"
            )
        ...
        if self.cpu_usage is not None:
            if (
                ...
                raise ValueError(
                    f"CPU usage must be between 0 and 100, got {self.cpu_usage}"
                )
        ...
        if self.throughput_rps is not None:
            if (
                ...
                raise ValueError(
                    f"Throughput must be non-negative, got {self.throughput_rps}"
                )
    ...
    def is_better_than(
        self, other: PerformanceMetrics, metric: str = "performance_score"
    ) -> bool:
