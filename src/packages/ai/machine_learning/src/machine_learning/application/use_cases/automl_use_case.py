...
class AutoMLResponse:
    ...
    algorithm_rankings: list | None = None
    ...
    async def execute(self, request: AutoMLRequest) -> AutoMLResponse:
        ...
        try:
            ...
            objective = objective_map.get(
                request.objective.lower(), OptimizationObjective.AUC
            )
            ...
            detector_id = await self.automl_service.create_optimized_detector(
                automl_result=automl_result, detector_name=request.detector_name
            )
            ...
            logger.info(
                f"AutoML completed successfully for dataset {request.dataset_id}. "
            ...
            return AutoMLResponse(
                success=False, error=str(e), message="AutoML optimization failed"
            )
