...
class ActiveLearningSession:
    ...
    def add_feedback(self, feedback: HumanFeedback) -> None:
        ...
        if feedback.sample_id not in self.selected_samples:
            raise ValueError(
                f"Sample {feedback.sample_id} not selected for this session"
            )
    ...
    def get_feedback_quality_metrics(self) -> dict[str, float]:
        ...
        feedback_weights = [
            feedback.get_feedback_weight() for feedback in self.feedback_collection
        ]
        ...
        return {
            ...
            "high_confidence_rate": len(high_confidence_feedback)
            / len(self.feedback_collection),
        }
    ...
    def is_session_complete(self) -> bool:
        ...
        if self.target_corrections:
            corrections = sum(
                1 for feedback in self.feedback_collection if feedback.is_correction()
            )
