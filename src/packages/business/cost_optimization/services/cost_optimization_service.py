...
from monorepo_detection.domain.entities.cost_optimization import (
    CloudResource,
    CostBudget,
...
class CostAnalysisEngine:
    ...
    def analyze_cost_trends(
        self, resources: list[CloudResource], days: int = 30
    ) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "top_cost_drivers": [],
            "cost_trends": {"7d_change": 0.0, "30d_change": 0.0, "growth_rate": 0.0},
        ...
        if cost_trends_7d:
            analysis["cost_trends"]["7d_change"] = (
                statistics.mean(cost_trends_7d) if cost_trends_7d else 0.0
            )
        if cost_trends_30d:
            analysis["cost_trends"]["30d_change"] = (
                statistics.mean(cost_trends_30d) if cost_trends_30d else 0.0
            )
            analysis["cost_trends"]["growth_rate"] = (
                analysis["cost_trends"]["30d_change"] / 30
            )  # Daily growth rate
    ...
    def predict_future_costs(
        self, resources: list[CloudResource], months_ahead: int = 12
    ) -> dict[str, Any]:
        ...
        if len(features) < 10:  # Not enough data for ML prediction
            ...
            trend_values = [
                r.cost_info.cost_trend_30d
                for r in resources
                if r.cost_info.cost_trend_30d != 0
            ]
            ...
            predictions["total_predicted_cost"] = sum(
                p["predicted_cost"] for p in predictions["monthly_predictions"]
            )
            predictions["key_assumptions"].append(
                "Used trend extrapolation due to limited data"
            )
            ...
            try:
                ...
                return self.predict_future_costs(
                    resources[:9], months_ahead
                )  # Reduce data to trigger fallback
    ...
    def identify_cost_anomalies(
        ...
        costs = [
            r.cost_info.monthly_cost for r in resources if r.cost_info.monthly_cost > 0
        ]
        if not costs:
            return anomalies
                ...
                anomalies.append(
                    {
                        ...
                        "idle_hours": resource.calculate_idle_time().total_seconds()
                        / 3600,
...
class RecommendationEngine:
    ...
    async def generate_recommendations(
        ...
        for resource in resources:
            ...
            recommendations.extend(
                await self._generate_rightsizing_recommendations(resource, strategy)
            )
            recommendations.extend(
                await self._generate_scheduling_recommendations(resource, strategy)
            )
            recommendations.extend(
                await self._generate_instance_type_recommendations(resource, strategy)
            )
            recommendations.extend(
            ...
            recommendations.extend(
                await self._generate_idle_cleanup_recommendations(resource, strategy)
            )
    ...
    async def _generate_rightsizing_recommendations(
        self, resource: CloudResource, strategy: OptimizationStrategy
    ) -> list[OptimizationRecommendation]:
        ...
        if metrics.is_underutilized():
            ...
            memory_reduction = (
                resource.memory_gb - suggested_memory
            ) / resource.memory_gb
            avg_reduction = (cpu_reduction + memory_reduction) / 2

            projected_cost = current_cost * (
                1 - avg_reduction * 0.8
            )  # 80% of reduction translates to cost savings
        ...
        # Upsize if overutilized
            ...
            memory_increase = (
                suggested_memory - resource.memory_gb
            ) / resource.memory_gb
        ...
        if resource.environment in ["development", "test", "staging"]:
            ...
            if monthly_savings > 5:  # Only recommend if savings > $5/month
                ...
                recommendation = OptimizationRecommendation(
                    ...
                    rationale="Non-production resource can be stopped when not in use",
                    current_monthly_cost=current_cost,
    ...
    async def _generate_instance_type_recommendations(
        ...
        if (
            resource.environment in ["development", "test", "staging"]
            and "spot" not in resource.instance_type.lower()
        ):
            ...
            if monthly_savings > 10:
                recommendation = OptimizationRecommendation(
                    ...
                    prerequisites=[
                        "Ensure workload is fault-tolerant",
                        "Test spot instance behavior",
                    ],
                )
                recommendations.append(recommendation)
        ...
        if (
            ...
            recommendation = OptimizationRecommendation(
                ...
                estimated_implementation_time="5-10 minutes",
                prerequisites=[
                    "Confirm 1-year usage commitment",
                    "Budget approval for upfront cost",
                ],
            )
            recommendations.append(recommendation)
    ...
    def _filter_recommendations(
        ...
        filters = strategy_filters.get(
            strategy, strategy_filters[OptimizationStrategy.BALANCED]
        )
        ...
        for rec in recommendations:
            if (
                rec.monthly_savings >= filters["min_savings"]
                and risk_levels.get(rec.risk_level, 2) <= max_risk_level
                and rec.confidence_score >= filters["min_confidence"]
            ):
                filtered.append(rec)
...
class CostOptimizationService:
    ...
    async def analyze_costs(self, tenant_id: UUID | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            optimization_potential = sum(
                r.get_optimization_potential() for r in resources
            )
            analysis["optimization_potential"] = {
                "total_resources": len(resources),
                "optimizable_resources": len(
    ...
    async def generate_optimization_plan(
        ...
        try:
            ...
            plan = CostOptimizationPlan(
                ...
                target_monthly_savings=sum(r.cost_info.monthly_cost for r in resources)
            ...
            plan.estimated_implementation_days = (
                len(quick_wins) + (len(recommendations) - len(quick_wins)) * 2
            )
    ...
    async def create_budget(self, budget: CostBudget) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Created budget {budget.name} with ${budget.monthly_limit:.2f}/month limit"
            )
            return True
