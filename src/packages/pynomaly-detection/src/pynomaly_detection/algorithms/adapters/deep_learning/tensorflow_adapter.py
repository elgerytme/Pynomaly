...
class KerasAutoEncoderConfig(BaseModel):
    ...
    encoder_dims: list[int] = Field(
        default=[128, 64, 32], description="Encoder layer dimensions"
    )
    ...
    validation_split: float = Field(
        default=0.2, ge=0.0, le=0.5, description="Validation split"
    )
    early_stopping_patience: int = Field(
        default=15, ge=1, description="Early stopping patience"
    )
...
class KerasVAEConfig(BaseModel):
    ...
    input_dim: int = Field(description="Input dimension")
    encoder_dims: list[int] = Field(
        default=[256, 128], description="Encoder dimensions"
    )
    latent_dim: int = Field(default=32, description="Latent dimension")
    decoder_dims: list[int] = Field(
        default=[128, 256], description="Decoder dimensions"
    )
    ...
    use_reconstruction_error: bool = Field(
        default=True, description="Use reconstruction error"
    )
    use_kl_divergence: bool = Field(
        default=False, description="Include KL divergence in score"
    )
...
class KerasLSTMConfig(BaseModel):
    ...
    recurrent_dropout: float = Field(
        default=0.2, ge=0.0, le=0.9, description="Recurrent dropout"
    )
...
def create_autoencoder_model(config: KerasAutoEncoderConfig) -> Model:
    ...
    # Latent space
    latent = layers.Dense(
        config.latent_dim, activation=config.activation, name="latent"
    )(x)
...
def create_transformer_model(config: TransformerConfig) -> Model:
    ...
    for _ in range(config.num_layers):
        x = transformer_block(
            x, config.d_model, config.num_heads, config.dff, config.dropout_rate
        )
...
class TensorFlowAdapter(DetectorProtocol):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray | None = None) -> TensorFlowAdapter:
        """Train the model."""
        try:
            # Preprocess data
    ...
    def _fit_autoencoder(self, X: np.ndarray):
        """Train AutoEncoder."""
        config = KerasAutoEncoderConfig(input_dim=X.shape[1], **self.model_config)
        ...
        logger.info(
            f"AutoEncoder training completed. Final loss: {history.history['loss'][-1]:.6f}"
        )

    def _fit_vae(self, X: np.ndarray):
        """Train VAE."""
        config = KerasVAEConfig(input_dim=X.shape[1], **self.model_config)
        ...
        logger.info(
            f"VAE training completed. Final loss: {history.history['loss'][-1]:.6f}"
        )

    def _fit_lstm(self, X: np.ndarray):
        """Train LSTM AutoEncoder."""
        config = KerasLSTMConfig(input_dim=X.shape[1], **self.model_config)
        ...
        logger.info(
            f"LSTM training completed. Final loss: {history.history['loss'][-1]:.6f}"
        )

    def _fit_transformer(self, X: np.ndarray):
        """Train Transformer model."""
        config = TransformerConfig(input_dim=X.shape[1], **self.model_config)
        ...
        logger.info(
            f"Transformer training completed. Final loss: {history.history['loss'][-1]:.6f}"
        )
