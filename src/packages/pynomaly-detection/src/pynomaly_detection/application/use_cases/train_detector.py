...
from pynomaly_detection.application.services.enhanced_algorithm_adapter_registry import (
...
class TrainDetectorResponse:
    ...
    training_warnings: list[str] | None = None
    ...
    @property
    def validation_results(self) -> dict | None:
    ...
    async def execute(self, request: TrainDetectorRequest) -> TrainDetectorResponse:
        ...
        if request.validate_data:
            ...
            if quality_report["missing_values"]:
                ...
                warnings.append(
                    f"Missing values detected in {len(quality_report['missing_values'])} features"
                )
            ...
            if quality_report["constant_features"]:
                ...
                warnings.append(
                    f"{len(quality_report['constant_features'])} constant features detected"
                )
        ...
        if (
            request.max_training_time
            and training_time_seconds > request.max_training_time
        ):
            warnings.append(
                f"Training time ({training_time_seconds:.2f}s) exceeded max time ({request.max_training_time}s)"
            )
