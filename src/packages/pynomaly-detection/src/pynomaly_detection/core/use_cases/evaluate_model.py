...
class EvaluateModelResponse:
    ...
    precision_recall_curve: dict[str, np.ndarray] | None = None
    cross_validation_scores: dict[str, list[float]] | None = None
...
class EvaluateModelUseCase:
    ...
    async def execute(self, request: EvaluateModelRequest) -> EvaluateModelResponse:
        ...
        if not request.test_dataset.has_target:
            raise ValueError("Test dataset must have target labels for evaluation")
    ...
    async def _cross_validate(
        ...
        avg_metrics = {
            metric: float(np.mean(scores)) for metric, scores in fold_scores.items()
        }
