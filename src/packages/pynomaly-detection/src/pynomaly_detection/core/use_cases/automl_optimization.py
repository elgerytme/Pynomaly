...
from pynomaly_detection.domain.exceptions import AutoMLError, DomainError
...
class AutoMLOptimizationUseCase:
    ...
    async def get_algorithm_recommendations(
        self, dataset_id: str, max_algorithms: int = 5
        ...
        try:
            ...
            try:
                ...
                result = await self.automl_service.optimize_hyperparameters(
                    dataset_id=dataset_id, algorithm=algorithm_name, objective=objective
                )

                # Create optimized detector
                detector_id = await self.automl_service.create_optimized_detector(
                    result, detector_name=f"AutoML_{algorithm_name}_{dataset_id[:8]}"
                )
                ...
                return AutoMLOptimizationResponse(
                    ...
                    optimization_summary=self.automl_service.get_optimization_summary(
                        result
                    ),
    ...
    async def auto_optimize(
        self, request: AutoMLOptimizationRequest
    ) -> AutoMLOptimizationResponse:
        """
        ...
        try:
            logger.info(
                f"Starting full AutoML optimization for dataset {request.dataset_id}"
            )
    ...
    def _generate_recommendation_reasoning(self, config, profile, score: float) -> str:
    ...
    def _generate_optimization_recommendations(self, result: AutoMLResult) -> list[str]:
        ...
        if result.best_score < 0.6:
            ...
            recommendations.append(
                "Try increasing optimization time for better results"
            )
            recommendations.append(
        ...
        if result.optimization_time < 60:
            recommendations.append(
                "Quick optimization completed - consider longer runs for production"
            )
        elif result.optimization_time > 1800:  # 30 minutes
            recommendations.append(
                "Extensive optimization performed - results are well-tuned"
            )

        # Algorithm-specific recommendations
        if result.best_algorithm == "IsolationForest":
            recommendations.append(
                "Isolation Forest excels with numerical features and high dimensions"
            )
            ...
            recommendations.append(
                "Neural network model may benefit from more training data"
            )
