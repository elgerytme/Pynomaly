...
from pynomaly_detection.domain.exceptions import InvalidDataError
...
class Dataset:
    ...
    def get_numeric_features(self) -> list[str]:
        ...
        return [
            ...
            if self.data[col].dtype in numeric_dtypes and col != self.target_column
        ]
    ...
    def get_categorical_features(self) -> list[str]:
        ...
        return [
            col
            for col in self.feature_names or []
            if self.data[col].dtype == "object" and col != self.target_column
        ]

    def sample(self, n: int, random_state: int | None = None) -> Dataset:
    ...
    def split(
        ...
        # Shuffle and split
        shuffled = self.data.sample(frac=1, random_state=random_state).reset_index(
            drop=True
        )
