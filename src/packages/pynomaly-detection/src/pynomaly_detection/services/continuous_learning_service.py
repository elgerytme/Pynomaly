...
from pynomaly_detection.domain.entities.drift_detection import DriftEvent
...
class ModelUpdateResult:
    ...
    performance_change: PerformanceDelta | None = None
...
class AdaptationAssessment:
    ...
    def is_learning_effectively(self) -> bool:
        ...
        return (
            self.overall_effectiveness > 0.6
            and self.adaptation_success_rate > 0.5
            and self.stability_score > 0.3
        )

    def needs_intervention(self) -> bool:
        """Check if manual intervention is needed."""
        return (
            self.overall_effectiveness < 0.3
            or self.adaptation_success_rate < 0.2
            or self.stability_score < 0.1
        )
...
class ContinuousLearningService:
    ...
    def __init__(
        self, storage_path: Path, default_config: LearningConfiguration | None = None
    ):
    ...
    async def process_feedback_batch(
        ...
        try:
            ...
            logger.info(
                f"Processed feedback batch for session {session_id}: "
    ...
    async def evaluate_adaptation_performance(
        ...
        logger.info(
            f"Evaluated adaptation performance for session {session_id}: "
    ...
    async def adapt_to_drift(
        ...
        try:
            ...
            logger.info(
                f"Adapted to drift for session {session_id}: "
    ...
    async def stop_learning_session(
        self, session_id: UUID, reason: str = "manual_stop"
    ) -> bool:
    ...
    def _calculate_overall_effectiveness(
        self, success_rate: float, velocity: float, stability: float
    ) -> float:
...
class PerformanceTracker:
    ...
    def get_recent_performance(self, n: int = 10) -> list[float]:
        ...
        return (
            self.performance_history[-n:]
            if len(self.performance_history) >= n
