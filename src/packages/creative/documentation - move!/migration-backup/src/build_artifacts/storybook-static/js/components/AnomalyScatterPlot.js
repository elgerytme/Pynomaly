...
class AnomalyScatterPlot {
  ...
  updateDimensions() {
    ...
    this.width =
      this.fullWidth - this.options.margin.left - this.options.margin.right;
    this.height =
      this.fullHeight - this.options.margin.top - this.options.margin.bottom;

    if (this.svg) {
      this.svg.attr("width", this.fullWidth).attr("height", this.fullHeight);
    }
  }
  ...
  createScales() {
    ...
    if (this.options.is3D) {
      this.zScale = d3.scaleLinear().range([0, 100]); // For 3D perspective
    }

    this.sizeScale = d3.scaleSqrt().range([2, 10]);

    this.opacityScale = d3.scaleLinear().range([0.3, 1]);
  }
  ...
  createLegend() {
    this.legend = this.svg
      ...
      .attr(
        "transform",
        `translate(${this.fullWidth - this.options.margin.right + 10}, 30)`,
      );
  }
  ...
  setupAccessibility() {
    ...
    this.container
      ...
      .attr("class", "sr-only")
      .text(
        "Interactive scatter plot showing data points with anomaly detection. Use keyboard navigation to explore points.",
      );
    ...
    this.svg
      ...
      .attr(
        "aria-label",
        "Anomaly detection scatter plot with keyboard navigation",
      )
  ...
  createDataTable() {
    this.dataTableContainer = this.container
      .append("div")
  ...
  sampleData(data, maxPoints) {
    ...
    const sampledAnomalies = this.randomSample(
      anomalies,
      Math.min(targetAnomalies, anomalies.length),
    );
  ...
  getPointColor(d) {
    if (this.options.clustering.enabled && d.cluster !== undefined) {
      return (
        this.options.colors[`cluster${d.cluster + 1}`] ||
        this.options.colors.normal
      );
    }

    if (d.isAnomaly) {
      return d.anomalyScore > 0.8
        ? this.options.colors.anomaly
  ...
  getPointOpacity(d) {
    ...
    if (this.brushedPoints.size > 0 && !this.brushedPoints.has(d.id))
      return 0.3;
  ...
  getPointStroke(d) {
    ...
    if (
      this.focusedPointIndex >= 0 &&
      this.data[this.focusedPointIndex].id === d.id
    ) {
    ...
    if (this.options.clustering.enabled) {
      ...
      clusters.forEach((cluster) => {
        legendItems.push({
          ...
          color:
            this.options.colors[`cluster${cluster + 1}`] ||
            this.options.colors.normal,
          type: "circle",
        });
      });
  ...
  navigatePoint(direction) {
    this.focusedPointIndex = Math.max(
      0,
      Math.min(
        this.data.length - 1,
  ...
  updateDataTable() {
    const table = this.dataTableContainer.selectAll("table").data([null]);
    ...
    // Update rows
    const rows = tbody.selectAll("tr").data(this.data.slice(0, 100)); // Limit to first 100 rows

    const rowsEnter = rows.enter().append("tr");

    const cells = rowsEnter
      .selectAll("td")
      .data((d) => {
