...
class ABTesting:
            ...
            for metric in self.metrics:
                if observation.has_outcome(metric.metric_name):
                    ...
                    if isinstance(value, (int, float)):
                        self.variant_performances[
                            observation.variant_id
                        ].add_observation(metric.metric_name, float(value))
    ...
    def _perform_statistical_test(
        ...
        treatment_values = np.array(
            treatment_performance.metric_values[metric.metric_name]
        )
        ...
        if len(control_values) > 1 and len(treatment_values) > 1:
            ...
            effect_size = (
                (treatment_value - control_value) / pooled_std
                if pooled_std > 0
                else 0.0
            )
        ...
        return ComparisonResult(
            ...
            practical_significance=metric.is_improvement(
    ...
    def get_test_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "significant_results": len(
                [r for r in self.comparison_results if r.is_significant]
            ),
        }
