...
class EnhancedDatabaseManager:
        ...
        try:
            ...
            if self.enable_query_optimization:
                self._query_optimizer = QueryOptimizer(
                    engine=async_engine, cache_size=1000, cache_ttl=3600
                )
    ...
    async def get_optimized_session(
        self,
    ) -> AsyncGenerator[tuple[AsyncSession, QueryOptimizer | None], None]:
    ...
    async def execute_optimized_query(
        ...
        return await self._query_optimizer.execute_with_optimization(
            query=query, params=params, use_cache=use_cache, cache_ttl=cache_ttl
        )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._database_pool:
                ...
                error_rate = self._database_pool.stats.failed_requests / max(
                    1, self._database_pool.stats.total_requests
                )
...
class OptimizedRepository:
    ...
    async def execute_cached_query(
        ...
        try:
            return await self.db_manager.execute_optimized_query(
                query=query, params=params, use_cache=True, cache_ttl=cache_ttl
            )
        except Exception as e:
            self.logger.error(
                "Cached query execution failed", query=query[:100], error=str(e)
            )
            raise
    ...
    async def execute_uncached_query(
        ...
        try:
            return await self.db_manager.execute_optimized_query(
                query=query, params=params, use_cache=False
            )
        except Exception as e:
            self.logger.error(
                "Uncached query execution failed", query=query[:100], error=str(e)
            )
            raise
...
class DatabaseHealthMonitor:
    ...
    def __init__(
        self, db_manager: EnhancedDatabaseManager, check_interval: float = 60.0
    ):
    ...
    def start_monitoring(self) -> None:
        ...
        if self._monitoring_task is None:
            ...
            logger.info(
                "Database health monitoring started", interval=self.check_interval
            )
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        pool_health = sum(1 for check in recent_checks if check["pool_healthy"]) / len(
            recent_checks
        )
