...
class ThreatDetectionConfig(BaseModel):
    ...
    strict_mode: bool = Field(
        default=False, description="Enable strict threat detection"
    )

    # Behavioral analysis settings
    behavior_analysis_enabled: bool = Field(
        default=True, description="Enable behavioral analysis"
    )
    learning_period_days: int = Field(
        default=14, ge=1, le=90, description="Learning period in days"
    )
    anomaly_threshold: float = Field(
        default=0.7, ge=0.1, le=1.0, description="Anomaly detection threshold"
    )
    min_samples_for_profile: int = Field(
        default=50, ge=10, le=1000, description="Minimum samples to build profile"
    )
    ...
    update_interval_seconds: int = Field(
        default=3600, ge=300, le=86400, description="Feed update interval"
    )

    # Data exfiltration detection
    data_exfiltration_enabled: bool = Field(
        default=True, description="Enable data exfiltration detection"
    )
    size_threshold_mb: float = Field(
        default=100.0, ge=1.0, le=10000.0, description="Data size threshold in MB"
    )
    time_window_seconds: int = Field(
        default=300, ge=60, le=3600, description="Detection time window"
    )
    request_count_threshold: int = Field(
        default=50, ge=5, le=1000, description="Request count threshold"
    )

    # Alert settings
    auto_mitigation_enabled: bool = Field(
        default=True, description="Enable automatic mitigation"
    )
    alert_cooldown_seconds: int = Field(
        default=300, ge=60, le=3600, description="Alert cooldown period"
    )
    max_alerts_per_user: int = Field(
        default=10, ge=1, le=100, description="Maximum alerts per user"
    )

    # IP and network settings
    trusted_networks: list[str] = Field(
        default_factory=list, description="Trusted network CIDRs"
    )
    blocked_ips: set[str] = Field(
        default_factory=set, description="Manually blocked IP addresses"
    )
    whitelist_ips: set[str] = Field(
        default_factory=set, description="Whitelisted IP addresses"
    )
    ...
    max_history_size: int = Field(
        default=10000, ge=1000, le=100000, description="Maximum alert history size"
    )
    cleanup_interval_seconds: int = Field(
        default=3600, ge=300, le=86400, description="Cleanup interval"
    )
...
class ThreatDetectionManager:
    ...
    def __init__(self, config: ThreatDetectionConfig | None = None):
    ...
    def _initialize_default_settings(self) -> None:
        ...
        self.detector_settings["brute_force"] = ThreatDetectorSettings(
            name="brute_force",
            enabled=True,
            config={"max_attempts": 5, "time_window": 300, "block_duration": 3600},
            priority=85,
        ...
        self.detector_settings["anomalous_access"] = ThreatDetectorSettings(
            ...
            enabled=True,
            config={"learning_period_days": self.config.learning_period_days},
            priority=70,
    ...
    def update_detector_config(
        self, detector_name: str, settings: ThreatDetectorSettings
    ) -> None:
...
_threat_detection_manager: ThreatDetectionManager | None = None
...
def init_threat_detection_manager(
