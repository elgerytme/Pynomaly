    ...
    def predict_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.combination_method == "average":
            ...
            if self.weights is None:
                self.weights = np.ones(len(self.base_detectors)) / len(
                    self.base_detectors
                )
...
class StackingEnsemble(BaseEnsembleDetector):
    ...
    def __init__(self, base_detectors: list[Any], meta_detector: Any = None):
        ...
        self.meta_detector = meta_detector or IsolationForest(
...
class AdaptiveEnsemble(BaseEnsembleDetector):
    ...
    def __init__(
        self, base_detectors: list[Any], adaptation_method: str = "performance"
    ):
            ...
            for j, scores_j in enumerate(all_scores):
                if i != j:
                    ...
                    if not np.isnan(corr):
                        diversity += 1 - abs(
                            corr
                        )  # Higher diversity for lower correlation
...
class EnsembleAdapter(DetectorProtocol):
    ...
    def _init_algorithm(self) -> None:
        ...
        try:
            ...
            raise AdapterError(
                f"Failed to initialize ensemble {self.detector.algorithm_name}: {e}"
            )
    ...
    def fit(self, dataset: Dataset) -> None:
        ...
        try:
            ...
            y_train = None
            if (
                self.detector.algorithm_name == "AdaptiveEnsemble"
                and dataset.target_column
                and dataset.target_column in dataset.data.columns
            ):
            ...
            logger.info(
                f"Successfully fitted ensemble detector: {self.detector.algorithm_name}"
            )
    ...
    def predict(self, dataset: Dataset) -> DetectionResult:
        ...
        try:
            ...
            threshold_val = np.percentile(
                scores, (1 - self.detector.parameters.get("contamination", 0.1)) * 100
            )
            ...
            return DetectionResult(
                ...
                metadata={
                    ...
                    "base_algorithms": self.detector.parameters.get(
                        "base_algorithms", []
                    ),
                    "combination_method": getattr(
                        self._ensemble, "combination_method", "unknown"
                    ),
                    ...
                    "performance_scores": getattr(
                        self._ensemble, "performance_scores", None
                    ),
                },
            )
        ...
        info = {
            "VotingEnsemble": {
                ...
                "parameters": {
                    "base_algorithms": {
                        ...
                        "description": "List of base algorithms",
                    },
                    "voting": {
                        "type": "str",
                        "default": "hard",
                        "description": "Voting type (hard/soft)",
                    },
                    "contamination": {
                        "type": "float",
                        "default": 0.1,
                    ...
                    "n_estimators": {
                        "type": "int",
                        "default": 100,
                        "description": "Number of estimators for tree-based methods",
                    },
                },
                "suitable_for": [
                    "diverse_algorithms",
                ...
                "cons": [
                    "Equal weight to all detectors",
                    "May not utilize detector strengths optimally",
                ],
            },
            "StackingEnsemble": {
                ...
                "parameters": {
                    ...
                    "meta_detector": {
                        ...
                        "description": "Meta-learner algorithm",
                    },
                    "contamination": {
                        "type": "float",
                        "default": 0.1,
            ...
            "AdaptiveEnsemble": {
                ...
                "parameters": {
                    ...
                    "adaptation_method": {
                        "type": "str",
                        "default": "performance",
                        "description": "Adaptation method (performance/diversity)",
                    },
                    "contamination": {
                        "type": "float",
                        "default": 0.1,
                ...
                "cons": [
                    "Requires labeled data for performance adaptation",
                    "Complex weight learning",
                ],
            },
            "AverageEnsemble": {
            ...
            "MaxEnsemble": {
                ...
                "suitable_for": [
                    ...
                    "high_precision_needs",
                    "critical_systems",
                ],
                "pros": [
                    "Conservative approach",
                    "Detects strong anomalies",
                    "High precision",
                ],
