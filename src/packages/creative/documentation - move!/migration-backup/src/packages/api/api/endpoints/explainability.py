...
async def explain_prediction(
    ...
    try:
        logger.info(
            f"User {current_user} requesting prediction explanation for detector {request.detector_id}"
        )
        ...
        # Log success
        logger.info(
            f"Prediction explanation completed for detector {request.detector_id}"
        )
...
async def explain_model(
    ...
    try:
        logger.info(
            f"User {current_user} requesting model explanation for detector {request.detector_id}"
        )
...
async def explain_cohort(
    ...
    try:
        logger.info(
            f"User {current_user} requesting cohort explanation for detector {request.detector_id}"
        )
...
async def compare_explanations(
    ...
    try:
        logger.info(
            f"User {current_user} requesting explanation comparison for detector {request.detector_id}"
        )
        ...
        domain_request = CompareExplanationsRequest(
            ...
            instance_data=request.instance_data,
            explanation_methods=[
                ExplanationMethod(method) for method in request.explanation_methods
            ],
            background_dataset_id=request.background_dataset_id,
        ...
        logger.info(
            f"Explanation comparison completed for detector {request.detector_id}"
        )
...
async def get_explanation_statistics(
    ...
    try:
        logger.info(
            f"User {current_user} requesting explanation statistics for detector {detector_id}"
        )
...
async def _analyze_feature_interactions_background(
    detector_id: str, dataset_id: str, user_id: str
) -> None:
    """Background task for analyzing feature interactions."""
    try:
        logger.info(
            f"Background analysis of feature interactions for detector {detector_id}"
        )
        ...
        logger.info(
            f"Completed background feature interaction analysis for detector {detector_id}"
        )
