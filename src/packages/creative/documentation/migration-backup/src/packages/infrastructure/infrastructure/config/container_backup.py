...
class Container(containers.DeclarativeContainer):
    ...
    # Data loaders
    csv_loader = providers.Factory(CSVLoader, delimiter=",", encoding="utf-8")

    parquet_loader = providers.Factory(ParquetLoader, use_pyarrow=True)

    # High-performance data loaders - only create providers if loaders are available
    if PolarsLoader is not None:
        polars_loader = providers.Factory(PolarsLoader, lazy=True, streaming=False)

    if ArrowLoader is not None:
        arrow_loader = providers.Factory(ArrowLoader, use_threads=True)

    if SparkLoader is not None:
        spark_loader = providers.Factory(
            SparkLoader, app_name="Pynomaly", master="local[*]"
        )
    ...
    if RateLimiter is not None:
        rate_limiter = providers.Singleton(
            RateLimiter, default_requests_per_minute=config.provided.api_rate_limit
        )
    ...
    if DetectorCacheDecorator is not None and RedisCache is not None:
        detector_cache_decorator = providers.Singleton(
            DetectorCacheDecorator, cache=redis_cache
        )
    ...
    if HealthService is not None:
        health_service = providers.Singleton(HealthService, max_history=100)

    # Database services - only create if available and configured
    if DatabaseManager is not None:
        database_manager = providers.Singleton(
            DatabaseManager, database_url=config.provided.database_url
        )
    ...
    if ShutdownService is not None:
        shutdown_service = providers.Singleton(ShutdownService, shutdown_timeout=60.0)
    ...
    if PHASE2_SERVICES_AVAILABLE and feature_flags.is_enabled("algorithm_optimization"):
        ...
        try:
            ...
            algorithm_optimization_service = providers.Singleton(
                AlgorithmOptimizationService
            )
        except ImportError:
            pass
    ...
    if PHASE2_SERVICES_AVAILABLE and feature_flags.is_enabled("cli_simplification"):
        try:
            ...
            workflow_simplification_service = providers.Singleton(
                WorkflowSimplificationService
            )
        except ImportError:
            pass
    ...
    if CONFIGURATION_SERVICES_AVAILABLE and feature_flags.is_enabled("advanced_automl"):
        ...
        try:
            ...
            autonomous_configuration_integration = providers.Singleton(
                AutonomousConfigurationIntegration,
                autonomous_service=providers.Object(
                    "autonomous_service"
                ),  # Will be injected
                configuration_service=configuration_capture_service,
                auto_save_successful=True,
    ...
    if AutoMLUseCase is not None and AutoMLService is not None:
        automl_use_case = providers.Factory(
            AutoMLUseCase, automl_service=automl_service
        )
    ...
    if SECURITY_AVAILABLE:
        ...
        input_sanitizer = providers.Singleton(
            InputSanitizer, config=sanitization_config
        )
        ...
        if DatabaseManager is not None:
            safe_query_builder = providers.Singleton(
                SafeQueryBuilder, metadata=database_manager.provided.metadata
            )
        ...
        encryption_service = providers.Singleton(
            EncryptionService, config=encryption_config
        )

        data_encryption = providers.Singleton(DataEncryption, config=encryption_config)

        field_encryption = providers.Singleton(
            FieldEncryption, encryption_service=encryption_service
        )
        ...
        security_monitor = providers.Singleton(
            SecurityMonitor, audit_logger=audit_logger
        )
    ...
    if config.use_database_repositories and config.database_configured:
        if DatabaseDetectorRepository is not None and DatabaseManager is not None:
            ...
            try:
                ...
                logging.warning(
                    f"Database initialization failed, falling back to in-memory: {e}"
                )
...
def _create_result_repository(config):
    ...
    if config.use_database_repositories and config.database_configured:
        if (
            DatabaseDetectionResultRepository is not None
            and DatabaseManager is not None
        ):
            # Create database manager and repository
            db_manager = DatabaseManager(
                database_url=config.database_url,
