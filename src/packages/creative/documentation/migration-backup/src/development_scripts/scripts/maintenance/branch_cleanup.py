...
def run_git_command(command: list[str], cwd: Path | None = None) -> tuple[bool, str]:
...
def analyze_branches(stale_days: int = 30, remote: bool = True) -> list[BranchInfo]:
    ...
    for branch in branches:
        ...
        branch_info.append(
            BranchInfo(name=branch, last_commit=last_commit, is_merged=is_merged)
        )
...
def analyze(
    stale_days: int = typer.Option(
        30, "--stale-days", help="Days after which a branch is considered stale"
    ),
    remote: bool = typer.Option(
        True, "--remote/--local", help="Analyze remote or local branches"
    ),
    output: str | None = typer.Option(
        None, "--output", help="Save analysis to JSON file"
    ),
    show_all: bool = typer.Option(
        False, "--show-all", help="Show all branches, not just stale ones"
    ),
):
    ...
    if remote:
        ...
        if not success:
            console.print(
                f"[yellow]Warning: Failed to fetch remote info: {output_msg}[/yellow]"
            )
...
def cleanup(
    ...
    merged_only: bool = typer.Option(
        False, "--merged-only", help="Only delete merged branches"
    ),
    remote: bool = typer.Option(
        True, "--remote/--local", help="Clean up remote or local branches"
    ),
):
    ...
    if merged_only:
        candidates = [
            b for b in all_branches if b.is_merged and b.days_stale >= stale_days
        ]
    ...
    for branch in candidates:
        ...
        if remote:
            success, output_msg = run_git_command(
            ...
            if success:
                ...
                console.print(
                    f"[red]❌ Failed to delete remote branch:[/red] {branch.name} - {output_msg}"
                )
            ...
            if success:
                ...
                console.print(
                    f"[red]❌ Failed to delete local branch:[/red] {branch.name} - {output_msg}"
                )
                failed_branches.append(branch.name)
...
def stash_check(
    ...
    if fix:
        if Confirm.ask(f"\nClear all stashes on '{branch}' branch?"):
            ...
            if success:
                console.print(
                    f"[green]✅ Cleared all stashes on '{branch}' branch[/green]"
                )
            else:
...
def report(
    output: str = typer.Option(
        "reports/branch-analysis-report.json", "--output", help="Output file for report"
    ),
    stale_days: int = typer.Option(
        30, "--stale-days", help="Days after which a branch is considered stale"
    ),
    ...
    if len(stale_branches) > 0:
        recommendations.append(
            f"Consider cleaning up {len(stale_branches)} stale branches"
        )
    if len(merged_branches) > 0:
        recommendations.append(
            f"Consider deleting {len(merged_branches)} merged branches"
        )
    if has_stashes:
    ...
    # Display summary
    console.print("📊 Branch Analysis Summary:")
